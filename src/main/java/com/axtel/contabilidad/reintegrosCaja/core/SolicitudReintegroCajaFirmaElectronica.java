...
public final class SolicitudReintegroCajaFirmaElectronica extends SolicitudFirmaElectronica {
	...
	private static final Logger log = Logger.getLogger(SolicitudReintegroCajaFirmaElectronica.class);
	...
	public String getAutLegend(Connection conn) {
		...
		try {
			if (tieneDelegatorioAut(conn)) {
				String voLegend = AUT_LEGEND_PREFIX 
								+ " Firma " + getTipoSuplenciaAut() 
								+ " de " + getNombreEmpleadoSuplidoAut() 
								+ " con fundamento en el oficio: " + getFolioOficioAut() 
								+ " de fecha: " + getFechaOficioAut();
				log.info(voLegend);
				return voLegend;
			} else
...
//		ConfiguraAplicativoBusinessLogic cabl = new ConfiguraAplicativoBusinessLogic(GestionInterface.ATT_CONEXION);
	...
	public String getVoBoLegend(Connection conn) throws Exception {
		...
		if (tieneDelegatorioVoBO(conn)) {
			String voLegend = VO_BO_LEGEND_PREFIX + 
				" Firma " + getTipoSuplencia() 
				+ " de " + getNombreEmpleadoSuplido() 
				+ " con fundamento en el oficio: " 
				+ getFolioOficioVoBo() 
				+ " de fecha: " + getFechaOficioVoBo();
			log.info(voLegend);
			return voLegend;
		} else
	...
	public void onCancelaTramite(Connection conn, String reason) throws Exception {
		...
		try{
			...
			if( rs.next() ){
				...
				String cuerpoCorreo  = String.format("<html>"
					+"\t<head>"
					+"\t\t<meta charset=\"UTF-8\">"
					+"\t\t<style type=\"text/css\">"
					+"\t\tbody{"
					...
					+"\t\t<br/> "
					+"\t\t<p>"
					+"\t\t\tSe hace de su conocimiento que la solicitud con folio: <b>%s</b> fue rechazada por el siguiente motivo:"
					+"\t\t</p>"
					+"\t\t<p>"
					+"\t\t\t<i>%s</i>"
					+"\t\t</p>"
					+"\t\t<p>"
					+"\t\t\tSe sugiere tome las medidas pertinentes."
					+"\t\t</p>"
					+"\t\t<p>"
					+"\t\t\tNotificaciones Automaticas"
					+"\t\t\t<br />"
					+"\t\t\tSistema de Administracion Integral"
					+"\t\t\t<br />"
					+"\t\t\t%s"
					+"\t\t</p>"
					+"\t</body>"
					+"</html>", nombreCapturista, folioTramite, reason, Util.getToday());
