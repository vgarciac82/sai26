...
public class ReintegrosCajaServlet extends HttpServlet {
	...
	private static final Logger	log					= Logger.getLogger(ReintegrosCajaServlet.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (idOper != null && !"".equals(idOper)) {				
				if("1".equals(idOper)){ //CAPTURISTA - INSERTA DATOS CAPTURADOS					
					boolean retorno = false;
					String existe = reintegrosCajaBL.buscaDatos( nFolioReinegroCaja );//BUSCA SI EXISTEN LOS DATOS EN BASE DE DATOS
					
					if(!"SI".equals( existe )) {//SI NO EXISTEN DATOS EN BASE DE DATOS ENTRA A INSERTAR
				...
				} //FIN CAPTURISTA - INSERTA DATOS CAPTURADOS
				
				else if("2".equals(idOper)){ //REVISOR - APLICA PRIMER POLIZA					
				...
				} //FIN REVISOR - APLICA PRIMER POLIZA
				
				else if("3".equals(idOper)){ //AUTORIZADOR - INSETA DATOS APLICA SEGUNDA POLIZA					
					...
					if ("SI".equals( rechazaAutorizador )) {
						if(!"SI".equals( existe )) { //SI NO EXISTEN LOS DATOS EN BASE DE DATOS PARA EL AUT ENTRA A INSERTAR
							...
							if (retornoAut) {//SI SE INSERTARON CORRECTAMENTE ENTRA PARA APLICAR CONTABLEMNETE
								try{								
