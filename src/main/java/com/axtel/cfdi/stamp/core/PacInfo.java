...
public class PacInfo {
	...
	private static final Logger	log	= Logger.getLogger( PacInfo.class );
	private String				user;
	...
	public PacInfo( Connection conn ) {
		...
		try {
			...
			log.debug( "Executing query to retrieve PAC info: " + query );
			...
			if ( rs.next() ) {
				...
				log.debug( "PAC User: " + user );
				log.trace( "PAC URL: " + url );  
			} else {
