		...
		for (int i = 0; i < procesar.length; i++) {
			...
			try {
				...
				if (comprobante.esCFDICombustible()) {
					...
					System.out.println( factura.getName() + "," + subtotal + "," + total.subtract(subtotal) + "," + 
					                    total + "," + bonificacion.getImporte() + "," +   bonificacion.getTraslado() + ","
					                    + bonificacion.getTotal() + "," + total.add(bonificacion.getTotal()));
				}
