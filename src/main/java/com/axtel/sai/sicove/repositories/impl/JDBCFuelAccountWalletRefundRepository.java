...
public class JDBCFuelAccountWalletRefundRepository implements FuelAccountWalletRefundRepository {
    ...
    public JDBCFuelAccountWalletRefundRepository() {
        selectHeader = new StringBuilder().append("SELECT id_refund AS idRefund ").append("      ,id_fuel_account_wallets AS idFuelAccountWallets ").append("      ,wallet_number AS walletNumber ").append("      ,refund_amount AS refundAmount ").append("      ,registration_date AS registrationDate ").append("      ,user_capture AS userCapture ").append("      ,(case when is_supplier_refund = 1 then 'true' else 'false' end) AS supplierRefund ").append("  FROM fuel_account_wallet_refund WITH(NOLOCK) ");
        queryInsert = new StringBuilder("INSERT INTO fuel_account_wallet_refund( ").append("            id_fuel_account_wallets ").append("           ,wallet_number ").append("           ,refund_amount ").append("           ,user_capture ").append("           ,registration_date ").append("           ,is_supplier_refund) ").append("     VALUES(?, ").append("           ?, ").append("           ?, ").append("           ?, ").append("           ?, ").append("           ?)  ");
        querySelect = new StringBuilder(selectHeader).append(" WHERE id_refund = ? ");
        queryDelete = new StringBuilder("DELETE FROM fuel_account_wallet_refund WHERE id_refund = ? ");
        queryUpdate = new StringBuilder("UPDATE fuel_account_wallet_refund ").append("   SET refund_amount = ? ").append("      ,registration_date = GETDATE() ").append(" WHERE id_refund = ? ");
    }
    ...
    public FuelAccountWalletRefund create(Connection conn, FuelAccountWalletRefund fuelAccountWalletRefund) throws SicoveException {
        ...
        BigDecimal newId;
        try {
            newId = runner.insert(conn, queryInsert.toString(), scalarHandler, fuelAccountWalletRefund.getIdFuelAccountWallets(), fuelAccountWalletRefund.getWalletNumber(), fuelAccountWalletRefund.getRefundAmount(), fuelAccountWalletRefund.getUserCapture(), fuelAccountWalletRefund.getRegistrationDate(), (fuelAccountWalletRefund.isSupplierRefund() ? 1 : 0));
