...
public class PenaltiesImplements extends DataSourceManager implements PenaltiesInterface {
	...
	private static Logger log = Logger.getLogger( PenaltiesImplements.class );
	...
	private void sendEmail( PenaltyAndDeduction penalty, Connection conn ) throws Exception {
		...
		try {
			...
			if ( penalty.getnIdOper() == 2 ) {// send email to adquisitions
				tipo = "CAPTURADO";
			...
			} else if ( penalty.getnIdOper() == 1 ) {// send email to capture
														// area
				tipo = "RECHAZADO";
			...
			} else if ( penalty.getnIdOper() == 3 ) {// send email to capture
														// area, requeried area
			...
			if ( isDebuger ) {
				...
				} else if ( penalty.getnIdOper() == 1 ) {// send email to
															// capture area
				...
				} else if ( penalty.getnIdOper() == 3 ) {// send email to
															// capture area,
															// requeried area
															// and responsible
															// area
