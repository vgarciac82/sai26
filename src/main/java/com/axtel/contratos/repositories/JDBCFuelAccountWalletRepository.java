...
public class JDBCFuelAccountWalletRepository implements FuelAccountWalletRepository {
    ...
    private static final Logger log = LoggerFactory.getLogger(JDBCFuelAccountWalletRepository.class);
    ...
    public FuelAccountWallet update(Connection conn, FuelAccountWallet fuelAccountWallet) throws ContratoException {
        ...
        try {
            runner.update(conn, queryUpdate.toString(), fuelAccountWallet.getVehicleInventoryId(), fuelAccountWallet.getStatus(), fuelAccountWallet.getIdFuelAccountWallet());
