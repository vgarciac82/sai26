...
public class FuelContractRepository {
	...
	private static final Logger									log									= Logger.getLogger( FuelContractRepository.class );
	...
	static {
			CONTRACT_ACCOUNT_HEADER_SQL = ( 
					new StringBuilder("SELECT id_account AS idAccount ")
			.append("      ,id_contract AS idContract ")
			.append("      ,employee_responsible AS employeeResponsible ")
			.append("      ,id_unit AS idUnit ")
			.append("      ,account_number AS accountNumber ")
			.append("      ,registration_date AS registrationDate ")
			.append("      ,user_registration AS userRegistration ")
			.append("      ,monthly_asignation AS monthlyAsignation ")
			.append("  FROM fuel_contract_account WITH(NOLOCK) ")
			).toString();
	}
	...
	public FuelContractAccount create( Connection conn, FuelContractAccount fuelContractAccount ) throws SQLException {
		...
		BigDecimal newId = runner.insert( conn, queryInsert.toString(), scalarHandler, 
				fuelContractAccount.getIdContract(), 
				fuelContractAccount.getEmployeeResponsible(), 
				fuelContractAccount.getIdUnit(), 
				fuelContractAccount.getAccountNumber(),
				fuelContractAccount.getUserRegistration(),
				fuelContractAccount.getMonthlyAsignation());
