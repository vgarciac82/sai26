...
public class QuestionnaireBussinessLogic extends SolicitudFirmaElectronica {
	...
	private static final Logger		log	= LogManager.getLogger( QuestionnaireBussinessLogic.class );
	...
	private StringBuilder generaLigaDoctos( Connection conn, String urlDescarga,String msgTable ) throws Exception {
		...
		expedientBody.append( msgTable );//"	En la tabla siguiente puede revisar el documento que sera firmado."
		expedientBody.append( "	</p>" );
	...
	private void sendRequiSign( Connection conn ) throws ContratoException {
		try {
			...
			generaArchivos( conn );
			if("RT".equalsIgnoreCase( requisition.getIdTipoSolicitud() )) {//Validar las requis de tineda
