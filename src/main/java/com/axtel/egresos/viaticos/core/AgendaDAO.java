...
public class AgendaDAO {
	    ...
	    public static ArrayList<Agenda> consultaListaAgendas( Connection conn, int idComision ) throws Exception {
			String query = "SELECT nidAgenda, fInicio, fFin, nidTipo, nidPais, nidEstado,nidMunicipio, cLocalidad, cMotivoComision, cActividades "
						 + " FROM tAgenda WITH (NOLOCK) WHERE nIdComision = ?" ; 
		...
		public static Agenda consultaFechaAgendaBitacora ( Connection conn, int idComision ) throws Exception {
			...
			ResultSet rs = null;
			Agenda agenda = new Agenda();
			String query = "SELECT nidcomision, MIN(finicio) fInicio, MAX(ffin) fFin FROM tAgendaBitacora WITH (NOLOCK) WHERE nIdComision = ? "
						+ "GROUP BY nIdComision" ; 
