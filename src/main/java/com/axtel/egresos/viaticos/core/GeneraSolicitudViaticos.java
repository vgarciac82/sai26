...
public class GeneraSolicitudViaticos {
    ...
    private static final Logger log = LoggerFactory.getLogger(GeneraSolicitudViaticos.class);
    ...
    public static EgresoEncabezado agregarDatos(Connection conn, Comision comision, EgresoEncabezado encabezado, int folioCaja) throws Exception {
        ...
        try {
            ...
            if (eventoDevengado.equals(comision.getEvento())) {
                ...
                tipoPoliza = "DI";
                tipo_oper = "1";
                //Borrar la cuenta bancaria cuando es comprobación
                encabezado.setCTAB("");
            }
    ...
    public static void borraFirmantes(Connection conn, int idComision, int noEmpleado) throws Exception {
        ...
        try {
            ...
            pst = conn.prepareStatement("INSERT INTO tFirmantesViaticosBitacora (nFolio, nRenglon, nIdEmpleado, cNombre,  cPuesto, d_email, cAutorizado,dfechaAutoriza, dFechaModificacion) " + "				SELECT nFolio, nRenglon, nIdEmpleado, cNombre,  cPuesto,  d_email, cAutorizado,dfechaAutoriza, GETDATE() " + "				from tFirmantesViaticos WITH (NOLOCK) where nfolio = " + idComision);
            pst.execute();
    ...
    public static void eliminarSolicitud(Connection conn, int idComision, int folioPago) throws Exception {
        PreparedStatement pst = null, pst1 = null, pst2 = null, pst3 = null, pst4 = null, pst5 = null, pst6 = null, pst7 = null, pst8 = null, pst9 = null, pst10 = null, pst11 = null, pst12 = null;
        String tipoPago = "RELACIONGASTOS";
        try {
            // Boletos de Avion
    ...
    public static String generarFolioCertificado(Connection conn, int idComision) throws Exception {
        ...
        String folioCer = "";
        try {
            pst = conn.prepareStatement("SELECT  seq_name + '-'+ RIGHT('000' + CAST( seq_value + 1 AS varchar(4)), 3) + '-' + CAST((SELECT aEjercicioFiscal FROM tEjercicioFiscal WITH (NOLOCK) WHERE cActivo = 1) AS varchar(4)) folio FROM CF_SEQUENCE SEQ WITH (NOLOCK) " + " WHERE seq_name = (SELECT 'CERT-' + cUnidadResponsable FROM tComision WITH (NOLOCK) WHERE nIdComision = ? )");
            pst.setInt(1, idComision);
            rs = pst.executeQuery();
            if (rs.next()) {
