...
public class TransporteDAO {
	...
	public static int actualizarLayout( Connection conn, int idComision, String estatus, int folioPago ) throws Exception {
		...
		String query =  "UPDATE layout set Status = ? FROM tLayoutVuelosDet layout "
				+ " INNER JOIN tTransporteAereo transporte "
				+ " ON layout.RFC = transporte.RFCVuelo AND layout.mTotal = transporte.mImporteBoleto AND layout.cReferencia = transporte.cReferencia "
				+ " WHERE transporte.nidComision = ? and transporte.nFolioPago = ? ";
	...
	public static int actualizarLayoutPorBoleto( Connection conn, TransporteAereo transporte, String estatus) throws Exception {
		...
		String query =  "UPDATE layout set Status = ? FROM tLayoutVuelosDet layout "
				+ " INNER JOIN tTransporteAereo transporte "
				+ " ON layout.RFC = transporte.RFCVuelo AND layout.mTotal = transporte.mImporteBoleto AND layout.cReferencia = transporte.cReferencia "
				+ " WHERE transporte.nidComision = ? and transporte.cReferencia = ? ";
	...
	public static int insertarRelacionTaxi( Connection conn, int idComision, int folio , String tipoPago ) throws Exception {
		...
		int actualizado = 0;
		try {
			pst = conn.prepareStatement( "INSERT INTO tRelacionTaxiPagos (nFolioRelaciongastos, cFolioTaxi, nidComision, mMonto, cTipoPago) "
										+ " SELECT ?, cFolioTaxi, nidComision, mMonto, ? FROM v_TaxisViaticos WHERE nidComision = ? " );
			pst.setInt( 1, folio );
