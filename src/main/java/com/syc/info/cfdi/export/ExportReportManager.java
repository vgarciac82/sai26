...
*/
    private static String searchByPolicie(Connection conn, String policieType, int policieNumber, String policieCostCenter) {
        String documentSearchQuery = "SELECT	TOP 1 cTipoDocumento, " + "      	cFolioDocumentoMovimiento " + "  FROM	tMovimiento WITH(NOLOCK)  " + " WHERE	nFolioPoliza = ?  " + "   AND	ccentrocontable = ? " + "   AND	ctipoPoliza = ?";
        String folioSearchQuery = "SELECT	folio, cdocumentohaplicado " + "  FROM	vTramitesExportar WITH(NOLOCK) " + " WHERE	cdocumento = ? " + "   AND	nfolio = ? " + "   AND	cdocumentohaplicado IN ('S','C')";
        String searchByPaymentQuery = "SELECT	cTipoPago, " + "      	nFolioPAGO  " + "  FROM	tPagadoEncabezado WITH(NOLOCK) " + " WHERE	nFolioPagado = ?";
