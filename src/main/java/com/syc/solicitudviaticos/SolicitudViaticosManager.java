...
public class SolicitudViaticosManager {
    ...
    private static final Logger log = LoggerFactory.getLogger(SolicitudViaticosManager.class);
    ...
    public static void creaViaticoTransporte(Connection conn, BigDecimal monto, int tipoTransporte, int folioSolicitud, int noKm, String origen, String placas, String tieneVales) throws Exception {
        ...
        try {
            ...
            log.info("Object: {}", "Se insertaron " + afectados + " para la solicituda de viaticos: " + folioSolicitud);
        } finally {
    ...
    public static void creaViatico(Connection conn, int folioViatico, String fechaCaptura, String loginCaptura, String numeroEmpleadoBeneficiario, String esSolicitudPropia, String cUnidadEjecutora, String eMail, String cuentaBanco) throws Exception {
        log.info("Object: {}", "Insertando informacion viaticos: " + folioViatico + "," + fechaCaptura + "," + loginCaptura + "," + numeroEmpleadoBeneficiario + "," + esSolicitudPropia + "," + cUnidadEjecutora);
        String queryInsert = "INSERT INTO tSolicitudViaticosEncabezado " + "        ( nFolioSolicitudViaticos , " + "          dFechaCaptura , " + "          cLoginCaptura , " + "          nNumEmpleadoBeneficiario , " + "          lEsSolicitudPropia , " + "          cUnidadEjecutora, " + "         cCorreoEmpleadoBeneficiario, " + "         subCuentasBancaria " + "        )" + "VALUES  ( ?," + "          ?," + "          ?," + "          ?," + "          ?," + "          ?," + "          ?," + "          ?" + "        )";
        PreparedStatement ps = null;
        try {
            ...
            int afectados = ps.executeUpdate();
            log.info("Object: {}", "Se insertaron " + afectados + " para la solicituda de viaticos: " + folioViatico);
        } finally {
    ...
    public static void creaViaticoAgenda(Connection conn, int folioSolicitud, String fInicio, String fFin, String lEsAnticipado, String lEsNacional, int nIdPais, int ID_ESTADO, int ID_MUNICIPIO, String cLocalidad, String cMotivo, BigDecimal mCuotaPorDia, int nIDPaquete, int nIDHomologacion, String cIDTipoMoneda, String cActividadesAgenda) throws Exception {
        PreparedStatement ps = null;
        PreparedStatement psRow = null;
        ResultSet rs = null;
        int nDocRenglon = 0;
        String query = "INSERT INTO dbo.tSolicitudViaticosAgenda " + "( nFolioSolicitudViaticos, nDocRenglon, fInicio, fFin, lEsAnticipado, lEsNacional, nIdPais, ID_ESTADO, ID_MUNICIPIO, cLocalidad, cMotivo, mCuotaPorDia, nIDPaquete, nIDHomologacion, cIDTipoMoneda, cActividadesAgenda ) " + "VALUES  ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )";
        try {
            ...
            if (nDocRenglon > 0) {
                ...
                int afectados = ps.executeUpdate();
                log.info("Object: {}", "Se insertaron " + afectados + " para la solicitud de viaticos agenda: " + folioSolicitud);
            }
        } finally {
            CloseObject.closeObject(ps, false);
        }
    ...
    public static NotificacionSolicitudViaticos getPrimerAutorizacion(Connection conn, int folioSolicitud) throws Exception {
        String querySel = "SELECT	solicitud.nfoliosolicitudviaticos,  " + "			solicitud.nDocRenglon,  " + "			solicitud.noempleado,  " + "			solicitud.idplaza,   " + "			solicitud.puesto,   " + "			solicitud.nombre,   " + "			solicitud.nautorizado,  " + "			empleados.d_email " + "  FROM	tsolicitudviaticosfirmantes solicitud WITH(NOLOCK) " + "			LEFT OUTER JOIN " + "			vEmpleadoViaticante empleados WITH(NOLOCK) " + "			ON solicitud.noempleado = empleados.c_empleado " + " WHERE	solicitud.ndocrenglon = ? " + "			AND solicitud.nFolioSolicitudViaticos = ? ";
        PreparedStatement ps = null;
    ...
    private static List<AgendaNotificacion> leeAgendaSolicitud(Connection conn, int folioSolicitud) throws Exception {
        String query = "SELECT clocalidad + ' en ' + municipio + ', ' + estado AS Destino, " + "       cmotivo, " + "       CONVERT(VARCHAR(32), finicio, 103)              AS fechaInicio, " + "       CONVERT(VARCHAR(32), ffin, 103)                 AS fechaFin, " + "       Datediff( day, finicio, ffin ) + 1              AS dias, " + "       totalimporte " + "FROM   vsolicitudviaticosagenda WITH(nolock) " + "WHERE Folio = ?";
        PreparedStatement ps = null;
        ResultSet rs = null;
    ...
    public static int actualizaEstausAutorizacion(Connection conn, NotificacionSolicitudViaticos autorizacionObj) throws Exception {
        int afectados = 0;
        String queryUpdate = "UPDATE	tSolicitudViaticosFirmantes " + "   SET	nAutorizado = ?, " + "          fAutorizacion = GETDATE() " + (StringUtils.isBlank(autorizacionObj.getMotivoRechazo()) ? "" : "          ,cMotivoRechazo = ? ") + " WHERE	nFolioSolicitudViaticos = ? " + "   AND	nDocRenglon = ? ";
        PreparedStatement ps = null;
        try {
            int parmConuter = 1;
    ...
    public static boolean notifiacionAtendida(Connection conn, Map<String, String> datosToken) throws Exception {
        ...
        String query = "SELECT	nAutorizado " + "  FROM	tsolicitudviaticosfirmantes solicitud WITH(NOLOCK) " + " WHERE	nFolioSolicitudViaticos = ? " + "   AND nDocRenglon= ?";
        PreparedStatement ps = null;
        ResultSet rs = null;
