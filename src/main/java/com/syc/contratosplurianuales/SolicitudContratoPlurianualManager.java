...
public class SolicitudContratoPlurianualManager {
	private static final Logger log = Logger.getLogger( SolicitudContratoPlurianualManager.class );
	...
	public static String generaExcelSolicitud(Connection conn, HttpServletRequest req, HttpServletResponse resp,  Map<String, String> plantilla) throws Exception {
		...
		String sSqlEnc = " SELECT EncR1, EncR2, EncR3, cDescripcionProyecto,fInicio, fFin, TotalContrato , JustificacionBeneficios, sPuestoSol,sFirmanteSol "
				+ " FROM vw_ContratosPlurianualEncabezado WITH (NOLOCK) WHERE Folio = " + sFolio;
...
public static String generaExcelSolicitudModificado(Connection conn, HttpServletRequest req, HttpServletResponse resp,  Map<String, String> plantilla) throws Exception {
		...
		String queryEnc = " SELECT EncR1, EncR2, EncR3, cDescripcionProyecto,fInicio, fFin, TotalContrato , JustificacionBeneficios, sPuestoSol,sFirmanteSol, FolioOrigen "
				+ " FROM vw_ContratosPlurianualEncabezadoMod WITH (NOLOCK) WHERE Folio = " + sFolio;
		String queryDet = " SELECT Folio, aEjercicioFiscal, ROW_NUMBER ( )  over (order by iEjercicioClave) nDocrenglon, substring(EPCorta,1,19)+  '00'  + SUBSTRING(epcorta,22,50) EPCorta , iEjercicioClave, RA, UR, FI, FN, SF, '00' RG, AI, PP, OG, TG,FF, EF, PPI,  " + 
				" sum(mMes1) mMes1, sum(mMes2) mMes2, sum(mMes3) mMes3, sum(mMes4) mMes4, sum(mmes5) mmes5, sum(mMes6) mMes6, sum(mMes7) mMes7, sum(mMes8) mMes8, sum(mMes9) mMes9, sum(mmes10) mmes10, sum(mMes11) mMes11, sum(mMes12) mMes12, SUM(mimporteTotal) mimporteTotal " + 
				" FROM vw_ContratosPlurianualDetalleMod WITH (NOLOCK) WHERE Folio = "	+ sFolio +
				" group by Folio, aEjercicioFiscal,  substring(EPCorta,1,19), SUBSTRING(epcorta,22,50), RA, UR, FI, FN, SF,  AI, PP, OG, TG,FF, EF,iEjercicioClave,PPI "; 
		...
		if ("0".equals( esModificado )) {
			sSqlEnc  = " SELECT EncR1, EncR2, EncR3, cDescripcionProyecto,fInicio, fFin, TotalContrato , JustificacionBeneficios, sPuestoSol,sFirmanteSol, Folio  FolioOrigen"
					+ " FROM vw_ContratosPlurianualEncabezado WITH (NOLOCK) WHERE Folio = " + folioOrigen;
			sSqlDet  = " SELECT Folio, aEjercicioFiscal, ROW_NUMBER ( )  over (order by iEjercicioClave) nDocrenglon, substring(EPCorta,1,19)+  '00'  + SUBSTRING(epcorta,22,50) EPCorta , iEjercicioClave, RA, UR, FI, FN, SF,'00' RG, AI, PP, OG, TG,FF, EF, PPI,  " + 
					" sum(mMes1) mMes1, sum(mMes2) mMes2, sum(mMes3) mMes3, sum(mMes4) mMes4, sum(mmes5) mmes5, sum(mMes6) mMes6, sum(mMes7) mMes7, sum(mMes8) mMes8, sum(mMes9) mMes9, sum(mmes10) mmes10, sum(mMes11) mMes11, sum(mMes12) mMes12, SUM(mimporteTotal) mimporteTotal " + 
					" FROM vw_ContratosPlurianualDetalle WITH (NOLOCK) WHERE Folio = " + folioOrigen +
					" group by Folio, aEjercicioFiscal,  substring(EPCorta,1,19), SUBSTRING(epcorta,22,50), RA, UR, FI, FN, SF,  AI, PP, OG, TG,FF, EF,iEjercicioClave,PPI ";
			...
			sSqlEnc  = " SELECT EncR1, EncR2, EncR3, cDescripcionProyecto,fInicio, fFin, TotalContrato , JustificacionBeneficios, sPuestoSol,sFirmanteSol, FolioOrigen "
					+ " FROM vw_ContratosPlurianualEncabezadoMod WITH (NOLOCK) WHERE Folio = " + folioUltimo;
			sSqlDet  = " SELECT Folio, aEjercicioFiscal, ROW_NUMBER ( )  over (order by iEjercicioClave) nDocrenglon, substring(EPCorta,1,19)+  '00'  + SUBSTRING(epcorta,22,50) EPCorta , iEjercicioClave, RA, UR, FI, FN, SF,'00' RG, AI, PP, OG, TG,FF, EF, PPI,   " + 
					" sum(mMes1) mMes1, sum(mMes2) mMes2, sum(mMes3) mMes3, sum(mMes4) mMes4, sum(mmes5) mmes5, sum(mMes6) mMes6, sum(mMes7) mMes7, sum(mMes8) mMes8, sum(mMes9) mMes9, sum(mmes10) mmes10, sum(mMes11) mMes11, sum(mMes12) mMes12, SUM(mimporteTotal) mimporteTotal " + 
					" FROM vw_ContratosPlurianualDetalleMod WITH (NOLOCK) WHERE Folio = "	+ folioUltimo +
					" group by Folio, aEjercicioFiscal,  substring(EPCorta,1,19), SUBSTRING(epcorta,22,50), RA, UR, FI, FN, SF,  AI, PP, OG, TG,FF, EF,iEjercicioClave,PPI ";
...
public static void descartaContrato(Connection conn, String uLogin, int nFolioContrato) throws Exception {
	...
	public static String ReporteExcelPlurianuales (ResultSet rsDet,ResultSet rsEnc, ResultSet rsDetM, String plantillaPath, int numAnios) throws Exception {
		...
		String file_name = System.getProperty( "java.io.tmpdir" ) + File.separatorChar + 
				"FormatoSOLContratoPLU" + "_" + System.currentTimeMillis() + "_" + String.valueOf((int) (Math.random() * 100)) + ".xlsx";		
		...
		while (rsDet.next()) {				
			...
			Util.createExcelCellRep(0, rw, rsDet, rsMetadata.getColumnName(3), rsMetadata.getColumnType(3), esTexto); //ndocrenglon
		...
		if (renglon > 15) {
			...
			deleteRow(row, sheet0);//esTextoFinal
			
			//Borrar el ultimo renglon de la primera tabla
			Row rw =(sheet0.getRow(iRowSec ) == null ? sheet0.createRow(iRowSec ) : sheet0.getRow(iRowSec));  
			deleteRow(rw, sheet0);//esTextoFinal
	...
	public static void exportaReportes( Connection conn, String reportPath, String fileName, String folio, int esModificado, int nFolioContratoPlurianual, int esOriginal, OutputStream out ) throws Exception {
		String query = "select MAX(nFolioContratoPlurianual) folioAnterior from tContratosPlurianualEnc_Mod where nFolioContratoOrigen =  " +nFolioContratoPlurianual  + 
				"		and nFolioContratoPlurianual <> " + folio ;
		ResultSet rs = null;
