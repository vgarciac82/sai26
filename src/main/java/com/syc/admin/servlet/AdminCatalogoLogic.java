...
public class AdminCatalogoLogic extends DataSourceManager {

    private static Logger log = LoggerFactory.getLogger(AdminCatalogoLogic.class);

    public AdminCatalogoLogic(String jniName) {
        super.init(jniName);
    }

    public int servicio(String evento, CatEmpleado u) throws AdminCatalogoException {
            ...
            if (evento.endsWith("User")) {
                if (evento != null) {
                    if (evento.equals("addUser")) {
                        retVal = CatalogoEmpleado.insert(conn, u);
                    } else if (evento.equals("updateUser")) {
                        retVal = CatalogoEmpleado.update(conn, u);
                    } else if (evento.equals("deleteUser")) {
                        retVal = CatalogoEmpleado.delete(conn, u.getId_empleado());
                    }
                }
            }
        } catch (SQLException exc) {
            log.error("Recuperando descripcion", exc);
            throw new AdminCatalogoException(exc);
        } finally {
            try {
                if (conn != null)
                    conn.close();
