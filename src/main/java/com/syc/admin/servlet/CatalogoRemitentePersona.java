...
public class CatalogoRemitentePersona {
	...
	public static int insert(Connection conn, CatRemPersona crp) throws SQLException {
		...
		try {
			String valor2 = "SELECT Count(*) total2 FROM cg_cat_remitente_persona" +
			" WHERE crp_id_persona='" +crp.getCrp_id_persona()+ "'" +
			" AND crp_nombres='" +crp.getCrp_nombres()+ "'" +
			" AND crp_apaterno='" +crp.getCrp_apaterno()+ "'" +
			" AND crp_amaterno='" +crp.getCrp_amaterno()+ "'" +
			" AND crp_id_area='" +crp.getCrp_id_area()+ "'" +
			" AND crp_puesto='" +crp.getCrp_puesto()+ "'" ;
			...
			if(rs.getString("total2").equals("0")){
				...
				pstmnt = conn
				.prepareStatement("INSERT INTO cg_cat_remitente_persona (crp_id_persona, crp_nombres, crp_apaterno, crp_amaterno, crp_id_area, crp_puesto) VALUES (?, ?, ?, ?, ?, ?)");
