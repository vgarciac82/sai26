...
public class ObraPublicaEPManager {
	...
	private static final Logger	log	= Logger.getLogger(ObraPublicaEPManager.class);
	...
	public static List<String> getEPFromVistas(Connection conn, String usuario, String ef, String cartera, String oli, String ur, String ue) throws Exception {
		...
		String query = 
					"SELECT ep AS epID "
						+"FROM   vsaldosanuales WITH(nolock) "
						+"WHERE  ncuentap = '82106' "
						+"       AND cpartida IN (SELECT cpartida "
						+"                        FROM   dbo.toppartidasinoli partidas WITH( nolock ) "
						+"                               INNER JOIN dbo.tvistasur vistas WITH( nolock ) "
						+"                                       ON partidas.cunidadejecutora = vistas.ur "
						+"                        WHERE  vistas.modulo = 'OBRAPUBLICA' "
						+"                               AND vistas.usuario = ?) "
						+"       AND cunidadejecutora IN (SELECT ur "
						+"                                FROM   dbo.tvistasur "
						+"                                WHERE  modulo = 'OBRAPUBLICA' "
						+"                                       AND usuario = ?) ";
	...
	public static boolean ueSinOLIS(Connection conn, String ue ) throws Exception{
		String query = 
					"SELECT Count(*) AS PartidasSinOLI " 
					+ "FROM   dbo.toppartidasinoli WITH(nolock) " 
					+ "WHERE  cunidadejecutora = ? ";
		ResultSet rs = null;
		PreparedStatement ps = null;
