...
public class ObraPublicaContractManager {
    ...
    private static Logger log = LoggerFactory.getLogger(ObraPublicaContractManager.class);
    ...
    public static byte SaveApartado(Connection conn, ObraPublicaContract opc, String[] listHeader, String[][] listDetail) throws SQLException {
        ...
        // Opción
        cs.setByte(1, Byte.parseByte("1"));
        cs.registerOutParameter(2, Types.INTEGER);
        // Folio
        cs.setString(3, listHeader[0]);
        // Cartera
        cs.setString(4, listHeader[1]);
        // OLI
        cs.setString(5, listHeader[2]);
        // monto
        cs.setFloat(6, Float.parseFloat(listHeader[3]));
        // porcentaje IVA
        cs.setFloat(7, Float.parseFloat(listHeader[4]));
        // Monto con IVA
        cs.setFloat(8, Float.parseFloat(listHeader[5]));
        // Tipo obra
        cs.setString(9, listHeader[6]);
        // Tipo recurso
        cs.setString(10, listHeader[7]);
        // Descripcion
        cs.setString(11, listHeader[8]);
        // Status
        cs.setByte(12, Byte.parseByte(listHeader[9]));
        // Fecha de aplicacion
        cs.setString(13, listHeader[10]);
        // Ejercicio fiscal
        cs.setString(14, listHeader[11]);
        // Login
        cs.setString(15, listHeader[12]);
        // Ramo
        cs.setString(16, listHeader[13]);
        // Unidad responsable
        cs.setString(17, listHeader[14]);
        // DocumentoAplicado
        cs.setString(18, listHeader[15]);
        // Folio poliza
        cs.setInt(19, Integer.parseInt(listHeader[16]));
        // Tipo poliza
        cs.setString(20, listHeader[17]);
        // Folio poliza
        cs.setInt(21, Integer.parseInt(listHeader[18]));
        // cancelacion
        // fecha cancelacion
        cs.setString(22, listHeader[19]);
        // Termina llenado
        ...
        try {
            ...
            // IdHeader
            cs.setInt(3, opc.getIdApartadoHeader());
            // EP, pero no importa su valor
            cs.setString(4, "");
            // Mes
            cs.setByte(5, Byte.parseByte("0"));
            // importe, pero no importa su valor
            cs.setFloat(6, 0);
            // Evento, pero no importa su valor
            cs.setString(7, "");
            // importe negativo, pero no importa su valor
            cs.setFloat(8, 0);
            // Centro contable, pero no importa su valor
            cs.setString(9, "");
            // DocRenglon , pero no importa su valor
            cs.setInt(10, 0);
            // Call the inherited PreparedStatement.executeUpdate( ) method
            cs.execute();
            ...
            for (int i = 0; i < listDetail.length; i++) {
                ...
                cs.setInt(3, opc.getIdApartadoHeader());
                // EP
                cs.setString(4, listDetail[i][0]);
                // Mes
                cs.setByte(5, Byte.parseByte(listDetail[i][1]));
                // importe
                cs.setFloat(6, Float.parseFloat(listDetail[i][2]));
                // Evento
                cs.setString(7, listDetail[i][3]);
                // importe
                cs.setFloat(8, Float.parseFloat(listDetail[i][4]));
                // negativo
                // Centro contable
                cs.setString(9, listDetail[i][5]);
                // DocRenglon
                cs.setInt(10, Integer.parseInt(listDetail[i][6]));
                // Call the inherited PreparedStatement.executeUpdate( ) method
                cs.execute();
    ...
    public static byte SavePreCompromiso(Connection conn, ObraPublicaContract opc, String[] listHeader, String[][] listDetail) throws SQLException {
        ...
        cs.setString(12, listHeader[9]);
        // Fecha de convocatoria
        cs.setString(13, listHeader[10]);
        // Fecha de junta de aclaración
        cs.setString(14, listHeader[11]);
        // Fecha de recepcion de propuesta
        cs.setString(15, listHeader[12]);
        // Fecha de fallo
        cs.setString(16, listHeader[13]);
        cs.setByte(17, Byte.parseByte(listHeader[14]));
        // porcentaje
        cs.setFloat(18, Float.parseFloat(listHeader[15]));
        // anticipo
        // Status
        ...
        cs.setString(20, listHeader[17]);
        // Ejercicio fiscal
        cs.setString(21, listHeader[18]);
        // Login
        cs.setString(22, listHeader[19]);
        // Ramo
        cs.setString(23, listHeader[20]);
        // Unidad responsable
        cs.setString(24, listHeader[21]);
        // DocumentoAplicado
        cs.setString(25, listHeader[22]);
        // Folio poliza
        cs.setInt(26, Integer.parseInt(listHeader[23]));
        // Tipo poliza
        cs.setString(27, listHeader[24]);
        // Folio poliza
        cs.setInt(28, Integer.parseInt(listHeader[25]));
        // cancelacion
        // fecha cancelacion
        cs.setString(29, listHeader[26]);
        // Termina llenado
        // Call the inherited PreparedStatement.executeUpdate( ) method
        // Código que estaba
        try {
            ...
            cs.registerOutParameter(2, Types.INTEGER);
            // IdHeader
            cs.setInt(3, opc.getIdPreCompromisoHeader());
            // EP, pero no importa valor que se mande
            cs.setString(4, "");
            // para
            // esta opción del Stored Procedure
            // Mes, pero no importa
            cs.setByte(5, Byte.parseByte("99"));
            // valor
            // que se mande para
            // esta
            // opción del Stored
            // Procedure
            // importe, pero no importa valor que se mande
            cs.setFloat(6, 0);
            // para esta opción del Stored Procedure
            // importeAnt, pero no importa su valor
            cs.setFloat(7, 0);
            // importeDif, pero no importa su valor
            cs.setFloat(8, 0);
            // Evento, pero no importa su valor
            cs.setString(9, "");
            // importe negativo, pero no importa valor que
            cs.setFloat(10, 0);
            // se mande
            // Centro contable, pero no importa valor
            cs.setString(11, "");
            // que
            ...
            cs.setInt(12, 0);
            // Call the inherited PreparedStatement.executeUpdate( ) method
            // Invoca el borrado a través del stored procedure
            cs.execute();
            // Inserta detalle
            cs = null;
            opc.listEvent.clear();
            ...
            for (int i = 0; i < listDetail.length; i++) {
                ...
                cs.setFloat(6, Float.parseFloat(listDetail[i][2]));
                // importeAnt
                cs.setFloat(7, Float.parseFloat(listDetail[i][3]));
                // importeDif
                cs.setFloat(8, Float.parseFloat(listDetail[i][4]));
                // Evento
                cs.setString(9, listDetail[i][5]);
                // importe
                cs.setFloat(10, Float.parseFloat(listDetail[i][6]));
                // negativo
                // Centro contable
                cs.setString(11, listDetail[i][7]);
                // DocRenglon
                cs.setInt(12, Integer.parseInt(listDetail[i][8]));
                // Call the inherited PreparedStatement.executeUpdate( ) method
                cs.execute();
                // Validar si evento existe en la lista
    ...
    public static byte SaveCompromiso(Connection conn, ObraPublicaContract opc, String[] listHeader, String[][] listDetail) throws SQLException {
        ...
        cs.setByte(19, Byte.parseByte(listHeader[16]));
        // porcentaje
        cs.setFloat(20, Float.parseFloat(listHeader[17]));
        // anticipo
        // Fecha de inicio de cotrato
        cs.setString(21, listHeader[18]);
        ...
        cs.setString(26, listHeader[23]);
        // Tipo adicional
        cs.setByte(27, Byte.parseByte(listHeader[24]));
        // oficio
        cs.setString(28, listHeader[25]);
        // fecha oficio
        cs.setString(29, listHeader[26]);
        // es plurianual
        cs.setByte(30, Byte.parseByte(listHeader[27]));
        // Monto anticipo
        cs.setFloat(31, Float.parseFloat(listHeader[28]));
        // fecha autorización ventanilla
        cs.setString(32, listHeader[29]);
        // Tiene convenio
        cs.setByte(33, Byte.parseByte(listHeader[30]));
        // modificatorio
        // Clave de convenio modificatorio
        cs.setString(34, listHeader[31]);
        // Status
        cs.setByte(34, Byte.parseByte(listHeader[31]));
        // Fecha de aplicacion
        cs.setString(35, listHeader[32]);
        // Ejercicio fiscal
        cs.setString(36, listHeader[33]);
        // Login
        cs.setString(37, listHeader[34]);
        // Ramo
        cs.setString(38, listHeader[35]);
        // Unidad responsable
        cs.setString(39, listHeader[36]);
        // DocumentoAplicado
        cs.setString(40, listHeader[37]);
        // Folio poliza
        cs.setInt(41, Integer.parseInt(listHeader[38]));
        // Tipo poliza
        cs.setString(42, listHeader[39]);
        // Folio poliza
        cs.setInt(43, Integer.parseInt(listHeader[40]));
        // cancelacion
        // fecha cancelacion
        cs.setString(44, listHeader[41]);
        // Termina llenado
        // Call the inherited PreparedStatement.executeUpdate( ) method
        // Código que estaba
        try {
            ...
            // IdHeader
            cs.setInt(3, opc.getIdCompromisoHeader());
            // EP, pero no importa valor que se mande
            cs.setString(4, "");
            // para
            // esta opción del Stored Procedure
            ...
            cs.setFloat(6, 0);
            // para esta opción del Stored Procedure
            // importeAnt, pero no importa valor que se
            cs.setFloat(7, 0);
            // mande
            // para esta opción del Stored Procedure
            // importeDif, pero no importa valor que se
            cs.setFloat(8, 0);
            // mande
            // para esta opción del Stored Procedure
            // Evento
    ...
    public static byte SelectIdApartadoHeaderAndEventos(Connection conn, ObraPublicaContract opc) throws SQLException {
        ...
        // FolioSAI
        ps.setString(1, opc.getFolioSAI());
        try {
            opc.listEvent.clear();
    ...
    public static byte GetSituacionContratoObraPublica(Connection conn, ObraPublicaContract opc) throws SQLException {
        ...
        // Comienza llenado de parametros de SP
        // Opción
        cs.setString(1, opc.getFolioSAI());
        try {
            // ResultSet rs = cs.executeQuery();
