...
public class ConfiguraAplicativoManager extends DataSourceManager {
	...
	private static final Logger			log					= Logger.getLogger(ConfiguraAplicativoManager.class);
	...
	public static ArrayList layoutDetalle(Connection conn, int cLayout) throws SQLException {
		String cQuery = "select [c_layout], [b_pk], [b_autoincrementa], [d_nombre], [d_alias], [d_tipo], [n_longitud], [d_formato], [b_visible], [d_constante], [d_campodestino], [d_sql], [d_param_in], [d_tipo_gen], [b_Ignorar], [n_longitudfija] from t_layoutcont l where l.c_layout = " + cLayout
			+ " order by c_layoutCont";
	...
	public static String replicaCataPresup(Connection conn, String aEjercicioFiscal, String cNewEjercicioFiscal, String cReplicaCatalogos, String cCierreAnual) throws SQLException {
		...
		try {
			if ("SI".equals(cReplicaCatalogos)) {
				...
				while (rs.next()) {
					...
					if (!map.containsKey(cTablaReplicar)) {
						try {
							...
							cQueryReplica = "INSERT INTO " + cTablaReplicar + "(" + cCamposInsert + ") " + " SELECT " + cCamposValues + " FROM " + cTablaReplicar + " WHERE aEjercicioFiscal = '" + aEjercicioFiscal + "'" + " AND " + cCamposIn + " not in (SELECT " + cCamposIn + " FROM "
								+ cTablaReplicar + " WHERE aEjercicioFiscal = '" + cNewEjercicioFiscal + "' )";
				...
				String cQuerySubCuentaConf = "INSERT INTO tTipoSubcuentaConf (cSubcuenta, aEjercicioFiscal, nOrden, Interno, NombreCatalogo, NombreCampo, EtiquetaCampo, CentroCostos) " + " SELECT cSubcuenta, '" + cNewEjercicioFiscal
					+ "', nOrden, Interno, NombreCatalogo, NombreCampo, EtiquetaCampo, CentroCostos FROM  tTipoSubcuentaConf WHERE aEjercicioFiscal = '" + aEjercicioFiscal + "'";
				String cQueryEventoConf = "INSERT INTO tEventoConfiguracion ( cEvento, nCuenta, aEjercicioFiscal, nDocRenglon, TipoCuenta, dComponente) " + " SELECT cEvento, nCuenta, '" + cNewEjercicioFiscal
					+ "', nDocRenglon, TipoCuenta, dComponente FROM tEventoConfiguracion WHERE aEjercicioFiscal =  '" + aEjercicioFiscal + "' "
					+ "    AND cEvento+nCuenta+convert(varchar,nDocRenglon)+TipoCuenta+dComponente not in (SELECT cEvento+nCuenta+convert(varchar,nDocRenglon)+TipoCuenta+dComponente FROM tEventoConfiguracion WHERE aEjercicioFiscal = '" + cNewEjercicioFiscal + "' )";
				String cQueryEventoConfDet = "INSERT INTO tEventoConfiguraDetalle (cEvento, nCuenta, aEjercicioFiscal, nDocRenglon, nOrden, dDetalleCuenta ) " + "SELECT cEvento, nCuenta, '" + cNewEjercicioFiscal
					+ "', nDocRenglon, nOrden, dDetalleCuenta FROM tEventoConfiguraDetalle WHERE aEjercicioFiscal = '" + aEjercicioFiscal + "' "
					+ "  AND cEvento+nCuenta+convert(varchar,nDocRenglon)+convert(varchar,nOrden)+dDetalleCuenta not in (SELECT cEvento+nCuenta+convert(varchar,nDocRenglon)+convert(varchar,nOrden)+dDetalleCuenta FROM tEventoConfiguraDetalle WHERE aEjercicioFiscal = '" + cNewEjercicioFiscal + "')";
				String cQueryCatUnidadResp = "INSERT INTO tCatUnidadResponsable ( aEjercicioFiscal, cUnidadResponsable, D_DESCRIPCION, nAlcance) " + " SELECT '" + cNewEjercicioFiscal + "', cUnidadResponsable, D_DESCRIPCION, nAlcance FROM tCatUnidadResponsable WHERE aEjercicioFiscal = '"
					+ aEjercicioFiscal + "'" + "   AND cUnidadResponsable+D_DESCRIPCION+convert(varchar,nAlcance) not in ( SELECT cUnidadResponsable+D_DESCRIPCION+convert(varchar,nAlcance) FROM tCatUnidadResponsable WHERE aEjercicioFiscal = '" + cNewEjercicioFiscal + "') ";
				String cQueryCatalogoURCC = " INSERT INTO tCatalogoURCC (aEjercicioFiscal, cUnidadResponsable, cCentroContable, nConsecutivo) " + " SELECT  '" + cNewEjercicioFiscal + "', cUnidadResponsable, cCentroContable, nConsecutivo FROM tCatalogoURCC WHERE aEjercicioFiscal = '"
					+ aEjercicioFiscal + "'" + "    AND cUnidadResponsable+cCentroContable+convert(varchar,nConsecutivo) not in (SELECT cUnidadResponsable+cCentroContable+convert(varchar,nConsecutivo) FROM tCatalogoURCC WHERE aEjercicioFiscal = '" + cNewEjercicioFiscal + "')";
				String cQueryPartidaCuenta = "INSERT INTO  tCatalogoPartidaCuenta ( cPartida, aEjercicioFiscal, nCuenta) " + " SELECT cPartida, '" + cNewEjercicioFiscal + "', nCuenta FROM tCatalogoPartidaCuenta WHERE aEjercicioFiscal = '" + aEjercicioFiscal + "' "
					+ "    AND cPartida+nCuenta not in ( SELECT cPartida+nCuenta FROM tCatalogoPartidaCuenta WHERE aEjercicioFiscal = '" + cNewEjercicioFiscal + "' )";

				String cQueryCaratulaCuenta = "INSERT INTO tCatalogoCaratulaCuenta ( ccaratula, aEjercicioFiscal, nCuenta) " + " SELECT ccaratula, '" + cNewEjercicioFiscal + "', nCuenta FROM tCatalogoCaratulaCuenta WHERE aEjercicioFiscal = '" + aEjercicioFiscal + "'"
					+ "    AND ccaratula+nCuenta not in (SELECT ccaratula+nCuenta  FROM tCatalogoCaratulaCuenta WHERE aEjercicioFiscal = '" + cNewEjercicioFiscal + "') ";
