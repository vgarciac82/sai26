...
public class ConfiguraAplicativoBusinessLogic extends DataSourceManager {
	...
	private static Logger	log	= Logger.getLogger(ConfiguraAplicativoBusinessLogic.class);
						...
						if (iIniciaRow <= i) {
							...
							while (numCol < arrLDetalle.size()) {
								...
								if ("0".equalsIgnoreCase(cVisible)) {
									nCelIgnore++; // se consideran las colubnas
							...
							String elRegreso = validaLinea(linea, arrLDetalle, tablaDestino, elDelim); // el
																										// 4
																										// son
																										// los
																										// que
