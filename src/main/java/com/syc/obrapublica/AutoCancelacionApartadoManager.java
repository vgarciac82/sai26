...
public class AutoCancelacionApartadoManager {
    ...
    private static final Logger log = LoggerFactory.getLogger(AutoCancelacionApartadoManager.class);
    ...
    public static String revisaTiempoLimite(Connection conn) throws AccountingEngineException {
        ...
        String sqlSelect = " SELECT " + "p.foliosai, p.nFolioOPAHeader, " + "CASE " + " WHEN p.ccvecontrato IS NULL " + "	   OR p.ccvecontrato = '' THEN 'No Capturado' " + " ELSE p.ccvecontrato " + "END                                                       AS ccvecontrato " + ", " + "CONVERT(VARCHAR, p.faplicacion, 103)                      AS " + "faplicacion, " + "'$' " + "+ CONVERT( VARCHAR, ( CONVERT( MONEY, p.nmontoconiva) ) ) AS nmontoconiva " + ", " + "p.u_login, " + "p.diasapartado" + " FROM   vobra_publica_apartado_pendiente p " + " WHERE  CONVERT(DATE, faplicacion, 103) < CONVERT(DATE, Dateadd(dd, ?, Getdate()), 103  )";
