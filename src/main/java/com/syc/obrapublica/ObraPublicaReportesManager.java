...
public class ObraPublicaReportesManager {
	private static final Logger		log				= Logger.getLogger(ObraPublicaReportesManager.class);
	private static final String[]	columnasTotales	= { "", "LP", "I3P", "AD", "CC", "", "", "", "", "", "totalma", "totalcontratosiniva", "montoIVA", "montoTotal", "totalCMAmp", "totalCMRed", "total_cnt_cm", "ret2pc", "ret5pc", "totalpagos", "totalejercido", "por_ejercer",
		"penalizaciones_aplicadas", "saldo_del_contrato", "" };
...
//			totalCNA[13] = formato.format(totalGeneral.get("monto_ejercido"));
	...
	public static Map<String, Map<String, ReporteSeguimientoContratosBean>> generaReporteSeguimientoContratos(Connection conn, String fechaI, String fechaF, String idunidadresponsable, String idArea) throws Exception {
		...
		String token = " WHERE ";
		String query = "";
		query += "SELECT a.ccvecontrato, " + "       a.cdescripcion, " + "       a.cbeneficiario, " + "       a.crfc, " + "       a.total_obra_conv, " + "       a.estado_realizacion_obra, " + "       a.finicio_obra, " + "       a.ffin_obra, " + "       a.avance_fisico, " + "       Round(CASE "
			+ "               WHEN a.total_pagado IS NULL OR a.total_obra_conv = 0 THEN CONVERT(FLOAT, 0.0) " 
			+ "               ELSE CONVERT(FLOAT, a.total_pagado * 100 / a.total_obra_conv) " 
			+ "             END, 2)AS avance_financiero, " 
			+ "       a.avance_financiero_total, " 
			+ "       a.observaciones, "
			+ "       a.cu_cc, " 
			+ "       a.cu_ue, " 
			+ "       a.cu_ur, " 
			+ "       a.id_area " 
			+ "FROM   (SELECT ce.ccvecontrato, " + "               ce.cdescripcion, " + "               ce.cbeneficiario, " + "               ce.crfc, " + "               ( CASE "
			+ "                   WHEN cm.totalcm IS NULL THEN ce.nmonto " + "                   ELSE cm.totalcm " + "                 END ) " + "               total_obra_conv, " + "               CONVERT(VARCHAR, Isnull(ef.dentidadfederativa, '')) AS "
			+ "                      estado_realizacion_obra, " + "               CONVERT(VARCHAR, ce.ffechainicontr, 103)            AS " + "               finicio_obra, " + "               CONVERT(VARCHAR, ce.ffechafincontr, 103)            AS ffin_obra, "
			+ "               ''                                                  AS " + "               avance_fisico, " + "               CONVERT(FLOAT, Isnull(pag_op.total_pagado, 0))      AS " + "               total_pagado, "
			+ "               Isnull(pag_op.total_pagado, 0)                      AS " + "                      avance_financiero_total, " + "               ''                                                  AS " + "               observaciones, " + "               ce.cu_cc, "
			+ "               ce.cu_ue, " + "               ce.cu_ur, " + "               u.id_area " + ",   pag_op.fprogramadapago      FROM   tobrapublicacompromisoencabezado ce  WITH(NOLOCK)" 
			+ "               LEFT OUTER JOIN tcatalogoentidadfederativa ef  WITH(NOLOCK)"
			+ "                            ON ce.cidentidadfederativa = ef.centidadfederativa " 
			+ "               LEFT OUTER JOIN convenio_modificatorio_obra_publica cm  WITH(NOLOCK)" 
			+ "                            ON ce.ccvecontrato = cm.ccvecontrato "
			+ "               LEFT OUTER JOIN vpagosobrapublica pag_op  WITH(NOLOCK)" 
			+ "                            ON ce.ccvecontrato = pag_op.cfoliocontratoobra " 
			+ "               LEFT OUTER JOIN vimx_usuario u  WITH(NOLOCK)" 
			+ "                            ON ce.u_login = u.u_login "
			+ "        WHERE  cdocumentohaplicado = 'S') AS a";
	...
	public static Map<String, Map<String, ReporteContratosAdjudicadosBean>> generaReporteContratosAdjudicados(Connection conn, String fechaI, String fechaF, String idunidadresponsable, String idArea) throws Exception {
		...
		query += "select distinct  " 
			+ "       ope.ccvecontrato,ent.siglas AS subdir,entUR.siglas AS acontratante  " 
			+ "       ,mtot.cgrupofuncional,mtot.cfuncion,mtot.csubfuncion,mtot.cunidadejecutora,  " 
			+ "       mtot.cactividadinstitucional,mtot.cprogramapresupuestario  " 
			+ "       ,ope.nMontoConIVA as monto  " 
			+ "       ,cadj.siglasTAdjudicacion AS tipo_adjudicacion,ope.cdescripcion  " 
			+ "       ,ope.cbeneficiario  " 
			+ "       ,af.mmontoestimacion as monto_asignado  "
			+ "       ,mtot.suma as monto_ejercido  " 
			+ "       ,CASE WHEN ope.ctiporecurso = 'CE' THEN 'X' ELSE '' END AS cexterno_si  " 
			+ "       ,CASE WHEN ope.ctiporecurso <> 'CE' THEN 'X' ELSE '' END  AS cexterno_no  " 
			+ "       ,cto.cdescripcion AS tipo_obra  " 
			+ "       ,ope.crfc as cRFC " 
			+ "       ,CONVERT(VARCHAR, ope.ffechainicontr, 103) AS fFechaIniContr " 
			+ "       ,CONVERT(VARCHAR, ope.ffechafincontr, 103) AS fFechaFinContr " 
			+ "       ,ope.cu_ur,u.id_area "
			
			
			+ "from tobrapublicacompromisoencabezado ope  "
			+ "        left join (select  ccvecontrato, sum(mmontoestimacion) mmontoestimacion, sum(mmontoFisicoEjecutado) mmontoFisicoEjecutado, sum(mmontofisicoprogramado ) mmontofisicoprogramado from tobrapublicaavancefisico group by ccvecontrato) af on af.ccvecontrato = ope.ccvecontrato " 
			+ "        inner join(select sum(tp.mImporteMasIva) as suma,tp.cfoliocontratoobra " 
			+ "                      ,pod.cgrupofuncional,pod.cfuncion,pod.csubfuncion,pod.cunidadejecutora, " 
			+ "                       pod.cactividadinstitucional,pod.cprogramapresupuestario " 
			+ "                       from tpagoobraencabezado as tp  " 
			+ "                       LEFT OUTER JOIN v_op_pagos_ep pod ON pod.nfoliopagoobra = tp.nfoliopagoobra  "
			+ "                       where cDocumentoHaplicado='S'  ";

			query+= "                 group by tp.cfoliocontratoobra,pod.cgrupofuncional,pod.cfuncion,pod.csubfuncion,pod.cunidadejecutora, " 
			+ "                       pod.cactividadinstitucional,pod.cprogramapresupuestario  " 
			+ "                       )mtot on mtot.cFolioContratoObra=ope.cCveContrato  " 
			+ "        INNER JOIN tcatalogoadjudicacion cadj   WITH(NOLOCK) ON ope.ctipoadjudica = cadj.cidtadjudicacion  " 
			+ "        INNER JOIN tcatalogotipoobra cto   WITH(NOLOCK) ON ope.ctipoobra = cto.cidtobra  "
			+ "        LEFT OUTER JOIN mCatalogoEntidadFederativaRpt ent   WITH(NOLOCK) ON ope.cu_ue = ent.cIdUnidadEjecutora  " 
			+ "        LEFT OUTER JOIN mCatalogoEntidadFederativaRpt entUR  WITH(NOLOCK) ON ope.cu_ur = entUR.cIdUnidadEjecutora  " 
			+ "        INNER JOIN CG_CAT_EMPLEADO u  WITH(NOLOCK) ON ope.u_login = u.CE_OS_RESPONSABLE  ";
...
/*
	...
	public static Map<String, Map<String, ReporteF10Bean>> generaReporteFormato10(Connection conn, String cCentroContable, String fechaI, String fechaF, String idunidadresponsable, boolean generarVacio, Workbook wb, String reportType, String strUsuario, String strCondicion, String strCondMultiR, String columnasBorrar) throws Exception {
		...
		query += "select a.ccveconcurso, a.LP, a.I3P, case when a.AD + a.LP + I3P + CC = '' then '1' else  a.AD end AD , a.CC, a.articulo, a.pasivo, a.numero, a.contratista, a.cdescripcion, case when a.totalma = 0 then a.montoTotal else a.totalma end , a.totalcontratosiniva, a.montoIVA, a.montoTotal, a.totalCMAmp, a.totalCMRed, a.total_cnt_cm, a.ret2pc, a.ret5pc, 0.0 ret2_3iva, a.totalpagos, a.totalejercido, CONVERT(MONEY, a.total_cnt_cm - totalejercido) AS por_ejercer, mPenalizacion penalizaciones, 0 Saldo, '' observaciones, a.cu_ur, a.cu_cc, a.faplicacion   " 
			+ "FROM   (SELECT mPenalizacion, ccveconcurso,  " 
			+ "               CASE  " 
			+ "                 WHEN ctipoadjudica = '01' THEN '1' " 
			+ "                 ELSE ''  "
			+ "               END                                              AS LP, " + "               CASE " + "                 WHEN ctipoadjudica = '08' THEN '1' " + "                 ELSE ''  " + "               END                                              AS I3P, "
			+ "               CASE  " 
			+ "                 WHEN ctipoadjudica = '23' " 
			+ "                       OR ctipoadjudica = '24' THEN '1' " 
			+ "                 ELSE '' " 
			+ "               END                                              AS AD, " + "               CASE  "
			+ "                 WHEN ctipoadjudica = '17' THEN '1' " 
			+ "                 ELSE ''  " + "               END                                              AS CC, " + "               b.carticulo                                      AS articulo, "
			+ "               ''                                               AS pasivo, " + "               a.ccvecontrato                                   AS numero,  " + "               a.cbeneficiario                                  AS contratista, " + "               a.cdescripcion,  "
			+ "               CASE " + "                 WHEN c.monto_total_ma IS NULL THEN 0 " + "                 ELSE   c.monto_total_ma " + "               END                                              AS totalma,  "
			+ "               a.nmonto                                         AS  " + "               totalcontratosiniva,  " + "               CONVERT(MONEY, a.nmonto * a.nporceiva)           AS montoIVA, " + "               a.nmontoconiva                                   AS montoTotal,  "
			+ "               CONVERT(MONEY, ( CASE  " + "                                  WHEN d.totalcmiva IS NULL THEN 0.0 " + "                                  ELSE d.totalcmiva - a.nmontoconiva  " + "                                END ))                          AS totalCMAmp, "
			+ "               CONVERT(MONEY, 0.0)                              AS totalCMRed,  " + "               CONVERT(MONEY, ( CASE  " + "                                  WHEN d.totalcmiva IS NULL THEN a.nmontoconiva " + "                                  ELSE d.totalcmiva  "
			+ "                                END ))                          AS total_cnt_cm, " + "               Isnull(CONVERT(MONEY, e.ret2pcc + e.ret2pci), 0) AS ret2pc,  " + "               Isnull(CONVERT(MONEY, e.ret5pc), 0)              AS ret5pc,  "
			+ "               Isnull(CONVERT(MONEY, e.total_pagado), 0)        AS totalpagos,  " + "               Isnull( CONVERT(MONEY, e.ret2pcc + e.ret2pci ), 0)  " + "               + Isnull(CONVERT(MONEY, e.ret5pc), 0)  "
			+ "               + Isnull(CONVERT(MONEY, e.total_pagado), 0)      AS totalejercido " 
			+ "               , " 
			+ "               a.cu_ur, " 
			+ "               a.cu_cc,  " 
			+ "               a.faplicacion  " 
			+ "               , dbo.fn_entidadContrato(a.cCveContrato) entidad  " 
			+ "        FROM   tobrapublicacompromisoencabezado a  WITH(NOLOCK) "
			+ "               INNER JOIN tcatalogoadjudicacion b  WITH(NOLOCK) " 
			+ "                       ON a.ctipoadjudica = b.cidtadjudicacion " 
			+ "               LEFT OUTER JOIN vcontratomultianualobrapublica c   WITH(NOLOCK) " 
			+ "                            ON a.ccvecontrato = c.ccvecontrato  "
			+ "               LEFT OUTER JOIN convenio_modificatorio_obra_publica d  WITH(NOLOCK) " 
			+ "                            ON a.ccvecontrato = d.ccvecontrato  " 
			+ "               LEFT OUTER JOIN (SELECT a.cfoliocontratoobra,  "
			+ "                                       Sum(b.mimportemasiva) AS total_pagado, " 
			+ "                                       Sum(b.mobra5)         AS ret5PC, " 
			+ "                                       Sum(b.mcnic)          AS ret2pcC, "
			+ "                                       Sum(b.mPenalizacion)          AS mPenalizacion, "
			+ "                                       Sum(b.mimdt)          AS ret2pcI " 
			+ "                                FROM   tpagoobraencabezado a WITH(nolock) " 
			+ "                                       INNER JOIN tpagoobradetalle b WITH(nolock "
			+ "                                                  ) " 
			+ "                                               ON a.cdocumentohaplicado = 'S' " 
			+ "                                                  AND b.cevento <> 'ANTICIPO' "
			+ "                                                  AND a.nfoliopagoobra =  " 
			+ "                                                      b.nfoliopagoobra ";
...
//		query = "{call sp_pMultiReporte_syc_OP(' AND aEjercicioFiscal_1 = ''2013''', '''81101'',''81102'',''81103'',''81104'',''81105'',''81106'',''81107'',''81108'',''81109'',''81110'',''82101'',''82102'',''82103'',''82104'',''82105'',''82106'',''82107'',''82108''', 'cSubCuenta', 'cSubCuenta', 'mSaldo12', 'GENERAL', 'WROBLES', @Salida OUTPUT)}";
		...
		query +="select  uNormativa,tipoArea,uEjecutora,Original,Modificado,montoContratado,montoContratado/1000 as por_Contratado,montoEjecutado, montoEjecutado/1000 as por_Ejecutado,montoEstimado, montoEstimado/1000 as Porc_Estimado   "
			  + " from vConsolidadoporAreaEjecutora    "
			  + " group by  UniEjec, uEjecutora,tipoArea,UniNormativa,uNormativa,Original,Modificado,montoContratado,montoEjecutado,montoEstimado "
              + " order by uNormativa,tipoArea, uEjecutora ";
		
} else if ("CONSOLI_POR_AREA_EJECUTORA_y_PROYECTO".equalsIgnoreCase(reportType)) {
		
		query +="select  uEjecutora,tipoArea,Original,Modificado,montoContratado,montoContratado/1000 as por_Contratado,montoEjecutado, montoEjecutado/1000 as por_Ejecutado,montoEstimado, montoEstimado/1000 as Porc_Estimado   "
			  + " from vConsolidadoporAreaEjecutorayProyecto   "
			  + " group by  UniEjec, uEjecutora,tipoArea,Original,Modificado,montoContratado,montoEjecutado,montoEstimado "
	...
	private static double[][] sumaResumen(double[][] resumenTotal, double[][] resumen) {
		for (int i = 0; i < resumen.length; i++)
			for (int j = 0; j < resumen[i].length; j++)
				resumenTotal[i][j] += resumen[i][j];
		return resumenTotal;
	}
...
*/
		...
		if (arrDetalle != null) {
			...
			if ("FORMATO_10".equalsIgnoreCase(reportType)) {
				...
				int numAgrup = sheet.getNumMergedRegions();
				int agrupBorrar = numAgrup;
				while (  agrupBorrar > 0 ){ // Quitar agrupaciones
					sheet.removeMergedRegion(agrupBorrar);
						agrupBorrar--;				
				}
				for (int regBorrar = numRenglones; regBorrar >= 5; regBorrar--) { //Borrar renglones
					try {
						sheet.removeRow(sheet.getRow(regBorrar));
					}
					catch (Exception hazNada) {
...
/*					if (!"".equalsIgnoreCase(ctaAnterior) && !(ctaAnterior.equalsIgnoreCase((String)arrAdecua.get(0)))) {
					...
					if (!valAnterior.equalsIgnoreCase(celda.getStringCellValue())){
						...
						for (int numCol = 0; numCol < 36; numCol++) {
							sheet.addMergedRegion(new CellRangeAddress(
								rowDesde, // mention first row here
								rowHasta-1, //mention last row here, it is 1 as we are doing a column wise merging
								numCol, //mention first column of merging
				...
				primerDetalle =12; //posicion en donde se dibujara el  primer detalle
				nrengXLS=primerDetalle;
				int nTotCols = 10;
				...
				while (j < arrDetalle.size()){
					...
					if ("".equalsIgnoreCase(ctaAnterior)){ //Cuando el encabezado entra por primera vez
						arrEncabezado[2][2] = (String)arrAdecua.get(1);
						formatoEncabezado.setValCelda(arrEncabezado);
						formatoEncabezado.print(sheet,nrengXLS++);
						nrengXLS++;
					}
					if ("".equalsIgnoreCase(grupoTitulo)){ //Cuando el encabezado entra por primera vez
						arrEncabezado[2][2] = (String)arrAdecua.get(1);
						formatoTotalTipoArea.setValCelda(arrTotalTipoArea);
						formatoTotalTipoArea.print(sheet,nrengXLS++);
						nrengXLS++;
		...
		while (j <= ultRow){
			...
			while ( i < numberOfColumns) {
				...
				if (laCelda != null) {
					...
					if (eltipo == CellType.FORMULA) {    // 2 = formula, 0 = numerico, 1 = string
