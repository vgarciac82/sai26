...
public class AdvertenciaCancelacionComposser extends DataSourceManager implements MessageComposer {
	...
	private static final Logger		log					= Logger.getLogger(AdvertenciaCancelacionComposser.class);
	private static ModuleProperties	moduleProperties;
	int								diasMaximoApartado	= -1;
	int								diasAdvertencia		= -1;
	private static final String		ENCABEZADO_CORREO	= "<b>Atenci\u00F3n.</b>" + "<br><br>El sistema ha detectado los siguientes contratos con recursos apartados autorizados a punto de llegar a la fecha l\u00EDmite de tolerancia. "
...
// IRD RO-0003 Se agrega parámetro MotivoCancela
	public List<AdvertenciaCancelacionCorreo> composeMessageList(String listaFolios, String motivoCancela) throws Exception {
		...
		String query = 
			 "SELECT v.foliosai, "
			+"      CASE  "
			+"         WHEN v.ccvecontrato IS NULL  "
			+"               OR v.ccvecontrato = '' THEN 'No Capturado'  "
			+"         ELSE v.ccvecontrato  "
			+"       END                                                       AS ccvecontrato  "
			+"       ,  "
			+"       CONVERT(VARCHAR, v.faplicacion, 103)                      AS  "
			+"       faplicacion,  "
			+"       '$'  "
			+"       + CONVERT( VARCHAR, ( CONVERT( MONEY, v.nmontoconiva) ) ) AS nmontoconiva  "
			+"       ,  "
			+"       v.u_login,  "
			+"       v.diasapartado,  "
			+"       ? - v.diasapartado                                        AS  "
			+"       diasRestantes, "
			+"      isnull(caso.u_nombre,uApart.u_nombre)  AS nombre, "
			+"       isnull(caso.u_email,uApart.u_email)  AS correo, isnull(gp_valor,'') mailcopy "
			+" FROM   vobra_publica_apartado_pendiente v INNER JOIN  "
			+"		(SELECT	a.c_folio,  "
			+"				a.co_responsable,  "
			+"				u.u_login, "
			+"				u.u_email, "
			...
			+"							INNER JOIN cg_caso_operacion o WITH( NOLOCK ) "
			+"									ON c.ID_CASO = o.ID_CASO "
			+"					WHERE	c.ID_TC = 23 "
			+"				) AS a LEFT OUTER JOIN cg_usuario u "
			+"						    ON a.co_responsable = u.u_nombre "
			+"		) as caso ON v.foliosai = caso.c_folio left join cg_usuario uApart on uApart.u_login = v.u_login "
			+"     left join cg_grupo_propiedades gp on G_NOMBRE ='PREFERENCIAS_CLIENTE' and GP_NOMBRE= 'mop.mail.copy' "
			+" WHERE  diasapartado >= ?  "
			+"       AND ? - v.diasapartado > 0 order by isnull(caso.u_email,uApart.u_email) , diasRestantes";
...
// IRD RO-0003 Se agrega parámetro MotivoCancela. Si listaFolios viene de una cancelación msiva y el query es diferente
			query = "select distinct v.foliosai, CASE   "
			+" 			         WHEN v.ccvecontrato IS NULL   "
			+" 			               OR v.ccvecontrato = '' THEN 'No Capturado'   "
			+" 			         ELSE v.ccvecontrato   "
			+" 			       END                                                       AS ccvecontrato   "
			...
			+" 			       faplicacion,   "
			+" 			       '$' + CONVERT( VARCHAR, ( CONVERT( MONEY, v.nmontoconiva) ) ) AS nmontoconiva,   "
			+" 			       v.u_login, Datediff(day, v.faplicacion, Getdate()) diasapartado, 0 diasRestantes       ,  "
			+" 			      isnull(caso.u_nombre,uApart.u_nombre)  AS nombre,  "
			+" 			       isnull(caso.u_email,uApart.u_email)  AS correo, '' mailcopy "
			+" 			 from tObraPublicaApartadoEncabezado v "
			+"               INNER JOIN   "
			+" 					(SELECT	a.c_folio,   "
			+" 							a.co_responsable,   "
			+" 							u.u_login,  "
			...
			+" 									    ON a.co_responsable = u.u_nombre  "
			+" 					) as caso ON v.foliosai = caso.c_folio left join cg_usuario uApart on uApart.u_login = v.u_login "
			+" 								 LEFT OUTER JOIN tobrapublicaprecompromisoencabezado p WITH(nolock)   ON v.foliosai = p.foliosai  where v.FolioSAI in(" + listaFolios +") order by isnull(caso.u_email,uApart.u_email) , diasRestantes";
...
// IRD RO-0003 Si viene de cancelación masiva el correo es diferente
						AdvertenciaCancelacionCorreo advertencia = null;
							...
							final String		ENCABEZADO_CORREO2	= "<b>Atenci\u00F3n.</b>" + "<br><br>El administrador ha cancelado los contratos listados a continuación por el motivo :'" + motivoCancela + "'.<br> "
								+ "<br>Se hace este hecho de su conocimiento para que tome las acciones pertinentes.<br>Listado de contratos:";
