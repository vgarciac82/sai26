...
public class ObraPublicaManager {
	private static Logger		log		= Logger.getLogger(WSManager.class);
	
	private static final String	QUERY_ACTUALIZA_CM_ENCABEZADO			= "UPDATE tobrapublicaconvmodifencabezado " + "SET    ffechainicontr = CONVERT( VARCHAR,CONVERT(date,?), 103), " + "       ffechafincontr = CONVERT( VARCHAR,CONVERT(date,?), 103), " + "       mmonto = ?, "
		+ "       nporceiva = ?, " + "       mmontoconiva = ?, " + "       cdescripcionconvenio = ?, " + "       mmontoincremento = ?, " + "       mmontoincrementoiva = ?, " + "		 cNoConvenio = ? " + "WHERE  nFolioOPConvHeader = ? ";

	// IRD 20131121 RO-0009 se agregan variables inser y actualiza pago pasivo
	private static final String	QUERY_ACTUALIZA_PAG_PAS_ENCABEZADO		= "UPDATE tObraPublicaPagoPasivoEncabezado " + "SET cTipoPoliza = 'OP',  mMonto = ?, " + "       [nPorceIVA] = ?, " + "       [mMontoConIVA] = ?, "
		+ "       cDescripcionPagoPasivo = ?, foliosai = ?, fAplicacion = CONVERT( VARCHAR,CONVERT(date,?), 103), aEjercicioFiscal = ?  WHERE  nFolioOPPagPasHeader = ? ";

	private static final String	QUERY_INSERTA_PAG_PAS_ENCABEZADO		= "INSERT INTO tObraPublicaPagoPasivoEncabezado(cCveContrato, FolioSAI, [mMonto],[nPorceIVA],[mMontoConIVA],fAplicacion, aEjercicioFiscal, cRamo, cUnidadResponsableContable, cDocumentoHaplicado, nFolioPoliza, cTipoPoliza, nFolioPolizaCancelacion, fCancelacion, cU_UR, cU_CC, cU_UE, cDescripcionPagoPasivo) "
		+ "  SELECT cCveContrato, FolioSAI, ? [mMonto],? [nPorceIVA],? [mMontoConIVA],? fAplicacion, ? aEjercicioFiscal, cRamo, cUnidadResponsableContable, '' cDocumentoHaplicado, null nFolioPoliza, 'OP' cTipoPoliza, null nFolioPolizaCancelacion, null fCancelacion, cU_UR, cU_CC, cU_UE, ? cDescripcionPagoPasivo "
		+ "  FROM tobrapublicaapartadoencabezado where foliosai = ?  SELECT @@IDENTITY AS cConsecutivo";

	// MLR 20131217 RO-0010 se agregan variables inser y actualiza Plurianual
	private static final String	QUERY_ACTUALIZA_PLURIANUAL_ENCABEZADO	= "UPDATE tObraPublicaPlurianualEncabezado " + "SET cTipoPoliza = 'OP',  mMonto = ?, " + "       [nPorceIVA] = ?, " + "       [mMontoConIVA] = ?, "
		+ "    foliosai = ?, fAplicacion = CONVERT( VARCHAR,CONVERT(date,?), 103), aEjercicioFiscal = ?  WHERE  nFolioOPPlurianualHeader = ? ";

	private static final String	QUERY_INSERTA_PLURIANUAL_ENCABEZADO		= "INSERT INTO tobraPublicaPlurianualEncabezado(cCveContrato, FolioSAI, [mMonto],[nPorceIVA],[mMontoConIVA],fAplicacion, aEjercicioFiscal, cRamo, cUnidadResponsableContable, cDocumentoHaplicado, nFolioPoliza, cTipoPoliza, nFolioPolizaCancelacion, fCancelacion, cU_UR, cU_CC, cU_UE) "
		+ "  SELECT cCveContrato, FolioSAI, ? [mMonto],? [nPorceIVA],? [mMontoConIVA],? fAplicacion, ? aEjercicioFiscal, cRamo, cUnidadResponsableContable, '' cDocumentoHaplicado, null nFolioPoliza, 'OP' cTipoPoliza, null nFolioPolizaCancelacion, null fCancelacion, cU_UR, cU_CC, cU_UE "
		+ "  FROM tobrapublicaapartadoencabezado where foliosai = ?  SELECT @@IDENTITY AS cConsecutivo";

	private static final String	QUERY_COPIA_CM_ENCABEZADO				= "INSERT INTO tObraPublicaConvModifEncabezado " + "            (cnoconvenio, " + "             ccvecontrato, " + "             ffechainicontr, " + "             ffechafincontr, " + "             mmonto, "
		+ "             nporceiva, " + "             mmontoconiva, " + "             cdescripcionconvenio, " + "             ffechaconvenio, " + "             cramo, " + "             cunidadresponsablecontable, " + "             ctipopoliza, " + "             aejerciciofiscal, "
		+ "             cunidadresponsable, " + "             nfolioconvmodif, " + "             cu_ur, " + "             cu_cc, " + "             cu_ue, " + "             mmontoincremento, " + "             mmontoincrementoiva, " + "             cconvenioencaptura,"
		+ "             nFolioOPConvHeaderCancel," + "				fAplicacion) " + "SELECT cnoconvenio, " + "       ccvecontrato, " + "       ffechainicontr, " + "       ffechafincontr, " + "       mmonto, " + "       nporceiva, " + "       mmontoconiva, " + "       cdescripcionconvenio, "
		+ "       Getdate() AS fFechaConvenio, " + "       cramo, " + "       cunidadresponsablecontable, " + "       ctipopoliza, " + "       aejerciciofiscal, " + "       cunidadresponsable, " + "       nfolioconvmodif, " + "       cu_ur, " + "       cu_cc, " + "       cu_ue, "
		+ "       mmontoincremento, " + "       mmontoincrementoiva, " + "       'S'       AS cConvenioEnCaptura," + "       nfolioopconvheader AS nFolioOPConvHeaderCancel, " + "       getDate()" + "FROM   tobrapublicaconvmodifencabezado WITH(NOLOCK)" + "WHERE  nfolioopconvheader = ? "
	...
	public static int actualizaEncabezadoConvenioModificatorio(Connection conn, String folioConvenio, String fFechaIniContr, String fFechaFinContr, double mMonto, double nPorceIVA, double mMontoConIVA, String cDescripcionConvenio, double mMontoIncremento, double mMontoIncrementoIVA,
		int nFolioOPConvHeader) throws Exception {
		int afectados = 0;
		PreparedStatement ps = null;
	...
	public static int registraEstimacionObra(Connection conn, EstimacionObra estimacion) throws Exception{
		String query = "INSERT INTO tObraPublicaAvanceFisico( FolioSAI ,cCveContrato,fEntregaVentanilla,noEstimacion,mMontoEstimacion,"
			         + "                                      eFiscalPago,nPorceAvanceFisicoEstimado,nPorceAvanceFisicoEjecutado,"
			         + "                                      nPorceAvanceFisicoProgramado,mmontoFisicoEjecutado,mmontoFisicoProgramado,"
			         + "                                      mesEstimado,fperiodoEstimacionIni,fperiodoEstimacionFin,mMontoEstimacionIva,"
			         + "                                      mMontoEstimacionMasIva,mMontoEstimacionAmortizado,mMontoEstimacionRetencion,"
			         + "                                      ultimaEstimacion,esCapitalizable)"
			+ "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		PreparedStatement ps = null;
		int insertados = 0;
	...
	public static int actualizaEstimacionInfoInmueble(Connection conn,  EstimacionObra estimacion, int idRePublicWorkPartial) throws Exception{
		String queryUpdate = 	  "UPDATE	tobrapublicaavancefisico "
								+ "   SET	nidRePublicWorkPartial = ? "
								+ " WHERE	foliosai = ? "
								+ "   AND	noestimacion = ? ";
		PreparedStatement ps = null;
		try{
