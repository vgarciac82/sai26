...
private static final long	serialVersionUID	= -1868632393274575540L;
	...
	private static final String	htmlBodyI			= "<HTML>\n " + "\t<HEAD><style type=\"text/css\">.dollars:before { content:'$'; }</style>\n " + "\t\t<TITLE>\n</TITLE>\n " + "\t\t<style type=\"text/css\">\n " + "table td {" + "\tborder-color: black;" + "\tborder-style: solid;" + "\tborder-width: thin;" + "}" + ".encabezado {"
														+ "\tfont-family: Arial, \"Helvetica Neue\", Helvetica, sans-serif;" + "\tfont-size: 5 px;" + "\tfont-weight: bold;" + "\ttext-align: center;" + "}" + ".titulo {" + "\tborder-style: none;"
														+ "\tfont-family: Calibri, Verdana, Ariel, sans-serif;" + "\tfont-size: 11 px;" + "\tfont-weight: bold;" + "}" + "\t\t</style>\n " + "\t</HEAD>\n " + "\t<BODY>\n";
	private static final String	htmlBodyF			= "\n\t</BODY>\n" + "</HTML>";

	private static final String	HTML_BODY_ROP_RCA_I	= "<HTML>" + "\t<HEAD>" + "\t\t<TITLE></TITLE>" + "\t\t<style type=\"text/css\">" + "\t\t\t.tituloPrincipal {" + "\t\t\t\tfont-family: Arial, Helvetica, sans-serif;" + "\t\t\t\tfont-size: 16 pt;" + "\t\t\t\ttext-align: center;"
														+ "\t\t\t\tfont-weight: bold;" + "\t\t\t\tborder-style: none;" + "\t\t\t}" + "\t\t\t" + "\t\t\t.tituloPrincipal2 {" + "\t\t\t\tfont-family: Arial, Helvetica, sans-serif;" + "\t\t\t\tfont-size: 14 pt;" + "\t\t\t\ttext-align: center;"
														+ "\t\t\t\tfont-weight: bold;" + "\t\t\t\tborder-style: none;" + "\t\t\t}" + "\t\t\t" + "\t\t\t.tituloPrincipal3 {" + "\t\t\t\tfont-family: Arial, Helvetica, sans-serif;" + "\t\t\t\tfont-size: 14 pt;" + "\t\t\t\ttext-align: left;"
														+ "\t\t\t\tfont-weight: bold;" + "\t\t\t\tborder-style: none;" + "\t\t\t}" + "\t\t\t" + "\t\t\t.tituloNormal {" + "\t\t\t\tfont-family: Arial, Helvetica, sans-serif;" + "\t\t\t\tfont-size: 12 pt;" + "\t\t\t\ttext-align: center;"
														+ "\t\t\t\tfont-weight: normal;" + "\t\t\t\tborder-style: none;" + "\t\t\t}" + "\t\t\t" + "\t\t\t.centradoVertical {" + "\t\t\t\tfont-family: Arial, sans-serif;" + "\t\t\t\tfont-size: 9.0 pt;" + "\t\t\t\ttext-align: center;"
														+ "\t\t\t\tfont-weight: normal;" + "" + "\t\t\t\tvertical-align: middle;" + "\t\t\t} \t.tituloTabla1 {" + "\t\t\t\tfont-family: Arial, sans-serif;" + "\t\t\t\tfont-size: 9.0 pt;" + "\t\t\t\ttext-align: center;"
														+ "\t\t\t\tfont-weight: normal;" + "\t\t\t\tbackground: #CCFFFF;" + "\t\t\t\tvertical-align: middle;" + "\t\t\t}" + "\t\t\t" + "\t\t\ttable td {" + "\t\t\t\tborder-color: black;" + "\t\t\t\tborder-style: solid;" + "\t\t\t\tborder-width: thin;"
														+ "\t\t\t\tfont-family: Arial, Helvetica, sans-serif;" + "\t\t\t\tfont-size: 8.0 pt;" + "\t\t\t\ttext-align: center;" + "\t\t\t}" + "\t\t</style>" + "\t</HEAD>" + "\t<BODY>";
...
String token = "";
				if (req.getParameter("original") != null ) {
					...
					*/
					...
					excelConPlantilla(req, resp, "60", cCentroContable, fechaI, fechaF, idunidadresponsable, generarVacio, "", reportType, u.getLogin(), strCondicion, strCondMultiR, columnasBorrar, strGeneral, strResumen); //Multireporte Obra Pública
			...
			try {
				...
				excelConPlantilla(req, resp, "61", "", "", "", idunidadresponsable, generarVacio, "", reportType, u.getLogin(), "", "", "", "", ""); //Exportación desde captura PAOP
				resp.setContentType("application/vnd.ms-excel");
				String postFijo = "";
				resp.addHeader("Content-Disposition","inline; filename=\"" + System.currentTimeMillis()+ "CONSOLI_AREA_RESP.xls" + postFijo +"\"; charset=UTF-8\" pageEncoding=\"utf-8\"");
				wb.write(resp.getOutputStream());
...
//							 boldStyle.setAlignment(HorizontalAlignment.CENTER);
		...
		while (j <= ultRow){
			...
			while ( i < numberOfColumns) {
				...
				if (laCelda != null) {
					CellType eltipo = laCelda.getCellType();
					if (eltipo == CellType.NUMERIC) {    // 2 = formula, 0 = numerico, 1 = string
