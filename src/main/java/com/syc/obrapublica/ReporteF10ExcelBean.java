...
public class ReporteF10ExcelBean {
	...
	public ReporteF10ExcelBean(int numReg, int numCol, Sheet sheet, int regInicio, int numAgupa) {
		...
		while (nRow < numReg) {
			...
			for (int nCol = 0; nCol < numCol; nCol++) {
				...
				switch  (celda.getCellType()) {// 2 = formula, 0 = numerico, 1 = string
					case NUMERIC:
	...
	public void acumula(String[][] pValCelda){
		for (int nCol = 0; nCol < this.numCol; nCol++) {
			switch  (this.tipoCelda[0][nCol]) {// 2 = formula, 0 = numerico, 1 = string
				case 0:
					try {
	...
	public void reinicia(){
		...
		for (int nCol = 0; nCol < this.numCol; nCol++) {
			switch  (this.tipoCelda[nRow][nCol]) {// 2 = formula, 0 = numerico, 1 = string
				case 0:
					try {
...
//				celda.setCellType(tipoCelda[nRow][nCol]);
				switch  (tipoCelda[nRow][nCol]) {// 2 = formula, 0 = numerico, 1 = string
					case 0:
						try {
		...
		for (nRow = 0; nRow < this.numAgupa; nRow++) {
			sheet.addMergedRegion(new CellRangeAddress(
				regInicio+arrayAgrupa[nRow][0], // mention first row here
				regInicio+arrayAgrupa[nRow][1], //mention last row here, it is 1 as we are doing a column wise merging
				arrayAgrupa[nRow][2], //mention first column of merging
				arrayAgrupa[nRow][3]  //mention last column to include in merge
                ));
		}
	}
