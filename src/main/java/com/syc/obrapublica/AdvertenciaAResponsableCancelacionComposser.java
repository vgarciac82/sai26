...
public class AdvertenciaAResponsableCancelacionComposser extends DataSourceManager implements MessageComposer {
	...
	private static final Logger		log					= Logger.getLogger(AdvertenciaAResponsableCancelacionComposser.class);
	...
	public String composeMessage() throws Exception {
		...
		ResultSet rs = null;
		String query = "SELECT v.foliosai, " + "       CASE " + "         WHEN v.ccvecontrato IS NULL " + "               OR v.ccvecontrato = '' THEN 'No Capturado' " + "         ELSE v.ccvecontrato " + "       END                                                       AS ccvecontrato "
			+ "       , " + "       CONVERT(VARCHAR, v.faplicacion, 103)                      AS " + "       faplicacion, " + "       '$' " + "       + CONVERT( VARCHAR, ( CONVERT( MONEY, v.nmontoconiva) ) ) AS nmontoconiva " + "       , " + "       v.u_login, " + "       v.diasapartado, "
			+ "       ? - v.diasapartado                                       AS " 
			+ "       diasRestantes " + "FROM   vobra_publica_apartado_pendiente v " 
			+ " WHERE  diasapartado >= ? "
			+ " AND       ? - v.diasapartado > 0 ";
