...
public class ProimproInfoReaderManager {
	...
	private static final Logger	log	= Logger.getLogger(ProimproInfoReaderManager.class);
	...
	public static List<String> getEPs(Connection conn, String ef, String cartera, String oli, String ur, String ue) throws Exception {
		...
		try {
			String queryOli = "SELECT CASE  " + "         WHEN epproinpro <> '-1' THEN epproinpro  " + "         ELSE ''  " + "       END        AS descEP,  " + "       epproinpro AS idEP  " + "FROM   (SELECT '-1' AS EPProInpro  " + "        UNION  " + "        SELECT ( CONVERT(VARCHAR," + ef
				+ ") " + "                 + '.16.' + Substring(idcvepres, 15, 3) + '.'  " + "                 + Substring(idcvepres, 1, 13) + '.'  " + "                 + Substring(idcvepres, 19, 14) + '.' + CASE Len(CONVERT(VARCHAR  " + "                 ,  " + "                 idestado))  "
				+ "                        WHEN 1 THEN '0' + CONVERT(VARCHAR, idestado) ELSE  " + "                 CONVERT(  " + "                 VARCHAR,  " + "                        idestado) END + '.' + ? ) AS EPProInpro  " + "        FROM   cnsolis  " + "        WHERE  monto > 0  "
	...
	public static String getImporteOliCarteraURUE(Connection conn, String cartera, String capitulo, String oli, String ur, String ue) throws Exception {
		...
		query = "select isnull(sum(Monto), 0) monto from cnsolis where idCvePres like ? and idProyecto in (" 
			  + "select distinct idProyecto from cnsolis "
			  + "where idCvePres like ? and idOLIOf = ?)";
		String cond = "";
