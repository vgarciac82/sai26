...
public class EgresosInterface implements TipoCasoInterface {
	...
	private static Logger				log				= Logger.getLogger( EgresosInterface.class );
	...
	public void onVenceCaso( Connection conn, String folio, int id_caso, int id_tc, int id_oper, int porc ) throws SQLException {
		...
		try {
			...
			if ( folioSeparado[0].equalsIgnoreCase( "PDIR" ) )// Pago Directo
				tipoPago = "PagoDirecto";
			else if ( folioSeparado[0].equalsIgnoreCase( "PDIV" ) )// Pago
																	// Diverso
				tipoPago = "PagoDiverso";
			else if ( folioSeparado[0].equalsIgnoreCase( "PFED" ) )// Pago
																	// Federalizado
				tipoPago = "PagoFederalizado";
			else if ( folioSeparado[0].equalsIgnoreCase( "POBR" ) )// Pago Obra
				tipoPago = "PagoObra";
			else if ( folioSeparado[0].equalsIgnoreCase( "RELG" ) )// Pago Obra
				...
				case 1:// captura
					...
					if ( porc > 800 && porc < 4000 ) {
						if ( !StringUtils.isEmpty( correo ) ) {
							 ...
							 * datos en tabla
							 */
						else if ( !existeInfo( conn, folio, tipoPago, nfolio ) ) {
