...
public class AltaProveedorManager {
	...
	private static Logger			log	= Logger.getLogger( AltaProveedorManager.class );
	...
	public boolean updateDatAltaProveedor( Connection conn, DatosProveedor datProv ) throws Exception {
		PreparedStatement ps = null;
		boolean success = false;
		String query = "UPDATE dbo.tAltaProveedor SET cIdTipoPersona=" + datProv.getnTipoPersona() + ",cTipoRegistro='" + datProv.getcTipoPB() 
		+ "' " + ",cIdRFC='" + datProv.getRfc() + "',cRazonSocial='" + datProv.getcRazonSocial() + "',cCURP='" + datProv.getcCurp() 
		+ "',cApellidoPaterno='" + datProv.getcApellidoPat() + "',cApellidoMaterno='" + datProv.getcApellidoMat() + "',cNombre='" 
		+ datProv.getcNombre() + "'" + ",cGiro='" + datProv.getcGiro() + "',cIdEntidadFederativa=" + datProv.getnEntidadFederativa() 
		+ ",cIdMunicipio=" + datProv.getnMunicipio() + ",cCalle='" + datProv.getcCalle() + "',cNumeroExterno='" 
		+ datProv.getcNumeroExt() + "'" + ",cNumeroInterno='" + datProv.getcNumeroInt() + "',cColonia='" + datProv.getcColonia() 
		+ "',cCodigoPostal='" + datProv.getcCodigoPost() + "',cEmail='" + datProv.getcEmail() + "',cUrl='" + datProv.getcPaginaWeb() 
		+ "',nIdPyme=" + datProv.getnPyme() + ",nIdTipoTelefono=" + datProv.getnTipoTelefono() + ",cTelefono='" + datProv.getcTelefono() 
		+ "',cExtranjero=" + datProv.getnExtranjero() + ",cPais='" + datProv.getcPais() + "' " + ",nIdEmpleado=" + ( datProv.getnTipoPersona() == 3 ? datProv.getnNumEmpleado() : "null" ) 
		+ ",cNumeroREPSE = '" + datProv.getNumRepse() + "' " + ",id_regimen_fiscal=" + datProv.getIdRegimenFiscal() + ",cIdLocalidad='"+datProv.getcLocalidad()+"' " 
		+ " WHERE cFolio='" + datProv.getcFolio() + "'";
		try {
			log.info( query );
			ps = conn.prepareStatement( query );
			success = ps.executeUpdate() > 0;
	...
	public boolean hayContratoPendienteDePago( Connection conn, String cIdRFC ) throws Exception {
		...
		String query = "select cIdContrato,comEnc.totalComp,pagos.Pagado " 
				+ "			from (select " 
				+ "				cIdContrato,SUM(mImporte)totalComp from tCompromisoEncabezado as comEnc with(nolock)" 
				+ "				inner join tCompromisoDetalle comDet with(nolock)" 
				+ "				on comDet.nFolioCompromiso=comEnc.nFolioCompromiso" 
				+ "				and comEnc.cDocumentoHaplicado='S'" 
				+ "				where comEnc.cTipoContrato='DI'" 
				+ "				group by cIdContrato" 
				+ "			)comEnc" 
				+ "			inner join(" 
				+ "				select cont.cIdContratoDefinitivo from mContrato as cont with(nolock)  left join mContratoTerminacionAnticipada as terAnt with(Nolock) on terAnt.cIdContratoDefinitivo=cont.cIdContratoDefinitivo where cont.nIdEstado=4  and terAnt.cIdContratoDefinitivo is null and cIdRFC='" + cIdRFC + "'" 
				+ "				union" 
				+ "				select cIdPedidoDefinitivo  from mPedido as ped with(nolock)where ped.nIdEstado=4 and cIdRFC='" + cIdRFC + "'" 
				+ "			)cont on cont.cIdContratoDefinitivo=comEnc.cIdContrato" 
				+ "			left join(" 
				+ "				SELECT " 
				...
				+ "			)pagos on pagos.folioContPed=cont.cIdContratoDefinitivo" 
				+ "			where comEnc.totalComp>isnull(pagos.Pagado,0) ";
		try {
			log.info( query );
			ps = conn.prepareStatement( query );
			rs = ps.executeQuery();
