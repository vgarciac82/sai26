...
public class AuditoriaManager {
    ...
    public static int insert(Connection conn, String usuario, String area, String aplicacion, String modulo, String accion, String valor_origen, String valor_destino, String valor, String valor_llave) throws SQLException {
        ...
        PreparedStatement pstmnt = null;
        try {
            pstmnt = conn.prepareStatement("INSERT INTO IMXAUDITORIA " + "(nombre_aplicacion, nombre_modulo, accion, valor_origen,valor_destino, usuario, area_usuario, fecha_operacion,query,valor_llave) " + "VALUES (?,?,?,?,?,?,?,GETDATE(),?,?)");
    ...
    public static int update(Connection conn, Auditoria a) throws SQLException {
        ...
        PreparedStatement pstmnt = null;
        try {
            pstmnt = conn.prepareStatement("UPDATE IMXAUDITORIA SET id_auditoria = ?, nombre_aplicacion = ?, nombre_modulo = ?" + ", accion = ?, valor = ?, usuario = ?, fecha_operacion = ? WHERE id_auditoria = ?");
    ...
    public static List select(Connection conn, String modulo, String accion, String login, String fechaini, String fechafin, String area) throws SQLException {
        ...
        PreparedStatement pstmnt = null;
        ResultSet rs = null;
        String where = " where au.area_usuario=convert(nchar,area.id_area) and au.usuario=usu.u_login and " + " area.ID_AREA=ur.ID_AREA ";
        String and = " and ";
        ...
        //Ethiel, ahora es esta la condicion para buscar sobre el afectado y no sobre el que hace la afectacion
        where += (login != null && !"".equals(login) ? and + " (au.valor_origen like '%" + login + "%' or au.valor_destino like '%" + login + "%')" : "");
        ...
        //Ethiel, ahora es esta la condicion para buscar sobre el afectado y no sobre el que hace la afectacion
        where += (area != null && !"".equals(area) ? and + " (au.valor_origen like '%" + area + "%' or au.valor_destino like '%" + area + "%')" : "");
        ...
        String query = "select au.id_auditoria, au.nombre_modulo, au.accion, au.valor_origen, au.valor_destino, au.usuario, au.area_usuario, usu.u_nombre," + " area.D_DESCRIPCION AS area_usuario_desc, " + //" (SELECT D_DESCRIPCION FROM CG_CAT_AREAS WHERE ID_AREA = area.ID_AREA_PADRE) AS area_padre_desc, "+
        " ur.cUnidadResponsable as UR, " + " au.fecha_operacion, au.query " + " FROM IMXAUDITORIA AS au, CG_CAT_AREAS AS area, cg_usuario as usu, tCatalogoUnidadResponsable ur  " + where + " order by au.fecha_operacion";
