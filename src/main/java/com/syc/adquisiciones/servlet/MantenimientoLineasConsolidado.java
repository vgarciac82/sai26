...
public class MantenimientoLineasConsolidado extends HttpServlet  implements GestionInterface{
	...
	private synchronized void aplicaContablemente( HttpServletRequest request,
		...
		try{
			...
			if(operacion==3) {//Cancela el precompromiso de todas las lineas del consolidado que no se adjudicaron
				cmst1 = conn.prepareCall("{?= call sp_mLiberaProcomMaterialesLineasSinAdjudicar (?,?,?,?,?,?,?)}");
			}
			cmst1.registerOutParameter(1, Types.INTEGER); 
			cmst1.setString(2, idCons);//@cIdConsolidado
			cmst1.setInt(3, nIdLineaCons);////@nIdLineaConsolidado
			cmst1.setInt(4,nFolioPrecom );//@nFolioPrecom
			cmst1.setString(5,cCentroContable );//@centroContable
			cmst1.setString(6, ramo);//@ramo
			cmst1.setString(7, unidadE);//@cIdUnidadEjecutora
			cmst1.setInt(8, operacion);//@operacion
			cmst1.execute();		
