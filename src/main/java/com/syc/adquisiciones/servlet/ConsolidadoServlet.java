...
public class ConsolidadoServlet extends HttpServlet implements GestionInterface {
		...
		if(request.getParameter("requisicion")!=null){
			try {
				...
				if(c!=null){
					...
					pstm = conn.prepareStatement("UPDATE mConsolidado SET FOLIO_AMPLIACION = ?, consecutivoAmpliacion=?" + 
						" WHERE cIdTipoConsolidado = ? "+
		...
		try{
			...
			if (acr.isSuccess()) {
				...
				if(outputValue == 0){
					...
					pstm = conn.prepareStatement("UPDATE mConsolidado SET nIdEstado = 2 , ConsecutivoPRECOMP = ?, C_FOLIO_PRE = ? " + 
						" WHERE cIdTipoConsolidado = ? "+
						" and cIdUnidadEjecutora = ?"+
						" and nIdConsecutivo = ?"+
						" and cEjercicio=?");
			...
			if (acr.isSuccess()) {
				...
				log.debug("folio "+ request.getParameter("nFolioPrecompromiso"));
					...
					if(outputValue == 0){
						...
						int estado;
						log.debug(partidas);
						if(partidas==0){ // no devuelve a capturado, solo elimina folios, el precompromiso viene de disponible
							estado=2 ;
						}else{ // se cambia el estado a capturado, el precompromiso viene de un apartado
							estado=  1;
						}
						pstm = conn.prepareStatement("UPDATE mConsolidado SET nIdEstado=?,  ConsecutivoPRECOMP = null, C_FOLIO_PRE=null " + 
							" WHERE cIdTipoConsolidado = ? "+
							" and cIdUnidadEjecutora = ?"+
							" and nIdConsecutivo = ?"+
							" and cEjercicio=?");
						...
						if(partidas==1){ // el precompromiso viene de un apartado
						...
						pstm1 = conn1.prepareStatement("UPDATE mConsolidado SET nIdEstado = 2 " + 
							" WHERE cIdTipoConsolidado = ? "+
							" and cIdUnidadEjecutora = ?"+
							" and nIdConsecutivo = ?"+
							" and cEjercicio=?");
	...
	private Caso getCaso(HttpSession session){
		...
		" FROM mConsolidado p, CG_CASO c, CG_CASO_OPERACION o "+
		" where p.C_FOLIO_PRE=c.C_FOLIO"+
		" and c.ID_CASO=o.ID_CASO"+
		" and p.cEjercicio=?"+
		" and p.cIdTipoConsolidado=?"+
...
//		}
	...
	private String cuantaDisponible(Connection conn,String cIdConsolidado) throws SQLException{
		...
		ResultSet rs=null;
		try{
			String query=" select distinct cNumCuentaDisp from mConsolidadoSolicitud consolSol WITH (NOLOCK)   inner join mSolicitud as sol WITH (NOLOCK) on sol.cIdSolicitud=consolSol.cIdSolicitud "
						+"where cIdConsolidado='"+cIdConsolidado+"'";
	...
	private void nuevoAutomatico(HttpServletRequest request,HttpServletResponse response,HttpSession session)throws ServletException, IOException{
		...
		try {
			...
			String query="select distinct mc.cIdUsuarioCreacion,mc.cIdConsolidado,u.U_NOMBRE from mConsolidado as mc with(nolock) "
				+"inner join mConsolidadoPreseleccionSolicitudes as mcps with(nolock) on mc.cIdConsolidado=mcps.cIdConsolidado "
				+"inner join cg_usuario as u with(nolock) on u.U_LOGIN=mc.cIdUsuarioCreacion "
				+"where mcps.cIdSolicitud='"+cIdSolicitut+"'";
			log.info("query que valida si la requisición ya se encuentra en un consolidado : "+query);
			ps = conn.prepareStatement(query);
			rs = ps.executeQuery();
			if(rs.next()){
				mensaje="La requisicion "+cIdSolicitut+" ya se encuentra en el consolidado "
					+rs.getString("cIdConsolidado")+".\nEl usuario creador fue "+rs.getString("U_NOMBRE");
				respuesta="0";
			...
			cmst.setString(6,usuario.getLogin()); //usuario
