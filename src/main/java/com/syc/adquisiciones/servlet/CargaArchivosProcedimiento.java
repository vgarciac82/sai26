...
public class CargaArchivosProcedimiento extends HttpServlet {
	 ...
	 */
	public CargaArchivosProcedimiento() {
		super();
	}
		...
		if ((contentType != null) && (contentType.indexOf("multipart/form-data") >= 0)) {
			...
			try{
				...
				response.sendRedirect("../Generador/SAICYS/ArchivosProcedimiento.jsp?cIdProcedimientoArchivo="+session.getAttribute(GestionInterface.ATT_ProTipoProcedimiento)+ "-" + session.getAttribute(GestionInterface.ATT_ProUnidadEjecutora)+ "-" + session.getAttribute(GestionInterface.ATT_ProConsecutivo) +
									...
									"&cIdConsolidadoArchivo="+cIdConsolidado+"&cTipoArchivo="+tipoArchivo+"&cMensaje=El archivo se cargo correctamente.");
			}
			catch(Exception e){
				response.sendRedirect("../Generador/SAICYS/ArchivosProcedimiento.jsp?cIdProcedimientoArchivo="+session.getAttribute(GestionInterface.ATT_ProTipoProcedimiento)+ "-" + session.getAttribute(GestionInterface.ATT_ProUnidadEjecutora)+ "-" + session.getAttribute(GestionInterface.ATT_ProConsecutivo) +
						"&cEjercicioArchivo="+session.getAttribute(GestionInterface.ATT_ProEjercicio)+
						"&cIdConsolidadoArchivo="+cIdConsolidado+"&cTipoArchivo="+tipoArchivo+"&cMensaje=Ocurrio un error al cargar el archivo.");
				e.printStackTrace();
			}
		}
	}
