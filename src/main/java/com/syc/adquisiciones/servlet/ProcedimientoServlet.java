...
public class ProcedimientoServlet extends HttpServlet implements GestionInterface {
	...
	private synchronized void aplicaContablemente(String strParam, HttpServletRequest request,
		HttpServletResponse response,HttpSession session,String[] responsable, String[] nombre) throws ServletException, IOException {
		...
		try{
			...
			if (param[6].equalsIgnoreCase("1")){
				...
				AccountingEngine accEng = new AccountingEngine();
				accEng.setValidaInsuficienciaDeSaldo(false);
				ps = conn.prepareStatement("select a.nFolioPrecomMateriales from tPrecomMaterialesEncabezado a with(nolock) inner join mConsolidado b with(nolock) on a.cIdConsolidado=b.cIdConsolidado "+
											" and a.nFolioPrecomMateriales=b.ConsecutivoPRECOMP "+
											" where a.cDocumentoHaplicado='S' and b.cIdConsolidado='"+cIdConsolidado+"'");
			...
			if (acr.isSuccess()) {
				...
				if(outputValue == 0){
					...
					pstm = conn.prepareStatement("UPDATE mProcedimiento SET  ConsecutivoPRECOMP = ?, C_FOLIO_PRE = ? " + 
						" WHERE cIdTipoProcedimiento = ? "+
					...
					log.debug("folio "+request.getParameter("folioCasoPreCompromiso"));
						...
						//////////////////////////////
						cmst=conn.prepareCall("{call pa_mGeneraPedidosyContratos (?,?,?,?,?,?)}");
						cmst.setString(1, param[0]);//ejercicio
						cmst.setString(2, param[1]);//tipo
						cmst.setString(3, param[2]);//unidad
						cmst.setInt(4,val);//consecutivo
						cmst.registerOutParameter(5, Types.INTEGER); 
						cmst.setInt(6,tipoProceso); //tipoProceso	
						cmst.execute();
	...
	private Caso getCaso(HttpSession session){
		...
		" FROM mProcedimiento p, CG_CASO c, CG_CASO_OPERACION o "+
		" where p.C_FOLIO_PRE=c.C_FOLIO"+
		" and c.ID_CASO=o.ID_CASO"+
		" and p.cEjercicio=?"+
		" and p.cIdTipoProcedimiento=?"+
		" and p.cIdUnidadEjecutora=?"+
		" and p.nIdConsecutivo=?";
		Caso c=null;
		try{
			conn = DataSourceManager.getConnection(jndiName);
			...
			if (acr.isSuccess()) {
				...
				log.debug("folio "+ request.getParameter("nFolioPrecompromiso"));
					...
					if(outputValue == 0){
						...
						jsonObj.put("Aplica", "1");
						pstm = conn.prepareStatement("UPDATE mProcedimiento SET nIdEstado = 1 , ConsecutivoPRECOMP = null, C_FOLIO_PRE = null " + 
							" WHERE cIdTipoProcedimiento = ? "+
							" and cIdUnidadEjecutora = ?"+
							" and nIdConsecutivo = ?"+
							" and cEjercicio=?");
						...
						if(param[6].compareToIgnoreCase("0")!=0){
							...
							pstm2 = conn.prepareStatement("UPDATE mConsolidado SET  ConsecutivoPRECOMP = ?, C_FOLIO_PRE = ? " + 
								" WHERE cIdConsolidado=?");
						...
						pstm1 = conn1.prepareStatement("UPDATE mProcedimiento SET nIdEstado = 2 " + 
							" WHERE cIdTipoProcedimiento = ? "+
							" and cIdUnidadEjecutora = ?"+
							" and nIdConsecutivo = ?"+
							" and cEjercicio=?");
	...
	private synchronized void creaguardaProcedimiento(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
		...
		try {
			try {
				...
				String query="select cIdProcedimiento,(select U_NOMBRE from cg_usuario where U_LOGIN=cIdUsuarioCreacion) nombreUsuario "+ 
							"from mProcedimiento with(Nolock) where nIdEstado in(1,2) and cIdConsolidado='"+cIdConsolidado+"'";
				log.info("query que valida si el consolidado ya se encuentra en un procedimiento : "+query);
				ps = conn.prepareStatement(query);
				rs = ps.executeQuery();
				if(rs.next()){
					mensaje="El consolidado "+cIdConsolidado+" ya se encuentra en el procedimiento "
						+rs.getString("cIdProcedimiento")+".\nEl usuario creador fue "+rs.getString("nombreUsuario");
				...
				String descripcion=new String(request.getParameter("descripcion").getBytes("ISO-8859-1"),"UTF-8");//request.getParameter("descripcion");
				...
				if("PA".equalsIgnoreCase(cIdTipoProcedimiento) || "PL".equalsIgnoreCase(cIdTipoProcedimiento)){
					...
					cmst.setString(16, cboUnidadEjecutoraInmueble);//cboUnidadEjecutoraInmueble
					cmst.setInt(17, cboDenominacionInmueble);//cboDenominacionInmueble
					cmst.setString(18, descripcionOtros);//descripcionOtros
					cmst.setFloat(19, area_construida);//area_construida
					cmst.setFloat(20, area_rentable);//area_rentable
					cmst.setFloat(21, numero_empleados);//numero_empleados
					cmst.setString(22, direccion_inmueble);//direccion_inmueble
				...
				if("PA".equalsIgnoreCase(cIdTipoProcedimiento) || "PL".equalsIgnoreCase(cIdTipoProcedimiento)){
					...
					String query3="insert into mRequisitoProcedimiento (cIdRequisito, cIdProcedimiento, cRequerido, cCumple, cObservaciones) "+
					"select r.cIdRequisito, p.cIdProcedimiento, r.cRequerido, '' as cCumple, '' as cObservacones from mProcedimiento as p WITH (NOLOCK),"+
					" mCatalogoRequisitosProcedimiento as r WITH (NOLOCK) where p.cIdProcedimiento ='"+cIdProcedimiento
					+"' and (p.cIdTipoProcedimiento = 'PS' or p.cIdTipoProcedimiento = 'PN')";
	...
	private void cancelaPrecomConsolidado(HttpSession session, HttpServletRequest request,HttpServletResponse response,String cIdConsolidado) throws IOException{
		...
		try{
			...
			AccountingEngine accEng = new AccountingEngine();
			accEng.setValidaInsuficienciaDeSaldo(false);
			ps = conn.prepareStatement("select a.nFolioPrecomMateriales from tPrecomMaterialesEncabezado a inner join mConsolidado b on a.cIdConsolidado=b.cIdConsolidado "+
										" and a.nFolioPrecomMateriales=b.ConsecutivoPRECOMP "+
										" where a.cDocumentoHaplicado='s' and b.cIdConsolidado='"+cIdConsolidado+"'");
