...
public class BaseSAC extends HttpServlet {
	...
	private static Logger log = Logger.getLogger(BaseSAC.class);
		...
		try {
			...
			switch(tipoOperacion){
				...
				case 2://Actualiza datos
					...
					break;
				case 3: //Consulta Procesos de Contrataci\u00f3n
					...
					respuesta=true;
					break;
				case 4://Se declara desierto el procedimiento
					datos=llenaDatosProcedSAC( request, usuario );
	...
	protected void nuevoProcedimientoSAC(HttpServletRequest request, HttpServletResponse response, Usuario usuario) throws UnsupportedEncodingException {
		...
		try {
			...
			out = response.getWriter();
			switch(tipoOperacion){
				case 1: //obtiene datos de los selects Nuevo Procedimiento
	...
	private DatosProcedSAC llenaDatosProcedSAC(HttpServletRequest request, Usuario usuario)throws Exception{
		...
		datos.setnTipoProcedimiento(request.getParameter("nTipoProcedimiento")==null 
				|| "".equalsIgnoreCase(request.getParameter("nTipoProcedimiento"))
				|| "null".equalsIgnoreCase(request.getParameter("nTipoProcedimiento"))?-1:Integer.parseInt(request.getParameter("nTipoProcedimiento"))  );
		datos.setnProcesoContratacion(request.getParameter("nProcesoContratacion")==null 
				|| "".equalsIgnoreCase(request.getParameter("nProcesoContratacion"))
				|| "null".equalsIgnoreCase(request.getParameter("nProcesoContratacion"))?0:Integer.parseInt(request.getParameter("nProcesoContratacion"))  );
