    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            for (DatosConexion dc : datosConexion) {
                try {
                    conn = DriverManager.getConnection("jdbc:sqlserver://" + dc.getDireccionServer() + ";databaseName=" + dc.getNombreBD() + ";user=" + dc.getUserBD() + ";password=" + dc.getPassBD() + ";");
                    stm = conn.createStatement();
                    rs = stm.executeQuery("select (case when total=0 then 0 else total/meses end) as pu from " + "(select COUNT(*) as meses, (case when SUM(mPrecioUnitario) is null then 0 else SUM(mPrecioUnitario) end) as total " + "from mProgramaAnualDetallePeriodo with(nolock)" + "where cIdCABM='" + param[0] + "' and cIdUnidadEjecutora='" + param[1] + "' AND mPrecioUnitario>0) as tab");
                    while (rs.next()) precioUnitarioPromedio = rs.getBigDecimal(1);
                    try {
                        conn.close();
                        stm.close();
                        rs.close();
