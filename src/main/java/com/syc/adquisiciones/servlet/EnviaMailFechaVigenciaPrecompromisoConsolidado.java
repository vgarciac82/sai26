...
public class EnviaMailFechaVigenciaPrecompromisoConsolidado extends HttpServlet implements Runnable {
                        ...
                        if ("false".equalsIgnoreCase(hiloactivo)) {
                            ...
                            for (int i = 0; i < listCorreoPrecompromisoLiberado.size(); i++) {
                                ...
                                if (res == "CANCELADO CONTABLEMENTE") {
                                    enviarMail(listCorreoPrecompromisoLiberado.get(i).getSubject(), listCorreoPrecompromisoLiberado.get(i).getBody(), listCorreoPrecompromisoLiberado.get(i).getTo(), listCorreoPrecompromisoLiberado.get(i).getFolioPrecom(), listCorreoPrecompromisoLiberado.get(i).getConsecutivoPrecom());
                                } else {
                                    enviarMail(listCorreoPrecompromisoLiberado.get(i).getSubject(), "NO SE PUDO LIBERAR EL PRECOMPROMISO  DEL CONSOLIDADO " + listPRECOMLiberarPrecompromiso.get(i).toString() + " AVISE AL ADMINISTRADOR", listCorreoPrecompromisoLiberado.get(i).getTo(), listCorreoPrecompromisoLiberado.get(i).getFolioPrecom(), listCorreoPrecompromisoLiberado.get(i).getConsecutivoPrecom());
                                }
    ...
    private void insertaCorreo(String subject, String to, int status, String c_folio_pre, int consecutivoPrecom, String error) {
        ...
        try {
            con = DataSourceManager.getConnection("jdbc/gestion");
            ps = con.prepareStatement("insert into mCorreoPrecompromisoVigencia (cEjercicio,cIdConsolidado,C_FOLIO_PRE,ConsecutivoPRECOMP,cIdUsuarioCreacion,fEnvio,fRecepcion,estatusCorreo,error,cIdUnidadEjecutora)values((select cvalor from mSistema with(nolock) where cParametro='cEjercicio'),?,?,?,?,GETDATE(),GETDATE(),?,?,?)");
    ...
    private synchronized void consultaFechasVencidasPrecompromiso() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (Integer.parseInt(rs.getString("diasVencer")) < 0) {
                    listCorreoPrecompromisoLiberado.add(new CCorreo(rs.getString("u_email"), rs.getString("cIdConsolidado"), "El Precompromiso del Consolidado " + rs.getString("cIdConsolidado") + " se liberó automaticamente ya que la fecha de vigencia expiró.", rs.getString("C_FOLIO_PRE"), rs.getInt("ConsecutivoPRECOMP")));
