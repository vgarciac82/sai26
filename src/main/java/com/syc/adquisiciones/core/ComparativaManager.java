...
public class ComparativaManager {
    ...
    public static List callComparativa(Connection conn, String cEjercicio, String cIdTipoProcedimiento, String cIdUnidadEjecutora, int nIdConsecutivo, int Linea, String orden) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                rComparativa.setcFirmante(rs.getString("estadoPartida"));
    ...
    public static List callLista(Connection conn, String cEjercicio, String cIdTipoProcedimiento, String cIdUnidadEjecutora, int nIdConsecutivo) throws SQLException {
        ...
        String Salida = "";
        try {
            pstmnt1 = conn.prepareStatement("select distinct nidLineaConsolidado from mProcedimientoCompleto pc WITH(NOLOCK) where" + "  cEjercicio=? and cIdTipoProcedimiento=?  and cIdUnidadEjecutora=? and nIdConsecutivo=?");
    ...
    public static List callProveedor(Connection conn, String cEjercicio, String cIdTipoProcedimiento, String cIdUnidadEjecutora, int nIdConsecutivo) throws SQLException {
        ...
        String Salida = "";
        try {
            pstmnt1 = conn.prepareStatement(" select distinct pc.cIdRFC, (select cp.cRazonSocial from mCatalogoProveedor cp WITH(NOLOCK) where " + " cp.cIdRFC=pc.cIdRFC) as razonSocial,fecha, cestadoProveedor from mProcedimientoCompleto pc WITH(NOLOCK) " + " inner join mProcedimientoFechas pf WITH(NOLOCK) on pc.cIdProcedimiento=pf.nIdProcedimiento inner " + " join mCatalogoFechas cf WITH(NOLOCK) on cf.nIdFecha=pf.nIdFecha inner join mProcedimientoAdjudicacion pa on pc.cIdRFC=pa.cIdRFC and pc.cIdProcedimiento=pa.cIdProcedimiento" + " and pa.nIdconsecutivoAdj=pc.nIdconsecutivoAdj where pc.cEjercicio=? and pc.cIdTipoProcedimiento=? and  pc.cIdUnidadEjecutora=? and pc.nIdConsecutivo=? and pf.nIdFecha=8 and cEstadoProveedor=1");
            pstmnt1.setString(1, cEjercicio);
            pstmnt1.setString(2, cIdTipoProcedimiento);
            pstmnt1.setString(3, cIdUnidadEjecutora);
            pstmnt1.setInt(4, nIdConsecutivo);
    ...
    public static List callFirmantes(Connection conn, String cIdTipoProcedimiento, String cIdUnidadEjecutora, int nIdConsecutivo) throws SQLException {
        ...
        try {
            pstmnt1 = conn.prepareStatement(" select cNombre+' '+cPaterno+' '+cMaterno as Nombre from mFirmantesComparativa fc WITH(NOLOCK) inner join mCatalogoFirmantes f WITH(NOLOCK) on" + " fc.cIdUnidadEjecutora=f.cIdUnidadEjecutora and fc.nIdFirmante=f.nIdFirmante where fc.cIdProcedimiento=?");
    ...
    public static List callProcedimiento(Connection conn, String cEjercicio, String cIdTipoProcedimiento, String cIdUnidadEjecutora, int nIdConsecutivo, int nIdFecha) throws SQLException {
        ...
        String idProcedimiento = cIdTipoProcedimiento + '-' + cIdUnidadEjecutora + '-' + nIdConsecutivo;
        try {
            pstmnt1 = conn.prepareStatement(" select cCategoria,cidProcedimiento from mProcedimiento p WITH(NOLOCK) inner join " + "mCatalogoCategoriaProcedimiento c WITH(NOLOCK) on p.nIdCategoria=c.nIdCategoria where cIdProcedimiento=? ");
            pstmnt1.setString(1, idProcedimiento);
            rs1 = pstmnt1.executeQuery();
            ProcedimientoComparativa rFirmantes;
            while (rs1.next()) {
    ...
    public static List callProveedorOrdenado(Connection conn, String cEjercicio, String cIdTipoProcedimiento, String cIdUnidadEjecutora, int nIdConsecutivo, int Linea, int query) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                /// monto minimoBruto
                rProveedor.setmontoMaximo(rs.getFloat("mMontoTotal"));
                rProveedor.setcTipoCambio(rs.getFloat("tipoCambio"));
                /// monto total con el tipo de cambio
                rProveedor.setmontoMaximoBruto(rs.getFloat("mMontoTotalBruto"));
                /// partida desierta
                rProveedor.setNumRfc(rs.getInt("agregaProveedor"));
                lineaProveedorList.add(rProveedor);
            }
            //System.out.println("Salida: "+Salida);
        } catch (Exception e) {
    ...
    public static List callPartidaOrdenado(Connection conn, String cEjercicio, String cIdTipoProcedimiento, String cIdUnidadEjecutora, int nIdConsecutivo, int Linea) throws SQLException {
        ...
        String Salida = "";
        try {
            pstmnt1 = conn.prepareStatement("select cl.nidlineaConsolidado, " + " '<td rowspan=''3'' style=''border-right:1.0pt solid black;border-bottom:1.0pt solid black;''>'+cl.cDescripcion+'</td>' as descripcion," + " '<td colspan=''3'' style=''border-right:1.0pt solid black;border-bottom:1.0pt solid black;''>'+cl.cDescripcion+'</td>' as descripcionObs from mConsolidadoLineas cl " + " INNER JOIN mProcedimiento p ON cl.cEjercicio = p.cEjercicio AND cl.cIdTipoConsolidado = p.cIdTipoConsolidado " + " AND cl.cIdUnidadEjecutora = p.cIdUnidadEjecutora AND cl.nIdConsecutivo = p.nIdConsecutivoConsolidado" + " inner join mProcedimientoCompleto pc ON p.cEjercicio = pc.cEjercicio" + " where  p.cEjercicio=? and p.cIdTipoProcedimiento=?  and p.cIdUnidadEjecutora=? and p.nIdConsecutivo=? and cl.nidlineaConsolidado=? " + " AND p.cIdTipoProcedimiento = pc.cIdTipoProcedimiento  AND p.cIdUnidadEjecutora = pc.cIdUnidadEjecutora " + " AND p.nIdConsecutivo = pc.nIdConsecutivo AND cl.nIdLineaConsolidado = pc.nIdLineaConsolidado  group by cl.nidlineaConsolidado, cl.cDescripcion");
            pstmnt1.setString(1, cEjercicio);
            pstmnt1.setString(2, cIdTipoProcedimiento);
            pstmnt1.setString(3, cIdUnidadEjecutora);
            pstmnt1.setInt(4, nIdConsecutivo);
    ...
    public static int ordena(Connection conn, String cEjercicio, String cIdTipoProcedimiento, String cIdUnidadEjecutora, int nIdConsecutivo, int Linea, String rfc, int posicion) throws SQLException {
        int val = -1;
        PreparedStatement pstmnt = null;
        //String rfc="";
        //	String aux="";
        ...
        try {
            ...
            pstmnt = conn.prepareStatement("update mprocedimientoCompleto set ganadorSug=? where cEjercicio=? and cIdTipoProcedimiento=? " + " and cIdUnidadEjecutora=? and nIdConsecutivo=? and nIdLineaConsolidado=? AND cIdRFC=?");
    ...
    public static int ordenaPrecio(Connection conn, String cEjercicio, String cIdTipoProcedimiento, String cIdUnidadEjecutora, int nIdConsecutivo, int Linea, String rfc, int posicion) throws SQLException {
        int val = -1;
        PreparedStatement pstmnt = null;
        //String rfc="";
        //String aux="";
        ...
        try {
            ...
            //	aux=posicion.get(i).toString();
            conn.setAutoCommit(false);
            pstmnt = conn.prepareStatement("update mprocedimientoCompleto set lugarPrecio=? where cEjercicio=? and cIdTipoProcedimiento=? " + " and cIdUnidadEjecutora=? and nIdConsecutivo=? and nIdLineaConsolidado=? AND cIdRFC=?");
            pstmnt.setInt(1, posicion);
            pstmnt.setString(2, cEjercicio);
            pstmnt.setString(3, cIdTipoProcedimiento);
            pstmnt.setString(4, cIdUnidadEjecutora);
