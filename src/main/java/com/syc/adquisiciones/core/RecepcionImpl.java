...
public class RecepcionImpl implements RecepcionInterfacce{
	private static Logger log = Logger.getLogger(RecepcionImpl.class);
	...
	private boolean enviarRecepcionMaterial(Connection conn, Usuario usuario, DatosRecepcion datosRecepcion) throws SQLException{
		...
		try{
			if(!datosRecepcion.isFactorAmortizacion()){//Sin factor de amortización
				hayAnticipo=hayAnticipos(conn, datosRecepcion);
				if(hayAnticipo){
					//Obtener montos de anticipo
					String queryAnticipos="select sum(isnull(rma.mMontoAnticipoConIVA,0.00)) mMontoAnticipoConIVA,sum(isnull(rma.mMontoAnticipoSinIVA,0.00))mMontoAnticipoSinIVA,sum(isnull(rma.mMontoAnticipoIVA,0.00))mMontoAnticipoIVA "
					+" from mRecepcionpMat as rm with(Nolock) inner join mRecepcionpMatAnticipo as rma with(Nolock) on rm.cIdRecepMat=rma.cIdRecepMat and rm.cIdpedContDef=rma.cIdpedContDef "
					+" and rm.nIdConsecutivoRecepM=rma.nIdConsecutivoRecepM	where rm.cIdpedContDef='"+datosRecepcion.getcIdPedContDef()+"' and rm.cIdRecepMat like 'RA-'+SUBSTRING('"
					+datosRecepcion.getcIdRecepcionMat()+"',4,3)+'%' and nIdEstadoRecepMat<>4";
				...
				if(datosRecepcion.getcIdPedContDef().indexOf("PLU")!=-1){
					String queryAnticiposEjerAnt="select sum(isnull(rmaEjeAnt.mMontoAnticipoConIVA,0.00)) mMontoAnticipoConIVA "+
					",sum(isnull(rmaEjeAnt.mMontoAnticipoSinIVA,0.00))mMontoAnticipoSinIVA,sum(isnull(rmaEjeAnt.mMontoAnticipoIVA,0.00))mMontoAnticipoIVA " +
					"From mRecepcionpMatAnticipoEjerciciosAnt rmaEjeAnt with(Nolock) where cIdpedContDef='"+datosRecepcion.getcIdPedContDef()+"'";
				...
				if(hayAnticipo || datosRecepcion.getcIdPedContDef().indexOf("PLU")!=-1){
					String queryRecepAmortizadas="select sum(rml.mMontoConIVARML)-sum(rm.mMontoConIVARM) mMontoConIVA,sum(rml.mMontoSinIVARML)-sum(rm.mMontoSinIVARM) mMontoSinIVA "
						+",sum(rml.mMontoIVARML)-sum(rm.mMontoIVARM) mMontoIVA from(select sum(isnull(rm.mMontoConIVA,0.00))mMontoConIVARM ,sum(isnull(rm.mMontoSinIVA,0.00)) mMontoSinIVARM "
						+",sum(isnull(rm.mMontoIVA,0.00)) mMontoIVARM ,rm.cIdpedContDef,rm.nIdConsecutivoRecepM,rm.cIdRecepMat,nIdEstadoRecepMat "
						+" from mRecepcionpMat as rm with(Nolock) group by  rm.cIdpedContDef,rm.nIdConsecutivoRecepM,rm.cIdRecepMat,nIdEstadoRecepMat "
						+" )rm inner join (select sum(isnull(rml.mMontoConIVA-isnull(mDescuentoConIVA,0.00),0.00))mMontoConIVARML ,sum(isnull(rml.mMontoSinIVA-isnull(mDescuentoSinIVA,0.00),0.00))mMontoSinIVARML"
						+"	,sum(isnull(rml.mMontoIVA-isnull(mDescuentoIVA,0.00),0.00))mMontoIVARML,rml.cIdpedContDef,cIdRecepMat,rml.nIdConsecutivoRecepM "
						+"	from mRecepcionpMatLineas as rml with(Nolock) group by rml.cIdpedContDef,cIdRecepMat,rml.nIdConsecutivoRecepM "
						+" )rml on rm.cIdRecepMat=rml.cIdRecepMat and rm.cIdpedContDef=rml.cIdpedContDef and rm.nIdConsecutivoRecepM=rml.nIdConsecutivoRecepM " +
						" and rm.nIdConsecutivoRecepM=rml.nIdConsecutivoRecepM where rm.cIdpedContDef='"+datosRecepcion.getcIdPedContDef()+"' and nIdEstadoRecepMat in(2,3,5)";
			...
			}else{//Con factor de amortización
			...
			String queryRecep="select isnull(mMontoConIVA,0.00) ,isnull(mMontoSinIVA,0.00),isnull(mMontoIVA,0.00) " 
						+" from mRecepcionpMat with(Nolock) where cIdpedContDef='"+datosRecepcion.getcIdPedContDef()+"' and cIdRecepMat='"+datosRecepcion.getcIdRecepcionMat()+"'";
	...
	private boolean hayAnticipos(Connection conn,DatosRecepcion datosRecepcion) throws SQLException{
		...
		boolean resp=false;
		try{
			String query="select *from mRecepcionpMat with(Nolock) where cIdpedContDef='"+datosRecepcion.getcIdPedContDef()+"' and cIdRecepMat like 'RA-'+SUBSTRING('"+datosRecepcion.getcIdRecepcionMat()+"',4,3)+'%'"
				+" and nIdEstadoRecepMat<>4";
			log.info(query);
	...
	private double[] obtieneMontos(Connection conn, String query) throws SQLException{
		...
		try{
			...
			rs = pstmt.executeQuery();
			if(rs.next()){
				montos[0]=rs.getDouble(1);//monto con iva
				montos[1]=rs.getDouble(2);//monto sin iva
				montos[2]=rs.getDouble(3);//monto iva
			}
		}finally{
			if(rs!=null){
				rs.close();
	...
	private double[] validaMontos(double montoAnticipo,double montoRecepMat){
		...
		if(montoAnticipo<montoRecepMat){
			montos[0]=0;//montoRemanenteAnt
			montos[1]=montoRecepMat-montoAnticipo;//montoRemanenteRM
		}else{
			montos[0]=montoAnticipo-montoRecepMat;//montoRemanenteAnt
			montos[1]=0;//montoRemanenteRM
		}
		return montos;
	}
	private boolean actualizaAnticipoAndRM(Connection conn, DatosRecepcion datosRecepcion,double montoRemanenteAnt[],double montoRemanenteRM[],int nEstatus) throws SQLException{
		...
		try{
			//Actualiza Anticipo
			String queryAnticipo="update mRecepcionpMat set mMontoConIVA="+montoRemanenteAnt[0]+",mMontoSinIVA="+montoRemanenteAnt[1]
					+",mMontoIVA="+montoRemanenteAnt[2]+" where cIdpedContDef='"+datosRecepcion.getcIdPedContDef()
					+"' and cIdRecepMat='"+datosRecepcion.getcIdRecepAnticipo()+"' and nIdEstadoRecepMat<>4";
			log.info(queryAnticipo);
			pstmAnt = conn.prepareStatement(queryAnticipo);
			pstmAnt.executeUpdate();
			//Actualiza Recepción de Material
			String queryRM="update mRecepcionpMat set mMontoConIVA="+montoRemanenteRM[0]+",mMontoSinIVA="+montoRemanenteRM[1]
				+",mMontoIVA="+montoRemanenteRM[2]+",nIdEstadoRecepMat="+nEstatus+" where cIdpedContDef='"+datosRecepcion.getcIdPedContDef()
				+"' and cIdRecepMat='"+datosRecepcion.getcIdRecepcionMat()+"'";
			log.info(queryRM);
