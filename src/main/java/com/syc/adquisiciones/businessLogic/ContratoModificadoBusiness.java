...
public class ContratoModificadoBusiness extends DataSourceManager {
	private static Logger log = Logger.getLogger(ContratoModificadoBusiness.class);
		...
		try{
			...
			if(resp){
				while (itr.hasNext()) {
					...
					if(manager.existePartidaContratoMod( conn, cIdcontratoDefinitivo, nConsecutivoMod, Integer.parseInt( fila.get(5))) ){//actualizar
		...
		try {
			...
			while(i<registros.size()){
				...
				if(c==null){
					...
					//Avanza el caso
					cbl.avanzaCaso(conn, c, usuario.getLogin(), "", new String[] { "CONSULTA_PAGOS" },
							new String[] { "consulta_compromiso" }, m, null);
