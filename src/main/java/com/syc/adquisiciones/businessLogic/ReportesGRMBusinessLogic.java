...
public class ReportesGRMBusinessLogic extends DataSourceManager {
	...
	private static Logger log = Logger.getLogger( ReportesGRMBusinessLogic.class );
	...
	public File generaReporteIndicadoresCNET( DatosPedidoContrato datos ) throws Exception {
		...
		try {
			...
			boolean esSAIAlterno = "true".equalsIgnoreCase( configApp.getSystemSetting( "SAI_AMBIENTAL" ) ) 
					|| "true".equalsIgnoreCase( configApp.getSystemSetting( "SAI_FONDEN" ) ) 
					|| "S".equalsIgnoreCase( configApp.getSystemSetting( "AMBIENTE_LOCAL" ))
					|| "true".equalsIgnoreCase( configApp.getSystemSetting( "AMBIENTE_DESARROLLO" ));
			String cEjercicioActivo = com.syc.adquisiciones.util.Util.obtieneEjercicioFiscalActivo( conn );
			cFileExcelPlantilla = new File( datos.getcNamePlantilla() );
			fs = new FileInputStream( cFileExcelPlantilla );
			Util.copiaArchivo( fs, file_name );
			...
			if ( !esSAIAlterno ) {
				...
				if(Integer.parseInt( cEjercicioActivo ) <2022) {//De 2022 en adelante se quito FONDEN
	...
	public File generaReportePSP( DatosPedidoContrato datos ) throws Exception {
		...
		try {
			...
			boolean esSAIAlterno = "true".equalsIgnoreCase(configApp.getSystemSetting("AMBIENTE_DESARROLLO"))
					||"true".equalsIgnoreCase(configApp.getSystemSetting("SAI_AMBIENTAL"))
					|| "true".equalsIgnoreCase(configApp.getSystemSetting("SAI_FONDEN"));
			if(esSAIDesarrollo) {
				desa="_desa";
			}
