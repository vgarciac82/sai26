...
public class SolicitudBusinessLogic extends DataSourceManager {
	private static Logger log = Logger.getLogger(SolicitudBusinessLogic.class);
	...
	public String devuelveApartado (HttpServletRequest req,DatosRequisicion datosRequi, HttpSession session, Usuario usuario) throws Exception{
		...
		try {
			...
			if(requisition.getIdEstado()==RequisitionStatus.REQUESTED) {//Si es estatus es 2
			...
			}else if(requisition.getIdEstado()==RequisitionStatus.REQUESTED || isDebuger) {//Si es estatus es 3
	...
	private void validacionesDevolverApartado(Connection conn,SolicitudManager manager,Requisition requisition)throws Exception {
		if(requisition.getIdEstado()==RequisitionStatus.CAPTURED) {
			...
			if(RequisitionType.DE_TIENDA_DIGITAL.equalsIgnoreCase( requisition.getIdTipoSolicitud() )) {//Validar si es una requi de tienda digital
			...
			}else if(RequisitionType.DE_MODIFICACION.equalsIgnoreCase( requisition.getIdTipoSolicitud() )){//Validar si es una requi de modificacion
