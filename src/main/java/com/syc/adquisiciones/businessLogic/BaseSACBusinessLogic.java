...
public class BaseSACBusinessLogic extends DataSourceManager{
	public static Logger log = Logger.getLogger(BaseSACBusinessLogic.class);
	...
	public JSONObject consultaSelectsProcedimiento(String cCoord,int nTipoProced) throws Exception {
		...
		try {
			...
			if(nTipoProced>0) {
				query="select convert(varchar,ROW_NUMBER() OVER (ORDER BY cat.nIdProcesoContratacion)-1)+'.- '+cProcesoContratacion as proceso,cat.nIdProcesoContratacion from mCatalogoProcesosContratacionSAC as cat with(Nolock) "
						+"inner join mRelacionCatProcedimiento_ProcesoContratacion as rel with(Nolock) on rel.nIdProcesoContratacion=cat.nIdProcesoContratacion "
						+"where rel.nIdCategoria="+nTipoProced+" order by cat.nIdProcesoContratacion";
			}
			arrayObj=manager.obtieneDatQuery(conn, query);
	...
	public JSONObject queryProcedimiento(DatosProcedSAC datos) throws Exception {
		...
		try {
			...
			//obtener los selects
			jsonObj=consultaSelectsProcedimiento((arrayObj.getJSONObject(0).getBoolean("HAYINFO"))?arrayObj.getJSONObject(0).getString("cAreaReq"):"0"
				,(arrayObj.getJSONObject(0).getBoolean("HAYINFO"))?arrayObj.getJSONObject(0).getInt( "nTipoProcedimiento" ):0);
