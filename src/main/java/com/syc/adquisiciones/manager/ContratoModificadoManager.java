...
public class ContratoModificadoManager {
	private static Logger log = Logger.getLogger(ContratoModificadoManager.class);
	...
	public boolean addPartidasContrato(Connection conn,ContratoModificado contMod,String lineaConsolidado)throws SQLException{
		...
		try{
			...
			query.append( "insert into mContratoModificadoPartida (cEjercicio,cIdContrato,cIdContratoDefinitivo,nConsecutivoModificacion"
					+ ",cIdUnidadEjecutora,cIdTipoConsolidado,cIdConsecutivoConsolidado,nIdLineaConsolidado,cDescripcion,nCantidad,mprecioUnitario,cIdSolicitud,cIdLineaSolicitud,mMontoNeto,cNewUE,mTotalAnterior,cIdunidadEjecutoraLinea) "
					+ " select "
					+ " '"+contMod.getcEjercicio()+"' as ejrcicioFiscal ,'"+contMod.getcIdContrato()+"' as cIdcontrato"
					+ ",'"+contMod.getcIdContratoDefinitivo()+"' as cIdContratoDefinitivo,"+contMod.getnConsecutivoModificacion()+" as nConsecutivoModificacion "
					+ ",cIdUnidadEjecutora,cIdTipoConsolidado,nIdConsecutivoConsolidado,nidlineaconsolidado "
	...
	public boolean deletePartidaContMod(Connection conn,ContratoModificado contMod,int lineaConsolidado)throws SQLException{
		...
		try{
			query=new StringBuilder();
			query.append( "DELETE FROM mContratoModificadoPartida "
					+ "WHERE cIdContratoDefinitivo = ? "
					+ "and nConsecutivoModificacion = ? "
					+ "and nIdLineaConsolidado = ?" );
			pstm=conn.prepareStatement(	query.toString());
			pstm.setString(1, contMod.getcIdContratoDefinitivo());
			pstm.setInt( 2, contMod.getnConsecutivoModificacion() );
	...
	public boolean updatePartidaContMod(Connection conn,ContratoModificado contMod,List<String> fila)throws SQLException{
		...
		try{
			...
			if("CV".equalsIgnoreCase( contMod.getcIdContrato().substring( 0,2 ))) {//Servicios
