...
public class BaseSACManager {
	private static Logger log = Logger.getLogger(BaseSACManager.class);
	...
	public boolean updateProcedSAC(Connection conn, DatosProcedSAC datos) throws Exception {
		...
		try {
			query="update mProcedimientoSAC \r\n" + 
					...
					",nEstatus=2" +
					",cObservaciones='"+datos.getcObservaciones() +"'"+
					"where nIdProcedimientoSAC="+datos.getnIdProcedimientoSAC();
			log.info(query);
			ps = conn.prepareStatement(query);
			success = ps.executeUpdate() > 0;
		} finally{
	...
	public boolean updateEstatusProcedSAC(Connection conn, DatosProcedSAC datos) throws Exception {
		...
		try {
			query="update mProcedimientoSAC \r\n" + 
					"set nEstatus=" +datos.getnEstatus()+
					" where nIdProcedimientoSAC="+datos.getnIdProcedimientoSAC();
			log.info(query);
			ps = conn.prepareStatement(query);
			success = ps.executeUpdate() > 0;
