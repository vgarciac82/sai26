...
public class LeeArchivosManager {
	private final Logger	log	= Logger.getLogger(LeeArchivosManager.class);
	...
	public boolean insertEFOSTMP(Connection conn,JSONObject jsonObj) throws SQLException, JSONException{
		...
		String query=null;
		try{
			query="insert into mHistoriaEFOSSAT (num 	,cIdRFC 	,cRazonSocial  	,cSituacion 	,cOficioPresuncion 	,fPublicacionSATPresunto 	,cOficioPresuncionGobal "
				+",fPublicacionDOFPresunto 	,cOficioDesvirtuado,fPublicacionSATDesvirtuado, cOficioGlobalDesvirtuado	,fPublicacionDOFDesvirtuado 	,cOficioDeFinitivo 	"
				+ ",fPublicacionSATDefinitivo,cOficioGlobalDOFDeFinitivo 	"
				+",fPublicacionDOFDefinitivo 	,cOficioSentFaovorable 	,fPublicacionSATSentFaovorable 	,cOficioSentFaovorableGlobal 	,fPublicacionDOFSentFaovorable )"
	...
	public int insertEFOS(Connection conn) throws SQLException{
		...
		String query=null;
		try{
			query="insert into mCatalogoEFOS  (cIdRFC,cRazonSocial,nSituacion,cOficioPresuncion,fPublicacionSATPresunto, " + 
					"fPublicacionDOFPresunto,fPublicacionSATDesvirtuado,cOficioDesvirtuado, " + 
					"fPublicacionDOFDesvirtuado,cOficioDeFinitivo,fPublicacionSATDefinitivo, " + 
					"fPublicacionDOFDefinitivo,cOficioSentFaovorable,fPublicacionSATSentFaovorable, " + 
					"fPublicacionDOFSentFaovorable,fFechaCarga,fFechaActualiza,cOficioGlobalDesvirtuado,cOficioGlobalDOFDeFinitivo) "
					+"select cat.cIdRFC ,cat.cRazonSocial,sit.cSituacion,cat.cOficioPresuncion,cat.fPublicacionSATPresunto,cat.fPublicacionDOFPresunto,cat.fPublicacionSATDesvirtuado "
					+",cat.cOficioDesvirtuado,cat.fPublicacionDOFDesvirtuado,cat.cOficioDeFinitivo,cat.fPublicacionSATDefinitivo,cat.fPublicacionDOFDefinitivo,cat.cOficioSentFaovorable "
					+",cat.fPublicacionSATSentFaovorable,cat.fPublicacionDOFSentFaovorable,getdate(),null,cOficioGlobalDesvirtuado,cOficioGlobalDOFDeFinitivo from mHistoriaEFOSSAT cat with(nolock) "
					+"inner join( select 	cIdRFC	,MAX(fPublicacionSATPresunto) fPublicacionSATPresunto 	from mHistoriaEFOSSAT with(nolock) 	group by cIdRFC )sinRep "
					+"on cat.cIdRFC=sinRep.cIdRFC and sinRep.fPublicacionSATPresunto=cat.fPublicacionSATPresunto "
					+"inner join tArticulo69B_Situacion as sit with(Nolock) on sit.dSituacion=cat.cSituacion "
					+"where cat.cIdRFC not in(select cIdRFC from mCatalogoEFOS with(Nolock))";
			pstmnt = conn.prepareStatement(query);
	...
	public int updateEFOSExisting (Connection conn) throws SQLException{
		...
		String query=null;
		try{
			query="Update mCatalogoEFOS"
				+" set mCatalogoEFOS.cRazonSocial = temp.cRazonSocial"
				+" ,mCatalogoEFOS.nSituacion=temp.cSituacion"
				+" ,mCatalogoEFOS.cOficioPresuncion = temp.cOficioPresuncion"
				+" ,mCatalogoEFOS.fPublicacionSATPresunto = temp.fPublicacionSATPresunto"
				+" ,mCatalogoEFOS.fPublicacionDOFPresunto = temp.fPublicacionDOFPresunto"
				+" ,mCatalogoEFOS.fPublicacionSATDesvirtuado = temp.fPublicacionSATDesvirtuado"
				+" ,mCatalogoEFOS.cOficioDesvirtuado = temp.cOficioDesvirtuado"
				+" ,mCatalogoEFOS.fPublicacionDOFDesvirtuado = temp.fPublicacionDOFDesvirtuado"
				+" ,mCatalogoEFOS.cOficioDeFinitivo = temp.cOficioDeFinitivo"
				+" ,mCatalogoEFOS.fPublicacionSATDefinitivo = temp.fPublicacionSATDefinitivo"
				+" ,mCatalogoEFOS.fPublicacionDOFDefinitivo = temp.fPublicacionDOFDefinitivo"
				+" ,mCatalogoEFOS.cOficioSentFaovorable = temp.cOficioSentFaovorable"
				+" ,mCatalogoEFOS.fPublicacionSATSentFaovorable = temp.fPublicacionSATSentFaovorable"
				+" ,mCatalogoEFOS.fPublicacionDOFSentFaovorable = temp.fPublicacionDOFSentFaovorable"
				+" ,mCatalogoEFOS.fFechaActualiza=GETDATE() "
				+" ,mCatalogoEFOS.cOficioGlobalDesvirtuado = temp.cOficioGlobalDesvirtuado"
				+" ,mCatalogoEFOS.cOficioGlobalDOFDeFinitivo = temp.cOficioGlobalDOFDeFinitivo"
				+" from mCatalogoEFOS efos with(nolock),"
				+" (	select cat.cIdRFC ,cat.cRazonSocial,sit.cSituacion,cat.cOficioPresuncion,cat.fPublicacionSATPresunto,cat.fPublicacionDOFPresunto,cat.fPublicacionSATDesvirtuado"
				+"	,cat.cOficioDesvirtuado,cat.fPublicacionDOFDesvirtuado,cat.cOficioDeFinitivo,cat.fPublicacionSATDefinitivo,cat.fPublicacionDOFDefinitivo,cat.cOficioSentFaovorable"
				+"	,cat.fPublicacionSATSentFaovorable,cat.fPublicacionDOFSentFaovorable "
				+"  ,cat.cOficioGlobalDesvirtuado,cat.cOficioGlobalDOFDeFinitivo "
				+"  from mHistoriaEFOSSAT cat with(nolock) "
				+"	inner join( select 	cIdRFC	,MAX(fPublicacionSATPresunto) fPublicacionSATPresunto 	from mHistoriaEFOSSAT with(nolock) 	group by cIdRFC )sinRep "
				+"	on cat.cIdRFC=sinRep.cIdRFC and sinRep.fPublicacionSATPresunto=cat.fPublicacionSATPresunto"
				+"	inner join tArticulo69B_Situacion as sit with(Nolock) on sit.dSituacion=cat.cSituacion"
				+" )temp "
				+" where temp.cIdRFC=efos.cIdRFC";
			pstmnt = conn.prepareStatement(query);
			resp=pstmnt.executeUpdate();
		}finally {
			if(pstmnt!=null){
	...
	public void sendEmailLoadEFOS(Connection conn,File file){
		...
		try {
			queryEmails="select U_EMAIL from mListaUsuariosSendEmailEFOS list with(Nolock)"
				+" inner join CG_USUARIO as usuario with(nolock)"
				+" on usuario.U_LOGIN=list.cLogin";
			...
			body= "<B>Atenci\u00f3n.</b></br>" + "Se notifica que fue cargado el layout EFOS del SAT en el sistema SAI.<br><br>"
				+ "Por lo anterior es necesario que se verifique y se valide el archivo adjunto. <br><br>"
				+ "Gracias y reciban un cordial saludo.<br> ";
			try{
	...
	public void writeSheet1ProveedoresEFOS(Connection conn,XSSFSheet firstSheet,XSSFCellStyle estiloTabla) throws Exception{
		String query="select catEFOS.cIdRFC,catProv.cRazonSocial,sit.dSituacion estatus_SAT,case when catProv.lHabilitado=1 then 'HABILITADO' ELSE 'DESHABILITADO' END estatus_SAI"
			+" ,catEFOS.cOficioPresuncion,convert(varchar,catEFOS.fPublicacionSATPresunto,103)fPublicacionSATPresunto,convert(varchar,catEFOS.fPublicacionDOFPresunto,103)fPublicacionDOFPresunto"
			+" ,catEFOS.fPublicacionSATDesvirtuado,catEFOS.cOficioDesvirtuado,catEFOS.fPublicacionDOFDesvirtuado,catEFOS.cOficioDeFinitivo"
			+" ,catEFOS.fPublicacionSATDefinitivo,catEFOS.fPublicacionDOFDefinitivo,catEFOS.cOficioSentFaovorable,catEFOS.fPublicacionSATSentFaovorable"
			+" ,catEFOS.fPublicacionDOFSentFaovorable,catProv.cGiro,catProv.cRepresentanteLegal,catProv.cEmail from mCatalogoEFOS as catEFOS with(Nolock)"
			+" inner join mCatalogoProveedor as catProv with(Nolock)"
			+" on replace(catProv.cIdRFC,'-','')=catEFOS.cIdRFC"
			+" inner join tArticulo69B_Situacion as sit with(nolock)"
			+" on sit.cSituacion=catEFOS.nSituacion order by catProv.lHabilitado desc";
		PreparedStatement ps = null;
		ResultSet rst = null;
	...
	public void writeSheet2ProveedoresEFOS(Connection conn,XSSFSheet secondSheet,XSSFCellStyle estiloTabla) throws Exception{
		...
		try {
			...
			query="select cEjercicio,isnull(cont.cIdRFC,'')cIdRFC "
					+",isnull(cont.cIdContrato,'')cIdContrato "
					+",isnull(cont.cNoProcedimientoCNET,'')cNoProcedimientoCNET "
					+",isnull(cont.nCodContratoCNET,'')nCodContratoCNET "
					+",isnull(cont.nCodExpedienteCNET,'')nCodExpedienteCNET "
					+",isnull(cont.cConceptoContrato,'')cConceptoContrato "
					+"from mCatalogoEFOS as catEFOS with(Nolock)  "
					+"inner join mCatalogoProveedor as catProv with(Nolock)  "
					+"on replace(catProv.cIdRFC,'-','')=catEFOS.cIdRFC  "
					+"inner join tArticulo69B_Situacion as sit with(nolock)  "
					+"on sit.cSituacion=catEFOS.nSituacion "
					+"inner join( "+cadenaQuery+")cont on cont.cIdRFC=catEFOS.cIdRFC "
	...
	public boolean insertDoctoContrato(Connection conn,String cIdContratoDefinitivo,int nIdDocumento,String cLogin) throws SQLException{
		...
		String query=null;
		try{
			query="insert into mDocumentosContrato (cIdContratoDefinitivo,nIdDocumento,fFechaCarga,cUsuarioCarga) " + 
					"values('"+cIdContratoDefinitivo+"',"+nIdDocumento+",GETDATE(),'"+cLogin+"') ";
			pstmnt = conn.prepareStatement(query);
			resp=pstmnt.executeUpdate()>0;
		}finally {
			if(pstmnt!=null){
	...
	public boolean updateDoctoContrato(Connection conn,String cIdContratoDefinitivo,int nIdDocumento,String cLogin) throws SQLException{
		...
		String query=null;
		try{
			query="update mDocumentosContrato set fFechaActualiza=GETDATE(),cUsuarioActualiza='"+cLogin
					+"' where cIdContratoDefinitivo='"+cIdContratoDefinitivo+"' and nIdDocumento="+nIdDocumento;
			pstmnt = conn.prepareStatement(query);
			resp=pstmnt.executeUpdate()>0;
		}finally {
			if(pstmnt!=null){
	...
	public boolean contratoAut(Connection conn,String cIdContratoDefinitivo) throws SQLException{
		...
		PreparedStatement ps = null;
		try {
			query="select cIdContratoDefinitivo from mContrato with(nolock) where cIdContratoDefinitivo='"+cIdContratoDefinitivo+"' and nidEstado=4 "
					+" union select cIdContratoDefinitivo from mPlurianualidadContrato with(Nolock) where nIdEstado=4 and cIdContratoDefinitivo='"+cIdContratoDefinitivo+"'";
			ps = conn.prepareStatement(query);
			rs = ps.executeQuery();
			if(rs.next()){
				resp=true;
	...
	public boolean insertTerminacionAnti(Connection conn,DatosArchivo datos,String cLogin) throws SQLException{
		...
		String query=null;
		try{
			query="insert into mContratoTerminacionAnticipada (cIdContratoDefinitivo,nTipoTerminacion,cCausa,fFechaTermino,fFechaCaptura,cLogin,fFechaLimitePagoPendiente, fFechaNotificacionUAF) " + 
					"values(?,?,?,convert(date,?),GETDATE(),?,convert(date,?),convert(date,?)) ";
			pstmnt = conn.prepareStatement(query);
	...
	public int generateConsolidado(Connection conn,Requisition requi) throws SQLException, JSONException{
		...
		try {
			...
			cmst.setString(6,requi.getIdUsuarioCreacion()); //usuario
	...
	public void readLayoutPresupuestoRequi(Connection conn,DatosArchivo datArchivo)throws Exception{
		...
		String[]encabezado= {"Ejercicio Fiscal",	"Unidad Ejecutora",	"Número de Requisición",
				"Clave SIAF o MAP",	"Clave Interna",	"Línea de la Requisición",	"CUCOP",
				"Partida Presupuestal",	"Descripción",	"Descripción Adicional",	"mes01",
				"mes02",	"mes03",	"mes04",	"mes05",	"mes06",	"mes07",
				"mes08",	"mes09",	"mes10",	"mes11",	"mes12",	"Costo Total por Línea"};
	...
	public boolean insertLayoutPresupuestoRequi(Connection conn,JSONObject jsonObj) throws SQLException, JSONException{
		...
		String query=null;
		try{
			query="insert into mLayoutPresupuestoRequi (cidSolicitud,cClaveEgresos,cClaveInterna,nIdLineaSol"
					+ " ,mes01,mes02,mes03,mes04,mes05,mes06,mes07,mes08,mes09,mes10,mes11,mes12) "
					+ " values(rtrim(ltrim(?)),rtrim(ltrim(?)),rtrim(ltrim(?)),?, ?,?,?,?, ?,?,?,?, ?,?,?,?)";
			pstmnt = conn.prepareStatement(query);
			for(int i=0;i<jsonObj.length();i++){
				if(i==3){
					pstmnt.setInt(i+1, jsonObj.getInt(""+i));
