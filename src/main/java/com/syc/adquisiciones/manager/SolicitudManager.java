...
public class SolicitudManager {
	private static Logger log = Logger.getLogger(SolicitudManager.class);
	public Caso getCaso(HttpSession session,Connection conn)throws Exception {
		...
		try {
			...
			sql = "SELECT MAX(c.ID_CASO) ID_CASO,MAX(c.ID_TC) ID_TC, MAX(o.ID_CASO_OPER) ID_CASO_OPER "+
				" FROM mSolicitud s with(Nolock), CG_CASO c with(Nolock), CG_CASO_OPERACION o with(Nolock) "+
				" where s.C_FOLIO_APA=c.C_FOLIO"+
				" and c.ID_CASO=o.ID_CASO"+
				" and s.cEjercicio=?"+
				" and s.cIdTipoSolicitud = ? "+
				" and s.cIdUnidadEjecutora = ?"+
				" and s.nIdConsecutivo = ?";
			sql1 = "SELECT count(c.ID_CASO) as ID_CASO "+
				" FROM mSolicitud s with(Nolock), CG_CASO c with(Nolock), CG_CASO_OPERACION o with(Nolock) "+
				" where s.C_FOLIO_APA=c.C_FOLIO"+
				" and c.ID_CASO=o.ID_CASO"+
				" and s.cEjercicio=?"+
	...
	public boolean creaEncabDetApartado(Connection conn,DatosRequisicion datosRequi,Usuario usuario)throws Exception{
		...
		try {
			...
			//Crea encabezado
			pstm = conn.prepareStatement(" INSERT INTO tApartadoEncabezado (nFolioApartado, fCarga, fAplicacion, cCentroContable, cRamo,cUnidadResponsable, caNoPreCompromiso,"+
				" cTipoPoliza, nMes, aEjercicioFiscal, nStatusFinanciero, fVigencia, nEnviadoSICOP, cIdSolicitud) "+
				" VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public boolean requiIsConvenio(Connection conn,String cIdSolicitud)throws Exception{
		...
		ResultSet rs=null;
		try {
			pstm = conn.prepareStatement("select cIdSolicitud,cIdPedidoDefinitivo from mPedidoModificadoPartida WITH(NOLOCK) WHERE cIdSolicitud = ? "
					+ " union select cIdSolicitud,cIdContratoDefinitivo from mContratoModificadoPartida WITH(NOLOCK) WHERE cIdSolicitud = ? ");
			pstm.setString(1,cIdSolicitud );
	...
	public boolean requiIsConsolidado(Connection conn,String cIdSolicitud)throws Exception{
		...
		ResultSet rs=null;
		try {
			pstm = conn.prepareStatement("select * from mSolicitud s WITH(NOLOCK) where (s.cIdSolicitud in( select distinct cIdSolicitud "
					+ "from mConsolidadoSolicitud WITH(NOLOCK)) or s.cIdSolicitud in(select distinct cIdSolicitud from mConsolidadoPreseleccionSolicitudes WITH(NOLOCK) )) AND cIdSolicitud= ?");
			pstm.setString(1,cIdSolicitud );
			rs=pstm.executeQuery();
			if(rs.next())
				resp=true;
