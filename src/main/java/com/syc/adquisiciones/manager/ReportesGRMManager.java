...
public class ReportesGRMManager {
	private static Logger log = Logger.getLogger(ReportesGRMManager.class);
	public void writeSheet1Formato1120(Connection conn,XSSFSheet firstSheet,XSSFCellStyle estiloTabla,String cEjercicio)throws Exception{
		...
		try {
			...
			rw = firstSheet.getRow(rows);// sheet0.createRow(rows);
	...
	public void writeSheet1Formato7030(Connection conn,XSSFSheet firstSheet,DatosPedidoContrato datos,String cEjercicio,String query)throws Exception{
		...
		try {
			String[] arreglofechaIni=datos.getcFechaInicio().split("/");
			String[] arreglofechaFin=datos.getcFechaFin().split("/");
			cadena="Periodo: del "+arreglofechaIni[0]+" de "+Util.getNameMonth(Integer.parseInt(arreglofechaIni[1]))+" al "+arreglofechaFin[0]
					+" de "+Util.getNameMonth(Integer.parseInt(arreglofechaFin[1]))+" "+cEjercicio;
			...
			for(int k=0;k<montos2.length;k++){//Totales capitilo 2
			...
			for(int k=0;k<montos3.length;k++){//Totales capitilo 3
			...
			for(int k=0;k<montos5.length;k++){//Totales capitilo 5
			...
			for(int k=0;k<montos5.length;k++){//Totales Total
	...
	public void writeSheet1PagoDirecto(Connection conn,XSSFSheet firstSheet,XSSFCellStyle estiloTabla,String cadena,DatosPedidoContrato datos)throws Exception{
		...
		try {
			query="select *from (select  "
					+ "enc.nFolioPagoDirecto "
					+ ",enc.caNoContrarrecibo "
					+ ",enc.cConcepto "
					+ ",ue.D_DESCRIPCION areaRequirente "
					+ ",enc.cIdRFC rfc"
					+ ",prov.cRazonSocial RazonSocial"
					+ ",convert(int,paas.cIdSubPartida) partida "
					+ ",sum(paas.mMontoNeto)montoNeto "
					+ ",convert(varchar,enc.fAplicacion,103)fAplicacion "
					+ ",enc.cUnidadResponsable cIdUnidadEjecutora "
					+ "from tPagoDirectoEncabezado as enc with(Nolock) "
					+ "inner join tPagoDirectoPAAS as paas with(Nolock) "
					+ "on paas.nFolioPagoDirecto=enc.nFolioPagoDirecto "
					+ "inner join tCatUnidadEjecutora as ue with(Nolock) "
					+ "on ue.cUnidadEjecutora=enc.cUnidadResponsable "
					+ "inner join mCatalogoProveedor as prov with(Nolock) "
					+ "on replace(prov.cIdRFC,'-','')=enc.cIdRFC "
					+ "where enc.cDocumentoHaplicado='S' "
					+ "group by enc.nFolioPagoDirecto "
					+ ",enc.caNoContrarrecibo "
					+ ",enc.cConcepto "
					+ ",ue.D_DESCRIPCION "
					+ ",enc.cIdRFC "
					+ ",paas.cIdSubPartida  "
					+ ",prov.cRazonSocial "
					+ ",enc.cUnidadResponsable,enc.fAplicacion ) sub"
					+" where 1=1 "+datos.getcWhere()
					+"order by sub.nFolioPagoDirecto";
			ps = conn.prepareStatement(query) ;
			rst = ps.executeQuery();
			rsMetadata = rst.getMetaData();
			int totalcolumnas=rsMetadata.getColumnCount();
	...
	public void writeSheetReporteTotalizado(Connection conn,XSSFSheet firstSheet,DatosPedidoContrato datos,String cEjercicioActivo)throws Exception{
		...
		try {
			...
			XSSFCell celdarsad = (rw.getCell(0) == null ? rw.createCell(0) : rw.getCell(0)) ;
			celdarsad.setCellValue(cadena);
			query = "select *from v_mReporteOCTotalizado with(Nolock) where 1=1 "+ datos.getcWhere()
					+" union select *from v_mReporteOCTotalizadoPlu  with(Nolock) where 1=1 "+datos.getcWhere()
					+" union select *from v_mreporteObraTotalizado  with(Nolock) where 1=1 "+datos.getcWhere()
					+" union select *from v_mReporteOcTotalizadoConvEjerAnt  with(Nolock) where 1=1 "+datos.getcWhere()
					+" union select *from v_mReporteOcTotalizadoRemanente  with(Nolock) where 1=1 "+datos.getcWhere()
					+" union select *from v_mReporteOCTotalizadoContratoArt25  with(Nolock) where 1=1 "+datos.getcWhere()
					;
			log.info( query );
			ps = conn.prepareStatement(query) ;
			rst = ps.executeQuery();
