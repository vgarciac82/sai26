...
public class ProcessAgreementManager {
	private static final Logger log = Logger.getLogger( ProcessAgreementManager.class );
	...
	public ArrayList<List<String>> getListAgreements(Connection conn, int ncantDays) throws SQLException{
		...
		try {
			query="select cIdUnidadEjecutora, \r\n" + //contratos del ejercicio actual centralizados
					...
					"and convert(date,fFin)>convert(date,GETDATE()) "+
					"and convert(date,fFin)<=dateadd(day,"+ncantDays+",convert(date,GETDATE()))\r\n" + 
					...
					" and convert(date,fechas.fFecha)>convert(date,GETDATE()) "+
					"and convert(date,fechas.fFecha)<=dateadd(day,"+ncantDays+",convert(date,GETDATE()))";
			log.info( query );
			ps = conn.prepareStatement(query);
			rs = ps.executeQuery();
