...
public class ConsolidacionRGManager {
	...
	public static Logger	log	= Logger.getLogger(ConsolidacionRGManager.class);
	...
	public static boolean esIntegracionProveedor(Connection conn, String nFolioRG) throws Exception {
		String query = "SELECT Count(*) AS total " 
			+ "FROM   dbo.tlayoutscreadosrelaciongastosheader layouts WITH (nolock) " 
			+ " INNER JOIN dbo.trelaciongastosencabezado encabezado WITH (nolock) " 
			+ " ON layouts.snocontrarrecibo = encabezado.canocontrarrecibo "
			+ " WHERE  id_destino_gasto IN ('CPRP','NORN','CLRE','GLRE') "
	...
	public static boolean aplicaConsolidacionRelacionGastos(Connection conn, String nIDIntegracion, String reportPath) throws Exception {
		...
		PreparedStatement upd_fAplicacion = null; //URVP.15082014. HACER UPDATE DE FECHA DE APLICACION AL INTEGRAR DE ACUERDO A LA FECHA DE SIAFF
	...
	private static void acutalizaFonden(Connection conn, String nIDIntegracion) throws Exception {
		...
		String query =   "SELECT Count(*) AS pagoFonden "
						+"FROM   tlayoutscreadospagosdiversosrgheader layouts WITH(nolock) "
						+"       INNER JOIN tpagodiversoencabezado pdEncabezado WITH(nolock) "
						+"               ON layouts.snocontrarrecibo = pdEncabezado.canocontrarrecibo "
						+"       INNER JOIN tpagodiversodetalle pdDetalle WITH(nolock) "
						+"               ON pdEncabezado.nfoliopagodiverso = pdDetalle.nfoliopagodiverso "
						+"WHERE  sauxiliarcomodin = ? "
						+"       AND pdDetalle.id_tipo_concepto IN ( 'PC', 'PB' ) ";
		
		String queryBuscaFondo = "SELECT TOP 1 pdEncabezado.cproyectofonden "
								+"FROM   dbo.tlayoutscreadospagosdiversosrgheader layouts WITH(nolock) "
								+"       INNER JOIN dbo.tpagodiversoencabezado pdEncabezado WITH(nolock) "
								+"               ON layouts.snocontrarrecibo = pdEncabezado.canocontrarrecibo "
								+"       INNER JOIN dbo.tpagodiversodetalle pdDetalle WITH(nolock) "
								+"               ON pdEncabezado.nfoliopagodiverso = pdDetalle.nfoliopagodiverso "
								+"WHERE  sauxiliarcomodin = ? "
								+"       AND pdDetalle.id_tipo_concepto IN ( 'PC', 'PB' ) "
								+"       AND pdEncabezado.cproyectofonden IS NOT NULL ";
	...
	public static boolean esIntegracionLaudosDevengados( Connection conn, String integracion ) throws Exception{
		String query =    "SELECT Count(*) AS total " 
						...
						+ " INNER JOIN dbo.trelaciongastosencabezado encabezado WITH (nolock) " 
						+ " ON layouts.snocontrarrecibo = encabezado.canocontrarrecibo "
						+ " WHERE  id_destino_gasto IN ('CLRE') "
						+ " AND layouts.sauxiliarcomodin = ? ";
			PreparedStatement ps = null;
			ResultSet rs = null;
	...
	private static void updateEventoFA(Connection conn, int nFolioConsolidacion) throws Exception {
		...
		String queryUpdate =  " UPDATE	tconsolidacionrelaciongastosdetalle "
							+ " SET		cevento = REPLACE(cevento, 'INT_', 'INT_FA_'), RFC = 'TESOFE' "
	...
	public static boolean esIntegracionISNQ( Connection conn, String integracion ) throws Exception{
		String query =    "SELECT Count(*) AS total " 
						...
						+ " INNER JOIN dbo.trelaciongastosencabezado encabezado WITH (nolock) " 
						+ " ON layouts.snocontrarrecibo = encabezado.canocontrarrecibo "
						+ " WHERE  id_destino_gasto IN ('2NRQ') "
						+ " AND layouts.sauxiliarcomodin = ? ";
			PreparedStatement ps = null;
			ResultSet rs = null;
