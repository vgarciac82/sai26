...
public class CondicionNoPolizaFormulda extends CondicionCierreMes {
	...
	public boolean CumpleCondicion(Connection conn, int nMes,
		...
		try {
			...
			int mesActualAbierto = MesContableManager
					.ultimoMesContableAbierto(conn, m);
			
			if (mesActualAbierto <= nMes) {
				List<DocPolizaEncabezado> l = PolizaManager
						.getPolizasEnCaptura(conn, cCentroContable, nMes,
								aEjercicioFiscal, mesActualAbierto == nMes);
				if (l.size() > 0) {
					StringBuffer msg = new StringBuffer(
							"No puede cerrar el mes ya que existen documentos sin  autorizar:<br><br>");
					/*
