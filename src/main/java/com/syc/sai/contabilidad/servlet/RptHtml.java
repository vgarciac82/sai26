...
public class RptHtml extends HttpServlet implements GestionInterface {
	...
	protected void printHtml(HttpServletResponse response) throws IOException
	{
		...
		response.addHeader("Content-Disposition", "inline; filename=\"reporte"
			+"rpt" + "_" + System.currentTimeMillis() + ".html\";");
		...
		for(int j=0;j<datos.size();j++)
		{
			...
			pw.println(formatoCols[colNow-1].equalsIgnoreCase("Double")?"<td style='mso-number-format:\"Standard\"'>"+formateador.format(Double.parseDouble(datos.get(j).equals("")?"0.0":datos.get(j)))+"</td>"
				                                                        :"<td>"+datos.get(j)+"</td>");
	...
	{	
		for(int i=0;i<numCols;i++)
		if(datos.get(inicio+i).equalsIgnoreCase("SUBTOTAL"))
			return true;		
		return false;			
	...
	{	
		for(int i=0;i<numCols;i++)
		if(datos.get(inicio+i).contains("TOTAL"))
			return true;		
		return false;			
	...
	private  void balanza(HttpServletRequest request)
	{
		...
		datos=(ArrayList<String>)rbl.ReporteBalanzaList( buscaCuentaIni, buscaCuentaFin,  aEjercicioFiscal,  DepuraLineas,  DepuraColumnas, cCentroContable,  TipoReporte,  mesIni,
