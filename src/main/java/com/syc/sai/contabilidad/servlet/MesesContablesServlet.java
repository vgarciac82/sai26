...
public class MesesContablesServlet extends HttpServlet implements
		GestionInterface {
	private static final long serialVersionUID = 3709991013139160920L;
	private static final Logger logSrvlt = Logger
			.getLogger(MesesContablesServlet.class);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if ("CIERRA_MES_CONTABLE".equals(action)) {
			try {
				...
				*/	
				if (nMes == null || "".equals(nMes) || aEjercicioFiscal == null || "".equals(aEjercicioFiscal))
					...
					String result = mcbl.cierraMesContable(
							Integer.parseInt(nMes),
							Integer.parseInt(aEjercicioFiscal),
							cCentroContable, u.getLogin(), cUnidadResponsable);
			...
			try {
				MesContableBusinessLogic mcbl = new MesContableBusinessLogic(
						condiciones);
				String nMes = req.getParameter("nMes");
				String aEjercicioFiscal = req.getParameter("aEjercicioFiscal");
				String cCentroContable = req.getParameter("cCentroContable");
				...
				if (nMes == null || "".equals(nMes) || aEjercicioFiscal == null
						|| "".equals(aEjercicioFiscal))
					ResponseSender.sendArrayMessages(resp, true, new String[] { "Se requiere al menos el ejercicio fiscal y el mes a cerrar." }, "msg");
				else {
					String result = mcbl.abreMesContable(
							Integer.parseInt(nMes),
							Integer.parseInt(aEjercicioFiscal),
							cCentroContable, u.getLogin(),cUnidadResponsable);
					if ("".equals(result) || null == result)
		...
		for (; e.hasMoreElements();) {
			...
			if (name.contains("condicion")) {
				...
				try {
					...
					CondicionCierreMes condicion = (CondicionCierreMes) clase
							.newInstance();
					condiciones.add(condicion);
				} catch (Exception ex) {
					logSrvlt.error(
							"Error GRAVE: No fue posible instanciar la condicion: "
									+ val + " debido al error " + ex, ex);
				}
			}
		}
	}
