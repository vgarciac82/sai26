...
public class CuentaContableServlet extends HttpServlet {
	...
	private static final Logger log = Logger
			.getLogger(CuentaContableServlet.class);
	private static final long serialVersionUID = -2302925421291178822L;

	private String actualizaCuentas(HttpServletRequest req,
	...
	private String altaCuenta(HttpServletRequest req, HttpServletResponse resp)
			throws Exception {
		CuentaContable cuenta = readAccountFromRequest(req);
	...
	private void autoCompletaCuentas(String term, HttpServletRequest req,
			HttpServletResponse resp) throws Exception {
	...
	private void autoCompletaGrupoEvento(String term, HttpServletRequest req,
		HttpServletResponse resp) throws Exception {
		...
		for (int i = 0; i < l.size(); i++) {
			arr += token + String.format(json, 
										l.get(i).getNidGrupoEvento(),
										l.get(i).getNidGrupoEvento() + "(" + l.get(i).getCnombreGrupo() +  ")",
										l.get(i).getCnombreGrupo());
			token = ",";
		}
		arr = "[" + arr + "]";
		log.debug(arr);
	...
	private void autoCompletaSubGrupoEvento(String term, HttpServletRequest req,
		HttpServletResponse resp) throws Exception {
		...
		if(nGrupo!=null){
			...
			for (int i = 0; i < l.size(); i++) {
				arr += token + String.format(json, 
												l.get(i).getNidSubGrupoEvento(),
												l.get(i).getNidSubGrupoEvento() + "(" + l.get(i).getCnombreSubGrupo() +  ")",
												l.get(i).getCnombreSubGrupo());
				token = ",";
			}
		}else{
			arr += token + String.format(json, 
	...
	private void autoCompletaEventoRelacion(String term, HttpServletRequest req,
		HttpServletResponse resp) throws Exception {
		...
		if(nGrupo!=null){
			if(nSubGrupo!=null){
				for (int i = 0; i < l.size(); i++) {
					arr += token + String.format(json, 
												l.get(i).getCevento(),
												l.get(i).getCevento() + " (" + l.get(i).getDevento() +  ")",
												l.get(i).getDevento());
					token = ",";
				}
			}else{
	...
	private void autoCompletaCABMS(String term, HttpServletRequest req,
		HttpServletResponse resp) throws Exception {
			...
			if(nSubGrupo!=null){
				if(nEvento!=null){
					...
					for (int i = 0; i < l.size(); i++) {
						arr += token + String.format(json, 
													l.get(i).getCcabms(),
													l.get(i).getCcabms() + " (" + l.get(i).getCdescripcion() +  ")",
													l.get(i).getCdescripcion());
						token = ",";
					}
					arr = "[" + arr + "]";
				}else{
	...
	private void selectCABMS(String type, String term, HttpServletRequest req,
		HttpServletResponse resp) throws Exception {				
		String id = term;
		...
		for (int i = 0; i < l.size(); i++) {
			arr += token + String.format(
								json, 
								l.get(i).getCcabms(), 
								l.get(i).getCcabms() + " (" + l.get(i).getCdescripcion() + ")", 
								l.get(i).getCdescripcion(),
								l.get(i).getCcucop().toString(),
								l.get(i).getCpartida());
			token = ",";
		}
		arr = "[" + arr + "]";				
	...
	//**********************************************************************************
	private void buscaCuentas(HttpServletRequest req, HttpServletResponse resp)
			throws Exception {
		CuentaContable cuenta = readAccountFromRequest(req);
		CuentaContableBusinessLogic ccbl = new CuentaContableBusinessLogic();
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (session == null) {
			ResponseSender.sendError(resp,
					"Sin session. Por favor reingrese al sistema.");
			return;
		}
		...
		if ("".equals(accion) || null == accion) {
			try {
				accion = req.getRequestURI().substring(
			...
			if ("".equals(accion) || null == accion) {
				ResponseSender
						.sendError(resp,
								"No se recibio el parametro \"accion\" reporte al administrador");
				return;
			}
		}
		...
		if ("VericaBloqueoCuenta".equals(accion)) {
			...
			try {
				cBloqueada = ccbl.cuentaBloqueada(nCuenta,
						Integer.parseInt(nMes), cCentroContable, operacion);
				ResponseSender.sendResult(resp, String.valueOf(cBloqueada));
			} catch (Exception e) {
				log.error(e, e);
				ResponseSender.sendError(resp,
						"Error mientras se verificaba la cuenta " + nCuenta
								+ "\n" + e.toString());
			}
			...
			try {
				...
				cuentasBloqueadas = ccbl.cuentasBloqueadas(cuentasCargo,
						cuentasAbono, cCentroContable, Integer.parseInt(nMes));
				ResponseSender.sendResult(resp, cuentasBloqueadas, "cuenta");
			} catch (Exception e) {
				log.error(e, e);
				ResponseSender.sendError(
						resp,
						"Error mientras se verificaban las cuentas \n"
								+ e.toString());
			}
		} else if ("AltaCuentas".equals(accion)) {
			try {
				String msg = altaCuenta(req, resp);
				ResponseSender.sendArrayMessages(resp, true, new String[] {
						"Atenci&oacute;n", msg }, "msg");
			} catch (Exception e) {
				log.error(e, e);
				ResponseSender.sendError(
						resp,
						"Error mientras se agregaba la cuenta \n"
								+ e.toString());
			}
			...
			try {
				...
				log.error(e, e);
				ResponseSender.sendArrayMessages(resp, true, new String[] {
						"Atenci&oacute;n", e.toString() }, "msg");
			}
	...
	private String eliminaCuentas(HttpServletRequest req,
			HttpServletResponse resp) throws Exception {
		CuentaContable cuenta = readAccountFromRequest(req);
		CuentaContableBusinessLogic cbl = new CuentaContableBusinessLogic();
	...
	private CuentaContable readAccountFromRequest(HttpServletRequest req) {
		...
		cuenta.setNivelCuenta(NivelCuenta != null ? Integer
				.parseInt(NivelCuenta) : -1);
		cuenta.setAplicacionCuenta(AplicacionCuenta);
		cuenta.setcSubcuenta(cSubcuenta);
		cuenta.setnCuentaLike(nCuentaLike);
		cuenta.setnOrdenBalanza(nOrdenBalanza != null
				&& !"".equals(nOrdenBalanza) ? Integer.parseInt(nOrdenBalanza)
				: -1);
		cuenta.setnNivelBalanza(nNivelBalanza != null
				&& !"".equals(nNivelBalanza) ? Integer.parseInt(nNivelBalanza)
				: -1);
		return cuenta;
	}
}
