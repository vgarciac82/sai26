...
public class ReportesCuentaPublicaServlet extends HttpServlet implements
		GestionInterface {

	private static final Logger log = Logger
			.getLogger(ReportesCuentaPublicaServlet.class);
	...
	private static final String htmlBodyI = "<HTML>\n "
			...
			+ "\t\t\t\tfont-size: 18;\n " + "\t\t\t}\n "
			+ ".encabezadoTopBottomRight { " + "	border-top-style: solid; "
			+ "	border-top-color: black; " + "	border-top-width: thin; "
			+ "	border-bottom-style: solid; " + "	border-bottom-color: black; "
			+ "	border-bottom-width: thin; " + "	border-right-style: solid; "
			+ "	border-right-color: black; " + "	border-right-width: thin; "
			+ "}" + "\t\t\t.encabezadoTopBottomLeft { "
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (u == null) {
			sendError(resp,
					"No hay usuario en session. Ingrese nuevamente al sistema");
			return;
		}
		try {
			...
			if ("C32AP390".equals(reportType)) {
				String reportBody = CuentaPublicaCuerpoReportes
						.getReportBody("C32AP390");

				if (reportBody == null)
					reportBody = CuentaPublicaCuerpoReportes.getReportBody(
							"C32AP390", getReportStream("C32AP390"));

				reportResult = cpbl.generateReportC32AP390();
				reportName = "C32AP390";
			} else if ("C32AP400".equals(reportType)) {
				String reportBody = CuentaPublicaCuerpoReportes
						.getReportBody("C32AP400");

				if (reportBody == null)
					reportBody = CuentaPublicaCuerpoReportes.getReportBody(
							"C32AP400", getReportStream("C32AP400"));
				...
				String reportBody = CuentaPublicaCuerpoReportes
						.getReportBody("C32AP405");

				if (reportBody == null)
					reportBody = CuentaPublicaCuerpoReportes.getReportBody(
							"C32AP405", getReportStream("C32AP405"));
				...
				String reportBody = CuentaPublicaCuerpoReportes
						.getReportBody("C11IF085I");

				if (reportBody == null)
					reportBody = CuentaPublicaCuerpoReportes.getReportBody(
							"C11IF085I", getReportStream("C11IF085I"));

				reportBody = CuentaPublicaCuerpoReportes
						.getReportBody("C11IF085S");

				if (reportBody == null)
					reportBody = CuentaPublicaCuerpoReportes.getReportBody(
							"C11IF085S", getReportStream("C11IF085S"));
				...
				String reportBody = CuentaPublicaCuerpoReportes
						.getReportBody(reportType);

				if (reportBody == null)
					reportBody = CuentaPublicaCuerpoReportes.getReportBody(
							reportType, getReportStream(reportType));
				
				if	((reportType.compareTo("PreCierre")==0)||
					(reportType.compareTo("ContraCuentas")==0)||
					(reportType.compareTo("ConciliacionCostoOperacion")==0)||
					(reportType.compareTo("ReporteCapitulo1000concepto")==0)||
					(reportType.compareTo("BalanzaDet")==0)||
					(reportType.compareTo("BalanzaDet_v2")==0)||
					(reportType.compareTo("BalanzaDet_Ac")==0)||
					(reportType.compareTo("DIOT")==0)||
					(reportType.compareTo("DIOT_v2")==0)||
					(reportType.compareTo("DIM")==0))
				{
	...
	private static void sendExcel(HttpServletResponse resp,
			String reportResult, String reportName) throws IOException {
		resp.setContentType("application/vnd.ms-excel");
		resp.addHeader("Content-Disposition", "inline; filename=\"reporte"
				+ reportName + "_" + System.currentTimeMillis() + ".xls\";");
		PrintWriter out = resp.getWriter();
	...
	private InputStream getReportStream(String reportType) {
		...
		try {
			reportSrcName = ReportesCuentaPublicaServlet.REPORTS_BODY_ROOT
					+ File.separator + reportType + ".bdy";
			...
			if (reportPath != null) {
				...
				is = getServletContext().getResource(reportSrcName)
						.openStream();
			}

		} catch (Exception e) {
			log.error("No se pudo cargar o no existe el archivo "
					+ reportSrcName, e);
		}
		return is;
	}
