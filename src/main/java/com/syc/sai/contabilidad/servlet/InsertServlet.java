	...
	for(int f=0;f<filas;f++)
	 {
		...
		INSERT+="("+datosFijos[0]+","
		    ...
		    +"'"+data[pointer+1]+"',"
			+"'"+data[pointer+3]+"',"
			+"'"+(data[pointer+5].replace("$","").equals("0.00")?"ABONO":"CARGO")+"',"
			+""+doubleTmp.replace(",","").replace("$","")+","
			+"'"+datosFijos[1]+"',"
			+"'"+datosFijos[3]+"',"
			+"'"+datosFijos[2]+"',"
			+"'"+data[pointer+4]+"',"
			...
			+"'"+datosFijos[5]+"',"
			+"'"+data[pointer+7]+"',"
			+""+(data[pointer+12].equals("")?"NULL":"'"+data[pointer+12]+"'")+","
			+""+(data[pointer+13].equals("")?"NULL":data[pointer+13])+","
			+""+(data[pointer+11].equals("")?"NULL":"'"+data[pointer+11]+"'")+","
			+""+data[pointer+8]+","
			+""+data[pointer+9]+","
			+"'"+data[pointer+10]+"',"
			+""+data[pointer+14]+"";
