...
public class CuentaPublicaBusinessLogic extends DataSourceManager {
	private static Logger log = Logger
			.getLogger(CuentaPublicaBusinessLogic.class);
	...
	public String generateReportC11IF085() {
		...
		StringBuffer sb1 = new StringBuffer(
				CuentaPublicaCuerpoReportes.getReportBody("C11IF085S"));
		StringBuffer sb2 = new StringBuffer(
				CuentaPublicaCuerpoReportes.getReportBody("C11IF085I"));
		...
		try {
			...
			if (conn != null)
				try {
					...
					log.warn("Problemas cerrando la conexion a la base de datos: "
							+ e2);
				}
			conn = null;
		}
	...
	public String generateReportC32AP400() {
		...
		StringBuffer sb = new StringBuffer(
				CuentaPublicaCuerpoReportes.getReportBody("C32AP400"));
		try {
			conn = getConnection();
	...
	public String generateReportC32AP405() {
		Connection conn = null;
		StringBuffer sb = new StringBuffer(
				CuentaPublicaCuerpoReportes.getReportBody("C32AP405"));
		try {
			conn = getConnection();
	...
	public String generateReportC32AP390() {
		Connection conn = null;
		StringBuffer sb = new StringBuffer(
				CuentaPublicaCuerpoReportes.getReportBody("C32AP390"));
		try {
			conn = getConnection();
	...
	public String generateReport(String reportName, boolean isSP,
			String[] condiciones) {
		...
		try {
			conn = getConnection();
			if (isSP) {
				String rpBdy = CuentaPublicaCuerpoReportes
						.getReportBody(reportName);
				...
				sb.append(CuentaPublicaManager.reportFromSP(conn, reportName,
						condiciones, includeClasses));
			}
			return tblI + sb.toString() + tblF;
		} catch (Exception e) {
			throw new ReportsException(e);
	...
	public String generateReport(String[] reportNames, boolean isSP,
			List<List<String>> condiciones) {
	...
	public void generateReport(String reportName, boolean isSP,
		String[] condiciones, HttpServletResponse resp ) {
	...
	try {
		...
		if (isSP) {
			...
			if (rpBdy != null && !"".equals(rpBdy))
			    ...
			    CuentaPublicaManager.reportFromSP(conn, reportName,
				condiciones, includeClasses,resp);
		}
