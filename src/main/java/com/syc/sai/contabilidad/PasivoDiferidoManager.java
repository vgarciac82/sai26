...
public class PasivoDiferidoManager {
    ...
    private static final Logger log = LoggerFactory.getLogger(PasivoDiferidoManager.class);
    ...
    private static void validaAplicarReduccion(Connection conn, int nfoliodisminuciondev) throws Exception {
        String query = " SELECT Count(*) AS documentosNoAplica " + "  FROM   tdisminuciondevdetalle disminucionDevengado WITH(nolock) " + "       LEFT OUTER JOIN tPasivoDiferidoEncabezado diferido WITH (NOLOCK) " + "                    ON disminucionDevengado.caNoContrarrecibo = " + "                       diferido.caNoContrarrecibo " + " WHERE  nfoliodisminuciondev = ? " + "       AND diferido.cDocumentoHaplicado = 'S' ";
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ps = conn.prepareStatement(query);
