...
public class CondicionCuentaSinHijos extends CondicionCuentaContable {
	Logger log = Logger.getLogger(CondicionCuentaSinHijos.class);
	...
	public boolean CumpleCondicion(Connection conn, CuentaContable cuenta)
		...
		ResultSet rs = null;
		String qry = "SELECT	TOP 1 nCuenta " + " FROM	tCuentas WITH(nolock) "
				+ " WHERE	nCuentaPadre = ? " + "  AND nCuenta != nCuentaPadre";
