...
public class CasoContable {
	...
	public synchronized static Caso nuevoCaso(Connection conn, Usuario u, String cCentroContable,int id_tc)
	...
	public synchronized static boolean finalizaPoliza(Caso caso, Usuario u, Connection conn){
		...
		pstmnt = conn.prepareStatement(" update CG_CASO_OPERACION set ID_OPER=CO.ID_OPER, CO_RESPONSABLE=CO.O_RESPONSABLE  from CG_OPERACION CO with(nolock) " +
			" where CO.ID_OPER=? and  CO.ID_TC=?  and ID_CASO=?");
