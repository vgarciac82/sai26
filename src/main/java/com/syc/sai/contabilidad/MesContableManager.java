...
public class MesContableManager {
    ...
    public static Logger log = LoggerFactory.getLogger(MesContableManager.class);
    ...
    public static int insertaMesContableEnBloque(Connection conn, MesContable m) throws AccountingEngineException {
        PreparedStatement pStmnt = null;
        int r = 0;
        String qry = "INSERT INTO tmesescontables " + "				            (nmes, " + "				             ccentrocontable, " + "				             aejerciciofiscal, " + "				             mesabierto, " + "				             fcierre, " + "				             usuariocerro) " + "				SELECT ?         AS nMes, " + "				       ccentrocontable, " + "				         ?       AS aEjercicioFiscal, " + "				         ?       AS mesAbierto, " + "				       Getdate() AS fcierre, " + "				       ?  AS usuarioCerro " + "				FROM   tcatalogocentrocontable WITH(NOLOCK) " + "				WHERE  ccentrocontable NOT IN (SELECT ccentrocontable " + "				                               FROM   tmesescontables WITH(NOLOCK) " + "				                               WHERE  nmes = ?); ";
        try {
            pStmnt = conn.prepareStatement(qry);
            pStmnt.setInt(1, m.getnMes());
