...
public class CuentasBusinessLogic extends DataSourceManager {
    ...
    Logger log = LoggerFactory.getLogger(CuentasBusinessLogic.class);
    ...
    public Hashtable<String, List<Cuentas>> getCuentasByPartida(HttpServletRequest req) throws CuentasEngineException {
        ...
        try {
            ...
            if (resultadoPartida.equals("0")) {
                String restrictions = " inner join (select nCuenta from tEventoManual with (nolock) where " + "cIdGrupoEvento=" + cIdGrupoEvento + " AND " + "cIdSubGrupoEvento=" + cIdSubGrupoEvento + " AND " + "cIdeventoManual=" + cIdeventoManual + " AND " + "cEvento='ABONO')t on c.nCuenta=t.nCuenta";
                result.put("CARGO", readCuentasBy(restrictions));
                restrictions = " inner join (select nCuenta from tEventoManual with (nolock) where " + "cIdGrupoEvento=" + cIdGrupoEvento + " AND " + "cIdSubGrupoEvento=" + cIdSubGrupoEvento + " AND " + "cIdeventoManual=" + cIdeventoManual + " AND " + "cEvento='CARGO' )t on c.nCuenta=t.nCuenta";
                result.put("ABONO", readCuentasBy(restrictions));
            } else {
                String restrictions = " inner join (select nCuenta from tEventoManual with (nolock) where " + "cIdGrupoEvento=" + cIdGrupoEvento + " AND " + "cIdSubGrupoEvento=" + cIdSubGrupoEvento + " AND " + "cIdeventoManual=" + cIdeventoManual + " AND " + "cEvento='ABONO' AND " + "cPartida='" + cPartida + "')t on c.nCuenta=t.nCuenta";
                result.put("CARGO", readCuentasBy(restrictions));
                restrictions = " inner join (select nCuenta from tEventoManual with (nolock) where " + "cIdGrupoEvento=" + cIdGrupoEvento + " AND " + "cIdSubGrupoEvento=" + cIdSubGrupoEvento + " AND " + "cIdeventoManual=" + cIdeventoManual + " AND " + "cEvento='CARGO' AND " + "cPartida='" + cPartida + "')t on c.nCuenta=t.nCuenta";
                result.put("ABONO", readCuentasBy(restrictions));
            }
            return result;
        } catch (Exception e) {
