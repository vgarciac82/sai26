...
public class CuentaContableManager {
	private static Logger log = Logger.getLogger(CuentaContableManager.class);

	public static boolean cuentaBloqueada(Connection conn, String nCuenta,
			int nMes, String cCentroContable, String operacion)
			throws AccountingEngineException {
		boolean result = false;
		String qry = "SELECT ncuenta, " + "       cnivelbloqueo, "
				+ "       Substring(cbloqueaabonos, ?, 1) AS bloqueoAbono, "
				+ "       Substring(cbloqueacargos, ?, 1) AS bloqueoCargo  "
				+ "FROM   tcuentas t WITH(nolock)  "
				+ " WHERE  cnivelbloqueo IS NOT NULL "
				+ "       AND ncuenta = ? "
				+ "       AND ( cnivelbloqueo = 'T' "
				+ "              OR cnivelbloqueo = ? ) ";
		String nivelBloqueo = CuentaContable.CENTRO_CONTABLE_LOCAL
				.equals(cCentroContable) ? "C" : "F";
		...
		try {
			...
			throw new AccountingEngineException(
					"Error analizando cuenta bloqueada " + e, e);
		} finally {
			if (pStmnt != null)
				try {
					pStmnt.close();
	...
	public static String[] cuentasBloqueadas(Connection conn,
			String cuentasCargo, String cuentasAbono, String cCentroContable,
			int nMes) throws AccountingEngineException {

		List<String> result = new ArrayList<String>();
		String nivelBloqueo = CuentaContable.CENTRO_CONTABLE_LOCAL
				.equals(cCentroContable) ? "C" : "F";
		String token = "";
		String qry = "SELECT ncuenta " + "FROM   tcuentas t WITH(nolock) "
				+ "WHERE  cnivelbloqueo IS NOT NULL " + "       AND ( ";

		if (cuentasCargo != null && !"".equals(cuentasCargo)) {
			qry += "			( ncuenta IN ( " + cuentasCargo + " ) "
					+ "               AND ( cnivelbloqueo = 'T' "
					+ "                      OR cnivelbloqueo = '"
					+ nivelBloqueo + "' ) "
					+ "               AND Substring(cbloqueacargos, "
					+ String.valueOf(nMes) + ", 1) = 1 " + "            ) ";
		...
		if (cuentasAbono != null && !"".equals(cuentasAbono)) {
			qry += token;
			qry += "            ( ncuenta IN ( " + cuentasAbono + " ) "
					+ "                   AND ( cnivelbloqueo = 'T' "
					+ "                          OR cnivelbloqueo = '"
					+ nivelBloqueo + "' ) "
					+ "                   AND Substring(cbloqueaabonos, "
					+ nMes + ", 1) = 1 " + "			)  ";
		}
		...
		try {
			...
			throw new AccountingEngineException(
					"Error validando bloqueo de cuentas " + e.toString(), e);
		} finally {
			if (pStmnt != null)
				try {
					pStmnt.close();
	...
	public static String calculaCuentaPadre(CuentaContable cuenta) {
		if (cuenta.getnCuentaPadre() != null
				&& !"".equals(cuenta.getnCuentaPadre()))
	...
	public static List<CuentaContable> buscaCuentasContables(Connection conn,
			CuentaContable modelo) throws AccountingEngineException {
		...
		try {
			if (modelo.getnCuenta() != null
					&& !"".equalsIgnoreCase(modelo.getnCuenta())) {
				cond = cond + " " + token + " nCuenta LIKE ? ";
				token = "AND";
			}
			if (modelo.getdCuenta() != null
					&& !"".equalsIgnoreCase(modelo.getdCuenta())) {
			 ...
			 */
			if (modelo.getnCuentaPadre() != null
					&& !"".equalsIgnoreCase(modelo.getnCuentaPadre())) {
			 ...
			 */
			if (modelo.getcSubcuenta() != null
					&& !"".equalsIgnoreCase(modelo.getcSubcuenta())) {
			...
			String qry = queryBase + cond + token
					+ "  ( not (TipoBalance='P' and TipoCuenta='P') AND nCuenta NOT LIKE '0%' ) ";
	...
	public static List<CuentaContable> buscaCuentasDeAplicacion(
			Connection conn, CuentaContable modelo)
			throws AccountingEngineException {
		PreparedStatement pStatement = null;
		ResultSet rs = null;
		String queryBase = " SELECT	* " + " FROM	tCuentas WITH(nolock) ";
		...
		try {
			...
			String qry = queryBase + cond + token
					+ "  ( not (TipoBalance='P' and TipoCuenta='P') AND nCuenta NOT LIKE '0%' AND AplicacionCuenta = 'S' ) ";
			pStatement = conn.prepareStatement(qry);
			int cnt = 1;
	...
	public static List<SubGrupoEvento> autoCompletaSubGrupoEvento(
		Connection conn, String nGrupo, String aux)
		throws AccountingEngineException {
		PreparedStatement pStatement = null;
		ResultSet rs = null;		
	...
	public static List<EventoRelacion> autoCompletaEventoRelacion(
		Connection conn, String nGrupo, String nSubGrupo, String nEvento)
		throws AccountingEngineException {
		PreparedStatement pStatement = null;
		ResultSet rs = null;
	...
	public static List<CatalogoCabms> autoCompletaCABMS(
		Connection conn, String nGrupo, String nSubGrupo, String nEvento, String nCAMBS)
		throws AccountingEngineException {
		PreparedStatement pStatement = null;
		ResultSet rs = null;
	...
	public static List<CatalogoCabms> selectCABMS(
		Connection conn, String type, String id)
		throws AccountingEngineException {
		PreparedStatement pStatement = null;
		ResultSet rs = null;
	...
	private static GrupoEvento extraeGrupoEvento(ResultSet rs)
			throws AccountingEngineException {
		try {
	...
	private static EventoRelacion extraeEventoRelacion(ResultSet rs)
		throws AccountingEngineException {
		try {
	...
	private static CatalogoCabms extraeCatalogoCabms(ResultSet rs)
		throws AccountingEngineException {
		try {
...
//***********************************************************************
	...
	private static CuentaContable extraeCuentaContable(ResultSet rs)
			throws AccountingEngineException {
		try {
			...
			cc.setcSubcuenta(rs.getString("cSubcuenta") == null ? "" : rs
					.getString("cSubcuenta"));
	...
	public static int insertCuentaContable(Connection conn,
			CuentaContable cuenta) throws AccountingEngineException {
		PreparedStatement pStmnt = null;
		String qry = "INSERT INTO tCuentas (nCuenta, dCuenta, TipoCuenta, nCuentaPadre, TipoBalance, "
				+ "					   VerificaSaldo, NaturalezaCuenta, NivelCuenta, AplicacionCuenta, cSubcuenta, "
				+ "					   nCuentaLike, nOrdenBalanza, nNivelBalanza) "
				+ "VALUES (?, ?, ?, ?, ?, " + "?, ?, ?, ?, ?," + "?, ?, ?)";
		...
		try {
			...
			pStmnt.setString(cnt++, ("".equals(cuenta.getcSubcuenta()) ? null
					: cuenta.getcSubcuenta()));
	...
	public static int updateCuentaContable(Connection conn,
			CuentaContable cuenta) throws AccountingEngineException {
		PreparedStatement pStmnt = null;
		...
		try {
			...
			if (cuenta.getAplicacionCuenta() != null
					&& !"".equals(cuenta.getAplicacionCuenta())) {
				set += token + "		AplicacionCuenta = ?";
				token = ", ";
			}
			if (cuenta.getcSubcuenta() != null
					&& !"".equals(cuenta.getcSubcuenta())) {
				set += token + "		cSubcuenta = ? ";
				token = ", ";
			}
			if (cuenta.getTipoCuenta() != null
					&& !"".equals(cuenta.getTipoCuenta())) {
				set += token + "		TipoCuenta = ? ";
				token = ", ";
			}
			if (cuenta.getTipoBalance() != null
					&& !"".equals(cuenta.getTipoBalance())) {
				set += token + "		TipoBalance = ? ";
				token = ", ";
			}
			if (cuenta.getNaturalezaCuenta() != null
					&& !"".equals(cuenta.getNaturalezaCuenta())) {
	...
	public static int eliminaCuentaContable(Connection conn,
			CuentaContable cuenta) throws AccountingEngineException {
		PreparedStatement pStmnt = null;
	...
	public static ArrayList <CuentaContable> obtenCuentasArbol(Connection conn)throws AccountingEngineException
	{
		...
		String qry = " SELECT nCuenta,dcuenta,TipoCuenta,TipoBalance,NivelCuenta, nOrdenBalanza,nNivelBalanza,nCuentaPadre,isnull(cSubcuenta,'') cSubcuenta, AplicacionCuenta,"
					+" NaturalezaCuenta,isnull((select top 1 tm.cTipoMovimiento from tMovimiento tm with(nolock) where tm.nCuenta=tc.nCuenta),'') Movimientos"
					+" ,isnull((select top 1 th.TipoCuenta from tCuentas th with (nolock) where th.nCuentaPadre=tc.nCuenta),'') Hijos"
					+" FROM tCuentas AS tc with (nolock) WHERE	not (TipoBalance='P' and TipoCuenta='P')"; 	
	...
	{	
		...
		for(int j=0;j<Cuentas.size();j++)				
			if(Cuentas.get(j).getnCuentaPadre().equals(CuentaPadre))					
