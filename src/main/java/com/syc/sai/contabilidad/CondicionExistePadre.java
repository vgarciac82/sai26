...
public class CondicionExistePadre extends CondicionCuentaContable {
    ...
    public boolean CumpleCondicion(Connection conn, CuentaContable cuenta) throws AccountingEngineException {
        ...
        ResultSet rs = null;
        String qry = "SELECT	* " + " FROM	tCuentas WITH(nolock) " + " WHERE	NivelCuenta = ? " + "		AND	nCuenta = ? ";
        ...
        try {
            if (cuenta.getNivelCuenta() == 1)
                ...
                pStmnt.setString(2, CuentaContableManager.calculaCuentaPadre(cuenta));
                rs = pStmnt.executeQuery();
                if (rs.next())
                    existePadre = true;
                else {
                    existePadre = false;
                    setMensaje("No existe la cuenta padre para la cuenta " + cuenta.getnCuenta());
                }
                return existePadre;
            }
        } catch (Exception e) {
