...
public class CondicionNoExisteCuentaContable extends CondicionCuentaContable {
    ...
    private static final Logger log = LoggerFactory.getLogger(CondicionNoExisteCuentaContable.class);

    @Override
    public boolean CumpleCondicion(Connection conn, CuentaContable cuenta) throws AccountingEngineException {
        String qry = " SELECT	1 as EXISTE " + " FROM	tCuentas WITH(nolock) " + " WHERE	nCuenta = ? ";
