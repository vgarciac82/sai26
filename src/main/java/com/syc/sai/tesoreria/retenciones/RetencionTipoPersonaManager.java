...
public class RetencionTipoPersonaManager {
	...
	private static Logger	log	= Logger.getLogger(RetencionTipoPersonaManager.class);
	...
	public static String obtieneTipoPersona(Connection conn, String cRFC) throws Exception {
		...
		try {
			log.debug("Buscando tipo persona para el RFC " + cRFC);
	...
	public static List<Retencion> obtieneRetenciones(Connection conn, String tipoPersona, String partidas, String cc, String tipoPago, String folioPago) throws Exception {
		...
		String query = "SELECT DISTINCT cIdTipoRetencion,cTipoRetencion,nPorcRetencion " 
					+ " FROM pCatalogoTipoRetencion catalogo WITH (NOLOCK) " 
					+ " INNER JOIN tRelacionPartidaRetencion Obligatorias WITH (NOLOCK) " 
					+ " ON catalogo.cIdTipoRetencion = Obligatorias.idRetencion "
					+ " WHERE partida IN (SELECT Registro FROM dbo.fn_Split_todos(',',?)) " 
					+ " AND 1 = CASE WHEN ? = 2 THEN cFisica WHEN ? = 1 THEN cMoral ELSE 0 END";
		
		PreparedStatement psInsert = null;
		String queryInsert = "INSERT INTO tPagoDirectoRetencion "
							+"	SELECT YEAR(GETDATE()), ?, ?, ?";
		try {
			log.debug("Buscando retenciones " + tipoPersona + " en la partida " + partidas);
			log.trace("Query: " + query);
