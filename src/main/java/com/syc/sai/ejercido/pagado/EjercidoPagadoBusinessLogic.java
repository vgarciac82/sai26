...
public class EjercidoPagadoBusinessLogic extends DataSourceManager {
    ...
    private static final Logger log = LoggerFactory.getLogger(EjercidoPagadoBusinessLogic.class);
                    ...
                    if (EjercidoPagadoBusinessLogic.EJERCIDO.equalsIgnoreCase(pago.getEstatusEjercido())) {
                        ...
                        if (EjercidoPagadoManager.existeIntegracionSICOP(conn, pago.getIntegracion())) {
                            ...
                            if (EjercidoPagadoManager.montosCorrectos(conn, pago.getIntegracion())) {
                                ...
                                EjercidoPagadoManager.insertaLog(conn, pago.getTipoPago(), pago.getFolioPagado(), pago.getIntegracion(), "-1", Util.getTodayESMX(), "", pago.getImporteNeto().doubleValue(), "EJERCIDO", "Los montos entre la integradora en SAI [" + pago.getIntegracion() + "] Total[" + pago.getImporteNeto() + "] no corresponde a la registrada en las tablas de SICOP/SIAF Total[" + EjercidoPagadoManager.obtenMontoIntegracionSicop(conn, pago.getIntegracion()));
                                continue;
                            }
                        } else {
                            EjercidoPagadoManager.insertaLog(conn, pago.getTipoPago(), pago.getFolioPagado(), pago.getIntegracion(), "-1", Util.getTodayESMX(), "", pago.getImporteNeto().doubleValue(), "EJERCIDO", "No se encontro la integracion [" + pago.getIntegracion() + " en las tablas de SICOP/SIAFF");
                            continue;
                        }
                    }
