...
public class PagadoManager {
	...
	public static List<Pagado> generaPagadoIntegracion(Connection conn, String integracion, String usuario, String tipoPoliza) throws Exception {
		...
		String queryEncabezado = "SELECT sai.ctipopago,  "
								...
								+"        CONVERT(VARCHAR(32), Getdate(), 103) AS fechaPagado  "
	...
	public static int insertaPagado( Connection conn, List<Pagado> pagado ) throws Exception{
		String queryInsertEnc =  "INSERT INTO tpagadoencabezado  "
								...
								+"               ?,  "
								+"               ?) ";
		String queryInsertDet =  "INSERT INTO tpagadodetalle  "
								+"             (ctipopago,  "
								+"              nfoliopago,  "
								+"              ndocrenglon,  "
								+"              cmes,  "
	...
	private static String calculaEventoPenas(Connection conn, String EP, String tipoPago, int folioPago) throws Exception {
		String querySelectPagoPenas = "SELECT	penasDetalle.cTipopago, " 
	                                + "		penasDetalle.nFolioPago,  " 
	                                + "		penasDetalle.caNoContrarrecibo " 
	                                + "  FROM	tPagoPenasConvEncabezado penasEncabezado WITH(NOLOCK) " 
	                                + "		INNER JOIN  " 
	                                + "		tPagoPenasConvDetalle penasDetalle WITH(NOLOCK) "
	                                + "		ON penasEncabezado.nFolioPagoPenasConv = penasDetalle.nFolioPagoPenasConv " 
	                                + " WHERE	penasEncabezado.nFolioPagoPenasConv = ?";
		...
		try {
			...
			if (rsBuscaPago.next()) {
				...
				String querySelecOrigen = "SELECT	DISTINCT "
                                        + "		pagoEncabezado.cRadicado, " 
                                        + "		pagoEncabezado.cIngresosPropios, " 
                                        + "		pagoEncabezado.ID_DESTINO_GASTO,  " 
                                        + "		pagoDetalle.ID_TIPO_CONCEPTO,  " 
                                        + "		pagoDetalle.ID_TIPO_MOVIMIENTO   " 
                                        + "  FROM	" + tblEncabezado
                                        + " pagoEncabezado WITH(NOLOCK) " 
                                        + "		INNER JOIN  " 
                                        + "		" + tblDetalle + " pagoDetalle WITH(NOLOCK) " 
                                        + "		ON pagoEncabezado." + tblFolioCol + " = " + "pagoDetalle." + tblFolioCol 
                                        + " WHERE	pagoEncabezado." + tblFolioCol + " = " + nFolioPago
                                        + "   AND	pagoEncabezado.caNoContrarrecibo = '" + caNoContrarecibo + "' " 
                                        + "   AND	pagoDetalle.ep='" + EP + "' " ;
