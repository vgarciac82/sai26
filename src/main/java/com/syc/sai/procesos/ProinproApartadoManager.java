...
public class ProinproApartadoManager {
    ...
    private static final Logger log = LoggerFactory.getLogger(ProinproApartadoManager.class);

    public static void revisaTiempoLimite(Connection conn, long maximoDias) throws AccountingEngineException {
        String sqlSelect = "update tAutoCancelaBitacora set fEjecucion = GETDATE() where tipoOperacion = 'PROINPRO' ";
        sqlSelect += " SELECT	nFolioDocOli " + "  FROM	tDocProinproEncabezado p " + " WHERE	CONVERT( DATE, fAplicacion, 103) < CONVERT( date,  DATEADD(DD,-" + maximoDias + ",GETDATE()), 103) " + "   AND	cDocumentoHaplicado = 'S' " + "   AND	cDocumentoEstatus = 'A' ";
