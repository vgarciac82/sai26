...
@WebServlet(name = "AdjuntaConcBanServlet", urlPatterns = { "/uploadConciliacionBan" })
public class UploadConciliacionServlet extends HttpServlet implements GestionInterface {

    private static final long serialVersionUID = 126320606200256721L;

    private static final Logger log = LoggerFactory.getLogger(UploadConciliacionServlet.class);
        ...
        if ("".equals(msgRetorno)) {
            ...
            try {
                ...
                while (iter.hasNext()) {
                    ...
                    if (item.isFormField()) {
                        if ("CTAB".equals(item.getFieldName()))
                            ctaBan = item.getString();
                        else if ("nMes".equals(item.getFieldName()))
                            strMes = item.getString();
                        else if ("nombreCarpeta".equals(item.getFieldName()))
                ...
                msgRetorno = ucbl.uploadCB(u, nombreCarpeta, nombreDestino, ctaBan, Integer.parseInt(strMes));
            } catch (Exception e) {
