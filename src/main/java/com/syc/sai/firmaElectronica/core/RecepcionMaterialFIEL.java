...
public class RecepcionMaterialFIEL extends SolicitudFirmaElectronica {
	...
	public String getCuerpoCorreoAutoriza( Connection conn ) throws Exception {
		...
		String urlDescarga = generaURLDescarga( conn ); // ConfiguraAplicativoManager.getSystemSetting(
														// conn, "URL_SAI" ) +
														// "/muestraDocumento";
		...
		try {
			...
			if ( SolicitudFirmaElectronica.AUT_RM.equals( tipoAutorizacion ) ) {
				...
				AlarmaManager.procesaAlarmaCNF( conn, ""
						, null
						, null
						, "Solicitud de visto bueno de ".concat( getRecepcionMaterial().getcIdRecepcionMat() ).concat( " del contrato " ).concat( getRecepcionMaterial().getcIdPedContDef() )
						, com.syc.adquisiciones.util.Util.getSystemSetting( conn, SolicitudFirmaElectronica.GP_BCC_ATENTA_NOTA )
						, null,null
						, getCuerpoCorreoVistoBueno( conn ));
