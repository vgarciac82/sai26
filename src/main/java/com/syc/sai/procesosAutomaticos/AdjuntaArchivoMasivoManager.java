...
public class AdjuntaArchivoMasivoManager {
	...
	private static final Logger	log		= Logger.getLogger( AdjuntaArchivoMasivoManager.class );
	...
	public static String validaEsAlimentacionBrigadistas( Connection conn, int nFolioDocumento ) throws Exception {
		...
		String sSql = "SELECT	CASE WHEN nEsAlimentacionBrigadistas = 1 THEN 'Alimentacion a Brigadistas' " 
				    + "WHEN nEsBoxLunch = 1 THEN 'Juegos Deportivos'"
		            + " ELSE '' END cNomDocto " + " FROM tRELACIONGASTOSEncabezado WITH (NOLOCK) WHERE nFolioRELACIONGASTOS = ?";
