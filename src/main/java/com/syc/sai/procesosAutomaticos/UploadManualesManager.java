...
public class UploadManualesManager {
	...
	private static final Logger	log	= Logger.getLogger(UploadManualesManager.class);

	public static Caso creaManualContable(Connection conn, Usuario u, int idTipoCaso, String centroContable, FolioGeneratorInterface fg) throws Exception {//OK
		/* Crea caso y expedientes */
		Caso c = CasoManager.nuevoCaso(conn, u, idTipoCaso, fg);//OK
		c = UploadManualesManager.creaEstrucDoctosManualContable(conn, c, u);//OK
	...
	private static Caso creaEstrucDoctosManualContable(Connection conn, Caso c, Usuario u) throws Exception {//OK
	...
	public static boolean existeManualContableCC(Connection conn, String centroContable, int manual, int nIDversion) throws Exception { //OK
	...
	public static boolean existeVersionManual(ManualContable cCont, int idManual) {//OK
	...
	public static int insertaDetalle(Connection conn, ManualContableDetalle detalle) throws Exception {//OK
	...
	private static int insertaEncabezado(Connection conn, ManualContableEncabezado encabezado) throws Exception {//OK
	...
	private static int insertar(Connection conn, ManualContable conCon) throws Exception {//OK
		int insertados = 0;
	...
	public static Caso readCasoOrigenManualContable(Connection conn, String centroContable, int idManual) throws Exception {//OK
	...
	public static ManualContable readManualContable(Connection conn, String folioSAI) throws Exception {//OK
		...
		try {
			...
			if (rs.next()) {
				...
				ManualContableEncabezado cbfe = readManualContableEncabezado(conn, folioSAI);//OK
				List<ManualContableDetalle> cbfd = readManualContableDetalle(conn, cbfe.getfolioManualContable() );//OK

				cbf = new ManualContable(cbfe, cbfd);//OK
			}
			return cbf;
		} finally {
			CloseObject.closeObject(rs);
	...
	private static List<ManualContableDetalle> readManualContableDetalle(Connection conn, int folioManual) throws Exception {//OK
		
		List<ManualContableDetalle> detalle = new ArrayList<ManualContableDetalle>();
		String query = "SELECT	nFolioManualContable, "
					 + " 		fCarga, "
					 + " 		cUsuarioCarga, "
					 + " 		nIDManual"
					 + "  FROM	tManualContableDetalle WITH(NOLOCK) " 
					 + " WHERE	nFolioManualContable = ? ";
		ResultSet rs = null;
		PreparedStatement ps = null;
	...
	private static ManualContableEncabezado readManualContableEncabezado(Connection conn, String folioSAI) throws Exception {//OK
		String query = " SELECT	nFolioManualContable , "				
					 + "		cCentroContable , "
					 + "		cFolioSAI , "
					 + "		nIDGabinete " 
					 + "  FROM	tManualContableEncabezado WITH(NOLOCK) " 
					 + " WHERE	cFolioSAI = ? ";
	...
	public static int ObtenerIDManual(Connection conn, String nombreCarpeta ) throws Exception {//OK
		PreparedStatement ps = null;
		ResultSet rs = null;
		int idManual = -1;
