...
public class VersionaMasivo extends DataSourceManager {
    ...
    private static final Logger logger = LoggerFactory.getLogger(VersionaMasivo.class);
    ...
    private List<Tramite> listaTramites() throws Exception {
        ...
        ResultSet rs = null;
        String query = " SELECT DISTINCT cxp,  " + "        tipopago, " + "        folio,  " + "        nfolio,  " + "        id_gabinete " + " FROM   tmp_pagos_regularizacion AS pagos WITH(nolock) " + "        LEFT OUTER JOIN vtramitesexportar exportar WITH(nolock) " + "                     ON pagos.tipopago = exportar.cdocumento  " + "                        AND pagos.cxp = exportar.canocontrarrecibo " + " WHERE  cxp <> '1'  " + " ORDER  BY folio";
