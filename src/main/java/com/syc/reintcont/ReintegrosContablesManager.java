...
public class ReintegrosContablesManager {
	public static final Logger log = Logger.getLogger(ReintegrosContablesManager.class);
	...
	public static String validaMes(Connection conn, Caso c, int folio, String fAcredit) {
		...
		try {
			...
			pstmnt.setInt(2, mes);
			log.debug("Folio: "+folio+" Mes: "+mes);
			rs = pstmnt.executeQuery();
			if(rs.next()){
				retval = rs.getString(1);
				log.debug("Mes "+mes+" abierto: "+retval);
			}
		} catch(SQLException s){
			log.warn(s);
			s.printStackTrace();
	...
	public static void autorizaReintegro(Connection conn, Caso c, String nNumSicop, String cRecMotivSicop, String nNumMAP, String cRecMotivMAP,String ulogin,String prefixPath, String fAcredit, String year) throws SQLException, GestionException {
		...
		try {
			...
			String sSQL="INSERT INTO tReintegroAutEncabezado (nFolioReintegroaut, fSolicitud, cTipoReintegro, cRamo, cUnidadResponsable, cDocumentoHaplicado, aEjercicioFiscal, observaciones, concepto, u_login, "
			...
			String query="INSERT INTO tReintegroAutDetalle(nFolioReintegroaut,nDocRenglon,noCLC,secCLC,EP,mImporteCLC,cMes,movto,cEvento, mImporte, mImporteNegativo, cCentroContable, nCapitulo,cxp,RFC,nFolioDependencia,ALM,nRenglonPagado,obgt,ctab,nidprograma,cSubPrograma,FFM,mAmortizacionAnticipo,mPasivoDiferido,cPasivo,mImportePC) "
		        ...
		        +"       rd.movto, "
		        +"       (SELECT CASE  "
		        +"                 WHEN ctipopago = 'PAGOOBRA' THEN CASE WHEN SUBSTRING(EP,40,1) = '4' THEN 'RNGIP' ELSE 'RNC' END "
		        +"                 WHEN ctipopago = 'PAGODIVERSO' THEN CASE WHEN SUBSTRING(EP,40,1) = '4' THEN 'RNGIP' ELSE 'RNC' END "
		        +"                 WHEN ctipopago = 'PAGODIRECTO' THEN cat_re.cPREFIJO_EVENTO "		        		        
		        +"                 WHEN ctipopago = 'RELACIONGASTOS' THEN cat_re.cPREFIJO_EVENTO "
		        +"                 WHEN ctipopago = 'NOMINA' THEN cat_re.cPREFIJO_EVENTO "
		        +"                 WHEN ctipopago = 'AJENAS' THEN CASE WHEN SUBSTRING(EP,40,1) = '4' THEN 'RNGIP' ELSE 'RN' END "		        
		        +"                 WHEN ctipopago = 'FEDERALIZADO' THEN CASE WHEN SUBSTRING(EP,40,1) = '4' THEN 'RNGIP' ELSE 'RNC' END "
		        +"                 ELSE '' "
		        +"               END AS dTipoPago "
		        ...
		        +"       nidprograma, " 
		        +"       cSubPrograma, "  
		        +"		 rd.OBGT + CASE WHEN RIGHT('00' + LTRIM(RTRIM(nidprograma)),2) = '13' THEN '0000' ELSE '" + year + "' END + RIGHT('00' + LTRIM(RTRIM(nidprograma)),2) + RIGHT('00' + LTRIM(RTRIM(cSubPrograma)),2) + '00' AS ffm, 0 AS mAmortizacionAnticipo, "
		        +" 		 rd.mPasivoDiferido*-1,"
		        +" 		 rd.cPasivo, "
		        +" 		 rd.mImportePC "	
		        +"FROM   treintegrodetalle rd WITH (nolock) "
		        +"INNER JOIN tReintegroEncabezado re WITH(nolock) "
		        +"ON re.nFolioReintegro = rd.nFolioReintegro "
		        +"INNER JOIN tCatalogo_Reintegros cat_re WITH(nolock) "
		        +"ON re.id_tiporeintegro = cat_re.id_reintegro "
	...
	public static ReintegroContEncabezado getReintegroEncabezadoNuevo(Connection conn,int folio) throws SQLException{
		...
		log.debug("Leyendo tReintegroEncabezado del Folio: "+folio);
		pstm.setInt(1, folio);
