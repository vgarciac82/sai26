...
public class ExpedienteExporterManager {
	private static final Logger log = Logger.getLogger(ExpedienteExporterManager.class);

	public static String[] listaExportarAuditoria(Connection conn) throws Exception {
		String query =    "SELECT DISTINCT viaticos.canocontrarrecibo, " 
						+ "	                        tramites.folio,  "
						+ "	                        tramites.id_gabinete, " 
	                 + "	                        Replace(viaticos.cComision, ';', '') AS cconcepto " 
						+ "	FROM   tviaticosTransparencia_V2 viaticos WITH(nolock)  "
						+ "	       LEFT OUTER JOIN dbo.vtramitesexportar tramites WITH(nolock) " 
						+ "	                    ON viaticos.canocontrarrecibo = tramites.canocontrarrecibo " 
						+ "	                       AND tramites.cdocumento <> 'RECTIFICACION'  "
						+ "	WHERE  1=1 " 
						+ "   AND  viaticos.caNoContrarrecibo <> ''" 
						+ "   AND  cExportado = 'N'" ;
//	                 + "   AND  cOBGT = '38501'";
...
//						+ "   AND viaticos.caNoContrarrecibo like '26cp%'";
	...
	public static String[] listaExportarAuditoriaError(Connection conn) throws Exception {
		String query =    "SELECT	caNoContrarrecibo, cFolio, idGabinete "
						+ "  FROM   tTramitesExportarError WITH(nolock) " 
						+ " ORDER BY 1 ";
