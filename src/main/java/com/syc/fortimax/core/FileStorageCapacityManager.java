...
public class FileStorageCapacityManager {
	...
	public static boolean isNotMeetQuota(Connection conn, String nombre_usuario, double bytes) throws SQLException {
		...
		try {
			...
			rs = stmntA.executeQuery("SELECT bytes_autorizados, bytes_usados FROM imx_usuario_expediente"
			...
			stmntB.addBatch("UPDATE imx_usuario_expediente SET bytes_usados = " + bytesUsadosUsr
					+ " WHERE nombre_usuario = '" + nombre_usuario + "'");
	...
	public static boolean decrementQuota(Connection conn, String nombre_usuario, double bytes) throws SQLException {
		...
		try {
			...
			rs = stmntA.executeQuery("SELECT bytes_usados" + " FROM imx_usuario_expediente"
					+ " WHERE nombre_usuario = '" + nombre_usuario + "'");
			...
			stmntB.addBatch("UPDATE imx_usuario_expediente SET bytes_usados = " + bytesUsados
					+ " WHERE nombre_usuario = '" + nombre_usuario + "'");
