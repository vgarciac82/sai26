...
public class ExpedientExporterBusinessLogic extends DataSourceManager {
	...
	private static final Logger	log		= Logger.getLogger(ExpedientExporterBusinessLogic.class);
	...
	public String[] exporSeveraltExpedients(String[] exportList, boolean standAlone) throws Exception {
		...
		Connection conn = null;
		String zipExportNameRaiz = System.getProperty("java.io.tmpdir") + File.separatorChar + "export_"
		...
		try {
			...
			for (String cFolio : exportList) {
				...
				System.out
						.println("Exportando expediente " + (i + 1) + " de " + exportList.length + " Folio: " + cFolio);
				...
				Carpeta cRaiz = CarpetaManager.getCarpetaRaiz(conn, c.getTipoCaso().getGavetaAsociada(),
						c.getIdGabinete());
				String raiz = cRaiz.getNombreCarpeta();

				List<Documento> exportL = DocumentoManager.listaDocumentosExportar(conn,
						c.getTipoCaso().getGavetaAsociada(), c.getIdGabinete(), raiz);
	...
	public String exportExpedients(String[] exportList, boolean standAlone) throws Exception {
		...
		Connection conn = null;
		String zipExportName = System.getProperty("java.io.tmpdir") + File.separatorChar + "export_" 
		+ System.currentTimeMillis() + "_" + (Math.random() * 1000) + ".zip";
		...
		try {
			...
			for (String infoRenglon : export) {
				...
				try {
					...
					log.info(String.format("Exportando expediente %d de %d CxP: %s Folio: %s Gabinete: %d", cnt, 
					export.length, cuentaPorPagar, folioCaso, idGabinete));
					String zipExportName = System.getProperty("java.io.tmpdir") + File.separatorChar + cuentaPorPagar 
					+ ".zip";
					...
					if ("RELACIONGASTOS".equalsIgnoreCase(c.getTipoCaso().getGavetaAsociada()))
						carpetaCFDI = (RelacionGastosManager.esRelacionPorOficio(conn, nFolioTramite) ? "Oficios" 
						: "CFDI");

					int totalSolicitud = CarpetaManager.getTotalDocumentosCarpeta(conn, 
					c.getTipoCaso().getGavetaAsociada(), c.getIdGabinete(), carpetaSolicitud);
					int totalComprobantes = CarpetaManager.getTotalDocumentosCarpeta(conn, 
					c.getTipoCaso().getGavetaAsociada(), c.getIdGabinete(), carpetaCFDI);
					...
					if (totalSolicitud > 0 || totalComprobantes > 0) {
						...
						List<Documento> exportL = DocumentoManager.listaDocumentosExportarAuditoria(conn, 
						c.getTipoCaso().getGavetaAsociada(), c.getIdGabinete(), cuentaPorPagar, 
						carpetaSolicitud, "Solicitud de Pago Firmada", concepto);
						List<Documento> exportC = DocumentoManager.listaDocumentosExportarAuditoria(conn, 
						c.getTipoCaso().getGavetaAsociada(), c.getIdGabinete(), cuentaPorPagar, carpetaCFDI, 
						null, null);

						if (exportC != null && !exportC.isEmpty())
							exportL.addAll(exportC);

						List<ExportLogDetallado> detalleRenglon = CFDIUtils.addToZip(zos,
						exportL.toArray(new Documento[exportL.size()]), cuentaPorPagar);
	...
	private void escribeLogs(List<ExportLog> exportLog, String nombre) {
		try {
			String pathExport = System.getProperty("java.io.tmpdir") + File.separatorChar + nombre + "_" 
					+ CFDIUtils.getTodayFile() + ".csv";
