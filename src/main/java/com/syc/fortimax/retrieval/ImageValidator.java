		...
		try {
			...
			while(!conn.isClosed() &&
				...
				while(!conn.isClosed() &&
					...
					String query2 = "SELECT V.UNIDAD_DISCO, V.RUTA_BASE, " +
									"V.RUTA_DIRECTORIO, P.NOM_ARCHIVO_VOL, " +
									"P.NOM_ARCHIVO_ORG, P.TIPO_PAGINA, " +
									"P.TITULO_APLICACION, P.ID_GABINETE, " +
									"P.ID_CARPETA_PADRE, P.ID_DOCUMENTO, " +
									"P.NUMERO_PAGINA, P.VOLUMEN, D.NOMBRE_DOCUMENTO  " +
									"FROM IMX_PAGINA P, IMX_DOCUMENTO D, IMX_VOLUMEN V WITH (NOLOCK) " +
									"WHERE V.VOLUMEN = P.VOLUMEN " +
									"AND P.TITULO_APLICACION = D.TITULO_APLICACION " +
									"AND P.ID_GABINETE = D.ID_GABINETE " +
									"AND P.ID_CARPETA_PADRE = D.ID_CARPETA_PADRE " +
									"AND P.ID_DOCUMENTO = D.ID_DOCUMENTO " +
									"AND P.TITULO_APLICACION='" + strTitApp + "' " +
									"AND P.VOLUMEN='" + strVolumen + "' " +
									"ORDER BY P.TITULO_APLICACION, P.ID_GABINETE, " +
									"P.ID_CARPETA_PADRE, P.ID_DOCUMENTO, P.NUMERO_PAGINA ";
					...
					while(!conn.isClosed() &&
						...
						String nombreArchivoImg = rs2.getString("UNIDAD_DISCO")
											 	+ rs2.getString("RUTA_BASE")
											 	+ rs2.getString("RUTA_DIRECTORIO")
											 	+ rs2.getString("NOM_ARCHIVO_VOL");
						//CONTABILIDAD_G271C0D0&image.index=9
						String claveExpediente  = rs2.getString("TITULO_APLICACION")
												+ "_G"
												+ rs2.getString("ID_GABINETE")
												+"C"
												+ rs2.getString("ID_CARPETA_PADRE")
												+"D"
												+ rs2.getString("ID_DOCUMENTO")
												+"&image.index="
												+ rs2.getString("NUMERO_PAGINA");
