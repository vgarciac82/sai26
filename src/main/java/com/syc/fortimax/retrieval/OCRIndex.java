...
public final class OCRIndex extends Thread {
    ...
    public OCRIndex(ThreadGroup someTG,
    				String someString,
    				Connection someConn,
    				IndexWriter someWriter,
    				FileWriter someLogFileWriter,
    				String[] someOCRParams, 
    				String[] someLuceneData,
    				String someDBSysDateFunction,
    				int someNumDocto) {
	...
	private int updateDB(boolean isError) {
		...
		int retVal = -1;
		try {
				boolean noNulls = luceneData[APLICACION]!=null &
								  luceneData[GABINETE]!=null &
								  luceneData[CARPETA]!=null &
								  luceneData[DOCUMENTO]!=null &
								  luceneData[PAGINA]!=null;
			    if (noNulls) {
			    	query = "UPDATE IMX_PAGINA_INDEX "
						  + "SET PROCESADO = ? , "
						  + "FH_MODIFICACION = "
						  + DBSysDateFunction
						  + " WHERE "
						  + "TITULO_APLICACION = ?  AND "
						  + "ID_GABINETE       = ?  AND "
						  + "ID_CARPETA_PADRE  = ?  AND "
						  + "ID_DOCUMENTO      = ?  AND "
						  + "NUMERO_PAGINA  = ? ";
