...
public class Searcher {
    ...
    public static Hits searchSingleIndex(String pathToSearch, String pathToStopTable, String queryString, String[] filters) {
        Hits myHits = null;
        // Create a searcher from the index file
        //String indexFile = "/tmp/fileindex";
        IndexSearcher searcher = null;
        if (pathToSearch == null || pathToSearch.trim().length() == 0) {
        ...
        if (myHits.length() > 0) {
            for (int i = 0; i < myHits.length(); i++) {
                ...
                try {
                    System.out.println("Path=[" + myHits.doc(i).get("path") + "], " + "Aplicacion=[" + myHits.doc(i).get("aplicacion") + "], " + "Gabinete=[" + myHits.doc(i).get("gabinete") + "], " + "Carpeta=[" + myHits.doc(i).get("carpeta") + "], " + "Documento=[" + myHits.doc(i).get("documento") + "], " + "Pagina=[" + myHits.doc(i).get("pagina") + "], ");
    ...
    public static Hits searchMultipleIndexes(String pathToSearch, String pathToStopTable, String queryString, String[] filters) {
        Hits myHits = null;
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (myPath == null || myPath.trim().length() == 0) {
                myPath = INDEX_DIR;
            }
            myPath += i + "";
            f = new File(myPath);
    ...
    public static Filter buildFilters(String[] filters) {
        ...
        //
        Filter Filter1 = null;
        if (filters != null && filters[0] != null && filters[0].trim().length() > 0) {
        ...
        //
        Filter Filter2 = null;
        if (filters != null && filters[1] != null && filters[1].trim().length() > 0) {
            int myGab = -1;
            try {
        ...
        //
        Filter Filter3 = null;
        if (filters != null && filters[2] != null && filters[2].trim().length() > 0) {
            int myCar = -1;
            try {
