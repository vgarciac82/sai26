...
public class Indexer {
    ...
    public static void index(Connection conn, String indexPath, File dataFile, IndexWriter someWriter) {
    ...
    public static IndexWriter open(String pathToWriter, IndexWriter writer) {
        ...
        if (writer == null || !(f.exists() && f.isDirectory())) {
            create = true;
        }
        //if(create) {
        try {
            writer = new IndexWriter(pathToWriter, new SpanishAnalyzer("./spanishStopWords.dic"), create, IndexWriter.MaxFieldLength.LIMITED);
        } catch (Exception e) {
            e.printStackTrace();
        }
        //}
    ...
    public static Document createDocument(String[] result, File f) throws java.io.FileNotFoundException {
        Document doc = null;
        String metatag = "";
    ...
    public static void getFiles(File baseFileDir) {
        try {
            ...
            Connection conn = java.sql.DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE", "fortimax", "fortimax");
    ...
    public static String[] searchFortimaxDB(Connection conn, String fileName) {
        ...
        try {
            String query = "SELECT * FROM IMX_PAGINA where VOLUMEN='" + strVolumen + "' AND NOM_ARCHIVO_VOL = '" + strNombre + "'";
            //+ "' AND TIPO_PAGINA = 'I'";
            prep = conn.prepareStatement(query);
            rs = prep.executeQuery();
            while (rs.next()) {
                System.out.println("[" + rs.getString("TITULO_APLICACION") + "] [" + rs.getString("ID_GABINETE") + "] [" + rs.getString("ID_CARPETA_PADRE") + "] [" + rs.getString("ID_DOCUMENTO") + "] [" + rs.getString("NUMERO_PAGINA") + "] [" + rs.getString("VOLUMEN") + "] [" + rs.getString("TIPO_VOLUMEN") + "] [" + rs.getString("NOM_ARCHIVO_VOL") + "] [" + rs.getString("NOM_ARCHIVO_ORG") + "] [" + rs.getString("TIPO_PAGINA") + "] [" + rs.getString("ESTADO_PAGINA") + "] [" + rs.getString("TAMANO_BYTES") + "]");
