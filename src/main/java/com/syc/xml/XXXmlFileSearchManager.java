...
public class XXXmlFileSearchManager
{
    ...
    public static String select(Connection conn, String titulo_aplicacion, int id_caso)
        throws SQLException
    {
        ...
        if(rs2.next())
        {
            ...
            Query.append("SELECT * "
            		      +"FROM imx_pagina a "
            		     +"WHERE a.titulo_aplicacion = ? "
            		       +"AND a.id_gabinete = ? " 
            		       +"AND a.id_carpeta_padre = 0");
    ...
    public static String[][] recuperaCasosSinFLimite(Connection conn) throws SQLException
    {
    	...
    	Query1.append("SELECT  COUNT(*) FROM "
    	    	      +"(" 
    	    	      +" SELECT  DISTINCT vr.id_caso AS ID_CASO" 
      			      +",		 v.unidad_disco + v.ruta_base + v.ruta_directorio + p.nom_archivo_vol AS ARCHIVO"
      				  +" FROM    vimx_reportes vr"
      				  +" INNER JOIN cg_caso_dato  cd"
      		          +" ON      (cd.id_caso = vr.id_caso)"
      		          +" INNER JOIN imx_pagina p"
      		          +" ON      (p.id_gabinete = vr.id_gabinete)"
      		          +" INNER JOIN imx_volumen v WITH (NOLOCK)"
      		    	  +" ON      (v.volumen = p.volumen)"
      		    	  +" WHERE   p.id_carpeta_padre  = 0"
      		    	  +" AND     p.titulo_aplicacion = '" + TITULO_APLICACION + "'"
      		    	  +" AND     p.id_documento      = "  + TIPO_DOCTO_EXT
      		    	  +" AND     cd.id_cd            = "  + ID_DPC_F_LIMITE
      		    	  +" AND     (cd.cd_valor IS NULL"
      		    	  +" OR      cd.cd_valor = '')"
    	    	      +") AS cuantos" 
      		    	  );
    ...
    public static String[] selectIdGabinete(Connection conn, String[] IdCasos) throws SQLException
    {
    	...
    	{
        	Query1.append("SELECT DISTINCT id_gabinete"
        				  +" FROM cg_caso"
        				 +" WHERE id_caso = ?");
