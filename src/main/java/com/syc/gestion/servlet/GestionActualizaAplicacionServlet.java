...
public class GestionActualizaAplicacionServlet extends HttpServlet implements GestionInterface {

	private static final long serialVersionUID = 1L;

	private static Logger log = Logger.getLogger(GestionActualizaAplicacionServlet.class);
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			for (int i = 0; ((!delete) && (i < dsc.length)); i++) {
				...
				if (update || ((value != null) && (!"".equals(value)) && (!"--".equals(value)))) {
					String[] values = { String.valueOf(dsc[i].getIdTipoDatos()),
							String.valueOf(dsc[i].getIndiceTipo()), ((value == null) ? "" : value),
			...
			if (actionOk) {
				...
				if (insert || update) {
					scrptPrfx[1] = "	window.open(\"../getexpedient?select=" + fimx.getTituloAplicacion()
							+ "&id_gabinete=" + ((insert) ? result[0] : String.valueOf(fimx.getIdGabinete()))
							+ "\",\"left\");";
				} else if (delete) {
					// session.removeAttribute(TREE_EXP_KEY);
					scrptPrfx[1] = "	window.open(\"ArbolExpediente.jsp?select=" + fimx.getTituloAplicacion()
							+ "&arbol.tipo=g\",\"left\");";
				}
