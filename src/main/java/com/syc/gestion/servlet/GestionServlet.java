...
public class GestionServlet extends HttpServlet implements GestionInterface {
	...
	public void doPost( HttpServletRequest req, HttpServletResponse resp ) throws ServletException, IOException {
		...
		switch ( command ) {
			...
			break;
			case CMD_ADVANCE_CASE: // 4
				try {
					if ( log.isDebugEnabled() )
						log.debug( "Avanzando Caso" );

					avanzaCaso( req, resp ); // Ethiel, se agrega el response

					try {// Solo adecuaciones en operacion 5 y 2
			...
			break;
			case CMD_GET_USRES_GROUPS_OPER: // 16
				try {
					if ( log.isDebugEnabled() )
			...
			break;
			case CMD_GET_DOCUMENT_FOR_NAME: // 18
				try {
			...
			break;
			case CMD_USER_LEAVE_CASE:// 22 al cerrar el caso el usuario lo
										// libera para que lso demas del grupo
										// lo puedan ver
				try { // MLR RO-0001 Libera el caso al abandonar la pantalla de
					...
					if ( c.getCasoOperacion( 0 ).getOperacion().getNombre().toUpperCase().contains( "CONSULTA" ) ) {
						...
						if ( redirect )
							resp.sendRedirect( "../caso/search.jsp" );// no lo
																		// podemos
																		// regresar
																		// a la
																		// de
