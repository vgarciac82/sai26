...
public class GestionSendFolderServlet extends HttpServlet implements GestionInterface {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (!asZip) {
			...
			String classPath[] = { getServletContext().getRealPath(preDir + "SelfExtractorFile.class"),
					getServletContext().getRealPath(preDir + "AsiExtraField.class"),
					getServletContext().getRealPath(preDir + "ExtraFieldUtils.class"),
					getServletContext().getRealPath(preDir + "JarMarker.class"),
					getServletContext().getRealPath(preDir + "UnixStat.class"),
					getServletContext().getRealPath(preDir + "UnrecognizedExtraField.class"),
					getServletContext().getRealPath(preDir + "ZipEntry.class"),
					getServletContext().getRealPath(preDir + "ZipExtraField.class"),
					getServletContext().getRealPath(preDir + "ZipFile$BoundedInputStream.class"),
					getServletContext().getRealPath(preDir + "ZipFile.class"),
					getServletContext().getRealPath(preDir + "ZipLong.class"),
					getServletContext().getRealPath(preDir + "ZipOutputStream.class"),
					getServletContext().getRealPath(preDir + "ZipShort.class") };
	...
	private void processChildren(JarOutputStream out, ITree t, ITreeNode p, String parent, boolean opcionVersiones)
			throws IOException {
	...
	private void appendToZipFile(JarOutputStream out, NodeInformation inf, Fortimax f, String parent)
			throws IOException {
		...
		if (f.isDocumento()) {
			...
			for (int i = 0; i < fn.length; i++) {
				if (fn.length > 1) {
					...
					String pagName = path + "p" + zeros.substring(0, zeros.length() - String.valueOf(i + 1).length())
							+ (i + 1) + ((fn[i].withExtension()) ? "." + fn[i].getExtension() : "");
	...
	private FortimaxFile[] getFilesList(Fortimax f) {
		...
		try {
			files = cbl.getArchivosDeDocumento(f.getTituloAplicacion(), f.getIdGabinete(), f.getIdCarpeta(), f
					.getIdDocumento());
