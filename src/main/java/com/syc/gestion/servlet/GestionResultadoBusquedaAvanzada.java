...
public class GestionResultadoBusquedaAvanzada extends HttpServlet implements GestionInterface {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (dsc == null) {
				out.println("</head>");
				out.println("<body leftmargin=\"0\" topmargin=\"0\" rightmargin=\"0\" bottommargin=\"0\" marginwidth=\"0\" "
			...
			for (int i = 0; i < data.length; i++) {
				...
				for (int j = 1; j < data[i].length ; j++) {
					...
					if (j == data[i].length -1) {
						...
						String URLseguimiento = req.getScheme() 
						  + "://"
						  + req.getServerName()
						  + ((req.getServerPort() == 80) ? "" : ":" + req.getServerPort())
						  + req.getContextPath()
						  + "/caso/seguimiento.jsp?"
						  + "mg=true"
						  + "&tc="
						  + m.get("id_tc")
						  + "&c="
						...
						out.println("<td align=\"center\">"
								+ ((withGabinete) ? "" : "<a target=\"_blank\" href=\""+URLseguimiento+"\"><font size=\"1\">seguimiento</font>")
								+ ((withGabinete) ? "" : "</a>") 
								+ "</td>");
					} else {
						out.println("<td align=\"center\">"
								+ ((withGabinete) ? "" : "<a target=\"_blank\" href=\"caso/show-caso?" + PRM_CASE + "=" + m.get("id_caso")
										+ "&" + PRM_CASE_OPER + "=" + m.get("id_caso_oper") + "\"><font size=\"1\">")
								+ ((data[i][j] == null) ? "" :java.net.URLDecoder.decode(java.net.URLEncoder.encode(data[i][j], resp.getCharacterEncoding()),resp.getCharacterEncoding())) + ((withGabinete) ? "" : "</font></a>") 
								+ "</td>");
					}
			...
			"title=\"Dejar en pausa esta operaci&oacute;n\">");
			out.println("</body>");
			out.println("</html>");
			out.flush();
			out.close();
