...
@WebServlet(name = "GestionActualizaAplicacionServlet", urlPatterns = { "/createexpedient" })
public class GestionConsultaAplicacionServlet extends HttpServlet implements GestionInterface {

    private static final long serialVersionUID = 1L;

    private static Logger log = LoggerFactory.getLogger(GestionConsultaAplicacionServlet.class);
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            out.println("</head>");
            out.println("<body bgcolor=\"#000000\" leftmargin=\"0\" topmargin=\"0\" rightmargin=\"0\" bottommargin=\"0\" " + "marginwidth=\"0\" marginheight=\"0\" scroll=\"no\">");
            ...
            for (int i = 0; i < desc.length; i++) {
                ...
                if (i == 0) {
                    ...
                    out.println("<table align=\"center\" width=\"100%\"><tr><td align=\"center\" class=\"bordetit\">" + fimx.getTituloAplicacion() + "</td></tr></table>");
                    ...
                    if (modificar) {
                        ...
                        out.println("<td>");
                        out.println("<input type=\"submit\" name=\"cmdEjecutar\" value=\"Buscar\" " + "onmouseover=\"self.status='Buscar Expediente'; return true;\" " + "onclick=\"return submitForm(1)\">");
                        out.println("</td>");
                    }
                    if (modificar) {
                        out.println("<td>");
                        out.println("<input type=\"button\" value=\"Cancelar\" " + "onmouseover=\"self.status='Cancelar Actualizaci&oacute;n'; " + "return true;\" onclick=\"history.back(-1)\">");
                        out.println("</td>");
                    }
                ...
                switch(d.getIdTipoDatos()) {
                    ...
                    case // Double, Float
                    7:
                        out.println("<input type=\"text\"" + ((d.getIndiceTipo() == 2) && modificar ? "" : " name=\"" + d.getNombreCampoLower()) + "\" size=\"" + d.getLongitudCampo() + "\"" + (d.getLongitudCampo() > 0 ? " maxlength=\"" + d.getLongitudCampo() + "\"" : "") + (modificar ? " value=\"" + ((datos[row][i + 1] == null) ? "" : datos[row][i + 1]) + "\"" : "") + (d.getIndiceTipo() == 2 && modificar ? " disabled" : " onkeypress=\"keystrokInteger()\"") + ">");
                        if ((d.getIndiceTipo() == 2) && modificar)
                            out.println("<input type=\"hidden\" name=\"" + d.getNombreCampoLower() + "\" value=\"" + ((datos[row][i + 1] == null) ? "" : datos[row][i + 1]) + "\">");
                        break;
                    case // Fecha
                    8:
                        ...
                        out.println("<td>Del <select" + ((d.getIndiceTipo() == 2) && modificar ? " disabled" : " name=\"_day_" + d.getNombreCampoLower() + "\"") + ">");
                        out.println("<option value=\"\"></option>");
                        for (int j = 1; j <= 31; j++) out.println("<option value=\"" + j + "\"" + ((j == day) ? "selected" : "") + ">" + j + "</option>");
                        out.println("</select></td>");
                        if ((d.getIndiceTipo() == 2) && modificar)
                            out.println("<input type=\"hidden\" name=\"_day_" + d.getNombreCampoLower() + "\" value=\"" + day + "\">");
                        // Meses
                        out.println("<td><select" + ((d.getIndiceTipo() == 2) && modificar ? " disabled" : " name=\"_mes_" + d.getNombreCampoLower() + "\"") + ">");
                        out.println("<option value=\"\" selected></option>");
                        for (int j = 1; j <= 12; j++) out.println("<option value=\"" + j + "\"" + ((j == month) ? "selected" : "") + ">" + j + "</option>");
                        out.println("</select></td>");
                        if ((d.getIndiceTipo() == 2) && modificar)
                            out.println("<input type=\"hidden\" name=\"_mes_" + d.getNombreCampoLower() + "\" value=\"" + month + "\">");
                        out.println("<td><select" + ((d.getIndiceTipo() == 2) && modificar ? " disabled" : " name=\"_year_" + d.getNombreCampoLower() + "\"") + ">");
                        out.println("<option value=\"\" selected></option>");
                        for (int j = end_year; j >= ini_year; j--) out.println("<option value=\"" + j + "\"" + ((j == year) ? "selected" : "") + ">" + j + "</option>");
                        out.println("</select></td>");
                        if ((d.getIndiceTipo() == 2) && modificar)
                            out.println("<input type=\"hidden\" name=\"_year_" + d.getNombreCampoLower() + "\" value=\"" + year + "\">");
                        //Ethiel, para rangos
                        out.println("<td>Al <select" + ((d.getIndiceTipo() == 2) && modificar ? " disabled" : " name=\"_dayfinal_" + d.getNombreCampoLower() + "\"") + ">");
                        out.println("<option value=\"\"></option>");
                        for (int j = 1; j <= 31; j++) out.println("<option value=\"" + j + "\"" + ((j == day) ? "selected" : "") + ">" + j + "</option>");
                        out.println("</select></td>");
                        if ((d.getIndiceTipo() == 2) && modificar)
                            out.println("<input type=\"hidden\" name=\"_dayfinal_" + d.getNombreCampoLower() + "\" value=\"" + day + "\">");
                        // Meses
                        out.println("<td><select" + ((d.getIndiceTipo() == 2) && modificar ? " disabled" : " name=\"_mesfinal_" + d.getNombreCampoLower() + "\"") + ">");
                        out.println("<option value=\"\" selected></option>");
                        for (int j = 1; j <= 12; j++) out.println("<option value=\"" + j + "\"" + ((j == month) ? "selected" : "") + ">" + j + "</option>");
                        out.println("</select></td>");
                        if ((d.getIndiceTipo() == 2) && modificar)
                            out.println("<input type=\"hidden\" name=\"_mesfinal_" + d.getNombreCampoLower() + "\" value=\"" + month + "\">");
                        out.println("<td><select" + ((d.getIndiceTipo() == 2) && modificar ? " disabled" : " name=\"_yearfinal_" + d.getNombreCampoLower() + "\"") + ">");
                        out.println("<option value=\"\" selected></option>");
                        for (int j = end_year; j >= ini_year; j--) out.println("<option value=\"" + j + "\"" + ((j == year) ? "selected" : "") + ">" + j + "</option>");
                        out.println("</select></td>");
                        if ((d.getIndiceTipo() == 2) && modificar)
                            out.println("<input type=\"hidden\" name=\"_yearfinal_" + d.getNombreCampoLower() + "\" value=\"" + year + "\">");
                        //Ethiel, termina
                    ...
                    10:
                        if (d.getLongitudCampo() < 80) {
                            out.println("<input type=\"text\"" + ((d.getIndiceTipo() == 2) && modificar ? "" : " name=\"" + d.getNombreCampoLower()) + "\" size=\"" + d.getLongitudCampo() + "\"" + (d.getLongitudCampo() > 0 ? " maxlength=\"" + d.getLongitudCampo() + "\"" : "") + (modificar ? " value=\"" + ((datos[row][i + 1] == null) ? "" : datos[row][i + 1]) + "\"" : "") + (d.getIndiceTipo() == 2 && modificar ? " disabled" : "") + ">");
                            if ((d.getIndiceTipo() == 2) && modificar)
                                out.println("<input type=\"hidden\" name=\"" + d.getNombreCampoLower() + "\" value=\"" + ((datos[row][i + 1] == null) ? "" : datos[row][i + 1]) + "\">");
                            break;
                        }
                    ...
                    12:
                        ...
                        out.println("<textarea" + ((d.getIndiceTipo() == 2) && modificar ? "" : " name=\"" + d.getNombreCampoLower()) + "\" cols=\"40\" rows=\"3\"" + " onkeydown=\"event.returnValue = validityLength(this," + (d.getLongitudCampo() - 1) + ")\"" + (((d.getIndiceTipo() == 2) && modificar) ? " disabled" : "") + ">" + (modificar ? ((datos[row][i + 1] == null) ? "" : datos[row][i + 1]) : "") + "</textarea>");
                        if ((d.getIndiceTipo() == 2) && modificar)
                            out.println("<input type=\"hidden\" name=\"" + d.getNombreCampoLower() + "\" value=\"" + ((datos[row][i + 1] == null) ? "" : datos[row][i + 1]) + "\">");
                        break;
                }
            ...
            for (int i = 0; i < desc.length; i++) {
                ...
                switch(d.getIdTipoDatos()) {
                    ...
                    case // Long Integer
                    4:
                        arrFnc.append(token + "\"checkInteger(f." + d.getNombreCampoLower() + ",'" + d.getNombreColumna() + "'," + !d.getRequeridoBoolean() + ")\"");
                        break;
                    // Decimal
                    case 5:
                    case // Double, Float
                    7:
                        arrFnc.append(token + "\"checkFloat(f." + d.getNombreCampoLower() + ",'" + d.getNombreColumna() + "'," + !d.getRequeridoBoolean() + ")\"");
                        break;
                    case // Fecha
                    8:
                        arrFnc.append(token + "\"checkDate(f._year_" + d.getNombreCampoLower() + ",f._mes_" + d.getNombreCampoLower() + ",f._day_" + d.getNombreCampoLower() + ",'Día'," + !d.getRequeridoBoolean() + ")\"");
                        //Ethiel, para rangos
                        arrFnc.append(token + "\"checkDate(f._yearfinal_" + d.getNombreCampoLower() + ",f._mesfinal_" + d.getNombreCampoLower() + ",f._dayfinal_" + d.getNombreCampoLower() + ",'Dia'," + !d.getRequeridoBoolean() + ")\"");
                        //Ethiel, termina
                        break;
                    // String
                    case 10:
                    case // Long String
                    12:
                        arrFnc.append(token + "\"checkString(f." + d.getNombreCampoLower() + ",'" + d.getNombreColumna() + "'," + !d.getRequeridoBoolean() + ")\"");
                        break;
                }
                token = ",";
            }
            ...
            out.println("      return confirm(\"Desea" + (modificar ? " modificar el " : " crear un ") + "expediente con los datos proporcionados?\");");
            out.println("   }");
    ...
    private void generaJavaScript(Descripcion[] desc, PrintWriter out) {
        ...
        if (jsInteger || jsString) {
            ...
            out.println("var mSuffixWhitespace = \"\\\" no debe tener espacios en blanco.\\n\\nProporcione información o dejelo vacio.\"");
        ...
        if (jsDate) {
            ...
            out.println("function daysInFebruary(year) { return (((year % 4 == 0) && ((!(year % 100 == 0)) || (year % 400 == 0))) ? 29 : 28); }");
            ...
            out.println("   if (isEmpty(dayField.value)) return ((emptyOK)? true: warnEmpty(dayField, labelString, emptyOK));");
            out.println("   if (isEmpty(monthField.value)) return ((emptyOK)? true: warnEmpty(monthField, \"Mes\", emptyOK));");
            out.println("   if (isEmpty(yearField.value)) return ((emptyOK)? true: warnEmpty(yearField, \"Año\", emptyOK));");
        ...
        if (jsInteger) {
            ...
            out.println("   if (isEmpty(theField.value)) return ((emptyOK)? true: warnEmpty(theField, labelString, emptyOK));");
