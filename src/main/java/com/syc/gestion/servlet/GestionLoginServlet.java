	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			if (jniName == null) {
				...
				log.info("Object: {}",
						"Environment Entry \"dataSourceRefName\" nula usando default \"" + jniName + "\"");
			} else
				log.info("Object: {}", "dataSourceRefName=" + jniName);
		} catch (NamingException exc) {
			jniName = "jdbc/gestion";
			log.info("Object: {}",
					"Environment Entry \"dataSourceRefName\" no definida usando default \"" + jniName + "\"");
		}

		try {
			ConfiguraAplicativoBusinessLogic systemConfig = new ConfiguraAplicativoBusinessLogic(jniName);
			employeeClient = new EmployeeClient(systemConfig.getSystemSetting("HR_EMPLOYEE_URL"),
					systemConfig.getSystemSetting("HR_EMPLOYEE_USER"),
					systemConfig.getSystemSetting("HR_EMPLOYEE_CODE"));
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if ("si".equals(redireccionaEjercicio) && !"2015".equals(ejercicio)) {
			String url = req.getScheme() + "://" + req.getServerName()
					...
					+ (!"2015".equals(ejercicio) ? "_" + ejercicio : "") + "/gstnmngr/login?login=" + uLogin
					+ "&redirEjer=no&ejercicio=" + ejercicio + (!StringUtils.isBlank(action) ? "&a=" + action : "")
					+ (!StringUtils.isBlank(ul) ? "&ul=" + ul : "") + (!StringUtils.isBlank(un) ? "&un=" + un : "")
					+ (!StringUtils.isBlank(d) ? "&d=" + d : "") + (!(f == null) ? "&f=" + f : "")
					+ (!(o == null) ? "&o=" + o : "");
		...
		try {
			...
			ABL.agregaAuditoria(u.getLogin(), ea.getId(),
					GestionInterface.ATT_CONEXION.substring(GestionInterface.ATT_CONEXION.indexOf("/") + 1), "Acceso",
					"Login", u.getLogin(), u.getLogin(), "SELECT * FROM CG_USUARIO WHERE U_LOGIN=" + u.getLogin(),
					u.getLogin());
		} catch (Exception audex) {
			log.error("Error occurred",
					"Error escribiendo en la bitacora de accesos al intentar login de:" + u.getLogin());
