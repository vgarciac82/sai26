...
public class GestionFileReceiverServlet extends HttpServlet implements GestionInterface, ViewerParametersInterface {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		while (i.hasNext()) {
			...
			try {
				...
				cbl.recibeDocumentoGestion(c, fimx.getIdCarpeta(), node.getName(), ext, new DataInputStream(item
					.getInputStream()), closeWin);

				item.delete();

				Fortimax f = new Fortimax(select);
				idx = cbl.getArchivosDocumento(f.getTituloAplicacion(), f.getIdGabinete(), f.getIdCarpeta(), f
					.getIdDocumento()).length - 1;
	...
	private void windowClose(PrintWriter out, String select, int index, boolean current) throws IOException {
		...
		if (current)
			out.print("opener.parent.parent.frames[\"main\"].location.href=\"" + "imgmng/image-viewer.jsp?select="
				+ select + "&" + INDEX_KEY + "=" + index + "\";");
		else
			out.print("opener.parent.parent.frames[\"main\"].location.href=\"" + "image-viewer.jsp?select=" + select
				+ "&" + INDEX_KEY + "=" + index + "\";");
