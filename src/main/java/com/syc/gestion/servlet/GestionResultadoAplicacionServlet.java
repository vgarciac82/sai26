...
public class GestionResultadoAplicacionServlet extends HttpServlet implements GestionInterface {
	...
	private static Logger log = Logger.getLogger(GestionResultadoAplicacionServlet.class);
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
				...
				for (int i = 0; i < dsc.length; i++) {
					...
					switch (dsc[i].getIdTipoDatos()) {
						...
						case 8: // Fecha
							...
							/*value = year + "-" + ((mes.length() == 1) ? "0" : "") + mes + "-"
									+ ((dia.length() == 1) ? "0" : "") + dia;
							if ((year == null) || (mes == null) || (dia == null) || "--".equals(value))
								value = new String();
							
							//Ethiel, para rangos
							String diafinal = req.getParameter("_dayfinal_" + dsc[i].getNombreCampoLower());
							String mesfinal = req.getParameter("_mesfinal_" + dsc[i].getNombreCampoLower());
							String yearfinal = req.getParameter("_yearfinal_" + dsc[i].getNombreCampoLower());
							if ((!"".equals(yearfinal)) && (!"".equals(mesfinal)) && (!"".equals(diafinal)))
								value +="-"+ yearfinal + "-" + ((mesfinal.length() == 1) ? "0" : "") + mesfinal + "-"
								+ ((diafinal.length() == 1) ? "0" : "") + diafinal;
							//Ethiel, termina
					...
					String[] values = { String.valueOf(dsc[i].getIdTipoDatos()),
							String.valueOf(dsc[i].getIndiceTipo()), value };
			...
			if (dsc == null) {
				out.println("</head>");
				out
						.println("<body leftmargin=\"0\" topmargin=\"0\" rightmargin=\"0\" bottommargin=\"0\" marginwidth=\"0\" "
			...
			if (withGabinete) {
				...
				out.println("<tr>");
				out.println("<td>");
				out.println("<a href=\"javascript:if (confirm('Realmente desea eliminar este expediente?')){"
						+ "window.open('delexpedient?select=" + selectId + "&id_gabinete=" + fimx.getIdGabinete()
						+ "','main')}\" onmouseover=\"self.status='Eliminar Expediente'; return true;\">Eliminar");
				out.println("</a>");
				out.println("</td>");
				out.println("<td>");
				out.println("<a href=\"getdrawer?select=" + selectId + "&id_gabinete=" + fimx.getIdGabinete()
						+ "\" onmouseover=\"self.status='Modificar Expediente'; return true;\">Modificar");
				out.println("</a>");
				out.println("</td>");
				out.println("</tr>");
				out.println("</table>");
...
//					}
					...
					log.warn("No se recupero caso operacion para (usr[" + u.getLogin() + "], app["
							+ fimx.getTituloAplicacion() + "], idGab[" + data[i][0] + "])", exc);
					continue;
				...
				if (m.isEmpty()) {
					log.warn("No se recupero caso operacion para (usr[" + u.getLogin() + "], app["
							+ fimx.getTituloAplicacion() + "], idGab[" + data[i][0] + "])");
					continue;
				}
				for (int j = 1; j < data[i].length; j++) {
					...
					out.println("<td align=\"center\">"
							+ ((withGabinete) ? "" : "<a href=\"caso/show-caso?" + PRM_CASE + "=" + m.get("id_caso")
									+ "&" + PRM_CASE_OPER + "=" + m.get("id_caso_oper") + "\">")
							+ ((data[i][j] == null||(Integer.parseInt(data[i][0])==-1&&j==2)) ? "" : data[i][j]) + ((withGabinete) ? "" : "</a>") + "</td>");
				}
				out.println("</tr>");
			}
