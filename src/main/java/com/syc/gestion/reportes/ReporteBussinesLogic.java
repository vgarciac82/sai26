...
public class ReporteBussinesLogic extends DataSourceManager {
	...
	private static Logger	log				= Logger.getLogger(ReporteBussinesLogic.class);
	...
	public StringBuffer ReporteGeneral_(String registroFechaIni, String registroFechaFin, String rem_idarea, String rem_area, String rem_id, String rem_nombre, String rem_tipoInstruccion, String rem_estatus, String rem_prioridad, String res_idarea, String res_area, String res_id, String res_nombre,
	...
	// los campos "Recibidos de" / "Enviados a" del Reporte General.
		...
		try {
			...
			sql = "SELECT * FROM (  " + "\rSELECT " + "\r		bo.FOLIO AS FOLIO, " + "\r		imx.REFERENCIA, " + "\r		ca1.D_DESCRIPCION AS DESC_AREA_REM, " + "\r		bo.REMITENTE_AREA, " + "\r		ce1.CE_AP_PATERNO + ' ' + ce1.CE_AP_MATERNO + ' ' + ce1.CE_NOMBRE_COMPLETO AS NOMBRE_REM, "

			+ "\r		cerrado = CASE WHEN bo.TERMINADA = 'S' THEN  " + "\r					isNull( (select distinct " + "\r								CASE " + "\r			  						WHEN cosq.user03 ='' THEN 'NO VENCIDO' " + "\r			  						WHEN cosq.user03 =' 23:59:59' THEN 'NO VENCIDO'    "
				+ "\r			  						WHEN cosq.user03 > GETDATE() THEN 'NO VENCIDO' " + "\r			  						WHEN cosq.user03 < GETDATE() THEN 'VENCIDO' " + "\r			  					END   " + "\r							from cg_caso_operacion cosq, cg_bitacora_operacion bosq  " + "\r							where  "
				+ "\r								cosq.id_caso_oper = bosq.SECUENCIAL_OPERACION and " + "\r								cosq.id_caso = bosq.id_caso  " + "\r								and bosq.folio = bo.folio " + "\r								and bosq.responsable_id = bo.remitente_id   " + "\r								and bosq.remitente_id = bo.remitente_id   "
				+ "\r								and bosq.terminada = 'N'),'CONCLUIDO')  " + "\r				ELSE " + "\r			  		CASE " + "\r			  			WHEN co.user03 ='' THEN 'NO VENCIDO' " + "\r			  			WHEN co.user03 =' 23:59:59' THEN 'NO VENCIDO'     " + "\r			  			WHEN co.user03 > GETDATE() THEN 'NO VENCIDO' "
				+ "\r			  			WHEN co.user03 < GETDATE() THEN 'VENCIDO' " + "\r			  		END   " + "\r				END, "

				+ "\r			isNull(CASE            " + "\r				WHEN co.USER01 LIKE '%Procedente%' THEN CO.USER01  " + "\r				WHEN co.USER01 LIKE '%Grupal%' THEN CO.USER01  " + "\r				WHEN co.USER01 LIKE '%Coordinada%' THEN CO.USER01 "
				+ "\r				WHEN bo.id_operacion = 7 and co.USER01 LIKE '%Conocimiento%' THEN CO.USER01   " + "\r				WHEN bo.id_operacion = 7 and co.USER01 NOT LIKE '%Conocimiento%' THEN 'COPIA'   " + "\r				ELSE " + "\r					(  " + "\r					select user01 "
				+ "\r					from cg_caso_operacion cosq WITH(NOLOCK), cg_bitacora_operacion bosq WITH(NOLOCK) " + "\r					where cosq.id_caso_oper = bosq.SECUENCIAL_OPERACION  " + "\r						and cosq.id_caso = bosq.id_caso  " + "\r						and bosq.folio = bo.folio " + "\r						and cosq.id_caso_oper IN  "
				+ "\r						( " + "\r						   SELECT MAX(cossq.id_caso_oper) FROM cg_caso_operacion cossq WITH(NOLOCK) " + "\r						   WHERE cossq.id_caso = bosq.id_caso " + "\r								and cossq.id_caso_oper < bo.SECUENCIAL_OPERACION " + "\r								and isnull(cossq.user01,'') <> ''  " + "\r						) "
				+ "\r					) " + "\r				END , 'Registro') AS TIPO_INSTRUCCION,  " + "\r			  CASE WHEN imx.prioridad = 'N' THEN 'NORMAL' ELSE 'URGENTE' END as prioridad, " + "\r			  co.CO_OBSERVACION AS ASUNTO,				 "
				+ "\r			  CASE WHEN bo.id_operacion <> 3 THEN ce2.CE_AP_PATERNO + ' ' + ce2.CE_AP_MATERNO + ' ' + ce2.CE_NOMBRE_COMPLETO ELSE " + "\r			  (select ce22.CE_AP_PATERNO + ' ' + ce22.CE_AP_MATERNO + ' ' + ce22.CE_NOMBRE_COMPLETO "
				+ "\r			   from dbo.CG_BITACORA_OPERACION AS bo2 WITH(NOLOCK), dbo.CG_CAT_EMPLEADO AS ce22 WITH(NOLOCK) " + "\r			   where  bo.id_caso = bo2.id_caso  "
				+ "\r					and bo.secuencial_operacion+1 = bo2.secuencial_operacion  "
				+ "\r					and ce22.CE_OS_RESPONSABLE = bo2.RESPONSABLE_ID ) "
				+ "\r			   END as nombre_resp,	 "
				+ "\r			  CASE WHEN bo.id_operacion <> 3 THEN ca2.D_DESCRIPCION ELSE "
				+ "\r			  ( select cat_area.d_descripcion FROM CG_CAT_AREAS cat_area where  bo.responsable_area = cat_area.id_area ) "
				+ "\r			   END as desc_area_resp,	 "
				+ "\r	       CONVERT(varchar,bo.fecha_inicio,103) AS FECHAREGISTRO, "
				// +"\r	       co.USER03 AS FECHALIMITE,  "
				+ "\r           CONVERT(VARCHAR, CASE " + "\r			  WHEN co.user03 NOT LIKE '%__/__/____%' OR co.USER03 IS NULL THEN " + "\r				 CONVERT(VARCHAR, dbo.FnRptGenResFechaLimite(bo.ID_CASO, bo.SECUENCIAL_OPERACION), 103) " + "\r			  ELSE co.user03 "
				+ "\r		   END, 103) AS FECHALIMITE, "
				+ "\r           CONVERT(VARCHAR,CASE "
				+ "\r              WHEN bo.id_operacion = 2 THEN bo.FECHA_INICIO "
				+ "\r              ELSE dbo.FnRptGenResFechaEnvio(bo.ID_CASO, bo.SECUENCIAL_OPERACION) "
				+ "\r           END, 103) AS fechaenvio "
				// +"\r	       case when bo.id_operacion = 2 then CONVERT(varchar,bo.FECHA_INICIO,103) else null end as fechaenvio "
				+ "\r 	 FROM  " + "\r	       dbo.CG_CASO_OPERACION AS co WITH(NOLOCK) LEFT OUTER JOIN " + "\r	       dbo.CG_BITACORA_OPERACION AS bo WITH(NOLOCK) ON bo.ID_CASO = co.ID_CASO AND bo.SECUENCIAL_OPERACION = co.ID_CASO_OPER INNER JOIN "
				+ "\r	       dbo.CG_CASO AS c WITH(NOLOCK) ON c.ID_CASO = co.ID_CASO LEFT OUTER JOIN " + "\r	       dbo.IMXEXPEDIENTES AS imx WITH(NOLOCK) ON imx.ID_GABINETE = c.C_ID_GABINETE INNER JOIN "
				+ "\r	       dbo.CG_CAT_EMPLEADO AS ce2 WITH(NOLOCK)ON ce2.CE_OS_RESPONSABLE = co.CO_RESPONSABLE INNER JOIN " + "\r	       dbo.CG_CAT_AREAS AS ca2 WITH(NOLOCK) ON ce2.ID_AREA = ca2.ID_AREA INNER JOIN "
				+ "\r	       dbo.CAT_PUESTOS AS cp2 WITH(NOLOCK) ON ce2.ID_PUESTO = cp2.ID_PUESTO INNER JOIN " + "\r	       dbo.CG_CAT_EMPLEADO AS ce1 WITH(NOLOCK) ON ce1.CE_OS_RESPONSABLE = bo.REMITENTE_ID INNER JOIN "
				+ "\r	       dbo.CG_CAT_AREAS AS ca1 WITH(NOLOCK) ON ce1.ID_AREA = ca1.ID_AREA "

				+ "\r	 WHERE (c.C_FOLIO NOT LIKE 'TMP-%')  " + "\r	       AND bo.id_operacion IN (2,3,7,10,9,11)   "

				+ where

				+ "\r	       AND bo.SECUENCIAL_OPERACION IN ( " + "\r	               SELECT MAX(maxSO.secuencial_operacion) " + "\r	               FROM CG_BITACORA_OPERACION maxSO WITH(NOLOCK) INNER JOIN "
				+ "\r	                dbo.IMXEXPEDIENTES AS maxIMX WITH(NOLOCK) ON maxIMX.ID_GABINETE = maxSO.ID_GABINETE "

				+ "\r	               WHERE maxSO.folio = c.C_FOLIO " + whereSubQuery + "\r	 ) "

				+ "\r	) T WHERE T.TIPO_INSTRUCCION NOT IN ('COPIA')  " + where_2 + "\r	ORDER BY T.FOLIO  ";
			...
			if (gral.isEmpty()) {
				...
				for (Iterator<?> iter = gral.iterator(); iter.hasNext(); row++) {
					...
					tr = RenglonDatoGralHTML(a.getFolio(), a.getReferencia(), a.getArea(), a.getAreaRemitente(), a.getNombreRemitente(), a.getCerrado(), a.getTipoInstruccion(), a.getPrioridad(), a.getAsunto(), a.getResponsableAreaDesc(), a.getResponsableNombre(), a.getFechaRegistro(),
						a.getFechaEnvio(), a.getFechaLimite(), row,
	...
	/* ************************************************************************ */
	public String ReporteGeneral(String registroFechaIni, String registroFechaFin, String rem_idarea, String rem_area, String rem_id, String rem_nombre, String rem_tipoInstruccion, String rem_estatus, String rem_prioridad, String res_idarea, String res_area, String res_id, String res_nombre,
	// Esteban Badillo. Fecha: 11/Sep/2009.
	// Descripcion: Se agrega un campo que condiciona la proyeccion de
	// los campos "Recibidos de" / "Enviados a" del Reporte General.
		String orden) {
		...
		try {
			...
			sql = "SELECT" + "       vimx.folio," + "       vimx.referencia," + "       vimx.desc_area_rem, " + "		  vimx.remitente_area," + "		  vimx.nombre_rem," + "		  cerrado = " + "       CASE " + "           WHEN vimx.terminada = 'S' THEN 'CONCLUIDOS'"
				+ "           WHEN vimx.terminada = 'N' AND vimx.fecha_compromiso < GETDATE() THEN 'VENCIDOS'" + "           WHEN vimx.terminada = 'N' AND vimx.fecha_compromiso > GETDATE() THEN 'NO VENCIDOS'" + "       END, " + "		  IsNull(vimx.tipo_instruccion, '') as tipo_instruccion,"
				+ "		  CASE WHEN vimx.prioridad = 'N' THEN 'NORMAL' ELSE 'URGENTE' END as prioridad ," + "		  vimx.asunto," + "		  vimx.nombre_resp," + "		  vimx.desc_area_resp," + "       vimx.fecharegistro, "

				// Esteban Badillo. Fecha: 09/Ago/2009.
				// Descripcion: Se el campo fechalimite como fecha limite de
				// Fecha de e al Reporte General.
				+ "       vimx.fechalimite, " + "       case " + "          when vimx.id_operacion = 2 then vimx.fecha_inicio " + "          else null end as fechaenvio "

				+ "FROM VIMX_REPORTES vimx " + "WHERE vimx.ID_OPERACION IN (2,3) " // Ricardo:
																					// Mauricio
																					// indica
																					// que
																					// solo
	...
	public String ReporteHomoViati(String no_oficio, String fechaini, String fechafin, String area) {
		...
		try {
			conn = getConnection();
			String sql = "select g.no_oficio as no_oficio,g.solicitante as area,max(b.fecha)as fecha " + " from " + " (select no_oficio,solicitante,id_gabinete from imxhomoviati where no_oficio not like 'HV-%') g," + " (select b_c_fecha_ini as fecha, b_c_id_gabinete as bgab  from cg_bitacora) b"
				+ " where " + " g.id_gabinete=b.bgab " + (fechaini != null && fechafin != null ? " and b.fecha BETWEEN convert(datetime,'" + fechaini + "') and convert(datetime,'" + fechafin + "') " : "")
				+ (no_oficio != null && !"".equals(no_oficio) ? " and g.no_oficio like '%" + no_oficio + "%'" : "") + (area != null && !"".equals(area) ? " and g.solicitante like '" + area + "'" : "") + " group by  g.no_oficio,g.solicitante";
	...
	public String ReportePresupuesto(String ep, String EjercicioFiscal, String Cuenta, String UnidadEjecutora, String RamoEP, String UnidadResponsableEP, String GrupoFuncional, String Funcion, String SubFuncion, String ProgramaGeneral, String ProgramaPresupuestario, String ActividadInstitucional,
		String Partida, String TipoGasto, String FuenteFinanciamiento, String EntidadFederativa, String Cartera, String UnidadNorativa, String ClaveCNA, String cOrddeBy) {
		Connection conn = null;
		StringBuffer sb = new StringBuffer();
		...
		try {
			conn = getConnection();
			String sql = "SELECT  s.nClaveCNA,  s.ClaveSIAFF, s.ClaveInterna, s.MontoEnero, s.MontoFebrero, s.MontoMarzo, s.MontoAbril, s.MontoMayo, s.MontoJunio, s.MontoJulio, " + "           s.MontoAgosto, s.MontoSeptiembre, s.MontoOctubre, s.MontoNoviembre, s.MontoDiciembre, s.MontoAnual "
				+ "     FROM  vSaldosAnuales s with(nolock) " +
				// "           tCatalogoUnidadResponsable u  "+
			...
			sql += (ep != "" && ep != null ? "AND s.ClaveSIAFF = '" + ep + "'" : ""); // tenia
																						// s.cSubCuenta
																						// like
																						// '%"+ep+"%'":"")
			...
			if (saldoList.isEmpty()) {
				...
				for (Iterator<?> iter = saldoList.iterator(); iter.hasNext(); row++) {
					...
					tr = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + ">" + "<td>" + epSaldo.getClaveCNA() + "</td>" + "<td>" + epSaldo.getClaveSIAFF() + "</td>" + "<td>" + epSaldo.getClaveInterna() + "</td>" + "<td>" + epSaldo.getMontoEnero() + "</td>" + "<td>"
						+ epSaldo.getMontoFebrero() + "</td>" + "<td>" + epSaldo.getMontoMarzo() + "</td>" + "<td>" + epSaldo.getMontoAbril() + "</td>" + "<td>" + epSaldo.getMontoMayo() + "</td>" + "<td>" + epSaldo.getMontoJunio() + "</td>" + "<td>" + epSaldo.getMontoJulio() + "</td>" + "<td>"
						+ epSaldo.getMontoAgosto() + "</td>" + "<td>" + epSaldo.getMontoSeptiembre() + "</td>" + "<td>" + epSaldo.getMontoOctubre() + "</td>" + "<td>" + epSaldo.getMontoNoviembre() + "</td>" + "<td>" + epSaldo.getMontoDiciembre() + "</td>" + "<td>" + epSaldo.getMontoAnual()
						+ "</td>" +
	...
	public List MultiReportePresupuestal(String EPs, String ProgramaPresupuestario, String Partida, String UnidadResponsableEP, String UnidadEjecutora, String Cartera, String ChkReintegros, String ChkRectificaciones, String ChkAdecuaciones, String ChkPagosAnticipados, String ChkPlurianuales,
		String ChkOriginal, String ChkModificado, String ChkAmpAutorizada, String ChkRedAutorizada, String ChkAmpenTramite, String ChkRedenTramite, String ChkReienTramite, String ChkRectificacion, String ChkRedSHCPenTramite, String ChkRedSHCPAplicada, String ChkApartado, String ChkPrecomprometido,
		String ChkComprometido, String ChkDevengado, String ChkEjernoPagado, String ChkDisponibleNeto, String ChkDisponibleBruto, String ChkEjercidoPagado, String sULogin) {
		Connection conn = null;
		...
		try {
			...
			ReportePrespuestalList = ReporteManager.callMultiReportePresupuestal(conn, EPs, ProgramaPresupuestario, Partida, UnidadResponsableEP, UnidadEjecutora, Cartera, ChkReintegros, ChkRectificaciones, ChkAdecuaciones, ChkPagosAnticipados, ChkPlurianuales, ChkOriginal, ChkModificado,
				ChkAmpAutorizada, ChkRedAutorizada, ChkAmpenTramite, ChkRedenTramite, ChkReienTramite, ChkRectificacion, ChkRedSHCPenTramite, ChkRedSHCPAplicada, ChkApartado, ChkPrecomprometido, ChkComprometido, ChkDevengado, ChkEjernoPagado, ChkDisponibleNeto, ChkDisponibleBruto,
				ChkEjercidoPagado, sULogin);
	...
	public String MultiReporteResultado(String ep, String EjercicioFiscal, String Cuenta, String UnidadEjecutora, String RamoEP, String UnidadResponsableEP, String GrupoFuncional, String Funcion, String SubFuncion, String ProgramaGeneral, String ProgramaPresupuestario,
		String ActividadInstitucional, String Partida, String TipoGasto, String FuenteFinanciamiento, String EntidadFederativa, String Cartera, String UnidadNormativa, String ClaveCNA, String cOrddeBy, String cGroupBy, String InfoRegMes, String TipoReporte, String Usuario, String Filtro,
		String nCtasPresup[], String dCtasPresup[], String Componentes, String Resumen, String tipoEP, String Capitulo) throws Exception {
		Connection conn = null;
		StringBuffer sb = new StringBuffer();
		String opcion = "";
		String unidadResp = "";
		...
		try {
			...
			log.debug("EXECUTE sp_pMultiReporte_syc '" + sql.replace("'", "''") + "', '" + Cuenta.replace("'", "''") + "', '" + cOrddeBy.replace("'", "''") + "', '" + cGroupBy.replace("'", "''") + "', '" + InfoRegMes.replace("'", "''") + "', '" + TipoReporte.replace("'", "''") + "', '"
				+ Usuario.replace("'", "''") + "', @Salida OUTPUT; ");
			...
			if (saldoList.isEmpty()) {
				...
				for (Iterator<?> iter = saldoList.iterator(); iter.hasNext(); row++) {
					...
					// log.debug("tr " + tr);
					if (!TipoReporte.equals("GENERAL")) {
						tr = tr + "<td align=\"right\">" + ReporteBussinesLogic.formateaNumero(epSaldo.getMontoEnero()) + "</td>" + "<td align=\"right\">" + ReporteBussinesLogic.formateaNumero(epSaldo.getMontoFebrero()) + "</td>" + "<td align=\"right\">"
							+ ReporteBussinesLogic.formateaNumero(epSaldo.getMontoMarzo()) + "</td>" + "<td align=\"right\">" + ReporteBussinesLogic.formateaNumero(epSaldo.getMontoAbril()) + "</td>" + "<td align=\"right\">" + ReporteBussinesLogic.formateaNumero(epSaldo.getMontoMayo()) + "</td>"
							+ "<td align=\"right\">" + ReporteBussinesLogic.formateaNumero(epSaldo.getMontoJunio()) + "</td>" + "<td align=\"right\">" + ReporteBussinesLogic.formateaNumero(epSaldo.getMontoJulio()) + "</td>" + "<td align=\"right\">"
							+ ReporteBussinesLogic.formateaNumero(epSaldo.getMontoAgosto()) + "</td>" + "<td align=\"right\">" + ReporteBussinesLogic.formateaNumero(epSaldo.getMontoSeptiembre()) + "</td>" + "<td align=\"right\">" + ReporteBussinesLogic.formateaNumero(epSaldo.getMontoOctubre())
							+ "</td>" + "<td align=\"right\">" + ReporteBussinesLogic.formateaNumero(epSaldo.getMontoNoviembre()) + "</td>" + "<td align=\"right\">" + ReporteBussinesLogic.formateaNumero(epSaldo.getMontoDiciembre()) + "</td>";
					}
					;
					tr = tr + "</tr>";
					sb.append(tr);
	...
	public String MultiReporteContableResultado(String ep, String EjercicioFiscal, String Cuenta, String UnidadEjecutora, String RamoEP, String UnidadResponsableEP, String GrupoFuncional, String Funcion, String SubFuncion, String ProgramaGeneral, String ProgramaPresupuestario,
		String ActividadInstitucional, String Partida, String TipoGasto, String FuenteFinanciamiento, String EntidadFederativa, String Cartera, String UnidadNormativa, String ClaveCNA, String cOrddeBy, String cGroupBy, String InfoRegMes, String TipoReporte, String Usuario, String Filtro,
		String nCtasPresup[], String cCentroContable, String Componentes, String Resumen) {
		...
		try {
			...
			log.debug("EXECUTE sp_pMultiReporteContable_syc '" + sql.replace("'", "''") + "', '" + Cuenta.replace("'", "''") + "', '" + cOrddeBy.replace("'", "''") + "', '" + cGroupBy.replace("'", "''") + "', '" + InfoRegMes.replace("'", "''") + "', '" + TipoReporte.replace("'", "''") + "', '"
				+ Usuario.replace("'", "''") + "', '" + cCentroContable.replace("'", "''") + "', " + "@Salida OUTPUT; ");
	...
	public String ContraCuentasResultado(String PolCtroContable, String PolEjercicioFiscal, String PolStatus, String PolFechCapturaIni, String PolFechCapturaFin, String PolTipo, String PolFechAplicacionIni, String PolFechAplicacionFin, String PolAutorizo, String PolNumeroIni, String PolNumeroFin,
		String PolMontoIni, String PolMontoFin, String PolOrigen, String PolAutomatica) {
		Connection conn = null;
		StringBuffer sb = new StringBuffer();
		...
		try {
			...
			// String sql = "";
			String sql = "select m.aEjercicioFiscal, " + "m.cCentroContable," + "m.cTipoPoliza, " + "m.nFolioPoliza,  " + "ROW_NUMBER() OVER(ORDER BY nConsecutivoMovimiento ASC) nDocRenglon, " + "p.nMes,  " + "m.nCuenta, " + "m.cTipoMovimiento, " + "m.mMovimiento, " + "p.cDescripcionPoliza "
				+ "from tMovimiento m " + "inner join (" + "select m.cCentroContable, " + "m.cTipoPoliza, m.nFolioPoliza, " + "m.nDocRenglon  , " + "m.nCuenta, " + "m.mMovimiento, " + "p.cDescripcionPoliza, " + "p.nMes " + "from tMovimiento m "
				+ "inner join tPoliza p on p.cCentroContable=m.cCentroContable and p.cTipoPoliza=m.cTipoPoliza and p.nFolioPoliza=m.nFolioPoliza " + "where m.nCuenta like  ''11301%''') p on p.cCentroContable=m.cCentroContable and p.cTipoPoliza=m.cTipoPoliza and p.nFolioPoliza=m.nFolioPoliza "
				+ "where m.cCentroContable=''10'' and m.cTipoPoliza='''EG'' and m.nFolioPoliza=''12''";
			...
			if (polizaList.isEmpty()) {
				...
				for (Iterator<?> iter = polizaList.iterator(); iter.hasNext(); row++) {
					...
					tr = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + ">";
					tr += "<td align=\"center\"><li id=\"tipotrabajo\" name=\"tipotrabajo\"><input id=\"selection\" name=\"selection\" value=\"" + "(v.nFolioPoliza=" + cPoliza.getNfolioPoliza() + " and v.cCentroContable='" + cPoliza.getCcentroContable() + "' and v.cTipoPoliza='"
						+ cPoliza.getCtipoPoliza() + "')" + "\" align=\"center\" type=\"checkbox\"></li></td>";
	...
	public String ConsultaPolizasResultado(String PolCtroContable, String PolEjercicioFiscal, String PolStatus, String PolFechCapturaIni, String PolFechCapturaFin, String PolTipo, String PolFechAplicacionIni, String PolFechAplicacionFin, String PolAutorizo, String PolNumeroIni, String PolNumeroFin,
		String PolMontoIni, String PolMontoFin, String PolOrigen, String PolAutomatica) {
		Connection conn = null;
		StringBuffer sb = new StringBuffer();
		...
		try {
			...
			String sql = "SELECT p.nFolioPoliza ,CONVERT(varchar(10), p.fCreacion, 103) fCreacion , isnull(p.cDescripcionPoliza,'') cDescripcionPoliza ,CONVERT(varchar(50), CONVERT(money,p.mTotalCargo), 1) as mTotalCargo ,CONVERT(varchar(50),CONVERT(money,p.mTotalAbono), 1) as mTotalAbono ,p.nMes ,p.nCuenta ,CONVERT(varchar(10), p.fAplicacion, 103) fAplicacion ,"
				+ " p.nPolizaAutomatica ,p.cCentroContable cCentroContable,ccc.cDescripcion cDescripcionCentroContable  ,p.aEjercicioFiscal, p.cTipoPoliza cTipoPoliza ,rtrim(ctp.cDescripcionPoliza) cDescripcionTipoPoliza ,p.cTipoDocumento ,p.nFolioDocumento ,"
				+ " p.DocHAplicado, isnull(p.cUsuarioAutorizo,'') cUsuarioAutorizo"
				+ " FROM vPolizaMovimientosContables p  WITH (NOLOCK) "
				+ " INNER JOIN tCatalogoTipoPoliza ctp  WITH (NOLOCK) ON ctp.cTipoPoliza=p.cTipoPoliza "
				+ " INNER JOIN tCatalogoCentroContable ccc  WITH (NOLOCK) on ccc.cCentroContable=p.cCentroContable " + " WHERE 1=1 ";
			...
			if (polizaList.isEmpty()) {
				...
				for (Iterator<?> iter = polizaList.iterator(); iter.hasNext(); row++) {
					...
					tr = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + ">";
					tr += "<td align=\"center\"><li id=\"tipotrabajo\" name=\"tipotrabajo\"><input id=\"selection\" name=\"selection\" value=\""
					// + "(v.nFolioPoliza="
					...
					// + "'')"
						+ "(" + cPoliza.getNfolioPoliza() + ",''" + cPoliza.getCcentroContable() + "'',''" + cPoliza.getCtipoPoliza() + "'')"

						+ "\" align=\"center\" type=\"checkbox\"></li></td>";
					tr += "<td align=\"center\">" + cPoliza.getCdescripcionCentroContable() + "</td>";
					tr += "<td align=\"center\">" + cPoliza.getCdescripcionTipoPoliza() + "</td>";
					tr += "<td align=\"center\">" + cPoliza.getNfolioPoliza() + "</td>";
					tr += "<td align=\"center\">" + cPoliza.getFcreacion() + "</td>";
	...
	public String ConsultaPolizasResultadoJSON(String PolCtroContable, String PolEjercicioFiscal, String PolStatus, String PolFechCapturaIni, String PolFechCapturaFin, String PolTipo, String PolFechAplicacionIni, String PolFechAplicacionFin, String PolAutorizo, String PolNumeroIni,
		String PolNumeroFin, String PolMontoIni, String PolMontoFin, String PolOrigen, String PolAutomatica) {
		Connection conn = null;
		StringBuffer sb = new StringBuffer();
		...
		try {
			...
			if (PolAutomatica.equals("")) {
				sql = "SELECT p.nFolioPoliza ,CONVERT(varchar(10), p.fCreacion, 103) fCreacion , isnull(p.cDescripcionPoliza,'') cDescripcionPoliza "
					+ ",CONVERT(varchar(50), CONVERT(money,p.mTotalCargo), 1) as mTotalCargo ,CONVERT(varchar(50),CONVERT(money,p.mTotalAbono), 1) as mTotalAbono , "
					+ "p.nMes ,p.nCuenta ,CONVERT(varchar(10), p.fAplicacion, 103) fAplicacion , p.nPolizaAutomatica ,p.cCentroContable cCentroContable, "
					+ "ccc.cDescripcion cDescripcionCentroContable  ,p.aEjercicioFiscal, p.cTipoPoliza cTipoPoliza  "
					+ ",rtrim(ctp.cDescripcionPoliza) cDescripcionTipoPoliza ,p.cTipoDocumento ,p.nFolioDocumento ,ISNULL( pd.cDocHAplicado,'A')as cDocHAplicado, "
					+ "isnull(p.cUsuarioAutorizo,'') cUsuarioAutorizo,P.ID_OPER  "
					+ "FROM vConsultaPoliza p  WITH (NOLOCK) "
					+ "LEFT JOIN tCatalogoTipoPoliza ctp  WITH (NOLOCK) "
					+ "ON ctp.cTipoPoliza=p.cTipoPoliza  "
					+ "LEFT JOIN tCatalogoCentroContable ccc  WITH (NOLOCK) "
					+ "on ccc.cCentroContable=p.cCentroContable "
					+ "LEFT join "
					+ "(select tpd.nFolioPoliza,tpd.nFolioPagoDirecto,tpd.cIdEntidadContable,tpd.cDocumentoHaplicado as cDocHAplicado, 'PAGODIRECTO' tipoDoc, tpd.cTipoPoliza tipoPoliza from tPagoDirectoEncabezado as tpd  WITH (NOLOCK)  "
					+ "union  "
					+ "select tapd.nFolioPoliza,tapd.nFolioPAGODIVERSO,tapd.cCentroContable,tapd.cDocumentoHaplicado as cDocHAplicado, 'PAGODIVERSO' tipoDoc, tapd.cTipoPoliza tipoPoliza from tPagoDiversoEncabezado as tapd  WITH (NOLOCK)  "
					+ "union  "
					+ "select pf.nFolioPoliza ,pf.nFolioPAGODIVERSO,pf.cCentroContable,pf.cDocumentoHaplicado as cDocHAplicado, 'PAGOFEDERALIZADO' tipoDoc, pf.cTipoPoliza  tipoPoliza from tPAGODIVERSOEncabezado as pf  WITH (NOLOCK)  "
					+ "union  "
					+ "select po.nFolioPoliza ,po.nFolioPAGOOBRA,po.cCentroContable,po.cDocumentoHaplicado as cDocHAplicado, 'PAGOOBRA' tipoDoc, po.cTipoPoliza tipoPoliza from tPAGOOBRAEncabezado as po  WITH (NOLOCK) "
					+ "union  "
					+ "select rg.nFolioPoliza, rg.nFolioRELACIONGASTOS,rg.cCentroContable,rg.cDocumentoHaplicado as cDocHAplicado, 'RELACIONGASTOS' tipoDoc, rg.cTipoPoliza tipoPoliza from tRELACIONGASTOSEncabezado as rg  WITH (NOLOCK) "
					+ "union  "
					+ "select distinct ae.nFolioPoliza ,ae.nFolioAdecuacionaut,ad.cCentroContable,ae.cDocumentoHaplicado as cDocHAplicado, 'ADECUACIONAUT' tipoDoc,ae.cTipoPoliza tipoPoliza from tADECUACIONAUTEncabezado ae  WITH (NOLOCK),tAdecuacionAutDetalle ad  WITH (NOLOCK) where ae.nFolioAdecuacionaut=ad.nFolioAdecuacionaut  "
					+ "union  "
					+ "select distinct ae.nFolioPoliza ,ae.nFolioAdecuacion,ad.cCentroContable,ae.cDocumentoHaplicado as cDocHAplicado, 'ADECUACION' tipoDoc,ae.cTipoPoliza tipoPoliza from tADECUACIONEncabezado ae  WITH (NOLOCK),tAdecuacionDetalle ad  WITH (NOLOCK) where ae.nFolioAdecuacion=ad.nFolioAdecuacion  "
					+ "union  "
					+ "select distinct ae.nFolioPoliza ,ae.nFolioApartado,ad.cCentroContable,ae.cDocumentoHaplicado as cDocHAplicado, 'APARTADO' tipoDoc,ae.cTipoPoliza tipoPoliza from tApartadoEncabezado ae  WITH (NOLOCK),tApartadoDetalle ad  WITH (NOLOCK) where ae.nFolioApartado=ad.nFolioApartado  "
					+ "union  "
					+ "select distinct ae.nFolioPoliza ,ae.nFolioCancelaCompromiso,ad.cCentroContable,ae.cDocumentoHaplicado as cDocHAplicado, 'COMPROMISO' tipoDoc,ae.cTipoPoliza tipoPoliza from tCancelaCompromisoEncabezado ae  WITH (NOLOCK),tCancelaCompromisoDetalle ad  WITH (NOLOCK) where ae.nFolioCancelaCompromiso=ad.nFolioCancelaCompromiso "
					+ "union  "
					+ "select distinct ae.nFolioPoliza ,ae.nFolioReintegro,ad.cCentroContable,ae.cDocumentoHaplicado as cDocHAplicado, 'REINTEGRO' tipoDoc,ae.cTipoPoliza tipoPoliza from tReintegroEncabezado ae  WITH (NOLOCK),tReintegroDetalle ad  WITH (NOLOCK) where ae.nFolioReintegro=ad.nFolioReintegro "
					+ "union  "
					+ "select distinct ae.nFolioPoliza ,ae.nFolioReintegroaut,ad.cCentroContable,ae.cDocumentoHaplicado as cDocHAplicado, 'REINTEGROAUT' tipoDoc,ae.cTipoPoliza tipoPoliza from tReintegroAutEncabezado ae  WITH (NOLOCK),tReintegroAutDetalle ad  WITH (NOLOCK) where ae.nFolioReintegroaut=ad.nFolioReintegroaut "
					+ "union  "
					+ "select nFolioPoliza ,nFolioNomina,cCentroContable,cDocumentoHaplicado as cDocHAplicado,'NOMINA' tipoDoc,cTipoPoliza tipoPoliza from tNOMINAEncabezado  WITH (NOLOCK)  "
					+ "union  "
					+ "select ch.nFolioPoliza ,ch.nFolioCheque,ch.cCentroContable,ch.cDocumentoHaplicado as cDocHAplicado, 'CHEQUE' tipoDoc,ch.cTipoPoliza from tChequeEncabezado as ch  WITH (NOLOCK)  "
					+ "union  "
					+ "select nFolioPoliza ,nFolioCompromisoNomina,cCentroContable,cDocumentoHaplicado as cDocHAplicado,'COMPROMISO' tipoDoc,cTipoPoliza tipoPoliza from tCompromisoNominaEncabezado  WITH (NOLOCK)  "
					+ "union   "
					+ "select nFolioPoliza ,nFolioCompromiso,cCentroContable,cDocumentoHaplicado as cDocHAplicado,'COMPROMISO' tipoDoc, cTipoPoliza tipoPoliza from tCompromisoEncabezado  WITH (NOLOCK)  "
					+ "union  "
					+ "SELECT distinct  pde.nFolioPoliza,pdd.nFolioPasivoDiferido,pdd.ccentrocontable,pde.cDocumentoHaplicado as cDocHAplicado, 'PASIVODIFERIDO' tipoDoc, pde.cTipoPoliza tipoPoliza FROM tPasivoDiferidoEncabezado pde WITH (NOLOCK),tPasivoDiferidoDetalle pdd  WITH (NOLOCK) where pde.nFolioPasivoDiferido=pdd.nFolioPasivoDiferido "
					+ "union  "
					+ "SELECT distinct  ee.nFolioPoliza,ed.nFolioEjercido,ed.ccentrocontable,ee.cDocumentoHaplicado as cDocHAplicado, 'EJERCIDO' tipoDoc, ee.cTipoPoliza tipoPoliza FROM tEjercidoEncabezado ee  WITH (NOLOCK),tEjercidoDetalle ed  WITH (NOLOCK) where ee.cTipoPago=ed.cTipoPago	and  ee.nFolioPAGO=ed.nFolioPAGO and ee.nFolioEjercido=ed.nFolioEjercido "
					+ "union  "
			...
			} // termina para polizas manuales y automaticas

			else if (PolAutomatica.equals("1")) // checa si es poliza automatica

			{
				...
				sql = "SELECT p.nFolioPoliza ,CONVERT(varchar(10), p.fCreacion, 103) fCreacion , isnull(p.cDescripcionPoliza,'') cDescripcionPoliza "
					...
					+ "select ch.nFolioPoliza ,ch.nFolioCheque,ch.cCentroContable,ch.cDocumentoHaplicado as cDocHAplicado, 'CHEQUE' tipoDoc,ch.cTipoPoliza from tChequeEncabezado as ch  WITH (NOLOCK)  "
					+ "union  "
					+ "select nFolioPoliza ,nFolioCompromisoNomina,cCentroContable,cDocumentoHaplicado as cDocHAplicado,'COMPROMISONOMINA' tipoDoc,cTipoPoliza tipoPoliza from tCompromisoNominaEncabezado  WITH (NOLOCK)  "
					+ "union   "
					+ "select nFolioPoliza ,nFolioCompromiso,cCentroContable,cDocumentoHaplicado as cDocHAplicado,'COMPROMISO' tipoDoc, cTipoPoliza tipoPoliza from tCompromisoEncabezado  WITH (NOLOCK)  "
					+ "union  " 					
					+ "SELECT distinct  pde.nFolioPoliza,pdd.nFolioPasivoDiferido,pdd.ccentrocontable,pde.cDocumentoHaplicado as cDocHAplicado, 'PASIVODIFERIDO' tipoDoc, pde.cTipoPoliza tipoPoliza FROM tPasivoDiferidoEncabezado pde WITH (NOLOCK),tPasivoDiferidoDetalle pdd  WITH (NOLOCK) where pde.nFolioPasivoDiferido=pdd.nFolioPasivoDiferido "
				...
				sql += (!"".equals(PolAutorizo) && PolAutorizo != null ? " AND cUsuarioAutorizo = '" + PolAutorizo + "'" : "");
				if (PolStatus.equals("A")) { // / checar tipo polstatus
					sql += (!"".equals(PolStatus) && PolStatus != null ? " AND P.ID_OPER=4  and cDocHAplicado!='C'"

					: "");
				} else if (PolStatus.equals("C")) { // / checar tipo polstatus
					sql += (!"".equals(PolStatus) && PolStatus != null ? " AND cDocHAplicado = '" + PolStatus + "'" : "");
				} else {
			...
			for (Iterator<?> iter = polizaList.iterator(); iter.hasNext(); row++) {
				...
				arrJSON += "\"" + "<input  id=\\\"selection\\\" " + "name=\\\"selection\\\" " + "value=\\\""
				...
				// + "'')"
					...
					+ "[" + cPoliza.getNfolioPoliza() + ",''" + cPoliza.getCcentroContable() + "'',''" + cPoliza.getCtipoPoliza() + "'',''" + cPoliza.getDocHAplicado() + "'']"
				...
				if (cPoliza.getNpolizaAutomatica().equals("0")) {
					arrJSON += "\""
						+ (((cPoliza.getIdOper().equals("4")) && (cPoliza.getDocHAplicado().equals("C"))) ? "CANCELADA" : cPoliza.getIdOper().equals("1") && (cPoliza.getDocHAplicado().equals("E")) ? "EN CAPTURA" // CHECAR
																																																					// ID_OPER
							: cPoliza.getIdOper().equals("0") && (cPoliza.getCdescripcionPoliza().equals("Cancelación de DOCPOLIZA")) ? "AUTORIZADA APLICADA" : cPoliza.getIdOper().equals("0") ? "EN REVISION" : cPoliza.getIdOper().equals("2") ? "EN REVISION"
								: cPoliza.getIdOper().equals("3") ? "EN AUTORIZACION" : cPoliza.getIdOper().equals("4") ? "EN CONSULTA" : "") + "\",";
				} else {
	...
	public String ReporteBalanza(String buscaCuentaIni, String buscaCuentaFin, String aEjercicioFiscal, String DepuraLineas, String DepuraColumnas, String cCentroContable, String TipoReporte, String mesIni, String mesFin, String FiltroSubcuenta) {
		...
		try {
			...
			log.debug("EXECUTE sp_tBalanzaAnalitico_syc @Salida OUTPUT, '" + buscaCuentaIni + "', '" + buscaCuentaFin + "', '" + aEjercicioFiscal + "', '" + DepuraLineas + "', '" + DepuraColumnas + "', '" + cCentroContable + "', '" + TipoReporte + "', '" + mesIni + "', '" + mesFin + "', "
				+ FiltroSubcuenta + "'; ");
				...
				for (Iterator<?> iter = balanzaList.iterator(); iter.hasNext(); row++) {
					...
					if (rBalanza.getNcuenta() != null && !rBalanza.getNcuenta().trim().equals("")) {
						...
						if (DepuraColumnas.equals("N")) {
							tr += "<td style='mso-number-format:\"Standard\"'>" + rBalanza.getSaldoInicial() + "</td>" + "<td style='mso-number-format:\"Standard\"'>" + rBalanza.getMovimientosAcumuladosDebe() + "</td>" + "<td style='mso-number-format:\"Standard\"'>"
								+ rBalanza.getMovimientosAcumuladosHaber() + "</td>" + "<td style='mso-number-format:\"Standard\"'>" + rBalanza.getSaldoMesAnterior() + "</td>" + "<td style='mso-number-format:\"Standard\"'>" + rBalanza.getMovimientosMesDebe() + "</td>"
								+ "<td style='mso-number-format:\"Standard\"'>" + rBalanza.getMovimientosMesHaber() + "</td>" + "<td style='mso-number-format:\"Standard\"'>" + rBalanza.getSaldoFinal() + "</td>";
						} else {
							tr += "<td style='mso-number-format:\"Standard\"'>" + rBalanza.getSaldoInicialDeudor() + "</td>" + "<td style='mso-number-format:\"Standard\"'>" + rBalanza.getSaldoInicialAcreedor() + "</td>" + "<td style='mso-number-format:\"Standard\"'>"
								+ rBalanza.getMovimientosMesDebe() + "</td>" + "<td style='mso-number-format:\"Standard\"'>" + rBalanza.getMovimientosMesHaber() + "</td>" +
								// "<td>"+rBalanza.getMovimientosAcumuladosDebe()+"</td>"+
	 ...
	 */
	public ArrayList<String> multiReporteObra(String cCentroContable, String fechaI, String fechaF, String idunidadresponsable, boolean generarVacio, String reportBody, String strUsuario, String strCondicion, String strCondMultiR, String columnasBorrar, String NomCols[], String ForCols[],
		ArrayList Todo) {
		...
		try {
			...
			if (auxiliaresList.isEmpty()) {
				...
				for (Iterator<?> iter = auxiliaresList.iterator(); iter.hasNext(); row++) {
					...
					if (rAuxiliares.getAuxTipo() != null
					// && !rAuxiliares.getAuxTipo().trim().equals("")
					) {
						// if (rAuxiliares.getNcuenta() != null
						// && !rAuxiliares.getNcuenta().trim().equals("")) {
						tr = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + ">" + "<td>" + rAuxiliares.getAuxTipo() + "</td>" + "<td>" + (rAuxiliares.getAuxNum().equals("0") ? "" : rAuxiliares.getAuxNum()) + "</td>" + "<td>" + rAuxiliares.getAuxCxP() + "</td>"
							+ "<td style='mso-number-format:\"dd/mm/yyyy\"'>" + rAuxiliares.getAuxFecha() + "</td>" + "<td>" + rAuxiliares.getAuxConcepto() + "</td>" + "<td>" + rAuxiliares.getAuxReferencia() + "</td>" + "<td style='mso-number-format:\"Standard\"'>" + rAuxiliares.getAuxCargos()
							+ "</td>" + "<td style='mso-number-format:\"Standard\"'>" + rAuxiliares.getAuxAbonos() + "</td>" + "<td style='mso-number-format:\"Standard\"'>" + rAuxiliares.getAuxSaldo() + "</td>"
		...
		try {
			...
			if (auditoria.isEmpty()) {
				...
				for (Iterator<?> iter = auditoria.iterator(); iter.hasNext(); row++) {
					...
					tr = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + "><td>&nbsp;</td>" + "<td>" + a.getNombre_modulo() + "</td>" + "<td>" + a.getAccion() + "</td>" + "<td>" + a.getFecha_operacion() + "</td>" + "<td>" + a.getValor_origen() + "</td>" + "<td>"
						+ a.getValor_destino() + "</td>" + "<td>" + a.getUsuario() + "</td>" + "<td>" + a.getNombre_usuario() + "</td>" + "<td>" + a.getArea_usuario() + "</td>" + "<td>" + a.getArea_padre() + "</td>" + "</tr>";
	...
	public String ReportePorArea(String area, String estructura, String regfechaini, String regfechafin, boolean detallado) {
		...
		try {
			 ...
			 * reporte.
			 */
			+ "SELECT DISTINCT " + "   t.id_caso, " + "   t.folio, " + "   t.referencia, " + "   t.nombre_rem, " + "   t.desc_area_rem, " + "   cerrado = CASE " + "      WHEN t.terminada = 'S' THEN 'CONCLUIDOS' " + "      WHEN t.terminada = 'N' AND t.fecha_compromiso < GETDATE() THEN 'VENCIDOS' "
				+ "      WHEN t.terminada = 'N' AND t.fecha_compromiso > GETDATE() THEN 'NO VENCIDOS' END, " + "   CASE " + "      WHEN t.prioridad = 'N' THEN 'NORMAL' " + "      ELSE 'URGENTE' END as prioridad, " + "   t.fecha_compromiso as fechalimite, " + "   t.responsable_area, "
				+ "   t.desc_area_resp, " + "   ca.area_estructura " + "INTO #TMP_VENCIDOS " + "FROM " + "   ( " + "   SELECT " + "	     vimx.* " + "   FROM " + "	     vimx_reportes vimx " + "   WHERE " + "	     1 = 1 " + "	     AND vimx.id_operacion in(2,3,7, 9,10,11) "
				+ "	     AND vimx.terminada = 'N' " + "	     AND vimx.fecha_compromiso < GETDATE() " + "	     AND vimx.titular = 'S' " + "	     "
				+ whereFechas
				+ "   ) t "
				+ "LEFT OUTER JOIN "
				+ "   CG_CAT_AREAS ca "
				+ "   ON (ca.id_area = t.responsable_area) "
				+ "WHERE "
				+ "   t.desc_area_resp IS NOT NULL "
				+ "SELECT DISTINCT "
				+ "   t.id_caso, "
				+ "   t.folio, "
				+ "   t.referencia, "
				...
				+ "   t.desc_area_rem, "
				+ "   cerrado = CASE "
				+ "      WHEN t.terminada = 'S' THEN 'CONCLUIDOS' "
				+ "      WHEN t.terminada = 'N' AND t.fecha_compromiso < GETDATE() THEN 'VENCIDOS' "
				+ "      WHEN t.terminada = 'N' AND t.fecha_compromiso > GETDATE() THEN 'NO VENCIDOS' END, "
				+ "   CASE "
				+ "      WHEN t.prioridad = 'N' THEN 'NORMAL' "
				+ "      ELSE 'URGENTE' END as prioridad, "
				+ "   t.fecha_compromiso as fechalimite, "
				+ "   t.responsable_area, "
				+ "   t.desc_area_resp, "
				+ "   ca.area_estructura "
				+ "INTO #TMP_NO_VENCIDOS "
				+ "FROM "
				+ "   ( "
				+ "   SELECT "
				+ "      vimx.* "
				...
				+ "WHERE "
				+ "   t.desc_area_resp IS NOT NULL "
				+ "SELECT * INTO #TMP_TODOS FROM #TMP_NO_VENCIDOS "
				+ "UNION ALL "
				+ "SELECT * FROM #TMP_VENCIDOS "
				+ "UNION ALL "
				+ "SELECT * FROM #TMP_CONCLUIDOS "
				+ "SELECT "
				+ "   responsable_area, "
				+ "   area_estructura, "
				+ "   desc_area_resp, "
				+ "   0 as casos, "
				+ "   count(folio) as pendientesNoVencidos, "
				+ "   0 as pendientesVencidos, "
				+ "   0 as pendientes, "
				+ "   0 as cerrado "
				+ "INTO #TMP_NO_VENCIDOS_RESUMEN "
				+ "FROM "
				+ "   #TMP_NO_VENCIDOS "
				+ "GROUP BY "
				+ "   folio, "
				...
				+ "   desc_area_resp, "
				+ "   0 as casos, "
				+ "   0 as pendientesNoVencidos, "
				+ "   count(folio) as pendientesVencidos, "
				+ "   0 as pendientes, "
				+ "   0 as cerrado "
				+ "INTO #TMP_VENCIDOS_RESUMEN "
				+ "FROM "
				+ "   #TMP_VENCIDOS "
				+ "GROUP BY "
				+ "   folio, "
				...
				+ "   0 as pendientesVencidos, "
				+ "   0 as pendientes, "
				+ "   count(folio) as cerrado "
				+ "INTO #TMP_CONCLUIDOS_RESUMEN "
				+ "FROM "
				+ "#TMP_CONCLUIDOS "
				+ "GROUP BY "
				+ "   folio, "
				...
				+ "   area_estructura, "
				+ "   desc_area_resp "
				+ "SELECT * INTO #TMP_TODOS_RESUMEN FROM #TMP_VENCIDOS_RESUMEN "
				+ "UNION ALL "
				+ "SELECT * FROM #TMP_NO_VENCIDOS_RESUMEN "
				+ "UNION ALL "
				+ "SELECT * FROM #TMP_CONCLUIDOS_RESUMEN "
				+ "SELECT "
				+ "   responsable_area, "
				+ "   area_estructura, "
				+ "   desc_area_resp, "
				+ "   sum(pendientesNoVencidos + pendientesVencidos + cerrado) as casos , "
				+ "   sum(pendientesNoVencidos) as pendientesNoVencidos, "
				+ "   sum(pendientesVencidos) as pendientesVencidos, "
				+ "   sum(pendientesNoVencidos + pendientesVencidos) as pendientes, "
				+ "   sum(cerrado) as cerrado "
				+ "FROM #TMP_TODOS_RESUMEN ca "
				+ "WHERE 1 = 1 "
				+ "   "
				+ whereEstructura + "GROUP BY " + "   responsable_area, " + "   area_estructura, " + "   desc_area_resp " + "ORDER BY " + "   desc_area_resp ";
	...
	private String RenglonDatoGralHTML(String area, String folio, String referencia, String remitente_nombre, String remitente_area, String responsable_nombre, String responsable_area, String fecha_envio, String fecha_atencion, String cerrado, String destinatario, String prioridad, String asunto, // Esteban
																																																																											// Badillo.
																																																																											// Fecha:
																																																																											// 19/Feb/2010.
																																																																											// Se
	...
	public String ReportePorAreaDetalle(String area, String estructura, String regfechaini, String regfechafin, int tipoacumulado) {
		...
		try {
			...
			// + "        t.desc_area_resp, "
			// + "        t.nombre_resp, "
			String sql = "" + "  SELECT DISTINCT " + "        t.folio, " + "        t.referencia, " + "        t.nombre_rem,  " + "        t.desc_area_rem,  " + "        cerrado = CASE " + "           WHEN t.terminada = 'S' THEN 'CONCLUIDOS'"
				+ "           WHEN t.terminada = 'N' AND t.fecha_compromiso < GETDATE() THEN 'VENCIDOS'" + "           WHEN t.terminada = 'N' AND t.fecha_compromiso > GETDATE() THEN 'NO VENCIDOS'" + "        END, "

				+ "        CASE WHEN t.prioridad = 'N' THEN 'NORMAL' ELSE 'URGENTE' END as prioridad, " + "        t.fecha_compromiso as fechalimite " + "	  FROM ( " + "					SELECT vimx.* " + "					  FROM vimx_reportes vimx " + "					 WHERE 1 = 1" + "                   " + whereBase
				+ "                   " + whereFechas + "			) t " + "			  LEFT OUTER JOIN CG_CAT_AREAS ca ON (ca.id_area = t.responsable_area) " + "   WHERE t.desc_area_resp IS NOT NULL " + "                     " + whereEstructura;
			/*
	...
	public StringBuffer[] ReportePorEmpleado(String area, String idArea, String empleado, String searea, String fechaIni, String fechaFin, String login) {
		...
		Connection conn = null;
		StringBuffer sb = new StringBuffer();
		StringBuffer[] resultado = new StringBuffer[5]; // Esteban Badillo.
														// Fecah:
														// 26/Febrero/2010. Se
														// agrega StringBuffer
														// para devolver los
		StringBuffer sbGrafColumnKeys = new StringBuffer(); // Esteban Badillo.
															// Fecah:
															// 26/Febrero/2010.
															// Se agrega
															// StringBuffer para
															...
															// valores que leer
															// reporte.
		StringBuffer sbGrafVencidos = new StringBuffer(); // Esteban Badillo.
															// Fecah:
															// 26/Febrero/2010.
															// Se agrega
															// StringBuffer para
															...
															// la grfica del
															// reporte.
		StringBuffer sbGrafNoVencidos = new StringBuffer(); // Esteban Badillo.
															// Fecah:
															// 26/Febrero/2010.
															// Se agrega
															// StringBuffer para
															...
															// la  del
															// reporte.
		StringBuffer sbGrafConcluidos = new StringBuffer(); // Esteban Badillo.
															// Fecah:
															// 26/Febrero/2010.
															// Se agrega
															// StringBuffer para
		...
		// ResultSet rs = null;
		// String tblAll = "";
		String param = "idRptMaestro=" + GestionInterface.RPT_POR_EMPLEADO + "&area=" + area + "&idarea=" + idArea + "&empleado=" + empleado // Esteban
																																				// Badillo.
																																				// Fecha:
																																				// 07/Abril/2010.
																																				// Agregado.
		...
		try {
			...
			+ "SELECT " + "ce_os_responsable as responsable_id, " + "ISNULL(ce_ap_paterno,'') + ' ' + ISNULL(ce_ap_materno,'') + ' ' + ISNULL(ce_nombre_completo,'') as nombre_resp, " + "id_area responsable_area, "

			+ "(select count(*) from dbo.CG_CASO_OPERACION AS co WITH(NOLOCK) LEFT OUTER JOIN  " + "	 dbo.CG_BITACORA_OPERACION AS bo WITH(NOLOCK) ON bo.SECUENCIAL_OPERACION = co.ID_CASO_OPER AND bo.ID_CASO = co.ID_CASO  "
				+ "  INNER JOIN dbo.IMXEXPEDIENTES AS imx WITH(NOLOCK)ON imx.ID_GABINETE = bo.ID_GABINETE " + "where  (bo.folio NOT LIKE 'TMP-%') and bo.id_operacion IN (2,3,10,9,11) " + "    and bo.secuencial_operacion IN (	 "
				+ "			select MAX(maxSO.secuencial_operacion) FROM CG_BITACORA_OPERACION maxSO  " + "			WHERE maxSO.folio = bo.folio  " + "				and maxSO.id_operacion in (2,3,9,10,11) " + "				and bo.responsable_id = maxSO.responsable_id  " + "				and bo.terminada = maxSO.terminada)   "
				+ "	and bo.TERMINADA = 'N' and co.user03 < GETDATE() " + "	and bo.responsable_id = ce_os_responsable "
				+ whereFecha
				+ " ) as pendientesVencidos, "

				+ "(select count(*) from dbo.CG_CASO_OPERACION AS co WITH(NOLOCK) LEFT OUTER JOIN  "
				+ "	 dbo.CG_BITACORA_OPERACION AS bo WITH(NOLOCK) ON bo.SECUENCIAL_OPERACION = co.ID_CASO_OPER AND bo.ID_CASO = co.ID_CASO  "
				+ "  INNER JOIN dbo.IMXEXPEDIENTES AS imx WITH(NOLOCK)ON imx.ID_GABINETE = bo.ID_GABINETE "
				+ "where  (bo.folio NOT LIKE 'TMP-%') and bo.id_operacion IN (2,3,10,9,11) "
				+ "    and bo.secuencial_operacion IN (	 "
				+ "			select MAX(maxSO.secuencial_operacion) FROM CG_BITACORA_OPERACION maxSO  "
				+ "			WHERE maxSO.folio = bo.folio  "
				+ "				and maxSO.id_operacion in (2,3,9,10,11) "
				+ "				and bo.responsable_id = maxSO.responsable_id  "
				+ "				and bo.terminada = maxSO.terminada)  "
				+ "	and bo.TERMINADA = 'N' and co.user03 > GETDATE() "
				+ "	and bo.responsable_id = ce_os_responsable "
				+ whereFecha
				+ " ) as pendientesNoVencidos, "
				...
				+ "				and bo.responsable_id = maxSO.responsable_id  "
				+ "				and bo.terminada = maxSO.terminada)  "
				+ "	and bo.TERMINADA = 'S'  " + "	and bo.responsable_id = ce_os_responsable " + whereFecha + " ) as cerrado "

				+ "FROM dbo.CG_CAT_EMPLEADO " + "WHERE 1=1 "

				+ " " + andArea + " " + andLogin

				+ " ORDER by ce_ap_paterno,ce_ap_materno,ce_nombre_completo ";
	...
	public String ReportePorEmpleadoDetalle(String area, String idArea, String fechaini, String fechafin, String login, int tipoacumulado) {
		...
		try {
			...
			String sql = "" + " SELECT bo.folio " + " ,imx.REFERENCIA " + " ,convert(varchar(20), case when bo.id_operacion = 2 then bo.FECHA_INICIO else dbo.FnRptGenResFechaEnvio(bo.ID_CASO, bo.SECUENCIAL_OPERACION) end, 103) as fecha_envio "
				+ " ,CASE WHEN imx.prioridad = 'N' THEN 'NORMAL' ELSE 'URGENTE' END  as prioridad " + " ,imx.dpc_f_registro as fecha_Registro " + " ,CASE WHEN bo.terminada = 'S' THEN 'CONCLUIDOS' ELSE  " + " CASE  " + " 	WHEN co.user03 ='' THEN 'NO VENCIDOS'  "
				+ " 	WHEN co.user03 =' 23:59:59' THEN 'NO VENCIDOS'  " + " 	WHEN co.user03 > GETDATE() THEN 'NO VENCIDOS' " + " 	WHEN co.user03 < GETDATE() THEN 'VENCIDOS' " + " END    " + " END as CERRADO " + " ,ca1.d_descripcion AS desc_area_rem "
				+ " ,ISNULL(ce1.ce_ap_paterno,'') + ' ' + ISNULL(ce1.ce_ap_materno,'') + ' ' + ISNULL(ce1.ce_nombre_completo,'') as nombre_rem " + " ,imx.dpc_f_limite AS fechalimite "
				+ " ,bo.TERMINADA "
				+ " ,substring(convert(varchar, CASE WHEN co.user03 NOT LIKE '%__/__/____%' OR co.USER03 IS NULL THEN CONVERT(VARCHAR, dbo.FnRptGenResFechaLimite(bo.ID_CASO, bo.SECUENCIAL_OPERACION), 103) ELSE co.user03 END, 103),0,12) AS USER03 "
				// + " ,co.USER03 "
				+ " ,co.CO_OBSERVACION AS ASUNTO "

				+ " FROM          " + " dbo.CG_CASO_OPERACION AS co WITH(NOLOCK)  " + " LEFT OUTER JOIN dbo.CG_BITACORA_OPERACION AS bo WITH(NOLOCK) ON bo.SECUENCIAL_OPERACION = co.ID_CASO_OPER AND bo.ID_CASO = co.ID_CASO  "
				+ " INNER JOIN  dbo.IMXEXPEDIENTES AS imx WITH(NOLOCK)ON imx.ID_GABINETE = bo.ID_GABINETE " + " INNER JOIN dbo.CG_CAT_EMPLEADO AS ce1 WITH(NOLOCK) ON ce1.CE_OS_RESPONSABLE = bo.responsable_id " + " INNER JOIN dbo.CG_CAT_AREAS AS ca1 WITH(NOLOCK) ON ca1.ID_AREA = bo.remitente_area  "

				+ " WHERE	bo.folio NOT LIKE 'TMP-%'  " + " AND bo.id_operacion in (2,3,9,10,11) "

				+ andLogin
				// + andArea
				+ whereFecha + whereBase

				+ " AND bo.secuencial_operacion = ( " + " 	SELECT 	MAX(s2bo.secuencial_operacion) FROM 	dbo.CG_BITACORA_OPERACION  s2bo WITH(NOLOCK)  " + " 	WHERE s2bo.id_operacion  in (2,3,9,10,11) " + "        AND bo.responsable_id = s2bo.responsable_id "
				+ "		  AND bo.Terminada = s2bo.Terminada " + "        AND bo.folio = s2bo.folio " + "   ) "
	...
	public String RenglonHeader(int idReporte) {
		...
		switch (idReporte) {
			case GestionInterface.RPT_GENERAL:
				tblHeader = "<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" class=\"scrollTable\">" + "	<thead class=\"fixedHeader\" id=\"fixedHeader\">" + "		<tr>" + "			<th>&Aacute;REA</th>" + "			<th>FOLIO</th>" + "			<th>REFERENCIA</th>" + "			<th>NOMBRE DEL REMITENTE</th>"
					+ "			<th>&Aacute;REA DEL REMITENTE</th>" + "			<th>FECHA LIMITE</th>" + "			<th>ESTATUS</th>" + "			<th>TURNADO A</th>" + "			<th>PRIORIDAD</th>" + "		</tr>" + "	</thead>" + "	<tbody class=\"scrollContent\">" + "		*" + "   </tbody>" + "</table>";
				break;
			case GestionInterface.RPT_CONSOLIDADO:
				tblHeader = "<table align=\"center\" cellpadding=\"0\" cellspacing=\"1\" border=\"0\" width=\"80%\" class=\"scrollTable\">" + "   <thead class=\"fixedHeader\" id=\"fixedHeader\">" + "       <tr>" + "           <td></td>" + "           <td id=\"idTh\" colspan=\"5\">PENDIENTES</td>"
					+ "           <td></td>" + "           <td></td>" + "           <td></td>" + "       </tr>" + "       <tr>" + "           <td id=\"idTh\">&Aacute;REA</td>" + "           <td id=\"idTh\">VENCIDOS</td>" + "           <td id=\"idTh\">%</td>"
					+ "           <td id=\"idTh\">NO VENCIDOS</td>" + "           <td id=\"idTh\">%</td>" + "           <td id=\"idTh\">TOTAL</td>" + "           <td id=\"idTh\">CONCLUIDOS</td>" + "           <td id=\"idTh\">%</td>" + "           <td id=\"idTh\">TOTAL</td>" + "	    </tr>"
					+ "   </thead>" + "   <tbody class=\"scrollContent\">" + "   	*" + "   </tbody>" + "</table>";
				break;

			case GestionInterface.RPT_POR_AREA:
				tblHeader = "<table align=\"center\" cellpadding=\"0\" cellspacing=\"1\" border=\"0\" width=\"80%\" class=\"scrollTable\">" + "   <thead class=\"fixedHeader\" id=\"fixedHeader\">" + "	    <tr>" + "		    <td></td>" + "		    <td id=\"idTh\" colspan=\"5\">PENDIENTES</td>"
					+ "		    <td></td>" + "		    <td></td>" + "		    <td></td>" + "	    </tr>" + "	    <tr>" + "		    <td id=\"idTh\">&Aacute;REA</td>" + "		    <td id=\"idTh\">VENCIDOS</td>" + "		    <td id=\"idTh\">%</td>" + "		    <td id=\"idTh\">NO VENCIDOS</td>"
					+ "		    <td id=\"idTh\">%</td>" + "		    <td id=\"idTh\">TOTAL</td>" + "		    <td id=\"idTh\">CONCLUIDOS</td>" + "		    <td id=\"idTh\">%</td>" + "		    <td id=\"idTh\">TOTAL</td>" + "	    </tr>" + "   </thead>" + "   <tbody class=\"scrollContent\">" + "	    {rpt_bdy} "
					+ "   </tbody>" + "</table>";
				break;

			case GestionInterface.RPT_POR_EMPLEADO:
				tblHeader = "<table align=\"center\" cellpadding=\"0\" cellspacing=\"1\" border=\"0\" width=\"80%\" class=\"scrollTable\">" + "   <thead class=\"fixedHeader\" id=\"fixedHeader\">" + "	<tr>" + "		<td></td>" + "		<td id=\"idTh\" colspan=\"5\">PENDIENTES</td>" + "		<td></td>"
					+ "		<td></td>" + "		<td></td>" + "	</tr>" + "	<tr>" + "		<td id=\"idTh\">FUNCIONARIO</td>" + "	    <td id=\"idTh\">VENCIDOS</td>" + "		<td id=\"idTh\">%</td>" + "		<td id=\"idTh\">NO VENCIDOS</td>" + "		<td id=\"idTh\">%</td>" + "		<td id=\"idTh\">TOTAL</td>"
					+ "		<td id=\"idTh\">CONCLUIDOS</td>" + "		<td id=\"idTh\">%</td>" + "		<td id=\"idTh\">TOTAL</td>" + "	</tr>" + "   </thead>" + "   <tbody class=\"scrollContent\">" + "	    *" + "   </tbody>" + "</table>";
				break;
	...
	private String RenglonDatoHTML(int idReporte, String desc, int vencidos, int novencidos, int pendientes, int cerrado, int casos, int row, String param) {
		...
		String renglon = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + "> " + "	    <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[1]) + "\">" + ((total) ? "" : "" + (row + 1)) + "</td>" + "	    <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[1]) + "\">" + desc + "</td>"
			+ "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + getAncla(idReporte, GestionInterface.RPT_CNS_VENCIDOS, vencidos, param) + "</td>" + "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">&nbsp;" + Math.round(porcentaje_vencidos * 100) + "%&nbsp;</td>" + "		<td id=\""
			+ ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + getAncla(idReporte, GestionInterface.RPT_CNS_NOVENCIDOS, novencidos, param) + "</td>" + "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">&nbsp;" + Math.round(porcentaje_novencidos * 100) + "%&nbsp;</td>" + "		<td id=\""
			+ ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + getAncla(idReporte, GestionInterface.RPT_CNS_PENDIENTES, pendientes, param) + "</td>" + "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + getAncla(idReporte, GestionInterface.RPT_CNS_CONCLUIDOS, cerrado, param) + "</td>"
			+ "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">&nbsp;" + Math.round(porcentaje_cerrados * 100) + "%&nbsp;</td>" + "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + getAncla(idReporte, GestionInterface.RPT_CNS_CASOS, casos, param) + "</td>" + "	</tr>";
		return renglon;
	}
	...
	private String RenglonDatoHTMLSoloTotalArea(int idReporte, String desc, int vencidos, int novencidos, int pendientes, int cerrado, int casos, int row, String param) {
		...
		boolean total = "TOTALES".equals(desc);
		String renglon = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + "> " + "	    <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[1]) + "\">" + ((total) ? "" : "" + (row + 1)) + "</td>" + "	    <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[1]) + "\">" + desc + "</td>"
			+ "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + getAncla(idReporte, GestionInterface.RPT_CNS_VENCIDOS * 10, vencidos, param) + "</td>" + "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">&nbsp;" + Math.round(porcentaje_vencidos * 100) + "%&nbsp;</td>"
			+ "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + getAncla(idReporte, GestionInterface.RPT_CNS_NOVENCIDOS * 10, novencidos, param) + "</td>" + "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">&nbsp;" + Math.round(porcentaje_novencidos * 100) + "%&nbsp;</td>"
			+ "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + getAncla(idReporte, GestionInterface.RPT_CNS_PENDIENTES * 10, pendientes, param) + "</td>" + "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">"
			+ getAncla(idReporte, GestionInterface.RPT_CNS_CONCLUIDOS * 10, cerrado, param) + "</td>" + "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">&nbsp;" + Math.round(porcentaje_cerrados * 100) + "%&nbsp;</td>" + "		<td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">"
			+ getAncla(idReporte, GestionInterface.RPT_CNS_CASOS * 10, casos, param) + "</td>" + "	</tr>";
	...
	private String RenglonDatoGralHTML(String folio, String referencia, String remitente_area_desc, String remitente_area, String remitente_nombre, String cerrado, String tipo_instruccion, String prioridad, String asunto, String responsable_nombre, String responsable_area_desc,
		 ...
		 */
		String fecha_envio, String fecha_limite,
		 ...
		 */
		String renglon = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + "> " + "	  <td id=\"" + ((total) ? tipoCSS[1] : tipoCSS[0]) + "\">" + ((total) ? "" : "" + (row + 1)) + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[1] : tipoCSS[2]) + "\" nowrap>" + folio + "</td>"
			+ "	  <td id=\"" + ((total) ? tipoCSS[1] : tipoCSS[2]) + "\" nowrap>" + referencia + "</td>";
		...
		renglon += "	  <td id=\"" + ((total) ? tipoCSS[1] : tipoCSS[2]) + "\">" + cerrado + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[1] : tipoCSS[2]) + "\">" + splitFechas(fecha_limite) + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[1] : tipoCSS[2]) + "\">" + splitFechas(fecha_envio)
			+ "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[1] : tipoCSS[2]) + "\">" + tipo_instruccion + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[1] : tipoCSS[2]) + "\">" + splitFechas(fecha_registro) + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[1] : tipoCSS[2]) + "\">" + prioridad
			+ "</td>"
			// + "	  <td width=\"2000\" id=\"" + ((total)? tipoCSS[1]:
			// tipoCSS[2]) + "\">" + asunto + "</td>"
			+ "	  <td id=\"" + ((total) ? tipoCSS[1] : tipoCSS[2]) + "\"><div COLS=100 ROWS=3 style=\"FONT-FAMILY: Verdana,Tahoma,Arial;\">" + asunto + "<div></td>"
	...
	private String RenglonDatoGralHTML(String area, String folio, String referencia, String remitente_nombre, String remitente_area, String fecha_atencion, String cerrado, String destinatario, String prioridad, int row) {
		...
		String[] tipoCSS = { "idTh", "idTdTexto", "idTdNumero" };
		boolean total = "FIN".equals(area);
		String renglon = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + "> " + "	  <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[1]) + "\">" + ((total) ? "" : "" + (row + 1))
			+ "</td>"
			// + "	  <td id=\"" + ((total)? tipoCSS[0]: tipoCSS[1]) + "\">"
			// + area + "</td>"
			+ "	  <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[1]) + "\">" + folio + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + referencia + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + remitente_nombre + "</td>" + "	  <td id=\""
			+ ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + fecha_atencion + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + cerrado + "</td>"
			// + "	  <td id=\"" + ((total)? tipoCSS[0]: tipoCSS[2]) + "\">"
			// + destinatario + "</td>"
	...
	private String RenglonDatoGralHTML(String area, String folio, String referencia, String remitente_nombre, String remitente_area, String fecha_envio, String fecha_atencion, String cerrado, String destinatario, String prioridad, String asunto, int row) {
		...
		String[] tipoCSS = { "idTh", "idTdTexto", "idTdNumero" };
		boolean total = "FIN".equals(area);
		String renglon = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + "> " + "	  <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[1])
			+ "\">"
			+ ((total) ? "" : "" + (row + 1))
			+ "</td>"
			// + "	  <td id=\"" + ((total)? tipoCSS[0]: tipoCSS[1]) + "\">"
			// + area + "</td>"
			+ "	  <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[1]) + "\">" + folio + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + referencia + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + remitente_nombre + "</td>" + "	  <td id=\""
			+ ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + remitente_area + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + fecha_envio + "</td>" + "	  <td id=\"" + ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + fecha_atencion + "</td>" + "	  <td id=\""
			+ ((total) ? tipoCSS[0] : tipoCSS[2]) + "\">" + cerrado + "</td>"
			// + "	  <td id=\"" + ((total)? tipoCSS[0]: tipoCSS[2]) + "\">"
			// + destinatario + "</td>"
	...
	public StringBuffer[] ReportePorArea(String area, String estructura, String regfechaini, String regfechafin, boolean detallado, String tipo_filtro, boolean areasHijas, int idArea) {
		...
		try {
			...
			sql = "" + "SELECT  " + "		ca1.ID_AREA as responsable_area, " + "		ca1.d_descripcion AS desc_area_resp, " + "		(select count(*) from  " + "			dbo.CG_CASO_OPERACION AS coV WITH(NOLOCK) LEFT OUTER JOIN "
				+ "			dbo.CG_BITACORA_OPERACION AS boV WITH(NOLOCK) ON boV.ID_CASO = coV.ID_CASO AND boV.SECUENCIAL_OPERACION = coV.ID_CASO_OPER INNER JOIN " + "			dbo.IMXEXPEDIENTES AS imx ON imx.ID_GABINETE = boV.ID_GABINETE "
				+ "		where  (boV.FOLIO NOT LIKE 'TMP-%') and boV.id_operacion IN (2,3,10,9,11) " + "			and boV.secuencial_operacion IN ( " + "					SELECT MAX(maxSO.secuencial_operacion) FROM CG_BITACORA_OPERACION maxSO, IMXEXPEDIENTES maxIMX "
				+ "					WHERE maxSO.folio = boV.folio  AND maxSO.FOLIO = maxIMX.FOLIO " + "                 and maxSO.TERMINADA = 'N' " + "                 and maxSO.responsable_area = convert(varchar,ca1.id_area)"
				+ whereSub
				+ "                 ) "
				+ "			and boV.TERMINADA = 'N' and coV.user03 < GETDATE() "
				// + "			and boV." + szTipoRep +
				...
				// 04/Febrero/2010.
				// Agregado.
				+ (areasHijas ? " AND boV.remitente_area = '" + idArea + "' " : "") // Esteban
																					// Badillo.
																					// Fecha:
																					// 04/Febrero/2010.
																					// Agregado
				...
				+ "		 ) as pendientesVencidos, "
				+ "		(select count(*) from  "
				+ "			dbo.CG_CASO_OPERACION AS coV WITH(NOLOCK) LEFT OUTER JOIN "
				+ "			dbo.CG_BITACORA_OPERACION AS boV WITH(NOLOCK) ON boV.ID_CASO = coV.ID_CASO AND boV.SECUENCIAL_OPERACION = coV.ID_CASO_OPER INNER JOIN "
				+ "			dbo.IMXEXPEDIENTES AS imx ON imx.ID_GABINETE = boV.ID_GABINETE "
				+ "		 where  (boV.FOLIO NOT LIKE 'TMP-%') and boV.id_operacion IN (2,3,10,9,11) "
				+ "			and boV.secuencial_operacion IN ( "
				+ "					SELECT MAX(maxSO.secuencial_operacion) FROM CG_BITACORA_OPERACION maxSO, IMXEXPEDIENTES maxIMX "
				+ "					WHERE maxSO.folio = boV.folio AND maxSO.FOLIO = maxIMX.FOLIO "
				+ "                 and maxSO.TERMINADA = 'N'"
				+ "			        and maxSO.responsable_area = convert(varchar,ca1.id_area) "
				+ whereSub
				+ "                 ) "
				+ "			and boV.TERMINADA = 'N' and coV.user03 > GETDATE() "
				// + "			and boV." + szTipoRep +
				...
				+ "			dbo.CG_CASO_OPERACION AS coV WITH(NOLOCK) LEFT OUTER JOIN "
				+ "			dbo.CG_BITACORA_OPERACION AS boV WITH(NOLOCK) ON boV.ID_CASO = coV.ID_CASO AND boV.SECUENCIAL_OPERACION = coV.ID_CASO_OPER INNER JOIN "
				+ "			dbo.IMXEXPEDIENTES AS imx WITH(NOLOCK) ON imx.ID_GABINETE = boV.ID_GABINETE "
				+ "		 where  (boV.FOLIO NOT LIKE 'TMP-%') and boV.id_operacion IN (2,3,10,9,11) "
				+ "			and boV.secuencial_operacion IN ( "
				+ "					SELECT MAX(maxSO.secuencial_operacion) FROM CG_BITACORA_OPERACION maxSO, IMXEXPEDIENTES maxIMX "
				+ "					WHERE maxSO.folio = boV.folio AND maxSO.FOLIO = maxIMX.FOLIO " + "			        and maxSO.TERMINADA = 'S'  " + "			        and maxSO.responsable_area = convert(varchar,ca1.id_area) " + whereSub + ")  " + "			and boV.TERMINADA = 'S'  "
				// + "			and boV." + szTipoRep +
				// " = convert(varchar,ca1.id_area) " //Esteban Badillo.
				// Fecha: 04/Febrero/2010. Eliminado.
				+ "			and boV.responsable_area = convert(varchar,ca1.id_area) "// Esteban
				...
				+ (areasHijas ? " AND boV.remitente_area = '" + idArea + "' " : "") // Esteban
																					...
																					// 04/Febrero/2010.
																					// Agregado
				+ "	     " + whereFechas + "		 ) as cerrado " + "		FROM dbo.CG_CAT_AREAS AS ca1 "
			...
			if (acum.isEmpty()) {
				...
				for (Iterator<?> iter = acum.iterator(); iter.hasNext(); row++) {
					...
					tr = RenglonDatoHTML(GestionInterface.RPT_POR_AREA_DETALLE, a.getDescripcion(), a.getVencidosNumero(), a.getNovencidosNumero(), a.getPendientes(), a.getConcluidos(), a.getTotalTurnos(), row,
					// param + "&area=" + a.getId());
		...
		try {
			...
			switch (tipoacumulado) {
				case GestionInterface.RPT_CNS_VENCIDOS:
					...
					// whereBase =
					// "                      AND vimx.id_operacion in(2,3,7, 9,10,11) "
					whereBase = "                      AND vimx.id_operacion in(2) "
					// BMEA 15/10/2009
					// + "					   AND vimx.terminada = 'N' "
			...
			String sql = "" + "  SELECT DISTINCT " + "        t.folio, " + "        t.referencia, " + "        t.nombre_rem,  " + "        t.desc_area_rem,  " + "        cerrado = CASE " + "           WHEN t.CERRADO = 'S' THEN 'CONCLUIDOS'" // SE
																																																													// CAMBIARON
																																																													// A
																																																													// CERRADO
																																																													// EN
																																																													...
																																																													// DE
																																																													// TERMINADA
				+ "           WHEN t.CERRADO = 'N' AND t.fecha_compromiso < GETDATE() THEN 'VENCIDOS'" + "           WHEN t.CERRADO = 'N' AND t.fecha_compromiso > GETDATE() THEN 'NO VENCIDOS'" + "        END, "

				+ "        CASE WHEN t.prioridad = 'N' THEN 'NORMAL' ELSE 'URGENTE' END as prioridad, " + "        t.fecha_compromiso as fechalimite, " /*
																																						 * Esteban
																																						 * Badillo
																																						 * .
																																						 * Fecha
				...
				+ "        t.fecha_inicio" /*
											 ...
											 * fecha_inicio
											 */
				+ "	  FROM ( " + "					SELECT vimx.* " + "					  FROM vimx_reportes vimx " + "					 WHERE 1 = 1" + "                   " + whereBase + "                   " + whereFechas
				 
				+ "                   " + (areasHijas ? whereAreasHijas : "")

				+ "			) t " + "			  LEFT OUTER JOIN CG_CAT_AREAS ca ON (ca.id_area = t.responsable_area) " + "   WHERE t.desc_area_resp IS NOT NULL " + "                     " + whereEstructura;
			/*
			 * + "GROUP BY " + "        t.responsable_area," +
			 * "        t.desc_area_resp," + "        t.folio," +
			 * "        t.referencia," + "        t.nombre_rem," +
			 ...
			 */
			sql = "EXEC ReporteAreaDetalle"
				// + " @AREA = " + (area.trim().equals("") ? "NULL" : area)
				// //BMEA 05/Febrero/2010
				+ " @AREA = " + idArea + ", @FECHA_INICIAL = " + (regfechaini.trim().equals("") ? "NULL" : "'" + regfechaini + "'") + ", @FECHA_FINAL = " + (regfechafin.trim().equals("") ? "NULL" : "'" + regfechafin + "'") + ", @AREAS_HIJAS = " + (areasHijas ? "1" : "0") + ", @TIPO_ACUMULADO = "
				+ (tipoacumulado) + ", @ID_AREA_PADRE = " + ((idAreaPadre == 0) || (!areasHijas) ? "NULL" : idAreaPadre);
			/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
			...
			if (gral.isEmpty()) {
				...
				for (Iterator<?> iter = gral.iterator(); iter.hasNext(); row++) {
					 ...
					 * row);
					 */
					tr = this.RenglonDatoGralHTML(a.getArea(), a.getFolio(), a.getReferencia(), a.getNombreRemitente(), a.getAreaRemitente(), a.getResponsableNombre(), a.getResponsableAreaDesc(), a.getFechaEnvio(), a.getFechaAtencion(), a.getCerrado(), a.getDestinatario(), a.getPrioridad(),
						a.getAsunto(), row);
	...
	public String ResultadoCreacionDelPresupuesto(String ep, String ejercicioFiscal, String ramoEP, String unidadResponsableEP, String grupoFuncional, String funcion, String subFuncion, String programaGeneral, String programaPresupuestario, String actividadInstitucional, String partida,
		String tipoGasto, String fuenteFinanciamiento, String entidadFederativa, String cartera, String unidadEjecutora, String unidadNormativa, String cOrderBy, String cGroupBy, String cuenta, String usuario, String filtro) throws Exception {
		...
		try {
			...
			if (!"".equals(ep)) {
				...
				sbRec = ReporteManager.callReporteAnteproyectoParametros(conn, ejercicioFiscal, ramoEP, unidadResponsableEP, grupoFuncional, funcion, subFuncion, programaGeneral, programaPresupuestario, actividadInstitucional, partida, tipoGasto, fuenteFinanciamiento, entidadFederativa, cartera,
					unidadEjecutora, unidadNormativa, cOrderBy, cGroupBy, cuenta, usuario, filtro);
			}
			String tr = "";
			if (sbRec.isEmpty()) {
				tr = "<tr class=\"alternateRow\">" + "	<td colspan=\"10\"><h3>No hay informaci&oacute;n para mostrar</h3></td>" + "</tr>";
				...
				while (iter.hasNext()) {
					...
					if (cuenta == null || cuenta == "") {
						...
						tr = DetalleHTML(cuenta, anteproyAut.getFolioAnteProyectoAut(), anteproyAut.getD_NConsecutivo(), anteproyAut.getEjercicioFiscal(), anteproyAut.getUnidadResponsableEP(), anteproyAut.getD_CUnidadEjecutora(), anteproyAut.getD_CClaveSiaff(), anteproyAut.getD_CClaveInterna(),
							anteproyAut.getD_MCalculado(), anteproyAut.getD_MOptimo(), anteproyAut.getD_MIreductible(), row);
						sbEnv.append(tr);
					}
				}
	...
	private String DetalleHTML(String folioAnteProyectoAut, String nConsecutivo, String ejercicioFiscal, String unidadResponsableEP, String unidadEjecutora, String cClaveSiaff, String cClaveInterna, String mCalculado, String mOptimo, String mIreductible, int row) {
		String[] tipoCSS = { "idTh", "idTdTexto", "idTdNumero" };
		String renglon = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + "> " + "	  <td id=\"" + folioAnteProyectoAut + "\">" + folioAnteProyectoAut + "</td>" + "	  <td id=\"" + nConsecutivo + "\">" + nConsecutivo + "</td>" + "	  <td id=\"" + ejercicioFiscal + "\">"
			+ ejercicioFiscal + "</td>" + "	  <td id=\"" + unidadResponsableEP + "\">" + unidadResponsableEP + "</td>" + "	  <td id=\"" + unidadEjecutora + "\">" + unidadEjecutora + "</td>" + "	  <td id=\"" + cClaveSiaff + "\">" + cClaveSiaff + "</td>" + "	  <td id=\"" + cClaveInterna + "\">"
			+ cClaveInterna + "</td>" + "	  <td id=\"" + mCalculado + "\">" + mCalculado + "</td>" + "	  <td id=\"" + mOptimo + "\">" + mOptimo + "</td>" + "	  <td id=\"" + mIreductible + "\">" + mIreductible + "</td>" + "</tr>";
		return renglon;
	}
	...
	public String ResultadoCreacionDelPresupuestoCalendarizado(String ep, String ejercicioFiscal, String ramoEP, String unidadResponsableEP, String grupoFuncional, String funcion, String subFuncion, String programaGeneral, String programaPresupuestario, String actividadInstitucional,
		String partida, String tipoGasto, String fuenteFinanciamiento, String entidadFederativa, String cartera, String unidadEjecutora, String unidadNormativa, String cOrderBy, String cGroupBy, String usuario, String filtro) throws Exception {
		...
		try {
			...
			if (!"".equals(ep)) {
				...
				sbRec = ReporteManager.callReporteAnteproyectoCalendarizadoParametros(conn, ejercicioFiscal, ramoEP, unidadResponsableEP, grupoFuncional, funcion, subFuncion, programaGeneral, programaPresupuestario, actividadInstitucional, partida, tipoGasto, fuenteFinanciamiento,
					entidadFederativa, cartera, unidadEjecutora, unidadNormativa, cOrderBy, cGroupBy, usuario, filtro);
			}
			String tr = "";
			if (sbRec.isEmpty()) {
				tr = "<tr class=\"alternateRow\">" + "	<td colspan=\"10\"><h3>No hay informaci&oacute;n para mostrar</h3></td>" + "</tr>";
				...
				while (iter.hasNext()) {
					...
					tr = CalendarioHTML(anteproyAutCal.getNFolioAnteProyectoAut(), anteproyAutCal.getNConsecutivo(), anteproyAutCal.getAEjercicioFiscal(), anteproyAutCal.getCUnidadResponsable(), anteproyAutCal.getCClaveSiaff(), anteproyAutCal.getCClaveInterna(), anteproyAutCal.getCCentroContable(),
						anteproyAutCal.getNFolioAnteProyecto(), anteproyAutCal.getMAnualAutorizado(), anteproyAutCal.getMEnero(), anteproyAutCal.getMFebrero(), anteproyAutCal.getMMarzo(), anteproyAutCal.getMAbril(), anteproyAutCal.getMMayo(), anteproyAutCal.getMJunio(), anteproyAutCal.getMJulio(),
						anteproyAutCal.getMAgosto(), anteproyAutCal.getMSeptiembre(), anteproyAutCal.getMOctubre(), anteproyAutCal.getMNoviembre(), anteproyAutCal.getMDiciembre(), row);
					sbEnv.append(tr);
				}
	...
	private String CalendarioHTML(String nFolioAnteProyectoAut, String nConsecutivo, String aEjercicioFiscal, String cUnidadResponsable, String cClaveSiaff, String cClaveInterna, String cCentroContable, String nFolioAnteProyecto, String mAnualAutorizado, String mEnero, String mFebrero,
		String mMarzo, String mAbril, String mMayo, String mJunio, String mJulio, String mAgosto, String mSeptiembre, String mOctubre, String mNoviembre, String mDiciembre, int row) {
		String[] tipoCSS = { "idTh", "idTdTexto", "idTdNumero" };
		String renglon = "<tr class=" + ((row % 2) == 0 ? "AlternateRow" : "NormalRow") + "> " + "<td id=\"" + nFolioAnteProyectoAut + "\">" + nFolioAnteProyectoAut + "</td>" + "<td id=\"" + nConsecutivo + "\">" + nConsecutivo + "</td>" + "<td id=\"" + aEjercicioFiscal + "\">" + aEjercicioFiscal
			+ "</td>" + "<td id=\"" + cUnidadResponsable + "\">" + cUnidadResponsable + "</td>" + "<td id=\"" + cClaveSiaff + "\">" + cClaveSiaff + "</td>" + "<td id=\"" + cClaveInterna + "\">" + cClaveInterna + "</td>" + "<td id=\"" + cCentroContable + "\">" + cCentroContable + "</td>"
			+ "<td id=\"" + nFolioAnteProyecto + "\">" + nFolioAnteProyecto + "</td>" + "<td id=\"" + mAnualAutorizado + "\">" + mAnualAutorizado + "</td>" + "<td id=\"" + mEnero + "\">" + mEnero + "</td>" + "<td id=\"" + mFebrero + "\">" + mFebrero + "</td>" + "<td id=\"" + mMarzo + "\">" + mMarzo
			+ "</td>" + "<td id=\"" + mAbril + "\">" + mAbril + "</td>" + "<td id=\"" + mMayo + "\">" + mMayo + "</td>" + "<td id=\"" + mJunio + "\">" + mJunio + "</td>" + "<td id=\"" + mJulio + "\">" + mJulio + "</td>" + "<td id=\"" + mAgosto + "\">" + mAgosto + "</td>" + "<td id=\"" + mSeptiembre
			+ "\">" + mSeptiembre + "</td>" + "<td id=\"" + mOctubre + "\">" + mOctubre + "</td>" + "<td id=\"" + mNoviembre + "\">" + mNoviembre + "</td>" + "<td id=\"" + mDiciembre + "\">" + mDiciembre + "</td>" + "</tr>";
		return renglon;
	}
