...
public class reporteSIIWebFlujoEfectivoManager implements Serializable {
	...
	public static String FlujoEfectivoExcel(Connection conn, int mesIni, int ejercicioFiscal, String tipo, Map<String, String> plantillas, int dia) throws Exception {
		...
		try {
			...
			else if(tipo.equals("explicaciones")){ //|| tipo.equals("146") || tipo.equals("147")){
			fileName = generaReporteExcelExplicaciones(rs, rs2, rs3, tipo, plantillas.get("145y146y147"), anio, mes, dia);
			}
			else if(tipo.equals("Com3110")){
	...
	public static File FlujoEfectivoCSV(Connection conn, int mesIni, int ejercicioFiscal, String tipo, String nFolio, String tipoFormato) throws Exception {
		...
		File f = null;
		int mes = mesIni;
		int anio = ejercicioFiscal;				
				
		if(tipo.equals("Org161") || tipo.equals("Mod163"))
			query = "{call sp_csv_Org161_Mod163( ?, ?, ? )}";
		
		else if(tipo.equals("Rad162") || tipo.equals("Sol164")){
				if (mes == 13){
					mes = mes - 1;
					query = "{call sp_csv_Rad162_Sol164Masivo( ?, ?, ? )}";
				}
				else
					query = "{call sp_csv_Rad162_Sol164( ?, ?, ? )}";	
		}
		else if(tipo.equals("Pag316")){
				if (mes == 13) 
					query = "{call sp_GastoProgramable_Pag316Masivo( ?, ? )}";
				else 
					query = "{call sp_GastoProgramable_Pag316( ?, ? )}";
		}
		else if(tipo.equals("Dev319"))
				if (mes == 13)
					query = "{call sp_GastoProgramable_Dev319Masivo( ?, ? )}";
				else
					query = "{call sp_GastoProgramable_Dev319( ?, ? )}";
		
		else if(tipo.equals("Com318") || tipo.equals("Com3110"))
			query = "{call sp_csv_Com318_Com3110( ?, ?, ? )}";	
		
		else if(tipo.equals("Org111") || tipo.equals("Mod1111"))
			query = "{call sp_csv_Org111_Mod1111( ?, ?, ? )}";
						
		String fileName = "";
		
		try {
			
			if(tipo.equals("Org161") || tipo.equals("Rad162") || tipo.equals("Mod163") || tipo.equals("Sol164") || tipo.equals("Com318") || tipo.equals("Com3110") || tipo.equals("Org111") ||tipo.equals("Mod1111")) {
				cs = conn.prepareCall(query);
				cs.setInt(1, anio);
				cs.setInt(2, mes);
				cs.setString(3, tipo);
			}
			else if(tipo.equals("210") || tipo.equals("221") || tipo.equals("222")){
				cs = conn.prepareCall(query);
				cs.setString(1, nFolio);
				cs.setString(2, tipoFormato);
			}
			else {
				cs = conn.prepareCall(query);
				cs.setInt(1, anio);
	...
	private static String generaReporteExcelExplicaciones(ResultSet rs, ResultSet rs2, ResultSet rs3, String tipoFormato, String plantillaPath, int anio, int mes, int dia) throws Exception {				
		File cFileExcelPlantilla = new File(plantillaPath);
		...
		int renglonInicio145 = 24;
		int renglonInicio146 = 24;
		int renglonInicio147 = 24;
		int columnInicio14 = 0;
 		int renglonFin = 150;
		int columnInicio = 22;
		int columnFin = 36;
		...
		String MesInicio = "01/" + mes + "/" + anio;		
		String MesFin = dia + "/" + mes + "/" + anio;
		
		Row rwEnc2 = (sheet0.getRow(7) == null ? sheet0.createRow(7) : sheet0.getRow(7));
		Cell cell2 = (rwEnc2.getCell(1) == null? rwEnc2.createCell(1): rwEnc2.getCell(1) );
		cell2.setCellValue("PERIODO: " + MesInicio + " AL " + MesFin);
		
		Row rwEnc7 = (sheet0.getRow(9) == null ? sheet0.createRow(9) : sheet0.getRow(9));
		Cell cell7 = (rwEnc7.getCell(12) == null? rwEnc7.createCell(12): rwEnc7.getCell(12) );
		cell7.setCellValue("RHQ");
		
		Row rwEnc4 = (sheet1.getRow(7) == null ? sheet1.createRow(7) : sheet1.getRow(7));
		Cell cell4 = (rwEnc4.getCell(1) == null? rwEnc4.createCell(1): rwEnc4.getCell(1) );
		cell4.setCellValue("PERIODO: " + MesInicio + " AL " + MesFin);		
		
		Row rwEnc8 = (sheet1.getRow(9) == null ? sheet1.createRow(9) : sheet1.getRow(9));
		Cell cell8 = (rwEnc8.getCell(12) == null? rwEnc8.createCell(12): rwEnc8.getCell(12) );
		cell8.setCellValue("RHQ");
				
		Row rwEnc6 = (sheet2.getRow(7) == null ? sheet2.createRow(7) : sheet2.getRow(7));
		Cell cell6 = (rwEnc6.getCell(1) == null? rwEnc6.createCell(1): rwEnc6.getCell(1) );
		cell6.setCellValue("PERIODO: " + MesInicio + " AL " + MesFin);		
		
		Row rwEnc9 = (sheet2.getRow(9) == null ? sheet2.createRow(9) : sheet2.getRow(9));
		Cell cell9 = (rwEnc9.getCell(11) == null? rwEnc9.createCell(11): rwEnc9.getCell(11) );
		cell9.setCellValue("RHQ");
		
		while (rs.next()) {
			cuenta = rs.getString("cuenta");
			
			int renglon = Util.buscaPrimerCoincidencia(sheet0, renglonInicio145, columnInicio14, cuenta);			
		...
		while (rs2.next()) {
			...
			int renglon = Util.buscaPrimerCoincidencia(sheet1, renglonInicio146, columnInicio14, cuenta);			
		...
		while (rs3.next()) {
			...
			int renglon = Util.buscaPrimerCoincidencia(sheet2, renglonInicio147, columnInicio14, cuenta);			
			sheet2 = Util.resultSetToExcelRow(rs3, sheet2, renglon, 9, 1, false);				
		}	
				 
		sheet0 = Util.EvaluaFormula(workbook, sheet0, renglonInicio145 - 4, 45, 9, 14);	
		sheet1 = Util.EvaluaFormula(workbook, sheet1, renglonInicio145 - 4, 45, 9, 14);	
		sheet2 = Util.EvaluaFormula(workbook, sheet2, renglonInicio147 - 4, 45, 9, 14);	
