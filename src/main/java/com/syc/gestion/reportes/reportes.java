...
public class reportes implements Serializable {
	...
	public static String EstadosFinancierosExcel( Connection conn, String fecha, String centroContable, String tipoCedula, String moneda, String nivel, String comentarios, String caracteristicas, Map<String, String> plantillas, String desagregado, String cEsConac, String esMesAA ) 
			throws Exception {
		...
		try {
			...
			if ( "BalanzaMayor.jasper".equals( tipoCedula ) ) {
				...
				else{ query = "{call sp_a_conciliacion_EG_excel( ?, ?, ? )}";					
				}
				cs = conn.prepareCall( query );
				cs.setInt( 1, mes );
				cs.setInt( 2, anio );
			...
			if ( "EdoSituacionFinanciera.jasper".equals( tipoCedula ) || "EdoCambiosSituacionFinanciera.jasper".equals( tipoCedula )
					|| "EdoActividades.jasper".equals( tipoCedula ) || "EdoAnaliticoPasivoCONAC.jasper".equals( tipoCedula )|| "ConciliacionCONACEg.jasper".equals( tipoCedula ) ) {
				fileName = generaReporteSFExcel( rs, rs2, rs3, plantillas.get( "excel" ), tipoCedula, fecha, desagregado, moneda );
			}
			else
			if ("ConciliacionCONAC.jasper".equals(tipoCedula)){
				fileName = generaReporteSFExcel( rs, rs2, rs3, plantillas.get( "excelIng" ), tipoCedula, fecha, desagregado, moneda );
			}
			else fileName = generaReporteExcel( rs, rs2, rs3, plantillas.get( "excel" ), tipoCedula, fecha, desagregado, moneda );
	...
	public void executeFIEL( Connection conn, String reportPath, String reportName, Map<String, Object> parms, String[] firmantes, int idReporte ) throws Exception {
		...
		log.debug( "Generado reporte para firma electronica en: " + pathReporte );
		String pathAcuse = ( ( EstadosFinancierosFirma ) fer ).generaAcuse( conn, volumen, null, false );
		log.debug( "Generado reporte para acuse en: " + pathAcuse );
