...
public class InformeSemestralServlet extends HttpServlet{
	...
	private String query_sp=null; //Variable donde se guardara el query que llama a la funcion
	String registros[][]=new String[350][5]; // registros, 13 columnas
	String cFileExcel=null; //nombre de la plantilla
	int nFil;
	...
	public void datosPlantilla(HttpServletRequest request, HttpServletResponse response,String tReporte)
		throws ServletException, IOException, SQLException, ClassNotFoundException,ParseException 
	{
		...
		if(tReporte.equals("1")) { //Es Reporte Avanc del ejercido de Contratos GENERAL
		   query_sp="execute sp_getreporteSemReembolso '"+request.getParameter("ParamRep")+"'"; //procedimiento para la consulta a base de datos 
		   String aux=null;
		   llenaExcel(cFileExcel,query_sp,response);
		}
		if(tReporte.equals("2")) {//Es reporte por AREA
	...
	public void llenaExcel( String plantilla, String query,HttpServletResponse response )
		throws ServletException, IOException, SQLException, ClassNotFoundException,ParseException 
		{
		...
		try{ //Inicia el procedimiento para insertar en la plantilla de Excel
			
		String etiqueta; // variable para guardar el dato y pasarlo a la celda	
		...
		// DATOS DEL LIBRO DE EXCEL
         ...
         WritableSheet Datos = copy.getSheet(2);  //Hoja donde se insertaran solo cadenas
         WritableSheet hojaA1 = copy.getSheet(3); //Hoja donde se insertaran cadenas y núneros
         WritableSheet hojaA2 = copy.getSheet(4); //Hoja donde se insertaran solo números
         WritableSheet hojaB = copy.getSheet(5);  //Hoja donde se insertaran solo números
         WritableSheet hojaC = copy.getSheet(6);  //Hoja donde se insertaran solo números
        ...
        // NumberFormat formNum = new NumberFormat("#,##0.00"); 
         WritableFont arial12n= new WritableFont(WritableFont.ARIAL,12); //Formato  Principal 
           ...
           for (int j=0;j<=3;j++); 
            {
    		 ...
    		 label.setCellFormat(arialFormat);
		 	 label.getCellFormat();
        	   ...
        	   {
        		 Datos.addCell(label); 
               }// fin del if pestaña Datos
        	  
       		else if(registros[i][0].equals("B"))              
        	   {
       		   	  hojaB.addCell(label); 
        	   }//fin del else if de pestaña cadena/num
        	 
        	 else    // es pestaña de solo numeros
        	 {
         ...
         for(int i=0;i<nFil;i++)// for para controlar filas
         {
           for (int j=0;j<=3;j++); // for para controlar columnas
            {
             int columna = (int)registros[i][2].charAt(0)-65; // convertir la letra de la columna a posicion numerica
            // log.info("posicion de la columna "+columna);
       		 int fila= ((int)Integer.parseInt(registros[i][1])-1); //convertir la posicion de la fila a entero
       		//log.info("posicion de la fila "+fila);
       		//log.info("valor del primer registro: "+registros[i][0]);
       	
       		
       		 
       		if( registros[i][0].equals("Datos"))              //pestaña=dato   Si es pestaña de solo cadenas
        	   {
       			// log.info("Empieza en la pestaña de DATOS");
        		 etiqueta =registros[i][3]; //posicion donde se encuentra los datos
        		 Label label= new Label (columna,fila,etiqueta);
        		 label.setCellFormat(arialFormat);
  		 	      label.getCellFormat();
        		 Datos.addCell(label); 
       		...
       		else if(registros[i][0].equals("B"))                //   pestaña=cadena/num    Si es pestaña de cadenas y numeros 
        	   {
        		 if(isNumeric(registros[i][3])==true)             //   dato==entero          Checa si el dato es numero
        		 {
        			 double valornum=(double)Double.parseDouble(registros[i][3]);       // Se convierte el valor a numero
