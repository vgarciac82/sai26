...
public class ReporteSOExTransferenciaServlet extends HttpServlet{
	...
	String registros[][]=new String[150][5]; // registros, 13 columnas
	...
	public void datosPlantilla(HttpServletRequest request, HttpServletResponse response)
		...
		String soe=request.getParameter("ParamRep");
			...
			query_sp="execute sp_ReporteSOExTransferencia '"+soe+"'"; //procedimiento para la consulta a base de datos 
			llenaExcel(cFileExcel,query_sp,soe,response);
		
	    }	


	public void llenaExcel( String plantilla, String query ,String nSoe,HttpServletResponse response)
		throws ServletException, IOException, SQLException, ClassNotFoundException,ParseException 
		{
        ...
        /*AQUI SE LLENAN LOS DATOS PARA LA HOJA XESTADOS*/
         String xEst="select dEntidadFederativa, ISNULL(SUM(ImporteFactura),0)Federal ,0 Contraparte,ISNULL(SUM(ImporteFactura),0)+0 total,ISNULL"
        	          +"(SUM(ImporteFactura),0) Transferido  from  tCatalogoEntidadFederativa  tef with(nolock) left join dSOETransfer dt with(nolock) "
        	          +" on dt.cEntidadFed=convert(int,tef.cEntidadFederativa) and numero_soe='"+nSoe+"' group by dEntidadFederativa";
        ...
        String xCat="select cartera,dCartera,SUM(ImporteFactura)importe from dSOETransfer dt inner join tCatalogoCartera tc on dt.cartera=tc.cCartera"
        	        +" and numero_soe='"+nSoe+"' group by cartera,dCartera ";
