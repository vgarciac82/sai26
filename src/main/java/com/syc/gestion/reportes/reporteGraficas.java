...
public class reporteGraficas extends DataSourceManager {
	...
	private static Logger log = Logger.getLogger(reporteGraficas.class);
	...
	public String graficaPresupuesto(int tipo) throws SQLException {
		...
		conn = getConnection();
		try {
			 if (tipo==1){ //programa anual general
				 	pstmnt=conn.prepareStatement("SELECT dbo.fn_mGraficas (?) as cadena");
					pstmnt.setInt(1, tipo);
					
				 }else{
					 if(tipo==3){ //tabla programa anual
						 pstmnt=conn.prepareStatement("SELECT * from fn_mGraficaProgAnualTabla(1)");
					
						//	rs=pstmnt.executeQuery();  
					 }else{
						 if(tipo==4){ //requsicion general
							 pstmnt=conn.prepareStatement("SELECT dbo.fn_mGraficas (?) as cadena");
								pstmnt.setInt(1, tipo);
							//	rs=pstmnt.executeQuery();  
						 }else{
							 if(tipo==5){ //requsicion tabla
								 pstmnt=conn.prepareStatement("SELECT * from fn_mGraficaRequisicionesTabla()");
							
								//	rs=pstmnt.executeQuery();  
							 }else{
								 if(tipo==6){ //consolidado por capitulo
									 pstmnt=conn.prepareStatement("SELECT dbo.fn_mGraficas (?) as cadena");
										pstmnt.setInt(1, tipo);
									//	rs=pstmnt.executeQuery();  
								 }else{
									 if(tipo==7){ //consolidado alcane
										 pstmnt=conn.prepareStatement("SELECT dbo.fn_mGraficas (?) as cadena");
											pstmnt.setInt(1, tipo);
										//	rs=pstmnt.executeQuery(); 
									 }else{
										 if(tipo==8){ //tabla consolidado
											 pstmnt=conn.prepareStatement("SELECT * from fn_mGraficaConsolidadoTabla()");
												
											//		rs=pstmnt.executeQuery();
										 }else{
											 if(tipo==9){ //procedimientoGeneral
												 pstmnt=conn.prepareStatement("SELECT dbo.fn_mGraficas (?) as cadena");
													pstmnt.setInt(1, tipo);
												//	rs=pstmnt.executeQuery();  
											 }else{
												 if(tipo==10){ //tabla procedimiento
													 pstmnt=conn.prepareStatement("SELECT * from fn_mGraficaProcedimientoTabla()");
													
													//		rs=pstmnt.executeQuery();
												 }else{
													 if(tipo==11){ //programa anual por capitulo
														 pstmnt=conn.prepareStatement("SELECT dbo.fn_mGraficas (?) as cadena");
														 pstmnt.setInt(1, tipo);
													 }else{
														 if(tipo==12){ //pedidos -- rfc mas adjudicados
															 pstmnt=conn.prepareStatement("SELECT dbo.fn_mGraficas (?) as cadena");
															 pstmnt.setInt(1, tipo);
															//		rs=pstmnt.executeQuery();
														 }else{
															 if(tipo==13){//tabla de pedido
																 pstmnt=conn.prepareStatement("SELECT * from fn_mGraficaPedidoTabla()"); 
															 }else{
																 if(tipo==14){//pedido por capitulo
																	 pstmnt=conn.prepareStatement("SELECT dbo.fn_mGraficas (?) as cadena");
																	 pstmnt.setInt(1, tipo); 
																 }else{
																	 if(tipo==15){//pedido por capitulo
																		 pstmnt=conn.prepareStatement("SELECT dbo.fn_mGraficas (?) as cadena");
																		 pstmnt.setInt(1, tipo); 
																	 } 
																 }
