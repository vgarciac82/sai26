...
public class ReporteParametroManager {
	...
	public static int insert(Connection conn, ReporteParametro rp) throws SQLException {
		...
		try {
			pstmnt = conn
				.prepareStatement("INSERT INTO cg_reporte_parametro (id_reporte, id_parametro, rp_nombre, rp_tipo, rp_longitud, rp_descripcion) VALUES (?, ?, ?, ?, ?, ?)");
	...
	public static int update(Connection conn, ReporteParametro rp) throws SQLException {
		...
		try {
			pstmnt = conn.prepareStatement("UPDATE cg_reporte_parametro" + 
										   " SET rp_nombre = ?, rp_tipo = ?, rp_longitud = ?, rp_descripcion = ?" 				+
										   " WHERE id_reporte = ?  AND id_parametro = ?");
