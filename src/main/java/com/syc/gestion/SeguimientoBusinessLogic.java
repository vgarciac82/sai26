...
public class SeguimientoBusinessLogic extends DataSourceManager {
	...
	private static Logger log = Logger.getLogger(SeguimientoBusinessLogic.class);
	...
	public PaginaData selectSeguimiento(int id_tc, int id_caso, String u_login, PaginaData param_pd)
	...
	public String activo_turno(String idResponsable,String idcaso,String estado_asunto) throws GestionException {
		...
		try {
			if(estado_asunto.equals("S"))return activo="Cerrado";//si todo el asunto esta cerrado ya para que checamos esto regresamos cerrado
			conn = getConnection();
			String query = "SELECT COUNT(*) as conteo FROM CG_BITACORA_OPERACION "+
				   " WHERE  ID_CASO ="+idcaso+" AND  ID_OPERACION =10 AND RESPONSABLE_ID ='"+idResponsable+"'"+
				   " AND TERMINADA ='N'";
			String query_res="select count(*) as conteo from cg_bitacora_operacion where " +
				   " id_caso ="+idcaso+" and id_operacion in(4,5,12) and remitente_id = '"+idResponsable+"'";
		    ...
		    if(conn!=null){
			  ...
			  if(activo.equals("Cerrado")){  //como no tiene respuestas rechazadas solo validaremos que efectivamente haya enviado por lo menos una respuesta
