...
public class TablasBusinessLogic extends DataSourceManager {
	...
	private static final Logger log = Logger.getLogger(TablasBusinessLogic.class);
...
public String getCancelaDocPoliza(String nFolioDocPoliza,HttpServletRequest request, HttpServletResponse response)  {
	...
	}catch(Exception e){log.debug("Error al crear la conexión");}
	...
	try{
		...
		//33879
	}catch(Exception e){log.debug("Error al crear el caso");}
	...
	try{
		...
		Querys[0]=" insert into tdocpolizaCancelEncabezado select nFolioDocPoliza,fCarga,fAplicacion,cCentroContable,cRamo,cUnidadResponsable,"
			      +" cDocumentoHaplicado,nFolioPoliza,'XX' cTipoPoliza,nMes,cRevisado,aEjercicioFiscal,cUnidadResponsableContable,nFolioPolizaCancelacion"
			      ...
			      +" nFormatoPoliza from tDocPolizaEncabezado with(nolock) where nFolioDocPoliza="+nFolioDocPoliza;
		
		Querys[1]="insert into tDocPolizaCancelDetalle  select nFolioDocPoliza,nDocRenglon,nCuenta,nSubCuenta,cEvento,mImporte *(-1) mImporte,cCentroContable,"
			      ...
			      +" nIdSubGrupoEvento,cIdEventoManual,nNumeroEvento from tDocPolizaDetalle with(nolock) where nFolioDocPoliza="+nFolioDocPoliza;
		
		Querys[2]="insert into tPoliza select nFolioPoliza,fCreacion,cDescripcionPoliza,mTotalCargo,mTotalAbono,nMes,nCuenta,fAplicacion,nPolizaAutomatica,"
        	      +" cCentroContable,aEjercicioFiscal,'XX' cTipoPoliza,'DOCPOLIZACANCEL' cTipoDocumento,nFolioDocumento,DocHAplicado,cUsuarioAutorizo"
        	      +" from tPoliza with(nolock) where 'DOCPOLIZA'=cTipoDocumento and nFolioDocumento="+nFolioDocPoliza+ " and not exists(select * from tPoliza with(nolock) "
        	      +" where 'DOCPOLIZACANCEL'=cTipoDocumento and nFolioDocumento="+nFolioDocPoliza+"  )"; //and cCentroContable='10'
