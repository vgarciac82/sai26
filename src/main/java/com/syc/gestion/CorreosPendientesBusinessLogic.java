...
public class CorreosPendientesBusinessLogic extends DataSourceManager {
	...
	private static final Logger	log	= Logger.getLogger(CorreosPendientesBusinessLogic.class);
	...
	public int reenviaAlertasPendientes() throws Exception {
		...
		try {
			...
			if (cpbList != null && cpbList.size() > 0)
				for (Iterator<CorreosPendientesBean> i = cpbList.iterator(); i.hasNext(); tEnviados++)
					try{
