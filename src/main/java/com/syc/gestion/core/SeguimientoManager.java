...
*/
	...
	*/
			...
			*/
            ...
            String //, "
            query = //+"					imx.asunto,"
            "SELECT Count(*) " + "     FROM ( " + "          SELECT s.tc_id_tc, " + "                 s.b_id_caso, " + "                 s.b_c_folio, " + "                 MIN(s.b_c_fecha_ini)   AS b_c_fecha_ini, " + "	                MAX(s.b_co_fecha_ini)  AS b_co_fecha_ini, " + //+"					imx.referencia"
            "                 MIN(CASE WHEN s.bc_cerrado = 'S' THEN 'cerrado' ELSE 'activo' END) AS estado " + "            FROM fx_seguimientoConsulta( ?, '') as s" + "            " + ("".equals(fromCBO) ? "" : "," + fromCBO) + " ,imxanp imx" + "           WHERE " + "                s.c_id_caso = c.id_caso " + "				AND imx.id_gabinete = c.c_id_gabinete " + "             AND e.ce_os_responsable = co.co_responsable" + "             AND a.id_area           = e.id_area" + "           " + //,imx.asunto,imx.referencia "
            ("".equals(whereCBO) ? "" : " " + whereCBO) + where + "          GROUP BY s.tc_id_tc, s.b_id_caso, s.b_c_folio " + "          ) as seg";
            ...
            //, "
            query = //+ "        imx.asunto ,"
            " SELECT s.tc_id_tc, " + "        s.b_id_caso, " + "        s.b_c_folio, " + "        MIN(s.b_c_fecha_ini)   AS b_c_fecha_ini, " + "	       MAX(s.b_co_fecha_ini)  AS b_co_fecha_ini, " + //+ "        imx.referencia "
            "        MIN(CASE WHEN s.bc_cerrado = 'S' THEN 'cerrado' ELSE 'activo' END) AS estado " + "  FROM fx_seguimientoConsulta( ?, '') as s" + ("".equals(fromCBO) ? "" : "," + fromCBO) + " ,imxanp imx" + " WHERE " + "  s.c_id_caso = c.id_caso " + "  AND imx.id_gabinete = c.c_id_gabinete" + "  AND  e.ce_os_responsable = co.co_responsable " + "  AND a.id_area           = e.id_area" + //,imx.asunto,imx.referencia ";
            where + ("".equals(whereCBO) ? "" : " " + whereCBO) + " GROUP BY s.tc_id_tc, s.b_id_caso, s.b_c_folio ";
