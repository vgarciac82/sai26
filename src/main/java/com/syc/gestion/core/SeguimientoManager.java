...
public class SeguimientoManager {
	...
	private static Logger log = Logger.getLogger(SeguimientoManager.class);
	...
	*/
			...
			*/
			...
			String query = "SELECT v.FECHA_ENVIO,v.id_caso " 
				+ ",v.folio "
				+ ",id_tc "
				+ ",id_operacion "
				+ ",id_caso_oper "
				+ ",secuencial_operacion"
				+ ",secuencial_anterior "
				+ ",id_gabinete "
				+ ",fecharegistro "
				+ ",fecha_compromiso "
				...
				+ ",user03 "
				+ ",user05 "
				+ ",replace(replace(user08,char(13),' '),char(10),' ') as user08 "
				+ ",replace(replace(user10,char(13),' '),char(10),' ') as user10 "
				+ ",responsable_asunto "
				+ ",nombre_resp "
				+ ",cerrado "
				+ ",terminada "
				+ ",replace(replace(asunto,char(13),' '),char(10),' ') as asunto "
				+ ",referencia "
				+ ",remitente_asunto "
				+ ",cargo "
				+ ",nombre_rem "
				+ ",cargo_rem "
				+ ",responsable_asunto "
				+ ",puesto_responsable_asunto "
				+ ",remitente_id "
				+ ",otroresponsable as responsable_id "
				+ ",tipo_instruccion "
				+ "FROM vimx_seguimiento v " 
				+ "WHERE 1=1 "+where +" "
...
*/			
		...
		//		s.setObservacion(rs.getString("b_co_observacion"));
				...
				s.setTipoInstruccion(rs.getString("tipo_instruccion"));  //RDMB: SE manda tipo de instruccion al set
...
//				s.setRespuesta(rs.getString("respuesta"));
	...
	public static String selectRespuesta(Connection conn, int id_caso) throws SQLException {
		...
		try {
			...
			String query = "SELECT v.user08 " 
				+ "FROM vimx_reportes v " 
				+ "WHERE  "+where +" "
				+ "AND id_operacion = 5 "
				+ "ORDER BY v.secuencial_operacion";
	...
	*/		
			...
			*/
			...
			String query = "SELECT Count(*) "
				...
				+ "                 s.b_id_caso, "
				+ "                 s.b_c_folio, "
				+ "                 MIN(s.b_c_fecha_ini)   AS b_c_fecha_ini, " 
				+ "	                MAX(s.b_co_fecha_ini)  AS b_co_fecha_ini, "				
				+ "                 MIN(CASE WHEN s.bc_cerrado = 'S' THEN 'cerrado' ELSE 'activo' END) AS estado "//, "
				//+"					imx.asunto,"	
				//+"					imx.referencia"
				+ "            FROM fx_seguimientoConsulta( ?, '') as s"
				+ "            " + ("".equals(fromCBO) ? "" : "," + fromCBO) + " ,imxanp imx"
				+ "           WHERE "
				+ "                s.c_id_caso = c.id_caso "
				+ "				AND imx.id_gabinete = c.c_id_gabinete "
                + "             AND e.ce_os_responsable = co.co_responsable" 
                + "             AND a.id_area           = e.id_area"
                + "           " + ("".equals(whereCBO) ? "" : " " + whereCBO)
                + where
			    + "          GROUP BY s.tc_id_tc, s.b_id_caso, s.b_c_folio "//,imx.asunto,imx.referencia "
			    + "          ) as seg";
			...
			String query_pag = " SELECT * FROM ( " 
								+ "            SELECT *, ROW_NUMBER() OVER (ORDER BY  s.b_c_folio) AS RowNumber "
								+ "              FROM (" + query + " ) as s    ) as seg "
								+ "  WHERE RowNumber BETWEEN " + (in_pd.getTamanoPaginas() * in_pd.getNumeroPagina() + 1) 
								+ " AND " + in_pd.getTamanoPaginas() * (in_pd.getNumeroPagina() + 1);
