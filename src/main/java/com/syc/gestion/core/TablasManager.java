...
{
	private static Logger	log	= Logger.getLogger(TablasManager.class);	
...
{
     String Query="update tDocPolizaEncabezado set mTotalCargos=(select SUM(case when cEvento='CARGO' then mImporte else 0 end) "
    	          +"from tDocPolizaDetalle where nFolioDocPoliza="+nFolioDocPoliza+"), mTotalAbonos=(select SUM(case when cEvento='ABONO' then mImporte else 0 end)"
    	          +"from tDocPolizaDetalle where nFolioDocPoliza="+nFolioDocPoliza+") where nFolioDocPoliza="+nFolioDocPoliza+"   ";
...
String     		   Query= "SELECT ndocrenglon, "
						+"       SUBSTRING(td.ncuenta,1,22) + '0' AS ncuenta, "
						+"       dcuenta, "
						+"       nsubcuenta, "
						+"       cconcepto, "
						+"       CASE "
						+"         WHEN cevento = 'CARGO' THEN mimporte "
						+"         ELSE 0 "
						+"       END                 Cargos, "
						+"       CASE "
						+"         WHEN cevento = 'ABONO' THEN mimporte "
						+"         ELSE 0 "
						+"       END                 Abonos, "
						+"       parcial, "
						+"       nidgrupoevento, "
						+"       nidsubgrupoevento, "
						+"       cideventomanual, "
						+"       Isnull(cpartida, '')cPartida, "
						+"       Isnull(ccabms, '')  cCABMS, "
						+"       Isnull(ccucop, '')  cCUCOP, "
						+"       nnumeroevento, "
						+"       td.ncuenta AS nCuentaAplicacion "
						+"FROM   tdocpolizadetalle td "
						+"       INNER JOIN (SELECT ncuenta, "
						+"                          dcuenta "
						+"                   FROM   tcuentas)ct "
						+"               ON nfoliodocpoliza = " + nFolioDocPoliza + " "
						+"                  AND td.ncuenta = ct.ncuenta "
...
String nFoliodocPoliza="";
	...
	try{
		conn.setAutoCommit(false);
           ...
           while (rs.next())
		 nFoliodocPoliza=rs.getString(1);      	   
...
String     		   Query="select isnull(nFolioPoliza,'-1'),SUM(case when cEvento='CARGO' then mImporte end)Cargos,SUM(case when cEvento='ABONO' then mImporte end) Abonos"
	                     +" ,MAX(nDocRenglon)ndocrenglon,max(nNumeroEvento)numEvento,nIdGrupoEvento,nIdSubGrupoEvento,cIdEventoManual,ID_OPER from tDocPolizaEncabezado te with(nolock)"
					     +" inner join tDocPolizaDetalle td with(nolock)"
					     +" on td.nFolioDocPoliza=te.nFolioDocPoliza"
					     +" and  te.nFolioDocPoliza="+nFolioDocPoliza
					     +" INNER JOIN CG_CASO_OPERACION CO ON co.ID_CASO=te.nIdCasoOrigen"
					     +" group by te.nFolioPoliza,nIdGrupoEvento,nIdSubGrupoEvento,cIdEventoManual,ID_OPER";
