...
public class OperacionSiguienteManager {
	...
	public static int delete(Connection conn, int id_tc, int id_oper, int id_oper_sigte) throws SQLException {
		...
		try {
			pstmnt = conn.prepareStatement("DELETE FROM cg_operacion_siguiente with(rowlock) " + "WHERE id_tc = ? "
					+ "AND id_oper = ? " + "AND id_oper_sigte = ?");
	...
	public static int insert(Connection conn, OperacionSiguiente os) throws SQLException {
		...
		try {
			pstmnt = conn.prepareStatement("INSERT INTO cg_operacion_siguiente "
					+ "(id_tc, id_oper, id_oper_sigte, os_responsable, os_operacion) " + "VALUES (?, ?, ?, ?, ?)");
	...
	public static int update(Connection conn, OperacionSiguiente os) throws SQLException {
		...
		try {
			pstmnt = conn.prepareStatement("UPDATE cg_operacion_siguiente with(rowlock) " + "SET os_responsable = ?"
					+ ", os_operacion = ?" + "WHERE id_tc = ? " + "AND id_oper = ? " + "AND id_oper_sigte = ?");
	...
	public static int getNextId(Connection conn, int id_tc, int id_oper) throws SQLException {
		...
		try {
			pstmnt = conn.prepareStatement("SELECT MAX(id_oper_sigte) FROM cg_operacion_siguiente with(nolock) "
					+ "WHERE id_tc = ? AND id_oper = ?");
