...
public class UsuarioManager {
	...
	public static Usuario select(Connection conn, Usuario u) throws SQLException {
		...
		try {
			...
			where.append(token + " u_estatus = 'A'");//24-01-2013
	...
	public static int update(Connection conn, Usuario u) throws SQLException {
		...
		try {
			pstmnt = conn.prepareStatement("UPDATE cg_usuario with(rowlock) SET u_password = ?, u_nombre = ?"
	...
	public static Usuario getRamoUR(Connection conn,Usuario u) throws SQLException{
		...
		ResultSet rs=null;
		try{
			pstmnt= conn.prepareStatement("SELECT u.cUnidadResponsable, u.cRamo " + 
			"  FROM tCatalogoURCC c with(nolock) " +
			"     , tCatalogoUnidadResponsable u with(nolock) " +
			"     , CG_CAT_EMPLEADO e with(nolock) " +
