...
public class BitacoraOperacionManager {
	...
	public static BitacoraOperacion select(
		Connection conn,
		BitacoraOperacion bo)
	throws SQLException {
	...
	public static BitacoraOperacion[] selectAll(
		Connection conn,
		BitacoraOperacion bo)
	throws SQLException {
		BitacoraOperacion[] retVal = null;
		...
		try {
			...
			pstmnt = conn.prepareStatement(
				"SELECT * FROM cg_bitacora_operacion "
				+ where.toString()
				+ " ORDER BY id_caso, secuencial_operacion");
	...
	public static int insert(Connection conn, BitacoraOperacion bo) throws SQLException {
		...
		try {
			...
			pstmnt = conn.prepareStatement(
				"INSERT INTO cg_bitacora_operacion "
				+ "(id_bitacora, id_caso, id_tipo_caso, id_operacion, "
				+ " secuencial_anterior, secuencial_operacion, secuencial_siguiente,"
				+ " status, fecha_inicio, fecha_compromiso, fecha_termino, "
				+ " remitente_id, remitente_area, "
				+ " responsable_id, responsable_area, "
				+ " turnado_id, turnado_area, "
				+ " terminada, instruccion, leida, titulo_aplicacion,"
				+ " id_gabinete, folio, opr_stack) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
				+         "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
				+         "?, ?, ?, ?)");
	...
	public static int update(Connection conn, BitacoraOperacion bo) throws SQLException {
		...
		try {
			String query = "UPDATE cg_bitacora_operacion "
						 + " SET status=?, fecha_termino=?, "
						 + " secuencial_anterior=?, secuencial_siguiente=?, "
						 + " turnado_id=?, turnado_area=?, "
						 + " terminada=?, leida=?, instruccion=?, "
						 + " remitente_id=?, remitente_area=?, "
						 + " responsable_id=?, responsable_area=? "
						 + " WHERE id_bitacora=?";
...
--------------------------------------------------------------------------------------------------------------------------------*/
	...
	public static BitacoraOperacion nuevaBitacoraOperacion(
		Connection conn,
		String remId,
		String instruccion,
		String turnado,
	...
	throws SQLException {
		...
		if (co.getUser03() != null &&
			co.getUser03().trim().length() > 0) {
	...
	public static BitacoraOperacion findPreviousOperation(Connection conn, BitacoraOperacion bo) throws SQLException {
		...
		try {
			...
			String strQuery =
				"SELECT * FROM cg_bitacora_operacion "
				+ where.toString()
				+ " order by secuencial_operacion desc";
			/*
	...
	public static List selectBitacoraOperacion(
		Connection conn,
		String u_login,
		int id_tc,
		int id_oper)
	throws SQLException {
		...
		try {
			...
			String query =
				"select * from cg_bitacora_caso bc, cg_bitacora_operacion bo "
				+ "where bc.id_caso = bo.id_caso "
				+ "and (bo.remitente_id = ? or bo.responsable_id = ?) "
				+ "and bc.tipo_caso = ? "
				+ "and bo.id_operacion = ? "
				+ "order by bo.id_operacion, bc.folio";
	...
	public static String getGestionXml(Connection conn,
									   String tituloAplicacion,
									   String folio,
									   String realPath) throws SQLException {
		String retVal = null;
		String query1 =
			"SELECT distinct * FROM vimx_arbol WHERE "
			+ "typerow = 2 "
			+ "AND typecolumn = 'docto.externo' "
			+ "AND titulo_aplicacion = ? "
			+ "AND namecolumn = ? "
			+ "ORDER BY typerow, id_carpeta_padre, id_carpeta_hija";
		...
		try {
			...
			if (rs1.next()) {
				...
				if (rs2.next()) {
					...
					String query3 =
						"SELECT * FROM imx_volumen WITH (NOLOCK) "
						+ "WHERE volumen = ?";
					...
					if (rs3.next()) {
						...
						retVal = getGestionXml(
							rs3.getString("UNIDAD_DISCO"),
							rs3.getString("RUTA_BASE"),
							rs3.getString("RUTA_DIRECTORIO"),
							realPath,
							rs2.getString("NOM_ARCHIVO_VOL"),
							rs2.getString("NOM_ARCHIVO_ORG"));
					}
				}
			}
	...
	public static String getGestionXml(Connection conn,
		...
		String retVal = null;
		String query =
			"SELECT distinct * FROM vimx_arbol "
	...
	public static String getGestionXml(
		String unidad,
		String rutaBase,
		String rutaDirectorio,
		String realPath,
		String nombreArchivoVolumen,
