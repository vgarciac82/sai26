...
public class OperacionManager {
	...
	public static int insert(Connection conn, Operacion o) throws SQLException {
		...
		try {
			pstmnt = conn.prepareStatement("INSERT INTO cg_operacion " + "(id_tc, id_oper, o_numero, o_nombre, o_responsable" + ", o_descripcion, o_plantilla, o_tiempo_limite, o_alarma, o_post_display" + ", o_post_submit, o_on_load, o_on_submit, o_folder_docto) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public static int update(Connection conn, Operacion o) throws SQLException {
		...
		try {
			pstmnt = conn.prepareStatement("UPDATE cg_operacion with(rowlock) SET o_numero = ?, o_nombre = ?" + ", o_responsable = ?, o_descripcion = ?, o_plantilla = ?, o_tiempo_limite = ?" + ", o_alarma = ?, o_post_display = ?, o_post_submit = ?, o_on_load = ?"
				+ ", o_on_submit = ?, o_folder_docto = ? WHERE id_tc = ? AND id_oper = ?");
