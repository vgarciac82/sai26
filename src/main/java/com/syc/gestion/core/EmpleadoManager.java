...
public class EmpleadoManager {
	...
	public static final Logger log = Logger.getLogger(EmpleadoManager.class);
	...
	public static int insert(Connection conn, Empleado e) throws SQLException {
		...
		try {
			pstmnt = conn
				.prepareStatement("INSERT INTO cg_cat_empleado " +
								  "(ID_AREA, ID_EMPLEADO, " +
								   "CE_AP_PATERNO, CE_AP_MATERNO, " +
								   "CE_NOMBRE_COMPLETO, CE_OS_RESPONSABLE, " +
								   "SALUTACION, CARGO) " +
								   "VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
	...
	private static String buscaCargoRH(Connection conn, String uLogin) throws SQLException {
		String query = "SELECT descripcion_puesto " 
	                 + "FROM   cg_usuario usuario WITH(nolock) " 
			         + "       INNER JOIN v_empleados_giro empleado WITH(nolock) " 
	                 + "               ON usuario.cnumeroempleado = empleado.clave "
			         + "WHERE  usuario.u_login = ? ";
	...
	public static int update(Connection conn, Empleado e) throws SQLException {
		...
		try {
			pstmnt = conn.prepareStatement("UPDATE cg_cat_empleado " +
										   "SET ID_AREA = ?, " +
										   "CE_AP_PATERNO = ?, " +
										   "CE_AP_MATERNO = ?, " +
										   "CE_NOMBRE_COMPLETO = ?, " +
										   "SALUTACION = ?, " +
										   "CARGO = ? " +
										   //OJO: Esto se pone como CE_OS_RESPONSABLE
