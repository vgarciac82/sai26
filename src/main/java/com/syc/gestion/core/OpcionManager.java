...
public class OpcionManager {
    ...
    public static Vector selectByUser(Connection conn, String u_login, String o_estaenmenu, int id_producto) throws SQLException {
        Vector v = new Vector();
        ...
        try {
            ...
            /*pstmnt = conn.prepareStatement("SELECT DISTINCT o.* " + "FROM   cg_opcion o" 
					...
					+ " AND   od.u_login   = ? "
					+ " ORDER BY o.o_orden");*/
            pstmnt = conn.prepareStatement("SELECT DISTINCT o.ID_OPCION, o.O_DESCRIPCION,o.O_ESTAENMENU, REPLACE(O_ACTION, '#DESC#', O_DESCRIPCION) as O_ACTION , o.O_TARGET, o.O_ID_PARENT, o.O_ORDEN, o.ID_PRODUCTO, o.O_DELEGABLE " + " FROM   cg_opcion o" + ", cg_role_opcion r" + ", cg_usuario_role u " + " WHERE  o.id_opcion = r.id_opcion " + " AND    u.r_nombre  = r.r_nombre " + " AND    u.u_login   = ? " + where.toString() + // se agregan las condiciones delegadas de administracion
            " UNION SELECT o.id_opcion, o_descripcion, o_estaenmenu, " + " REPLACE(O_ACTION, '#DESC#', O_DESCRIPCION) as O_ACTION , o_target, o_id_parent, o_orden, 3 as id_producto, O_DELEGABLE " + " FROM  cg_opcion o, cg_usuario_opcion_delegada od WHERE  o.id_opcion =  od.id_opcion " + " AND   od.u_login   = ? " + " ORDER BY o.o_orden");
