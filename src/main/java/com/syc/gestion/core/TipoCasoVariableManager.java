...
public class TipoCasoVariableManager {
    ...
    public static int insert(Connection conn, TipoCasoVariable tcv) throws SQLException {
        ...
        PreparedStatement pstmnt = null;
        try {
            pstmnt = conn.prepareStatement("INSERT INTO cg_tipo_caso_variable (id_tc, id_tcv, tcv_nombre, " + "tcv_descripcion, tcv_etiqueta, tcv_tipo, tcv_longitud, tcv_indice, tcv_en_gaveta) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public static int update(Connection conn, TipoCasoVariable tcv) throws SQLException {
        ...
        PreparedStatement pstmnt = null;
        try {
            pstmnt = conn.prepareStatement("UPDATE cg_tipo_caso_variable SET tcv_nombre = ?" + ", tcv_descripcion = ?, tcv_etiqueta = ?, tcv_tipo = ?, tcv_longitud = ?" + ", tcv_indice = ?, tcv_en_gaveta = ? WHERE id_tc = ? AND id_tcv = ?");
