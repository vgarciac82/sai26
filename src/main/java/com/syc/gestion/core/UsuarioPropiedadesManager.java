...
public class UsuarioPropiedadesManager {
	...
	public static void insert(Connection conn, Map m) throws SQLException {
		if (!m.isEmpty())
			for (Iterator iter = m.keySet().iterator(); iter.hasNext();)
				insert(conn, (UsuarioPropiedades) m.get((String) iter.next()));
	}
	...
	public static int insert(Connection conn, UsuarioPropiedades up) throws SQLException {
		...
		try {
			pstmnt = conn
					.prepareStatement("INSERT INTO cg_usuario_propiedades (u_login, up_nombre, up_valor) VALUES (?, ?, ?)");
	...
	public static int update(Connection conn, UsuarioPropiedades up) throws SQLException {
		...
		try {
			pstmnt = conn.prepareStatement("UPDATE cg_usuario_propiedades with(rowlock) SET up_valor = ? "
					+ "WHERE u_login = ? AND up_nombre = ?");
