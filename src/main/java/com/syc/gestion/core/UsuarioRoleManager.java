...
public class UsuarioRoleManager {
    ...
    public static void insert(Connection conn, Map m) throws SQLException {
        if (!m.isEmpty())
            for (Iterator iter = m.keySet().iterator(); iter.hasNext(); ) insert(conn, (UsuarioRole) iter.next());
    }
    ...
    public static Map<String, Role> selectRoles(Connection conn, String u_login) throws SQLException {
        ...
        ResultSet rs = null;
        try {
            pstmnt = conn.prepareStatement("SELECT r.r_nombre, r.r_descripcion " + "FROM cg_role r with(nolock), cg_usuario_role ur with(nolock) WHERE r.r_nombre = ur.r_nombre AND ur.u_login = ?");
            pstmnt.setString(1, u_login);
            rs = pstmnt.executeQuery();
            while (rs.next()) {
                Role r = new Role();
    ...
    public static Map selectUsuarios(Connection conn, String r_nombre) throws SQLException {
        ...
        ResultSet rs = null;
        try {
            pstmnt = conn.prepareStatement("SELECT u.u_login, u.u_password, u.u_nombre, u.u_descripcion " + "FROM cg_usuario u with(nolock), cg_usuario_role ur with(nolock) WHERE u.u_login = ur.u_login AND ur.g_nombre = ?");
