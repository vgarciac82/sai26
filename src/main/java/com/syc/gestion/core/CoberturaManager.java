...
public class CoberturaManager {
	...
	public static Cobertura selectByIdUsuario(Connection conn, String u_login, int id_tabla, int id_producto, String titulo_aplicacion) throws SQLException {
		...
		try {
			...
			pstmnt = conn.prepareStatement("SELECT " +
					"    cb.* " +
					"FROM " +
					"    cg_usuario_cobertura uc " +
					",   cg_cobertura         cb " +
					"WHERE " +
					"    uc.u_login      = ? " +
					"AND uc.id_tabla     = cb.id_tabla " +
					"AND uc.id_cobertura = cb.id_cobertura " +
					"AND cb.id_tabla     = ? " +
					"AND cb.id_producto  = ?");
	...
	public static boolean isCasoVisible(Connection conn, Cobertura c, String id_caso) throws SQLException {
		...
		try {
			...
			String query = "SELECT DISTINCT count(c.id_caso) " +
						   " from " + c.getCoFromClause() +
						   " WHERE c.id_caso = ? " + 
						   " AND c_folio not like 'TMP-%' " +
						   c.getCoWhereClause();
	...
	public static HashMap casosFiltrados(Connection conn, FiltroCasos fc) throws SQLException {
		...
		String token = " WHERE ";
		if (fc.getTipoAsunto() != null && 
		    fc.getTipoAsunto().length() > 0) {
			where.append(token + "TIPOASUNTO = '"+fc.getTipoAsunto()+"' ");
			token = " AND ";
			if (fc.getTipoAsunto().equals("I")) {
				if (fc.getRemitenteInternoLogin() != null && 
					fc.getRemitenteInternoLogin().length() > 0) {
					where.append(token + " REMINULOGIN = '"+fc.getRemitenteInternoLogin()+"' ");
					token = " AND ";
				}
				if (fc.getRemitenteInternoNombre() != null && 
					fc.getRemitenteInternoNombre().length() > 0) {
					where.append(token + " REMINUNOMBRE = '"+fc.getRemitenteInternoNombre()+"' ");
					token = " AND ";
				}
				if (fc.getRemitenteInternoPuesto() != null && 
					fc.getRemitenteInternoPuesto().length() > 0) {
					where.append(token + " REMINPTONOM = '"+fc.getRemitenteInternoPuesto()+"' ");
					token = " AND ";
				}
				if (fc.getRemitenteInternoArea() != null && 
					fc.getRemitenteInternoArea().length() > 0) {
					where.append(token + " REMINDDESC = '"+fc.getRemitenteInternoArea()+"' ");
					token = " AND ";
				}
				
			} else if (fc.getTipoAsunto().equals("E")) {
				if (fc.getRemitenteExternoNombre() != null && 
					fc.getRemitenteExternoNombre().length() > 0) {
					where.append(token + " RENOMBRE = '"+fc.getRemitenteExternoNombre()+"' ");
					token = " AND ";
				}
				if (fc.getRemitenteExternoCargo() != null && 
					fc.getRemitenteExternoCargo().length() > 0) {
					where.append(token + " RECARGO = '"+fc.getRemitenteExternoCargo()+"' ");
					token = " AND ";
				}
				if (fc.getRemitenteExternoProcedencia() != null && 
					fc.getRemitenteExternoProcedencia().length() > 0) {
					where.append(token + " REPROCEDENCIA = '"+fc.getRemitenteExternoProcedencia()+"' ");
					token = " AND ";
				}
				if (fc.getRemitenteExternoEstado() != null && 
					fc.getRemitenteExternoEstado().length() > 0) {
					where.append(token + " REESTADO = '"+fc.getRemitenteExternoEstado()+"' ");
					token = " AND ";
				}
				if (fc.getRemitenteExternoMunicipio() != null && 
					fc.getRemitenteExternoMunicipio().length() > 0) {
					where.append(token + " REMUNICIPIO = '"+fc.getRemitenteExternoMunicipio()+"' ");
					token = " AND ";
				}
				if (fc.getRemitenteExternoLocalidad() != null && 
					fc.getRemitenteExternoLocalidad().length() > 0) {
		...
		String query = "SELECT id_caso from " +
					   "fx_FiltroCasos2('"+fc.getFechaInicial()+"','"+fc.getFechaFinal()+"') "+
					   where.toString()	;
