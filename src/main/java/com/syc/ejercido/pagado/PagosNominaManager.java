...
public class PagosNominaManager {
	...
	public static ArrayList<String> BuscaCompromisos(Connection conn,String listaIds, String cConcepto, String listaFechas, String listaLeyendas, String sUsuario, String solicitudPago, String sCuentaBancaria)throws Exception{
		...
		String nFolioSicop = "";
		//PSC
		String Sql = " SELECT distinct tCE.nFolioNomina, " +
				" 'H' AS Header, " +
				" CONVERT(nvarchar(10), tCE.fAplicacion,103), " +
				" CONVERT(nvarchar(10), tCE.fAplicacion,103) ," +
				" tCE.cRamo," +
				" tCE.cRamo," +
				" tCE.cRamo, " +
				" 'RHQ' UnidadResponsable," +
				" 'RHQ' UnidadResponsable," +
				" 'RHQ' UnidadResponsable, " +
				" 'N' ID_TIPO_MOVIMIENTO, " +
				" '7' AS OrigenPpto," +
				" '5' AS TipoSol, " +
				" 'MXN' TipoMoneda , " +
				" '1' TipoCambio, " +
				" 'NOM' TIPO_PAGO, " +
				" 'PENDIENTE' CveLeyenda, " +
				" isnull(B.sSicop,''), " +
				" 'Cuenta' CUENTA_BANCARIA, " +
				" tCE.RFC, " +
				" '1000', " +
				" rtrim(tCE.caNoContrarrecibo) FechaReferencia, " +
				" rtrim(tCE.caNoContrarrecibo) Referencia1, " +
				" rtrim(tCE.caNoContrarrecibo) Referencia2, " +
				" REPLACE(LEFT(tCE.cConcepto, 70),',','') cConcepto, " +
				" '' NotasReverso, " +
				" '' AMF, " +
				" '' NO_ACMI, " +
				" '' AuxiliarComodin, " +
				...
				" '' IVAANT, " +
				" 'NA' ID_DESTINO_GASTO, " + 
				" tComp.nFolioSicop AS nFolioSicop " +
				" FROM tNominaEncabezado tCE WITH (NOLOCK) " +
				" LEFT JOIN tBeneficiarioCapituloMil B WITH (NOLOCK) ON tce.RFC = B.sCodigoEntidad " +
				" LEFT JOIN pCatalogoTipoDocumento CTD WITH (NOLOCK) ON tCE.cIdTipoDocumento = CTD.cIdTipoDocumento " +
				" LEFT JOIN tCompromisoNominaEncabezado tComp WITH (NOLOCK) ON tComp.caNoCompromiso = tCE.caNoCompromiso " +
				" WHERE tCE.caNOcontrarrecibo in ('" + listaIds + "') ";
		...
		try {
			...
			while (rs.next()){
				...
				String encabezado = rs.getString(2)				//A
								+","+rs.getString(3)			//B
								+","+rs.getString(4).trim()		//C
								+","+rs.getString(5).trim()		//D
								+","+rs.getString(6).trim()		//E	
								+","+rs.getString(7).trim()		//F
								+","+rs.getString(8).trim()		//G
								+","+rs.getString(9).trim()		//H
								+","+rs.getString(10).trim()	//I
								+","+rs.getString(11).trim()	//J	
								+","+rs.getString(12).trim()	//K	
								+","+rs.getString(13).trim()	//L
								+","+rs.getString(14).trim()	//M
								+","+rs.getString(15).trim()	//N
								+","+rs.getString(16).trim()	//O
								+","+vcLeyenda					//P
								+","							//Q
								+","+vcBenef					//R
								+","+vRFC						//S
								+","+rs.getString(21).trim()	//T
								+","+rs.getString(22).trim()	//U
								+","+rs.getString(23).trim()	//V    
								+","+rs.getString(24).trim()	//W   
								+","							//X   
								+","+rs.getString(33).trim();	//Y   
				...
				//'" + solicitudPago + "'
				String Sql2 = " SELECT DISTINCT '" + id_evento + "' ID_EVENTO,'" + evento + "' EVENTO,ltrim(TCEP.cRamo) ID_RAMO_ML, 'RHQ', TCEP.aEjercicioFiscal, " +
				       	" TCEP.cGrupoFuncional, tCEP.cFuncion, tCEP.cSubFuncion, " +
				       	" /*tCEP.cProgramaGeneral*/ CASE WHEN tCEP.cProgramaGeneral IN (SELECT cProgramaGeneral FROM tCat_ProGeneralPlurianual WITH (NOLOCK)) THEN '00' ELSE tCEP.cProgramaGeneral END AS cProgramaGeneral, " + // CUANDO ES PROGRAMA GENERAL '09' SE MODIFICA PARA QUE MUESTRE '00' PARA EVITAR ERROR EN SICOP.
				       	...
				       	" substring(cpartida,3,1) CPARG_300, substring(cpartida,4,2) CPAR_159, tCEP.cTipoGasto, tCEP.cFuenteFinanciamiento, tCEP.cEntidadFederativa, " +
				       	" SUBSTRING( dbo.CambiaEPCarteraMeta(TPDD.EP),45, 11) AS cCartera, " +
						" '0000000000' as cUnidadEjecutora, '00' CCOP_163, " + //URVP.20102014 Se cambia el campo de cUnidadEjecutora a '0000000000' y CCOP_163 a '00'  
				        " '000' PL, '000' OFI, '00000' AUX1, '00000' AUX2, '0000000000' AUX3, " +
						" '" +nFolioSicop+ "', " +
						" convert(decimal(16,2), SUM(TPDD.mImporteNeto)), " +
						" MONTH(GETDATE()) MES_149, " +
						" CASE " +
						"	WHEN (SELECT dbo.fn_u_existenPartidasGD("+ rs.getString(1)+"))>0 THEN 'GD' "+
						"	WHEN cPartida IN (SELECT cPartida FROM cat_tipo_concepto_partida WITH (NOLOCK) WHERE id_tipo_concepto='GD') THEN 'GD' " +
						"	ELSE 'PT' END TIPO_CONTRATO, " + //TPDD.ID_TIPO_CONCEPTO TIPO_CONTRATO, Right(TPDD.ID_TIPO_MOVIMIENTO, 3) CONC_MOV, " + //URVP.19012014 SE CAMBIA A PT 000 DEBIDO A QUE SE INDICA QUE PARA EL ARCHIVO DE CARGA A SICOP SIEMPRE VA ESTA VARIABLE
						" '000' CONC_MOV, "+
						" isnull(TPDE.solicitudPago, '') SPAG_176 " + //, '' id_ctr_intdet, '' id_ctr_intdet2 " +
						" FROM tNominaDetalle TPDD WITH (NOLOCK) inner join tNominaEncabezado TPDE WITH (NOLOCK)" +
						" on TPDD.nFolioNomina = TPDE.nFolioNomina " +
						" inner join tCatalogoEP TCEP WITH (NOLOCK) on TPDD.EP = TCEP.EP " +
						" LEFT JOIN tLayoutCompromisos LC WITH (NOLOCK) ON TPDD.cIdRelacion = LC.caNoCompromiso " +
						" where TPDD.nFolioNomina = " + rs.getString(1) + filtro_concepto  +
						" group by TCEP.cRamo, TCEP.aEjercicioFiscal,TCEP.cGrupoFuncional, tCEP.cFuncion, " +
						" tCEP.cSubFuncion, tCEP.cProgramaGeneral, tCEP.cActividadInstitucional,tCEP.cProgramaPresupuestario, " +
						" TCEP.cPartida,TCEP.cTipoGasto,TCEP.cFuenteFinanciamiento,TCEP.cEntidadFederativa,SUBSTRING( dbo.CambiaEPCarteraMeta(TPDD.EP),45, 11), " +
						" LC.nDocumento,TPDD.ID_TIPO_CONCEPTO,TPDD.ID_TIPO_MOVIMIENTO, TPDE.solicitudPago ";
	
				pstmntD = conn.prepareStatement(Sql2);
				rs2 = pstmntD.executeQuery();
				int nLoop = 33; // SE CAMBIA DE 40 A 33 YA QUE SE MODIFICO LAYOUT QUE RECIBE SICOP  20/09/2018 FAV.
				//int nLoop2 = 1;
					if (solicitudPago != ""){
						nLoop = 33; // SE CAMBIA DE 40 A 33 YA QUE SE MODIFICO LAYOUT QUE RECIBE SICOP  20/09/2018 FAV.
					}
					while (rs2.next()){
	...
	public static ArrayList<String> CreaDocumentacionComprobatoria(Connection conn,String listaIds, String sConcepto)throws Exception{
		...
		//PSC
		String Sql = " select PDE.nFolioNomina, 'H' H, PDE.cRamo, 'RHQ', '' SOL_PAGO, '4', PDE.caNoContrarrecibo FOLIO_INTERNO, PDE.caNoContrarrecibo COMODIN from " +
		" dbo.tNominaEncabezado PDE WITH (NOLOCK) " +
		" WHERE PDE.caNOcontrarrecibo in ('" + listaIds + "') ";
		...
		try {
				...
				while (rs.next()){
					...
					String Sql2 = " SELECT top(1) DISTINCT PDE.cRamo, " +
							" DCD.DCD_FACTURA, " +
							" CONVERT(nvarchar(10), DCD.DCD_FECHA_FACTURA,103), " +
							" CONVERT(nvarchar(10), DCD.DCD_FECHA_RECEPCION,103) + ' 12:00:00 a.m.', " +
							" case cEstatus when 'Operada Pagada' then cpd.cCampoAdi3 else ISNULL(BB.CBEN, isnull(DCD.DCD_CBEN,'')) end, " +
							" case when B.cExtranjero = 1 then '05' else '04' end 'TipoBen', " +
							" DCD.DCD_TIPO_OPE, " +
							" CONVERT(decimal(17,2), DCD.DCD_VALOR), " +
							" CONVERT(decimal(17,2), PDE.mImporteNeto),  " +
							" CONVERT(decimal(17,2), DCD.DCD_IVADES), " +
							" CONVERT(decimal(17,2), DCD.DCD_IVA), " +
							" CONVERT(decimal(17,2), DCD.DCD_ISR), " +
							" CONVERT(decimal(17,2), DCD.DCD_MIL5), " +
							" CONVERT(decimal(17,2), DCD.DCD_MIL2), " +
							" CONVERT(decimal(17,2), DCD.DCD_OTRAS_RET), " +
							" CONVERT(decimal(17,2), DCD.DCD_PENALIZACION), " +
							" CONVERT(decimal(17,2), DCD.DCD_CONTRIBUCION), " +
							" DCD.DCD_CTOEXT, LEFT(DCD.DCD_CONCEPTO, 70) " +
							" from dbo.tNominaEncabezado PDE WITH (NOLOCK) " +
							" inner join dbo.tNominaDetalle PDD WITH (NOLOCK) on PDE.nFolioNomina = PDD.nFolioNomina " +
							" inner join [dbo].[tBeneficiario] B WITH (NOLOCK) on PDE.RFC = B.dRFC " +
							" where PDE.nFolioNomina = "+ nFolioCompromiso + " --and PDD.ID_TIPO_CONCEPTO = '" + sConcepto + "'";
	...
	public static boolean updateHeaderCompromisosRealimentacion(Connection conn, Integer nEnviadoSICOP, String caNoCompromiso) throws SQLException{
		...
		String queryUpdateEstatus = "UPDATE tCompromisoEncabezado SET nEnviadoSICOP = " + nEnviadoSICOP +
	...
	public static ArrayList<String> BuscaCLCNomina(Connection conn,String clcNomina)throws Exception{
		...
		String sqlEnc = " SELECT DISTINCT tnclcE.nFolioNOMINACLC, " +
				"'H' AS Header, " +
				"CONVERT(nvarchar(10), tnclcE.fAplicacion,103) AS fAplicacion, " +
				"CONVERT(nvarchar(10), tnclcE.fAplicacion,103) ,  " +
				"tnclcE.cRamo, " +
				"tnclcE.cRamo, " +
				"tnclcE.cRamo, " +
				"'RHQ' UnidadResponsable, " +
				...
				"CASE WHEN B.sSicop='S24676' AND cConcepto LIKE 'FONAC%' THEN '16' ELSE B.cCveLeyenda END CveLeyenda, " + //URVP.27012015 Se cambia a dato de columna agregada al catalogo de benef cap mil
				"isnull(B.sSicop,''), " +
				"tnclcE.cuentaBancaria AS CUENTA_BANCARIA, " +
				"tnclcE.cIdRFC, " +
				"'NOM', " +
				"'' AS FechaReferencia, " +
				"CASE " +
				"		WHEN B.sSicop='S24676' AND cConcepto LIKE 'FONAC%' THEN '0001-08082011FONAC' " +
				"		WHEN B.sSicop='S24676' AND cConcepto LIKE 'ISR%' THEN REPLACE(cConcepto,' ','') + 'VIAPEC' " +
				"       WHEN B.sSicop=BR.sSicop AND tnclcE.cIdRFC = BR.cRFC THEN BR.cReferancia " +
				"		ELSE '' " +
				"  	END Referencia1, " +
				"'' Referencia2, " +
				"REPLACE(cConcepto,',','') cConcepto, " +
				"'' NotasReverso, " +
				"rtrim(tnclcE.caNoContrarreciboCLC) NO_ACMI, " +
				"rtrim(tnclcE.caNoContrarreciboCLC) AuxiliarComodin, " +
				"'' CTR , " +
				"'' FolioDC, " +
				"'' IVAANT, " +
				"'6' ID_DESTINO_GASTO " + //URVP.23012015 se cambia el campo 'NA' por un 6
				" FROM tNOMINACLCEncabezado AS tnclcE (NOLOCK) LEFT JOIN tNOMINACLCDetalle (NOLOCK) AS tnclcD ON tnclcE.nFolioNOMINACLC = tnclcD.nFolioNOMINACLC "
				+ " LEFT JOIN tBeneficiarioCapituloMil B (NOLOCK) ON tnclcE.cIdRFC = B.scodigoentidad "
				+ " LEFT JOIN tBeneficiarioCapituloMilRef BR WITH (NOLOCK) ON tnclcE.cIdRFC = BR.cRFC AND B.sSicop = BR.sSicop "
				+ " WHERE caNoContrarreciboCLC = '"+clcNomina+"'";
		...
		try {
				...
				while (rs.next()){
					...
					String encabezado = rs.getString(2)							//A
									...
									+ "," + rs.getString(15).trim()				//N
									+ "," + rs.getString(16).trim()				//O
									+ "," + rs.getString(17).trim()				//P
									+ "," + rs.getString(18).trim()				//Q
									+ "," + rs.getString(19).trim()				//R
									+ "," + rs.getString(20).trim()				//S
									+ "," + rs.getString(21).trim()				//T
									+ "," + rs.getString(22).trim()				//U
									+ "," + rs.getString(23).trim()				//V
									+ "," + rs.getString(24).trim()				//W
									+ "," + rs.getString(25).trim()				//X
									+ "," + rs.getString(26).trim()				//Y
									+ "," + rs.getString(27).trim()				//Z
									+ "," + rs.getString(28).trim()				//AA
									+ "," + rs.getString(29).trim()				//AB
									+ "," + rs.getString(30).trim()				//AC
									+ "," + rs.getString(31).trim()				//AD
									+ "," + rs.getString(32).trim();			//AE
					encabezado = encabezado + "\r\n";
					arrListaComp.add(encabezado);
		
					String sqlDet = "  SELECT '622' ID_EVENTO, " +
									"'404_CLC' EVENTO, " +
									"Rtrim(tCEP.cRamo) ID_RAMO_ML, " +
									"'RHQ', " +
									...
									"SUBSTRING( dbo.CambiaEPCarteraMeta(tnDet.EP),45, 11) AS cCartera," +
									//"ltrim('0000000' + RTRIM(tCEP.cUnidadEjecutora)), " +
									...
									"'00000' AUX2, " +
									"'0000000000' AUX3, " +
									" isnull( CONVERT(decimal(17,2), tnDet.mImporteNeto),'0.00') Monto2, " + 
									" isnull( CONVERT(decimal(17,2), tnDet.mImporteNeto),'0.00') Monto, " +
									" MONTH(GETDATE()) MES_149, " +
									"'01' NRES, " +
									"'01' AS PPAG_177, " + //20.07.2015 SE CAMBIA A "01" (NOMINA ORDINARIA) Y NO "02" (EXTRAORDINARIA)
									" CASE " +
									"	WHEN (SELECT dbo.fn_u_existenPartidasGD((SELECT TOP 1 nFolioNOMINA FROM tNOMINACLCDetalle WITH(NOLOCK) WHERE nFolioNOMINACLC=" + nFolioCLC + ")))>0 THEN 'GD' " +
									"	WHEN cPartida IN (SELECT cPartida FROM cat_tipo_concepto_partida WHERE id_tipo_concepto='GD') THEN 'GD' " +
									"	ELSE 'PT' END TIPO_CONTRATO, " +
									"'000' CONC_MOV," + // Right(tnDet.ID_TIPO_MOVIMIENTO, 3) CONC_MOV, "
									" '' solicitudPago, '' id_ctr_intdet, '' id_ctr_intdet2, tnDet.caNoContrarrecibo AS recibo, " +
									" ( select top 1 cne.nFolioSicop from tNOMINAEncabezado ne, tCompromisoNominaEncabezado cne where tnDet.caNoContrarrecibo = ne.caNoContrarrecibo and ne.caNoCompromiso = cne.caNoCompromiso ) , " +
									" ( select top 1 ne.solicitudPago from tNOMINAEncabezado ne, tCompromisoNominaEncabezado cne where tnDet.caNoContrarrecibo = ne.caNoContrarrecibo and ne.caNoCompromiso = cne.caNoCompromiso ) " +
									" FROM tNOMINACLCEncabezado tNEnc INNER JOIN tNOMINACLCDetalle tnDet ON tNEnc.nFolioNOMINACLC = tnDet.nFolioNOMINACLC " +
									" INNER JOIN tCatalogoEP tCEP ON tCEP.EP = tnDet.EP " +
									" LEFT JOIN tLayoutCompromisos LC ON tnDet.cIdRelacion = LC.caNoCompromiso " +
									" WHERE tnDet.nFolioNOMINACLC = "+nFolioCLC+" ";
					...
					while (rs2.next()){
							...
							while (rs3.next()){
								...
								solicitudPago = rs3.getString("SPAG_176");
									...
									String detalle = rs2.getString(1)+","+rs2.getString(2)+","+rs2.getString(3)+","+rs2.getString(4).trim()+","+rs2.getString(5).trim()+","+rs2.getString(6)+","+
													 rs2.getString(7)+","+rs2.getString(8)+","+rs2.getString(9)+","+rs2.getString(10).trim()+","+rs2.getString(11).trim()+","+rs2.getString(12)+","+
													 rs2.getString(13)+","+rs2.getString(14)+","+rs2.getString(15)+","+rs2.getString(16).trim()+","+rs2.getString(17).trim()+","+rs2.getString(18)+","+
													 rs2.getString(19)+","+rs2.getString(20)+","+rs2.getString(21)+","+rs2.getString(22).trim()+","+rs2.getString(23).trim()+","+rs2.getString(24)+","+
													 rs2.getString(25)+","+rs2.getString(26)+","+rs2.getString(27)+","+
													 rs2.getString(29).trim()+","+
													 rs2.getString(28).trim()+","+
													 rs2.getString(38)+","+
													 rs2.getString(30)+","+rs2.getString(31)+","+rs2.getString(32)+","+rs2.getString(33)+","+rs2.getString(39);
									detalle = detalle + "\r\n";
									arrListaComp.add(detalle.toString());
					}
				}
		...
		try {
			...
			rs2 = pstmntD.executeQuery();
				...
				while (rs2.next()){
						...
						String detalle = rs2.getString(1)+","+rs2.getString(2)+","+rs2.getString(3)+","+rs2.getString(4).trim()+","+rs2.getString(5).trim()+"," +
	...
	public static ArrayList<String> BuscaEjercidoComparativoCapitulo (Connection conn)throws Exception{
		...
		String qryEnc = " SELECT vEC.unidadSicop, "
				 +" isnull((select SUM(vE.importe_148Sicop) "
			...
			//sai
				+" vEC.unidadSicop, " 
				+" isnull((select SUM(mImporteNeto) as mImporteNetoSAI from tEjercidoEncabezado tEE with(nolock), tEjercidoDetalle tED with(nolock) "
		...
		String unidadSicop = "";//, mil = "", dosMil = "", tresMil = "", cuatroMil = "", cincoMil = "", seisMil = "", totalGeneral = "";
		...
		while(rsEnc.next()){
			...
			String qryDet =  " SELECT vEC.unidadSicop, vEC.partidaSicop, "
				+"  isnull((select SUM(vE.importe_148Sicop) "
				+" 		from v_EjercidoComparativoCapitulo vE with(nolock) "
				+"  	where vE.capituloSicop = 1 " 
					+" 		and vE.unidadSicop = vEC.unidadSicop "
			...
			+" WHERE unidadSicop = '"+ unidadSicop +"' GROUP BY vEC.unidadSicop, vEC.partidaSicop " 
		...
		if(rsEnc != null){ rsEnc.close(); }
		if(rsDet != null){ rsDet.close(); }
		if(rsTotal != null){ rsTotal.close(); }
		if(pstEnc != null){ pstEnc.close(); }
		if(pstDet != null){ pstDet.close(); }
