...
public class ReportePasivosManager extends DataSourceManager {
    ...
    public String insertaTemp(Connection conn, String rfc, String pc, String mes, double mImporteNetoEnc, String fAplicacion, String ur, String cEjecicicioFiscal, String login, String estatus, int fTempGral) throws SQLException {
        ...
        try {
            ...
            psEnc = conn.prepareStatement(" INSERT INTO tpasivosContingentesLaborales_temp " + "		  ( folioTempGral,  idPasivoContingente, 		cRFC, 	 cMes, 		mImporte, 			  estatus, 	 	ur, 		usuario,	  fechaCaptura ) " + " VALUES( " + fTempGral + ",  " + IDPasivoContingente + ", '" + rfc + "', " + mes + ", '" + mImporteNetoEnc + "','" + estatus + "', '" + ur + "' , '" + login + "' , '" + fAplicacion + "' )");
            psEnc.executeUpdate();
            valor = "guardado";
        } catch (Exception e) {
    ...
    public String insertaPasivosLaborales(Connection conn, String fTempGral) throws SQLException {
        ...
        try {
            ...
            psEnc = conn.prepareStatement(" INSERT INTO tpasivosContingentesLaborales " + "	SELECT folioTempGral, idPasivoContingente, cMes, mImporte FROM tpasivosContingentesLaborales_temp WHERE folioTempGral =  " + fTempGral);
            psEnc.execute();
            valor = "guardado";
        } catch (Exception e) {
            log.error("Error al guardar el dato " + e);
