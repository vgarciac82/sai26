...
public class CLCAttachmentBusinessLogic extends DataSourceManager {
	...
	private static final Logger	log							= Logger.getLogger(CLCAttachmentBusinessLogic.class);
	...
	private static final String	queryInsertFolder			=   "INSERT INTO imx_carpeta " 
	                                                          + "            (titulo_aplicacion, " 
		                                                      + "             id_gabinete, " 
	                                                          + "             id_carpeta, " 
		                                                      + "             nombre_carpeta, " 
	...
	private static final String	queryNexFolderID			= "SELECT MAX(id_carpeta) FROM imx_carpeta WITH (NOLOCK) WHERE titulo_aplicacion = ? " 
	                                                          + "AND id_gabinete = ?";
	...
	private static final String	querySearchFolder			= "SELECT titulo_aplicacion, " + "       id_gabinete, " + "       id_carpeta, " + "       nombre_carpeta, " + "       nombre_usuario, " + "       bandera_raiz, " + "       fh_creacion, " + "       fh_modificacion, "
																+ "       numero_accesos, " + "       numero_carpetas, " + "       numero_documentos, " + "       descripcion, " + "       password " + "FROM   imx_carpeta WITH (NOLOCK) " + "WHERE  bandera_raiz <> 'S' "
																+ "       AND nombre_carpeta = ? " + "       AND id_gabinete = ? " + "       AND titulo_aplicacion = ?";

	private static final String	querySearchVolumen			= "SELECT volumen "
                                                            + "FROM   imx_volumen WITH (NOLOCK)" 
		                                                    + "WHERE  unidad_disco = ?  " 
                                                            + "        AND tipo_volumen = ? AND capacidad = ?";

	private static final String	querySearchVolumenUnit		= "SELECT unidad, " 
	                                                        + "       tipo_dispositivo, " 
		                                                    + "       ruta_base " 
	                                                        + "FROM   imx_unidad_volumen WITH (NOLOCK)" 
		                                                    + "WHERE  estado_unidad = ?";

	private static final String	queryBuscaPago				= "SELECT  caso.c_id_gabinete, " 
	                                                        + "	       tipoCaso.tc_gaveta_asociada " 
		                                                    + "	FROM   cg_caso caso WITH(nolock) " 
	                                                        + "	       INNER JOIN dbo.cg_tipo_caso tipoCaso WITH(nolock) " 
		                                                    + "	               ON caso.id_tc = tipoCaso.id_tc "
															+ "	WHERE  c_folio = ? ";
	
	private static final String queryBuscaSNP               =	"SELECT 'CAJA'     AS cTipoPago, " 
															  + "       id_caso AS nFolioPAGO "
															  + "FROM   tcajaencabezado with(nolock)  " 
															  + "WHERE  nfoliocaja = ? ";
																	
	private static final String queryBuscaGabineteSNP       =	"SELECT  c_id_gabinete, " 
															+	"       c_folio  "
															+	"FROM   cg_caso WITH(nolock) " 
															+	"WHERE  id_caso = ?"; 
		...
		try {
			...
			for (int i = 0; i < files.length; i++) {
				try {
					...
					List<AttachResult> resultProc = CLCAttachmentManager.attachComprobanteBancario(conn, centroContable, ignorarExistentes, cbFilePath, csBuscaCxP, csBuscaGabinete, psInsertFolderHierarchy, psInsertFolder, psNexIDFolder, psSearchFolder, psSearchVolumenUnit, psSearchVolumen,
						psSearchVolPath, tipoPago);
