...
public class ContratoCarga  extends DataSourceManager{
			...
			while ((sCadenas = brr.readLine()) != null) {
				...
				if (nRegistro > 1){
					...
					if(tipoArchivo.equals("cargaArchivo")){
						...
						String EntidadFederativa = celdas[0].trim();  	// Debe existir en la Tabla tCatalogoEntidadFederativa
						String Ejecutor = celdas[1].trim();  			// Debe existir en la Tabla dCat_Entidad_Ejec_Resp.
						String NoContrato = celdas[2].trim();  			// Debe tener cuando mucho 50 caracteres y no existir uno igual anterior.
						String Prestamo = celdas[3].trim();  			// Debe existir en la Tabla dPrestamo
						String CategoriaInversion = celdas[4].trim();  	// Debe existir en la Tabla dCategoriaInversion_prestamo y estar en el mismo registro del id_prestamo
						String Componente = celdas[5].trim();  			// Debe existir en la Tabla dComponente_Tecnico y estar en el mismo registro del id_prestamo
						String EjercicioFiscal = celdas[6].trim();
						String ImporteOriginal = celdas[7].trim();
						String ImporteTotal = celdas[8].trim();
						String RFCBeneficiario = celdas[9].trim();  	// Debe tener cuando mucho 20 caracteres
						String NombreBeneficiario = celdas[10].trim();  // Debe tener cuando mucho 300 caracteres
						String FechaFirma = celdas[11].trim();
						String FechaInicio = celdas[12].trim();
						String FechaTerminacion = celdas[13].trim();
						String ObjetoObra = celdas[14].trim();  		// Debe tener cuando mucho 500 caracteres
						String TipoAdjudicacion = celdas[15].trim();  	// Debe existir en la Tabla oCatalogoTipoAdjudicacion
						...
						else
						{
							...
							if(!"S&iacute; Existe Prestamo-Componente-Categoria".equals(valor)) szError = szError + valor;
							if(!"S&iacute; Existe Entidad Federativa".equals(valor2)) szError = szError + ", " + valor2;
							if(!"S&iacute; Existe Tipo Adjudicaci&oacute;n".equals(valor3)) szError = szError + ", " + valor3;
							if(!"S&iacute; Existe Ejecutora Responsable".equals(valor4)) szError = szError + ", " + valor4;
							if(!"No Existe Contrato".equals(valor5)) szError = szError + ", " + valor5;
							if(!"".equals(longitudContrato)) szError = szError + ", " + longitudContrato;
							if(!"".equals(longitudRFC)) szError = szError + ", " + longitudRFC;
							if(!"".equals(longitudBeneficiario)) szError = szError + ", " + longitudBeneficiario;
							if(!"".equals(longitudObjeto)) szError = szError + ", " + longitudObjeto + "\n";
							szError += ";";
						}
					}
				}
	...
	public String insertaLineaComprometidoEnc(Connection conn, String EntidadFederativa, String Ejecutor, String NoContrato, String Prestamo, String CategoriaInversion, String Componente, String EjercicioFiscal, String ImporteOriginal, String ImporteTotal, String RFCBeneficiario, String NombreBeneficiario,
