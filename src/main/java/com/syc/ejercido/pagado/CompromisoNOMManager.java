...
public class CompromisoNOMManager {
		...
		try {
				...
				while (rs.next()){
					...
					if(tipoContratoD == 1 || tipoContratoD == 4){
						...
						erogacion = "2";   //servicio
					}
					else if(tipoContratoD == 5){
						erogacion = "3";    //obra
					}
		
					//else if(tipoContratoO !=0 ){
					else {
						erogacion = "3";     //obra
					}
					erogacion = "8";
					...
					encabezado.append(rs.getString("Header").trim()).append(",");       // A
					encabezado.append(fechaALayout.trim()).append(",");                 // B
					encabezado.append(fechaLayout.trim()).append(",");                  // C
					encabezado.append(rs.getString("cRamo").trim()).append(",");        // D
					encabezado.append(rs.getString("cRamo").trim()).append(",");        // E
					encabezado.append(rs.getString("cRamo").trim()).append(",");        // F
					encabezado.append(rs.getString("cUnidadResponsableEP").trim()).append(","); // G
					encabezado.append(rs.getString("cUnidadResponsableEP").trim()).append(","); // H
					encabezado.append(rs.getString("cUnidadResponsableEP").trim()).append(","); // I
					encabezado.append(tipoMovto).append(",");                            // J
					encabezado.append(compromiso).append(",");                           // K
					encabezado.append(erogacion).append(",");                            // L
					encabezado.append(rs.getString("nFolioSICOP")).append(",");         // M
					encabezado.append(tipoOp).append(",");                               // N
					encabezado.append(rs.getString("cIdContrato")).append(",");         // O
					encabezado.append(rs.getString("cConceptoContrato")).append(",");   // P
					encabezado.append(rs.getString("CBEN").trim()).append(",");         // Q
					encabezado.append(rs.getString("cIdRFC").trim()).append(",");       // R
					encabezado.append(rs.getString("RepresLegal")).append(",");         // S
					encabezado.append(rs.getString("TPROC")).append(",");               // T
					encabezado.append(rs.getString("ESQ_PRECIO")).append(",");          // U
					encabezado.append(rs.getString("CONTRATACION")).append(",");        // V
					encabezado.append(fContratoIni.trim()).append(",");                 // W
					encabezado.append(fContratoFin.trim()).append(",");                 // X
					encabezado.append(fContratoIni.trim()).append(",");                 // Y
					encabezado.append("N").append(",");                                 // Z
					encabezado.append(rs.getString("APROB_PLA")).append(",");           // AA
					encabezado.append(rs.getString("ACTO_JURIDICO")).append(",");       // AB
					encabezado.append(monto).append(",");                         		// AC
					encabezado.append(rs.getString("cCodigoMonedaSiaff")).append(",");  // AD
					encabezado.append(rs.getString("TCAM")).append(",");                // AE
					encabezado.append(monto).append(",");                            	// AF
					encabezado.append(monto).append(",");                            	// AG
					encabezado.append(monto).append(",");                             	// AH
					encabezado.append("N").append(",");                                 // AI
					encabezado.append(rs.getString("CONV_MOD")).append(",");            // AJ
					encabezado.append(fechaALayout.trim()).append(",");                 // AK
					encabezado.append(rs.getString("Plurianual")).append(",");          // AL
					encabezado.append(rs.getString("Plurianual")).append(",");          // AM
					encabezado.append(rs.getString("Plurianual")).append(",");          // AN
					encabezado.append(rs.getString("nMes").trim()).append(",");         // AO
					encabezado.append(rs.getString("caNoCompromiso")).append(",");      // AP
					encabezado.append(rs.getString("ID_CTR_INT")).append(",");          // AQ
					encabezado.append(rs.getString("caNoCompromiso")).append(",");      // AR
					encabezado.append(rs.getString("caNoCompromiso")).append(",");      // AS
					encabezado.append("N").append(",");                                 // AT
					encabezado.append("NOMINA").append(",");                            // AU
					encabezado.append("0.00").append(",");                              // AV
					encabezado.append("0").append(",");                                 // AW
					...
					while (rs2.next()){
							...
							detalle.append(",").append(rs2.getString("EVENTO"));      // EVENTO
							detalle.append(",").append(rs2.getString("cRamo").trim()); // tCEP.cRamo
							detalle.append(",").append(rs2.getString("cUnidadResponsableEP").trim()); // tCEP.cUnidadResponsableEP
							detalle.append(",").append(rs2.getString("aEjercicioFiscal").trim()); // tCEP.aEjercicioFiscal
							detalle.append(",").append(rs2.getString("cGrupoFuncional").trim()); // tCEP.cGrupoFuncional
							detalle.append(",").append(rs2.getString("cFuncion").trim()); // tCEP.cFuncion
							detalle.append(",").append(rs2.getString("cSubFuncion").trim()); // tCEP.cSubFuncion
							detalle.append(",").append(rs2.getString("cProgramaGeneral")); // tCEP.cProgramaGeneral
							detalle.append(",").append(rs2.getString("cActividadInstitucional")); // tCEP.cActividadInstitucional
							detalle.append(",").append(rs2.getString("cProgramaPresupuestario")); // tCEP.cProgramaPresupuestario
							detalle.append(",").append(rs2.getString(12));            // SUBSTRING(tCEP.cPartida,1,1)
							detalle.append(",").append(rs2.getString(13));            // SUBSTRING(tCEP.cPartida,2,1)
							detalle.append(",").append(rs2.getString(14));            // SUBSTRING(tCEP.cPartida,3,1)
							detalle.append(",").append(rs2.getString(15));            // SUBSTRING(tCEP.cPartida,4,2)
							detalle.append(",").append(rs2.getString("cTipoGasto"));  // tCEP.cTipoGasto
							detalle.append(",").append(rs2.getString("cFuenteFinanciamiento")); // tCEP.cFuenteFinanciamiento
							detalle.append(",").append(rs2.getString("cEntidadFederativa")); // tCEP.cEntidadFederativa
							detalle.append(",").append(rs2.getString("cCartera"));    // tCEP.cCartera
							detalle.append(",").append(rs2.getString("CAU"));         // CAU
							detalle.append(",").append(rs2.getString("COP"));         // COP
							detalle.append(",").append(rs2.getString("PL"));          // PL
							detalle.append(",").append(rs2.getString("OF_"));         // OF_
							detalle.append(",").append(rs2.getString("AUX1"));        // CAMBIA VALOR FIJO '00000' // AUX1
							detalle.append(",").append(rs2.getString("AUX2"));        // AUX2
							detalle.append(",").append(rs2.getString("AUX3"));        // AUX3
							detalle.append(",").append(rs2.getString("Suficiencia")); // Suficiencia
							detalle.append(",").append(rs2.getString("Sol_OLI"));     // Sol_OLI
							detalle.append(",").append(rs2.getString("Enero"));       // Enero
							detalle.append(",").append(rs2.getString("Febrero"));     // Febrero
							detalle.append(",").append(rs2.getString("Marzo"));       // Marzo
							detalle.append(",").append(rs2.getString("Abril"));       // Abril
							detalle.append(",").append(rs2.getString("Mayo"));        // Mayo
							detalle.append(",").append(rs2.getString("Junio"));       // Junio
							detalle.append(",").append(rs2.getString("Julio"));       // Julio
							detalle.append(",").append(rs2.getString("Agosto"));      // Agosto
							detalle.append(",").append(rs2.getString("Septiembre"));  // Septiembre
							detalle.append(",").append(rs2.getString("Octubre"));     // Octubre
							detalle.append(",").append(rs2.getString("Noviembre"));   // Noviembre
							detalle.append(",").append(rs2.getString("Diciembre"));   // Diciembre
							detalle.append(",").append(rs2.getString("Importe"));     // Importe
							detalle.append("\r\n");
	...
	public static boolean insertRegisterLayout(Connection conn, String clave, String cRamo, String cUnidadResponsable, String folioSICOP, String idProceso, String cCentroContable,
			java.sql.Date fExpedicion, float total, String cTipoPoliza, String nFolioPoliza, String nPolizaCancelacion, String tipoMovimiento, String origenPresupuesto,
			String cuentaBancaria, String noSolicitud, String tCambio, String tMoneda, String tSolicitud, String volante, String rfc, String caNoCompromiso, String codSemarnat2,
			String estatus, java.sql.Date fAplicacion, String documento, String nDocumento, String descripcion) throws Exception {
		PreparedStatement pstmnt = null;
		...
		if (existReg == false) {
			try {
				...
				queryInsert.append("INSERT INTO tLayoutCompromisos(")
				           .append("cClave,cRamo,cUnidadCreadora,cFolioCompromisoSICOP,cIdProceso,")
				           .append("cCentroContable,fExpedicion,mImporte,cIdTipoPoliza,nPolizaSICOP,")
				           .append("nPolizaCancelacionSICOP,cIdTipoMovimiento,nOrigenPresupuesto,nCuentaBancaria,cNoSolicitud,")
				           .append("cTipoCambio,cMoneda,cTipoSolicitud,cVolante,dRFC,")
				           .append("caNoCompromiso,caNoSemarnat2,cEstatus,fAplicacionSICOP,tDocumento,")
				           .append("nDocumento,cDescripcion) ")
				           .append("VALUES('")
				           .append(clave).append("','")
				           .append(cRamo).append("','")
				           .append(cUnidadResponsable).append("','")
				           .append(folioSICOP).append("','")
				           .append(idProceso).append("','")
				           .append(cCentroContable).append("','")
				           .append(fExpedicion).append("',")
				           .append(total).append(",'")
				           .append(cTipoPoliza).append("','")
				           .append(nFolioPoliza).append("','")
				           .append(nPolizaCancelacion).append("','")
				           .append(tipoMovimiento).append("','")
				           .append(origenPresupuesto).append("','")
				           .append(cuentaBancaria).append("','")
				           .append(noSolicitud).append("','")
				           .append(tCambio).append("','")
				           .append(tMoneda).append("','")
				           .append(tSolicitud).append("','")
				           .append(volante).append("','")
				           .append(rfc).append("','")
				           .append(caNoCompromiso).append("','")
				           .append(codSemarnat2).append("','")
				           .append(estatus).append("','")
				           .append(fAplicacion).append("','")
				           .append(documento).append("','")
				           .append(nDocumento).append("','")
