...
public class ImpresionChequeManager {
	...
	private static final Logger log = Logger.getLogger(ImpresionChequeManager.class);
	...
	public static int reemplazaCheque(Connection conn, int nFolioCheque) throws Exception {
		...
		String queryInsertEncabezado =   " INSERT INTO tChequeEncabezado ("
										...
										+")" +
				  					   ...
				  					   "       ,null" +
				  					   "       ,NULL" +
				  					  // "       ,'Reemplazo del Cheque ' +CAST(? AS NVARCHAR(20))+' de la cuenta ' + ?" + // Cheque Anterior Banco
				  					   "       ,NULL " +
				  					   "       ,cBeneficiarioAlternativo " +
				  					   "       ,nfoliopolizapago " +
				  					   "       ,? "  +//NvoConcepto
				  					   "FROM   tChequeEncabezado WITH(NOLOCK) " +
				  					   "WHERE  nFolioCheque = ?  " + //Cheque anterior
		...
		String queryInsertDetalle = "INSERT INTO tChequeDetalle " +
				              	    "SELECT cEjercicio " +
				              	    "       ,cCentroContable " +
				              	    "       ,cUnidadResponsable " +
				              	    "       ,CTAB " +
				              	    ...
				              	    "       ,mImporte " +
				              	    "       ,mImporteNegativo " +
				              	    "FROM   tChequeDetalle WITH(NOLOCK) " +
				              	    "WHERE  nFolioCheque=?"; // Folio Cheque Anterior
		...
		try {
			...
			concepto = concepto.replaceAll("Generación del cheque "+chequeanterior, "Generación del cheque "+chequeSigte); //Cambiar el Concepto con nuevo numero de cheque
