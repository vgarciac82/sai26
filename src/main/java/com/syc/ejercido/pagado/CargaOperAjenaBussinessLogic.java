...
public class CargaOperAjenaBussinessLogic  extends DataSourceManager {
	private static Logger log = Logger.getLogger(AplicacionContable.class);
	
	/* Valida Informacion de Layout Operacion Ajena */
	...
	public String validarInformacion(InputStream in, String fAplicacion, String ur, String cEjecicicioFiscal, String login, String mes, String cContable)throws FileNotFoundException {
		...
		try {					
				conn = getConnection();
				CargaOperAjenaManager validaInformacion = new CargaOperAjenaManager();		
				// Tomar Folio Encabezado				
			
				
				fTemp = validaInformacion.folioTemp(conn);						
				if(fTempGral == 0){ fTempGral = fTemp; }
