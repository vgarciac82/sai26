            ...
            while ((sCadenas = brr.readLine()) != null) {
                ...
                if (nRegistro > 1) {
                    ...
                    if (tipoArchivo.equals("cargaArchivo")) {
                        ...
                        String NoContrato = celdas[0].trim();
                        // Debe existir en la Tabla dContrato y estar en el mismo registro del id_Contrato
                        String Prestamo = celdas[1].trim();
                        // Debe tener cuando mucho 20 caracteres
                        String NoFactura = celdas[2].trim();
                        ...
                        // Debe tener cuando mucho 10 caracteres
                        String Estimacion = celdas[10].trim();
                        // Debe tener cuando mucho 40 caracteres
                        String Concepto = celdas[11].trim();
                        ...
                        if ("S&iacute; Existe Pr&eacute;stamo-Contrato".equals(existe) && "".equals(longitudNoFactura) && "".equals(longitudEstimacion) && "".equals(longitudConcepto) && "No Existe Factura".equals(existeNoFactura)) {
                            ...
                            if (!"S&iacute; Existe Pr&eacute;stamo-Contrato".equals(existe))
                                szError = szError + existe;
                            if (!"No Existe Factura".equals(existeNoFactura))
                                szError = szError + ", " + existeNoFactura;
                            if (!"".equals(longitudNoFactura))
                                szError = szError + ", " + longitudNoFactura;
                            if (!"".equals(longitudEstimacion))
                                szError = szError + ", " + longitudEstimacion;
                            if (!"".equals(longitudConcepto))
                                szError = szError + ", " + longitudConcepto + "\n";
                            szError += "; ";
                        }
                    }
                }
