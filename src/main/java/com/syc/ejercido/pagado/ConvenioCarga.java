...
public class ConvenioCarga  extends DataSourceManager{
			...
			while ((sCadenas = brr.readLine()) != null) {
				...
				if (nRegistro > 1){
					...
					if(tipoArchivo.equals("cargaArchivo")){
						...
						String EjercicioFiscal = celdas[0].trim(); 		// Quitar este dato y poner el del Contrato en la tabla dConvenio.
						String NoContrato = celdas[1].trim(); 			// Debe existir en la Tabla dContrato
						String NoPrestamo = celdas[2].trim(); 			// Debe existir en dContrato y estar en el mismo registro que el dato anterior
						String ImporteConvenio = celdas[3].trim(); 		// Debe ser cuando mucho el 15% del Monto Original del Contrato
						String FechaFirma = celdas[4].trim(); 			// Debe ser posterior a la Fecha Firma del Contrato
						String NoConvenio = celdas[5].trim(); 			// Debe tener cuando mucho 50 caracteres
						String respFecha = "";
						...
						else
						{
							...
							if(!"S&iacute; Existe Contrato-Pr&eacute;stamo".equals(existe)) szError = szError + existe;
							if(!"".equals(longitudNoConvenio)) szError = szError + ", " + longitudNoConvenio;
							if(!"Fecha Correcta".equals(respFecha)) szError = szError + ", " + respFecha;
							if(!"No Existe Convenio".equals(valor5)) szError = szError + ", " + valor5;
							if(!respuestaImportes) szError = szError + ", el Importe del Convenio es mayor al 15 por ciento del Importe del Contrato";
							szError += ";";
	...
	public boolean comparaImportes ( Connection conn,double ImporteConvenio, String idC ) throws SQLException {
		...
		try{
			...
			respuesta = (impContrato*.15) >= ImporteConvenio; // en TRUE sí hace la inserción
