...
public class CargaPagosBoletajeManager {
	...
	private static final Logger	log	= Logger.getLogger(CargaPagosBoletajeManager.class);
	...
	public static int insertaVuelos(Connection conn, List<Vuelo> vuelos, int folioPago, String centroContable, String login, String nombreArchivo) throws Exception {
		String qureryInsert = "INSERT INTO	tPagoDiversoBoletajeAvion( nFolioPagoDiverso, RFC, cNombre, cReferencia, cRuta, cLineaAerea, mQ, mTarifa, mIVA, mTUA, mYR, mTotal, cPartida, cUnidadEjecutora, fFechaSalida, fFechaRegreso, cCentroContable, U_LOGIN, fRegistro, cNombreArchivo ) "
			+ " VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )";
	...
	public static boolean existeFolioPago(Connection conn, int nFolioPago) throws Exception {
		...
		String sQueryRead = " SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END AS bExiste FROM tPagoDiversoBoletajeAvion WITH (NOLOCK) " +
	...
	public static int insertaLayoutEncabezado(Connection conn, List<Vuelo> vuelos, int folioPago, String centroContable, String login, String nombreArchivo) throws Exception {
		...
		String qureryInsert = "INSERT INTO	tLayoutVuelosEnc( nFolioVuelos, cFolioVuelos, cCentroContable, U_LOGIN, fRegistro, Status ) "
			+ " SELECT ?, ?, ?, ?, GETDATE() AS fRegistro, 'A' AS Status  ";
		PreparedStatement ps = null;
		int insertados = 0;
		int nFolioVuelo = 0;	
	...
	public static int insertaLayoutDetalle(Connection conn, List<Vuelo> vuelos, int folioPago, String centroContable, String login, String nombreArchivo) throws Exception {
		...
		String qureryInsert = "INSERT INTO	tLayoutVuelosDet( RFC, cNombre, cReferencia, cRuta, cLineaAerea, mQ, mTarifa, mIVA, mTUA, mYR, mTotal, cPartida, cUnidadEjecutora, fFechaSalida, fFechaRegreso, cCentroContable, U_LOGIN, fRegistro, cNombreArchivo, nFolioVuelos, cFolioVuelos, Status, nDocRenglon, mImporteServicio , mImporteIVAServicio, mGranTotal) "
			+ " SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'A' AS Status, (SELECT (ISNULL(MAX(nDocRenglon), 0)) + 1 AS Renglon FROM tLayoutVuelosDet WITH (NOLOCK) WHERE nFolioVuelos = ?) , ?, ?, ? ";
		PreparedStatement ps = null;
		int insertados = 0;
		int nFolioVuelo = 0;		
		...
		try {
			...
			if (rs.next()) {
				...
				if ("1".equalsIgnoreCase(existe)){
					String sUpdateLayoutVuelos =   " UPDATE	vuelosdet SET vuelosdet.Status = 'A' " +
						" FROM		tLayoutVuelosDet vuelosdet WITH (NOLOCK) " +
						" INNER JOIN tComisionesSinComprobacionDet comisiondet WITH(NOLOCK) " +
							" ON " +
							" ( " +
							"	comisiondet.cBoleto = vuelosdet.cReferencia " +
							"	AND comisiondet.RFCVuelo = vuelosdet.RFC " +
							"	AND comisiondet.cNombreRFC = vuelosdet.cNombre " +
							"	AND comisiondet.mImporteBoleto = vuelosdet.mTotal " +
							" ) " +
	...
	private static boolean validaBeneficiarios(Connection conn, List<Vuelo> vuelos, int nFolio) throws Exception {
		...
		String sSql = 	" INSERT INTO tLayoutVuelosDet_Tmp (nFolioVuelos, iRenglon, RFC, cNombre, cReferencia, cRuta, mTotal, cPartida) " +
						" SELECT ?, ?, ?, ?, ?, ?, ?, ? ";
		
		String sSqlUpdate = " UPDATE	L " +
							" SET		L.bExiste = '1' " +
							" FROM	tLayoutVuelosDet_Tmp L WITH (NOLOCK) " +
							" WHERE EXISTS ( SELECT * FROM tBeneficiario WITH (NOLOCK) WHERE dRFC = L.RFC ) " +
							" AND		L.nFolioVuelos = ?  ";
	...
	private static boolean validaLayoutDetTmp(Connection conn, int nFolio) {
		...
		String sSql = 	" SELECT * FROM tLayoutVuelosDet_Tmp WITH (NOLOCK) " +
						" WHERE nFolioVuelos = ? AND RFC NOT IN ('INV38301000000', 'INV44102000000') AND bExiste IS NULL  " +
						" ORDER BY iRenglon ";	
		...
		try {
			...
			log.info(sSql + " [" + nFolio + "]" );
			log.info(sSqlDelete + " [" + nFolio + "]" );
