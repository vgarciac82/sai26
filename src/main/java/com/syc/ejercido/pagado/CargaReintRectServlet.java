 ...
 * @author Propietario
 */
@WebServlet(name = "CargaReintRectServlet", urlPatterns = { "/CargaReintRectServlet" })
public class CargaReintRectServlet extends HttpServlet implements GestionInterface {
    ...
    private static final Logger log = LoggerFactory.getLogger(CargaReintRectServlet.class);
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            fileItems = Util.parseRequest(req, CargaReintRectServlet.TEMP_DIR, -1);
            ...
            clc = conn.prepareCall(" { CALL SubirModificadoMAP (?, ?, ?) } ");
            clc.setString(1, archivoTabla);
            clc.setString(2, ruta);
            clc.setInt(3, mes);
            int res = clc.executeUpdate();
            if (res == 1) {
                log.info("Registros Insertados Exitosamente ");
                String msg = "Registros insertados exitosamente.";
