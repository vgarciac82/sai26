...
public class LayoutGeneralManager {
    ...
    public static ArrayList<String> ConsultaIngresosEgreso(Connection conn, String sWhereCla) throws Exception {
        ...
        arrDoc.add(titulo);
        try {
            String sql = "SELECT folioSai, banco, cuentaBancaria, dFecha, cMedioPago, cConcepto, TipoMovimiento, Convert(varchar(20), mImporte) as mImporteSinF " + "FROM v_RdbCargaArchivoRDB tbl WITH(NOLOCK) WHERE " + sWhereCla;
            //System.out.println(sql);
            pstm = conn.prepareStatement(sql);
            rs = pstm.executeQuery();
            while (rs.next()) {
