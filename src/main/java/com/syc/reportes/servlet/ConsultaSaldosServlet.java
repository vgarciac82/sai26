...
public class ConsultaSaldosServlet extends HttpServlet implements GestionInterface {
	...
	private synchronized void consultaSaldos(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			sql = "SELECT DISTINCT nCuenta " 
				+", REPLACE (cSubCuenta, '&', '&amp' )  cSubCuenta" 
				+", cCentroContable "  
				+", CONVERT(varchar, CAST( "+ sql_ini +" AS money), 1) AS SaldoInicial "
				+", CONVERT(varchar, CAST( "+ sql_cargos + " AS money), 1) AS Cargos "
				+", CONVERT(varchar, CAST( "+ sql_abonos + " AS money), 1) AS Abonos "
				+", CONVERT(varchar, CAST( "+ sql_ini + " + (CASE WHEN naturalezaDeLaCuenta = 'D' THEN " + " ( " + sql_cargos + " ) - ( " + sql_abonos + " ) "
				+" ELSE "+ " ( " + sql_abonos + " ) - ( " + sql_cargos + " )" 
				+" END ) AS money), 1) AS SaldoFinal "
				+"FROM dbo.tSaldosVista (NOLOCK) " 
				+" WHERE nCuenta LIKE '"   + cCuenta  +"' "
				+" AND cSubCuenta LIKE '"+ cSubCuenta +"' "
				+" AND cCentroContable LIKE '"+ Centro +"' " 
				+"GROUP BY nCuenta, cSubCuenta, cCentroContable, naturalezaDeLaCuenta";
