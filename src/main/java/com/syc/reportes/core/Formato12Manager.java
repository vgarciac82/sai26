	...
	public static String generaReporteFormato12(Connection conn, Map<String, String> plantillas) throws Exception {
		...
		try {
			...
			String query= "SELECT SUM (CASE "
				+ "						    WHEN (compOB.cCveOLI <>'-1' "
				+ "						                  AND compOB.cCveOLI <>'' "
				+ "						                     AND compOB.cTipoAdjudica='08') THEN compOB.nmontoconiva "
				+ "						              ELSE 0 "
				+ "						           END) AS TOTAL_Obra_I3P, "
				+ "						          SUM (CASE "
				+ "					                  WHEN ((cCveOLI ='-1' "
				+ "							                         OR cCveOLI ='') "
				+ "						                         AND cTipoAdjudica='08') THEN compOB.nmontoconiva "
				+ "							                    ELSE 0 "
				+ "							                END) AS TOTAL_Servicios_I3P, "
				+ "							               SUM (CASE "
				+ "							                        WHEN (compOB.cCveOLI <>'-1' "
				+ "							                              AND compOB.cCveOLI <>'' "
				+ "							                              AND compOB.cTipoAdjudica='23') THEN compOB.nmontoconiva "
				+ "							                        ELSE 0 "
				+ "							                    END) AS TOTAL_Obra_AD, "
				+ "							                   SUM (CASE "
				+ "							                            WHEN ((cCveOLI ='-1' "
				+ "							                                   OR cCveOLI ='') "
				+ "							                                  AND cTipoAdjudica='23') THEN compOB.nmontoconiva "
				+ "							                            ELSE 0 "
				+ "							                        END) AS TOTAL_Servicios_AD, "
				+ "							                         SUM (compOB.nmontoconiva) as monto_total "
				+ "							FROM tobrapublicacompromisoencabezado compOB WITH (NOLOCK) "
