...
public class ReporteNotasManager {
    ...
    private static final Logger log = LoggerFactory.getLogger(ReporteNotasManager.class);
    ...
    private static String generaReporteResumen(ResultSet rs, ResultSet rs1, ResultSet rs2, ResultSet rs3, ResultSet rs4, ResultSet rs5, ResultSet rs6, ResultSet rs7, ResultSet rs8, ResultSet rs9, ResultSet rs10, ResultSet rs11, ResultSet rs12, ResultSet rs13, ResultSet rs14, ResultSet rs15, ResultSet rs16, ResultSet rs17, ResultSet rs18, ResultSet rs19, ResultSet rs20, String plantillaPath, String fechaFin, int miles) throws Exception {
    ...
    public static void escribeExcel(int sheet, Workbook workbook, int renglonInicio, ResultSet rs, int totalcolumnas, int columnaInicio, int renglonPeriodo, String fechaFin) throws Exception {
        ...
        if (columnaInicio == 0) {
            ...
            colFecha = columnaInicio + totalcolumnas - 2;
        ...
        if (columnaInicio == 0 || columnaInicio == 1) {
            while (rs.next()) {
                ...
                if (contieneTipo) {
                    ...
                    colTipo = 0;
                if (rs.isLast() || colTipo == 1) {
                    //Cuando es total o es la ultima fila
            ...
            while (rs.next()) {
                ...
                if (contieneTipo) {
                    ...
                    colTipo = 0;
                if (rs.isLast() || colTipo == 1) {
                    //Cuando es la ultima columna o el total
                    for (int i = columnaInicio; i < totalcolumnas + columnaInicio; i++) {
                        if (i == 0)
    ...
    public static XWPFDocument generaReporteNotasWord(Connection conn, String fechaFin, int miles, XWPFDocument document, String efectivo, String derechos, String derAnt, String almacen, String cxp, String otrascxp, String pasivo, String pasivo2, String eventos, String eventos2, String eventos3, String fechaAut) throws Exception {
    ...
    public static double generaSaldos(Connection con, int mes, String cuenta) throws Exception {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = " declare @mes int = ? " + "	select (case @mes  when 1 then sum(msaldo1) " + " when 2 then sum(msaldo2) " + " when 3 then sum(msaldo3) " + " when 4 then sum(msaldo4) " + " when 5 then sum(msaldo5) " + " when 6 then sum(msaldo6) " + " when 7 then sum(msaldo7) " + " when 8 then sum(msaldo8) " + " when 9 then sum(msaldo9) " + " when 10 then sum(msaldo10)" + " when 11 then sum(msaldo11) " + " when 12 then sum(msaldo12) end) importe" + " from tsaldos (NOLOCK) where ncuenta like  ? + '%'";
        try {
    ...
    public static int obtieneJuicios(Connection con, String fechaFin) throws Exception {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = " SELECT count(*) juicios FROM ( " + " SELECT distinct cRFC juicios FROM tpasivosContingentes (NOLOCK)" + " WHERE (cFechaBaja > ? OR cFechaBaja IS NULL)) as tblJuicio";
        try {
            ps = con.prepareStatement(query);
