...
public class ReporteRetencionesManager {
	...
	public static final Logger log = Logger.getLogger( ReporteRetencionesManager.class );
	...
	private static String generaReporteTESOFE1CC(ArrayList<ResultSet> resultsets, String plantillaPath, String fechaInicio, String fechaFin
			, ArrayList<String> nombres, int tipoAjena) throws Exception {
		File cFileExcelPlantilla = new File(plantillaPath);
		String file_name =System.getProperty( "java.io.tmpdir" ) + File.separatorChar + "ReporteRetenciones" + "_" + System.currentTimeMillis() + "_" + String.valueOf((int) (Math.random() * 100)) + ".xls";
	...
	private static void recorreResultset (Workbook workbook, Sheet sheet, int renglonInicio, ResultSet rs,  int tipoAjena) throws Exception {
				...
				if (i==1)
					...
					if (i ==0 ){
						...
						if (!ur.equals(cell20.getStringCellValue())) {
							...
							celdasTotales.add( renglonInicio + cnt ); //Guardar las celdas donde se inserta un total
		...
		if (rs.isAfterLast()) {
			if (renglon < renglonIntermedio ) {
				...
				celdasTotales.add( renglonInicio + cnt + 2 ); //Guardar las celdas donde se inserta un total
