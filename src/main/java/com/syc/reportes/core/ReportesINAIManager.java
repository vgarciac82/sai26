...
public class ReportesINAIManager {
    ...
    public String generaReporteFormato7(Connection conn, Map<String, String> plantillas, int tipoReporte) throws Exception {
        ...
        String cadenaFecha = "Período: " + numCard + " de " + com.syc.adquisiciones.util.Util.getNameMonth(Integer.parseInt(fechaIni[1])) + " al " + fechaFin[0] + " de " + com.syc.adquisiciones.util.Util.getNameMonth(Integer.parseInt(fechaFin[1])) + " " + fechaIni[2] + " (*) (1)";
        InputStream fs = null;
        ...
        try {
            ...
            //formato de moneda _($* #,##0.00_);_($* (#,##0.00);_($* "-"??_);_(@_)  formato de numero #,##0.00
            estiloCell.setDataFormat(format.getFormat("_($* #,##0.00_);_($* (#,##0.00);_($* \"-\"??_);_(@_)"));
    ...
    private String queryFormatoV2Backup(int nMes) {
        ...
        return "select " + "'16'ramo  " + ",'RHQ'ur  " + ",'CONAFOR'dependencia " + ",cIdcontratoDefinitivo " + ",cConceptoContrato " + ",nCantidadClavesCucop " + ",cCucop " + ",nCantidadPartidas " + ",cPartidas" + ",mMontoContratado " + ",cTipoMoneda" + ",bMonedaExtranjera " + ",fFechaFallo " + ",fFechaInicio " + ",fFechaTermino " + ",cEsPluri " + ",cEstatus " + ",cExisteClausulaPenalizacion " + ",cExisteClausulaGarantia " + ",cPorcentajeGarantia " + ",nNumeroProveedores " + ",cRFCProveedores " + ",cRazonSocial " + ",cTipoProveedor " + ",cProcedimientoContratación " + ",cFundamentoLegal " + ",cRFCServPublicoSolicitante" + ",cRFCServPublicoAut " + ",cRFCServPublicoJuridico" + ",mMontoContratadoEjercicioAnt" + ",mPagoContEjercicioAnt" + ",mPagoContEjercicioAct " + "from mV2MatrizContratos with(nolock) " + where;
    }

    private String queryFormatoV2(String nameDB, String cMes) {
        return "select   " + "'16'ramo  " + ",'RHQ'ur  " + ",'CONAFOR'dependencia  " + ",sub.cIdContrato,sub.cConceptoContrato,numeroClavesCucop,cucop,numeroDePartidas,pardidas,montoContrato,tipoMoneda,monedaExtranjera  " + ",fechaFallo,FechaInicio,FechaTermino,esPluri,ESTATUS,existeClausulaPenalizacion,existeClausulaGarantia,porcentajeGarantia  " + ",numProveedores,rfcProveedor,razonSocial,tipoProveedor,procedimientoContratacion,fundamentoLeg,rfcServPubSolicitante  " + ",rfcServPubAut,rfcServPubJuridico  " + ",isnull((select " + nameDB + ".[dbo].[fn_mMontoContratadoProveedor]( rfcProveedor,nIdCategoria, nIdFundamentoLeg, cucop )),0) montoContratoEjercicioAnt  " + ",isnull((select " + nameDB + ".[dbo].[fn_mMontoPagadoProveedor]( rfcProveedor,nIdCategoria, nIdFundamentoLeg, cucop ," + cMes + ")),0) pagosContEjercicioAnt  " + ",pagosContEjercicioAct  " + "from  " + "	(select   " + "	contDiv.cIdContrato  " + "	,replace(REPLACE(REPLACE(contratos.cConceptoContrato,CHAR(10),''),CHAR(13),''),',','')  cConceptoContrato  " + "	,case when contratos.tipoProced=1 then(select dbo.fn_mCantidadCUCOP(contratos.cIdConsolidado))  " + "	else (select dbo.fn_mCantidadCUCOP2(contDiv.cIdContrato)) end numeroClavesCucop  " + "	,case when contratos.tipoProced=1 then rtrim((select dbo.fn_mCadenaCUCOP(contratos.cIdConsolidado)))   " + "		else rtrim((select dbo.fn_mCadenaCUCOP2(contDiv.cIdContrato))) end cucop  " + "	,(select [dbo].[fn_mCantidadPartidasContrato](contDiv.cIdContrato)) numeroDePartidas  " + "	,(select [dbo].[fn_mCadenaPartidasContrato](contDiv.cIdContrato))pardidas  " + "	,contratos.montoContrato+isnull(convenios.totalMod,0) montoContrato  " + "	,'PESOS'tipoMoneda  " + "	,0 monedaExtranjera  " + "	,convert(date,contratos.fFallo) fechaFallo  " + "	,convert(date,fContratoIni) FechaInicio  " + "	,convert(date,fContratoFin)FechaTermino  " + "	,case when contDiv.lEsPlurianual=1 then 'SI' else 'NO' end esPluri  " + "	,case when GETDATE()>convert(date,fContratoFin) then 'CONCLUIDO' ELSE 'VIGENTE'END ESTATUS  " + "	,'No se cuenta con la información'existeClausulaPenalizacion  " + "	,case when contratos.mTotalGarantias>0 then 'SI' else 'NO'end existeClausulaGarantia  " + "	,case when contratos.mTotalGarantias>0 then convert(varchar,round(contratos.mTotalGarantias*100/contratos.montoContrato,2))+' %' else '0 %'end porcentajeGarantia  " + "	,1 numProveedores  " + "	,contratos.cIdRFC rfcProveedor  " + "	,catProveedor.cRazonSocial razonSocial  " + "	,case when catProveedor.nIdPyme=6 then 'PUBLICO' else 'PRIVADO' end tipoProveedor  " + "	,catCategoriaProced.cDescripcionCorta procedimientoContratacion  " + "	,catFundamentoLeg.cFundamentoLegal fundamentoLeg  " + "	,''rfcServPubSolicitante  " + "	,''rfcServPubAut  " + "	,''rfcServPubJuridico  " + "	,ISNULL(pagos.Pagado,0) pagosContEjercicioAct  " + "	,catCategoriaProced.nIdCategoria  " + "	,catFundamentoLeg.nIdFundamentoLeg  " + "	from pContratoDiverso contDiv with(Nolock)  " + "	inner join(  " + "		select cIdContratoDefinitivo,cont.cIdProcedimiento,cont.cIdRFC,fFallo  " + "		,case when isnull(adjPart.montoMaximo,0)>isnull(adjPart.minimo,0) then isnull(adjPart.montoMaximo,0) else isnull(adjPart.minimo,0) end montoContrato  " + "		,cont.cConceptoContrato,adjPart.cIdConsolidado,isnull(documentacion.mTotalGarantias,0)mTotalGarantias  " + "		,adjPart.nIdFundamentoLeg,adjPart.nIdCategoria  " + "		,mMontoTotalPlurianual  " + "		,1 tipoProced  " + "		from mcontrato as cont with(Nolock)  " + "		inner join mDocumentacionContrato as documentacion with(Nolock) on documentacion.cIdContrato=cont.cIdContratoDefinitivo  " + "		inner join(select adjPart.cIdProcedimiento,SUM(mMontoNetoMinimo) as minimo,SUM(mMontoNetoLineaMax)as montoMaximo,procedAdj.cIdRFC,adjPart.cIdConsolidado   " + "			,procedAdj.nIdFundamentoLeg,proced.nIdCategoria  " + "			,proced.cEjercicio,proced.cIdUnidadEjecutora,proced.cIdTipoProcedimiento, proced.nIdConsecutivo  " + "			,procedAdj.mMontoTotalPlurianual,adjPart.nIdconsecutivoAdj  " + "			from mProcedimientoAdjudicacionPartidas as adjPart with(Nolock)   " + "			INNER JOIN mProcedimiento as proced with(Nolock) on proced.cEjercicio=adjPart.cEjercicio  " + "			and proced.cIdUnidadEjecutora=adjPart.cIdUnidadEjecutora  " + "			and proced.cIdTipoProcedimiento=adjPart.cIdTipoProcedimiento  " + "			and proced.nIdConsecutivo=adjPart.nIdConsecutivo  " + "			and proced.cIdProcedimiento=adjPart.cIdProcedimiento  " + "			INNER JOIN mProcedimientoAdjudicacion AS procedAdj with(Nolock) on procedAdj.cEjercicio=adjPart.cEjercicio  " + "			and procedAdj.cIdUnidadEjecutora=adjPart.cIdUnidadEjecutora  " + "			and procedAdj.cIdTipoProcedimiento=adjPart.cIdTipoProcedimiento  " + "			and procedAdj.nIdConsecutivo=adjPart.nIdConsecutivo  " + "			and procedAdj.cIdProcedimiento=adjPart.cIdProcedimiento  " + "     		and procedAdj.cIdRFC=adjPart.cIdRFC  " + "     		and procedAdj.nIdconsecutivoAdj=adjPart.nIdconsecutivoAdj  " + "			group by adjPart.cIdProcedimiento,procedAdj.cIdRFC,adjPart.cIdConsolidado,procedAdj.nIdFundamentoLeg,proced.nIdCategoria  " + "			,proced.cEjercicio,proced.cIdUnidadEjecutora,proced.cIdTipoProcedimiento, proced.nIdConsecutivo  " + "			,procedAdj.mMontoTotalPlurianual,adjPart.nIdconsecutivoAdj  " + "		)adjPart on adjPart.cEjercicio=cont.cEjercicio   " + "		and  adjPart.cIdUnidadEjecutora=cont.cIdUnidadEjecutora  " + "		and adjPart.cIdTipoProcedimiento=cont.cIdTipoProcedimiento  " + "		and adjPart.nIdConsecutivo=cont.nIdConsecutivoProcedimiento  " + "		and adjPart.cIdProcedimiento=cont.cIdProcedimiento and cont.cIdRFC=adjPart.cIdRFC  and  cont.nIdconsecutivoAdj=adjPart.nIdconsecutivoAdj" + "		where cont.nIdEstado=4  " + /*"		-----------------Plurianuales---------------  "+*/
        "		union  " + "		select   " + "		pluriContrato.cidcontratodefinitivo  " + "		,'N/A'cidProcedimiento  " + "		,pluriContrato.cidrfc  " + "		,convert(date,pluriContrato.fFallo)fFallo  " + "		,pluriContrato.mMontoNeto  " + "		,pluriContrato.cConceptoContrato  " + "		,'N/A'cidconsolidado  " + "		,0 totalGarantias  " + "		,pluriContrato.nIdFundamentoLeg  " + "		,pluriContrato.nIdCategoria  " + "		,contPlu.mMontoTotalPlurianual totalPluri  " + "		,2tipoProced  " + "		from mplurianualidadContrato pluriContrato with(Nolock)  " + "		inner join mContratoPlurianualidad as contPlu with(Nolock)  " + "		on contPlu.cIdContratoDefinitivo=pluriContrato.cIdContratoDefinitivo  " + "		where pluriContrato.nIdEstado=4  " + /*"		-----------------------Remanentes--------------  "+*/
        "		Union  " + "		select   " + "		cIdContratoDefinitivo  " + "		,'N/A'cidProcedimiento  " + "		,case when len(remanente.cidrfc)=12 then substring(remanente.cidrfc,1,3)+'-'+ substring(remanente.cidrfc,4,6)+'-'+ substring(remanente.cidrfc,10,LEN(remanente.cidrfc))  " + "		else substring(remanente.cidrfc,1,4)+'-'+ substring(remanente.cidrfc,5,6)+'-'+ substring(remanente.cidrfc,11,LEN(remanente.cidrfc)) end rfcComp  " + "		,convert(date,remanente.fFallo)fFallo  " + "		,remanente.mTotalRemanente  " + "		,remanente.cConcepto  " + "		,'N/A'cidconsolidado  " + "		,0 totalGarantias  " + "		,remanente.nIdFundamentoLeg  " + "		,remanente.nIdCategoria  " + "		,0 totalPluri  " + "		,3 tipoProced  " + "		from mContratoRemanenteEjercicioAnterior as remanente with(Nolock)  " + "		where nEstado=4  " + /*"		-------------------Convenios de ejercicios anteriores---------------  "+*/
        "		union  " + "		select   " + "			cIdContratoDefinitivo  " + "			,'N/A'cidProcedimiento  " + "			,cIdRFC  " + "			,convert(date,convEjerAnt.fFallo)fFallo  " + "			,convEjerAnt.mTotalModificacion  " + "			,convEjerAnt.cObjetoConvenio  " + "			,'N/A'cidconsolidado  " + "			,0 totalGarantias  " + "			,convEjerAnt.nIdFundamentoLeg  " + "			,convEjerAnt.nIdCategoria  " + "			,0 totalPluri  " + "			,4 tipoProced  " + "		from mContratoModificado convEjerAnt with(Nolock)  " + "		where nEstado=4 and isConvEjercicioAnt=1  " + "		and tipoMod=0  " + "	)contratos on contratos.cIdContratoDefinitivo=contDiv.cIdContrato  " + "	inner join mCatalogoProveedor catProveedor with(nolock) on catProveedor.cIdRFC=contratos.cIdRFC  " + "	inner join mCatalogoCategoriaProcedimiento as catCategoriaProced with(Nolock) on catCategoriaProced.nIdCategoria=contratos.nIdCategoria  " + "	inner join mCatalogoFundamentoLegal as catFundamentoLeg with(Nolock) on catFundamentoLeg.nIdCategoria=contratos.nIdCategoria  " + "	and catFundamentoLeg.nIdFundamentoLeg=contratos.nIdFundamentoLeg  " + /*"	----Convenios-----  "+*/
        "	LEFT join(select   " + "		contMod.cIdContratoDefinitivo  " + "		,sum(isnull(contMod.mTotalModificacion,0))as totalMod  " + "		From mContratoModificado as contMod  " + "			where contMod.nEstado=4   " + "			and isConvEjercicioAnt=0  " + "			and tipoMod=0  " + "			group by contMod.cIdContratoDefinitivo  " + "	)convenios	on convenios.cIdContratoDefinitivo=contratos.cIdContratoDefinitivo  " + "	left outer JOIN (		  " + "					SELECT   " + "						SUM(ISNULL(sub.pagos,0)-ISNULL(sub.totalReintegro,0))Pagado  " + "						,sub.cFolioPAGODIVERSO folioContPed  " + "						FROM	  " + "							(SELECT   " + "								CONVERT(VARCHAR,sum(isnull(d.mImporteMasIva,0)),1)pagos  " + "								,ISNULL(reintegro.totalReintegro,0)totalReintegro  " + "								,e.cFolioPAGODIVERSO  " + "								FROM dbo.tPAGODIVERSOEncabezado e WITH (NOLOCK)	  " + "								INNER JOIN dbo.tPAGODIVERSODetalle d WITH (NOLOCK) ON e.nFolioPAGODIVERSO = d.nFolioPAGODIVERSO AND e.cDocumentoHaplicado='S'	  " + "								inner join tPagadoEncabezado as pe with(Nolock) on pe.caNoContrarrecibo=e.caNoContrarrecibo and pe.cDocumentoHaplicado='S'  " + "								LEFT join (  " + "											SELECT   " + "											SUM(mImporte) totalReintegro,cxp  " + "											FROM  tReintegroEncabezado as reintEnc with(Nolock)  " + "											inner join tReintegroDetalle as reintDet with(Nolock)   " + "											on reintDet.nFolioReintegro=reintEnc.nFolioReintegro and reintEnc.cDocumentoHaplicado='S'  " + "											where month(reintEnc.fAplicacion)=" + cMes + "	GROUP BY cxp  " + "								)reintegro ON reintegro.cxp=e.caNoContrarrecibo  " + "								WHERE month(e.faplicacion)=" + cMes + "  " + "								GROUP BY reintegro.totalReintegro,e.cFolioPAGODIVERSO  " + "							)sub  " + "						  " + "							GROUP BY sub.cFolioPAGODIVERSO	  " + "				) AS pagos ON pagos.folioContPed=contratos.cIdContratoDefinitivo  " + "	)sub order by sub.fechaFallo ";
    }
    ...
    private static String generaQuery(int tipoReporte) {
        String query = "";
        switch(tipoReporte) {
            case //Contratistas y Proveedores
            1:
                query = "SELECT " + "(SELECT TOP(1) Datepart (year, b_co_fecha_ini) " + " FROM   cg_bitacora WITH (nolock) " + " WHERE  b_c_folio = cfolio " + "        AND b_co_responsable_sigte = 'Autoriza_Proveedor')      AS Ejercicio, " + "(SELECT TOP(1)  CONVERT(VARCHAR, fInicio, 103)fInicio FROM  mCatalogoTrimestre WITH (nolock) " + "WHERE nIdTrimestre=(Isnull(Datepart(quarter, (SELECT TOP(1) CONVERT(VARCHAR, b_co_fecha_ini, 103) " + "                          FROM   cg_bitacora WITH (nolock)  " + "                          WHERE  b_c_folio = cfolio  " + "                                 AND b_co_responsable_sigte = " + "                                     'Autoriza_Proveedor')), 1)))fInicioTrimestre, " + "(SELECT TOP(1)  CONVERT(VARCHAR, fFin, 103)fFin FROM  mCatalogoTrimestre WITH (nolock) " + "WHERE nIdTrimestre=(Isnull(Datepart(quarter, (SELECT TOP(1) CONVERT(VARCHAR, b_co_fecha_ini, 103) " + "                          FROM   cg_bitacora WITH (nolock)  " + "                          WHERE  b_c_folio = cfolio  " + "                                 AND b_co_responsable_sigte = " + "                                     'Autoriza_Proveedor')), 1)))fFinTrimestre, " + "CASE " + "  WHEN cidtipopersona = 2 THEN 'Física' " + "  ELSE 'Moral' " + "END AS Tipo, " + "CASE " + "  WHEN cidtipopersona = 1 THEN '' " + "  ELSE alta.cnombre " + "END AS nombre, " + "CASE " + "  WHEN cidtipopersona = 1 THEN '' " + "  ELSE alta.capellidopaterno " + "END AS APaterno, " + "CASE " + "  WHEN cidtipopersona = 1 THEN '' " + "  ELSE alta.capellidomaterno " + "END AS AMaterno, " + "CASE " + "  WHEN cidtipopersona = 1 THEN alta.crazonsocial " + "  ELSE '' " + "END AS RazonSocial, " + "pyme.cpyme AS " + "Estratificacion, " + "CASE " + "  WHEN cextranjero = 1 THEN 'Internacional' " + "  ELSE 'Nacional' " + "END AS Origen, " + "ef.centidadfederativa " + "EntidadFederativa, " + "cpais      Pais, " + "CASE " + "  WHEN cidtipopersona = 1 THEN alta.cidrfc " + "  ELSE '' " + "END AS RFC, " + "'No'       AS " + "Subcontrataciones, " + "alta.cgiro AS giro, " + "CASE " + "  WHEN cidtipopersona = 1 THEN 'Calle' " + "  ELSE '' " + "END AS tipoVialidad, " + "CASE " + "  WHEN cidtipopersona = 1 THEN alta.ccalle " + "  ELSE '' " + "END AS Calle, " + "CASE " + "  WHEN cidtipopersona = 1 THEN alta.cnumeroexterno " + "  ELSE '' " + "END AS NumExterno, " + "CASE " + "  WHEN cidtipopersona = 1 THEN alta.cnumerointerno " + "  ELSE '' " + "END AS NumInterno, " + "CASE " + "  WHEN cidtipopersona = 1 THEN 'Ciudad' " + "  ELSE '' " + "END AS " + "tipoAsentamiento, " + "CASE " + "  WHEN cidtipopersona = 1 THEN mun.mpo_nombre " + "  ELSE '' " + "END AS " + "nombreAsentamiento, " + "CASE " + "  WHEN cidtipopersona = 1 THEN mun.mpo_abreviatura " + "  ELSE '' " + "END AS " + "claveLocalidad, " + "CASE " + "  WHEN cidtipopersona = 1 THEN mun.mpo_nombre " + "  ELSE '' " + "END AS " + "NombreLocalidad, " + "CASE " + "  WHEN cidtipopersona = 1 THEN mun.mpo_abreviatura " + "  ELSE '' " + "END AS " + "claveMunicipio, " + "CASE " + "  WHEN cidtipopersona = 1 THEN mun.mpo_nombre " + "  ELSE '' " + "END AS " + "NombreMunicipio, " + "CASE " + "  WHEN cidtipopersona = 1 THEN mun.mpo_abreviatura " + "  ELSE '' " + "END AS " + "claveEntidadFederativa, " + "CASE " + "  WHEN cidtipopersona = 1 THEN ef.centidadfederativa " + "  ELSE '' " + "END AS " + "EntidadFederativa, " + "CASE " + "  WHEN cidtipopersona = 1 THEN alta.ccodigopostal " + "  ELSE '' " + "END AS CP, " + "CASE " + "  WHEN cidtipopersona = 1 THEN alta.cnombre " + "  ELSE '' " + "END AS nombre, " + "CASE " + "  WHEN cidtipopersona = 1 THEN alta.capellidopaterno " + "  ELSE '' " + "END AS APaterno, " + "CASE " + "  WHEN cidtipopersona = 1 THEN alta.capellidomaterno " + "  ELSE '' " + "END AS AMaterno, " + "''  AS telefono, " + "''  AS correo, " + "CASE " + "  WHEN cidtipopersona = 1 THEN 'IDENTIFICACION' " + "  ELSE '' " + "END AS " + "tipoAcreditacion, " + "CASE " + "  WHEN cidtipopersona = 1 THEN alta.curl " + "  ELSE '' " + "END AS WEB, " + "''  AS Telefono, " + "''  AS correo, " + "'https://sites.google.com/site/cnetrupc/rupc ' AS " + "HiperVinculoRegistro, " + "'http://directoriosancionados.funcionpublica.gob.mx/SanFicTec/jsp/Ficha_Tecnica/SancionadosN.htm' " + "HiperVinculoSancionados, " + "(SELECT TOP(1) CONVERT(VARCHAR, b_co_fecha_ini, 103) " + "FROM   cg_bitacora WITH (nolock) " + "WHERE  b_c_folio = cfolio " + "AND b_co_responsable_sigte = 'Autoriza_Proveedor')      AS fechaValidacion, " + "'Coordinación General de Administración' AS " + "ResponsableInformacion, " + "(SELECT TOP(1) Datepart (year, b_co_fecha_ini) " + "FROM   cg_bitacora  WITH (nolock) " + "WHERE  b_c_folio = cfolio " + "AND b_co_responsable_sigte = 'Autoriza_Proveedor')      AS Año, " + "CONVERT(VARCHAR, Getdate(), 103)  AS " + "FechaActualizacion, " + "CASE " + "WHEN cidtipopersona = 1 THEN " + "'Los datos de las columnas AE,AF,AI y AJ no se proporcionan toda vez que son datos personales que se encuentran protegidos de conformidad con los principios contenidos en el artículo 16 y lo establecido en el artículo 1 , párrafo cuarto de la Ley General de Protección de Datos Personales en Posesión de los Sujetos Obligados, así como en los artículos 68, fracción VI, 100 y 116 de la Ley General de Transparencia y Acceso a la Información Pública, 97 y 113, fracción I de la Ley Federal de Transparencia y Acceso a la Información Pública, y el trigésimo octavo, fracción I de los Lineamientos Generales en materia de clasificación y desclasificación de la Información, así como para la elaboración de versiones públicas.' " + "ELSE " + "'Los datos de las columnas  L, de la O a la AA, AE,AF,AI y AJ no se proporcionan toda vez que son datos personales que se encuentran protegidos de conformidad con los principios contenidos en el artículo 16 y lo establecido en el artículo 1 , párrafo cuarto de la Ley General de Protección de Datos Personales en Posesión de los Sujetos Obligados, así como en los artículos 68, fracción VI, 100 y 116 de la Ley General de Transparencia y Acceso a la Información Pública, 97 y 113, fracción I de la Ley Federal de Transparencia y Acceso a la Información Pública, y el trigésimo octavo, fracción I de los Lineamientos Generales en materia de clasificación y desclasificación de la Información, así como para la elaboración de versiones públicas.' " + "END AS Nota " + "FROM   taltaproveedor alta WITH (nolock) " + "       INNER JOIN mcatalogoproveedor catProv WITH (nolock) " + "               ON alta.cidrfc = catProv.cidrfc " + "       INNER JOIN mcatalogoentidadfederativa EF WITH (nolock) " + "               ON alta.cidentidadfederativa = ef.cidentidadfederativa " + "       INNER JOIN cat_municipio mun WITH (nolock) " + "               ON mun.id_municipio = alta.cidmunicipio " + "       INNER JOIN mcatalogopyme pyme WITH (nolock) " + "               ON pyme.nidpyme = alta.nidpyme " + "WHERE  catProv.lhabilitado = 1 " + "       AND cdocumentohaplicado IN ( 'S', 'V', 'R', 'P', " + "                                    'M', 'A', 'D' ) ";
                break;
            case //Adjudicacion Directa
            2:
                query = "SELECT " + " * FROM v_Reporte_INAI_Adj_Directa ";
                break;
        }
        return query;
    }
    ...
    private String queryServidoresPublicos(String fIni, String fFin) {
        //tabla 588058
    ...
    private String querypersonasBeneficiarias(String fIni, String fFin) {
        //tabla 588026
        return " select nombreCompletoPersonaBeneficiaria_508026,nombre,dApellidoPaterno,dApellidoMaterno " + " from [fn_mOrdenContratosINAIBenef] ('" + fIni + "','" + fFin + "') ";
    }
