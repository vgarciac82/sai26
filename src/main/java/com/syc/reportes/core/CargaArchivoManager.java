...
public class CargaArchivoManager {
    ...
    public static boolean insertArchivo(Connection conn, java.sql.Date fExpTmp, java.sql.Date fAppTmp, int folio, String descripcion, String codTran, String sucursal, String deposito, String retiro, String saldo) throws Exception {
        ...
        try {
            ...
            String queryInsert = "INSERT INTO tArchivoEjercidoPagado (dfechaOperacion, dfecha, nreferencia, cdescripcion, ncodtransac, nsucursal, mdepositos, mretiros, msaldo ) values('" + fExpTmp + "' ,'" + fAppTmp + "', " + folio + " ,'" + descripcion + "','" + codTran + "','" + sucursal + "','" + deposito + "','" + retiro + "','" + saldo + "')";
    ...
    public static boolean insertArchivoTaxis(Connection conn, java.sql.Date fAplicacion, String idEmpleado, String destino, String ur, String idComision, String folioTaxi, String monto) throws Exception {
        ...
        try {
            // falta cambiar la tabla del insert
            String queryInsert = "INSERT INTO tLayoutTaxis (fAplicacion, nIdEmpleado, cDestino, cUnidadResponsable, nidComision, cFolioTaxi, mMonto ) VALUES('" + fAplicacion + "' ," + idEmpleado + ",'" + destino + "','" + ur + "','" + idComision + "','" + folioTaxi + "','" + monto.trim() + "')";
            pstmnt = conn.prepareStatement(queryInsert);
            int reg = pstmnt.executeUpdate();
            if (reg == 1) {
                insertReg = true;
    ...
    public static String insertaPagado(Connection conn, int folio, Date fAplicacion) throws Exception {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                String queryEjer = "INSERT INTO tPagadoEncabezado (cTipoPago,nFolioPAGO,caNoContrarrecibo,nFolioPoliza,cTipoPoliza, " + " U_LOGIN, fAplicacion, nFolioPolizaCancelacion, cDescripcionPoliza, cUnidadResponsableContable, nFolioSICOP, cRamo, " + " cIdUsuarioCaptura, FechaAplicacionSicop, FechaPagoSicop, SolicitudPago,NumeroProceso,nFolioSIAFF,FechaPagado, nFolioPagado)" + " VALUES ('" + tipoPago + "', " + folio + ", RTRIM(LTRIM('" + canoContrarrecibo + "')), 0, 'EG', 'administrador', CAST ('" + fecha + "' AS DATE), 0, '" + descripcion + "','" + unidad + "','-1', '16', 'administrador', getdate(), getdate(), 9999, 9999, 9999, getdate()," + strFolioPagado + ")";
    ...
    public static boolean existeFolio(Connection conn, int folio, String importe) throws Exception {
        ...
        try {
            ...
            if (solicitudExiste > 0) {
                //La solicitud Bancaria existe en la base
                if (importeBanco == total) {
    ...
    public static String insertaEjercido(Connection conn, int folio, Date fAplicacion) throws Exception {
        ...
        try {
            ...
            if (rs2.next()) {
                ...
                String queryEjer = "INSERT INTO tEjercidoEncabezado (cTipoPago,nFolioPAGO,caNoContrarrecibo,nFolioPoliza,cTipoPoliza, " + " U_LOGIN, fAplicacion, nFolioPolizaCancelacion, cDescripcionPoliza, cUnidadResponsableContable, nFolioSICOP, cRamo, " + " cIdUsuarioCaptura, FechaAplicacionSicop, FechaPagoSicop, SolicitudPago,NumeroProceso,nFolioSIAFF,nFolioEjercido)" + " VALUES ('" + tipoPago + "', " + folio + ", RTRIM(LTRIM('" + canoContrarrecibo + "')), 0, 'DI', 'administrador', CAST ('" + fecha + "' AS DATE), 0, '" + descripcion + "','" + unidad + "','-1', '16', 'administrador', getdate(), getdate(), 9999, 9999, 9999, " + strFolioEjercido + ")";
                pstInsertar = conn.prepareStatement(queryEjer);
