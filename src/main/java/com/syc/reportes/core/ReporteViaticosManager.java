		...
		private static String generaReporteViaticos(ResultSet rs, String plantillaPath, int mesIni, int mesFin) throws Exception {
			...
			while (rs.next()) {
				...
				for (int i = 0; i < rsMetadata.getColumnCount(); i++) {
					if (rsMetadata.getColumnName(i + 1).equals("dSalida")
						|| rsMetadata.getColumnName(i + 1).equals("dRegreso")  
						|| rsMetadata.getColumnName(i + 1).equals("dFechaInforme") 
						|| rsMetadata.getColumnName(i + 1).equals("dFechaValidacion") 
						|| rsMetadata.getColumnName(i + 1).equals("dFechaActualizacion") ){
				...
				for (int i = 0; i < rsMetadata.getColumnCount(); i++) {
					...
					if (nombreColumna.equals("nIdComision")) {
						...
						if (brincar) {
							...
							Util.createExcelCellRep(i, rw, "", estiloTabla); // Celda vacía si es la misma comision
						}
