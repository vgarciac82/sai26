...
public class ConciliacionGastoDevengadoManager {
	...
	private static String generaReporte(Connection conn, ResultSet rs, ResultSet rs2, String plantillaPath, int mes, int anio) throws Exception {
		...
		pstm_8251 = conn.prepareStatement("SELECT CASE cTipoMovimiento WHEN 'C' THEN SUM(mMovimiento) ELSE 0 END AS cargo " +
											" FROM dbo.tMovimiento WITH (NOLOCK) " +  
											" WHERE SUBSTRING(nCuenta,1,4) IN('8251') " + 
													" AND MONTH(fMovimiento) BETWEEN 1 AND " +  mes +
													" AND YEAR(fMovimiento) = " + anio +		
													" AND cTipoMovimiento = 'C' " +
		...
		pstm_reint = conn.prepareStatement("SELECT SUM(cargo) AS cargo " +
											" FROM ( " +
													" SELECT '8251RP' AS cuenta " +
															" , CASE SUBSTRING(nCuenta,1,4) WHEN '8251' THEN 'egresos_presupuestarios' END AS concepto " +
															" , SUM(mMovimiento) AS cargo " +
															" , 0 AS abono " +		
														" FROM dbo.tMovimiento WITH (NOLOCK) " + 
														" JOIN vReintegrosAutorizados AS r ON cFolioDocumentoMovimiento = nFolioReintegroaut AND cTipoDocumento = tipoDocumento	" +
														" WHERE SUBSTRING(nCuenta,1,4) IN('8251') " + 
															" AND MONTH(fMovimiento) BETWEEN 1 AND " + mes +
															" AND YEAR(fMovimiento) =  " + anio +															
															" AND cTipoDocumento LIKE 'REINTEGRO%' " +	
															" AND SUBSTRING(r.cEvento,1,2) NOT IN ('RN') " +
														" GROUP BY SUBSTRING(nCuenta,1,4), cTipoMovimiento " +
													" UNION " +
														" SELECT SUBSTRING(nCuenta,1,4)+'RP' AS cuenta " +
															" , CASE SUBSTRING(nCuenta,1,4) WHEN '8251' THEN 'egresos_presupuestarios' END AS concepto " +
															" , (CASE cTipoMovimiento WHEN 'C' THEN SUM(mMovimiento) ELSE 0 END) AS cargo " +
															" , (CASE cTipoMovimiento WHEN 'A' THEN SUM(mMovimiento) ELSE 0 END) AS abono " +
														" FROM tMovimiento as m WITH (NOLOCK) " +	
														" WHERE SUBSTRING(nCuenta,1,4) IN ('8251') " + 
															" AND MONTH( fMovimiento) BETWEEN 1 AND " + mes +
															" AND YEAR(fMovimiento) = " + anio +
															" AND m.cTipoDocumento = 'DOCPOLIZA' " +
															" AND cFolioDocumentoMovimiento = 1088 " +	
														" GROUP BY SUBSTRING(nCuenta,1,4), cTipoMovimiento) TBL");
		
		pstm_551 = conn.prepareStatement("SELECT SUM(mSaldo" + mes + ") AS saldo FROM dbo.tSaldos WITH (NOLOCK) WHERE nCuenta LIKE '551%'");
		
		pstm_5 = conn.prepareStatement("SELECT SUM(cargo) - SUM(abono) AS saldo " +
										" FROM (SELECT CASE cTipoMovimiento WHEN 'C' THEN SUM(mMovimiento) ELSE 0 END AS cargo " +
													" , CASE cTipoMovimiento WHEN 'A' THEN SUM(mMovimiento) ELSE 0 END AS abono " +
												" FROM dbo.tMovimiento WITH (NOLOCK) " + 
												" WHERE nCuenta LIKE '5%' " +
													" AND MONTH(fMovimiento) BETWEEN 1 AND " + mes + 
													" AND YEAR(fMovimiento) = " + anio +
												" GROUP BY cTipoMovimiento " +
										" ) tbl");
		if(anio == 2015){
