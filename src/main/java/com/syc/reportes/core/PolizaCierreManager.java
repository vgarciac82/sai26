...
public class PolizaCierreManager {
	...
	public static float creaPolizaCierreManager(Connection conn, String usuario, String ur, String cc, int id_documento) throws Exception {
		...
		String queryC =   "SELECT SUM(cargo) - SUM(abono) AS dif FROM ( " 
						+ "SELECT CASE WHEN cEvento = 'CARGO' THEN SUM(mImporte) ELSE 0 END AS cargo " 
						+ ", CASE WHEN cEvento = 'ABONO' THEN SUM(mImporte) ELSE 0 END AS abono "
						+ "FROM tPolizaCierreDetalle WITH (NOLOCK) "
						+ "WHERE nFolioPolizaCierre = ? " + "GROUP BY cEvento) dif";
