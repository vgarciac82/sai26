...
public class ConciliacionesBienesManager {
	...
	public static double consultaSaldos(Connection conn, String tipo, int mes) throws Exception {
		...
		if("1".equals(tipo)){
			...
			cuenta = "nCuenta in ('12311-00000-00000-00001','12313-00000-00000-00000','12332-00001-00000-00001'"
					+ " ,'12332-00003-00000-00000','12332-00004-00000-00000'/*,'12362-00001-00000-00001'*/,'12369-00001-00000-00001') " 
					+ " AND cSubCuenta IN ('58101','58301','62201','62202','62905')";
		}else if("6".equals(tipo)){
			cuenta = "nCuenta LIKE '126%00001%' AND cSubCuenta LIKE '%58301%'";
		}
		
		String query2 = "SELECT ISNULL(CASE WHEN " + mes + " = 1 THEN SUM(mSaldo1) "
										+ "	WHEN " + mes + " = 2 THEN SUM(mSaldo2) "
										+ "	WHEN " + mes + " = 3 THEN SUM(mSaldo3) "
										+ "	WHEN " + mes + " = 4 THEN SUM(mSaldo4) "
										+ "	WHEN " + mes + " = 5 THEN SUM(mSaldo5) "
										+ "	WHEN " + mes + " = 6 THEN SUM(mSaldo6) "
										+ "	WHEN " + mes + " = 7 THEN SUM(mSaldo7) "
										+ "	WHEN " + mes + " = 8 THEN SUM(mSaldo8) "
										+ "	WHEN " + mes + " = 9 THEN SUM(mSaldo9) "
										+ "	WHEN " + mes + " = 10 THEN SUM(mSaldo10) "
										+ "	WHEN " + mes + " = 11 THEN SUM(mSaldo11) "
										+ "	WHEN " + mes + " = 12 THEN SUM(mSaldo12) "
								+ "	END, 0) AS saldoFin "
								+ "	FROM TSALDOS "
								+ " WHERE " + cuenta; 
	...
	public static double consultaSaldosGRMO(Connection conn, String tipo, int mes) throws Exception {
		...
		if("1".equals(tipo) || "5".equals(tipo) || "7".equals(tipo)){
			cargoAlmacen = "SELECT ROUND(SUM(mImporte),2) AS cargo"								
							+ "	FROM tSistemaControlInventarios " 
							+ "	WHERE idTipoConciliacion = " + tipo 
							+ "		AND nMes = " + mes 
							+ "		AND (cTipoPoliza LIKE 'entrada%' OR cTipoPoliza LIKE 'alta%') ";							
			
			abonoAlmacen = "SELECT ROUND(SUM(mImporte),2) AS abono"								
							+ "	FROM tSistemaControlInventarios " 
							+ "	WHERE idTipoConciliacion = " + tipo 
							+ "		AND nMes = " + mes 		
							+ "		AND (cTipoPoliza LIKE 'salida%' OR cTipoPoliza LIKE 'baja%') ";
			...
			cargoAlmacen = "SELECT ROUND(SUM(mImporte),2) AS cargo"								
							...
							+ "	WHERE idTipoConciliacion = " + tipo 
							+ "		AND nMes = " + mes 
							+ " AND (SUBSTRING(cTipoPoliza,1,6) = 'salida' OR SUBSTRING(cTipoPoliza,14,5) = 'bajas' OR SUBSTRING(cTipoPoliza,14,6) IN ('actual','anteri'))";										
			
			abonoAlmacen =  "SELECT ROUND(SUM(mImporte),2) AS abono"								
							+ "	FROM tSistemaControlInventarios " 
							+ "	WHERE idTipoConciliacion = " + tipo 
							+ "		AND nMes = " + mes 
							+ " AND (SUBSTRING(cTipoPoliza,14,7) = 'mensual' OR SUBSTRING(cTipoPoliza,1,6) = 'entrad' OR SUBSTRING(cTipoPoliza,14,5) = 'altas')";							
		}
		...
		try {
			...
			if("1".equals(tipo) || "5".equals(tipo) || "7".equals(tipo) ){ //DEUDORA
				sFin = sIni + cargo - abono;
				//sFin = new BigDecimal(sIni + cargo - abono);
			}else if("2".equals(tipo) || "6".equals(tipo)){ //ACREEDORA
				sFin = sIni - cargo + abono;
				//sFin = new BigDecimal(sIni - cargo + abono);
			}
		...
		while (rs.next()) {
			...
			if ("baja_inventario".equals(seccion)) {
				...
				for (int i = 1; i < 5; i++) {						
					...
					else Util.createExcelCellRep(i, rw, rs, rsMetadata.getColumnName(i + 1), rsMetadata.getColumnType(i + 1), estiloMoneda);					
				}
				/*suma del subtotal*/
		...
		while (rs2.next()) {
			...
			if ("A_inventario".equals(seccion) || "B_inventario".equals(seccion)) {			
				...
				for (int i = 1; i < 3; i++) {						
					...
					else Util.createExcelCellRep(i, rw, rs2, rs2Metadata.getColumnName(i + 1), rs2Metadata.getColumnType(i + 1), estiloMoneda);					
				}
				/*suma del subtotal*/
