...
public class ReportePolizasDetManager {
	...
	private static String generaReportePolizasDet(ResultSet rs, String plantillaPath, String fechaInicio, String fechaFin) throws Exception {
		...
		String file_name = System.getProperty( "java.io.tmpdir" ) + File.separatorChar + 
				"ReportePolizasDet" + "_" + System.currentTimeMillis() + "_" + String.valueOf((int) (Math.random() * 100)) + ".xlsx";
...
private static String generaReporteExcelIngresos(Connection conn, ResultSet rs, String plantillaPath, int mesFin, String fechaFin) throws Exception {
	...
	PreparedStatement pstm_8151 = null, pstm_4 = null; //, pstm_reint = null, pstm_551 = null, pstm_559 = null;
	ResultSet nSI = null, n4 = null; //, nSIR = null, n551 = null, n559 = null ;
	    
	pstm_8151 = conn.prepareStatement("SELECT CASE cTipoMovimiento WHEN 'A' THEN SUM(mMovimiento) ELSE SUM(-mMovimiento) END AS cargo " +
										" FROM dbo.tMovimiento WITH (NOLOCK) " +  
										" WHERE SUBSTRING(nCuenta,1,4) IN('8151') " + 
	...
	pstm_4 = conn.prepareStatement("SELECT CASE cTipoMovimiento WHEN 'A' THEN SUM(mMovimiento) ELSE SUM(-mMovimiento) END AS importe " +
										" FROM dbo.tMovimiento WITH (NOLOCK) " +  
										" WHERE SUBSTRING(nCuenta,1,1) IN('4') " + 
												" AND MONTH(fMovimiento) BETWEEN 1 AND " +  mes +
												" AND YEAR(fMovimiento) = " + anio +		
										" GROUP BY SUBSTRING(nCuenta,1,4), cTipoMovimiento");
...
public static String conciliaDevengadoIngManager(Connection conn, int mesFin, Map<String, String> plantillas) throws Exception {
	...
	try {
		...
		cs.setInt(1, mesFin);	
		rs = cs.executeQuery();	
		fileName = generaConciliaDevengadoIng(rs, /*rs2,*/ plantillas.get("CONCILIADEVENGADOING"), mesFin);			
...
private static String generaConciliaDevengadoIng(ResultSet rs, /*ResultSet rs2,*/ String plantillaPath, int mesFin) throws Exception {
	File cFileExcelPlantilla = new File(plantillaPath);
...
public static String conciliaOrgDispModEjeIngManager(Connection conn, int mesFin, Map<String, String> plantillas) throws Exception {
	...
	try {
		...
		cs.setInt(1, mesFin);	
		rs = cs.executeQuery();	
		fileName = generaConciliaOrgDispModEjeIng(rs, /*rs2,*/ plantillas.get("CONCILIAORGDISPMODEJEING"), mesFin);			
...
private static String generaConciliaOrgDispModEjeIng(ResultSet rs, /*ResultSet rs2,*/ String plantillaPath, int mesFin) throws Exception {
	File cFileExcelPlantilla = new File(plantillaPath);
...
public static String conciliaRadicadoPagado(Connection conn, int mesFin, Map<String, String> plantillas) throws Exception {
	...
	try {
		...
		cs.setInt(1, mesFin);	
		rs = cs.executeQuery();	
		fileName = generaConciliaRadicadoPagado(rs, /*rs2,*/ plantillas.get("ConciliaRadicadoPagado"), mesFin);			
...
private static String generaConciliaRadicadoPagado(ResultSet rs, /*ResultSet rs2,*/ String plantillaPath, int mesFin) throws Exception {
	File cFileExcelPlantilla = new File(plantillaPath);
