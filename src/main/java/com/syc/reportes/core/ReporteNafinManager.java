...
public class ReporteNafinManager {
	...
	public static ArrayList<String> ReportePagosManager(Connection conn, String fechaInicio, String fechaFin, String tipoReporte, Map<String, String> plantillas) throws Exception {
		...
		try {//Obtiene el ultimo folio del cf_sequence que se envio
	...
	public static void actualizaFechasPagado(Connection conn) throws Exception {
		...
		String query="UPDATE P SET FAPLICACION = CONVERT(DATE, FMOVIMIENTO,102) FROM dbo.tMovimiento M (NOLOCK) INNER JOIN dbo.tPagadoEncabezado P (NOLOCK)" 
				+ "ON M.cFolioDocumentoMovimiento = P.nFolioPagado WHERE cTipoDocumento = 'PAGADO' AND MONTH(fAplicacion) <> MONTH(fMovimiento)";
			try {
