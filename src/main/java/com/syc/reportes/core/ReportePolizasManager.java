...
public class ReportePolizasManager {
	private static final Logger log = Logger.getLogger( ReportePolizasManager.class );
	...
	public static String generaReporteREP(Connection conn, String unidad, String rfc,  Map<String, String> plantillas) throws Exception {
		...
		String query = "SELECT cTipoPago, nFolioPago, caNoContrarrecibo,cUnidadEjecutora,FechaPagado,FacturaUUID,RFC,RazonSocial,MontoFactura,MontoTotalREP	,MontoSaldoAnterior	,MontoPagado	,MontoInsoluto	,nComprobado	,mPendienteComprobar	,conceptoPago "
				+ " FROM v_tReciboElectronico_PagoFactura "
				+ " WHERE RFC like '" + rfc + "' AND cUnidadEjecutora like '" + unidad + "'";
	...
	public static String generaReporteBoletosAdjuntos(Connection conn, Map<String, String> plantillas) throws Exception {
		...
		String query = "SELECT  nFolioRelacionGastos, RFC, cConcepto, cUnidadResponsable, fAplicacion, cEsFirmaElectronica, Documento, cnombre , NOMBRE_DOCUMENTO "
				+ " FROM v_solicitudes_con_boleto";
	...
	public static String generaReporteBoletosPendientes(Connection conn, Map<String, String> plantillas) throws Exception {
		...
		String query = "SELECT nFolioComision, RFC, cInformeComision, cUnidadResponsable, faplicacion, cEsFirmaElectronica,  Documento, cBoleto, cRuta, mImporteBoleto, nombreCompleto "
				+ " FROM v_solicitudes_pendientes_boleto ";
	...
	public static void insertarDatosCfdi( Connection conn, List<String[]>  renglonesArchivo ) throws Exception {
		...
		for (int i = 0; i < 102; i++) { // 102 campos
		    sql.append("?");
		    if (i < 101) {
		        sql.append(", ");
		    }
		...
		try {
	        ...
	        for (String[] item : renglonesArchivo) {
	            ...
	            for (int i = 0; i < 102; i++) {
	                ...
	                ps.setString(i + 1, valor); // PreparedStatement es 1-based
	                sql2.append( valor + "," );
	            }
