...
public class FormatoAltaProveedorManager {
	...
	private static final Logger	log	= Logger.getLogger(FormatoAltaProveedorManager.class);
	...
	public static String generaReporteFormatoAltaProveedor( Connection conn, Map<String, String> plantillas,String folio, boolean altaRapida) throws Exception {
		...
		try {
			String query = ""
				+ "SELECT CASE "
				+ "         WHEN alt.cactualizacion = 1 THEN 'MODIFICACION' "
				+ "         ELSE 'ALTA' "
				+ "       END                        AS TipoMovimiento, "
				+ "       tipoRFC.ctipopersonarfc    AS TipoBeneficiario, "
				+ "       alt.cidrfc                 AS RFC, "
				+ "       alt.ccurp                  AS CURP, "
				+ "       capellidopaterno           AS ApellidoPaterno, "
				+ "       capellidomaterno           AS ApellidoMaterno, "
				+ "       cnombre                    AS Nombre, "
				+ "       CASE "
				+ "         WHEN alt.crazonsocial = '' THEN "
				+ "         capellidopaterno + ' ' + capellidomaterno + "
				+ "         ' ' "
				+ "         + cnombre "
				+ "         ELSE alt.crazonsocial "
				+ "       END                        AS Beneficiario, "
				+ "       ccalle                     AS Calle, "
				+ "       cnumeroexterno             AS NumeroExterno, "
				+ "       cnumerointerno             AS NumeroInterno, "
				+ "       ccolonia                   AS Colonia, "
				+ "       mun.mpo_nombre             AS Localidad, "
				+ "       estados.dEntidadFederativa AS Estado, "
				+ "       ccodigopostal              AS CP, "
				+ "       ctelefono                  AS Telefono, "
				+ "       cemail                     AS Email "
				+ "FROM   taltaproveedor alt WITH(nolock) "
				+ "       INNER JOIN pcatalogotipopersonarfc tipoRFC WITH (nolock) "
				+ "               ON tipoRFC.cidtipopersonarfc = alt.cidtipopersona "
				+ "       INNER JOIN tCatalogoEntidadFederativa estados WITH (nolock) "
				+ "               ON CAST (estados.cEntidadFederativa AS INT) = alt.cidentidadfederativa "
				+ "       INNER JOIN cat_municipio mun WITH (nolock) "
				+ "               ON mun.id_municipio = alt.cidmunicipio "
				+ "WHERE  alt.cfolio = ?";
			...
			String query2 = ""
				+ "SELECT dcuentabancaria as cuenta, "
				+ "       subcuentabancaria as clabe, "
				+ "       dbanco as banco, "
				+ "       CASE "
				+ "         WHEN alt.crazonsocial = '' THEN "
				+ "         alt.capellidopaterno + ' ' "
				+ "         + alt.capellidomaterno + ' ' + alt.cnombre "
				+ "         ELSE alt.crazonsocial "
				+ "       END AS nombre "
				+ "FROM   tbeneficiariocuentasbancarias banc WITH (nolock) "
				+ "       INNER JOIN taltaproveedor alt  WITH (nolock) "
				+ "               ON Replace (alt.cidrfc, '-', '') = banc.drfc "
				+ "WHERE  alt.cfolio = ?";
			...
			String query3 = ""
				+ "SELECT dcuentabancaria as cuenta, "
				+ "       cBanco+cPlaza+dCuentaBancaria+dDigitoVerificador as clabe, "
				+ "       dbanco as banco, "
				+ "       CASE "
				+ "         WHEN alt.crazonsocial = '' THEN "
				+ "         alt.capellidopaterno + ' ' "
				...
				+ "         ELSE alt.crazonsocial "
				+ "       END AS nombre "
				+ "FROM   tBeneficiarioCuentasBancariasTmp banc WITH (nolock) "
				+ "       INNER JOIN taltaproveedor alt  WITH (nolock) "
				+ "               ON Replace (alt.cidrfc, '-', '') = banc.drfc "
				+ "WHERE  alt.cfolio = ?";
			...
			if( altaRapida)
				query4  = "SELECT ccben AS CBEN "
						+ "FROM   tcbenaltaempleado cben WITH(NOLOCK) "
						+ "       INNER JOIN taltaproveedor prov WITH(NOLOCK) "
						+ "               ON Replace(cben.crfc, '-', '') = Replace(prov.cidrfc, '-', '') "
						+ "WHERE  prov.cfolio = ? "
						+ "       AND cben.ntipocben = " +  AltaProveedorBusinessLogic.CBEN_PROVEEDOR;
			else
