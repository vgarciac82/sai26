...
public class PasivosContingentesServlet extends HttpServlet implements GestionInterface {
	...
	private static final Logger			log					= Logger.getLogger(PasivosContingentesServlet.class);
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			if ("1".equals(tipoReporte)){//cedula pasatrimoniale								
				rrs.generaCedula(req, resp, plantillas);
			}else if ("3".equals(tipoReporte)){//cedula pasatrimoniale							
				rrs.generaCedulaLaboral(req, resp, plantillas);
			}
			else if ("5".equals(tipoReporte)){//cedula antigüedad
				rrs.generaCedualAntiguedad(req, resp, plantillas, unidad);
			}
			else if ("6".equals(tipoReporte)){//cedula trimestral - 2018 y anteriores  mensual - 2019 y subsecuebntes
				rrs.generaCedualTrimestral(req, resp, plantillas, unidad);
			}
			else if ("7".equals(tipoReporte)){//cedula antigüedad por expediente
				if ("A".equals( tipoCedula )) {//Analitica
					rrs.generaCedualAntiguedadExpediente(req, resp, plantillas, unidad);
				}
				else if ("C".equals( tipoCedula )) {//Consolidada
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if ("2".equals(tipoReporte)) {//poliza mensual pasivo patrimonial
			...
			}else if ("4".equals(tipoReporte)) {//poliza mensual pasivo laboral
				String fAplicacion = req.getParameter("fecha_aplicacion");
