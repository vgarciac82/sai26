...
public class PasivosContingentesManager {
	...
	private static final Logger log = Logger.getLogger(ReporteAcreedoresDeudoresManager.class);
	...
	public static void PasivosEjerciciosAnteriores(Connection conn, int anio) throws Exception {
		...
		String query = "INSERT INTO tpasivosContingentesAnt "
					+ 	" SELECT idPasivoContingente " 
					+ 	" 	, (SELECT aEjercicioFiscal FROM dbo.tEjercicioFiscal WHERE cActivo = 1) AS nEjercicioFiscal "
					+	" 	, ISNULL(SUM(nEnero),0) + ISNULL(SUM(nFebrero),0) + ISNULL(SUM(nMarzo),0) + ISNULL(SUM(nAbril),0) + ISNULL(SUM(nMayo),0) + ISNULL(SUM(nJunio),0) + ISNULL(SUM(nJulio),0) + ISNULL(SUM(nAgosto),0) + ISNULL(SUM(nSeptiembre),0) + ISNULL(SUM(nOctubre),0) + ISNULL(SUM(nNoviembre),0) + ISNULL(SUM(nDiciembre),0) AS nInteresesAnuales "
					+	" FROM dbo.tpasivosContingentesAct "
					+	" GROUP BY idPasivoContingente"; 				
