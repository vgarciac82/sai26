...
public class AdecuacionDetalleManager {
	...
	private static final Logger			log					= Logger.getLogger( AdecuacionDetalleManager.class );
		...
		try {
			...
			while ( rs.next() ) {
				...
				for ( int i = 0; i < Util.NOMBRE_MESES_ADECUACIONES.length; i++ )
					montos.add( rs.getDouble( Util.NOMBRE_MESES_ADECUACIONES[i] ) );
