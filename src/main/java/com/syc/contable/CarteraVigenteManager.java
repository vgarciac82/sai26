...
public class CarteraVigenteManager {
	...
	private static final Logger	log	= Logger.getLogger(CarteraVigenteManager.class);
	...
	public static double getMontoTotalCartera(Connection conn, String ep) throws Exception {
		...
		PreparedStatement ps = null;
		String query = "select idestado from vcarteravigente where idproyecto = ( select  SUBSTRING('"+ ep +"', 45,11))  and estatus ='Vigente' " +
				" and idestado = (select  SUBSTRING( '"+ ep +"' , 42,2)) and idcvepres like + '% (select  SUBSTRING( '"+ ep +"' , 32,5))" + "%'";
		try {
