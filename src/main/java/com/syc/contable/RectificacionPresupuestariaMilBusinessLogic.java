...
public class RectificacionPresupuestariaMilBusinessLogic extends DataSourceManager {
	...
	private static Logger	log					= Logger.getLogger(RectificacionPresupuestariaMilBusinessLogic.class);
	...
	public boolean insertTRectificacionEncabezado(int folio, String nIDCaso, String cEjercicio, String cRamo, String cUnidad, String cCentroContable, java.sql.Date fExp, java.sql.Date fApl, String conceptoRectificacion, String cTipoMovto, String nOrigenPPTO, String nMes, String oficioRectif,
		String ctr_int, String cTipoRectificacion, String nFolioSICOP, String caNoContrarrecibo, String cTipoPoliza, String cDescripcionPoliza, String u_login, String nFolioPoliza, String cUnidadResponsableContable, String nFolioSIAFF, String totalDebe, String totalDice, String esIP) throws Exception {
		Connection conn = null;
		...
		try {
			...
			insertar = RectificacionPresupuestariaManager.insertarEncabezado(conn, folio, nIDCaso, cEjercicio, cRamo, cUnidad, cCentroContable, fExp, fApl, conceptoRectificacion, cTipoMovto, nOrigenPPTO, nMes, oficioRectif, ctr_int, cTipoRectificacion, nFolioSICOP, caNoContrarrecibo, cTipoPoliza,
				cDescripcionPoliza, u_login, nFolioPoliza, cUnidadResponsableContable, nFolioSIAFF, totalDebe, totalDice, esIP);
	...
	public String cancelarAppContableNuevo(Caso c, Map<?, ?> m, String prefixPath, String uLogin, String cFecha, Usuario usuario) throws Exception {
		...
		try {
			...
			if (acr.isSuccess()) {
				...
				String body = (!correoProduccion ? "CORREO DE PRUEBA <br>" : "") + (!correoProduccion ? "este correo le hubiera llegado a: " + to + "<br> <br>" : "") + "Para su conocimiento y efectos correspondientes, se le informa que <b>fue cancelado</b> el folio siguiente:<br>"
					+ c.getTipoCaso().getDescripcion() + " No. SAI: <b>" + c.getFolio() + "</b><br>" + "Mismo que ya cuenta con estatus de cancelado en el SAI.";
				try {
					if (!correoProduccion)
					...
					if (valor != null) {
						if (fila == 2) {
							...
							if (celda == 8) {
								...
								recE.setCaNoContrarreciboMil(valor); // CAMBIAR CUANDO SE TENGA EL MIL, MIENTRAS ESTÁ LA VISTA ESTO ES PARA PRUEBAS
							}
							if (celda == 9)
					...
					if (secuenciaCLC == -1 && rd.getEvento().contains("DICE")) {
						...
						if (docRenglon[0] == -1 && rd.getEvento().contains("DICE")) {
							...
							if (!banderaRemanente) {
								...
								throw new Exception("No hay suficiente remanente para poder aplicar la rectificacion para " + rd.getEp() + " renglon " + secuenciaCLC + " mes " + rd.getMes() + " cxp " + recE.getCaNoContrarrecibo() + " remanente " + remanente + " importe a reintegrar "
									+ rd.getImporte() + "\\n");
							}
						}
					}
			...
			if(mensaje == ""){
				...
				if (nIdCaso > 0) {
					...
					if (acr.isSuccess()) {
						conn.commit();
						String to = usuario.getU_email();
						String body = (!correoProduccion ? "CORREO DE PRUEBA <br>" : "") + (!correoProduccion ? "este correo le hubiera llegado a: " + to + "<br> <br>" : "") + "Para su conocimiento y efectos correspondientes, se le informa que <b>fue autorizado</b> el folio siguiente:<br>"
							+ c.getTipoCaso().getDescripcion() + " No. SAI: <b>" + c.getFolio() + "</b><br>" + "Mismo que ya cuenta con estatus de autorizado en el SAI.";
						try {
							if (!correoProduccion)
								to = "vgarciac@axtel.com.mx;"; 							
							AlarmaManager.procesaAlarmaCNF(conn, prefixPath, c.getCasoOperacion(0), c, "", to, body);
