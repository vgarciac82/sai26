...
public class PolizaManager {
	...
	public static final Logger log = Logger.getLogger(PolizaManager.class);

	public static int borraDetallePolizaAplicada(Connection con,
			String nFolioPoliza, String cCentroContable, String cTipoPoliza,
			String aEjercicioFiscal) throws PolizaException {
		String qry = " DELETE " + " FROM	tMovimiento "
				+ " WHERE	nFolioPoliza = ? " + " AND	cCentroContable = ? "
				+ " AND cTipoPoliza = ? and aEjercicioFiscal = ?";
	...
	public static int borraEncabezadoPolizaAplicada(Connection con,
			String nFolioPoliza, String cCentroContable, String cTipoPoliza,
			String aEjercicioFiscal) throws PolizaException {
		String qry = " DELETE " + " FROM	tPoliza " + " WHERE	nFolioPoliza = ? "
				+ " AND	cCentroContable = ? " + " AND cTipoPoliza = ? "
				+ " AND	aEjercicioFiscal = ?";
		PreparedStatement pstmnt = null;
		int r = 0;
		try {
			pstmnt = con.prepareStatement(qry);
	...
	public static int borraPolizaAplicada(Connection con, String nFolioPoliza,
			String cCentroContable, String cTipoPoliza, String aEjercicioFiscal)
			throws PolizaException {
		int r = 0;

		r = PolizaManager.borraDetallePolizaAplicada(con, nFolioPoliza,
				cCentroContable, cTipoPoliza, aEjercicioFiscal);
		r += PolizaManager.borraEncabezadoPolizaAplicada(con, nFolioPoliza,
				cCentroContable, cTipoPoliza, aEjercicioFiscal);
	...
	public static List<Movimiento> cargaDetallePolizaAplicada(Connection con,
			String nFolioPoliza, String cCentroContable, String cTipoPoliza,
			String aEjercicioFiscal) throws PolizaException {
		List<Movimiento> detalle = new ArrayList<Movimiento>();
		PreparedStatement pStmnt = null;
		ResultSet rs = null;
		String queryEnc = " SELECT	* " + " FROM	tMovimiento WITH(NOLOCK) "
				+ " WHERE	nFolioPoliza = ? " + " AND	cCentroContable = ? "
				+ " AND cTipoPoliza = ? " + " AND	aEjercicioFiscal = ?";
		try {
			...
			while (rs.next()) {
				...
				renglonDetalle.setAejerciciofiscal(rs
						.getString("aEjercicioFiscal"));
				renglonDetalle.setCcentrocontable(rs
						.getString("cCentroContable"));
				renglonDetalle
						.setCtipodocumento(rs.getString("cTipoDocumento"));
				...
				renglonDetalle.setCdescripcionmovpol(rs
						.getString("cDescripcionMovPol"));
				renglonDetalle.setnConsecutivoMovimiento(rs
						.getLong("nConsecutivoMovimiento"));
				renglonDetalle.setCramo(rs.getString("cRamo"));
				renglonDetalle.setFoperacionmovimiento(rs
						.getDate("fOperacionMovimiento"));
				renglonDetalle.setCfoliodocumentomovimiento(rs
						.getLong("cFolioDocumentoMovimiento"));
				renglonDetalle.setCcancelamovimiento(rs
						.getString("cCancelaMovimiento"));
				renglonDetalle.setFmovimiento(rs.getDate("fMovimiento"));
				renglonDetalle.setDconceptomovimiento(rs
						.getString("dConceptoMovimiento"));
				renglonDetalle.setCmoneda(rs.getString("cMoneda"));
				renglonDetalle.setCunidadresponsable(rs
						.getString("cUnidadResponsable"));
	...
	public static DocPoliza cargaDocPoliza(Connection con,
			long nFolioDocPoliza, String cCentroContable, String cTipoPoliza,
			String aEjercicioFiscal) throws PolizaException {
		List<DocPolizaDetalle> detalle = cargaDocPolizaDetalle(con,
				nFolioDocPoliza, cCentroContable, cTipoPoliza, aEjercicioFiscal);
		DocPolizaEncabezado encabezado = cargaDocPolizaEncabezado(con,
				nFolioDocPoliza, cTipoPoliza, cCentroContable, aEjercicioFiscal);
		DocPoliza poliza = new DocPoliza(encabezado, detalle);
		return poliza;
	}

	public static List<DocPolizaDetalle> cargaDocPolizaDetalle(Connection con,
			long nFolioDocPoliza, String cCentroContable, String cTipoPoliza,
			String aEjercicioFiscal) throws PolizaException {

		List<DocPolizaDetalle> detalle = new ArrayList<DocPolizaDetalle>(0);
		String qry = "SELECT	nFolioDocPoliza, nDocRenglon, nCuenta, nSubCuenta, cEvento, "
				+ "        mImporte, cCentroContable, cTipoPoliza, aEjercicioFiscal, cConcepto "
				+ "       ,nTipoAjuste "
				+ "       ,Periodo13 "
				+ "       ,ADEFAS "
				+ "       ,parcial "
				+ "  FROM	tDocPolizaDetalle WITH(NOLOCK) "
	...
	public static DocPolizaEncabezado cargaDocPolizaEncabezado(Connection con,
			long nFolioDocPoliza, String cTipoPoliza, String cCentroContable,
			String aEjercicioFiscal) throws PolizaException {
		String qry = " SELECT " + "	 aEjercicioFiscal, "
				+ " 	 cCentroContable, " + " 	 cComentarios, "
				+ " 	 cConcepto, " + " 	 cDescripcionPoliza, "
				+ " 	 cDocumentoHaplicado, " + " 	 cidOrigen, "
				+ " 	 cIdUsuarioAprobacion, " + " 	 cIdUsuarioCaptura, "
				+ " 	 cIdUsuarioRevision, " + " 	 cRamo, " + " 	 cRevisado, "
				+ " 	 cTipoDocumento, " + " 	 cTipoPoliza, "
				+ " 	 cUnidadResponsable, " + " 	 cUnidadResponsableContable, "
				+ " 	 fAplicacion, " + " 	 fCancelacion, " + " 	 fCarga, "
				+ " 	 mTotalAbonos, " + " 	 mTotalCargos, " + " 	 nCambio, "
				+ " 	 nFolioDocPoliza, " + " 	 nFolioPoliza, "
				+ " 	 nFolioPolizaCancelacion, " + " 	 nIdCasoOrigen, "
				+ " 	 nMes  " + "       ,nTipoAjuste " + "       ,Periodo13 "
				+ "       ,ADEFAS "
				// + ", cReferenciaPoliza "
				+ " FROM	tDocPolizaEncabezado with(nolock) "

				+ " WHERE	nFolioDocPoliza = ? " + "    AND	cTipoPoliza = ? "
				+ " AND	cCentroContable = ? " + " AND	aEjercicioFiscal = ? ";
	...
	public static EncabezadoPoliza cargaEncabezadoPolizaAplicada(
			Connection con, String nFolioPoliza, String cCentroContable,
			String cTipoPoliza) throws PolizaException {
		PreparedStatement pStmnt = null;
		ResultSet rs = null;
		String queryEnc = " SELECT	* " + " FROM	tPoliza with (nolock) "
				+ " WHERE	nFolioPoliza = ? " + " AND	cCentroContable = ? "
				+ " AND cTipoPoliza = ?";
		...
		try {
			...
			if (rs.next()) {
				...
				encPoliza.setcDescripcionPoliza(rs
						.getString("cDescripcionPoliza"));
				...
				encPoliza.setnPolizaAutomatica(rs
						.getString("nPolizaAutomatica"));
			}
			return encPoliza;
	...
	public static Poliza cargaPolizaAplicada(Connection con,
			String nFolioPoliza, String cCentroContable, String cTipoPoliza,
			String aEjercicioFiscal) throws PolizaException {
		Poliza p = new Poliza();
		p.setEncabezado(PolizaManager.cargaEncabezadoPolizaAplicada(con,
				nFolioPoliza, cCentroContable, cTipoPoliza));
		p.setDetalle(PolizaManager.cargaDetallePolizaAplicada(con,
				nFolioPoliza, cCentroContable, cTipoPoliza, aEjercicioFiscal));
		return p;
	}

	private static List<DocPolizaEncabezado> extraeEncabezado(ResultSet rs)
			throws SQLException {
		...
		while (rs.next()) {
			...
			encabezado
					.setcDescripcionPoliza(rs.getString("cDescripcionPoliza"));
			encabezado.setcDocumentoHaplicado(rs
					.getString("cDocumentoHaplicado"));
			encabezado.setCidOrigen(rs.getString("cidOrigen"));
			encabezado.setcIdUsuarioAprobacion(rs
					.getString("cIdUsuarioAprobacion"));
			encabezado.setcIdUsuarioCaptura(rs.getString("cIdUsuarioCaptura"));
			encabezado
					.setcIdUsuarioRevision(rs.getString("cIdUsuarioRevision"));
			...
			encabezado
					.setcUnidadResponsable(rs.getString("cUnidadResponsable"));
			encabezado.setcUnidadResponsable(rs
					.getString("cUnidadResponsableContable"));
			...
			encabezado.setnFolioPolizaCancelacion(rs
					.getInt("nFolioPolizaCancelacion"));
	...
	public static List<DocPolizaEncabezado> getPolizasEnCaptura(
			Connection conn, String cCentroContable, int nMes,
			int aEjercicioFiscal, boolean mesActual) throws PolizaException {
		PreparedStatement pStmnt = null;
		ResultSet rs = null;
		String qry = "SELECT	* "
				+ "  FROM	tDocPolizaEncabezado t WITH(nolock)"
				+ " WHERE	(t.cDocumentoHaplicado is null OR t.cDocumentoHaplicado = 'E' or (select  COUNT(*) from CG_CASO_OPERACION WITH(nolock) where id_caso = t.nIdCasoOrigen and ID_OPER!=4 )!=0 )"
				+ "		AND	 t.aEjercicioFiscal = ?";
		if (cCentroContable != null && !"".equals(cCentroContable))
			qry += "		AND	 t.cCentroContable = ?";
		qry += "		AND	 t.nMes " + (mesActual ? "=" : "<")
			+ " ?  AND t.nMes > 0";
		int cnt = 1;
		try {
	...
	public static int[] insertTMovimiento(Connection con,
			List<Movimiento> movimientos) throws PolizaException {
		String qry = "INSERT INTO tmovimiento "
				+ " (nfoliopoliza,  "
				+ " ndocrenglon, "
				+ " ncuenta, "
				+ " nsubcuenta, "
				...
				+ " aejerciciofiscal, "
				+ " ctipopoliza,  "
				+ " cunidadresponsable, nTipoAjuste, periodo13, ADEFAS, parcial) "
				+ " VALUES     ( ?, " + " ?, " + " ?, " + " ?, " + " ?, "
				+ " ?, " + " ?, " + " ?, " + " ?, " + " ?, " + " ?, " + " ?, "
				+ " ?, " + " ?, " + " ?, " + " ?, " + " ?, " + " ?, " + " ?, "
				+ " ?, " + " ?, " + " ?, " + " ?) ";
		...
		try {
			...
			for (Iterator<Movimiento> i = movimientos.iterator(); i.hasNext();) {
				...
				pStmnt.setTimestamp(11,
						(movimiento.getFoperacionmovimiento() == null ? null
								: new Timestamp(movimiento
										.getFoperacionmovimiento().getTime())));

				pStmnt.setLong(12, movimiento.getCfoliodocumentomovimiento());
				pStmnt.setString(13, movimiento.getCcancelamovimiento());
				pStmnt.setTimestamp(14,
						(movimiento.getFmovimiento() == null ? null
								: new Timestamp(movimiento.getFmovimiento()
										.getTime())));
	...
	public static int insertTMovimiento(Connection con, Movimiento movimiento)
			throws PolizaException {
		String qry = "INSERT INTO tmovimiento "
				+ " (nfoliopoliza,  "
				+ " ndocrenglon, "
		...
		try {
			...
			pStmnt.setDate(11,
					(movimiento.getFoperacionmovimiento() == null ? null
							: new Date(movimiento.getFoperacionmovimiento()
									.getTime())));
			pStmnt.setLong(12, movimiento.getCfoliodocumentomovimiento());
			pStmnt.setString(13, movimiento.getCcancelamovimiento());
			pStmnt.setDate(14, (movimiento.getFmovimiento() == null ? null
					: new Date(movimiento.getFmovimiento().getTime())));
			pStmnt.setString(15, movimiento.getDconceptomovimiento());
	...
	public static List<DocPolizaEncabezado> resumenPolizas(Connection conn,
			DocPolizaEncabezado modelo, String preferredDateRange)
			throws PolizaException {
		PreparedStatement pStmnt = null;
		ResultSet rs = null;
		...
		if (modelo.getcCentroContable() != null
				&& !"".equals(modelo.getcCentroContable())) {
		...
		if (modelo.getcUnidadResponsable() != null
				&& !"".equals(modelo.getcUnidadResponsable())) {
			queryBase += token + " cUnidadResponsable = ? ";
			token = "AND";
		}
		if (modelo.getcDocumentoHaplicado() != null
				&& !"".equals(modelo.getcDocumentoHaplicado())) {
		...
		if (modelo.getcTipoPoliza() != null
				&& !"".equals(modelo.getcTipoPoliza())) {
		...
		if (modelo.getaEjercicioFiscal() != null
				&& !"".equals(modelo.getaEjercicioFiscal())) {
			queryBase += token + " aEjercicioFiscal = ? ";
			token = "AND";
		}
		if (modelo.getcUnidadResponsableContable() != null
				&& !"".equals(modelo.getcUnidadResponsableContable())) {
		...
		if (modelo.getcDescripcionPoliza() != null
				&& !"".equals(modelo.getcDescripcionPoliza())) {
		...
		if (modelo.getcIdUsuarioCaptura() != null
				&& !"".equals(modelo.getcIdUsuarioCaptura())) {
			queryBase += token + " cIdUsuarioCaptura = ? ";
			token = "AND";
		}
		if (modelo.getcIdUsuarioRevision() != null
				&& !"".equals(modelo.getcIdUsuarioRevision())) {
			queryBase += token + " cIdUsuarioRevision = ? ";
			token = "AND";
		}
		if (modelo.getcIdUsuarioAprobacion() != null
				&& !"".equals(modelo.getcIdUsuarioAprobacion())) {
		...
		if (modelo.getmTotalCargos() != null
				&& !"".equals(modelo.getmTotalCargos())) {
			queryBase += token + " mTotalCargos = ? ";
			token = "AND";
		}
		if (modelo.getmTotalAbonos() != null
				&& !"".equals(modelo.getmTotalAbonos())) {
			queryBase += token + " mTotalAbonos = ? ";
			token = "AND";
		}
		if (modelo.getcTipoDocumento() != null
				&& !"".equals(modelo.getcTipoDocumento())) {
			queryBase += token + " cTipoDocumento = ? ";
			token = "AND";
		}
		if (modelo.getcComentarios() != null
				&& !"".equals(modelo.getcComentarios())) {
		...
		try {
			...
			if (modelo.getfAplicacion() != null) {
				pStmnt.setDate(cnt++, new Date(modelo.getfAplicacion()
						.getTime()));
			}
			if (modelo.getcCentroContable() != null
					&& !"".equals(modelo.getcCentroContable())) {
			...
			if (modelo.getcRevisado() != null
					&& !"".equals(modelo.getcRevisado())) {
			...
			if (modelo.getfCancelacion() != null) {
				pStmnt.setDate(cnt++, new Date(modelo.getfCancelacion()
						.getTime()));
			}
			if (modelo.getcDescripcionPoliza() != null
					&& !"".equals(modelo.getcDescripcionPoliza())) {
			...
			if (modelo.getcConcepto() != null
					&& !"".equals(modelo.getcConcepto())) {
			...
			if (modelo.getCidOrigen() != null
					&& !"".equals(modelo.getCidOrigen())) {
	...
	public static int updateDocPolizaEncabezado(Connection con,
			DocPolizaEncabezado dpe) throws PolizaException {
		int r = -1;
		String qry = " UPDATE	tDocPolizaEncabezado "
				+ " SET	cComentarios  = ?, " + "	cConcepto  = ?, "
				+ "	cDescripcionPoliza  = ?, " + "	cDocumentoHaplicado = ?, "
				+ "	cidOrigen  = ?, " + "	cIdUsuarioAprobacion  = ?, "
				+ "	cIdUsuarioCaptura  = ?, " + "	cIdUsuarioRevision  = ?, "
				+ "	cRamo = ?, " + "	cRevisado = ?, "
				+ "	cTipoDocumento  = ?, " + "	cUnidadResponsable = ?, "
				+ "	fAplicacion = ?, " + "	fCancelacion = ?, "
				+ "	fCarga = ?, " + "	mTotalAbonos  = ?, "
				+ "	mTotalCargos  = ?, " + "	nCambio  = ?, "
				+ "	nFolioPoliza  = ?, " + "	nFolioPolizaCancelacion  = ?, "
				+ "	nIdCasoOrigen  = ?, " + "	nMes = ? "
		...
		try {
			...
			pStmnt.setDate(13, (dpe.getfAplicacion() != null ? new Date(dpe
					.getfAplicacion().getTime()) : null));
			pStmnt.setDate(14, (dpe.getfCancelacion() != null ? new Date(dpe
					.getfCancelacion().getTime()) : null));
			pStmnt.setDate(15, (dpe.getfCarga() != null ? new Date(dpe
					.getfCarga().getTime()) : null));
	...
	public static int updateEncabezadoPolizaAplicada(Connection con,
			EncabezadoPoliza encabezado) throws PolizaException {
		int r = 0;
		String qry = "UPDATE	tPoliza " + "SET	cDescripcionPoliza = ?, "
				+ "		mTotalCargo = ?, " + "		mTotalAbono = ?, "
				+ "		nCuenta = ?, " + "		nMes = ?, "
				+ "		nPolizaAutomatica = ?, " + "		aEjercicioFiscal = ?, "
				+ "		cTipoDocumento = ?, " + "		DocHAplicado = ?, "
				+ "		cUsuarioAutorizo = ? "
				// + "		,cReferencia = ? "
				+ "WHERE	nFolioPoliza = ? " + "AND	cCentroContable = ? "
				+ "AND	cTipoPoliza = ? ";
