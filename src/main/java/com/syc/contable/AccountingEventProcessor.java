...
public class AccountingEventProcessor implements AccountingEventProcessorInterface {
    ...
    private static final Logger log = LoggerFactory.getLogger(AccountingEventProcessor.class);
    ...
    private List<AccountingEvent> getEventsFromDatabase(Connection conn, String event, String ejercicioFiscal) throws AccountingEventProcessorException {
        ...
        ResultSet rs = null;
        try {
            pstmnt = conn.prepareStatement("SELECT m.cEvento, m.nCuenta, m.TipoCuenta, m.nDocRenglon, d.nOrden, m.dComponente, dDetalleCuenta FROM tEventoConfiguracion AS m " + "WITH (NOLOCK) LEFT OUTER JOIN tEventoConfiguraDetalle AS d WITH (NOLOCK) ON (d.cEvento = m.cEvento AND d.aEjercicioFiscal = m.aEjercicioFiscal " + "AND d.nCuenta = m.nCuenta AND d.nDocRenglon = m.nDocRenglon) WHERE m.cEvento = ? AND m.aEjercicioFiscal = ? ORDER BY m.cEvento, m.nCuenta, " + "nDocRenglon, d.nOrden");
        ...
        for (AccountingEvent ec : ecList) {
            try {
                if (Double.parseDouble(values.get(ec.getComponente())) != 0) {
                    ...
                    movtos.add(new AccountingMovement(cta.getCuenta(), cta.isSubCuentaRequerida() ? values.get(cta.getSubCuenta()).trim() : null, Double.parseDouble(values.get(ec.getComponente())), ec.esCargo(), cta.esAcreedora(), cta.verificaSaldo(), cta.getCentroContable(), cta.getEjercicioFiscal(), Integer.parseInt(values.get(FIELD_DOC_RENGLON)), cta.getDescCuenta(), cta.isPresupuestal(), values.get("cUnidadResponsable")));
                }
    ...
    private Account buildAccount(Connection conn, AccountingEvent ec, Map<String, String> values) throws AccountingEventProcessorException {
        ...
        try {
            ...
            if (!ec.getDetalle().isEmpty() && (bloques.length != ec.getDetalle().size()))
                throw new AccountingEventProcessorException("El detalle del evento (" + ec.getEvento() + ") no coincide con los bloques de la cuenta '" + ec.getCuenta() + "' (" + bloquesPorCta + " vs " + ec.getDetalle().size() + ")");
            ...
            if (!cuentas.containsKey(cuenta.toString())) {
                ...
                cta = new Account(rs.getString("nCuenta"), rs.getString("cSubcuenta"), "S".equalsIgnoreCase(rs.getString("VerificaSaldo")), "D".equalsIgnoreCase(rs.getString("NaturalezaCuenta")), centroContable, ejercicoFiscal, rs.getString("dCuenta"), rs.getString("TipoBalance"), rs.getString("TipoCuenta"));
