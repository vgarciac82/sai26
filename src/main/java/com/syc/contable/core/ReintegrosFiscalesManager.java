...
public class ReintegrosFiscalesManager {
    ...
    public static double getRemanente(Connection conn, String ep, String cxp, int nRenglon) throws SQLException {
        ...
        "               (SELECT Count(*) AS sumaReint " + "                FROM   tReintegroIngresoEncabezado ree WITH (nolock), " + "                       tReintegroIngresoDetalle red WITH (nolock) " + // 5
        "                WHERE  red.ep = ? " + "                       AND ree.cdocumentohaplicado = 'S' " + // 6
        "                       AND ree.nfolioreintegro = red.nfolioreintegro " + "                       AND red.cxp = ? " + "                       AND red.ep = pd.ep " + "                       AND red.nrenglonpagado = ?)           AS tReintegros, " + "               Isnull((SELECT Isnull(Sum(rd.mimporte), 0) AS impR " + "                       FROM   tReintegroIngresoEncabezado re WITH(nolock), " + // 7
        "                              tReintegroIngresoDetalle rd WITH(nolock) " + // 8
        ...
        "                              trectificacionencabezado h WITH (nolock) " + "                       WHERE  r.nfoliorectificacion = h.nfoliorectificacion " + "                              AND h.canocontrarrecibo = ? " + "                              AND r.ep =  ? " + "               AND h.cdocumentohaplicado = 'S' " + //12
        "                       GROUP  BY r.ndocrenglon), 0)          AS impRect " + //13
