...
public class ReintegrosFiscalesManager {
    ...
    public static void autorizaReintegro(Connection conn, Caso c, String nNumSicop, String cRecMotivSicop, String nNumMAP, String cRecMotivMAP, String ulogin, String prefixPath, String fAcredit) throws SQLException, GestionException {
        ...
        try {
            ...
            pstmnt = conn.prepareStatement("INSERT INTO tReintegroIngresoAutEncabezado (nFolioReintegroIngreso, fSolicitud, cTipoReintegro, cRamo, cUnidadResponsable, cDocumentoHaplicado, aEjercicioFiscal, observaciones, concepto, u_login, " + "cUnidadResponsableContable,nFolioTramiteSicop, cTipoPoliza, fAplicacion,cdescripcionpoliza, formaDePago, clvRastreo,fichaDeposito, clvBanco,cuentaBancaria,lc,importeLC,clcSicop,folioDependencia,mvto,aviso,tipoAviso,causaAviso) " + "SELECT nFolioReintegroIngreso, fSolicitud, cTipoReintegro, cRamo,cUnidadResponsable, 'N', aEjercicioFiscal, observaciones, concepto, " + "u_login, 'RHQ',nFolioTramiteSicop, 'EG','" + fAcredit + "','AUTORIZACION DE REINTEGRO PRESUPUESTAL FOLIO ' + CAST(nFolioReintegroIngreso as varchar),formaDePago, clvRastreo,fichaDeposito, clvBanco,cuentaBancaria,lc,importeLC,clcSicop,folioDependencia,mvto,aviso,tipoAviso,causaAviso FROM tReintegroIngresoEncabezado WITH (NOLOCK) WHERE nFolioReintegroIngreso = ?");
    ...
    public static double getRemanente(Connection conn, String ep, String cxp, int nRenglon) throws SQLException {
        ...
        "                       FROM   tReintegroIngresoEncabezado re WITH(nolock), " + // 8
        "                              tReintegroIngresoDetalle rd WITH(nolock) " + // 9
        ...
        "                                         END), 0) AS imp " + //11
        "                       FROM   trectificaciondetalle r WITH (nolock), " + "                              trectificacionencabezado h WITH (nolock) " + "                       WHERE  r.nfoliorectificacion = h.nfoliorectificacion " + "                              AND h.canocontrarrecibo = ? " + "                              AND r.ep =  ? " + //12
        "               AND h.cdocumentohaplicado = 'S' " + //13
