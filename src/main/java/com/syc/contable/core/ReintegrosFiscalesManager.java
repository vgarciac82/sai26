...
public class ReintegrosFiscalesManager {
    ...
    public static double getRemanente(Connection conn, String ep, String cxp, int nRenglon) throws SQLException {
        ...
        "                FROM   tReintegroIngresoEncabezado ree WITH (nolock), " + "                       tReintegroIngresoDetalle red WITH (nolock) " + "                WHERE  red.ep = ? " + // 5
        "                       AND ree.cdocumentohaplicado = 'S' " + "                       AND ree.nfolioreintegro = red.nfolioreintegro " + // 6
        "                       AND red.cxp = ? " + "                       AND red.ep = pd.ep " + "                       AND red.nrenglonpagado = ?)           AS tReintegros, " + "               Isnull((SELECT Isnull(Sum(rd.mimporte), 0) AS impR " + "                       FROM   tReintegroIngresoEncabezado re WITH(nolock), " + "                              tReintegroIngresoDetalle rd WITH(nolock) " + // 7
        ...
        "                       WHERE  r.nfoliorectificacion = h.nfoliorectificacion " + "                              AND h.canocontrarrecibo = ? " + "                              AND r.ep =  ? " + "               AND h.cdocumentohaplicado = 'S' " + "                       GROUP  BY r.ndocrenglon), 0)          AS impRect " + //12
        "        FROM   tpagadoencabezado pe WITH (nolock), " + //13
