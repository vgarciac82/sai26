...
public class ReintegrosFiscalesManager {
    ...
    public static double getRemanente(Connection conn, String ep, String cxp, int nRenglon) throws SQLException {
        ...
        "                       AND rd.ndocrenglon = ?)               AS tRectificacion, " + "               (SELECT Count(*) AS sumaReint " + "                FROM   tReintegroIngresoEncabezado ree WITH (nolock), " + "                       tReintegroIngresoDetalle red WITH (nolock) " + // 4
        "                WHERE  red.ep = ? " + "                       AND ree.cdocumentohaplicado = 'S' " + "                       AND ree.nfolioreintegro = red.nfolioreintegro " + // 5
        "                       AND red.cxp = ? " + "                       AND red.ep = pd.ep " + // 6
        "                       AND red.nrenglonpagado = ?)           AS tReintegros, " + "               Isnull((SELECT Isnull(Sum(rd.mimporte), 0) AS impR " + "                       FROM   tReintegroIngresoEncabezado re WITH(nolock), " + "                              tReintegroIngresoDetalle rd WITH(nolock) " + "                       WHERE  re.nfolioreintegro = rd.nfolioreintegro " + "                              AND re.cdocumentohaplicado = 'S' " + // 7
        "                              AND rd.cxp = ? " + // 8
        "                              AND rd.ep = ? " + // 9
