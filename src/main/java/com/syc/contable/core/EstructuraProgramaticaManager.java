...
public class EstructuraProgramaticaManager {
	...
	public static ArrayList<String> validaEPDetalle(Connection conn, String ejercicio_fiscal, int IterCelElx, String cValor, int IterRegElx, String cSubcuenta) throws SQLException {
		...
		try {
			pstmnt = conn.prepareStatement("SELECT 'SELECT count(' + NombreCampo + ') as nTotal FROM ' + NombreCatalogo + ' WITH (NOLOCK)  WHERE '+ NombreCampo + '= ? ' as subQuery, NombreCatalogo "+
	...
	public static String validaRMPOG (Connection conn, String cEP, String ejercicio_fiscal, String cSubcuenta ) throws SQLException {
		...
		ResultSet rs = null;
		if ("EPA".equals(cSubcuenta)){
			cQueryValida="SELECT COUNT(*) "
			    + "  FROM tCatalogoEPAnteProy e WITH (NOLOCK)"
			    + "     , tCatalogoRMPOG r WITH (NOLOCK)"
			    + " WHERE e.EP = ? "
			    + "   AND e.aEjercicioFiscal = r.aEjercicioFiscal "
			    + "   AND e.cRamoEP = r.cRamo "
			    + "   AND e.cProgramaPresupuestario = r.cProgramaPresupuestario "
			    + "   AND e.cPartida = r.cPartida ";
		}else{
			cQueryValida = "SELECT COUNT(*) "
			    + "  FROM tCatalogoEP e WITH (NOLOCK)"
			    + "     , tCatalogoPartidaValidaPP r WITH (NOLOCK)"
			    + " WHERE e.EP = ? "
			    + "   AND e.aEjercicioFiscal = r.aEjercicioFiscal "
			    + "   AND e.cRamoEP = r.cRamo "
			    + "   AND e.cProgramaPresupuestario = r.cProgramaPresupuestario "
	...
	public static String validaRUGFFSFPGAIPP (Connection conn, String cEP, String ejercicio_fiscal, String cSubcuenta ) throws SQLException {
		...
		ResultSet rs = null;
		if ("EPA".equals(cSubcuenta)){
			cQueryValida= "select count(*)  as nExiste "
			    + "  from tCatalogoEPAnteProy e WITH (NOLOCK)"
			    + "     , tCatalogoRUGFFSFPGAIPP r WITH (NOLOCK)"
			    + " WHERE e.EP = ? AND " 
			    + "   e.aEjercicioFiscal = r.aEjercicioFiscal "
			    + "   AND e.cGrupoFuncional = r.cGrupoFuncional "
			    + "   AND e.cFuncion = r.cFuncion "
			    + "   AND e.cSubFuncion = r.cSubFuncion "
			    + "   AND e.cActividadInstitucional = r.cActividadInstitucional "
			    + "   AND e.cUnidadResponsableEP = r.cUnidadResponsable";
		}else{
			cQueryValida = "select count(*)  as nExiste " 
			    + "  from tCatalogoEP e WITH (NOLOCK)"
			    + "     , tCatalogoEstProgAut r WITH (NOLOCK)"
			    + " WHERE e.EP = ? AND "
			    + "    e.aEjercicioFiscal = r.aEjercicioFiscal "
			    + "   AND e.cGrupoFuncional = r.cGrupoFuncional "
			    + "   AND e.cFuncion = r.cFuncion "
	...
	public static String validaROGTGFF (Connection conn, String cEP, String ejercicio_fiscal, String cSubcuenta ) throws SQLException {
		...
		if (!"EPA".equals(cSubcuenta)){
			cQueryValida="SELECT count(*) as nExiste " +
			 "  FROM tCatalogoEP e WITH (NOLOCK) " +
			 "     , tCatalogoROGTGFF f WITH (NOLOCK) " +
			 " WHERE e.ep = ? AND " +
			 "    e.cRamoEP = f.cRamo " +
			 "   AND e.aEjercicioFiscal = f.aEjercicioFiscal " +
			 "   AND e.cPartida = f.cPartida " +
			 "   AND e.cFuenteFinanciamiento =  f.cFuenteFinanciamiento " +
			 "   AND e.cTipoGasto = f.cTipoGasto";
		}else{
			cQueryValida="SELECT count(*) as nExiste " + 
			 "  FROM tCatalogoEPAnteProy e WITH (NOLOCK)" +
			 "     , tCatalogoEstProgAut f WITH (NOLOCK)" +
			 " WHERE e.ep = ? AND " +
			 "    e.cRamoEP = f.cRamo " +
			 "   AND e.aEjercicioFiscal = f.aEjercicioFiscal " +
			 "   AND e.cPartida = f.cPartida " +
	...
	public static String insertEP(Connection conn, String cEP, String cSubCuenta, String aEjercicioFiscal)throws SQLException {
		...
		try{
			...
			if (iExiste > 0){
				...
				pstmnt = conn.prepareStatement("INSERT INTO [dbo].[tCatalogoEPAnteProy] "+
						"(aEjercicioFiscal,EP,ClaveSIAFF,ClaveInterna,cRamoEP,cUnidadResponsableEP,cGrupoFuncional,cFuncion,cSubFuncion, "+
						" cProgramaGeneral,cActividadInstitucional,cProgramaPresupuestario,cPartida,cTipoGasto,cFuenteFinanciamiento,"+
						" cEntidadFederativa,cCartera,cUnidadNorativa,cUnidadEjecutora,cRamo )VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
