...
public class RectificacionPresupuestariaManager {
	 ...
	 */
	static String				ur;
	private static final Logger	log	= Logger.getLogger(RectificacionPresupuestariaManager.class);
	...
	public static Rectificacion filtraCLC(Connection conn, String clc) throws CLCNoPagadaException, Exception {
		...
		String SqlH = "SELECT " + "			case cTipoPago  when 'PAGOOBRA' then 'COMPROMISO'" + "							when 'PAGODIVERSO' then 'COMPROMISO'" + "							when 'PAGODIRECTO' then 'DIRECTA' " + "							when 'RELACIONGASTOS' then 'DIRECTA' "
			+ "							when 'NOMINA' then 'DIRECTA' when 'AJENAS' then 'DIRECTA' when 'FEDERALIZADO' then 'DIRECTA' " + "							ELSE '' END AS dTipoPago " + "			,nFolioPagado" + "			,caNoContrarrecibo" + "			,nFolioPoliza" + "			,nFolioSIAFF" + "			,cDescripcionPoliza"
			+ "			,cUnidadResponsableContable" + "			,nFolioSICOP " + " FROM tPagadoEncabezado WHERE nCLC = ? AND cDocumentoHaplicado = 'S'";
	...
	public static Rectificacion filtraFolioSICOP(Connection conn, String folioSICOP, String CXP) throws Exception {
		Rectificacion rectificacion = null;
		String SqlH = "SELECT CASE cTipoPago WHEN 'PAGOOBRA' THEN 'COMPROMISO' WHEN 'PAGODIVERSO' THEN 'COMPROMISO' WHEN 'PAGODIRECTO' THEN 'DIRECTA' WHEN 'RELACIONGASTOS' THEN 'DIRECTA' WHEN 'NOMINA' THEN 'COMPROMISO' when 'AJENAS' then 'DIRECTA' when 'FEDERALIZADO' then 'COMPROMISO' "
			+ "ELSE '' END AS dTipoPago,nFolioPagado,cTipoPago,cDocumentoHaplicado,caNoContrarrecibo,nFolioPoliza,nFolioSIAFF,cDescripcionPoliza,cUnidadResponsableContable,nFolioSICOP FROM tPagadoEncabezado WITH(NOLOCK) WHERE nFolioSICOP = ? ";
		...
		if (!"".equals(CXP) && CXP != null) {
			SqlH = "SELECT CASE ctipopago " + "         WHEN 'PAGOOBRA' THEN 'COMPROMISO' " + "         WHEN 'PAGODIVERSO' THEN 'COMPROMISO' " + "         WHEN 'PAGODIRECTO' THEN 'DIRECTA' " + "         WHEN 'RELACIONGASTOS' THEN 'DIRECTA' " + "         WHEN 'NOMINA' THEN 'COMPROMISO' "
				+ "         WHEN 'AJENAS' THEN 'DIRECTA' " + "         WHEN 'FEDERALIZADO' THEN 'COMPROMISO' " + "         ELSE '' " + "       END AS dTipoPago, " + "       nfoliopagado, " + "       ctipopago, " + "       cdocumentohaplicado, " + "       canocontrarrecibo, "
				+ "       nfoliopoliza, " + "       nfoliosiaff, " + "       cdescripcionpoliza, " + "       cunidadresponsablecontable, " + "       nfoliosicop " + "FROM   tpagadoencabezado WITH(nolock) " + "WHERE  nfoliosicop = ? " + "       AND canocontrarrecibo = ? ";
			SqlD = "SELECT * FROM   tpagadodetalle WITH(nolock) " + "WHERE  nfoliopagado = (SELECT nfoliopagado " + "                       FROM   tpagadoencabezado " + "                       WHERE  nfoliosicop = ? " + "                              AND canocontrarrecibo = ?) "
				+ "ORDER  BY ndocrenglon ";
		}
	...
	public static Rectificacion filtraFolioSICOPRect(Connection conn, String folioSICOP, String CXP, String contrarrecibo) throws Exception {
		Rectificacion rectificacion = null;
		String SqlH = "SELECT CASE cTipoPago WHEN 'PAGOOBRA' THEN 'COMPROMISO' WHEN 'PAGODIVERSO' THEN 'COMPROMISO' WHEN 'PAGODIRECTO' THEN 'DIRECTA' WHEN 'RELACIONGASTOS' THEN 'DIRECTA' WHEN 'NOMINA' THEN 'COMPROMISO' when 'AJENAS' then 'DIRECTA' when 'FEDERALIZADO' then 'COMPROMISO' "
			+ "ELSE '' END AS dTipoPago,nFolioPagado,cTipoPago,cDocumentoHaplicado,caNoContrarrecibo,nFolioPoliza, ISNULL( nFolioSIAFF, -1 ) AS nFolioSIAFF,cDescripcionPoliza,cUnidadResponsableContable,nFolioSICOP FROM tPagadoEncabezado WITH(NOLOCK) WHERE nFolioSICOP = ? ";
		String SqlD = "SELECT * FROM tPagadoDetalle WITH(NOLOCK) WHERE nFolioPagado = (SELECT nFolioPagado FROM tPagadoEncabezado WHERE nFolioSICOP = ? ) ORDER BY nDocRenglon";
		if (!"".equals(CXP) && CXP != null) {
			SqlH = "SELECT CASE ctipopago WHEN 'PAGOOBRA' THEN 'COMPROMISO' WHEN 'PAGODIVERSO' THEN 'COMPROMISO' WHEN 'PAGODIRECTO' THEN 'DIRECTA' WHEN 'RELACIONGASTOS' THEN 'DIRECTA' WHEN 'NOMINA' THEN 'COMPROMISO' "
				+ "         WHEN 'AJENAS' THEN '' WHEN 'FEDERALIZADO' THEN 'COMPROMISO' ELSE '' END AS dTipoPago, nfoliopagado, ctipopago, cdocumentohaplicado, canocontrarrecibo, "
				+ "       nfoliopoliza, ISNULL( nFolioSIAFF, -1 ) AS nFolioSIAFF, cdescripcionpoliza, cunidadresponsablecontable, nfoliosicop FROM   tpagadoencabezado WITH(nolock) WHERE  nfoliosicop = ? AND canocontrarrecibo = ? ";
			...
			if("".equals(contrarrecibo)){
				...
				SqlD = " SELECT nDocRenglon AS nDocRenglonP, " 
					 + " 		Ep as EPPAG, "
					 + " 		mTotal AS mImportePagado, "
					 + " 		cMes AS nMesP, "
					 + " 		caNoContrarrecibo, "
					 + " 		'-1' AS SubPrograma, "
					 + " 		'-1' AS programa "
					 + " FROM tOperAjenasDetalle WITH (NOLOCK) "  
					 + " WHERE nFolioOperAjenas = (SELECT nFolioOperAjenas " 
					 + "							FROM tOperAjenasEncabezado WITH (NOLOCK) "
	...
	public static Rectificacion filtraFolioSIAFF(Connection conn, String folioSIAFF) throws Exception {
		Rectificacion rectificacion = null;
		String SqlH = "SELECT " + "			case cTipoPago  when 'PAGOOBRA' then 'COMPROMISO'" + "							when 'PAGODIVERSO' then 'COMPROMISO'" + "							when 'PAGODIRECTO' then 'DIRECTA' " + "							when 'RELACIONGASTOS' then 'DIRECTA' "
			+ "							when 'NOMINA' then 'COMPROMISO' when 'AJENAS' then 'DIRECTA' when 'FEDERALIZADO' then 'COMPROMISO' " + "							ELSE '' END AS dTipoPago " + "			,nFolioPagado" + "			,caNoContrarrecibo" + "			,nFolioPoliza" + "			,cTipoPoliza" + "			,cDescripcionPoliza"
			+ "			,cUnidadResponsableContable" + "			,nFolioSICOP " + " FROM tPagadoEncabezado WHERE nFolioSIAFF = ? AND cDocumentoHaplicado = 'S'";
	...
	public static Rectificacion filtraFolioSAI(Connection conn, String folioSAI) throws Exception {
		...
		String SqlH = "SELECT " + "			case cTipoPago  when 'PAGOOBRA' then 'COMPROMISO'" + "							when 'PAGODIVERSO' then 'COMPROMISO'" + "							when 'PAGODIRECTO' then 'DIRECTA' " + "							when 'RELACIONGASTOS' then 'DIRECTA' "
			+ "							when 'NOMINA' then 'COMPROMISO' when 'AJENAS' then 'DIRECTA' when 'FEDERALIZADO' then 'COMPROMISO' " + "							ELSE '' END AS dTipoPago " + "			,nFolioPAGado" + "			,caNoContrarrecibo" + "			,nFolioPoliza" + "			,cTipoPoliza" + "			,cDescripcionPoliza"
			+ "			,cUnidadResponsableContable" + "			,nFolioSICOP" + " FROM tPagadoEncabezado with (nolock) WHERE nFolioSAI = ? AND cDocumentoHaplicado = 'S'";
	...
	public static boolean insertarEncabezado(Connection conn, int folio, String nIDCaso, String cEjercicio, String cRamo, String cUnidad, String cCentroContable, java.sql.Date fExp, java.sql.Date fApl, String conceptoRectificacion, String cTipoMovto, String nOrigenPPTO, String nMes,
		String oficioRectif, String ctr_int, String cTipoRectificacion, String nFolioSICOP, String caNoContrarrecibo, String cTipoPoliza, String cDescripcionPoliza, String u_login, String nFolioPoliza, String cUnidadResponsableContable, String nFolioSIAFF, String totalDebe, String totalDice, String esIP)
		throws SQLException {
		...
		String insertQuery = "INSERT INTO tRectificacionEncabezado(nFolioRectificacion,nIDCaso,aEjercicioFiscal,cRamo,cUnidadResponsable" + ",cCentroContable,fExp,fAplicacion,cConceptoRectificacion,cTipoMovto,nOrigenPPTO"
			+ "	 ,nMes,oficioRectif,ctr_int,cTipoRectificacion,nFolioSICOP,caNoContrarrecibo" + " ,nFolioPoliza,cTipoPoliza,cDescripcionPoliza,cU_LoginCaptura" + "	,cUnidadResponsableContable,nFolioSIAFF,totalDebe,totalDice, cEsIP) " + "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public static Rectificacion queryPagado(Connection conn, String criterio, String SqlH, String SqlD, String CXP) throws URInaccesibleException, CLCNoPagadaException, Exception {
		...
		// Realizar conexion
		log.debug("[SQLH]" + SqlH + "[CxP=" + criterio + "]");
		log.debug("[SQLD]" + SqlD + "[CxP=" + criterio + "]");
		try {
	...
	public static void autorizaRectificacion(Connection conn, Caso c, String ulogin, String prefixPath, String tipoAplicacion, String fApl) throws SQLException, GestionException {
		...
		try {
			...
			/*ARLA SE CAMBIA EL CALCULO DE EVENTO SI ES RECTIFICACION DE INGRESO PROPIO LEERA LA TABLA tEventoRectificacion_IP*/
			else if("S".equals(EsIP) && "N".equals(EsFFM)){
			...
			/*ARLA SE CAMBIA EL CALCULO DE EVENTO SI ES UN PAGO DEL FONDO FORESTAL MEXICANO Y ES DE INGRESO FISCAL*/
			else if("N".equals(EsIP) && "S".equals(EsFFM)){
	...
	public static void autorizaRectificacionMil(Connection conn, Caso c, String ulogin, String prefixPath, String tipoAplicacion) throws SQLException, GestionException {
		...
		try {
			...
			if (nExiste == 0) {
				pstmnt = conn
					.prepareStatement("INSERT INTO tRectificacionAutEncabezadoMil (nFolioRectificacionMilAut, nIdCaso, aEjercicioFiscal, cRamo, cUnidadResponsable, cCentroContable, fExp, fAplicacion, cConceptoRectificacion, cTipoMovto, "
				...
				pstmnt = conn.prepareStatement("INSERT INTO tRectificacionAutDetalleMil(nFolioRectificacionMilAut,nDocRenglon,EP,cEvento,mImporte, mImporteNegativo, cMes, cCentroContable, OBGT) " + "SELECT rd.nFolioRectificacionMil,rd.nDocRenglon,rd.EP,CASE ( Substring(cevento, 1, 4) ) "
					+ " WHEN 'DICE' THEN 'DICE_DI_' + cnoevento "
					+ " WHEN 'DEBE' THEN 'DEBE_DECIR_' + cnoevento END AS cEvento,rd.mImporte,rd.mImporteNegativo,rd.cMes, rd.cCentroContable,SUBSTRING(EP,32,5) AS obgt "
					+ " FROM tRectificacionDetalleMil rd WITH (NOLOCK) JOIN tEventoRectificacion WITH (NOLOCK) ON SUBSTRING(EP,32,5) = cPartida WHERE rd.nFolioRectificacionMil = ?");
				pstmnt.setInt(1, nIdCaso);
	...
	public static RectificacionEncabezado getRectificacionEncabezadoSicop(Connection conn, int folio) throws SQLException {
		...
		PreparedStatement pstm = null;
		RectificacionEncabezado re = null;
		pstm = conn.prepareStatement("select 'H' as posicion1, r.fAplicacion, r.fExp, r.cRamo, r.cRamo, r.cRamo, r.cUnidadResponsable, r.cUnidadResponsable, r.cUnidadResponsable, isnull(r.cTipoMovto,'') as cTipoMovto, "
			+ " isnull(r.nOrigenPPTO,'') as nOrigenPPTO, ISNULL(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(r.cConceptoRectificacion, Char(13), ''),Char(10), ''),Char(9), ''),',',''),'.',''),'\"',''),'') as cConceptoRectificacion, "
			+ " isnull(R.ctr_int,'') as ctr_int, R.oficioRectif, R.oficioRectif, R.nFolioRectificacion, R.nIDCaso, R.totalDICE,R.totalDEBE "
			+ " from tRectificacionEncabezado r with (nolock) WHERE nFolioRectificacion=?");
		pstm.setInt(1, folio);
		res = pstm.executeQuery();
		if (res.next()) {
			re = new RectificacionEncabezado();
	...
	public static void insertaRectificacionMil(Connection conn, RectificacionEncabezado recE, ArrayList<RectificacionDetalle> recDetalles, int folio, String folioCompleto) throws Exception {
		...
		try {
			psInsertEncabezado = conn.prepareStatement("INSERT INTO tRectificacionEncabezadoMil(nFolioRectificacionMil,nIDCaso,cDocumentoHaplicado,aEjercicioFiscal,cRamo,cUnidadResponsable"
				+ " ,cCentroContable,fExp,fAplicacion,cConceptoRectificacion,cTipoMovto,nOrigenPPTO,nMes,oficioRectif,ctr_int,cTipoRectificacion,nCLC" + " ,nFolioSICOP,nFolioSAI,nFolioSIAFF,caNoContrarrecibo,caNoContrarreciboMil,nFolioPoliza,cTipoPoliza,cDescripcionPoliza,nFolioPolizaCancelacion"
				+ " ,fCancelacion,nFolioTramiteSicop,cUnidadResponsableContable,totalDice,totalDebe)" + " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			log.debug("[" + folio + "]");
			log.debug("[" + recE.getnIdCaso() + "]");
			log.debug("[" + "N" + "]");
			log.debug("[" + recE.getaEjercicioFiscal() + "]");
			log.debug("[" + recE.getcRamo() + "]");
			log.debug("[" + recE.getcUnidadResponsable() + "]");
			log.debug("[" + recE.getcCentroContable() + "]");
			log.debug("[" + recE.getfExp() + "]");
			log.debug("[" + recE.getfAplicacion() + "]");
			log.debug("[" + recE.getcConceptoRectificacion() + "]");
			log.debug("[" + recE.getcTipoMovto() + "]");
			log.debug("[" + recE.getnOrigenPPTO() + "]");
			log.debug("[" + recE.getnMes() + "]");
			log.debug("[" + folioCompleto + "]");
			log.debug("[" + recE.getCtr_int() + "]");
			log.debug("[" + recE.getcTipoRectificacion() + "]");
			log.debug("[" + recE.getnCLC() + "]");
			log.debug("[" + recE.getnFolioSicop() + "]");
			log.debug("[" + recE.getnFolioSAI() + "]");
			log.debug("[" + recE.getnFolioSIAFF() + "]");
			log.debug("[" + recE.getCaNoContrarrecibo().trim() + "]");
			log.debug("[" + recE.getCaNoContrarreciboMil().trim() + "]");
			log.debug("[" + recE.getnFolioPoliza() + "]");
			log.debug("[" + recE.getcTipoPoliza() + "]");
			log.debug("[" + recE.getcDescripcionPoliza() + "]");
			log.debug("[" + recE.getnFolioPolizaCancelacion() + "]");
			log.debug("[" + recE.getfCancelacion() + "]");
			log.debug("[" + recE.getnFolioTramiteSicop() + "]");
			log.debug("[" + recE.getcUnidadResponsableContable() + "]");
			log.debug("[" + recE.getTotalDice() + "]");
			log.debug("[" + recE.getTotalDebe() + "]");
	...
	public static RectificacionEncabezado getRectificacionEncabezadoSicopMil(Connection conn, int folio) throws SQLException {
		...
		RectificacionEncabezado re = null;
		pstm = conn.prepareStatement("select 'H' as posicion1, r.fAplicacion, r.fExp, r.cRamo, r.cRamo, r.cRamo, r.cUnidadResponsable, r.cUnidadResponsable, r.cUnidadResponsable, isnull(r.cTipoMovto,'') as cTipoMovto, "
			+ " isnull(r.nOrigenPPTO,'') as nOrigenPPTO, isnull(r.cConceptoRectificacion,'') as cConceptoRectificacion, isnull(R.oficioRectif,'') as ctr_int, R.oficioRectif, R.oficioRectif, R.nFolioRectificacionMil, R.nIDCaso, R.totalDICE,R.totalDEBE "
			+ " from tRectificacionEncabezadoMil r with (nolock) WHERE nFolioRectificacionMil=?");
		pstm.setInt(1, folio);
		res = pstm.executeQuery();
		if (res.next()) {
			re = new RectificacionEncabezado();
	...
	public static StringBuilder getRectificacionDetalleSicopMil(Connection conn, int folio) throws SQLException {
		...
		pstm = conn.prepareStatement("select distinct isnull(r.nOrigenPPTO,0) as nOrigenPPTO,'0',r.nFolioSICOP, d.nDocRenglon, e.cRamoEP, e.cUnidadResponsableEP, e.aEjercicioFiscal, e.cGrupoFuncional, e.cFuncion, "
				...
				+ " e.cTipoGasto, e.cFuenteFinanciamiento, e.cEntidadFederativa, e.cCartera, '0000000000' as cUnidadEjecutora,"
				+ " '00' as cUnidadNormativa, '000','000','00000','00000','0000000000', CONVERT(VARCHAR,d.mImporte), d.cMes, '' as cFillRellen1 , b.sSicop, '' as cFillRellen2 , '0' as sol_oli, dp.ID_TIPO_CONCEPTO, '000' as tipo_concepto,"
				+ " '0' as concepto_mov,"
				+ " '0' retencion_isr, '0', CASE WHEN d.cEvento LIKE '%DICE%' THEN 'A' ELSE 'C' END, isnull(R.oficioRectif,'') as ctr_int from tRectificacionDetalleMil d with (nolock), tRectificacionEncabezadoMil r with (nolock), tCatalogoEP e with (nolock), tPagadoEncabezado P with (nolock), tPagadoDetalle dp  with (nolock), tBeneficiarioCapituloMil b with (nolock)"
				+ " WHERE d.nFolioRectificacionMil =  r.nFolioRectificacionMil and d.EP =  e.EP and r.caNoContrarrecibo = P.caNoContrarrecibo and p.nFolioPagado = dp.nFolioPagado and dp.RFC = b.sCodigoEntidad AND r.nFolioRectificacionMil=? AND d.nFolioRectificacionMil=?");
		try {	
			pstm.setInt(1, folio);
			pstm.setInt(2, folio);
			res = pstm.executeQuery();
