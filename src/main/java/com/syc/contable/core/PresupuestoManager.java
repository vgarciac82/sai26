...
public class PresupuestoManager {
    ...
    public static String reAplicaDoctos(Connection conn) throws SQLException {
        ...
        String cQueryConstructor = //AND a.nFolioAdecuacion = 9999 " +
        "select aEjercicioFiscal, cRamo, cUnidadResponsable, fAplicacion, nFolioDocto, cTipoDocto, Folio " + " from ( select isnull(p.aEjercicioFiscal,'2012') as aEjercicioFiscal, p.cRamo, p.cUnidadResponsable, p.fAplicacion, p.nFolioPresupuesto as Folio, e.FOLIO as nFolioDocto, 'PRESUPUESTO' as cTipoDocto from tPresupuestoEncabezado p with (nolock), IMXPRESUPUESTO e with (nolock) where e.FOLIO = 'CP-'+convert(varchar,p.nFolioPresupuesto ) AND p.cDocumentoHaplicado = 'R'" + " union " + "        select a.aEjercicioFiscal,  a.cRamo, a.cUnidadResponsable,  a.fAplicacion,   a.nFolioAdecuacion as Folio,    e.FOLIO as nFolioDocto, 'Adecuacion' as cTipoDocto     from tAdecuacionEncabezado a with (nolock), IMXADECUACION e with (nolock) where e.FOLIO like '2012-%'+convert(varchar,a.nFolioAdecuacion) AND cDocumentoHaplicado = 'R' " + //AND au.nFolioAdecuacionaut = 9999 " +
        " union " + "        select au.aEjercicioFiscal, au.cRamo, au.cUnidadResponsable, au.fAplicacion, au.nFolioAdecuacionaut as Folio, e.FOLIO as nFolioDocto, 'ADECUACIONAUT' as cTipoDocto  from tADECUACIONAUTEncabezado au with (nolock) , IMXADECUACION e with (nolock)  where e.FOLIO like '2012-%'+convert(varchar,au.nFolioAdecuacionaut) AND cDocumentoHaplicado = 'R' " + //AND pd.nFolioPagoDirecto = 9999 " +
        " union " + "        select pd.aEjercicioFiscal, pd.cRamo, pd.cUnidadResponsable, pd.fAplicacion, pd.nFolioPagoDirecto as Folio, e.FOLIO as nFolioDocto, 'PagoDirecto' as cTipoDocto    from tPagoDirectoEncabezado pd with (nolock), IMXPAGODIRECTO e with (nolock) where e.FOLIO like 'PDIR-%-'+convert(varchar,pd.nFolioPagoDirecto) AND cDocumentoHaplicado = 'R' " + //AND PV.nFolioPagoDiverso = 9999 " +
        " union " + "        select pv.aEjercicioFiscal, pv.cRamo, pv.cUnidadResponsable, pv.fAplicacion, PV.nFolioPagoDiverso as Folio, e.FOLIO as nFolioDocto, 'PAGODIVERSO' as cTipoDocto    from tPAGODIVERSOEncabezado pv with (nolock), IMXPAGODIVERSO e with (nolock) where e.FOLIO like 'PDIV-%'+ CONVERT(VARCHAR,PV.nFolioPagoDiverso)  AND cDocumentoHaplicado = 'R' " + //AND rg.nFolioRELACIONGASTOS = 9999 " +
        " union " + "        select rg.aEjercicioFiscal, rg.cRamo, rg.cUnidadResponsable, rg.fAplicacion, rg.nFolioRELACIONGASTOS as Folio, e.FOLIO as nFolioDocto, 'RELACIONGASTOS' as cTipoDocto from tRELACIONGASTOSEncabezado rg with (nolock) , IMXRELACIONGASTOS e with (nolock) where e.FOLIO like 'RELG-%-'+convert(varchar,rg.nFolioRELACIONGASTOS) AND cDocumentoHaplicado = 'R' " + //AND n.nFolioNOMINA = 9999 " +
        " union  " + "        select n.aEjercicioFiscal,   n.cRamo, n.cUnidadResponsable,   n.fAplicacion, n.nFolioNOMINA as Folio, e.FOLIO as nFolioDocto, 'NOMINA' as cTipoDocto         from tNOMINAEncabezado n with (nolock) , IMXNOMINA e with (nolock)  where e.FOLIO  like 'NOM-%-' + convert(varchar,n.nFolioNOMINA) AND cDocumentoHaplicado = 'R' " + " ) a " + " order by a.fAplicacion ";
        try {
