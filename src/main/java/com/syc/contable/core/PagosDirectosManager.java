...
public class PagosDirectosManager {
	...
	private static final Logger log = Logger.getLogger( PagosDirectosManager.class );
	...
	public static void validarTotalRetenciones(Connection con, BigDecimal total, String listaIds) throws Exception {
		...
		ResultSet rs = null;
		BigDecimal importe = new BigDecimal(0);
		String query = "SELECT SUM(mISRHonorarios + mISRArrenda + mimporteISRResico + m23Iva + mImporteIvaHonorarios + mImporteIvaArrenda  + mImporteFlete23 + ISNULL(mImporteIva6, 0) + ISNULL(mImporteFlete4, 0) + mObra5 + mRetImpuestoCedular) totalRetenciones "
                + "        FROM tPagoDirectoDetalle (NOLOCK) WHERE nFolioPagoDirecto IN (" + listaIds +")";
	...
	private static String generaEncabezadoLayoutCompPDNomina( Connection conn, int folioRelacionGastosCompromiso, String ctaBancaria, String noIntegracion, String foliosPDNomina, int leyenda ) throws Exception {
		...
		queryEncabezadoLayout.append( "SELECT top 1	1, " + "      	'H' AS Header, " );// A
		queryEncabezadoLayout.append( "      	CONVERT(nvarchar(10), GETDATE(), 103) FECHA_EXP, " ); // B
		queryEncabezadoLayout.append( "      	CONVERT(nvarchar(10), GETDATE(), 103) FECHA_APL, " );// C
		queryEncabezadoLayout.append( "      	tCE.cRamo ID_RAMO, " ); // D
		queryEncabezadoLayout.append( "      	tCE.cRamo ID_RAMO_CR, " ); // E
		queryEncabezadoLayout.append( "      	tCE.cRamo ID_RAMO_REC, " ); // F
		queryEncabezadoLayout.append( "      	'RHQ'  ID_UNIDAD, " ); // G
		queryEncabezadoLayout.append( "      	'RHQ'  ID_UNIDAD_CR, " ); // H
		queryEncabezadoLayout.append( "      	'RHQ'  ID_UNIDAD_REC, " ); // I
		queryEncabezadoLayout.append( "      	'N' ID_TIPO_MOVIMIENTO,  " ); // J
		queryEncabezadoLayout.append( "      	'4' AS OrigenPpto, " ); // K
		queryEncabezadoLayout.append( "      	'9' AS TipoSol, " ); // L
		queryEncabezadoLayout.append( "      	'MXN' TipoMoneda, " ); // M
		queryEncabezadoLayout.append( "      	'1' TipoCambio, " ); // N
		queryEncabezadoLayout.append( "      	'1' TIPO_PAGO, " ); // O
		queryEncabezadoLayout.append( "      	'" + leyenda + "' AS CVE_LEYENDA_66," ); // P
		queryEncabezadoLayout.append( "      	CBEN, " ); // Q /*CAMBIAR AL CBEN DEL PROVEEDOR*/
		queryEncabezadoLayout.append( "         '" + ctaBancaria.trim() + "' CUENTA_BANCARIA, " ); // R
		queryEncabezadoLayout.append( "      	tce.cIdRFC RFC_227, " ); // S
		queryEncabezadoLayout.append( "      	'FAC' TDOC_87, " ); // T
		queryEncabezadoLayout.append( "      	'' FechaReferencia, " ); // U
		queryEncabezadoLayout.append( "      	'' Referencia1, " ); // V
		queryEncabezadoLayout.append( "      	'' Referencia2, " ); // W
		queryEncabezadoLayout.append( "      	REPLACE(cDescripcionPoliza,',','') CPAG_76, " ); // X
		queryEncabezadoLayout.append( "      	'' NotasReverso, " ); // Y
		queryEncabezadoLayout.append( "      	'' AMF, " ); // Z
		queryEncabezadoLayout.append( "      	rtrim(tCE.caNoContrarrecibo) NO_ACMI, " ); // AA
		queryEncabezadoLayout.append( "      	rtrim(tCE.caNoContrarrecibo) AuxiliarComodin, " ); // AB
		queryEncabezadoLayout.append( "      	'' CTR, " ); // AC
		queryEncabezadoLayout.append( "      	CONVERT(decimal(17, 2), sum(DCD_ISR),0)  AS ISR_303," ); // Ad
		queryEncabezadoLayout.append( "       	CONVERT(decimal(17, 2), sum(DCD_IVADES), 0) IVA_304, " ); // AE
		queryEncabezadoLayout.append( "       	CONVERT(decimal(17, 2), sum(DCD_MIL5),0) MIL5_305, " ); // AF
		queryEncabezadoLayout.append( "       	CONVERT(decimal(17, 2), sum(DCD_MIL2),0) MIL2_306, " ); // AG
		queryEncabezadoLayout.append( "       	CASE when DCD_OTRAS_RET = 0 or DCD_OTRAS_RET is null THEN '0' ELSE CONVERT(varchar(20), sum(DCD_OTRAS_RET)) end OTRASRET, " ); // AH
		queryEncabezadoLayout.append( "       	'0' PENALIZACION_308, " ); // AI
		queryEncabezadoLayout.append( "       	'0' CONTRIBUCION_309, " ); // AJ
		queryEncabezadoLayout.append( "       	CONVERT(decimal(17, 2), sum(DCD_IVA), 0) AS IVADES_310,	" ); // AK
		queryEncabezadoLayout.append( "       	'' IVAANT_311, " ); // AL
		queryEncabezadoLayout.append( "      	'' FolioDC, " ); // AM
		queryEncabezadoLayout.append( "      	'NA' ID_DESTINO_GASTO,   " ); // AN
		queryEncabezadoLayout.append( "      	'' TTRANS_60, " ); // AO
		queryEncabezadoLayout.append( "      	'' ANT_CON_IVA_413" ); // AP
	...
	private static String generaDetalleLayoutCompPDNomina( Connection conn, int folioRelacionGastosCompromiso, String folioAutSICOP ) throws Exception {
		...
		queryLayoutDetalle.append( " SELECT '1' ID_EVENTO,  	" ); // A
		queryLayoutDetalle.append( " '24.0.001' 	  EVENTO,  	" ); // B
		queryLayoutDetalle.append( " Substring(D.EP, 6, 2)  ID_RAMO_ML, " ); // C
		queryLayoutDetalle.append( " 'RHQ',  					" ); // D
		queryLayoutDetalle.append( " Substring(D.EP, 1, 4)  aEjercicioFiscal," );// E
		queryLayoutDetalle.append( " Substring(D.EP, 13, 1) cGrupoFuncional, " );// F
		queryLayoutDetalle.append( " Substring(D.EP, 15, 1) cFuncion,  " ); // G
		queryLayoutDetalle.append( " Substring(D.EP, 17, 2) cSubFuncion, " ); // H
		queryLayoutDetalle.append( " CASE WHEN Substring(D.EP, 20, 2) IN (SELECT cProgramaGeneral FROM tCat_ProGeneralPlurianual WITH (NOLOCK)) THEN '00' ELSE Substring(D.EP, 20, 2) END AS cProgramaGeneral, " ); // I
		queryLayoutDetalle.append( " Substring(D.EP, 23, 3) cActividadInstitucional, " ); // J
		queryLayoutDetalle.append( " Substring(D.EP, 27, 4) cProgramaPresupuestario," ); // K
		queryLayoutDetalle.append( " Substring(D.EP, 32, 1) CCAP_157,  " ); // L
		queryLayoutDetalle.append( " Substring(D.EP, 33, 1) CCON_158,  " ); // M
		queryLayoutDetalle.append( " Substring(D.EP, 34, 1) CPARG_300, " ); // N
		queryLayoutDetalle.append( " Substring(D.EP, 35, 2) CPAR_159,  " ); // O
		queryLayoutDetalle.append( " Substring(D.EP, 38, 1) cTipoGasto," ); // P
		queryLayoutDetalle.append( " Substring(D.EP, 40, 1) cFuenteFinanciamiento," ); // Q
		queryLayoutDetalle.append( " Substring(D.EP, 42, 2) cEntidadFederativa, " ); // R
		queryLayoutDetalle.append( " SUBSTRING( dbo.CambiaEPCarteraMeta(D.EP),45, 11) AS cCartera," ); // S
		queryLayoutDetalle.append( " '0000000000' AS CCAU_162,  		" ); // T
		queryLayoutDetalle.append( " '00'                   CCOP_163,  	" ); // U
		queryLayoutDetalle.append( " '000'                  PL,  		" ); // V
		queryLayoutDetalle.append( " '000'                  OFI,  		" ); // W
		queryLayoutDetalle.append( " '00000'                AUX1,  		" ); // X
		queryLayoutDetalle.append( " '00000'                AUX2,  		" ); // Y
		queryLayoutDetalle.append( " '0000000000'           AUX3,  		" ); // Z
		queryLayoutDetalle.append( " '" + StringUtils.trimToEmpty( folioAutSICOP ) + "' AS NCOM_35," );// AA
		queryLayoutDetalle.append( " Sum(D.mImporteNeto) AS MONTO,  " );// AB
		queryLayoutDetalle.append( " D.cmes                 MES_149,  	" ); // AC
		queryLayoutDetalle.append( " '0'                    NRES,  		" ); // AD
		queryLayoutDetalle.append( " CASE WHEN Substring(ep, 32, 5) = '35801' THEN 'GD' ELSE 'PN' END TIPO_CONTRATO, " ); // AE
		queryLayoutDetalle.append( " '000'                  CONC_MOV,  	" ); // AF
		queryLayoutDetalle.append( " CONVERT(decimal(17, 2),sum(mimporteISRResico + mISRArrenda + mISRHonorarios+ mISROtros + mImporteISRLaudos),0)  AS RETENCIONES, " ); // AG
		queryLayoutDetalle.append( " CONVERT(decimal(17, 2),sum(mImporteIvaArrenda + mImporteIvaHonorarios + mImporteFlete23+  mImporteFlete4),0) IVA_43,  	" ); // AH
		queryLayoutDetalle.append( " CONVERT(decimal(17, 2),sum(isnull(mObra5, 0) + isnull(mImporteObra,0))) MIL5_44,  						" ); // AI
		queryLayoutDetalle.append( " CONVERT(decimal(17, 2),sum(isnull(m2Millar,0))) MIL2_312,  						" ); // AJ
		queryLayoutDetalle.append( " 0 CONTRIBUCION_48,  				" ); // AK
		queryLayoutDetalle.append( " CONVERT(decimal(17, 2),sum(isnull(mRetImpuestoCedular,0))) OTRAS_RET_313, " ); // AL
		queryLayoutDetalle.append( " 0 IVADES_45,  						" ); // AM
		queryLayoutDetalle.append( " 0 ANTICIPO_46,  					" ); // AN
		queryLayoutDetalle.append( " 0 PENALIZACION_314,  				" ); // AO
		queryLayoutDetalle.append( " 0 IVAANT_47,  						" ); // AP
		queryLayoutDetalle.append( " ''  id_ctr_intdet  				" ); // AQ
	...
	private static void insertaLayoutCompPDNominaEncabezado( Connection conn, int leyenda, String ctaBancaria, String noIntegracion, String foliosRGIntegrados, String usuario, int folioRelacionGastosCompromiso ) throws Exception {
		...
		queryInsertaLayoutEnc.append( "SELECT   getdate(), tCE.nFolioPagoDirecto, 'H' AS Header, " );// A
		queryInsertaLayoutEnc.append( "     	CONVERT(nvarchar(10), tCE.fAplicacion,103) FECHA_EXP, " );// B
		queryInsertaLayoutEnc.append( "     	CONVERT(nvarchar(10), tCE.fAplicacion,103) FECHA_APL, " ); // C
		...
		queryInsertaLayoutEnc.append( "     	'" + leyenda + "' AS CVE_LEYENDA_66," ); // P
		queryInsertaLayoutEnc.append( "     	CBEN, " ); // Q
		queryInsertaLayoutEnc.append( "         '" + ctaBancaria.trim() + "' CUENTA_BANCARIA, " );// R
		queryInsertaLayoutEnc.append( "     	dRFC RFC_227, " ); // S
		...
		queryInsertaLayoutEnc.append( "     	LEFT(cConcepto,500) CPAG_76, " );// X
		queryInsertaLayoutEnc.append( "      	'' NotasReverso, " ); // Y
		queryInsertaLayoutEnc.append( "     	'' AMF, " ); // Z
		queryInsertaLayoutEnc.append( "      	tCE.canocontrarrecibo NO_ACMI, " ); // AA
		queryInsertaLayoutEnc.append( "      	'" + noIntegracion + "' AuxiliarComodin, " ); // AB
		queryInsertaLayoutEnc.append( "     	'' CTR, " ); // AC
		queryInsertaLayoutEnc.append( "     	'' FolioDC, " ); // AD
		queryInsertaLayoutEnc.append( "     	CONVERT(decimal(17, 2),SUM(DCD_ISR),0)  AS ISR_303, " ); // AE
		queryInsertaLayoutEnc.append( "      	CONVERT(decimal(17, 2), SUM(DCD_IVA), 0) IVA_304, " ); // AF
		queryInsertaLayoutEnc.append( "      	CONVERT(decimal(17, 2),0) MIL5_305, " ); // AG
		queryInsertaLayoutEnc.append( "      	CONVERT(decimal(17, 2),0) MIL2_306, " ); // AH
		queryInsertaLayoutEnc.append( "      	0 mImporteRetencion, " ); // AI
		queryInsertaLayoutEnc.append( "      	CONVERT(decimal(17, 2),0) PENALIZACION_308, " ); // AJ
		queryInsertaLayoutEnc.append( "      	CONVERT(decimal(17, 2), 0) CONTRIBUCION_309, " ); // AK
		queryInsertaLayoutEnc.append( "      	CONVERT(decimal(17, 2), SUM(DCD_IVADES), 0) AS IVADES_310,	" ); // AL
		queryInsertaLayoutEnc.append( "      	0 IVAANT_311, " ); // AM
		queryInsertaLayoutEnc.append( "     	ID_DESTINO_GASTO,   " ); // AN
	...
	private static void insertaLayoutCompPDNominaDetalle( Connection conn, String foliosPDNomina, String folioAutSICOP, int folioRelacionGastosCompromiso ) throws Exception {
		...
		queryInsertLayoutCompRG.append( " SELECT " + foliosPDNomina + " nFolio, '1'                    ID_EVENTO, " ); // A
		queryInsertLayoutCompRG.append( "       '24.0.001'             EVENTO, " ); // B
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 6, 2)  ID_RAMO_ML, " ); // C
		queryInsertLayoutCompRG.append( "       'RHQ', " ); // D
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 1, 4)  aEjercicioFiscal, " );// E
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 13, 1) cGrupoFuncional, " ); // F
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 15, 1) cFuncion, " ); // G
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 17, 2) cSubFuncion, " ); // H
		queryInsertLayoutCompRG.append( "       CASE WHEN Substring(D.EP, 20, 2) IN (SELECT cProgramaGeneral FROM tCat_ProGeneralPlurianual WITH (NOLOCK)) THEN '00' ELSE Substring(D.EP, 20, 2)  END   AS cProgramaGeneral, " );// I
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 23, 3) cActividadInstitucional, " );// J
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 27, 4) cProgramaPresupuestario, " );// K
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 32, 1) CCAP_157, " );// L
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 33, 1) CCON_158, " );// M
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 34, 1) CPARG_300, " );// N
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 35, 2) CPAR_159, " );// O
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 38, 1) cTipoGasto, " );// P
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 40, 1) cFuenteFinanciamiento, " );// Q
		queryInsertLayoutCompRG.append( "       Substring(D.EP, 42, 2) cEntidadFederativa, " ); // R
		queryInsertLayoutCompRG.append( "       SUBSTRING( dbo.CambiaEPCarteraMeta(D.EP),45, 11) AS cCartera, " ); // S
		queryInsertLayoutCompRG.append( "       ltrim('0000000' + tCEP.cUnidadEjecutora) AS CCAU_162, " ); // T
		queryInsertLayoutCompRG.append( "       substring(TCEP.cUnidadNorativa,2,2) AS CCOP_163, " ); // U
		queryInsertLayoutCompRG.append( "       '000'                  PL, " ); // V
		queryInsertLayoutCompRG.append( "       '000'                  OFI, " ); // W
		queryInsertLayoutCompRG.append( "       '00000'                AUX1, " ); // X
		queryInsertLayoutCompRG.append( "       '00000'                AUX2, " ); // Y
		queryInsertLayoutCompRG.append( "       '0000000000'           AUX3, " ); // Z
		queryInsertLayoutCompRG.append( "       '" + StringUtils.trimToEmpty( folioAutSICOP ) + "' AS NCOM_35," );// AA
		queryInsertLayoutCompRG.append( "       SUM(D.mImporteNeto) AS MONTO, " );// AB
		queryInsertLayoutCompRG.append( "       D.cmes                 MES_149, " ); // AC
		queryInsertLayoutCompRG.append( "       '0'                    NRES, " ); // AD
		queryInsertLayoutCompRG.append( "       CASE WHEN Substring(d.ep, 32, 5) = '35801' THEN 'GD' ELSE 'PN' END  TIPO_CONTRATO, " );// AE
		queryInsertLayoutCompRG.append( "       '000'                  CONC_MOV, " ); // AF
		queryInsertLayoutCompRG.append( "       CONVERT(decimal(17, 2),ISNULL(SUM(mimporteISRResico + mImporteISRLaudos + mISROtros + mISRArrenda + mISRHonorarios),0),0)    AS RETENCIONES, " ); // AG
		queryInsertLayoutCompRG.append( "       CONVERT(decimal(17, 2),SUM(mimporteivaArrenda + mimporteivahonorarios + mimporteflete23 + mImporteFlete4 ),0) IVA_43, " ); // AH
		queryInsertLayoutCompRG.append( "       0 MIL5_44, " ); // AI
		queryInsertLayoutCompRG.append( "       0 MIL2_312, " ); // AJ
		queryInsertLayoutCompRG.append( "       0 CONTRIBUCION_48, " ); // AK
		queryInsertLayoutCompRG.append( "       0 OTRAS_RET_313, " ); // AL
		queryInsertLayoutCompRG.append( "       0 IVADES_45, " ); // AM
		queryInsertLayoutCompRG.append( "       0 ANTICIPO_46, " ); // AN
		queryInsertLayoutCompRG.append( "       0 PENALIZACION_314, " ); // AO
		queryInsertLayoutCompRG.append( "       0 IVAANT_47, " ); // AP
		queryInsertLayoutCompRG.append( "       ''  id_ctr_intdet " ); // AQ
	...
	private static PreparedStatement generaEncabezadoLayoutCompPD( Connection conn, String listaFolios ) throws Exception {
		StringBuilder queryEncabezadoLayout = new StringBuilder();
		queryEncabezadoLayout.append( "SELECT top 1	tCE.nFolioPagoDirecto, 	'H' AS Header, " );// A
		queryEncabezadoLayout.append( "      	CONVERT(nvarchar(10), GETDATE(), 103) FECHA_EXP, " ); // B
		queryEncabezadoLayout.append( "      	CONVERT(nvarchar(10), GETDATE(), 103) FECHA_APL, " );// C
		queryEncabezadoLayout.append( "      	tCE.cRamo ID_RAMO, " ); // D
		queryEncabezadoLayout.append( "      	tCE.cRamo ID_RAMO_CR, " ); // E
		...
		queryEncabezadoLayout.append( "      	'RHQ'  ID_UNIDAD_REC, " ); // I
		queryEncabezadoLayout.append( "      	'N' ID_TIPO_MOVIMIENTO,  " ); // J
		queryEncabezadoLayout.append( "      	'5' AS OrigenPpto, " ); // K
		queryEncabezadoLayout.append( "      	'9' AS TipoSol, " ); // L
		queryEncabezadoLayout.append( "      	'MXN' TipoMoneda, " ); // M
		queryEncabezadoLayout.append( "      	'1' TipoCambio, " ); // N
		queryEncabezadoLayout.append( "      	'1' TIPO_PAGO, " ); // O
		queryEncabezadoLayout.append( "      	1 AS CVE_LEYENDA_66," ); // P
		queryEncabezadoLayout.append( "      	CBEN, " ); // Q
		queryEncabezadoLayout.append( "         CTAB CUENTA_BANCARIA, " ); // R
		queryEncabezadoLayout.append( "      	cIdRFC RFC_227, " ); // S
		queryEncabezadoLayout.append( "      	'FAC' TDOC_87, " ); // T
		queryEncabezadoLayout.append( "      	 '' FechaReferencia, " ); // U
		queryEncabezadoLayout.append( "      	'' Referencia1, " ); // V
		queryEncabezadoLayout.append( "      	'' Referencia2, " ); // W
		queryEncabezadoLayout.append( "      	LEFT(cDescripcionPoliza,500) CPAG_76, " ); // X
		queryEncabezadoLayout.append( "      	'' NotasReverso, " ); // Y
		queryEncabezadoLayout.append( "      	'' AMF, " ); // Z
		queryEncabezadoLayout.append( "      	caNoContrarrecibo NO_ACMI, " ); // AA
		queryEncabezadoLayout.append( "      	caNoContrarrecibo AuxiliarComodin, " ); // AB
		queryEncabezadoLayout.append( "      	caNoContrarrecibo CTR, " ); // AC
		queryEncabezadoLayout.append( "      	CONVERT(decimal(17, 2), sum(DCD_ISR),0)  AS ISR_303," ); // Ad
		queryEncabezadoLayout.append( "       	CONVERT(decimal(17, 2), sum(DCD_IVA), 0) IVA_304, " ); // AE
		queryEncabezadoLayout.append( "       	CONVERT(decimal(17, 2), sum(DCD_MIL5),0) MIL5_305, " ); // AF
		queryEncabezadoLayout.append( "       	CONVERT(decimal(17, 2), sum(DCD_MIL2),0) MIL2_306, " ); // AG
		queryEncabezadoLayout.append( "       	ISNULL(CONVERT(varchar(20), sum(DCD_OTRAS_RET)),0) OTRASRET, " ); // AH
		queryEncabezadoLayout.append( "       	'0' PENALIZACION_308, " ); // AI
		queryEncabezadoLayout.append( "       	'0' CONTRIBUCION_309, " ); // AJ
		queryEncabezadoLayout.append( "       	CONVERT(decimal(17, 2), sum(DCD_IVADES), 0) AS IVADES_310,	" ); // AK
		queryEncabezadoLayout.append( "       	'' IVAANT_311, " ); // AL
		queryEncabezadoLayout.append( "      	'' FolioDC, " ); // AM
		queryEncabezadoLayout.append( "      	'NA' ID_DESTINO_GASTO,   " ); // AN
		queryEncabezadoLayout.append( "      	'' TTRANS_60, " ); // AO
	...
	private static PreparedStatement generaDetalleLayoutCompPD( Connection conn, String folio) throws Exception {
		StringBuilder queryLayoutDetalle = new StringBuilder();
		queryLayoutDetalle.append( " SELECT d.nFolioPagoDirecto, '1' ID_EVENTO,  	" ); // A
		queryLayoutDetalle.append( " '24.0.001' 	  EVENTO,  	" ); // B
		queryLayoutDetalle.append( " Substring(D.EP, 6, 2)  ID_RAMO_ML, " ); // C
		queryLayoutDetalle.append( " 'RHQ',  					" ); // D
		queryLayoutDetalle.append( " Substring(D.EP, 1, 4)  aEjercicioFiscal," );// E
		...
		queryLayoutDetalle.append( " '00000'                AUX2,  		" ); // Y
		queryLayoutDetalle.append( " '0000000000'           AUX3,  		" ); // Z
		queryLayoutDetalle.append( " nFolioAutSICOP  AS NCOM_35," );// AA
		queryLayoutDetalle.append( " Sum(D.mImporteNeto)  AS MONTO,  " );// AB
		queryLayoutDetalle.append( " D.cmes                 MES_149,  	" ); // AC
		queryLayoutDetalle.append( " nFolioSuficiencia as NRES, " ); // AD
		queryLayoutDetalle.append( " CASE WHEN Substring(ep, 32, 5) = '35801' THEN 'GD' ELSE 'PN' END TIPO_CONTRATO, " ); // AE
		queryLayoutDetalle.append( " '000'                  CONC_MOV,  	" ); // AF
		queryLayoutDetalle.append( " CONVERT(decimal(17, 2),sum(mimporteISRResico + mISRArrenda + mISRHonorarios+ mISROtros + mImporteISRLaudos),0)  AS RETENCIONES, " ); // AG
		queryLayoutDetalle.append( " CONVERT(decimal(17, 2),sum(mImporteIvaArrenda + mImporteIvaHonorarios + mImporteFlete23+  mImporteFlete4),0) IVA_43,  	" ); // AH
		queryLayoutDetalle.append( " CONVERT(decimal(17, 2),sum(isnull(mObra5, 0) + isnull(mImporteObra,0))) MIL5_44,  						" ); // AI
		queryLayoutDetalle.append( " CONVERT(decimal(17, 2),sum(isnull(m2Millar,0))) MIL2_312,  						" ); // AJ
		queryLayoutDetalle.append( " CONVERT(decimal(17, 2),sum(isnull(mRetImpuestoCedular,0))) OTRAS_RET_313, " ); // AK
		queryLayoutDetalle.append( " 0 PENALIZACION_314,   						" ); // AL
		queryLayoutDetalle.append( " 0 CONTRIB,  				" ); // AM
		queryLayoutDetalle.append( " CONVERT(decimal(17, 2),ROUND( mimportemasiva - (  mImporteMasIva /1.16 ), 2 ))  IVADES_45,  " ); // AN
		queryLayoutDetalle.append( " 0 ANTICIPO_46,				" ); // AO
		queryLayoutDetalle.append( " 0 IVAANT_47,  				" ); // AP
		queryLayoutDetalle.append( " ''  id_ctr_intdet			" ); // AQ
		queryLayoutDetalle.append( " FROM   tPagoDirectoDetalle D WITH (NOLOCK)  " );
