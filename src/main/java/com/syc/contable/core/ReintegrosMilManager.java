...
public class ReintegrosMilManager {
    ...
    private static final Logger log = LoggerFactory.getLogger(ReintegrosMilManager.class);
    ...
    public static void autorizaReintegro(Connection conn, Caso c, String nNumSicop, String cRecMotivSicop, String nNumMAP, String cRecMotivMAP, String ulogin, String prefixPath, String fAcredit, String EjercicioFiscal) throws SQLException, GestionException {
        ...
        try {
            ...
            if ("N".equals(existe)) {
                pstmnt = conn.prepareStatement("INSERT INTO tReintegroAutEncabezadoMil (nFolioReintegroMilaut, fSolicitud, cRamo, cUnidadResponsable, cDocumentoHaplicado, aEjercicioFiscal, observaciones, concepto, u_login, " + "cUnidadResponsableContable,nFolioTramiteSicop, cTipoPoliza, fAplicacion,cdescripcionpoliza, formaDePago, clvRastreo,fichaDeposito, clvBanco,cuentaBancaria,lc,importeLC,clcSicop,folioDependencia,mvto,aviso,tipoAviso,causaAviso) " + "SELECT nFolioReintegroMil, fSolicitud, cRamo,cUnidadResponsable, 'N', aEjercicioFiscal, observaciones, concepto, " + "u_login, 'RHQ',nFolioTramiteSicop, 'EG', '" + fAcredit + "','AUTORIZACION DE REINTEGRO PRESUPUESTAL FOLIO ' + CAST(nFolioReintegroMil as varchar),formaDePago, clvRastreo,fichaDeposito, clvBanco,cuentaBancaria,lc,importeLC,clcSicop,folioDependencia,mvto,aviso,tipoAviso,causaAviso FROM tReintegroEncabezadoMil WITH (NOLOCK) WHERE nFolioReintegroMil = ?");
                ...
                if ("2015".equals(EjercicioFiscal))
                    ...
                    tipoRein = "RC_EG";
                pstmnt = conn.prepareStatement("INSERT INTO treintegroautdetallemil  " + "            (nfolioreintegromilaut,  " + "             ndocrenglon,  " + "             noclc,  " + "             secclc,  " + "             ep,  " + "             cmes,  " + "             cevento,  " + "             mimporte,  " + "             mimportenegativo,  " + "             ccentrocontable,  " + "             ncapitulo,  " + "             cxp,  " + "             rfc,  " + "             nfoliodependencia,  " + "             alm,  " + "             nrenglonpagado,  " + "             tipoconcepto,  " + "             tipomovimiento,  " + "             cxpnomina," + "             OBGT," + "             CTAB," + "             mPasivoDiferido" + ")  " + "SELECT rd.nfolioreintegromil,  " + "       rd.ndocrenglon,  " + "       rd.noclc,  " + "       rd.secclc,  " + "       rd.ep,  " + "       rd.cmes,  " + "		 ? + SUBSTRING(cEvento, 6, len(cEvento)), " + "       rd.mimporte,  " + "       rd.mimportenegativo,  " + "       rd.ccentrocontable,  " + "       rd.ncapitulo,  " + "       rd.cxp,  " + "       rd.rfc,  " + "       rd.nfoliodependencia,  " + "       (SELECT TOP(1) alm  " + "        FROM   tpagadodetalle WITH(nolock),  " + "               tpagadoencabezado WITH(nolock)  " + "        WHERE  ep = rd.ep  " + "               AND nfoliosiaff = rd.noclc  " + "               AND tpagadodetalle.nfoliopagado = tpagadoencabezado.nfoliopagado  " + "               AND tpagadoencabezado.canocontrarrecibo = rd.cxp) AS ALM,  " + "       rd.nrenglonpagado,  " + "       rd.tipoconcepto,  " + "       rd.tipomovimiento,  " + "       rd.cxpnomina, " + "       rd.OBGT, " + "       rd.CTAB, " + "       rd.mPasivoDiferido " + "FROM   treintegrodetallemil rd WITH (nolock)  " + "WHERE  rd.nfolioreintegromil = ?");
    ...
    public static double getRemanente(Connection conn, String ep, String cxp, int nRenglon, String cxpnomina) throws SQLException {
        ...
        ResultSet rs = null;
        PreparedStatement pstm = null;
        String queryRemanente = "SELECT CASE tReintegros+tRectificacion " + " WHEN 0 THEN mImporteNeto " + " ELSE " + " CASE WHEN tReintegros!=0 AND tRectificacion=0 THEN mImporteNeto-impReint ELSE" + " impRect-impReint" + " END " + " END as Remanente" + " FROM (SELECT pd.mImporteNeto," + " (select COUNT(*) as sumaRect FROM tRectificacionEncabezadoMil re with (nolock), tRectificacionDetalle rd with (nolock) WHERE rd.EP=? AND re.cDocumentoHaplicado='S' AND re.nFolioRectificacionMil=rd.nFolioRectificacion AND re.caNoContrarreciboMil=? AND rd.EP=pd.EP and rd.nDocRenglon=? and re.caNoContrarrecibo=?) as tRectificacion," + " (select COUNT(*) as sumaReint FROM tReintegroEncabezadoMil ree with (nolock), tReintegroDetalleMil red with (nolock) WHERE red.EP=?  AND ree.nFolioReintegroMil=red.nFolioReintegroMil AND ree.cDocumentoHaplicado='S' AND red.cxp=? AND red.EP=pd.EP and red.nRenglonPagado=? and red.cxpnomina=?) as tReintegros," + " ISNULL((select ISNULL(SUM(rd.mImporte),0) as impR from tReintegroEncabezadoMil re WITH(NOLOCK),tReintegroDetalleMil rd WITH(NOLOCK) where re.nFolioReintegroMil =rd.nFolioReintegroMil AND re.cDocumentoHaplicado='S' and rd.cxp=? and rd.EP=? and rd.nRenglonPagado=? and rd.cxpnomina=?),0) as impReint," + " ISNULL((select ISNULL(SUM(case substring(cEvento,1,4)  when 'DICE' then  mImporte " + " WHEN 'DEBE' then mImporteNegativo end ),0) as imp " + " FROM tRectificacionDetalleMil r with (nolock), tRectificacionEncabezadoMil h with (nolock)" + " WHERE r.nFolioRectificacionMil = h.nFolioRectificacionMil  " + " AND h.caNoContrarreciboMil = ? and h.caNoContrarrecibo=? and r.nDocRenglon  = ? AND h.cDocumentoHaplicado='S' " + " AND ep not in (select  ep from tRectificacionDetalle d with (nolock) WHERE d.nDocRenglon = r.nDocRenglon " + " AND d.cEvento like 'DICE%' and d.ep= ?) " + " GROUP BY r.nDocRenglon),0) as impRect " + " FROM tPagadoEncabezado pe with (nolock), tPagadoDetalle pd with (nolock), tNominaCLCDetalle nd with (nolock), tNominaCLCEncabezado ne with (nolock)" + " WHERE pe.nFolioPagado=pd.nFolioPagado and nd.nFolioNOMINACLC=ne.nFolioNOMINACLC and pe.caNoContrarrecibo=? and pd.EP=? and pd.nDocRenglon=? and pe.caNoContrarrecibo=ne.caNoContrarreciboCLC and pd.EP=nd.EP and pd.ID_TIPO_CONCEPTO=nd.ID_TIPO_CONCEPTO and pd.ID_TIPO_MOVIMIENTO=nd.ID_TIPO_MOVIMIENTO and ne.caNoContrarreciboCLC=? and nd.EP=? and nd.nDocRenglonCLC=pd.nDocRenglon) tNueva " + " WHERE 1=1";
        //cxpnomina 10CP
        //cxp 10NC
    ...
    public static ArrayList<ReintegroDetalle> getReintegroLayout(Connection conn, int folio) throws SQLException {
        ...
        String //" and d.EP =  e.EP and d.cxp = P.caNoContrarrecibo and p.nFolioPagado = dp.nFolioPagado and dp.RFC = b.sCodigoEntidad  and rclc.EP=d.EP and rclc.EP=e.EP and rclc.caNoContrarrecibo=d.cxp and rclc.caNoContrarrecibo=p.caNoContrarrecibo and rclc.SEC=d.secCLC and d.nFolioReintegro=? and r.nFolioReintegro=?");
    ...
    public static void insertaReintegro(Connection conn, ReintegroEncabezadoMil reinE, ArrayList<ReintegroDetalleMil> reinDetalles, int folio, String folioCompleto, Usuario usuario, Caso c) throws Exception {
        ...
        try {
            ...
            //cRamo
            psInsertEncabezado.setString(3, reinE.getcRamo());
            //cUnidadResponsable
            psInsertEncabezado.setString(4, reinE.getcUnidadResponsable());
            psInsertEncabezado.setString(5, reinE.getcDocumentoHaplicado());
            //aEjercicioFiscal (2012,2013...)
            psInsertEncabezado.setString(6, reinE.getaEjercicioFiscal());
            //observaciones
            psInsertEncabezado.setString(7, reinE.getObservaciones());
            //concepto
            psInsertEncabezado.setString(8, reinE.getConcepto());
            psInsertEncabezado.setString(9, usuario.getLogin());
            //forma de pago (efectivo, transferencia) [catalogo]
            psInsertEncabezado.setString(10, reinE.getFormaDePago());
            //clave de rastreo
            psInsertEncabezado.setString(11, reinE.getClvRastreo());
            //ficha de deposito en el banco
            psInsertEncabezado.setString(12, reinE.getFichaDeposito());
            //clave del pago en el banco
            psInsertEncabezado.setString(13, reinE.getClvBanco());
            //cuenta bancaria
            psInsertEncabezado.setString(14, reinE.getCuentaBancaria());
            //linea de captura clave
            psInsertEncabezado.setString(15, reinE.getLc());
            //importe de la linea de captura
            psInsertEncabezado.setDouble(16, Double.parseDouble(reinE.getImporteLC()));
            //cTipoPoliza
            psInsertEncabezado.setString(17, reinE.getcTipoPoliza());
    ...
    public static String getCTAB(Connection conn, String caNoContrarrecibo, String ep) throws SQLException {
        ...
        try {
            //String queryRFC="SELECT top(1) ed.RFC FROM tEjercidoDetalle ed with(nolock), tEjercidoEncabezado ee with(NOLOCK) WHERE ee.caNoContrarrecibo = ? AND ee.nFolioEjercido = ed.nFolioEjercido and ed.EP=?";
            String queryRFC = "SELECT TOP (1) pd.ctab " + " FROM dbo.tNOMINACLCEncabezado AS nclc WITH (NOLOCK) " + " JOIN dbo.tNOMINACLCDetalle AS n WITH (NOLOCK) ON nclc.nFolioNOMINACLC = N.nFolioNOMINACLC " + " JOIN dbo.tPagadoEncabezado AS p WITH (NOLOCK) ON nclc.caNoContrarreciboCLC = P.caNoContrarrecibo " + " JOIN dbo.tPagadoDetalle AS pd WITH (NOLOCK) ON p.nFolioPAGO = pd.nFolioPAGO " + " WHERE n.caNoContrarrecibo = ? AND pd.EP = ? ";
            pstmnt = conn.prepareStatement(queryRFC);
            pstmnt.setString(1, caNoContrarrecibo);
            pstmnt.setString(2, ep);
            rs = pstmnt.executeQuery();
    ...
    public static String eventoReintegroMil(Connection conn, String caNoContrarrecibo, String EP) {
        ...
        String evento = "";
        try {
            ps = conn.prepareStatement("SELECT  TOP 1 'RC_IN_' + SUBSTRING(cEvento, 3, LEN(cEvento)) " + "	FROM    dbo.tPagadoDetalle " + "	WHERE   nFolioPagado = ( SELECT nFolioPagado " + "	                         FROM   dbo.tPagadoEncabezado " + "	                         WHERE  caNoContrarrecibo = ? )" + "	    AND EP = ? " + "	    AND mPasivoDiferido = 0");
