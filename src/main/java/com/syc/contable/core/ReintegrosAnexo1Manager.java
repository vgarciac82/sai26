...
public class ReintegrosAnexo1Manager {
    ...
    public static double getRemanente(Connection conn, String ep, String cxp, int nRenglon) throws SQLException {
        ...
        "                FROM   tReintegroAnexo1Encabezado ree WITH (nolock), " + "                       tReintegroAnexo1Detalle red WITH (nolock) " + "                WHERE  red.ep = ? " + // 5
        "                       AND ree.cdocumentohaplicado = 'S' " + "                       AND ree.nfolioreintegro = red.nfolioreintegro " + // 6
        "                       AND red.cxp = ? " + "                       AND red.ep = pd.ep " + "                       AND red.nrenglonpagado = ?)           AS tReintegros, " + "               Isnull((SELECT Isnull(Sum(rd.mimporte), 0) AS impR " + "                       FROM   tReintegroAnexo1Encabezado re WITH(nolock), " + "                              tReintegroAnexo1Detalle rd WITH(nolock) " + // 7
    ...
    public static ArrayList<ReintegroDetalle> getReintegroLayout(Connection conn, int folio) throws SQLException {
        ...
        "					  CASE LEN(CGEO_164) WHEN 1 THEN '0'+CGEO_164 WHEN 2 THEN CGEO_164 END+'.'+ " + "					  CASE CPPI_166 WHEN '0' THEN '00000000000' ELSE CPPI_166 END " + "	WHERE nFolioReintegroAnexo1 = ?";
        ReintegroDetalle rd;
