...
public class ReintegrosAnexo1Manager {
    ...
    public static double getRemanente(Connection conn, String ep, String cxp, int nRenglon) throws SQLException {
        ...
        "               (SELECT Count(*) AS sumaReint " + "                FROM   tReintegroAnexo1Encabezado ree WITH (nolock), " + "                       tReintegroAnexo1Detalle red WITH (nolock) " + // 5
        "                WHERE  red.ep = ? " + "                       AND ree.cdocumentohaplicado = 'S' " + // 6
        "                       AND ree.nfolioreintegro = red.nfolioreintegro " + "                       AND red.cxp = ? " + "                       AND red.ep = pd.ep " + "                       AND red.nrenglonpagado = ?)           AS tReintegros, " + "               Isnull((SELECT Isnull(Sum(rd.mimporte), 0) AS impR " + "                       FROM   tReintegroAnexo1Encabezado re WITH(nolock), " + // 7
        "                              tReintegroAnexo1Detalle rd WITH(nolock) " + // 8
