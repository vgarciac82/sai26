...
public class OperacionAjenaManager {
	...
	public static final Logger log = Logger.getLogger( OperacionAjenaManager.class );
	...
	public static ArrayList<String> buscaAjenasIntegradas( Connection conn, String cxpAI, String fecha, String cBancaria, String Leyenda, String nFolio, String cBEN, int tipo ) throws Exception {
		...
		try {
				...
				sql.append("SELECT DISTINCT ")
				   .append("tCE.nFolioOperAjenasInt, 'H' AS Header, ")
				   .append("CONVERT(nvarchar(10), tCE.fCaptura, 103) FECHA_EXP, ")
				   .append("CONVERT(nvarchar(10), tCE.fCaptura, 103) FECHA_APL, ")
				   .append("tCE.cRamo, tCE.cRamo, tCE.cRamo, ")
				   .append("'RHQ' ID_UNIDAD, 'RHQ' ID_UNIDAD_CR, 'RHQ' ID_UNIDAD_REC, ")
				   .append("CASE WHEN opa.cIdTipoDocumento = 'L' THEN 'I' ELSE opa.cIdTipoDocumento END AS TIPO_MOVTO, ")
				   .append("'1' AS OrigenPpto, 'MXN' TipoMoneda, '1.00' TipoCambio, ")
				   .append("CASE WHEN opa.cIdTipodocumento = 'O' THEN '1' ELSE '2' END TIPO_PAGO, ")
				   .append("'").append(Leyenda).append("' CveLeyenda, ")
				   .append("'").append(cBEN).append("' CBEN, ")
				   .append("'").append(cBancaria).append("' CUENTA_BANCARIA, ")
				   .append("RTRIM(tCE.cIdRFC) RFC, 'OTR', '' FechaReferencia, ")
				   .append("LEFT(opa.cConcepto, 40) Referencia1, '' Referencia2, ")
				   .append("LEFT(opa.cConcepto, 40), '' AMF, ")
				   .append("RTRIM(tCE.caNoContrarreciboInt) NO_ACMI, ")
				   .append("RTRIM(tCE.caNoContrarreciboInt) AuxiliarComodin, ")
				   .append("'' CTR, '' FolioDC ")
				   .append("FROM tOperAjenasIntegradoEnc tCE (NOLOCK) ")
				   .append("LEFT JOIN tBeneficiario B (NOLOCK) ON tCE.cIdRFC = B.dRFC ")
				   .append("INNER JOIN tBeneficiarioCuentasBancarias BCB (NOLOCK) ON tCE.cIdRFC = BCB.dRFC ")
				   .append("INNER JOIN tGrupoOpAjenas opa (NOLOCK) ON opa.nIdGrupoOpAjena = tCE.cBeneficiario ")
				   .append("WHERE tCE.nFolioOperAjenasInt = ").append(nFolio);
				...
				if ( rs.next() ) {
				    ...
				    joiner.add(rs.getString(2))
				          .add(rs.getString(3))
				          .add(trim(rs.getString(4)))
				          .add(trim(rs.getString(5)))
				          .add(trim(rs.getString(6)))
				          .add(trim(rs.getString(7)))
				          .add(trim(rs.getString(8)))
				          .add(trim(rs.getString(9)))
				          .add(trim(rs.getString(10)))
				          .add(trim(rs.getString(11)))
				          .add(trim(rs.getString(12)))
				          .add(trim(rs.getString(13)))
				          .add(trim(rs.getString(14)))
				          .add(trim(rs.getString(15)))
				          .add(Leyenda)
				          .add(trim(rs.getString(17)))
				          .add(cBancaria)
				          .add(trim(rs.getString(19)))
				          .add(trim(rs.getString(20)))
				          .add(trim(rs.getString(21)))
				          .add(vreferencia)
				          .add(trim(rs.getString(23)))
				          .add(vreferencia)
				          .add(trim(rs.getString(25)))
				          .add(trim(rs.getString(26)))
				          .add(trim(rs.getString(27)))
				          .add(trim(rs.getString(28)))
				          .add(trim(rs.getString(29)));
	...
	public static boolean esPagoFonden( Connection conn, int nFolioPago ) throws Exception {
		...
		queryIntegraAjenas.append( "SELECT ajenasDetalle.cTipoDoc,  ajenasDetalle.nFolioDoc, ajenasDetalle.caNoContrarrecibo 	" )
				.append(  " FROM   toperajenasencabezado ajenasEncabezado WITH(nolock) ")
				.append(  " INNER JOIN  toperajenasdetalle ajenasDetalle WITH(nolock) ")
				.append(  " 	ON ajenasEncabezado.nFolioOperAjenas = ajenasDetalle.nFolioOperAjenas  	")
