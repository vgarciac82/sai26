	...
	public static String CancelacionMasiva(Connection conn,String usuario,HttpServletRequest request){
		...
		try {																		
			pstmntUp = conn.prepareStatement("" + 
					"SELECT " + 
						"CGCO.ID_CASO id_caso,CGCO.ID_CASO_OPER id_caso_oper,TPA.nFolioPoliza " +  
					"FROM " + 
						"CG_CASO_OPERACION CGCO " +
						"INNER JOIN " +
						"CG_CASO CGC " +
							"ON CGCO.ID_CASO = CGC.ID_CASO AND CGCO.ID_TC = CGC.ID_TC " + 
						"INNER JOIN " +
						"tPAnticipadoEncanbezado TPA " +
							"ON CGC.C_FOLIO = TPA.cFolioSai " + 
					"WHERE " + 
						"CGCO.ID_TC = 29 AND " +
						"CGCO.ID_OPER IN (2,3) AND " +
						"EXISTS(SELECT 1 FROM tPAnticipadoDetalle A WHERE A.cMes <= DATEPART(MONTH,GETDATE()) AND A.nFolioPagoAnticipado = TPA.nFolioPagoAnticipado) " +
					"");
