...
public class RetencionManager {
	 ...
	 */
	static String				ur;
	private static final Logger	log	= Logger.getLogger( RetencionManager.class );
	...
	public static Retencion filtraFolioIP( Connection conn, String folioSAI ) throws Exception {
		Retencion retencion = null;
		String SqlH = "SELECT '' nCompromisoSICOP, pagado.cTipoPago tipoPago, pagado.cDescripcionPoliza cConcepto, YEAR(pagado.faplicacion) aEjercicioFiscal " 
				+ ", CASE WHEN pagado.cTipoPago = 'RELACIONGASTOS' THEN (SELECT cIdrfc FROM tRELACIONGASTOSEncabezado (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo)  " 
				+ "		WHEN pagado.cTipoPago = 'PAGODIRECTO' THEN (SELECT cIdrfc  FROM TPAGODIRECTOENCABEZADO (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo)  " 
				+ "		WHEN pagado.cTipoPago = 'PAGODIVERSO' THEN (SELECT RFC FROM tpagodiversoencabezado (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo)  " 
				+ "		WHEN pagado.cTipoPago = 'FEDERALIZADO' THEN (SELECT rfc FROM tpagofederalizadoEncabezado (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo)  END cIdRFC " 
				+ ", CASE WHEN pagado.cTipoPago = 'RELACIONGASTOS' THEN (SELECT cNombre FROM tRELACIONGASTOSEncabezado (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo)  " 
				+ "		WHEN pagado.cTipoPago = 'PAGODIRECTO' THEN (SELECT (select dNombre + ' ' + ISNULL(dapellidoPaterno,'') + ' ' + ISNULL(dApellidoMaterno,'') from tBeneficiario(nolock) where dRFC =TPAGODIRECTOENCABEZADO.cIdRfc)  FROM TPAGODIRECTOENCABEZADO (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo)  " + "		WHEN pagado.cTipoPago = 'PAGODIVERSO' THEN (SELECT Nombre FROM tpagodiversoencabezado (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo)  " 
				+ "		WHEN pagado.cTipoPago = 'FEDERALIZADO' THEN (SELECT Nombre FROM tpagofederalizadoEncabezado (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo) END NOMBRE " 
				+ ", CASE WHEN pagado.cTipoPago = 'RELACIONGASTOS' THEN (SELECT (SELECT TOP 1 sCuenta_bancaria FROM tLayoutsCreadosRelacionGastosHeader (nolock) where sNocontrarrecibo = tRELACIONGASTOSEncabezado.canocontrarrecibo) FROM tRELACIONGASTOSEncabezado (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo)  " 
				+ "		WHEN pagado.cTipoPago = 'PAGODIRECTO' THEN (SELECT CTAB  FROM TPAGODIRECTOENCABEZADO (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo)  " 
				+ "		WHEN pagado.cTipoPago = 'PAGODIVERSO' THEN (SELECT CTAB FROM tpagodiversoencabezado (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo)  " 
				+ "		WHEN pagado.cTipoPago = 'FEDERALIZADO' THEN (SELECT CTAB FROM tpagofederalizadoEncabezado (NOLOCK) WHERE caNoContrarrecibo = pagado.caNoContrarrecibo)  END CTAB, caNoContrarrecibo, nFolioPago " 
				+ " FROM tPagadoEncabezado pagado (nolock) " + " INNER JOIN tPagadoDetalle det (nolock)  " 
				+ "    ON pagado.cTipoPago = det.ctipopago and pagado.nFolioPAGO = det.nfoliopago " 
				+ " WHERE pagado.cDocumentoHaplicado = 'S' AND PAGADO.cTipoPago <> 'AJENAS' " 
				+ " 	AND SUBSTRING(det.ep,40,1) = 4 " + " 	AND caNoContrarrecibo = ?";
	...
	public static void insertarEncabezado( Connection conn, RetencionEncabezado encabezado ) throws Exception {
		...
		String insertQuery = "INSERT INTO tRetencionEncabezado (nFolioRetencion, nIdCaso, cUnidadResponsable, fCarga, fAplicacion, cRamo, nOrigenPPTO, aEjercicioFiscal, cCentroContable " 
		+ ", cIdRFC, CTAB, NOMBRE, cConcepto, mImporteRetencion, cTipoPago, caNoContrarrecibo, cIdUsuarioCaptura, nEnviadoSICOP, nCompromisoSICOP, cUnidadResponsableContable, nFolioPoliza, cTipoPoliza, cEsIP, nFolioPago, cDescripcionPoliza) " 
		+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public static void insertarDetalle( Connection conn, List<RetencionDetalle> retenciones ) throws Exception {
		...
		try {
			for ( int i = 0; i < retenciones.size(); i++ ) {
				...
				String insertQuery = "INSERT INTO tRetencionDetalle ( nFolioRetencion, nDocRenglon, cMes, cEvento, ep, mImporte, m2Millar, mObra5, mImporteFlete4, mISRHonorarios, mISRArrenda, mRetImpuestoCedular, mImporteIvaArrenda, mImporteIvaHonorarios, mImporteISRLaudos, mISROtros, mImporteIva6" 
							+ "	,RFC, OBGT, cUnidadResponsable, cPasivo, mPasivoDiferido, cEjercicio, cCentroContable	) " 
							+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? ,?,?,?,?,?, ?,?)";
				pstmnt = conn.prepareStatement( insertQuery );
	...
	public static Retencion queryPagado( Connection conn, String folio, String SqlH, String SqlD, String CXP ) throws URInaccesibleException, CLCNoPagadaException, Exception {
		...
		// Realizar conexion
		log.debug( "[SQLH]" + SqlH + "[CxP=" + folio + "]" );
		log.debug( "[SQLD]" + SqlD + "[CxP=" + folio + "]" );
		try {
	...
	public static ArrayList<String> generaCompromisoSICOP( Connection conn, int folio ) throws Exception {
		...
		try {
			...
			String Sql2 = "	SELECT '668' as ID_EVENTO"
					+ "			 , '304_TOCN' as EVENTO"
					+ "			 , tCEP.cRamo"
					...
					+ "			 , tCEP.cFuncion"
					+ "			 , tCEP.cSubFuncion"
					+ "			 , SUBSTRING( dbo.CambiaEPPlurianual(tCD.EP), 20, 2) AS cProgramaGeneral"
					...
					+ "			 , SUBSTRING( dbo.CambiaEPCarteraMeta(tCEP.EP),45, 11) AS cCartera"
					+ "			 , '000000' + '0000' as CAU "
					+ "			 , '00' as COP"
					+ "			 , '000' as PL"
					+ "			 , '000' as OF_"
					...
					+ "			 , '' as Suficiencia"
					+ "			 , '' as Sol_OLI"
					+ "			 , SUM(CASE tCD.cMes when 1 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Enero"
					+ "			 , SUM(CASE tCD.cMes when 2 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Febrero"
					+ "			 , SUM(CASE tCD.cMes when 3 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Marzo"
					+ "			 , SUM(CASE tCD.cMes when 4 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Abril"
					+ "			 , SUM(CASE tCD.cMes when 5 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Mayo"
					+ "			 , SUM(CASE tCD.cMes when 6 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Junio"
					+ "			 , SUM(CASE tCD.cMes when 7 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Julio"
					+ "			 , SUM(CASE tCD.cMes when 8 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Agosto"
					+ "			 , SUM(CASE tCD.cMes when 9 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Septiembre"
					+ "			 , SUM(CASE tCD.cMes when 10 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Octubre"
					+ "			 , SUM(CASE tCD.cMes when 11 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Noviembre"
					+ "			 , SUM(CASE tCD.cMes when 12 then convert(decimal(14,2), ABS(mImporte)) ELSE 0 END) as Diciembre"
					+ "			 , SUM( convert(decimal(14,2), abs(tCD.mImporte)) ) as Importe"
					+ "			 	FROM tRetencionDetalle tCD (NOLOCK),tCatalogoEP tCEP (NOLOCK), tRetencionEncabezado tCE (NOLOCK)"
					+ "			 	WHERE tCD.nFolioRetencion = ?"
					+ "			 	AND tCD.EP = tCEP.EP"
					+ "			 	AND tCE.nFolioRetencion = tCD.nFolioRetencion "
					+ "			 	group by tCEP.cRamo,  tCEP.aEjercicioFiscal, tCEP.cUnidadResponsableEP, tCEP.cGrupoFuncional, tCEP.cFuncion, tCEP.cSubFuncion, "
					+ "			  SUBSTRING( dbo.CambiaEPPlurianual(tCD.EP), 20, 2), tCEP.cActividadInstitucional, SUBSTRING( dbo.CambiaEPCarteraMeta(tCEP.EP),45, 11),"
					+ "			  tCEP.cProgramaPresupuestario, tCEP.cPartida, tCEP.cTipoGasto, tCEP.cFuenteFinanciamiento, tCEP.cEntidadFederativa, tCEP.cCartera";
	...
	public static String buscaCompromiso (Connection conn, int folio) throws Exception {
		...
		try {
			...
			log.debug( "El folio de sicop es " + folioSicop);
