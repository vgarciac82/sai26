...
public class RectificacionAnexo1Manager {
	 ...
	 */
	static String				ur;
	private static final Logger	log	= Logger.getLogger(RectificacionAnexo1Manager.class);
	...
	public static boolean insertarEncabezado(Connection conn, int folio, String nIDCaso, String cEjercicio, String cRamo, String cUnidad, String cCentroContable, java.sql.Date fExp, java.sql.Date fApl, String conceptoRectificacion, String cTipoMovto, String nOrigenPPTO, String nMes,
		String oficioRectif, String ctr_int, String cTipoRectificacion, String nFolioSICOP, String caNoContrarrecibo, String cTipoPoliza, String cDescripcionPoliza, String u_login, String nFolioPoliza, String cUnidadResponsableContable, String nFolioSIAFF, String totalDebe, String totalDice)
		throws SQLException {
		...
		String insertQuery = "INSERT INTO tRectificaAnexo1Encabezado(nFolioRectificaAnexo1,nIDCaso,aEjercicioFiscal,cRamo,cUnidadResponsable" 
			+ ",cCentroContable,fExp,fAplicacion,cConceptoRectificacion,cTipoMovto,nOrigenPPTO"
			+ "	 ,nMes,oficioRectif,ctr_int,cTipoRectificacion,nFolioSICOP,caNoContrarrecibo" 
			+ " ,nFolioPoliza,cTipoPoliza,cDescripcionPoliza,cU_LoginCaptura" 
			+ "	,cUnidadResponsableContable,nFolioSIAFF,totalDebe,totalDice) " 
			+ "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public static boolean insertarDetalle(Connection conn, ArrayList<RectificacionAnexo1Detalle> rectificaciones, String caNoContrarrecibo) throws SQLException {
		...
		try {
			...
			for (int i = 0; i < rectificaciones.size(); i++) {
				...
				String insertQuery = "INSERT INTO tRectificaAnexo1Detalle( " +
									 "		nFolioRectificaAnexo1," +
									 "		nDocRenglon," +
									 "		cEvento," +
									 "		EP," +
									 "		mImporte," +
									 ...
									 "		?," +
									 "		?," +
									 "		(SELECT TOP 1 sCUENTA_BANCARIA FROM tLayoutsCreadosAnexo1Header WITH(NOLOCK) WHERE cEstatus='ACTIVO' AND sAuxiliarComodin='" + caNoContrarrecibo.trim() + "' ORDER BY ID DESC)," +
									 " 		SUBSTRING(?,32,5)," +
									 "		?" +
									 ")";
				pstmnt = conn.prepareStatement(insertQuery);
	...
	public static void autorizaRectificacion(Connection conn, Caso c, String ulogin, String prefixPath, String tipoAplicacion, String fApl) throws SQLException, GestionException {
		...
		try {
			...
			if (nExiste == 0) {
				pstmnt = conn
					.prepareStatement("INSERT INTO tRectificacionAutEncabezado (nFolioRectificacionAut, nIdCaso, aEjercicioFiscal, cRamo, cUnidadResponsable, cCentroContable, fExp, fAplicacion, cConceptoRectificacion, cTipoMovto, "
						...
						+ "'"
						+ fApl
						+ "', cConceptoRectificacion, cTipoMovto, "
				...
				pstmnt = conn.prepareStatement("INSERT INTO tRectificacionAutDetalle(nFolioRectificacionAut,nDocRenglon,EP,cEvento,mImporte, mImporteNegativo, cMes, cCentroContable, OBGT) " + "SELECT rd.nFolioRectificacion,rd.nDocRenglon,rd.EP, CASE '" + tipoAplicacion
					+ "' WHEN 'Rectificacion' THEN CASE (SUBSTRING(cEvento,1,4)) WHEN 'DICE' THEN 'DICE_' + cNoEvento WHEN 'DEBE' THEN 'DEBE_DECIR_' + cNoEvento END "
					+ " WHEN 'Reclasificacion' THEN CASE (SUBSTRING(cEvento,1,4)) WHEN 'DICE' THEN 'DICE_AUT_DI' WHEN 'DEBE' THEN 'DEBE_DECIR_A' END END as cEvento,rd.mImporte,rd.mImporteNegativo,rd.cMes, rd.cCentroContable, SUBSTRING(EP,32,5) AS obgt "
					+ " FROM tRectificacionDetalle rd WITH (NOLOCK) JOIN tEventoRectificacion WITH (NOLOCK) ON SUBSTRING(EP,32,5) = cPartida WHERE rd.nFolioRectificacion = ?");
				pstmnt.setInt(1, nIdCaso);
				pstmnt.execute();
	...
	public static RectificacionAnexo1Encabezado getRectificacionEncabezadoSicop(Connection conn, int folio) throws SQLException {
		...
		PreparedStatement pstm = null;
		RectificacionAnexo1Encabezado re = null;
		String query = "SELECT  'H' AS posicion1 , " +
					   "	r.fAplicacion , " +
					   "	r.fExp , " +
					   "	r.cRamo , " +
					   "	r.cRamo , " +
					   "	r.cRamo , " +
					   "	r.cUnidadResponsable , " +
					   "	r.cUnidadResponsable , " +
					   "	r.cUnidadResponsable , " +
					   "	ISNULL(r.cTipoMovto, '') AS cTipoMovto , " +
					   "	ISNULL(r.nOrigenPPTO, '') AS nOrigenPPTO , " +
					   "	ISNULL(r.cConceptoRectificacion, '') AS cConceptoRectificacion , " +
					   "	ISNULL(R.ctr_int, '') AS ctr_int , " +
					   "	R.oficioRectif , " +
					   "	R.oficioRectif , " +
					   "	R.nFolioRectificaAnexo1 , " +
					   "	R.nIDCaso , " +
					   "	R.totalDICE , " +
					   "	R.totalDEBE " +
					   "FROM tRectificaAnexo1Encabezado r WITH ( NOLOCK ) " +
					   "WHERE nFolioRectificaAnexo1 = ? ";
	...
	public static String getRectificacionDetalleSicop(Connection conn, int folio) throws SQLException {
		...
		String query = "SELECT DISTINCT " +
					   "	ISNULL(r.nOrigenPPTO, 0) AS nOrigenPPTO , " +
					   "	'0' , " +
					   "	r.nFolioSICOP , " +
					   "	d.nDocRenglon , " +
					   "	e.cRamoEP , " +
					   ...
					   "	e.cCartera , " +
					   "	'0000000000' AS cUnidadEjecutora , " +
					   "	'00' AS cUnidadNormativa , " +
					   "	'000' , " +
					   "	'000' , " +
					   "	'00000' , " +
					   "	'00000' , " +
					   "	'0000000000' , " +
					   "	CONVERT(VARCHAR, d.mImporte) , " +
					   "	d.cMes , " +
					   "	'' AS cFillRellen1 , " +
					   "	'S04929' AS CBEN, " + 
					   "	'' AS cFillRellen2 , " +
					   "	'0' AS sol_oli , " +
					   "	CASE WHEN cPartida = '35801' THEN 'GD' " +
					   "	ELSE 'PN' " +
					   "	END ID_TIPO_CONCEPTO , " +
					   "	'000' AS tipo_concepto , " +
					   "	'0' AS concepto_mov , " +
					   "	'0' retencion_isr , " +
					   "	'0' , " +
					   "	CASE WHEN d.cEvento LIKE '%DICE%' THEN 'A' " +
					   "	ELSE 'C' " +
					   "	END , " +
					   "	ISNULL(R.ctr_int, '') AS ctr_int " +
					   "FROM tRectificaAnexo1Detalle d WITH ( NOLOCK ) , " +
					   "	tRectificaAnexo1Encabezado r WITH ( NOLOCK ) , " +
					   "	tCatalogoEP e WITH ( NOLOCK ) , " +
					   "	tBeneficiario b WITH ( NOLOCK ) " +
					   "WHERE d.nFolioRectificaAnexo1 = r.nFolioRectificaAnexo1 " +
					   "	AND d.EP = e.EP " +
					   "	AND b.dRFC = 'TESOFE' " +
					   "	AND r.nFolioRectificaAnexo1 = ? " + 
					   "	AND d.nFolioRectificaAnexo1 = ? ";
