...
public class PagoProgFederalizadosManager {
	...
	public static ArrayList<String> BuscaCompromisos(Connection conn,String listaIds, String listaCuentaBancaria, String listaFechas, String listaLeyendas, String sUsuario)throws Exception{
		...
		try {
			...
			while (rs.next()){
				...
				String encabezado = rs.getString(2)+","+arrFechas[intIndice].trim()+","+rs.getString(4).trim()+","+rs.getString(5).trim()+","+
								rs.getString(6).trim()+","+rs.getString(7).trim()+","+rs.getString(8).trim()+","+rs.getString(9).trim()+","+rs.getString(10).trim()+","+
								rs.getString(11).trim()+","+rs.getString(12).trim()+","+rs.getString(13).trim()+","+rs.getString(14).trim()+","+rs.getString(15).trim()+","+
								rs.getString(16).trim()+","+vcLeyenda+","+vcBenef+","+vcCtaBancaria+","+vRFC+","+
								rs.getString(21).trim()+","+rs.getString(22).trim()+","+rs.getString(23).trim()+","+rs.getString(24).trim()+","+rs.getString(25).trim().replaceAll("[\r\n]{2,}", " ")+","+
								rs.getString(26).trim()+","+rs.getString(27).trim()+","+rs.getString(28).trim()+","+rs.getString(29).trim()+","+rs.getString(30).trim()+","+
								rs.getString(31).trim()+","+rs.getString(32).trim()+","+rs.getString(33).trim()+","+rs.getString(34).trim()+","+rs.getString(35).trim()+","+
								rs.getString(36).trim()+","+rs.getString(37).trim()+","+rs.getString(38).trim()+","+rs.getString(39).trim()+","+rs.getString(40).trim()+","+
								rs.getString(41);
				...
				while (rs2.next()){
					...
					for (int i = 1; i < 44; i++) { //URVP.27012015 se cambia la condicion de <42 a 38 debido a las 5 columnas que se eliminaron
						if (i == 28) {
		...
		try {
				...
				while (rs.next()) {
					...
					String encabezado = rs.getString(2) 
						...
						+ "," + vcLeyenda + "," + vcBenef + "," + vcCtaBancaria + "," + vRFC 
	...
	public static ArrayList<String> CreaDocumentacionComprobatoria(Connection conn,String listaIds)throws Exception{
		...
		try {
			//PSC
			String Sql = " SELECT nFolioPAGOFEDERALIZADO, 'H' H, cRamo, 'RHQ', '' SOL_PAGO, '3', caNoContrarrecibo FOLIO_INTERNO, caNoContrarrecibo COMODIN " +
						 " FROM tPAGOFEDERALIZADOEncabezado (NOLOCK) " +
						 " WHERE nFolioPAGOFEDERALIZADO in (" + listaIds + ") " +
						 " ORDER BY nFolioPAGOFEDERALIZADO";
	...
	public static ResultSet generaDetalleCompromisoFed (Connection conn, String ids) throws Exception {
		...
		StringBuilder sql = new StringBuilder();
				sql.append(" SELECT DISTINCT  " );
/*A*/			sql.append(" '1' ID_EVENTO, " );
/*B*/			sql.append(" '24.0.001' EVENTO, " );
/*C*/			sql.append(" ltrim(TCEP.cRamo) ID_RAMO_ML,  " );
/*D*/			sql.append(" 'RHQ',  " );
/*E*/			sql.append(" TCEP.aEjercicioFiscal,  " );
/*F*/			sql.append(" TCEP.cGrupoFuncional,  " );
/*G*/			sql.append(" tCEP.cFuncion,  " );
/*H*/			sql.append(" tCEP.cSubFuncion,  " );
/*I*/			sql.append(" CASE WHEN tCEP.cProgramaGeneral IN ( SELECT cProgramaGeneral FROM tCat_ProGeneralPlurianual WITH (NOLOCK) ) THEN '00' ELSE tCEP.cProgramaGeneral END AS cProgramaGeneral, " );
/*J*/			sql.append(" tCEP.cActividadInstitucional,  " );
/*K*/			sql.append(" tCEP.cProgramaPresupuestario,  " );
/*L*/			sql.append(" ltrim(substring(cpartida,1,1)) CCAP_157,  " );
/*M*/			sql.append(" substring(cpartida,2,1) CCON_158,  " );
/*N*/			sql.append(" substring(cpartida,3,1) CPARG_300,  " );
/*O*/			sql.append(" substring(cpartida,4,2) CPAR_159,  " );
/*P*/			sql.append(" tCEP.cTipoGasto,  " );
/*Q*/			sql.append(" tCEP.cFuenteFinanciamiento,  " );
/*R*/			sql.append(" tCEP.cEntidadFederativa,  " );
/*S*/			sql.append(" SUBSTRING( dbo.CambiaEPCarteraMeta(TPDD.EP),45, 11) cCartera, " );
/*T*/			sql.append(" '0000000000', " );
/*U*/			sql.append(" '00', " );
/*V*/			sql.append(" '000' PL,  " );
/*W*/			sql.append(" '000' OFI,  " );
/*X*/			sql.append("  '00000' AUX1,  " );
/*Y*/			sql.append(" '00000' AUX2,  " );
/*Z*/			sql.append(" '0000000000' AUX3,  " );
/*AA*/			sql.append(" ISNULL( (SELECT TOP 1 nFolioAutSICOP FROM tCompromisoEncabezado WITH (NOLOCK) where cIdContrato IN (TPDE.cFolioContratoObra ) and nFolioSuficiencia is not null and nFolioSuficiencia !=1 ORDER BY nFolioCompromiso DESC ), '') folioCompromiso," ); 
/*AB*/			sql.append(" SUM(CONVERT(decimal(17, 2), TPDD.mImporteNeto)) Monto, " );
/*AC*/			sql.append(" MONTH(GETDATE()) MES_149,  " );
/*AD*/			sql.append(" ( SELECT top 1 nFolioSuficiencia FROM tCompromisoEncabezado WITH (NOLOCK) where cIdContrato IN (TPDE.cFolioContratoObra ) and nFolioSuficiencia is not null and nFolioSuficiencia !=1 ORDER BY nFolioCompromiso DESC ) folioSuficiencia, " );
/*AE*/			sql.append(" 'GD' TIPO_CONTRATO,  " );
/*AF*/			sql.append(    " '000' CONC_MOV,  " );
/*AG*/			sql.append(" CONVERT(decimal(17, 2), sum(TPDD.mISRHonorarios + TPDD.mISRArrenda + TPDD.mimporteISRResico)) DCD_ISR, " ); 
/*AH*/			sql.append(" CONVERT(decimal(17, 2), sum(TPDD.m23Iva + mImporteIvaHonorarios + mImporteIvaArrenda + TPDD.mImporteFlete23 + ISNULL(TPDD.mImporteFlete4, 0)+ ISNULL(TPDD.mImporteIva6, 0))) DCD_IVA, " );
/*AI*/			sql.append(" CONVERT(decimal(17, 2), sum(TPDD.mObra5)) DCD_MIL5, " );
/*AJ*/			sql.append(" ISNULL( CONVERT(decimal(17, 2), sum(TPDD.mCNIC + TPDD.mIMDT)) , 0)DCD_MIL2, " ); 
/*AK*/			sql.append(" ISNULL( CONVERT(decimal(17, 2), 0) , 0) DCD_CONTRIBUCION, " );
/*AL*/			sql.append(" ISNULL( CONVERT(decimal(17, 2), sum(ISNULL(TPDD.mRetImpuestoCedular, 0)))  , 0) AS  DCD_OTRAS_RET, " );
/*AM*/			sql.append(" ISNULL( CONVERT(decimal(17,2), sum(ISNULL(TPDD.mIva,0))) , 0), " );
/*AN*/			sql.append(" ISNULL( CONVERT(decimal(17,2), Sum(TPDD.mAmortizacionAnticipo)) , 0), " );
/*AO*/			sql.append(" ISNULL( CONVERT(decimal(17, 2), sum(TPDD.mPenalizacion + TPDD.mTesofe)) , 0) DCD_PENALIZACION, " );
/*AP*/			sql.append(" ISNULL( CONVERT(decimal(17,2), Sum( TPDD.mAmortizacionAnticipo - (TPDD.mAmortizacionAnticipo / ( 1 + (TPDE.mImporteIVA * .01))) )), 0 )  ANTIVA, " );
/*AQ*/			sql.append(" '' id_ctr_intdet " );
	...
	public static void validarTotalRetenciones(Connection con, BigDecimal total, String listaIds) throws Exception {
		...
		ResultSet rs = null;
		BigDecimal importe = new BigDecimal(0);
		String query = "SELECT SUM(mimporteISRResico + mImporteISRLaudos + mISROtros +mimporteIvaArrenda + mimporteivahonorarios + mimporteflete23 + mImporteFlete4 + mObra5) totalRetenciones "
					+ " FROM tPAGOFEDERALIZADODetalle (NOLOCK) WHERE nFolioPAGOFEDERALIZADO IN (" + listaIds + ")";
