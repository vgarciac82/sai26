	...
	public static StringBuffer BuscaCuentas(Connection conn, String BeneficiariosCuentasBancarias) throws SQLException{
		...
		try{
			...
			for(int i = 0; i < registros.length; i++){
				...
				String SqlCB = "SELECT estatusL, cRamo, CBEN, clabe, CBAN, sucursal FROM vListaCuentasBancarias " +
			              " WHERE banco = " + campos[0] + " and cuenta = " + campos[1] + " and CBEN = " + campos[2] ;
				pstmntCB = conn.prepareStatement(SqlCB);	
				rsCB = pstmntCB.executeQuery();
				while (rsCB.next()){
					String registro = FECHA_EFECTIVA +              
					            "," + rsCB.getString(1) +    //activo          
	                            "," + rsCB.getString(2) +    //cRamo
	                            "," + rsCB.getString(3) +    //CBEN
	                            "," + rsCB.getString(4) +    //clabe
	                            "," + rsCB.getString(5) +	 //CBAN
	                            "," + rsCB.getString(6);     //sucursal
	...
	public static int UpdateStatusBCB(Connection conn, String BeneficiariosCuentasBancarias) throws SQLException{
		...
		try{
			for(int i = 0; i < registrosU.length; i++){
				...
				String SqlUpdate = "UPDATE tBeneficiarioCuentasBancarias SET nBCBEnviadoSICOP = 1 " +
							   	   " WHERE dCuentaBancaria = " + camposU[1] + " AND cBanco = (SELECT cBanco from tBancos where dBancoAbreviado = " + camposU[0] + ")" ;
