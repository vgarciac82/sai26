...
public class ControlAccesoManager {
    ...
    public static void apagarAsuntosAbiertos(Connection conn, int tc, String ur) throws SQLException {
        PreparedStatement pstm = null;
        boolean res = true;
        String sQuery = "UPDATE  cg_caso_operacion SET CO_RESPONSABLE = O.O_RESPONSABLE " + " FROM CG_OPERACION AS O " + " where cg_caso_operacion.ID_OPER = O.ID_OPER " + " AND cg_caso_operacion.ID_TC = O.ID_TC " + " AND CG_CASO_OPERACION.CO_RESPONSABLE NOT LIKE '%CONSULTA%' " + " AND cg_caso_operacion.CO_RESPONSABLE <> O.O_RESPONSABLE";
        if (tc == -2) {
