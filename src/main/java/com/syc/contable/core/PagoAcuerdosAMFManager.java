...
public class PagoAcuerdosAMFManager {
	...
	public static ArrayList<String> BuscaCompromisos(Connection conn,String listaIds, String listaCuentaBancaria, String listaFechas, String listaLeyendas, String sUsuario)throws Exception{
		...
		// Encabezado
		String Sql = "SELECT 'H' AS Header, numPagoAMF, '16' AS ramo, 'RHQ' cUR, '1' AS tipoCLC, 'MXN' AS divisa, '1' AS TipoCambio, '16' AS ClaveBeneficiario," +
				" rfcAMF, cuentaBancaria, '2' AS Leyenda, '48' AS FolioAMF, CONVERT(VARCHAR,fechaPago,103) AS fechaPago " +
				" FROM tPagoAMF " +
				" WHERE numPagoAMF in (" +listaIds+ ") ";
		...
		while (rs.next()){
			...
			String encabezado = rs.getString(1)+","+rs.getString(2)+","+rs.getString(3)+","+rs.getString(4).trim()+","+rs.getString(5)+","+rs.getString(6)+
			...
			String Sql2 = "SELECT '65' AS idEvento, '340004' AS Evento, 16 as Ramo, 'RHQ' cUR,CONVERT(VARCHAR,CONVERT(money,importePago)) AS importePago " +
					" FROM tPagoAMF " +
					" WHERE numPagoAMF = '"+rs.getString(2)+ "'";
	...
	public static ArrayList<String> CreaDocumentacionComprobatoria(Connection conn,String listaIds)throws Exception{
		...
		String Sql = " SELECT " +
				"		nFolioOperAjenas , 'H' H, cRamo, 'RHQ', '' SOL_PAGO, " +
		...
		while (rs.next()){
			...
			" inner join [dbo].[tBeneficiario] B (NOLOCK) on PDE.cIdRFC = B.dRFC " +
			" inner join [dbo].[CAT_TIPO_IVA] TI (NOLOCK) on TI.TIVA = DCD.DCD_TIVA " +
	...
	public static ArrayList<String> buscarCierreAMFExport(Connection conn,String listaIds)throws Exception{
		...
		String exportDoc = "SELECT docto, caNoContrarrecibo, cIdRFC, cnombre, fAplicacion, mImporteNeto, mImporteTotal,nFolioPago"
            + " FROM vMultilistadorPagosAMFCierre "
            + " WHERE caNoContrarrecibo IN ("+listaIds+")";
		...
		while (rs.next()){
			...
			String exportDocDetalle = "SELECT  EP,mImporteNeto"
	            + " FROM vPagosDetalles "
	            + " WHERE nfoliopagos = "+rs.getString(8)+" and documento = '"+rs.getString(1)+"' ";
	...
	public static ArrayList<String> buscarAdefaExport(Connection conn,String listaIds)throws Exception{
		...
		// Trae informacion para Exportar
		String exportDoc = "SELECT docto, caNoContrarrecibo, cIdRFC, cnombre, cUnidadResponsable, aEjercicioFiscal, fAplicacionF, mImporteNeto, nFolioEnc, docAplicado"
            + " FROM vAdefasEncabezado "
            + " WHERE caNoContrarrecibo IN ("+listaIds+") ORDER BY docto ASC";
		...
		while (rs.next()){
			...
			String exportDocDetalle = "SELECT  EP, mImporteNeto"
	            + " FROM vAdefasDetalle "
	            + " WHERE nFolioDetalle = "+rs.getString(9)+" and documento = '"+rs.getString(1)+"' ";
