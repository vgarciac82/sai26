...
public class ReintegrosManager {
    ...
    public static boolean insertaDetReintegro(Connection conn, int folioReintegro, ReintegroDetalle datos, String evento, String cCentro, int nDocRenglon) throws SQLException {
        ...
        boolean retval = false;
        try {
            String query = "INSERT INTO tReintegroDetalle (nFolioReintegro,nDocRenglon,noCLC,secCLC,cEvento,EP,mImporteCLC,mImporte,mImporteNegativo,cMes,cCentroContable,nCapitulo,cxp,RFC,nRenglonPagado,nFolioDependencia,ALM,obgt ) " + " SELECT nFolioReintegro,nDocRenglon,noCLC,secCLC,cEvento,EP,mImporteCLC,mImporte,mImporteNegativo,cMes,cCentroContable,nCapitulo,cxp,RFC,nRenglonPagado,nFolioDependencia,ALM,obgt " + " FROM tReintegroDetallePaso with(nolock) WHERE nFolioReintegro=?";
    ...
    public static void autorizaReintegro(Connection conn, Caso c, String nNumSicop, String cRecMotivSicop, String nNumMAP, String cRecMotivMAP, String ulogin, String prefixPath, String fAcredit, String year) throws SQLException, GestionException {
        ...
        try {
            ...
            if ("N".equals(existe)) {
                ...
                pst2 = conn.prepareStatement("INSERT INTO tReintegroAutDetalle(nFolioReintegroaut,nDocRenglon,noCLC,secCLC,EP,mImporteCLC,cMes,movto,cEvento, mImporte, mImporteNegativo, cCentroContable, nCapitulo,cxp,RFC,nFolioDependencia,ALM,nRenglonPagado, obgt,ctab,nidprograma,cSubPrograma, ffm, mAmortizacionAnticipo,mPasivoDiferido,cPasivo,mImportePC) " + " SELECT rd.nfolioreintegro, " + "       rd.ndocrenglon, " + "       rd.noclc, " + "       rd.secclc, " + "       rd.ep, " + "       rd.mimporteclc, " + "       rd.cmes, " + "       rd.movto, " + "       (SELECT CASE " + "                 WHEN ctipopago='PAGOOBRA' AND cEvento NOT IN( 'REIN_TRAM_IR','REIN_TRAM_IP' ) THEN 'RC_EG' " + "				   WHEN ctipopago='PAGOOBRA' AND cEvento IN ('REIN_TRAM_IP') THEN 'RD' " + "                 WHEN ctipopago='PAGODIVERSO' AND cEvento NOT IN( 'REIN_TRAM_IR','REIN_TRAM_IP' ) THEN 'RC_EG' " + "				   WHEN ctipopago='PAGODIVERSO' AND cEvento IN ('REIN_TRAM_IP') THEN 'RD' " + "                 WHEN ctipopago='PAGODIRECTO' THEN 'RD_EG' " + "				   WHEN ctipopago='PAGODIRECTO' AND cEvento IN ('REIN_TRAM_IP') THEN 'RD' " + "                 WHEN ctipopago='RELACIONGASTOS' AND cEvento NOT IN( 'REIN_TRAM_IR','REIN_TRAM_IP' ) THEN 'RD_EG' " + "				   WHEN ctipopago='RELACIONGASTOS' AND cEvento IN ('REIN_TRAM_IP') THEN 'RD_EG' " + "                 WHEN ctipopago='NOMINA' THEN 'RD_EG' " + "                 WHEN ctipopago='AJENAS' THEN CASE " + "												    WHEN cEvento LIKE '%LAUDOS' THEN 'RD_' " + "													WHEN cEvento LIKE '%IP'	THEN 'RD_' " + "													ELSE 'RC_EG' " + "												END " + "                 WHEN ctipopago='FEDERALIZADO' THEN 'RC_EG' " + "				   WHEN ctipopago='FEDERALIZADO' AND cEvento IN ('REIN_TRAM_IP') THEN 'RD' " + "               END AS dTipoPago " + "        FROM   tpagadoencabezado WITH(nolock) " + "        WHERE  canocontrarrecibo = rd.cxp)" + "         " + "       + SUBSTRING(cEvento, 6, LEN(cEvento)), " + "       rd.mimporte, " + "       rd.mimportenegativo, " + "       rd.ccentrocontable, " + "       rd.ncapitulo, " + "       rd.cxp, " + "       rd.rfc, " + "       rd.nfoliodependencia, " + "       (SELECT TOP(1) alm " + "        FROM   tpagadodetalle WITH(nolock), " + "               tpagadoencabezado WITH(nolock) " + "        WHERE  ep = rd.ep " + "               AND nfoliosiaff = rd.noclc " + "               AND tpagadodetalle.nfoliopagado = tpagadoencabezado.nfoliopagado " + "               AND tpagadoencabezado.canocontrarrecibo = rd.cxp) AS ALM, " + "       rd.nrenglonpagado," + "       rd.obgt, " + "		 rd.ctab, " + "       nidprograma ," + "		 cSubPrograma ," + "       rd.OBGT + CASE WHEN RIGHT('00' + LTRIM(RTRIM(nidprograma)),2) = '13' THEN '0000' ELSE '" + year + "' END + RIGHT('00' + LTRIM(RTRIM(nidprograma)),2) + RIGHT('00' + LTRIM(RTRIM(cSubPrograma)),2) + '00' AS ffm, " + "		 0.00 AS mAmortizacionAnticipo, " + "		 mPasivoDiferido, " + "		 cpasivo, " + "		 mImportePC " + " FROM   treintegrodetalle rd WITH (nolock) " + " JOIN dbo.tReintegroEncabezado re WITH (NOLOCK) ON rd.nFolioReintegro = re.nFolioReintegro " + " WHERE  rd.nfolioreintegro = ?");
    ...
    public static double getRemanenteOA(Connection conn, String ep, String cxp, int nRenglon, String caNoContrarreciboOA) throws SQLException {
        ...
        // 3
        queryRemanente = // 4
        "SELECT mimporteneto - imprect - impreint AS Remanente " + " FROM   (SELECT pd.mimporteneto, " + "               (SELECT Count(*) AS sumaRect " + "                FROM   trectificacionencabezado re WITH (nolock), " + "                       trectificaciondetalle rd WITH (nolock) " + "                WHERE  rd.ep = ? " + "                       AND re.cdocumentohaplicado = 'S' " + "                       AND re.nfoliorectificacion = rd.nfoliorectificacion " + "                       AND re.canocontrarrecibo = ? " + "                       AND rd.ep = pd.ep " + "                       AND rd.ndocrenglon = ?)               AS tRectificacion, " + "               (SELECT Count(*) AS sumaReint " + "                FROM   treintegroencabezado ree WITH (nolock), " + // 5
        "                       treintegrodetalle red WITH (nolock) " + "                WHERE  red.ep = ? " + // 6
        "                       AND ree.cdocumentohaplicado = 'S' " + "                       AND ree.nfolioreintegro = red.nfolioreintegro " + "                       AND red.cxp = ? " + "                       AND red.ep = pd.ep " + "                       AND red.nrenglonpagado = ?)           AS tReintegros, " + "               Isnull((SELECT Isnull(Sum(rd.mimporte), 0) AS impR " + // 7
        "                       FROM   treintegroencabezado re WITH(nolock), " + // 8
        "                              treintegrodetalle rd WITH(nolock) " + // 9
        "                       WHERE  re.nfolioreintegro = rd.nfolioreintegro " + "                              AND re.cdocumentohaplicado = 'S' " + "                              AND rd.cxp = ? " + "                              AND rd.ep = ? " + "               AND rd.nrenglonpagado = ?), 0) AS impReint, " + "               Isnull((SELECT Isnull(Sum(CASE Substring(cevento, 1, 4) " + "                                           WHEN 'DICE' THEN mimporte " + "                                           WHEN 'DEBE' THEN mimportenegativo " + //10
        "                                         END), 0) AS imp " + // 11
        "                       FROM   trectificaciondetalle r WITH (nolock), " + "                              trectificacionencabezado h WITH (nolock) " + // 12
        "                       WHERE  r.nfoliorectificacion = h.nfoliorectificacion " + "                              AND h.canocontrarrecibo = ? " + "                              AND r.ep =  ? " + "               AND h.cdocumentohaplicado = 'S' " + //13
        "              AND r.canocontrarrecibo =  ? " + // 14
        "                       GROUP  BY r.ndocrenglon), 0)          AS impRect " + "        FROM   tpagadoencabezado pe WITH (nolock), " + "               tpagadodetalle pd WITH (nolock) " + //15
        "        WHERE  canocontrarrecibo = ? " + "               AND pd.ep = ? " + "               AND pe.cdocumentohaplicado = 'S' " + "               AND pe.nfoliopagado = pd.nfoliopagado " + "               AND pd.ndocrenglon = ?) tNueva " + " WHERE  1 = 1 ";
    ...
    public static void insertaReintegro(Connection conn, ReintegroEncabezadoMil reinE, ArrayList<ReintegroDetalleMil> reinDetalles, int folio, String folioCompleto, Usuario usuario, String cIngresoPropio) throws Exception {
        ...
        try {
            ...
            if ("S".equals(cIngresoPropio))
                // cTipoPoliza
                psInsertEncabezado.setString(18, "PD");
            else
                // cTipoPoliza
            ...
            if ("S".equals(cIngresoPropio)) {
                //tipo Presupuestal IF - 0 Presupuestal IP - 2 Contable IF - 1 Contable IP - 5
                psInsertEncabezado.setInt(27, 2);
            } else {
            ...
            psInsertDetalle = conn.prepareStatement("INSERT INTO tReintegroDetalle (nFolioReintegro,nDocRenglon,noCLC,secCLC,cEvento,EP,mImporteCLC,mImporte,mImporteNegativo,cMes,cCentroContable,nCapitulo,cxp,RFC,nRenglonPagado,nFolioDependencia,ALM,obgt,ctab,FFM,mPasivoDiferido,cPasivo,mImportePC)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            /*,mPasivoDiferido*/
    ...
    public static boolean getValidaGeneraRecepcion(Connection conn, int folioReint) throws SQLException {
        ...
        String cIdContrato = "";
        String cIdRecepcion = "";
        String query = " SELECT 	CASE WHEN COUNT(*) > 0 THEN '1' ELSE '0' END AS esPagoDiv, " + " LTRIM(RTRIM(cFolioPAGODIVERSO)) AS cIdContrato, cIdRecepMat AS cIdRecepcion " + " FROM 	tPAGODIVERSOEncabezado WITH (NOLOCK) " + " WHERE 	caNoContrarrecibo = (SELECT TOP 1 cxp FROM tReintegroDetalle WITH (NOLOCK) WHERE nFolioReintegro = ? ) " + " GROUP BY cFolioPAGODIVERSO, cIdRecepMat ";
        try {
            pstmnt = conn.prepareStatement(query);
            ...
            if (rs.next()) {
                ...
                if ("1".equals(sEsPagoDiv)) {
                    String sTieneRecep = "";
                    String query2 = " SELECT CASE WHEN COUNT(*) > 0 THEN '1' ELSE '0' END AS tieneRecep " + " FROM 	tReintegroRecepcionEncabezado WITH (NOLOCK) " + " WHERE 	nFolioReintegro = ? AND cIdContrato = ? AND cIdRecepcion = ? ";
    ...
    public static String getEsRadicado(Connection conn, String caNoContrarrecibo) throws SQLException {
        ...
        String retval = "";
        try {
            String queryEsRadicado = "SELECT cRadicado FROM tPagadoEncabezado AS PAG WITH (NOLOCK) " + "JOIN ( " + "		SELECT caNoContrarrecibo, cRadicado FROM tPAGODIVERSOEncabezado AS DIV WITH (NOLOCK) " + "		WHERE cDocumentoHaplicado = 'S' " + "		UNION " + "		SELECT caNoContrarrecibo, 'N' AS cRadicado FROM tPagoDirectoEncabezado AS DIR WITH (NOLOCK) " + "		WHERE cDocumentoHaplicado = 'S' " + "		UNION " + "		SELECT caNoContrarrecibo, cRadicado FROM tPAGOOBRAEncabezado AS OBR WITH (NOLOCK) " + "		WHERE cDocumentoHaplicado = 'S' " + "		UNION " + "		SELECT caNoContrarrecibo, cRadicado FROM tRELACIONGASTOSEncabezado AS REL WITH (NOLOCK) " + "		WHERE cDocumentoHaplicado = 'S' " + "		UNION " + "		SELECT caNoContrarrecibo, 'N' AS cRadicado FROM tPAGOFEDERALIZADOEncabezado AS FED WITH (NOLOCK) " + "		WHERE cDocumentoHaplicado = 'S' " + "	) AS TRA ON PAG.caNoContrarrecibo = TRA.caNoContrarrecibo " + "WHERE cDocumentoHaplicado = 'S' AND PAG.caNoContrarrecibo = ? ";
    ...
    public static String getEventoReintegroApartado(Connection conn, String CxP, String EP) throws SQLException {
        ...
        ResultSet rs = null;
        String evento = "";
        String sSQL = "SELECT DISTINCT CASE " + "    WHEN ctipopago='PAGOOBRA' AND cEvento NOT IN( 'REIN_TRAM_IR','REIN_TRAM_IP' ) THEN 'RC_IN_' " + "    WHEN ctipopago='PAGODIVERSO' AND cEvento NOT IN( 'REIN_TRAM_IR','REIN_TRAM_IP' ) THEN 'RC_IN_' " + "    WHEN ctipopago='PAGODIRECTO' THEN 'RD_IN_' " + "    WHEN ctipopago='NOMINA' THEN 'RD_IN_' " + "    WHEN ctipopago='FEDERALIZADO' THEN 'RC_IN_' " + "    WHEN ctipopago='RELACIONGASTOS' THEN 'RD_IN_' " + "  END + SUBSTRING(cEvento, 3, LEN(cEvento)) AS cEvento " + " FROM    vPagadoyRectificacion " + " WHERE   caNoContrarrecibo = '" + CxP + "' " + "		AND EP = '" + EP + "' ";
        try {
            pstmnt = conn.prepareStatement(sSQL);
            rs = pstmnt.executeQuery();
            while (rs.next()) {
    ...
    public static String getEsIngresoPropio(Connection conn, String caNoContrarrecibo) throws SQLException {
        ...
        String retval = "";
        try {
            String queryEsRadicado = "SELECT cIngresosPropios FROM tPagadoEncabezado AS PAG WITH (NOLOCK) " + "JOIN ( " + "		SELECT caNoContrarrecibo, cIngresosPropios FROM tPAGODIVERSOEncabezado AS DIV WITH (NOLOCK) " + "		WHERE cDocumentoHaplicado = 'S' " + "		UNION " + "		SELECT caNoContrarrecibo, CASE WHEN SUBSTRING(EP,40,1) = '4' THEN 'S' ELSE 'N' END AS cIngresosPropios FROM tPagoDirectoEncabezado AS DIR WITH (NOLOCK) " + "		JOIN tPagoDirectoDetalle AS PDET WITH(NOLOCK) ON DIR.nFolioPagoDirecto = PDET.nFolioPagoDirecto " + "		WHERE DIR.cDocumentoHaplicado = 'S' " + "		UNION " + "		SELECT caNoContrarrecibo, cIngresosPropios FROM tPAGOOBRAEncabezado AS OBR WITH (NOLOCK) " + "		WHERE cDocumentoHaplicado = 'S' " + "		UNION " + "		SELECT caNoContrarrecibo, CASE WHEN SUBSTRING(EP,40,1) = '4' THEN 'S' ELSE 'N' END AS cIngresosPropios FROM tRELACIONGASTOSEncabezado AS REL WITH (NOLOCK) " + "		JOIN tRELACIONGASTOSDetalle AS RDET WITH(NOLOCK) ON REL.nFolioRELACIONGASTOS = RDET.nFolioRELACIONGASTOS " + "		WHERE REL.cDocumentoHaplicado = 'S' " + "		UNION " + "		SELECT caNoContrarrecibo, CASE WHEN SUBSTRING(EP,40,1) = '4' THEN 'S' ELSE 'N' END AS cIngresosPropios FROM tPAGOFEDERALIZADOEncabezado AS FED WITH (NOLOCK) " + "		JOIN tPAGOFEDERALIZADODetalle AS PDET WITH(NOLOCK) ON FED.nFolioPAGOFEDERALIZADO = PDET.nFolioPAGOFEDERALIZADO " + "		WHERE FED.cDocumentoHaplicado = 'S' " + "	) AS TRA ON PAG.caNoContrarrecibo = TRA.caNoContrarrecibo " + "WHERE cDocumentoHaplicado = 'S' AND PAG.caNoContrarrecibo = ? ";
            pstmnt = conn.prepareStatement(queryEsRadicado);
            pstmnt.setString(1, caNoContrarrecibo);
            rs = pstmnt.executeQuery();
            if (rs.next()) {
    ...
    public static String getSubcuentaFFM(Connection conn, String cxp, String ep) throws SQLException {
        ...
        ResultSet rs = null;
        String ffm = "";
        String sSQL = "SELECT FFM FROM ( " + "SELECT DISTINCT enc.caNoContrarrecibo " + ", CASE WHEN RECTIFICACIONDET.EP IS NULL THEN det.EP ELSE RECTIFICACIONDET.EP END EP " + ", CASE WHEN RECTIFICACIONDET.FFM IS NULL THEN det.FFM ELSE RECTIFICACIONDET.FFM END FFM FROM tPAGOFEDERALIZADOEncabezado enc  " + "JOIN tPAGOFEDERALIZADODetalle det ON enc.nFolioPAGOFEDERALIZADO = det.nFolioPAGOFEDERALIZADO  " + "LEFT JOIN tRectificacionAutEncabezado RECTIFICACION ON enc.caNoContrarrecibo = RECTIFICACION.caNoContrarrecibo AND RECTIFICACION.cDocumentoHaplicado = 'S' " + "LEFT JOIN tRectificacionAutDetalle RECTIFICACIONDET ON RECTIFICACION.nFolioRectificacionAut = RECTIFICACIONDET.nFolioRectificacionAut AND RECTIFICACIONDET.cEvento LIKE 'DEBE%' " + ") TABLA_FFM  " + "WHERE caNoContrarrecibo = ? AND EP = ? ";
        try {
            pstmnt = conn.prepareStatement(sSQL);
            pstmnt.setString(1, cxp);
            pstmnt.setString(2, ep);
    ...
    public static void insertaPasivoDiferido(Connection conn, ReintegroEncabezadoMil reinE, ArrayList<ReintegroDetalleMil> reinDetalles, int folio, String folioCompleto, Usuario usuario, String cIngresoPropio) throws Exception {
        PreparedStatement psInsertDetalle = null;
        try {
            psInsertDetalle = conn.prepareStatement("INSERT INTO tReintegroDetalle (nFolioReintegro, nDocRenglon, noCLC, secCLC, EP, mImporteCLC, cMes, movto, cEvento, mImporte, mImporteNegativo, cCentroContable, nCapitulo, cxp, RFC, nRenglonPagado,  nFolioDependencia, ALM, OBGT, CTAB, FFM, mPasivoDiferido) " + " SELECT nFolioReintegro " + "			, nDocRenglon + (SELECT COUNT(nFolioReintegro) " + "							 FROM tReintegroDetalle WITH (NOLOCK) " + "							 WHERE nFolioReintegro = ?) AS nDocRenglon " + "			, noCLC " + "			, secCLC " + "			, EP " + "			, 0 AS mImporteCLC " + "			, cMes " + "			, NULL AS movto " + "			, cEvento " + "			, 0 AS mImporte " + "			, 0 AS mImporteNegativo " + "			, cCentroContable " + "			, nCapitulo " + "			, cxp " + "			, 'TESOFE' AS RFC " + "			, nRenglonPagado " + "			, nFolioDependencia " + "			, ALM " + "			, OBGT " + "			, CTAB " + "			, FFM " + "			, mImporte " + "	FROM tReintegroDetalle WITH (NOLOCK) " + "	WHERE nFolioReintegro = ?");
    ...
    public static boolean actualizaAmortizacion(Connection conn, int folio) throws Exception {
        ...
        boolean retval = false;
        try {
            pstmntUp = conn.prepareStatement("UPDATE REINT SET REINT.mAmortizacionAnticipo = DET.mAmortizacionAnticipo " + "FROM tPAGODIVERSOEncabezado ENC " + "	JOIN tPAGODIVERSODetalle DET ON ENC.nFolioPAGODIVERSO = DET.nFolioPAGODIVERSO " + "	JOIN tReintegroAutDetalle REINT ON ENC.caNoContrarrecibo = cxp " + "WHERE nFolioReintegroaut = ? AND DET.nDocRenglon = REINT.nDocRenglon");
    ...
    public static boolean esPagoSICOP(Connection conn, int folio) throws Exception {
        ...
        try {
            ps = conn.prepareStatement("SELECT DISTINCT CASE WHEN cTipoPago IN ('RELACIONGASTOS') OR SUBSTRING(EP,42,1) = 4  THEN 'N' ELSE 'S' END pagoSICOP " + "FROM tReintegroDetalle REINT " + "JOIN tPagadoEncabezado PAG ON REINT.cxp = PAG.caNoContrarrecibo " + "WHERE nFolioReintegro = ?");
            ps.setInt(1, folio);
            rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public static void insertaReintegroPasivo(Connection conn, int folio) throws Exception {
        PreparedStatement pstmnt = null;
        try {
            pstmnt = conn.prepareStatement("INSERT INTO tReintegroDetalle (nFolioReintegro,nDocRenglon,noCLC,secCLC,EP,mImporteCLC,cMes,movto,cEvento,mImporte,mImporteNegativo,cCentroContable,nCapitulo,cxp,RFC,nRenglonPagado,nFolioDependencia,ALM,OBGT,CTAB,FFM,mPasivoDiferido) " + "SELECT nFolioReintegro\r\n" + "	, nDocRenglon + (SELECT COUNT(*) FROM tReintegroDetalle WHERE nFolioReintegro = ?)\r\n" + "	, noCLC\r\n" + "	, secCLC\r\n" + "	, EP\r\n" + "	, 0\r\n" + "	, cMes\r\n" + "	, movto\r\n" + "	, cEvento\r\n" + "	, 0\r\n" + "	, 0\r\n" + "	, cCentroContable\r\n" + "	, nCapitulo\r\n" + "	, cxp\r\n" + "	, 'TESOFE'\r\n" + "	, 0\r\n" + "	, nFolioDependencia\r\n" + "	, ALM\r\n" + "	, OBGT\r\n" + "	, CTAB\r\n" + "	, FFM\r\n" + "	, mImporte\r\n" + "FROM tReintegroDetalle \r\n" + "WHERE nFolioReintegro = ?\r\n" + "");
            pstmnt.setInt(1, folio);
            pstmnt.setInt(2, folio);
            pstmnt.execute();
        } finally {
    ...
    public static String getEventoReintegroAjena(Connection conn, String CxP, String EP) throws SQLException {
        ...
        ResultSet rs = null;
        String evento = "";
        String sSQL = "SELECT DISTINCT CASE " + "    WHEN cEvento LIKE '%LAUDOS' THEN 'RD_' " + "    WHEN cEvento LIKE '%IP'	THEN 'RD_' " + "    ELSE 'RC_IN_' " + "  END + SUBSTRING(cEvento, 3, LEN(cEvento)) AS cEvento " + " FROM    vPagadoyRectificacion " + " WHERE   caNoContrarrecibo = '" + CxP + "' " + "		AND EP = '" + EP + "' ";
        try {
            pstmnt = conn.prepareStatement(sSQL);
            rs = pstmnt.executeQuery();
            while (rs.next()) {
    ...
    public static boolean insertarDetalle(Connection conn, List<ReintegroBoletos> boletos) throws Exception {
        PreparedStatement pstmnt = null;
        String insertQuery = "INSERT INTO tReintegroBoletajeAvion (nfolioReintegro, nFolioPagoDiverso,RFC,cNombre,cReferencia,mTotal,	cPartida) " + " VALUES (?,?,?,?,?,?,?)";
        try {
    ...
    public static int insertaDetalleCompleto(Connection conn, String cxp, int nFolio) throws Exception {
        ...
        int renglones = 0;
        String query = "INSERT INTO tReintegroBoletajeAvion (nfolioreintegro, nFolioPagoDiverso, RFC, cNombre, cReferencia, mTotal, cPartida) VALUES " + "					SELECT ? ,  bol.nFolioPagoDiverso, bol.RFC, cNombre, cReferencia, mTotal, cPartida " + "					FROM tPagoDiversoBoletajeAvion BOL " + "					INNER JOIN tPAGODIVERSOEncabezado PAGO ON BOL.nFolioPagoDiverso = PAGO.nFolioPAGODIVERSO " + "					WHERE PAGO.caNoContrarrecibo = ? ";
        try {
    ...
    public static void liberarBoletos(Connection conn, String folio) throws Exception {
        PreparedStatement pst = null;
        ResultSet rs = null;
        String query = "UPDATE vuelos set Status = 'A' FROM tReintegroBoletajeAvion reintegro " + "inner join tLayoutVuelosDet vuelos on reintegro.cReferencia = vuelos.cReferencia and reintegro.RFC = vuelos.RFC " + "WHERE reintegro.nFolioReintegro =  ? ";
        try {
            pst = conn.prepareStatement(query);
            pst.setString(1, folio);
            pst.executeUpdate();
    ...
    public void eliminarRelacionReintegroComision(Connection conn, Caso c) throws Exception {
        ...
        try {
            ...
            pst.setInt(1, folio);
            pst.executeUpdate();
            log.debug("Object: " + String.valueOf(comision));
        } finally {
            CloseObject.closeObject(pst);
        }
    }
    ...
    public static String getDecrementoEncabezado(Connection conn, int folio, String cFolio) throws Exception {
        ...
        while (rs.next()) {
            ...
            ",").// E
            append(// E
            rs.getString("ID_RAMO").trim()).// F
            append(// F
            ",").// F
            append(// F
            rs.getString("ID_RAMO_CR").trim()).// G
            append(// G
            ",").// G
            append(// G
            rs.getString("ID_RAMO_REC").trim()).// H
            append(// H
            ",").// H
            append(// H
            rs.getString("ID_UNIDAD").trim()).// I
            append(// I
            ",").// I
            append(// I
            rs.getString("ID_UNIDAD_CR").trim()).// J
            append(// J
            ",").// J
            append(// J
            rs.getString("ID_UNIDAD_REC").trim()).// K
            append(// K
            ",").// K
            append(// K
            rs.getString("TIPO_MOVTO_49").trim()).// L
            append(// L
            ",").// L
            append(// L
            compromiso).// M
            ...
            ",").// P
            append(// P
            rs.getString("CTOEXT_30").trim()).// Q
            append(// Q
            ",").// Q
            append(// Q
            rs.getString("CPAG_31").trim().replaceAll(",", " ")).// R
            append(// R
            ",").// R
            append(// R
            rs.getString("CBEN_25").trim()).// S
            append(// S
            ",").// S
            append(// S
            rs.getString("RFC_26").trim()).// T
            append(// T
            ",").// T
            append(// T
            rs.getString("REPRESENTANTE_LGL_38").trim()).// U
            append(// U
            ",").// U
            append(// U
            rs.getString("TPROC_172")).// V
            append(// V
            ",").// V
            append(// V
            rs.getString("ESQ_PRECIO_36")).// W
            append(// W
            ",").// W
            append(// W
            rs.getString("CONTRATACION_35")).// X
            append(// X
            ",").// X
            append(// X
            fContratoIni.trim()).// Y
            ...
            ",").// AA *
            append(// AA *
            rs.getString("ES_PLURIANUAL_33")).// AB*
            append(// AB*
            ",").// AB*
            append(// AB*
            rs.getString("APROB_PLA_34")).// AC
            append(// AC
            ",").// AC
            append(// AC
            rs.getString("ACTO_JURIDICO_32")).// AD
            append(// AD
            ",").// AD
            append(// AD
            montoMonori).// AE
            ...
            ",").// AE
            append(// AE
            rs.getString("TMON_41")).// AF
            append(// AF
            ",").// AF
            append(// AF
            rs.getString("TCAM_64")).// AG
            append(// AG
            ",").// AG
            append(// AG
            montoEjer).// AH
            ...
            ",").// AJ
            append(// AJ
            rs.getString("CONV_MOD_173")).// AK
            append(// AK
            ",").// AK
            append(// AK
            rs.getString("NUM_CONVENIO_46")).// AL
            append(// AL
            ",").// AL
            append(// AL
            rs.getString("FECHA_MODIFICACION_47")).// AM
            append(// AM
            ",").// AM
            append(// AM
            org.apache.commons.lang.StringUtils.trimToEmpty(rs.getString("CODIGO_EXPEDIENTE_37"))).// AN
            append(// AN
            ",").// AN
            append(// AN
            org.apache.commons.lang.StringUtils.trimToEmpty(rs.getString("NUM_PROCEDIMIENTO_39"))).// AO
            append(// AO
            ",").// AO
            append(// AO
            org.apache.commons.lang.StringUtils.trimToEmpty(rs.getString("CODIGO_CONTRATO_40"))).// AP
            append(// AP
            ",").// AP
            append(// AP
            rs.getString("MES_149").trim()).// AQ
            append(// AQ
            ",").// AQ
            append(// AQ
            rs.getString("ID_CTR_INT_301")).// AR
            append(// AR
            ",").// AR
            append(// AR
            rs.getString("TTRANS_21")).// AS
            append(// AS
            ",").// AS
            append(// AS
            rs.getString("COMODIN4_500")).// AT
            append(// AT
            ",").// AT
            append(// AT
            rs.getString("COMODIN4_501")).// AU
            append(// AU
            ",").// AU
            append(// AU
            rs.getString("ETIQUETA_COMPRANET_178")).// AV
            append(// AV
            ",").// AV
            append(// AV
            rs.getString("JUSTIFICA_COMPRANET_416")).// AW
            append(// AW
            ",").// AW
            append(// AW
            rs.getString("IVA_MON_ORIG_414")).// AX
            append(// AX
            ",").// AX
            append(// AX
            rs.getString("IMP_CONT_SIVA_413")).// AY
            append(// AY
            ",").// AY
            append(// AY
            rs.getString("IMP_CONV_MOD_415")).append(",").append(fContratoModifica.trim()).append("\r\n");
    ...
    public static String getDecrementoDetalle(Connection conn, int folio, String cFolio) throws SQLException {
        ...
        while (rs2.next()) {
            ...
            append(// tCEP.cRamo
            rs2.getString("EVENTO")).// tCEP.cUnidadResponsableEP
            append(// tCEP.cUnidadResponsableEP
            ",").// tCEP.cUnidadResponsableEP
            append(// tCEP.cUnidadResponsableEP
            rs2.getString("cRamo").trim()).// tCEP.aEjercicioFiscal
            ...
            ",").// Importe
            append(// Importe
            rs2.getString("Diciembre")).append(",").append(rs2.getString("Importe")).append("\r\n");
