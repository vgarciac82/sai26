...
public class AnteProyectoManager {
    ...
    public static boolean creaHederAnteProyecto(Caso c, Usuario usuario, Connection conn, int nFolio, String aEjercicioFiscal, String aEjercicio, String nCuenta, int nPorcentajeAmpliacion, int nPorcentajeReduccion, String cAutorizado, String cDescripcion, String cUnidadEjecutora, String cCampo, String cUnidadNormativa) throws SQLException {
        ...
        String cQueryCrea = "INSERT INTO tAnteProyectoEncabezado (nFolioAnteProyecto ,aEjercicioFiscal, cUnidadResponsable " + " ,fCarga, nFolioAnteProyectoAnterior, aEjercicioFiscalAnterior, nCuenta, nIncremento " + " ,nDecremento, cCampoCalculo, cAutorizado, cDescripcion, cUnidadNormativa, id_caso) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,? )";
        try {
    ...
    public static boolean creaDetalleAnteProyecto(Connection conn, int nIdCaso, String nCuenta, int nIncremento, int nDecremento, String cUnidadResponsable, String cUnidadNormativa, int nFolioAnteProyecto) throws SQLException {
        ...
        try {
            ...
            if (!"".equals(nCuenta)) {
                cQueryCrea = "INSERT INTO tAnteProyectoDetalle (nConsecutivo,nFolioAnteProyecto,aEjercicioFiscal,cUnidadResponsable,cClaveSiaff,cClaveInterna,mCalculado,mOptimo,mIreductible,nPorcentajeReduccion,nPorcentajeIncremento) " + " SELECT ROW_NUMBER() OVER(order by e.cUnidadEjecutora desc)  AS nDocREnglon," + nIdCaso + " as nFolio, (SELECT e.aEjercicioFiscal+1 FROM tEjercicioFiscal e with (nolock) WHERE cActivo = 1 ) as cEjercicioFiscal, e.cUnidadEjecutora as cUnidadResponsable " + "	  , replace(e.ClaveSIAFF,'" + aEjercicioFiscal + "','" + aEjercicioFiscalAnteProy + "') as ClaveSIAFF, e.ClaveInterna ";
                if (0 != nIncremento) {
                ...
                cQueryCrea += "	      , 0 as mOptimo, 0 as mIreductible, 0 as nPorcentajeReduccion, 0 as nPorcentajeIncremento " + "		   FROM tSaldos s with (nolock)       , tCatalogoEPAnteProy e with (nolock)  " + "		   WHERE s.cSubCuenta = REPLACE(e.EP,'" + aEjercicioFiscalAnteProy + "','" + aEjercicioFiscal + "')    " + "		     AND s.nCuenta LIKE '" + nCuenta + "-%' " + "		     AND e.cUnidadEjecutora = '" + cUnidadResponsable + "' " + "		     AND e.cUnidadNorativa = '" + cUnidadNormativa + "' " + "       GROUP BY e.cUnidadEjecutora, REPLACE(e.ClaveSIAFF,'" + aEjercicioFiscal + "','" + aEjercicioFiscalAnteProy + "'), e.ClaveInterna ";
                pstm.close();
                pstm = conn.prepareStatement(cQueryCrea);
                retVar = pstm.execute();
                ...
                cQueryCrea = "INSERT INTO tAnteProyectoDetalle (nConsecutivo,nFolioAnteProyecto,aEjercicioFiscal,cUnidadResponsable,cClaveSiaff,cClaveInterna,mCalculado,mOptimo,mIreductible,nPorcentajeReduccion,nPorcentajeIncremento) " + " SELECT ROW_NUMBER() OVER(order by e.cUnidadEjecutora desc)+ " + nConsecutivo + "  AS nDocREnglon," + nIdCaso + "  as nFolio, (SELECT e.aEjercicioFiscal+1 FROM tEjercicioFiscal e with (nolock) WHERE cActivo = 1 ) as cEjercicioFiscal, e.cUnidadEjecutora as cUnidadResponsable " + "      , e.ClaveSIAFF, e.ClaveInterna, 0 as mCalculado, 0 as mOptimo, 0 as mIreductible, 0 as nPorcentajeReduccion, 0 as nPorcentajeIncremento  	" + "  FROM tCatalogoEPAnteProy e WITH (NOLOCK) " + " WHERE e.ep not in (select e.ep from tCatalogoEPAnteProy e with (nolock) " + "     , tAnteProyectoDetalle d with (nolock) WHERE e.ClaveSIAFF = d.cClaveSiaff " + "    AND e.ClaveInterna = d.cClaveInterna AND d.nFolioAnteProyecto = " + nIdCaso + " " + "    AND d.cUnidadResponsable = e.cUnidadEjecutora " + "	AND e.cUnidadEjecutora = '" + cUnidadResponsable + "'  AND e.cUnidadNorativa = '" + cUnidadNormativa + "' )" + "    AND e.cUnidadEjecutora = '" + cUnidadResponsable + "'  AND e.cUnidadNorativa = '" + cUnidadNormativa + "' ";
                pstm = conn.prepareStatement(cQueryCrea);
                retVar = pstm.execute();
            } else {
    ...
    public static ArrayList<String> obtenTechoUEPP(Connection conn, String cRoolUsuario, String cUnidadResponsable) throws SQLException {
        ...
        try {
            //sQuery=" SELECT p.cProgramaPresupuestario as txtGridtProgPresupC, p.dProgramaPresupuestario as txtGridtProgPresupD, mTechoMonto as mMontoTopeProgPres FROM tTechoProgramaPresupuestario t with (nolock), tCatalogoProgramaPresupuestario p with (nolock) WHERE t.cProgramaPresupuestario = p.cProgramaPresupuestario ";
            sQuery = "SELECT p.cProgramaPresupuestario as txtGridtProgPresupC " + "    , p.dProgramaPresupuestario as txtGridtProgPresupD " + "    , u.cUnidadResponsable " + "    , u.D_DESCRIPCION " + "    , mTechoMonto as mMontoTopeProgPres " + " FROM tTechoProgPresupUniEjec t with (nolock) " + "    , tCatalogoProgramaPresupuestario p with (nolock) " + "    , tCatUnidadResponsable u with (nolock)  " + "WHERE t.cProgramaPresupuestario = p.cProgramaPresupuestario " + "  AND t.cUnidadResponsable      = u.cUnidadResponsable";
            pstm = conn.prepareStatement(sQuery);
            rs = pstm.executeQuery();
            while (rs.next()) {
                ArrayList<Object> arrmODatos = new ArrayList<>();
    ...
    public static ArrayList<String> obtenTechoPAUE(Connection conn, String cUnidadResponsable, String cRoolUsuario) throws SQLException {
        ...
        try {
            //sQuery=" SELECT p.cPartida as txtGridOGTOC, p.dPartida as txtGridOGTOD, g.cTipoGasto as txtGridCTG, g.dTipoGasto as txtGridDTG, mTechoPartida as mTechoPartida FROM tTechosPartida t with (nolock), tCatalogoPartida p with (nolock), tCatalogoTipoGasto g with (nolock) WHERE t.cTipoGasto = g.cTipoGasto AND t.cPartida = p.cPartida ";
            sQuery = " SELECT p.cPartida as txtGridOGTOC " + "	    , p.dPartida as txtGridOGTOD " + "	    , g.cTipoGasto as txtGridCTG " + "	    , g.dTipoGasto as txtGridDTG " + "	    , t.cUnidadResponsable  " + "	    , u.D_DESCRIPCION " + "	    , mTechoPartida as mTechoPartida " + "    FROM tTechosPartidaUniEjec t with (nolock) " + "	    , tCatalogoPartida p with (nolock) " + "	    , tCatalogoTipoGasto g with (nolock)  " + "	    , tCatUnidadResponsable u with (nolock)  " + "    WHERE t.cTipoGasto = g.cTipoGasto  " + "	   AND t.cPartida = p.cPartida  " + "	   AND t.cUnidadResponsable = u.cUnidadResponsable";
            pstm = conn.prepareStatement(sQuery);
            rs = pstm.executeQuery();
            while (rs.next()) {
                ArrayList<Object> arrmODatos = new ArrayList<>();
    ...
    public static ArrayList<ArrayList<String>> obtenAnteProyecto(Connection conn, String cUnidadResponsable, int nFolio, String cEjercicioFiscal, Caso c) throws SQLException {
        ...
        try {
            id_caso = c.getIdCaso();
            sQuery = " SELECT d.nConsecutivo, d.aEjercicioFiscal, d.cClaveSiaff + '.'+ d.cClaveInterna as Ep, d.mCalculado, d.mOptimo " + "	, d.mIreductible, d.nPorcentajeReduccion, d.nPorcentajeIncremento, isnull(d.cMotivoRechazo,'') " + "  from tAnteProyectoDetalle d with (nolock), " + "       tAnteproyectoEncabezado a with (nolock)" + " WHERE d.nFolioAnteproyecto = a.nFolioAnteProyecto " + "   AND d.aEjercicioFiscal   = a.aEjercicioFiscal+1 " + "   AND d.cUnidadResponsable = a.cUnidadResponsable " + "   AND a.nFolioAnteProyecto = " + nFolio + " " + "   AND a.id_caso = " + id_caso + " ";
    ...
    public static ArrayList<String> ValidaDetalleAnte(Connection conn, String cUnidadResponsable, int nFolio, int nConsecutivo, String cEP, Double mCalculado, Double mOptimo, Double mIreductible, int iReduccion, int iIncremento, String cEjercicioFiscal, Caso c, boolean bCRUD) throws SQLException {
        ...
        String sQuery = "SELECT count(*) FROM tAnteProyectoDetalle d with (nolock), tAnteProyectoEncabezado a with (nolock), tCatalogoEPAnteProy e  with (nolock) " + "	WHERE d.nFolioAnteproyecto = a.nFolioAnteProyecto AND d.aEjercicioFiscal   = a.aEjercicioFiscal+1 " + "	  AND d.cUnidadResponsable = a.cUnidadResponsable  AND d.cClaveSiaff = e.ClaveSiaff " + "	  AND d.cClaveInterna = e.ClaveInterna AND e.ep = '" + cEP.trim() + "' AND a.nFolioAnteProyecto = " + nFolio + " and a.id_caso= " + c.getIdCaso() + " ";
        try {
    ...
    public static String validaTechoPP(Connection conn, int nFolio) throws SQLException {
        String cQueryEF = "SELECT e.mMontoTecho - a.mCalculado as mCalculado, e.mMontoTecho - mOptimo AS mOptimo, e.mMontoTecho - mIreductible AS mIreductible, cEntidadFederativa " + "  FROM (SELECT sum(d.mCalculado) as mCalculado, sum(d.mOptimo) as mOptimo, sum(d.mIreductible) as mIreductible, t.cEntidadFederativa " + "		  FROM tTechoEntidadFederativa t with (nolock)" + "		     , tCatalogoEntidadFederativa e with (nolock)" + "			 , tCatalogoEP p with (nolock)" + "			 , tAnteProyectoDetalle d with (nolock)" + "		WHERE e.cEntidadFederativa = p.cEntidadFederativa " + "		  AND p.ClaveSIAFF = replace(d.cClaveSiaff,convert(varchar,convert(int, substring(d.cClaveSiaff,1,4))),convert(varchar,convert(int, substring(d.cClaveSiaff,1,4))-1)) " + "		  AND p.ClaveInterna = p.ClaveInterna " + "		  AND d.nFolioAnteProyecto = " + nFolio + "		GROUP BY t.cEntidadFederativa ) AS a " + "	 , tTechoEntidadFederativa e " + " WHERE a.cEntidadFederativa = e.cEntidadFederativa";
        String cQueryPP = "select p.mTechoMonto - a.mCalculado as mCalculado, p.mTechoMonto - mOptimo as mTechoMonto, p.mTechoMonto - mIreductible as mTechoMonto, cProgramaPresupuestario " + "  from (select sum(d.mCalculado) as mCalculado, sum(d.mOptimo) as mOptimo, sum(d.mIreductible) as mIreductible, t.cProgramaPresupuestario  " + "		  from tTechoProgramaPresupuestario t with (nolock) " + "		     , tCatalogoEP e with (nolock) " + "		     , tAnteProyectoDetalle d with (nolock)" + "		  WHERE t.cProgramaPresupuestario = e.cProgramaPresupuestario " + "		   AND e.ClaveInterna = d.cClaveInterna  " + "		   AND e.ClaveSIAFF   = replace(d.cClaveSiaff,convert(varchar,convert(int, substring(d.cClaveSiaff,1,4))),convert(varchar,convert(int, substring(d.cClaveSiaff,1,4))-1)) " + "		   and d.nFolioAnteProyecto =  " + nFolio + "		 group by t.cProgramaPresupuestario ) as A " + "	 , tTechoProgramaPresupuestario p " + " where p.cProgramaPresupuestario = a.cProgramaPresupuestario ";
        String cQueryTPA = "select p.mTechoPartida - a.mCalculado as mCalculado, p.mTechoPartida - a.mOptimo as mOptimo, p.mTechoPartida - a.mIreductible as mIreductible, cPartida, cTipoGasto " + "  from (select sum(d.mCalculado) as mCalculado, sum(isnull(d.mOptimo,0)) as mOptimo, sum(isnull(d.mIreductible,0)) as mIreductible, t.cPartida, t.cTipoGasto  " + "		  from tTechosPartida t  with (nolock)" + "		     , tCatalogoEP e  with (nolock)" + "			 , tAnteProyectoDetalle d with (nolock)" + "		 WHERE t.cPartida = e.cPartida  " + "		   AND t.cTipoGasto = e.cTipoGasto " + "		   AND e.ClaveSIAFF = replace(d.cClaveSiaff,convert(varchar,convert(int, substring(d.cClaveSiaff,1,4))),convert(varchar,convert(int, substring(d.cClaveSiaff,1,4))-1)) " + "		   AND e.ClaveInterna =  d.cClaveInterna  " + "		   and d.nFolioAnteProyecto = " + nFolio + "		group by t.cPartida, t.cTipoGasto  ) as A " + "	, tTechosPartida p " + " WHERE p.cPartida = a.cPartida " + "  AND p.cTipoGasto = p.cTipoGasto ";
        String cQueryUE = "select u.mTechoMonto - a.mCalculado as mCalculado, u.mTechoMonto - a.mOptimo as mOptimomOptimo , u.mTechoMonto -mIreductible as mIreductible, cUnidadResponsable " + "  from (select sum(d.mCalculado) as mCalculado, sum(d.mOptimo) as mOptimo, sum(d.mIreductible) as mIreductible, t.cUnidadResponsable  " + "		  from tTechoUEjecutora t  with (nolock)" + "		     , tCatUnidadResponsable u with (nolock)" + "			 , tCatalogoEP e  with (nolock)" + "			 , tAnteProyectoDetalle d " + "		 WHERE t.cUnidadResponsable = u.cUnidadResponsable " + "		   AND e.cUnidadEjecutora = t.cUnidadResponsable  " + "		   AND t.cUnidadResponsable = d.cUnidadResponsable  " + "		   AND e.ClaveSIAFF = replace(d.cClaveSiaff,convert(varchar,convert(int, substring(d.cClaveSiaff,1,4))),convert(varchar,convert(int, substring(d.cClaveSiaff,1,4))-1)) " + "		   AND e.ClaveInterna = d.cClaveInterna  " + "		  and d.nFolioAnteProyecto = " + nFolio + "		 group by t.cUnidadResponsable ) a " + "	 , tTechoUEjecutora  u " + " WHERE a.cUnidadResponsable = u.cUnidadResponsable ";
        String cQueryUN = " SELECT u.mTechoMonto - a.mCalculado as mCalculado, u.mTechoMonto - a.mOptimo as mOptimo, u.mTechoMonto - a.mIreductible as mIreductible, a.cUnidadNormativa " + "   FROM (SELECT sum(d.mCalculado) as mCalculado, sum(d.mOptimo) as mOptimo, sum(d.mIreductible) as mIreductible, t.cUnidadNormativa " + " 		  from tTechoUNormativa t  with (nolock)" + " 		     , tCatUnidadResponsable u with (nolock)" + " 			 , tCatalogoEP e  with (nolock)" + " 			 , tAnteProyectoDetalle d with (nolock) " + " 		 WHERE t.cUnidadNormativa = u.cUnidadResponsable " + " 		   AND e.cUnidadNorativa = t.cUnidadNormativa  " + " 		   AND e.ClaveInterna = replace(d.cClaveSiaff,convert(varchar,convert(int, substring(d.cClaveSiaff,1,4))),convert(varchar,convert(int, substring(d.cClaveSiaff,1,4))-1)) " + " 		   AND e.ClaveInterna = d.cClaveInterna  " + " 		   AND d.nFolioAnteProyecto = " + nFolio + " 		group by t.cUnidadNormativa  ) as A " + " 	 , tTechoUNormativa u " + "  WHERE a.cUnidadNormativa =  u.cUnidadNormativa ";
    ...
    public static ArrayList<String> VercionesAnteproyecto(Connection conn, String cUnidadResponsable, String cRoolUsuario) throws SQLException {
        ...
        String cClaveFolio = "";
        String nDescripcion = "";
        String sQuery = "select nFolioAnteProyecto, convert(varchar,nFolioAnteProyecto)+'   ' +upper(cDescripcion) as cDescripcion " + "  from tAnteProyectoEncabezado a with (nolock) " + " where a.cUnidadResponsable = '" + cUnidadResponsable + "'";
        //	int nFolio=0;
        //	String cDescVercion="";
        try {
            pstm = conn.prepareStatement(sQuery);
    ...
    public static ArrayList<String> ValidaTechoUN(Connection conn, int nFolio, String cUnidadResponsable) throws SQLException {
        ...
        String cTipoGasto = "";
        String cPartida = "";
        String cQueryUN = "SELECT tn.mTechoMonto - a.mTecho as mTecho, a.cUnidadNorativa FROM (select sum(d.mCalculado) as mTecho, e.cUnidadNorativa from tAnteProyectoDetalle d with (nolock), tCatalogoEPAnteProy e with (nolock)" + "		WHERE d.cClaveSiaff = e.ClaveSIAFF AND ltrim(rtrim(d.cClaveInterna)) = ltrim(rtrim(e.ClaveInterna)) AND d.nFolioAnteProyecto = " + nFolio + " " + " 	group by e.cUnidadNorativa )  a " + " 	, tTechoUNormativa tn " + " where a.cUnidadNorativa = tn.cUnidadNormativa" + "  AND a.mTecho > tn.mTechoMonto";
        String cQueryUE = "SELECT te.mTechoMonto - a.mTecho, a.cUnidadEjecutora " + " FROM (select sum(d.mCalculado) as mTecho, e.cUnidadEjecutora " + "	 from tAnteProyectoDetalle d with (nolock)" + "	    , tCatalogoEPAnteProy e with (nolock) " + "	WHERE d.cClaveSiaff = e.ClaveSIAFF " + "	  AND ltrim(rtrim(d.cClaveInterna)) = ltrim(rtrim(e.ClaveInterna)) " + "	  AND d.nFolioAnteProyecto = " + nFolio + " " + "	group by e.cUnidadEjecutora )  a " + "	, tTechoUEjecutora te " + " where a.cUnidadEjecutora = te.cUnidadResponsable " + "  AND a.mTecho > te.mTechoMonto";
        String cQueryEF = "SELECT ted.mMontoTecho - a.mTecho, a.cEntidadFederativa " + " FROM (select sum(d.mCalculado) as mTecho, e.cEntidadFederativa " + "	 from tAnteProyectoDetalle d " + "	    , tCatalogoEPAnteProy e " + "	WHERE d.cClaveSiaff = e.ClaveSIAFF " + "	  AND ltrim(rtrim(d.cClaveInterna)) = ltrim(rtrim(e.ClaveInterna)) " + "	  AND d.nFolioAnteProyecto = " + nFolio + " " + "	group by e.cEntidadFederativa )  a " + "	, tTechoEntidadFederativa ted " + " where a.cEntidadFederativa = ted.cEntidadFederativa " + "  AND a.mTecho > ted.mMontoTecho";
        String cQueryPP = "SELECT tpp.mTechoMonto - a.mTecho, a.cProgramaPresupuestario " + " FROM (select sum(d.mCalculado) as mTecho, e.cProgramaPresupuestario " + "		 from tAnteProyectoDetalle d with (nolock)" + "		    , tCatalogoEPAnteProy e with (nolock)" + "		WHERE d.cClaveSiaff = e.ClaveSIAFF " + "		  AND ltrim(rtrim(d.cClaveInterna)) = ltrim(rtrim(e.ClaveInterna)) " + "		  AND d.nFolioAnteProyecto = " + nFolio + " " + "	group by e.cProgramaPresupuestario )  a " + "	, tTechoProgramaPresupuestario tpp " + " where a.cProgramaPresupuestario = tpp.cProgramaPresupuestario " + "  AND a.mTecho > tpp.mTechoMonto ";
        String cQueryPA = " SELECT tpa.mTechoPartida - a.mTecho, a.cPartida, a.cTipoGasto " + " FROM (select sum(d.mCalculado) as mTecho, e.cPartida, e.cTipoGasto " + " 		 from tAnteProyectoDetalle d  with (nolock)" + " 		    , tCatalogoEPAnteProy e  with (nolock)" + " 		WHERE d.cClaveSiaff = e.ClaveSIAFF " + " 		  AND ltrim(rtrim(d.cClaveInterna)) = ltrim(rtrim(e.ClaveInterna)) " + " 		  AND d.nFolioAnteProyecto =  " + nFolio + " " + " 	group by e.cPartida, e.cTipoGasto )  a " + " 	, tTechosPartida tpa  " + "  where a.cPartida = tpa.cPartida " + "  AND  a.cTipoGasto = tpa.cTipoGasto  " + "  AND a.mTecho > tpa.mTechoPartida ";
        //Partida Restringuida por unidad Ejecutora
        String cQueryPAUE = " SELECT tpa.mTechoPartida - a.mTecho, a.cPartida, a.cTipoGasto, a.cUnidadEjecutora  " + "		 FROM (select sum(d.mCalculado) as mTecho, e.cPartida, e.cTipoGasto, e.cUnidadEjecutora  " + "				 from tAnteProyectoDetalle d   with (nolock)" + "				    , tCatalogoEPAnteProy e  with (nolock) " + "				WHERE d.cClaveSiaff = e.ClaveSIAFF  " + "				  AND ltrim(rtrim(d.cClaveInterna)) = ltrim(rtrim(e.ClaveInterna))  " + "				  AND d.nFolioAnteProyecto = " + nFolio + " " + "			group by e.cPartida, e.cTipoGasto, e.cUnidadEjecutora )  a  " + "			, tTechosPartida tpa   " + "		 where a.cPartida = tpa.cPartida  " + "		  AND  a.cTipoGasto = tpa.cTipoGasto   " + "		  AND a.cUnidadEjecutora = '" + cUnidadResponsable + "' " + "		  AND a.mTecho > tpa.mTechoPartida   ";
        // Programa Presupuestario por Unidad Ejecutora
        String cQueryPPUE = " SELECT tpp.mTechoMonto - a.mTecho, a.cProgramaPresupuestario, a.cUnidadEjecutora  " + "		 FROM (select sum(d.mCalculado) as mTecho, e.cProgramaPresupuestario, e.cUnidadEjecutora   " + "				 from tAnteProyectoDetalle d  with (nolock) " + "				    , tCatalogoEPAnteProy e  with (nolock) " + "				WHERE d.cClaveSiaff = e.ClaveSIAFF  " + "				  AND ltrim(rtrim(d.cClaveInterna)) = ltrim(rtrim(e.ClaveInterna))  " + "				  AND d.nFolioAnteProyecto = " + nFolio + " " + "			group by e.cProgramaPresupuestario, e.cUnidadEjecutora )  a   " + "			, tTechoProgramaPresupuestario tpp   " + "		 where a.cProgramaPresupuestario = tpp.cProgramaPresupuestario  " + "		  AND a.mTecho > tpp.mTechoMonto   " + "		  AND a.cUnidadEjecutora = '" + cUnidadResponsable + "' ";
        //
        String monto = "";
        try {
            //Unidad Normativa
    ...
    public static int MarcaEstatus(Connection conn, String cUnidadResponsable, int nFolio, String cQuien, String cEstatus) throws SQLException {
        int iRetVar = 0;
        PreparedStatement pstm = null;
        //AND cUnidadResponsable = ? ";
        String cQueryRev = " UPDATE tAnteProyectoEncabezado set cRevisor= '" + cEstatus + "' where nFolioAnteProyecto = ? ";
        //AND cUnidadResponsable = ? ";
        String cQueryAut = " UPDATE tAnteProyectoEncabezado set cAutorizado= '" + cEstatus + "' where nFolioAnteProyecto = ? ";
        try {
    ...
    public static int ConsolidaAnteProyecto(Connection conn, int nFolio) throws SQLException {
        ...
        try {
            ...
            if (iExiste == 0) {
                ...
                String cQueryInsertHeder = "INSERT INTO tAnteProyectoEncabezadoAut (nFolioAnteProyectoAut,aEjercicioFiscal,cUnidadResponsable,fCarga,nFolioAnteProyectoAutAnterior,aEjercicioFiscalAnterior,nCuenta,nIncremento,nDecremento,cCampoCalculo,cRevisor,cAutorizado,cDescripcion,cUnidadNormativa )" + " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
                pstmi = conn.prepareStatement(cQueryInsertHeder);
                //el folio nuevo, pero mas abajo el anterior le pones el mismo que este. Si es un identity no hay necesidad de ponerlo. Para qué haces una consulta para obtenerlo, ese folio viene en el caso.
                pstmi.setInt(1, iExiste);
                //quite tu hardcode de "2013"
                pstmi.setString(2, String.valueOf(Integer.parseInt(AdecuacionManager.obtenEjercicioFiscal(conn)) + 1));
                pstmi.setString(3, "RHQ");
                pstmi.setTimestamp(4, new Timestamp(System.currentTimeMillis()));
                //el folio anterior le pones el mismo que el actual??
                pstmi.setInt(5, iExiste);
                ...
                //esto es un select max a detalleaut el cual no ha sido insertado, se pregunta por iExiste, este iExiste tiene el folio del último , de que me sirve?
                rs = pstm.executeQuery();
                if (rs.next()) {
    ...
    public static int getTechoExiste(Connection conn, String cUnidad, String tabla, String campo, String campo2, String valorCampo2, String campo3, String valorCampo3) throws SQLException {
        ...
        if (campo2 != null) {
            if (campo3 != null)
                cQuery = "SELECT COUNT(*) AS TOTAL FROM " + tabla + " with (nolock) WHERE " + campo + "=? AND " + campo2 + "=? AND " + campo3 + "=?";
            else
                cQuery = "SELECT COUNT(*) AS TOTAL FROM " + tabla + " with (nolock) WHERE " + campo + "=? AND " + campo2 + "=?";
        } else
    ...
    public static ArrayList<String> ValidaAnteProyectoEP(Connection conn, String cUnidadResponsable, int nFolio, String aEjercicioFiscal) throws SQLException {
        ...
        String cQueryValida = "select 'La secuencia '+CONVERT(varchar,nConsecutivo)+' Contiene una EP no valida' from tAnteProyectoDetalle d with (nolock) " + " where d.nFolioAnteProyecto = " + nFolio + "   AND d.aEjercicioFiscal = '" + aEjercicioFiscalMasUno + "'" + "   AND d.cUnidadResponsable = '" + cUnidadResponsable + "'" + "  AND ltrim(d.cClaveSiaff+'.'+d.cClaveInterna) not in (select ep from tCatalogoEPAnteProy with (nolock) )";
        try {
    ...
    public static ArrayList<String> ObtenTablaSubcuenta(Connection conn, String cSubCuenta, int nEjercicioFiscal, String cEP, Map mapValues) throws SQLException {
        ...
        int nConsecEP = 0;
        String cSTRQuery = "";
        String cQueryTablas = "select t.EtiquetaCampo, t.NombreCatalogo, t.NombreCampo, s.object_id " + "  from IMX_CATALOGO c with (nolock) " + "     , tTipoSubcuentaConf t with (nolock)" + "	 , sys.tables s with (nolock) " + " WHERE c.NOMBRE_CATALOGO = t.NombreCatalogo " + "   AND t.NombreCatalogo = s.name " + "   AND t.cSubcuenta = '" + cSubCuenta + "'" + "   AND t.aEjercicioFiscal = '" + nEjercicioFiscal + "' " + " order by t.nOrden ";
        String cQueryCampos = "select c.name, c.max_length " + "   from sys.columns c with (nolock) where c.object_id = ? " + "   order by c.column_id ";
        try {
            ...
            while (rst.next()) {
                ...
                if (mapValues.get(cNameCampo) != null)
                    // obtiene el valor del Map
                    cActivaSelected = mapValues.get(cNameCampo).toString();
                else
                    cActivaSelected = "";
