...
public class GestionPoliza extends HttpServlet {
	...
	final static String CREAENCABCANCEL=" insert into tdocpolizaCancelEncabezado select nFolioDocPoliza,fCarga,fAplicacion,cCentroContable,cRamo,cUnidadResponsable,"
		+" null cDocumentoHaplicado,nFolioPoliza,'XX' cTipoPoliza,nMes,cRevisado,aEjercicioFiscal,cUnidadResponsableContable,nFolioPolizaCancelacion"
		+" ,fCancelacion,cDescripcionPoliza,cConcepto,cIdUsuarioCaptura,cIdUsuarioRevision,cIdUsuarioAprobacion,cidOrigen,mTotalCargos*(-1)"
		+" mTotalCargos,mTotalAbonos*(-1)mTotalAbonos,cTipoDocumento,cComentarios,nCambio,nIdCasoOrigen,Periodo13,ADEFAS,nTipoAjuste,"
		+" nFormatoPoliza from tDocPolizaEncabezado with(nolock) where nFolioDocPoliza=?";

	final static String CREADETALLEC   ="insert into tDocPolizaCancelDetalle  select nFolioDocPoliza,nDocRenglon,nCuenta,nSubCuenta,cEvento,mImporte *(-1) mImporte,cCentroContable,"
		+" 'XX' cTipoPoliza,aEjercicioFiscal,cConcepto,nIdCasoOrigen,Periodo13,ADEFAS,nTipoAjuste,parcial,cCABMS,cCUCOP,cPartida,nIdGrupoEvento,"
		+" nIdSubGrupoEvento,cIdEventoManual,nNumeroEvento from tDocPolizaDetalle with(nolock) where nFolioDocPoliza=?";

	final static String CREAPOLIZAC    ="insert into tPoliza select nFolioPoliza,fCreacion,cDescripcionPoliza,mTotalCargo,mTotalAbono,nMes,nCuenta,fAplicacion,nPolizaAutomatica,"
		+" cCentroContable,aEjercicioFiscal,'XX' cTipoPoliza,'DOCPOLIZACANCEL' cTipoDocumento,nFolioDocumento,'E' DocHAplicado,cUsuarioAutorizo"
		+" from tPoliza with(nolock) where 'DOCPOLIZA'=cTipoDocumento and nFolioDocumento=? and not exists(select * from tPoliza with(nolock) "
		+" where 'DOCPOLIZACANCEL'=cTipoDocumento and nFolioDocumento=?)";
	...
	final static String UPPOLCAN       ="update tDocPolizaEncabezado WITH (ROWLOCK) set  cDescripcionPoliza='Póliza Cancelada con la póliza ? '+cDescripcionPoliza,cConcepto='Póliza"
		+" Cancelada con la póliza ? '+cConcepto,nFolioPolizaCancelacion=?,fCancelacion=convert(date,getDate()) ,cDocumentoHaplicado='C' "
		+"where nFolioDocPoliza=*";
	final static String GETFECHA       ="SELECT case when tm.nMes=DATEPART(MONTH,GETDATE()) then CONVERT(date,GETDATE()) else convert(date,DATEADD(s,-1,DATEADD(mm, "
		+"DATEDIFF(m,0,'1/'+CONVERT(varchar,tm.nMes)+'/'+CONVERT(varchar,TM.aEjercicioFiscal))+1,0))) end fAplicacion from tMesesContables"
	...
	final static String CREATPOL       ="insert into tPoliza (nFolioPoliza,fCreacion,cDescripcionPoliza,mTotalCargo,mTotalAbono,nMes,nCuenta,fAplicacion,nPolizaAutomatica,"
										+" cCentroContable,aEjercicioFiscal,cTipoPoliza,cTipoDocumento,nFolioDocumento,DocHAplicado,cUsuarioAutorizo) select distinct dpe.nFolioPoliza,"
										+"dpe.fCarga fCreacion,dpe.cDescripcionPoliza,dpe.mTotalCargos mTotalCargo,dpe.mTotalAbonos mTotalAbono,dpe.nMes,NULL nCuenta,dpe.fAplicacion,0 "
										+"nPolizaAutomatica,dpe.cCentroContable,dpe.aEjercicioFiscal,dpe.cTipoPoliza,'DOCPOLIZA' cTipoDocumento,dpe.nFolioDocPoliza nFolioDocumento,"
										+"dpe.cDocumentoHaplicado DocHAplicado,NULL cUsuarioAutorizo from tDocPolizaEncabezado dpe with(nolock) where dpe.nFolioDocPoliza =?";
			...
			if(id_oper==0)
			{
				...
				else
				{
					...
					if(id_oper>=status)
						...
						if(retroceder(id_oper,nfolioD,NOMBREUSR,CO_RESPONSABLE,st,conn))
						{
							commit(conn);
							if(id_oper==1)
