...
public class CapturaRectificaAnexo1Servlet extends HttpServlet {
	...
	throws ServletException, IOException {
		...
		//**********************************************
		boolean regresaSinError = true; //para saber qué redirect voy a usar, el del catch o el del finally
			...
			if(com.syc.contable.util.Math.truncate(dblTotalDice, 2) == com.syc.contable.util.Math.truncate(dblTotalDebeDecir, 2) ){
				...
				for(int i=0;i<detalle.length;i+=6){
					...
					if(rd.getEvento().contains("DICE")){
						...
						int[] docRenglon = new int[]{1}; // JGDS.20171226 recAnexo1BL.getNDocRenglon(caNoContrarrecibo, rd.getEp(), rd.getMes(), folioR);
						if (docRenglon[0] == -1) {
					...
					if(res!=null){
						if(res.getOficioRectif()==null){
							if(!"".equals(nFolioSICOP))
								insertado = recAnexo1BL.insertTRectificacionEncabezado(folioR,id_caso,cEjercicio,cRamo,cUnidad,cCentroContable,
																					 fExp,fApl,conceptoRectificacion,cTipoMovto,nOrigenPPTO,
																					 nMes,oficioRectif,ctr_int,cTipoRectificacion,nFolioSICOP,
																					 caNoContrarrecibo,cTipoPoliza,cDescripcionPoliza,u_login,
																					 nFolioPoliza,cUnidadResponsableContable,nFolioSIAFF,totalDebe,totalDice,rectificaciones);
