	...
	throws ServletException, IOException {
		...
		try {
			...
			if(com.syc.contable.util.Math.truncate(dblTotalDice, 2) == com.syc.contable.util.Math.truncate(dblTotalDebeDecir, 2) ){
				...
				String cTipoPoliza = "RE"; //RE NUEVO TIPO DE POLIZA PORQUE LOS DE CONTABILIDAD NO QUIEREN QUE PEGUE
					...
					if(res!=null){
						if(res.getOficioRectif()==null){
							if(!"".equals(nFolioSICOP))
								insertado = recPresBL.insertTRectificacionEncabezado(folioR,id_caso,cEjercicio,cRamo,cUnidad,cCentroContable,
																					 ...
																					 nFolioPoliza,cUnidadResponsableContable,nFolioSIAFF,totalDebe,totalDice,esIP);
