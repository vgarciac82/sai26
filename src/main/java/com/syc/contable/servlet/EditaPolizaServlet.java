...
public class EditaPolizaServlet extends HttpServlet implements GestionInterface {
	...
	private String[] htmlResp = {
			" <!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"> "
					...
					+ " 	var bClicBtn = false; "
					+ "   		   function regresar(){ "
					+ "   					opener.parent.document.getElementById(\"pb_send\").disabled = false; "
					+ "   					opener.comprobacionPoliza(); "
					+ "   					opener.parent.document.getElementById(\"pb_send\").click(); "
					+ "   				window.close(); " + "   		   } " + " </SCRIPT> "
					+ "   <BODY> ", "", "</BODY> " + " </HTML>" };
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if ("EDITAR".equals(action)) {
			try {
				...
				pbl.reEditaPoliza(nFolioDocumento, nFolioPoliza,
						cTipoDocumento, cCentroContable, cTipoPoliza,
						aEjercicioFiscal);
				RequestDispatcher dispatch = req
						.getRequestDispatcher("../gstnmngr/gestion?cmd=1");
			...
			try {
				...
				pbl.terminaEditaPoliza(Long.parseLong(nFolioDocPoliza),
				...
				for (int i = 0; i < htmlResp.length; i++)
					out.println(htmlResp[i]);
				out.flush();
				out.close();
			} catch (Exception e) {
				log.error(e, e);
			...
			try {
				...
				int r = pbl.cambiaEstatusPoliza(nFolioPoliza, cCentroContable,
						cTipoPoliza, aEjercicioFiscal, polizaEstatus);
			...
			try {
				...
				pbl.actualizaMovimientosPoliza(Long.parseLong(nFolioDocPoliza),
						cCentroContable, cTipoPoliza, aEjercicioFiscal);
