...
public class AdecuacionServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (request.getParameter("accion") != null && "1".equals(request.getParameter("accion"))) {
				...
				adecua.actualizaJustificaciones(request.getParameter("justificacionA") != null ? new String(request.getParameter("justificacionA").getBytes("ISO-8859-1"), "UTF-8") : "",
					request.getParameter("justificacionR") != null ? new String(request.getParameter("justificacionR").getBytes("ISO-8859-1"), "UTF-8") : "", request.getParameter("justificacionN") != null ? new String(request.getParameter("justificacionN").getBytes("ISO-8859-1"), "UTF-8") : "",
					nFolio);
			}
	...
	public void creaExcelAdecuacionCarga(HttpServletResponse response, HttpServletRequest request) throws IOException, SQLException {
		...
		for (Iterator<?> iter = saldoList.iterator(); iter.hasNext(); row++) {
			...
			anual = Double.valueOf(epSaldo.getMontoEnero()) + Double.valueOf(epSaldo.getMontoFebrero()) + Double.valueOf(epSaldo.getMontoMarzo()) + Double.valueOf(epSaldo.getMontoAbril()) + Double.valueOf(epSaldo.getMontoMayo()) + Double.valueOf(epSaldo.getMontoJunio())
				+ Double.valueOf(epSaldo.getMontoJulio()) + Double.valueOf(epSaldo.getMontoAgosto()) + Double.valueOf(epSaldo.getMontoSeptiembre()) + Double.valueOf(epSaldo.getMontoOctubre()) + Double.valueOf(epSaldo.getMontoNoviembre()) + Double.valueOf(epSaldo.getMontoDiciembre());
