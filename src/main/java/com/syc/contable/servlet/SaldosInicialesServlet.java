...
public class SaldosInicialesServlet extends HttpServlet {
	...
	private static Logger log = Logger
			.getLogger(RespuestaCompromisosServlet.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SaldosBussinessLogic saldos = new SaldosBussinessLogic(
		...
		try {
			...
			response.sendRedirect(basePath
					+ "Generador/CompromisosDevueltos.jsp?mensaje=" + mensaje);
		...
		if (sb.length() != 0) {
			try {
				...
				response.sendRedirect(basePath
						+ "Generador/CompromisosDevueltos.jsp?mensaje="
						+ mensaje);
	...
	@SuppressWarnings("unchecked")
	public StringBuffer cargarArchivoXLS(HttpServletRequest request,
...
//				return actual.getInputStream();
	...
	public static boolean enviaRuta(InputStream in)
		...
		CargaNominaBussinessLogic nominaBL = new CargaNominaBussinessLogic(
				GestionInterface.ATT_CONEXION);
				...
				if (primeraLinea) {
					for (int j = 0; j < llaves.length; j++) {
						if (mapa.contains(llaves[j])) {
							nomColumna.put(new String(llaves[j]),
									new Integer(j));
						}
					}
					primeraLinea = false;
					...
					String EP = celdas[4] + "." + celdas[2] + "." + celdas[3]
							+ "." + celdas[5] + "." + celdas[6] + "."
							+ celdas[7] + "." + celdas[8] + "." + celdas[9]
							+ "." + celdas[10] + "." + celdas[11] + celdas[12]
							+ celdas[13] + celdas[14] + "." + celdas[15] + "."
							+ celdas[16] + "." + celdas[17] + "." + celdas[18]
							+ "." + claveInterna;
