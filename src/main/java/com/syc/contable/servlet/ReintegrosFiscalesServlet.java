...
import com.syc.contable.ReintegrosFiscalesBusinessLogic;
...
public class ReintegrosFiscalesServlet extends HttpServlet {
	...
	private static final Logger	log					= Logger.getLogger(ReintegrosFiscalesServlet.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String ctab = request.getParameter("ctab"); // Validar si no se usa borrar
		...
		if (borraTodo != null && !"".equals(borraTodo)&& "1".equals(borraTodo)) { //ESTO BORRA DE LA BASE DE DATOS LOS REGISTROS Y 
			...
			try {
				...
				reintegro.borraReintegro(Integer.parseInt(folioReintegro,10)); //Llamada al businesslogic
				Caso c = (Caso) session.getAttribute(GestionInterface.ATT_CASE);
				...
				response.sendRedirect(basePath + "plantillasCasos/reintegroIngreso.jsp");
			} catch (SQLException e) {
				response.sendRedirect(basePath + "plantillasCasos/reintegroIngreso.jsp");
				e.printStackTrace();
			} catch(Exception e){
	...
	public String cancelaReintegro(String folio) throws SQLException {
		...
		try {
			...
			//CANCELAR REINTEGRO
			boolean reintegroResultado=ae.cancelAccountingApplication(conn,"REINTEGROINGRESO",folio,"tReintegroIngresoEncabezado","tReintegroIngresoDetalle","nFolioReintegroIngreso");
	...
	public void creaExcelReporte(String folio,HttpServletResponse response,HttpServletRequest request) throws IOException, SQLException{
		...
		String r_Interno = request.getParameter("r_Interno");
	...
	public void generaCasoInserts(HttpServletResponse response,HttpServletRequest request) throws Exception{
		...
		//TODO: ID_TC 59
		cRein = generaCaso(u, 59, fg, "CAPTURA_REINTEGROINGRESO", "Reintegro Ingreso");
		...
		String cTipoReintegro = request.getParameter("cTipoReintegro");
		String cEvento = request.getParameter("cEvento");
		String r_Interno = request.getParameter("r_Interno");
		...
		if("IF".equals(cTipoReintegro)){
			re.setcTipoPoliza("IN");//TODO: Tipo Poliza Reintegro Ingreso Fiscal.
		}else{
			re.setcTipoPoliza("EG");//TODO: Tipo Poliza Reintegro Ingreso Propio.
		}
	    ...
	    re.setcTipoReintegro(cTipoReintegro);
	    
		ArrayList<ReintegroIngresoDetalle> rd = new ArrayList<ReintegroIngresoDetalle>();
		ReintegroIngresoDetalle rdaux;
		double sumaImportes=0;
		
		for(int i=0;i<valuesCLC.length;i++){
			rdaux = new ReintegroIngresoDetalle();
			rdaux.setnSIAFF(valuesCLC[i]);
			rdaux.setCxp(valuesCXP[i]);
			rdaux.setEP(valuesEP[i]);
			rdaux.setMes(Integer.parseInt(valuesMes[i],10));
			...
			rdaux.setAlm(reintegro.getALM(rdaux.getCxp(), rdaux.getEP(), rdaux.getMes(), 0, Integer.parseInt(rdaux.getnDocRenglon(),10)));
			...
			if("IF".equals(cTipoReintegro)){
				if("RN".equals( r_Interno )) {
					rdaux.setcEvento("RN");//TODO: Evento Apartado para Ingreso Fiscal.
				} else {
					rdaux.setcEvento("REIN_TRAM_IF");//TODO: Evento Apartado para Ingreso Fiscal.
				}
			}else{
				rdaux.setcEvento(cEvento);//TODO: Evento para Ingreso Propio.
			}			
	...
	private Caso generaCaso(Usuario u, int idTCaso, FolioGeneratorInterface fg, String opResponsable, String concepto) throws GestionException, SQLException {
		...
		// Cambia el usuario al grupo captura para que aparezca en el inbox.
