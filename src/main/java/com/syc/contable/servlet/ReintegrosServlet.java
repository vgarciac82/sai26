...
import com.syc.reintcont.ReintegroContBussinesLogic;
...
public class ReintegrosServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String autorizaMil = request.getParameter("autorizaMil");
		String aplicaMil = request.getParameter("aplicaMil");
		String cancelaMil = request.getParameter("cancelaMil");
		...
		String fAcredit = request.getParameter("fAcredit");	//reintegro normal
		String fechaAcredit = request.getParameter("fechaAcredit"); //reintegro mil
		...
		String fin_anio = request.getParameter("cEsFinAnio");
		...
		if (cancelaDocumentoMil != null && !"".equals(cancelaDocumentoMil)&& "true".equals(cancelaDocumentoMil)) {
			try {
				...
				//String folio = request.getParameter("folio");
				if (folio != null && !"".equals(folio)){
					mensaje+=cancelaReintegroMil(folio);	
				}
				else {
					mensaje+= "Todos los campos deben llenarse";
				}
		...
		if (generaExcel != null && !"".equals(generaExcel)&& "2".equals(generaExcel)) {
			try {
				creaExcelReintegroCargaMil("ReintegroCapituloMil",response,request);
			} catch (SQLException e) {
				e.printStackTrace();
			}
		}
		...
		if (autoriza != null && !"".equals(autoriza)&& "1".equals(autoriza)) {
		    ...
		    try{
					...
					out.println(new String(reintegro.AutorizaReintegroNuevo(c,"","","","",m,prefixPath,usuario.getLogin(), usuario, fAcredit, fin_anio ).getBytes("UTF-8"),"ISO-8859-1"));
					//Liberar boletos de avion
					reintegro.liberarBoletosAvion(folio);
				}
		    catch (Exception ex) {
				ex.printStackTrace();
			}
		...
		if (aplicaMil != null && !"".equals(aplicaMil)&& "1".equals(aplicaMil)) {
			HttpSession session = request.getSession(false);
			PrintWriter out = response.getWriter(); 
			if (session == null) {
				response.sendRedirect("../index.jsp");
		    ...
		    String prefixPath = getServletContext().getRealPath("/WEB-INF/mail-bodies/") + File.separator;
		    try{
		    	out.println(new String(reintegroMil.ValidaReintegro(new Integer(c.getFolio().substring(c.getFolio().lastIndexOf('-') + 1)).intValue(),usuario,adecProy.obtenEjercicioFiscal(),usuario.getU_Ramo(), usuario.getU_UR(), c, usuario.getPropiedad("CCENTROCONTABLE").getValor(), c.getCasoDato("FECHA_AP_CONT").getValor(), m, prefixPath,usuario.getLogin()).getBytes("UTF-8"),"ISO-8859-1"));
		    }
		    catch (Exception ex) {
				ex.printStackTrace();
				log.warn(mensaje);
		...
		if (autorizaMil != null && !"".equals(autorizaMil)&& "1".equals(autorizaMil)) {
			HttpSession session = request.getSession(false);	
			PrintWriter out = response.getWriter(); 
			if (session == null) {
				response.sendRedirect("../index.jsp");
		    ...
		    try{
				...
				String[] ipServidor = ipNombreServidor.split("/");
				if (ipServidor[1].equals(GestionInterface.SYS_IP_PRODUCCION))
					reintegroMil.correoProduccion=true;
			
				out.println(new String(reintegroMil.AutorizaReintegroNuevo(c,"","","","",m,prefixPath,usuario.getLogin(),usuario,fechaAcredit,fin_anio).getBytes("UTF-8"),"ISO-8859-1"));
		    }
		    catch (Exception ex) {
				ex.printStackTrace();
			}
		...
		if (cancelaMil != null && !"".equals(cancelaMil)&& "1".equals(cancelaMil)) {
			HttpSession session = request.getSession(false);		
			PrintWriter out = response.getWriter(); 
			if (session == null) {
				response.sendRedirect("../index.jsp");
		    ...
		    String prefixPath = getServletContext().getRealPath("/WEB-INF/mail-bodies/") + File.separator;
		    try{
		    	 out.println(new String(reintegroMil.cancelarAppContableNuevo(c,m,prefixPath,usuario.getLogin(), c.getCasoDato("FECHA_AP_CONT").getValor(),usuario).getBytes("UTF-8"),"ISO-8859-1"));
		    }catch (Exception ex) {
				ex.printStackTrace();
			}		
		}
		
		if (borraTodo != null && !"".equals(borraTodo)&& "1".equals(borraTodo)) { //ESTO BORRA DE LA BASE DE DATOS LOS REGISTROS Y 
			...
			try {
				...
				reintegro.borraReintegro(Integer.parseInt(folioReintegro)); //Llamada al businesslogic
				Caso c = (Caso) session.getAttribute(GestionInterface.ATT_CASE);
				...
				response.sendRedirect(basePath + "plantillasCasos/reintegroPresupuestal.jsp");
			} catch (SQLException e) {
				response.sendRedirect(basePath + "plantillasCasos/reintegroPresupuestal.jsp");
				e.printStackTrace();
			} catch(Exception e){
		...
		if (borraTodo != null && !"".equals(borraTodo)&& "2".equals(borraTodo)) { //ESTO BORRA DE LA BASE DE DATOS LOS REGISTROS Y 
			//LIMPIA EL ARCHIVO PARA SUBIR UNA RECTIFICACION NUEVA EN EL MISMO FOLIO
			try {
				HttpSession session = request.getSession(false);		
				if (session == null) {
				...
				DocumentoBussinessLogic dbl = new DocumentoBussinessLogic();
				String folioReintegro = request.getParameter("folioReintegro");
				reintegroMil.borraReintegro(Integer.parseInt(folioReintegro)); //Llamada al businesslogic
				Caso c = (Caso) session.getAttribute(GestionInterface.ATT_CASE);
				dbl.limpiaDocumento(c.getTipoCaso().getGavetaAsociada(), c.getIdGabinete(), 2, 1);
				CasoBusinessLogic casoTx = new CasoBusinessLogic("jdbc/gestion");
				ITree tree = casoTx.getArbolCaso(c);
				session.setAttribute("tree.model", tree);
				response.sendRedirect(basePath + "plantillasCasos/reintegroPresupuestalMil.jsp");
			} catch (SQLException e) {
				response.sendRedirect(basePath + "plantillasCasos/reintegroPresupuestalMil.jsp");
				e.printStackTrace();
			} catch(Exception e){
		...
		if (generaCaso != null && !"".equals(generaCaso)&& "1".equals(generaCaso)) { //Reintegros Normales
			try {
		...
		if (generaCaso != null && !"".equals(generaCaso)&& "2".equals(generaCaso)) { //Reintegros MIL
			try {
				generaCasoInsertsMil(response,request);
			} catch (SQLException e) {
				e.printStackTrace();
			} catch (GestionException e) {
				e.printStackTrace();
		...
		if (generaCaso != null && !"".equals(generaCaso)&& "3".equals(generaCaso)) { //Reintegros Contables
			try {
				generaCasoInsertsAut(response,request);
			} catch (SQLException e) {
				e.printStackTrace();
			} catch (GestionException e) {
				e.printStackTrace();
	...
	public String cancelaReintegroMil(String folio) throws SQLException {
		Connection conn = null;
		String mensaje ="";
		try {
			AccountingEngine ae = new AccountingEngine();
			...
			//CANCELAR REINTEGROAUT
			boolean reintegroAutResultado=ae.cancelAccountingApplication(conn,"REINTEGROAUT",folio,"tReintegroAutEncabezadoMil","tReintegroAutDetalleMil","nFolioReintegroMilAut");
			mensaje+="Cancela reintegroAut Resultado:"+new Boolean(reintegroAutResultado).toString();

			//CANCELAR REINTEGRO
			boolean reintegroResultado=ae.cancelAccountingApplication(conn,"REINTEGRO",folio,"tReintegroEncabezadoMil","tReintegroDetalleMil","nFolioReintegroMil");
			mensaje+=", Cancela reintegro Resultado:"+new Boolean(reintegroResultado).toString();
			conn.commit();
		}catch(Exception e){
			e.printStackTrace();
	...
	public void creaExcelReintegroCarga(String folio,HttpServletResponse response,HttpServletRequest request) throws IOException, SQLException{
		...
		for( int i = 0; i < valuesCLC.length; i++ ){
			...
			System.out.print(" clc: " + valuesCLC[i]);
			System.out.print(" cxp: " +valuesCXP[i]);
			System.out.print(" ep: " +valuesEP[i]);
			System.out.print(" mes: " +valuesMes[i]);
	...
	public void creaExcelReintegroCargaMil(String folio,HttpServletResponse response,HttpServletRequest request) throws IOException, SQLException{
		HttpSession session = request.getSession();
		Usuario usuario = (Usuario) session.getAttribute(GestionInterface.ATT_USER);
		String file_name= ""+folio+"";
		response.setContentType("application/vnd.ms-excel");
		...
		String[] valuesCLC = request.getParameterValues("clc");
		String[] valuesCXPSAI = request.getParameterValues("cxpsai");
		String[] valuesCXP = request.getParameterValues("cxp");
		String[] valuesEP = request.getParameterValues("ep");
		String[] valuesMes = request.getParameterValues("mes");
		String[] valuesImporte = request.getParameterValues("importe");
		String[] valuesConcepto = request.getParameterValues("tipoConcepto");
		String[] valuesMovimiento = request.getParameterValues("tipoMovimiento");
		double total=0;
		...
		celda = fila.createCell(9);
		celda.setCellValue("ID TIPO MOVIMIENTO");
		...
		for( int i = 0; i < valuesCLC.length; i++ ){
			...
			celda.setCellValue(valuesImporte[i]);
			celda = fila.createCell(6);
			celda.setCellValue(valuesCXPSAI[i].trim());
			celda = fila.createCell(7);
			celda.setCellValue(valuesCXP[i].trim());
			celda = fila.createCell(8);
			celda.setCellValue(valuesConcepto[i].trim());
			celda = fila.createCell(9);
			celda.setCellValue(valuesMovimiento[i].trim());
	...
	public void generaCasoInserts(HttpServletResponse response,HttpServletRequest request) throws Exception{
		...
		cRein = generaCaso(u, 15, fg, "CAPTURISTA_REINTEGRO", "Reintegro Presupuestal");
		...
		if(re.getnId_TipoReintegro() == 0){
			re.setcTipoPoliza("IN");//TIPOPOLIZA REINTEGRO NORMAL
		}else if(re.getnId_TipoReintegro() == 1 || re.getnId_TipoReintegro() == 5 || re.getnId_TipoReintegro() == 2){
			re.setcTipoPoliza("PD");//TIPOPOLIZA REINTEGRO NORMAL
		}
		...
		for(int i=0;i<valuesCLC.length;i++){
			...
			String cRadicado = reintegro.getEsRadicado(rdaux.getCxp());
			cIngresoPropio =  reintegro.getEsIngresoPropio(rdaux.getCxp());
			String cAjena =  reintegro.getEsAjena(rdaux.getCxp());
				
			if("BMN930209927".equals((reintegro.getRFC(rdaux.getCxp(), rdaux.getEP())))){
				rdaux.setFFM(reintegro.getFFM(rdaux.getCxp(), rdaux.getEP())); 
			}	
			
			if("S".equals( cAjena )) {
				rdaux.setcEvento(reintegro.getEventoReintegroAjena(rdaux.getCxp(), rdaux.getEP()));
			} else {
				if("N".equals(cRadicado) && "N".equals(cIngresoPropio) ){
					//rdaux.setcEven to("REIN_TRAM");
					rdaux.setcEvento(reintegro.getEventoReintegroApartado(rdaux.getCxp(), rdaux.getEP()));			
				}else if("S".equals(cRadicado) && "N".equals(cIngresoPropio)){
					rdaux.setcEvento("REIN_TRAM_IR");
				}else if("N".equals(cRadicado) && "S".equals(cIngresoPropio)){
					rdaux.setcEvento("REIN_TRAM_IP");
				}
			}
			
			//String cTipoPago = reintegro.getTipoPago(rdaux.getCxp());
			rdaux.setcPasivo(reintegro.getPasivo(rdaux.getCxp()));
			rdaux.setmImportePC(reintegro.getImportePC(rdaux.getCxp()));
		...
		reintegro.insertaReintegro(re, rd, idCasoReintegro, folioCasoReintegro, u, cIngresoPropio);
		
		if(re.getnId_TipoReintegro() == 0){
			String TipoPago = ReintegrosBusinessLogic.buscaCXP(idCasoReintegro);
			
			if("RELACIONGASTOS".equals( TipoPago ) && "N".equals(cIngresoPropio)) {
				ReintegrosBusinessLogic.insertaCompromisoReintegro(re, rd, idCasoReintegro, folioCasoReintegro, u);			
			}
		}
		
		//VALIDAR SI ES REINTEGRO DIFERENTE DE RELACION DE GASTOS
		//if("COMPROMISO".equals(cTipoPago)){
		//	reintegro.insertaPasivoDiferido(re, rd, idCasoReintegro, folioCasoReintegro, u, cIngresoPropio);			
		//}
	...
	public void generaCasoInsertsMil(HttpServletResponse response,HttpServletRequest request) throws Exception{
		HttpSession session = request.getSession(false);
		if (session == null) {
			response.sendRedirect("../index.jsp");
			return;
		...
		Usuario u = (Usuario) session.getAttribute(GestionInterface.ATT_USER);
		cRein = generaCaso(u, 26, fg, "CAPTURISTA_REINTEGRO_MIL","Reintegro Presupuestal Capitulo Mil");
		int idCasoReintegro = new Integer(cRein.getFolio().substring(cRein.getFolio().lastIndexOf('-') + 1)).intValue();
		String folioCasoReintegro = cRein.getFolio();
		...
		ReintegroEncabezadoMil re = new ReintegroEncabezadoMil();
		re.setObservaciones(new String( request.getParameter("observaciones").getBytes("ISO-8859-1"),"UTF-8" ));
		re.setConcepto(new String( request.getParameter("concepto").getBytes("ISO-8859-1"),"UTF-8"));
		re.setMovimiento(request.getParameter("CatMovimientoReintegro"));
		re.setTipoAviso(request.getParameter("CatTipoCausaAvisoReintegro"));
		re.setCausaAviso(request.getParameter("CatCausaAvisoReintegro"));
		re.setFormaDePago(request.getParameter("CatFormaPagoAvisoReintegro"));
		...
		//re.setcTipoPoliza("PD");//TIPOPOLIZA REINTEGRO MIL
		re.setcDocumentoHaplicado("N");
		re.setcUnidadResponsable(u.getU_UR());
		re.setnFolioReintegro(idCasoReintegro);
		re.setfSolicitud(fecha);
		...
		re.setCuentaBancaria("N/A");
		re.setLc("N/A");
		re.setfAcreditacion(today);
		re.setfAplicacion(today);
		re.setFolioDependencia("N/A");
		...
		for(int i=0;i<valuesCLC.length;i++){
			...
			rdaux.setnSIAFF(valuesCLC[i]);
			rdaux.setCxp(valuesCXP[i]);
			rdaux.setCLCSAI(valuesCXPSAI[i]);
			rdaux.setTipoConcepto(valuesTipoConcepto[i]);
			rdaux.setTipoMovimiento(valuesTipoMovimiento[i]);
			rdaux.setEP(valuesEP[i]);
			rdaux.setMes(Integer.parseInt(valuesMes[i]));
			rdaux.setmImporteCLC(Double.parseDouble(valuesImporte[i]));
			rdaux.setnDocRenglon(String.valueOf(i+1));
			rdaux.setRenglonPagado(String.valueOf(reintegroMil.getNDocRenglon(rdaux.getCLCSAI(), rdaux.getEP(), rdaux.getMes(), rdaux.getCxp(),rdaux.getTipoMovimiento(),rdaux.getTipoConcepto())));
			rdaux.setcCentroContable(reintegroMil.getCCNormal(rdaux.getCxp(), rdaux.getEP(), rdaux.getMes(), 0, rdaux.getnDocRenglon()));
			rdaux.setAlm(reintegroMil.getALM(rdaux.getCxp(), rdaux.getEP(), rdaux.getMes(), rdaux.getCLCSAI()));
			rdaux.setcPartida(reintegroMil.getcPartida(rdaux.getEP()));
			rdaux.setSecCLC(String.valueOf(reintegroMil.secCLC(String.valueOf(rdaux.getnSIAFF()).replace(".0", ""), rdaux.getEP(), 0)));
			rdaux.setFolioDependenciaSicop(reintegroMil.folioDependencia(String.valueOf(rdaux.getnSIAFF()).replace(".0", ""), rdaux.getEP(), 0));
			rdaux.setRfc(reintegroMil.getRFC(rdaux.getCxp(), rdaux.getEP()));
			rdaux.setcEvento(reintegroMil.eventoReintegroMil(valuesCXPSAI[i], valuesEP[i]));
			//rdaux.setcEvento("REIN_TRAM");
			rdaux.setcCentroContable(reintegroMil.getCCNormal(rdaux.getCLCSAI(), rdaux.getEP(), rdaux.getMes(), idCasoReintegro, rdaux.getCxp()));
			rdaux.setObgt( valuesEP[i].substring(31, 36) );
			rdaux.setCtab(reintegroMil.getCTAB(rdaux.getCxp(), rdaux.getEP()));
		...
		reintegroMil.insertaReintegro(re, rd, idCasoReintegro, folioCasoReintegro, u, cRein);
	...
	public void generaCasoInsertsAut(HttpServletResponse response,HttpServletRequest request) throws Exception{
		HttpSession session = request.getSession(false);
		if (session == null) {
			response.sendRedirect("../index.jsp");
			return;
		...
		re.setcRamo(u.getU_Ramo());
		re.setcUnidadResponsableContable("RHQ");
		re.setcTipoPoliza("PD");//TIPO POLIZA REINTEGRO CONTABLE0
		re.setcDocumentoHaplicado("N");
		re.setLc("N/A");
		re.setcUnidadResponsable(u.getU_UR());
		re.setnFolioReintegro(idCasoReintegro);
	    ...
	    String sEsIP = request.getParameter("esIngresoPropio");
		...
		reintegro.insertaReintegro(re, rd, idCasoReintegro, folioCasoReintegro, u, sEsIP);
