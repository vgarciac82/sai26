...
public class RectificacionIngresoFiscalManager {
	...
	public static boolean insertarEncabezado(Connection conn, int folio, String nIDCaso, String cEjercicio, String cRamo, String cUnidad, String cCentroContable, java.sql.Date fExp, java.sql.Date fApl, String conceptoRectificacion, String cTipoMovto, String nOrigenPPTO, String nMes,
		...
		String insertQuery = "INSERT INTO tRectificacionIngresosEncabezado " 
			+ "( nFolioRectificaIngreso, nIDCaso, aEjercicioFiscal, cRamo, cUnidadResponsable" 
	...
	public static boolean insertarDetalle(Connection conn, ArrayList<RectificacionIngresoFiscalDetalle> rectificaciones, String caNoContrarrecibo) throws SQLException {
		...
		try {
			...
			for (int i = 0; i < rectificaciones.size(); i++) {
				...
				String insertQuery = "	INSERT INTO tRectificacionIngresosDetalle " + 
				        			 " ( " +
									 "		nFolioRectificaIngreso, " +
									 "		nDocRenglon, " +
									 "		cEvento, " +
									 "		EP, " +
									 "		mImporte, " +
									 ...
									 "		?," +
									 "		?," +
									 "		(SELECT TOP 1 sCUENTA_BANCARIA FROM tLayoutsCreadosRegistroIngresoEncabezado WITH(NOLOCK) WHERE cEstatus='ACTIVO' AND sAuxiliarComodin='" + caNoContrarrecibo.trim() + "' ORDER BY ID DESC)," +
									 " 		SUBSTRING(?,32,5)," +
									 "		?" +
									 ")";
				pstmnt = conn.prepareStatement(insertQuery);
	...
	public static RectificacionIngresoFiscalEncabezado getRectificacionEncabezadoSicop(Connection conn, int folio) throws SQLException {
		...
		String query = "SELECT  'H' AS posicion1 , " +
					   ...
					   "	R.oficioRectif , " +
					   "	R.oficioRectif , " +
					   "	R.nFolioRectificaIngreso , " +
					   "	R.nIDCaso , " +
					   "	R.totalDICE , " +
					   "	R.totalDEBE " +
					   "FROM tRectificacionIngresosEncabezado r WITH ( NOLOCK ) " +
					   "WHERE nFolioRectificaIngreso = ? ";
	...
	public static String getRectificacionDetalleSicop(Connection conn, int folio) throws SQLException {
		...
		String query = "SELECT DISTINCT " +
					   ...
					   "	END , " +
					   "	ISNULL(R.ctr_int, '') AS ctr_int " +
					   " FROM tRectificacionIngresosDetalle d WITH ( NOLOCK ) , " +
					   "	tRectificacionIngresosEncabezado r WITH ( NOLOCK ) , " +
					   "	tCatalogoEP e WITH ( NOLOCK ) , " +
					   "	tBeneficiario b WITH ( NOLOCK ) " +
					   "WHERE d.nFolioRectificaIngreso = r.nFolioRectificaIngreso " +
					   "	AND d.EP = e.EP " +
					   "	AND b.dRFC = 'TESOFE' " +
					   "	AND r.nFolioRectificaIngreso = ? " + 
					   "	AND d.nFolioRectificaIngreso = ? ";
