...
public class AccountingWithoutEventProcessor implements AccountingEventProcessorInterface {
	...
	private static final Logger log = Logger.getLogger(AccountingWithoutEventProcessor.class);
	...
	public List<AccountingMovement> getMovements(Connection conn, String event, Map<String, String> values) throws AccountingEventProcessorException {
		...
		AccountingMovement ac = new AccountingMovement(cta.getCuenta(),
		        cta.isSubCuentaRequerida() ? values.get(cta.getSubCuenta()).trim() : null,
		        Double.parseDouble(values.get(FIELD_COMPONENTE)),
		        EVENTO_CARGO.equalsIgnoreCase(event),
		        cta.esAcreedora(),
		        cta.verificaSaldo(),
		...
		try {
			...
			if (!cuentas.containsKey(cuenta)) {
				...
				cta = new Account(rs.getString("nCuenta"),
				        (alternativeSubActName == null ? rs.getString("cSubcuenta") : alternativeSubActName),
