...
public class AccountingEngine extends DataSourceManager {
	...
	private static final Logger log = Logger.getLogger(AccountingEngine.class);

	private static final String sqlInsertPoliza = "INSERT INTO tPoliza (nFolioPoliza, fCreacion, cDescripcionPoliza, mTotalCargo, mTotalAbono, nMes, nCuenta, fAplicacion, "
	        + "nPolizaAutomatica, cCentroContable, aEjercicioFiscal, cTipoPoliza, cTipoDocumento, nFolioDocumento) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	private static final String sqlUpdatePoliza = "UPDATE tPoliza WITH(ROWLOCK) SET mTotalCargo = ?, mTotalAbono = ? WHERE aEjercicioFiscal = ? AND cCentroContable = ? AND cTipoPoliza = ? "
	        + "AND nFolioPoliza = ?";

	private static final String sqlInsertMovto = "INSERT INTO tMovimiento (aEjercicioFiscal, cCentroContable, nCuenta, nSubCuenta, nFolioPoliza, cRamo, cUnidadResponsable, "
	        + "nDocRenglon, mMovimiento, cTipoMovimiento, cDescripcionMovPol, cTipoDocumento, fOperacionMovimiento, cFolioDocumentoMovimiento, cCancelaMovimiento, "
	        + "fMovimiento, dConceptoMovimiento, cMoneda, cTipoPoliza, Periodo13, ADEFAS, nTipoAjuste, parcial) "
	        + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	private static final String sqlExisteSaldo = "SELECT * FROM tSaldos WITH(ROWLOCK) "
	        + "WHERE cRamo = ?  AND cUnidadResponsable = ? AND aEjercicioFiscal = ? AND cCentroContable = ? AND nCuenta = ? AND ISNULL(cSubCuenta, '') = ISNULL(?, '')";//TENIA-->ISNULL(cSubCuenta, 'x') = ISNULL(?, 'x')

	private static final String sqlInsertSaldo = "INSERT INTO tSaldos (aEjercicioFiscal, cCentroContable, cRamo, cUnidadResponsable, nCuenta,  cSubCuenta, cMoneda, "
	        + "nMesPrimerMovimiento, nMesArrastre, mSaldoArrastre, mSaldo0, mSaldo1, mHaber1, mDeber1, mSaldo2, mHaber2, mDeber2, mSaldo3, mHaber3, mDeber3, "
	        + "mSaldo4, mHaber4, mDeber4, mSaldo5, mHaber5, mDeber5, mSaldo6, mHaber6, mDeber6, mSaldo7, mHaber7, mDeber7, mSaldo8, mHaber8, mDeber8, "
	        + "mSaldo9, mHaber9, mDeber9, mSaldo10, mHaber10, mDeber10, mSaldo11, mHaber11, mDeber11, mSaldo12, mHaber12, mDeber12, mSaldo13, mHaber13, mDeber13) "
	        + "VALUES (?, ?, ?, ?, ?, isnull(?,''), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";//TENIA ? EN LUGAR DE isnull(?,'')

	private static final String sqlUpdateSaldo = "UPDATE tSaldos WITH(ROWLOCK) SET nMesArrastre = ?, mSaldoArrastre = ?, mSaldo1 = ?, mHaber1 = ?, mDeber1 = ?, "
	        + "mSaldo2 = ?, mHaber2 = ?, mDeber2 = ?, mSaldo3 = ?, mHaber3 = ?, mDeber3 = ?, mSaldo4 = ?, mHaber4 = ?, mDeber4 = ?, "
	        + "mSaldo5 = ?, mHaber5 = ?, mDeber5 = ?, mSaldo6 = ?, mHaber6 = ?, mDeber6 = ?, mSaldo7 = ?, mHaber7 = ?, mDeber7 = ?, "
	        + "mSaldo8 = ?, mHaber8 = ?, mDeber8 = ?, mSaldo9 = ?, mHaber9 = ?, mDeber9 = ?, mSaldo10 = ?, mHaber10 = ?, mDeber10 = ?, "
	        + "mSaldo11 = ?, mHaber11 = ?, mDeber11 = ?, mSaldo12 = ?, mHaber12 = ?, mDeber12 = ?, mSaldo13 = ?, mHaber13 = ?, mDeber13 = ?, nMesPrimerMovimiento = ? "
	        + "WHERE cRamo = ? AND cUnidadResponsable = ? AND aEjercicioFiscal = ? AND cCentroContable = ? AND nCuenta = ? AND ISNULL(cSubCuenta, 'x') = ISNULL(?, '')";// = ISNULL(?, 'x')
	...
	public boolean makeAllAccountingApplication(Connection conn, String[] documentsNames) throws AccountingEngineException {
		...
		setValidaInsuficienciaDeSaldo(false); // Para que NO VALIDE la insuficiencia de saldo
		setValidaDocumentoAplicadoCancelado(false); // Para que NO VALIDE documento aplicado contablemente
		...
		try {
			...
			for (int i = 0; i < documentsNames.length; i++) {
				...
				sql.append(token).append("SELECT '" + documentsNames[i] + "' AS docName, fAplicacion, " + fieldName + " AS folio FROM t" + documentsNames[i] + TABLE_HEADER_SUFIX
				        + "  WITH (NOLOCK) WHERE cDocumentoHaplicado = 'R' AND DATEPART(YEAR, fAplicacion) = (select aEjercicioFiscal from tEjercicioFiscal where cActivo=1)\n");
				token = "UNION\n";
			}
	...
	public boolean makeAllAccountingApplication(Connection conn, String[] documentsNames, String[] fieldsNames) throws AccountingEngineException {
		...
		try {
			for (int i = 0; i < documentsNames.length; i++) {
				log.info(String.format("Procesando documentos de %s...", documentsNames[i]));

				try {
					psSelect = conn.prepareStatement("SELECT " + fieldsNames[i] + " FROM t" + documentsNames[i] + TABLE_HEADER_SUFIX
					        + " WITH (NOLOCK) WHERE cDocumentoHaplicado = 'R'");
					rs = psSelect.executeQuery();

					while (rs.next())
						makeAccountingApplication(conn, documentsNames[i], rs.getString(fieldsNames[i]), "t" + documentsNames[i], fieldsNames[i]);
				} finally {
					try {
						if (rs != null)
							rs.close();
				...
				log.info(String.format("Documentos de %s procesados", documentsNames[i]));
			}
	...
	public boolean makeAccountingApplication(Connection conn, String documentName, String id, String tablePrefix, String fieldName, int initialCapacity)
	        throws AccountingEngineException, DocumentAppliedException {
	...
	public boolean makeAccountingApplication(Connection conn, String documentName, String id, String tableHeader, String tableDetail, String fieldName)
	        throws AccountingEngineException,DocumentAppliedException {
		return makeAccountingApplication(conn, documentName, id, tableHeader, tableDetail, fieldName, INITIAL_CAPACITY);
	}

	public boolean makeAccountingApplication(Connection conn, String documentName, String id, String tableHeader, String tableDetail, String fieldName, int initialCapacity)
	        throws AccountingEngineException, DocumentAppliedException {
	...
	public boolean makeAccountingApplication(Connection conn, String documentName, String id, String tableHeader, String tableDetail, String fieldName,
	        AccountingEventProcessorInterface aep, int initialCapacity) throws AccountingEngineException, DocumentAppliedException {
		...
		try {
			...
			if (log.isDebugEnabled()) {
				...
				log.debug(String.format("Documento %s con encabezado de %,.0f columnas y detalle de %,.0f columnas con %,.0f renglones, cargado en %s",
				        documentName.toUpperCase(),
				        (double) header.size(),
				        (double) (detail.size() > 0 ? detail.get(0).size() : 0),
				        (double) detail.size(),
				        timeFormatter.format(new Date(System.currentTimeMillis() - start))));
			}
			...
			if (log.isInfoEnabled()) {
				...
				log.info(String.format("Procesando documento %s (%s) con %,.0f movimientos...", documentName, id, (double) movements.size()));
			}
			...
			if (retVal = pmr.isSuccess()) {
				...
				if (log.isDebugEnabled()) {
					log.debug(sqlUpdateDocto + " [" + pmr.getFolioPoliza() + ", (" + fieldName + ")" + header.get(fieldName) + "]");
					start = System.currentTimeMillis();
				}
			}else{
			...
			if (log.isInfoEnabled()) {
				DateFormat timeFormatter = new TimeFormat();
				log.info(String.format("Documento %s (%s) con %,.0f movimientos procesado en %s.",
				        documentName,
				        id,
				        (double) movements.size(),
				        timeFormatter.format(new Date(System.currentTimeMillis() - start))));
			}
		} catch (Exception exc) {
			retVal = false;
	...
	public boolean makeAccountingApplicationWithoutEvent(Connection conn, String documentName, String id, String tablePrefix, String fieldName, int initialCapacity)
	        throws AccountingEngineException,DocumentAppliedException {
	...
	public boolean makeAccountingApplicationWithoutEvent(Connection conn, String documentName, String id, String tableHeader, String tableDetail, String fieldName)
	        throws AccountingEngineException,DocumentAppliedException {
		return makeAccountingApplicationWithoutEvent(conn, documentName, id, tableHeader, tableDetail, fieldName, INITIAL_CAPACITY);
	}

	public boolean makeAccountingApplicationWithoutEvent(Connection conn, String documentName, String id, String tableHeader, String tableDetail, String fieldName,
	        int initialCapacity) throws AccountingEngineException,DocumentAppliedException {
	...
	public boolean cancelAccountingApplication(Connection conn, String documentName, String id, String tablePrefix, String fieldName, int initialCapacity)
	...
	public boolean cancelAccountingApplication(Connection conn, String documentName, String id, String tableHeader, String tableDetail, String fieldName)
		    throws AccountingEngineException {
	...
	public boolean cancelAccountingApplication(Connection conn, String documentName, String id, String tableHeader, String tableDetail, String fieldName, String fechaCancelaDocto)
		    throws AccountingEngineException {
		return cancelAccountingApplication(conn, documentName, id, tableHeader, tableDetail, fieldName, INITIAL_CAPACITY, fechaCancelaDocto);
	}

	public boolean cancelAccountingApplication(Connection conn, String documentName, String id, String tableHeader, String tableDetail, String fieldName, int initialCapacity, String fechaCancelaDocto)
	        throws AccountingEngineException {

		boolean retVal = false;
		String sqlMovs = "SELECT m.cCentroContable, m.nCuenta, CASE LEN(LTRIM(RTRIM(m.nSubCuenta))) WHEN 0 THEN NULL ELSE LTRIM(RTRIM(m.nSubCuenta)) END AS nSubCuenta, "
		        + "m.mMovimiento * -1 AS mMovimiento, m.cTipoMovimiento AS cargo, c.NaturalezaCuenta AS naturaleza, c.VerificaSaldo AS verificaSaldo, "
		        + "m.aEjercicioFiscal, m.nDocRenglon, c.dCuenta, case when TipoBalance = 'P' and TipoCuenta = 'P' then 1 else 0 end as bPresupuesto FROM tMovimiento AS m WITH (NOLOCK), tCuentas AS c WITH (NOLOCK) "
		        + "WHERE m.nCuenta = c.nCuenta AND cTipoDocumento = ? AND cFolioDocumentoMovimiento = ? AND cCancelaMovimiento IS NULL "
		        + "ORDER BY m.cCentroContable, m.nCuenta, [nSubCuenta]";
		...
		try {
			...
			log.debug(sqlMovs + " [" + documentName + ", " + id + "]");
...
//			}
			...
			if (retVal = pmr.isSuccess()) {
				...
				log.debug(sqlUpdMovs + " [" + documentName + ", " + id + ", " + header.get("nFolioPoliza") + "]");
				...
				log.debug(sqlUpdDoc + " [" + new Date(now) + ", " + pmr.getFolioPoliza() + ", " + header.get(fieldName) + "]");
	...
	private DocumentAccounting readDocumentData(Connection conn, String id, String tableHeader, String tableDetail, String fieldName, int initialCapacity)
	        throws AccountingEngineException {
		...
		try {
			...
			if (rs.next())
				for (int i = 0; i < maxCol; i++)
					data.put(mrs.getColumnName(i + 1), rs.getString(i + 1) == null ? "" : rs.getString(i + 1));
		} catch (Exception exc) {
			if (exc instanceof AccountingEngineException)
				throw (AccountingEngineException) exc;
			else
	...
	private void readDataDetailFromResultSet(ResultSet rs, List<Map<String, String>> data) throws AccountingEngineException {
		try {
			...
			while (rs.next()) {
				...
				for (int i = 0; i < maxCol; i++)
					row.put(mrs.getColumnName(i + 1), rs.getString(i + 1) == null ? "" : rs.getString(i + 1));
				data.add(row);
			}
		} catch (Exception exc) {
			if (exc instanceof AccountingEngineException)
		...
		while (rs.next()) {
			movements.add(
					new AccountingMovement(
					        ...
					        rs.getString("nSubCuenta"),
					        rs.getDouble("mMovimiento"),
					        "C".equals(rs.getString("cargo")),
					        "D".equals(rs.getString("naturaleza")),
					        "S".equals(rs.getString("verificaSaldo")),
					        rs.getString("cCentroContable"),
					        rs.getString("aEjercicioFiscal"),
					        rs.getInt("nDocRenglon"),
					        rs.getString("nCuenta"), 
					        rs.getBoolean("bPresupuesto"),
					        ""
			        )
	...
	*/
	...
	private ProcessMovementsResult processMovements(Connection conn, String documentName, String tableHeader, String fieldName, List<AccountingMovement> movements,
        Map<String, String> values, int nFolioPoliza) throws AccountingEngineException {
		return processMovements(conn, documentName, tableHeader, fieldName, movements, values, nFolioPoliza, fechaCancelaDocto);
	}
	
	private ProcessMovementsResult processMovements(Connection conn, String documentName, String tableHeader, String fieldName, List<AccountingMovement> movements,
	        Map<String, String> values, int nFolioPoliza, String fechaCancelaDocto) throws AccountingEngineException {
		...
		try {
			...
			log.info(String.format("Procesando %,.0f movimientos de %s(%s)...", (double) movements.size(), documentName, values.get(fieldName)));
			...
			log.info(String.format("%,.0f movimientos de %s(%s) procesados en %s",
			        (double) movements.size(),
			        documentName,
			        values.get(fieldName),
			        timeFormatter.format(new Date(endTime - startTime))));
			...
			log.info(String.format("Procesando %,.0f saldos de %s(%s)...", (double) balancesToUpdate.size(), documentName, values.get(fieldName)));
			...
			log.info(String.format("%,.0f saldos de %s(%s) procesados en %s",
			        (double) balancesToUpdate.size(),
			        documentName,
			        values.get(fieldName),
			        timeFormatter.format(new Date(endTime - startTime))));
		} catch (Exception exc) {
	...
	private synchronized int getNextFolioPoliza(Connection conn, String ejercicioFiscal, String centroContable, String tipoPoliza) throws AccountingEngineException {
		...
		try {
			...
			if (rs.next()){
				...
				log.debug(sqlUpdSeq + " [" + retVal + ",SEQ_POLIZA_" + ejercicioFiscal + "_" + centroContable + "_" + tipoPoliza + "]");
...
*/
	private void addPoliza(Connection conn, PreparedStatement psInsertPoliza, String documentName, String fieldName, int folioPoliza, double cargo, double abono,
	        Map<String, String> values, AccountingMovement mov, String fechaCancelaDocto) throws Exception {
		...
		psInsertPoliza.setInt(1, folioPoliza); // nFolioPoliza
		psInsertPoliza.setTimestamp(2, new Timestamp(now)); // fCreacion
		psInsertPoliza.setString(3, values.get("cDescripcionPoliza")); // cDescripcionPoliza (TODO cDescripcionPoliza en todos los encabezados de documentos)
		psInsertPoliza.setDouble(4, Math.truncate(cargo, 2)); // mTotalCargo
		psInsertPoliza.setDouble(5, Math.truncate(abono, 2)); // mTotalAbono
		psInsertPoliza.setInt(6, cal.get(Calendar.MONTH) + 1); // nMes (Para Calendar el mes de ENERO empieza en 0) (TODO fAplicacion en todos los encabezados de documentos)
		//psInsertPoliza.setString(7, mov.getCuenta()); // nCuenta
		psInsertPoliza.setNull(7, Types.VARCHAR); // nCuenta
		psInsertPoliza.setDate(8, fAplicacion); // fAplicacion (fAplicacion en todos los encabezados de documentos)
		psInsertPoliza.setInt(9, getPolizaAutomatica(conn, documentName, values.get("cTipoPoliza"))); // nPolizaAutomatica (TODO cTipoPoliza en todos los encabezados de documentos)
		psInsertPoliza.setString(10, mov.getCentroContable()); // cCentroContable
		psInsertPoliza.setString(11, mov.getEjercicioFiscal()); // aEjercicioFiscal
		psInsertPoliza.setString(12, values.get("cTipoPoliza")); // cTipoPoliza (TODO cTipoPoliza en todos los encabezados de documentos)
		psInsertPoliza.setString(13, values.get("cTipoDocumento")); // cTipoDocumento
		psInsertPoliza.setString(14, values.get(fieldName)); // nFolioDocumento
		...
		if (log.isDebugEnabled()) {
			...
			msg.append("\n[")
			        .append(folioPoliza)
			        .append(", ")
			        .append(new Timestamp(now))
			        .append(", ")
			        .append(values.get("cDescripcionPoliza"))
			        .append(", ")
			        .append(Math.truncate(cargo, 2))
			        .append(", ")
			        .append(Math.truncate(abono, 2))
			        .append(", ")
			        ...
			        .append(fAplicacion)
			        .append(", ")
			        .append(getPolizaAutomatica(conn, documentName, values.get("cTipoPoliza")))
			        .append(", ")
			        .append(mov.getCentroContable())
			        .append(", ")
			        .append(mov.getEjercicioFiscal())
			        .append(", ")
			        .append(values.get("cTipoPoliza"))
			        .append(", ")
			        .append(values.get("cTipoDocumento"))
			        .append(", ")
			        .append(values.get(fieldName))
			        .append("]");
			log.debug(msg);
		}
	}

	private void updatePoliza(PreparedStatement psUpdatePoliza, String ejercicioFiscal, String centroContable, String tipoPoliza, int folioPoliza, double cargos, double abonos)
	        throws Exception {
		...
		if (log.isDebugEnabled()) {
			...
			msg.append("\n[")
			        .append(Math.truncate(cargos, 2))
			        .append(", ")
			        .append(Math.truncate(abonos, 2))
			        .append(", ")
			        .append(ejercicioFiscal)
			        .append(", ")
			        .append(centroContable)
			        .append(", ")
			        .append(tipoPoliza)
			        .append(", ")
			        .append(folioPoliza)
	...
	private void updateSaldo(Connection conn, PreparedStatement psInsertSaldo, PreparedStatement psUpdateSaldo, double cargos, double abonos, AccountingMovement mov,
		...
		try {
			...
			psExisteSaldo.setString(2, values.get("cUnidadResponsableContable")); // cUnidadResponsableContable
			...
			log.debug(sqlExisteSaldo + " [" + values.get("cRamo") + ", " + values.get("cUnidadResponsableContable") + ", " + mov.getEjercicioFiscal() + ", "
			        + (mov.getbPresupuesto() ? "0" : mov.getCentroContable()) + ", " + mov.getCuenta() + ", " + mov.getSubCuenta() + "]");

			rs = psExisteSaldo.executeQuery();

			if (rs.next()) {
				double[] deber = new double[] { rs.getDouble("mDeber1"), rs.getDouble("mDeber2"), rs.getDouble("mDeber3"), rs.getDouble("mDeber4"), rs.getDouble("mDeber5"),
				        rs.getDouble("mDeber6"), rs.getDouble("mDeber7"), rs.getDouble("mDeber8"), rs.getDouble("mDeber9"), rs.getDouble("mDeber10"), rs.getDouble("mDeber11"),
				        rs.getDouble("mDeber12"), rs.getDouble("mDeber13") };
				double[] haber = new double[] { rs.getDouble("mHaber1"), rs.getDouble("mHaber2"), rs.getDouble("mHaber3"), rs.getDouble("mHaber4"), rs.getDouble("mHaber5"),
				        rs.getDouble("mHaber6"), rs.getDouble("mHaber7"), rs.getDouble("mHaber8"), rs.getDouble("mHaber9"), rs.getDouble("mHaber10"), rs.getDouble("mHaber11"),
				        rs.getDouble("mHaber12"), rs.getDouble("mHaber13") };
				double[] saldo = new double[] { rs.getDouble("mSaldo1"), rs.getDouble("mSaldo2"), rs.getDouble("mSaldo3"), rs.getDouble("mSaldo4"), rs.getDouble("mSaldo5"),
				        rs.getDouble("mSaldo6"), rs.getDouble("mSaldo7"), rs.getDouble("mSaldo8"), rs.getDouble("mSaldo9"), rs.getDouble("mSaldo10"), rs.getDouble("mSaldo11"),
				        rs.getDouble("mSaldo12"), rs.getDouble("mSaldo13") };
				...
				if (mesArrastre >= (mesAplicacion + 1)) {
					...
					for (int i = mesArrastre; i <= mesAplicacion; i++)
						saldo[i] = saldoArrastre;
				...
				psUpdateSaldo.setInt(index++, mesArrastre); // nMesArrastre
				psUpdateSaldo.setDouble(index++, Math.truncate(saldoArrastre, 2)); // mSaldoArrastre (diferencia dependiendo de naturaleza)

				for (int i = 0; i < 13; i++) {
					psUpdateSaldo.setDouble(index++, Math.truncate(saldo[i], 2)); // mSaldo(1..13)
					psUpdateSaldo.setDouble(index++, Math.truncate(haber[i], 2)); // mHaber(1..13)
					psUpdateSaldo.setDouble(index++, Math.truncate(deber[i], 2)); // mDeber(1..13)
				}

				psUpdateSaldo.setInt(index++, mesPrimerMovimiento);

				psUpdateSaldo.setString(index++, values.get("cRamo")); // cRamo
				psUpdateSaldo.setString(index++, values.get("cUnidadResponsableContable")); // cUnidadResponsableContable
				psUpdateSaldo.setString(index++, mov.getEjercicioFiscal()); // aEjercicioFiscal
				psUpdateSaldo.setString(index++, mov.getbPresupuesto() ? "0" : mov.getCentroContable()); // cCentroContable
				psUpdateSaldo.setString(index++, mov.getCuenta()); // nCuenta
				psUpdateSaldo.setString(index++, mov.getSubCuenta()); // cSubCuenta
				...
				if (log.isDebugEnabled()) {
					...
					msg.append(", ")
					        .append(mesPrimerMovimiento)
					        .append(", ")
					        .append(values.get("cRamo"))
					        .append(", ")
					        .append(values.get("cUnidadResponsableContable"))
					        .append(", ")
					        .append(mov.getEjercicioFiscal())
					        .append(", ")
					        .append(mov.getbPresupuesto() ? "0" : mov.getCentroContable())
					        .append(", ")
					        .append(mov.getCuenta())
					        .append(", ")
					        .append(mov.getSubCuenta())
					        .append("]");
				...
				if (validaInsuficienciaDeSaldo() && mov.isVerificaSaldo() && (diferencia < 0d))
					throw new AccountingEngineException(String.format("No se puede reducir el importe %,.2f en clave inexistente %s (C.C. %s, Cta. %s [%s])",
					        diferencia,
					        mov.getSubCuenta(),
					        mov.getbPresupuesto() ? "0" : mov.getCentroContable(),
					        mov.getDescCuenta(),
					        mov.getCuenta()));

				psInsertSaldo.setString(index++, mov.getEjercicioFiscal()); // aEjercicioFiscal
				// ++++++++++++++++++++++++
				// NOTA: SOLO PARA CONAGUA
				// Las cuentas que empiezan con 8 su centro contable es 0
				// ++++++++++++++++++++++++
				psInsertSaldo.setString(index++, mov.getbPresupuesto() ? "0" : mov.getCentroContable()); // cCentroContable
				psInsertSaldo.setString(index++, values.get("cRamo")); // cRamo
				psInsertSaldo.setString(index++, values.get("cUnidadResponsableContable")); // cUnidadResponsableContable
				psInsertSaldo.setString(index++, mov.getCuenta()); // nCuenta
				psInsertSaldo.setString(index++, mov.getSubCuenta()); // cSubCuenta
				psInsertSaldo.setString(index++, null); // cMoneda (TODO cMoneda y nTipoCambio en todos los encabezados de documentos)
				psInsertSaldo.setInt(index++, mesAplicacion + 1); // nMesPrimerMovimiento (El mes de fAplicacion)
				psInsertSaldo.setInt(index++, mesAplicacion + 1); // nMesArrastre (El mes de fAplicacion)
				psInsertSaldo.setDouble(index++, Math.truncate(diferencia, 2)); // mSaldoArrastre (diferencia dependiendo de naturaleza)
				psInsertSaldo.setDouble(index++, 0); // mSaldo0

				for (int i = 0; i < 13; i++) {
					psInsertSaldo.setDouble(index++, (i == mesAplicacion) ? Math.truncate(diferencia, 2) : 0); // mSaldo(1..13)
					psInsertSaldo.setDouble(index++, (i == mesAplicacion) ? Math.truncate(abonos, 2) : 0); // mHaber(1..13)
					psInsertSaldo.setDouble(index++, (i == mesAplicacion) ? Math.truncate(cargos, 2) : 0); // mDeber(1..13)
				}
				...
				if (log.isDebugEnabled()) {
					...
					msg.append("\n[")
					        ...
					        .append("null")
					        .append(", ")
					        .append(mesAplicacion + 1)
					        .append(", ")
					        .append(mesAplicacion + 1)
					        .append(", ")
					        .append(Math.truncate(diferencia, 2))
					        .append(", ")
					        .append(0)
					        .append(", ");

					for (int i = 0; i < 13; i++) {
						msg.append((i == mesAplicacion) ? Math.truncate(diferencia, 2) : 0)
						        .append(", ")
						        .append((i == mesAplicacion) ? Math.truncate(abonos, 2) : 0)
						        .append(", ")
						        .append((i == mesAplicacion) ? Math.truncate(cargos, 2) : 0);
					}
	...
	private void addMovement(PreparedStatement psInsertMovto, String fieldName, AccountingMovement mov, Map<String, String> values) 
		throws Exception {
		addMovement(psInsertMovto,  fieldName, mov, values, fechaCancelaDocto);
	}
	...
	private void addMovement(PreparedStatement psInsertMovto, String fieldName, AccountingMovement mov, Map<String, String> values, String fechaCancelaDocto) throws Exception {
		...
		psInsertMovto.setString(1, mov.getEjercicioFiscal()); // aEjercicioFiscal
		// psInsertMovto.setString(2, mov.getCentroContable()); // cCentroContable
		psInsertMovto.setString(2, mov.getCentroContable()); // cCentroContable
		psInsertMovto.setString(3, mov.getCuenta()); // nCuenta
		psInsertMovto.setString(4, mov.getSubCuenta()); // nSubCuenta
		psInsertMovto.setInt(5, mov.getFolioPoliza()); // nFolioPoliza
		psInsertMovto.setString(6, values.get("cRamo")); // cRamo
		psInsertMovto.setString(7, values.get("cUnidadResponsableContable")); // cUnidadResponsableContable
		psInsertMovto.setInt(8, mov.getDocRenglon()); // nDocRenglon
		//psInsertMovto.setInt(9, -1); // nConsecutivoMovimiento (Es auto-incrementable tipo identity)
		psInsertMovto.setDouble(9, Math.truncate(mov.getMovimiento(), 2)); // mMovimiento
		psInsertMovto.setString(10, mov.getTipoMovimiento()); // cTipoMovimiento
		psInsertMovto.setString(11, values.get("cDescripcionMovPol")); // cDescripcionMovPol
		psInsertMovto.setString(12, values.get("cTipoDocumento")); // cTipoDocumento
		psInsertMovto.setTimestamp(13, new Timestamp(now)); // fOperacionMovimiento
		psInsertMovto.setString(14, values.get("cFolioDocumentoMovimiento")); // cFolioDocumentoMovimiento
		//psInsertMovto.setString(15, null); // cCancelaMovimiento
		psInsertMovto.setNull(15, Types.CHAR); // cCancelaMovimiento (valor inicial NULL)
		psInsertMovto.setTimestamp(16, fMovimiento); // fMovimiento
		psInsertMovto.setString(17, values.get("dConceptoMovimiento")); // dConceptoMovimiento
		psInsertMovto.setString(18, values.get("cMoneda")); // cMoneda
		psInsertMovto.setString(19, values.get("cTipoPoliza")); // cTipoPoliza
		psInsertMovto.setString(20, values.get("Periodo13") == null ? "N" : values.get("Periodo13")); // Periodo13
		psInsertMovto.setString(21, values.get("ADEFAS") == null ? "N" : values.get("ADEFAS")); // ADEFAS
		psInsertMovto.setString(22, values.get("nTipoAjuste") == null ? "0" : values.get("nTipoAjuste")); // nTipoAjuste
		psInsertMovto.setString(23, mov.getParcial() == null ? "N" : mov.getParcial()); // Parcial
		...
		if (log.isDebugEnabled()) {
			...
			msg.append("\n[")
			        ...
			        .append(values.get("cUnidadResponsableContable"))
			        .append(", ")
			        .append(mov.getDocRenglon())
			        .append(", ")
			        .append(Math.truncate(mov.getMovimiento(), 2))
			        .append(", ")
			        .append(mov.getTipoMovimiento())
			        .append(", ")
			        
			        .append(values.get("cDescripcionMovPol"))
			        .append(", ")
			        .append(values.get("cTipoDocumento"))
			        .append(", ")
			        .append(fMovimiento)
			        ...
			        .append(", ")
			        .append(values.get("dConceptoMovimiento"))
			        .append(", ")
			        .append(values.get("cMoneda"))
			        .append(", ")
			        .append(values.get("cTipoPoliza"))
			        .append(", ")
			        .append(values.get("Periodo13"))
			        .append(", ")
			        .append(values.get("ADEFAS"))
			        
			        .append(", ")
			        .append(values.get("nTipoAjuste"))
			        .append(", ")
			        .append(mov.getParcial() == null ? "N" : mov.getParcial())
			        .append("]");
