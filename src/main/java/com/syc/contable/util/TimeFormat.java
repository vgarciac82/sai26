...
public class TimeFormat extends DateFormat {
	...
	private static final long serialVersionUID = 3595688393642933614L;
	private static final long ONE_MILLISECOND = 1l;
	private static final long ONE_SECOND = ONE_MILLISECOND * 1000l;
	...
	private SimpleDateFormat millisecondsFormat = new SimpleDateFormat("S'ms'");
	...
	public StringBuffer format(Date date, StringBuffer toAppendTo, FieldPosition fieldPosition) {
		...
		if (time >= ONE_MILLISECOND && time % ONE_SECOND != 0) {
			millisecondsFormat.format(date, toAppendTo, fieldPosition);
		}
