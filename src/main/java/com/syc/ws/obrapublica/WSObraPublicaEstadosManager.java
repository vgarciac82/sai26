...
public class WSObraPublicaEstadosManager {
	...
	private static Logger log = Logger.getLogger(WSObraPublicaEstadosManager.class);
	...
	private static int validaEstatusWS(Connection conn) throws SQLException, JSONException {
		...
		try {
			String query =  " SELECT * FROM CG_GRUPO_PROPIEDADES WITH (NOLOCK) " +
				           	" WHERE G_NOMBRE = 'WS_OBRAPUBLICA' " + 
				           	" AND GP_NOMBRE = 'Activa_WS_ObraPublica' AND GP_VALOR = 'TRUE'";
	...
	private static JSONObject ObtenerObraPublicaEstadosJson(Connection conn, int nIdEstado) throws Exception {
		...
		String url = urlWS_ObraPublicaEstados(conn); //"http://10.61.0.9:8080/conafor-real-estate-ws-app/services/realEstate/getRealEstatesInState";
