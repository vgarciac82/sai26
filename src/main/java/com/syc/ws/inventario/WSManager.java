...
public class WSManager {
	...
	private static int sendAdquisitionJson(Connection conn, int nFolio, String cIDPagoDiverso, String cIdRecepMat, Usuario u, int oper, String unidadEjecutora, int nFolioPoliza, String cc, int entradaAlmacen, String capitulo, String rutaFortimax) throws Exception {
		...
		String url = urlWS(conn, oper); // "http://10.0.0.34:8080/inventory-ws-app/services/inventory/transaction/inputAcquisition";
		...
		jsonObj.put("purchaseReceptionCode", cIdRecepMat);// cIdRecepMat
	...
	private static int sendPayAdquisitionJson(Connection conn, String cIdRecepMat, String cIDPagoDiverso, Usuario u, int oper, String unidadEjecutora) throws Exception {
		...
		String url = urlWS(conn, oper); // "http://10.0.0.34:8080/inventory-ws-app/services/inventory/transaction/payAcquisition";
		String metodo = "POST", tipoRespuesta = "application/json;charset=UTF-8";
		String respWS = "";
		JSONObject jsonObj = new JSONObject();
	...
	private static int sendCancelAdquisitionJson(Connection conn, int nFolioPago, String cIdRecepMat, String cIDPagoDiverso, Usuario u, int oper, String unidadEjecutora) throws Exception {
		...
		String url = urlWS(conn, oper); // "http://10.0.0.34:8080/inventory-ws-app/services/inventory/transaction/cancelAcquisition";
		String metodo = "POST", tipoRespuesta = "application/json;charset=UTF-8";
		String respWS = "";
		JSONObject jsonObj = new JSONObject();
		...
		jsonObj.put("purchaseFolio", cIDPagoDiverso);
		jsonObj.put("purchaseReceptionCode", cIdRecepMat);// cIdRecepMat
		jsonObj.put("notes", "Cancelación de prueba."); //idTransaction
	...
	private static JSONArray jsonFacturas(Connection conn, String nfoliopago) throws SQLException, JSONException {
		PreparedStatement pstmnt = null;
		String query = "SELECT	cfactura AS folio, CONVERT(VARCHAR(10), Getdate(), 120) AS date," 
				+ " 		crfcfactura AS Proveedor, motrosimpuestos AS otherTaxes, mimporteiva AS totalIva, mimporteconiva as total, nfoliopago, cRazonSocial " 
				+ " FROM   tpagofactura WITH(nolock) "
			+ " WHERE  ctipopago = 'PAGODIVERSO'  AND nfoliopago = ? ";
		ResultSet rs = null;
	...
	private static int cambiaEstatusRecepcion(Connection conn, int nStatusRecepcion, int nFolioPagoDiverso) throws Exception {
		String query = "UPDATE	mRecepcionpMat " + "   SET	nIdEstadoRecepMat = ? " + " WHERE nIdEstadoRecepMat = 3 " + "   AND	cIdRecepMat = (SELECT ISNULL(cIdRecepMat,'') FROM tPAGODIVERSOEncabezado WHERE nFolioPAGODIVERSO = ?) "
			+ "	AND cIdpedContDef = (SELECT cFolioPAGODIVERSO FROM tPAGODIVERSOEncabezado WHERE nFolioPAGODIVERSO = ? )";
	...
	public static RespuestaWS generatePublicWork(Connection conn, String nFolio,Usuario u) throws Exception {
		...
		try {
			...
			int oper = 3;// la operacion 3 es la url ws de nueva obra
			String url = urlWS(conn, oper); // "http://10.0.0.34:8080/inventory-ws-app/services/inventory/transaction/inputAcquisition";
			String metodo = "POST", tipoRespuesta = "application/json;charset=UTF-8";
			String respWS = "";

			// Validar si el ws está encendido
			int WsActivo = 0; //validaEstatusWS(conn);
			if (WsActivo != 0) {
				log.info("El Web service se encuentra configurado para que no se ejecute.");
			...
			if (rs.next()) {
				...
				//Guardar en bitacora.
				query="insert into tBitacoraGeneratePublickWork (nIdPublicWorkSAI,nIdRealEstate,cNumContrato,cUrlWS,fFechaCaptura,nCode,cEstatus,nIdPublicWork,cLogin) "
					+" values("+rs.getInt(1)+","+rs.getInt(2)+",'"+rs.getString(8)+"','"+url+"',GETDATE(),"+jsonrespWS.getInt("code")+",'"+jsonrespWS.getString("estatus")+"',"+jsonrespWS.getInt("idRePublicWork")+",'"+u.getLogin()+"')";
	...
	public static RespuestaWS generatePublicWorkPartial( Connection conn, String folioSAI, int numEstimacion, boolean esCapitalizable, boolean ultimaEstimacion,Usuario u  ) throws Exception {
		RespuestaWS respuestaWS = null;
		String query =  "SELECT	compromisoObra.nidRePublicWork AS idRePublicWork,"
						+"		compromisoObra.nFolioOPComHeader AS idPublicWorkSAI, "
						+"		compromisoObra.nIDRealEstate AS idRealEstate,"
						+"		estimacionObra.noestimacion AS estimationNo,"
						+"		 round(estimacionObra.mMontoEstimacionMasIva+estimacionObra.mMontoEstimacionRetencion,2) AS amount"						
						+"  FROM	tObraPublicaCompromisoEncabezado compromisoObra WITH( nolock )"
						+"		INNER JOIN"
						+"		tobrapublicaavancefisico estimacionObra WITH(NOLOCK)"
						+"		ON compromisoObra.FolioSAI = estimacionObra.foliosai"
						+" WHERE	compromisoObra.FolioSAI = ? "
						+"   AND estimacionObra.noestimacion = ?";
		PreparedStatement ps = null;
		ResultSet rs = null;
		JSONObject jsonObj = null;
		JSONObject jsonrespWS = null;
		try {
			respuestaWS = new RespuestaWS();
			int oper = 4;// la operacion 4 es la url ws de nueva estimacion.
			...
			if (rs.next()) {
				...
				//Guardar en bitacora.
				query="insert into tBitacoraGeneratePublickWork (nIdPublicWorkSAI,nIdRealEstate,cNumContrato,cUrlWS,fFechaCaptura,nCode,cEstatus,nIdPublicWork,nIdPublicWorkPartial,cLogin) "
					+" values("+rs.getInt(1)+","+rs.getInt(2)+",'"+folioSAI+"','"+url+"',GETDATE(),"+jsonrespWS.getInt("code")+",'"+jsonrespWS.getString("estatus")+"',"
					+rs.getString("idRePublicWork")+","+jsonrespWS.getInt("idRePublicWork")+",'"+u.getLogin()+"')";
				com.syc.adquisiciones.util.Util.updateQuery(query,conn);
	...
	public static RespuestaWS generatePublicWorkPartial( Connection conn, String caNoContrarrecibo, String folioSAI  ) throws Exception {
		...
		try {
			...
			if (rs.next()) {
				...
				//Guardar en bitacora.
				String queryI = "insert into tBitacoraGeneratePublickWork (nIdPublicWorkSAI,nIdRealEstate,cNumContrato,cUrlWS,fFechaCaptura,nCode,cEstatus,nIdPublicWork,nIdPublicWorkPartial,cLogin) "
					+" values("+rs.getInt(1)+","+rs.getInt(2)+",'"+folioSAI+"','"+url+"',GETDATE(),"+jsonrespWS.getInt("code")+",'"+jsonrespWS.getString("estatus")+"',"
					+rs.getString("idRePublicWork")+","+jsonrespWS.getInt("idRePublicWork")+",'ADMIN')";
						...
						if(rs.getInt("ultimaEstimacion") == 1) {
							...
							String evento = ObraPublicaManager.calculaEvento(conn, folioSAI, respuestaWS.getRegimen(), respuestaWS.getLastAppraisalDate(), caNoContrarrecibo, esCapitalizable);
							 if (!"".equals( evento )) {
								 ...
								 oper = 5;// la operacion 5 es la url ws de la poliza de capitalizacion.
								 url = urlWS(conn, oper);
