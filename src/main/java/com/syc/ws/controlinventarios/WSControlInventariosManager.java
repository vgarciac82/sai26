...
public class WSControlInventariosManager {
	...
	private static Logger log = Logger.getLogger(WSControlInventariosManager.class);
	...
	private static int sendTypeCtrlInventoryJson(Connection conn, int iMonth, int iYear, int iType) throws Exception {
		...
		String url = urlWS_Ctrl_Inventarios(conn, iType); //"http://10.61.0.9:8080/conciliacion-inventory-ws-app/services/sai/getMonthlyConciliationData";
	...
	private static int validaEstatusWS(Connection conn) throws SQLException, JSONException {
		...
		ResultSet rs=null;
		try {
			String query =  " SELECT * FROM CG_GRUPO_PROPIEDADES WITH (NOLOCK) " +
				           	" WHERE G_NOMBRE = 'WS_CONTROL_INVENTARIO' " + 
				           	" AND GP_NOMBRE = 'Activa_WS_CtrlInventario' AND GP_VALOR = 'TRUE'";
	...
	private static int insertaCtrlInventario(Connection conn, int iMonth, int iYear, int iType, Date fAplicacion, String cPartida, double mImporte, 
		int nFolioPoliza, String cTipoPoliza, String cEvento) throws Exception {
		
		PreparedStatement ps = null;
		int iResultado = 0;
		int nAcumulada = 0;
		String sSql = 	" INSERT INTO tSistemaControlInventarios (idTipoConciliacion, nMes, fAplicacion, mImporte, nFolioPoliza, cPartida, cTipoPoliza, cEvento, nAcumulada) " +
						" VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ";
	...
	private static int validaRowCtrlInventario(Connection conn, int iMonth, int iType, String cPartida, int nFolioPoliza, double mImporte, int nAcumulada, String cTipoPoliza) throws Exception {
		...
		String sSql = 	" SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END AS existe FROM tSistemaControlInventarios WITH (NOLOCK) " +
						" WHERE idTipoConciliacion = ? AND nMes = ? AND nFolioPoliza = ? AND cPartida = ? AND mImporte = ? AND nAcumulada = ? AND cTipoPoliza = ? ";
	...
	private static Date devuelveFechaMesAnterior(Connection conn, int iMonth, int iYear) throws Exception {
		...
		String sSql = 	" DECLARE @mydate DATETIME " +
						" SET @mydate = CONVERT(DATETIME, '" + Integer.toString(iYear) + "-" + Integer.toString(iMonth) + "-1', 120) " +
						" SELECT DATEADD(dd,-(DAY(@mydate)),@mydate) AS Fecha ";
