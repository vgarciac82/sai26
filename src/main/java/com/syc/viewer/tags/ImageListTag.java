...
public class ImageListTag extends TagSupport implements ViewerParametersInterface {
	...
	private static final Logger log = Logger.getLogger(ImageListTag.class);
	...
	public int doEndTag() throws JspException {
		...
		finPaginate = ((finPaginate == -1) ? ((filesNames.length > paginateCount) ? paginateCount : filesNames.length)
				: finPaginate);
			...
			if (filesNames.length > 0) {
				if (filesNames[0] != null) {
					...
					for (int i = iniPaginate; i < finPaginate; i++) {
						...
						out.print("<a name=\"imgList\" href=\"VisualizadorDeImagen.jsp?" + getParam() + "=" + selectId
								+ "&" + INDEX_KEY + "=" + i + "\" target=\"viewerFrame\">");
						out.print("<img src=\"" + request.getContextPath() + "/imagestore/" + thumbnail + "?select="
								+ selectId + "&" + THUMBNAIL_KEY + "=true" + "&" + INDEX_KEY + "=" + i + "\" alt=\""
								+ filesNames[i].getName() + "\" border=\"1\">");
