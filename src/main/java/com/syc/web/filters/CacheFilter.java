    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            if (!file.exists() || (file.exists() && current.lastModified() > file.lastModified()) || cacheTimeout < now - file.lastModified()) {
                ...
                // 2003-12-03 BUGFIX : replace assumed "/" with File.seperatorChar to work with windows
                //        name = name.substring(0,name.lastIndexOf("/")==-1?0:name.lastIndexOf("/"));
                name = name.substring(0, name.lastIndexOf(File.separatorChar) == -1 ? 0 : name.lastIndexOf(File.separatorChar));
                new File(name).mkdirs();
                //        ByteArrayOutputStream baos = new ByteArrayOutputStream();
                //        CacheResponseWrapper wrappedResponse = new CacheResponseWrapper(response, baos);
                FileOutputStream fos = new FileOutputStream(file);
                CacheResponseWrapper wrappedResponse = new CacheResponseWrapper(response, fos);
