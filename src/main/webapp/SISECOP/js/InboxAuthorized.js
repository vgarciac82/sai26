		...
		columns: [
			...
			{ data: 'usuario_captura' },
			{
				...
				render: function(data, type) {
					...
					if (type === 'display') {
						return '<span><a href="#" onclick="showRequest(' + data + ');return false;"><i class="fa-solid fa-arrow-right"></i></a></span>'
							+ '<span><a href="#" onclick="printFormat(' + data + ');return false;"><i class="fa-solid fa-print"></i></a></span>'
							;
					}
...
const showRequest = function(folio) {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			parent.window.frames['content-iframe'].location.href = 'ReviewProjectInfo.jsp?folio=' + folio + '&consulta=true';
		} else {
			return;
		}
	});
...
const makeCondition = function() {
	...
	if ($("#executiveUnit").val() == "*")
		return "estatusid IN (10)"
	else
		return "estatusid IN (10) AND  cUnidadEjecutora = '" + $("#executiveUnit").val() + "'";
...
const printFormat = function(id) {
	const url = `anexo2/download?id=${id}`;
