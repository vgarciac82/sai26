		...
		{				
			...
			$('body')		// eventos del body
				...
				.click(
					...
					{
						var objElemento = (e.target || e.srcElement); 
						if (objElemento.className=="btnAyuda") { 
							var szfa = "txt"+$(objElemento).attr("id").substr(3);
							var myObj  = document.getElementById(szfa);
							szMiWhere = $(myObj).attr("filtroAdicional");
						}
						...
						if (objElemento.className=="btnAyuda" &&
							objElemento.disabled == false) 
								// s oprimieron el boton de mostrar grid
								//y esta deshabilitado
						{
							//alert("Detecto click!");
	...
	function subTimerKeyUp(pControl)
	{
		...
		for (i=0; i<arrVsControl.length; i++)
		{
			...
			if ($("#lb" + arrVsControl[i]+ "Desc").attr("className") == "Helloc" )
			{
				// busca las dependencias de txt .. Desc
				
				szValor = " Id_Del = " + $("#txt" + arrVsControl[i]).val();	
				if (szValor == "MAL")
				{
					return;
				}
				
				subWSAyudaTraeDesc($("#txt" + arrVsControl[i]).attr("tabla") ,szValor);
			}
		}	
	}
	...
	{	
		...
		szTemp = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
		szTemp +="<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:q0=\"http://webservice.syc.com\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">\n";
		szTemp +="   <soapenv:Body>\n";
		szTemp +="      <q0:GetAyudaInfo>\n";
		szTemp +="         <q0:in0>" + pTabla + "</q0:in0> \n";
		szTemp +="         <q0:in1>" + pValor + "</q0:in1> \n";
		szTemp +="      </q0:GetAyudaInfo>\n";
		szTemp +="   </soapenv:Body>\n";
		szTemp +="</soapenv:Envelope>\n";
		xmlHttpReq.open('POST', document.location.protocol + "//" + (document.location.host==""?"localhost":document.location.host) + "/gestion_cna/services/Catalogo", true); //http://wscatalogos.syc.com
		xmlHttpReq.setRequestHeader('Content-Type', 'text/xml; charset=utf-8');
		xmlHttpReq.setRequestHeader('SOAPAction', 'http://tempuri.org/gestion_cna/services/Catalogo/GetAyudaInfo');

		xmlHttpReq.onreadystatechange = function() 
		{
			if (xmlHttpReq.readyState == 4) 
			{
				szTemp = xmlHttpReq.responseText.funCambiaLlaves();
...
*/
	...
	function funGeneraWhere()
	{
		...
		//alert("szMiWhere=["+szMiWhere+"]");
		
		if (szMiWhere != undefined &&
			szMiWhere.length != 0) {
			//alert("szMiWhere.length != 0");
			if (szWhere.length != 0) {
				szWhere += " AND ";
			}			
			szWhere += szMiWhere;
	...
	{			
		...
		szTemp +="<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:q0=\"http://webservice.syc.com\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">\n";
		szTemp +="   <soapenv:Body>\n";
		szTemp +="      <q0:GetAyudaDatosGrid>\n";
		szTemp +="         <q0:in0>" + szTabla + "</q0:in0> \n";
		szTemp +="         <q0:in1>" + funGeneraWhere() + "</q0:in1> \n";
		szTemp +="      </q0:GetAyudaDatosGrid>\n";
		szTemp +="   </soapenv:Body>\n";
		szTemp +="</soapenv:Envelope>\n";
		...
		xmlHttpReq.open('POST', document.location.protocol + "//" + (document.location.host==""?"localhost":document.location.host) + "/gestion_cna/services/Catalogo", true); //http://wscatalogos.syc.com
		xmlHttpReq.setRequestHeader('Content-Type', 'text/xml; charset=utf-8');
		xmlHttpReq.setRequestHeader('SOAPAction', 'http://tempuri.org/gestion_cna/services/Catalogo/GetAyudaDatosGrid');

		xmlHttpReq.onreadystatechange = function() 
		{
			if (xmlHttpReq.readyState == 4) 
			{
				var szResponse = xmlHttpReq.responseText.funCambiaLlaves(); 
				szResponse = szResponse.substr(szResponse.indexOf("<table"));
				szResponse = szResponse.substr(0,szResponse.indexOf("</table>")+8);
...
*/
	...
	$.subCreaAyudaGrid = function()
	{
		...
		objDiv.style.overflow = 'auto'; objDiv.style.left = '0px'; objDiv.style.top = '0px'; objDiv.style.width = '0px'; objDiv.style.height = '385px'; //385px
		...
		$(objDiv).append("<div id='divGridToolbar' style='POSITION: relative; LEFT: 0px; TOP: 0px; WIDTH: 100%; HEIGHT: 30px; BACKGROUND-COLOR: #dddddd'>");
		
		$("#divGridToolbar").append("Puede usar el comodin % (Ejemplo: Codigo Postal = 01090%)<BR><img id='btnGridFiltrar' name='btnGridFiltrar' class='btnGrid' src='../imagenes/boton-filter.jpg' title='Buscar registros que cumplan la condicion' style='POSITION: relative; LEFT: 5px; TOP: 5px'>");
		
		$("#divGridToolbar").append("<div id='divConte' style='overflow:auto; POSITION: relative; HEIGHT: 338px; TOP: 8px'>"); //338
		
		$("#divConte").append("<div id='divTXTyTables' style='OVERFLOW: auto; HEIGHT: 314px;'>"); //314
