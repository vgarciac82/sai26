	$.addGridControl = function(t,p) {
		...
		var grid = {
				...
				populate: function() {	
					if (!this.loading) {
						...
						//$.get("http://makoomba.altervista.org/grid/dyndata.php/page/"+this.page+"/JSON",function(xml) { grid.addJSONData(xml) });
					}
				}
			}
		...
		grid.width = 516; //$.getCSS ? $.getCSS(t,"width") : $.getCSS(t,"width"); 
		...
		$(grid.hDiv)
			...
			.append(grid.hTable)
			.prepend('<div class="loading">Cargando... </div>')			
