...
DatePickerControl.show = function()
{
	if (!this.displayed){
		...
		var custoffset = (input.name.substring(0,1) == "t") ? -250 : 0;
		
		if (window.location.href.indexOf('dlg.jsp')>0) custoffset = 0;
		...
		this.calContainer.style.top        = top + input.offsetHeight + this.offsetY + custoffset + "px";
...
DatePickerControl.hide = function()
{
	if (this.displayed){
		...
		try
		{
			if (window.location.href.indexOf('dlg.jsp')>0 && this.inputControl.id.indexOf("Between") < 0 ) 
			{
				if (document.getElementById( this.inputControl.id + "Between"  ).value == "") document.getElementById( this.inputControl.id + "Between"  ).value = this.originalValue;
			}
			if (window.location.href.indexOf('dlg.jsp')>0 && this.inputControl.id.indexOf("Between") > 0 ) 
			{
				if (document.getElementById(this.inputControl.id.substr(0,this.inputControl.id.length-7)).value == "") document.getElementById( this.inputControl.id.substr(0,this.inputControl.id.length-7) ).value = this.originalValue;
			}
		}
		catch (ex)
		{
...
DatePickerControl.relocateButtons = function()
{
	...
	for (key in divElements){
		if (divElements[key].id && divElements[key].id.indexOf(this.buttonIdPrefix) == 0){
			...
			var nTop = getObject.getSize("offsetTop", input); 
			if (window.location.href.indexOf('dlg.jsp')>0) nTop -= 62;
			var nLeft = getObject.getSize("offsetLeft", input);
			calButton.style.top = (nTop + Math.floor((input.offsetHeight-calButton.offsetHeight)/2) + this.buttonOffsetY) + "px";
			var btnOffX         = Math.floor((input.offsetHeight - calButton.offsetHeight) / 2);
			if (this.buttonPosition == "in"){
