...
<%
	...
	if( request.getParameter("fuenteFinancimiento") != null && !"".equals(request.getParameter("fuenteFinancimiento")))
	...
	String destinoGasto = null;
	if( request.getParameter("destinoGasto") != null )
		destinoGasto = request.getParameter("destinoGasto");
		
	String cEsIngreso = null;
	if( request.getParameter("cEsIngreso") != null )
		cEsIngreso = request.getParameter("cEsIngreso");
%>
...
<!-- 		<script type="text/javascript" src="../Generador/js/jquery.dataTables.js"></script> -->
		...
		$(document).ready(function() {
			...
			if( window.opener.frmcontratoDeObra.cIDContratoObra )
				esFederalizado = false;
			else 
				esFederalizado = true;
			
			if ($("#fuenteFinan").val()=="4")
				fFinanciamiento = " AND fuentefinanciamiento=4 ";
			else
				fFinanciamiento = " AND fuentefinanciamiento<>4 ";
				
			if (<%=cEsRadicado%>=="1")
				radicado = " AND cRadicado = 'S' ";
			else
				radicado = " AND cRadicado = 'N' ";
			
			if (<%=cEsIngreso%>=="1"){
				//TODO: Mostrar DIV con Boton para seleccionar el disponible del IFR
				radicado = " AND cRadicado = 'N' ";
				fFinanciamiento = " AND fuentefinanciamiento<>4 ";
			}
			...
			$("#tblEP tbody").click(function(event) {
				...
				if( esFederalizado )
					szWhere =  " (cIdContrato = '" + $("#cIdContrato").val() + "' OR cIdContrato = (SELECT cIdContrato FROM dbo.tCompromisoEncabezado WHERE nFolioCompromiso = "+$("#nFolioCompromiso").val()+")) and cCentroContable = '" + $("#cCentroContable").val() + "' and cTipoContrato = '" + ctipodocto + "' and EP = '" + sEP + "' " + radicado; 
				else
					szWhere =  " (cIdContrato = '" + $("#cIdContrato").val() + "'OR cIdContrato = (SELECT DISTINCT cIdContrato FROM dbo.tCompromisoEncabezado WHERE cIdContrato = '"+$("#cIdContrato").val()+"')) and cCentroContable = '" + $("#cCentroContable").val() + "' and cTipoContrato = '" + ctipodocto + "' and EP = '" + sEP + "' " + radicado; //SASV 25/05/2015 se cambia por nueva vista

				
				var szTabla = "VSALDOSANUALESOBRA";                                                                                         
				$.getJSON("../catalogos/SelectJson.jsp",{Tabla: szTabla, Param: szWhere, MaxReg: 10, ajax: 'false'}, 
					...
					{                      
    					...
    					[ formatCurrency(j[i].Col4), formatCurrency(j[i].Col5), formatCurrency(j[i].Col6), formatCurrency(j[i].Col7), 
    					 ...
    					 formatCurrency(j[i].Col12),formatCurrency(j[i].Col13),
    					 formatCurrency(j[i].Col14), formatCurrency(j[i].Col15), formatCurrency(j[i].Col16),$("#nclavecna").val(j[i].Col0) ]);
			...
			.click(function() {
					...
					window.opener.frmcontratoDeObra.ep.value=window.MultiReporteHija.ep.value ;
					window.opener.frmcontratoDeObra.codSIAFF.value=window.MultiReporteHija.nclavecna.value ;
		...
		//SASV 25/05/2015
		if( window.opener.frmcontratoDeObra.cIDContratoObra )
			window.MultiReporteHija.nFolioCompromiso.value=window.opener.frmcontratoDeObra.cIDContratoObra.value; 
		else
			window.MultiReporteHija.nFolioCompromiso.value=window.opener.frmcontratoDeObra.nFolioCompromiso.value;
		
		window.MultiReporteHija.TO_TIPO_DOCTO.value=window.opener.frmcontratoDeObra.TO_TIPO_DOCTO.value;
		window.MultiReporteHija.cIdContrato.value=window.opener.frmcontratoDeObra.cIdContrato.value;
		window.MultiReporteHija.cCentroContable.value=window.opener.frmcontratoDeObra.cCentroContable.value;
		//window.MultiReporteHija.cUnidadEjecutora.value=window.opener.formPagos.cUnidadResponsable.value;
		...
		if (cunidadej == 'A02'){
			...
			//cunidadej = " and UnidadEjecutora='" + cunidadej + "'";  //URVP.10112014 se comentariza y se agrega siguiente linea para hacer uso de las vistas generadas Usuario/UR/Modulo
		...
		if( esFederalizado )
			urlAjaxSource = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vConsultaCompromisoEP &qw=" + "nFolioCompromiso=" + encodeURIComponent( nFolioCompromiso + cunidadej + fFinanciamiento + w );
		else{
			if("<%=destinoGasto%>"=="CPGM"){
				w = w + " AND UnidadNorativa IN (SELECT cUnidadNormativa FROM tCatUnidadNormativaGreenMex WITH (NOLOCK))";				
			} else {
				w = w + " AND UnidadNorativa NOT IN (SELECT cUnidadNormativa FROM tCatUnidadNormativaGreenMex WITH (NOLOCK))";
			}
			
			urlAjaxSource = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vConsultaCompromisoEP_obra&qw=" + "cIdContrato='" + encodeURIComponent( nFolioCompromiso + "'" + cunidadej + fFinanciamiento + w );
			
		}
			//SASV 25/05/2015 se crea nueva vista para Convenios Modificatorios 
	...
	<form id="MultiReporteHija" name="MultiReporteHija">
		<div id="container" class="container">
			<h1>Estructura Programática</h1>
				...
				<input type="hidden" id="fuenteFinan" name="fuenteFinan" value="<%=cFuenteFinanciamiento%>"/>
