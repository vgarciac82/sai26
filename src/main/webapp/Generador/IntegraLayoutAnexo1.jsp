...
<html>
  <head>
    <title>Layout Solicitud de Recursos x Pagar</title>
	...
	<script type="text/javascript" charset="utf-8">
		...
		$(document).ready(function(){
			...
			//if ('<%=cUR%>' != 'A02') //URVP. SE COMENTARIZA PARA QUE DESHABILITE EL COMBO PARA TODAS LAS UR's
 			...
 			try {
				...
				if (cont!=1){
					...
					if (confirm("Esta seguro de regresar la Integracion: " +$('#sDataFolios').val())){
						$("#u_login").val("<%=user%>");
						$.ajax({
							url: '../gstnmngr/Anexo1ConLayoutServlet',
							dataType: 'json',
							type: "POST",
							...
							error: function(j){								
								alert("No se pudo regresar la consolidada, Intente mas tarde o avise al Administrador.");
								location.reload(true);
							}
						});
					}
		...
		function fnClickAddRow(row) {
			try{
					...
					if (bExiste){
						...
						Monto = Monto + parseFloat(row.cells[4].childNodes[0].toString().replace(/,/g,""));
		...
		function generar(){
 			try {
            	...
            	document.location.href='../gstnmngr/generaLayoutAnexo1';
  	...
  	<body id="dt_example" onLoad="inicio();" >  	
		<div  id="container" >
			<h1>Layout Solicitud de Recursos x Pagar<label id="lbOperacion" style="font-size: 8pt"></label></h1>
					
        	<div id="tabs">        	     		           
        		<ul>
					<li><a href="#tabs-1" >Pendientes de generar Layout</a></li> <!-- onclick="fnGrid($('#cIdUnidadEjecutora').val(), $('#FechaInicial').val(), $('#FechaFinal').val());" -->
				...
				<div id="tabs-1">
		        	...
		        	<table width="100%" border="0">
				        ...
				        <tr>
				        	...
				        	<!-- <td style="font-weight: bold">
				        		<input type="checkbox" id="chkIntegra" name="chkIntegra" disabled checked>&nbsp;Aplicar Integración 
				        	</td>  -->
					...
					<fieldset>
						...
						<jsp:include page="listaAnexoUno.jsp"></jsp:include>
					</fieldset>
					</form>	
				</div>
				<div id="tabs-2">				
				    <fieldset>
				    	...
				    	<jsp:include page="listaAnexoUnoEnviados.jsp"></jsp:include>
					</fieldset>
				</div>
		        <div id="tabs-3">
		        	<table width="100%" border="0">
		        	...
		        	<button id="btnDevuelveConsolidada" onClick="devuelveConsolidada();">Devolver Integraci&oacute;n</button>		        	
				    <fieldset>
						<legend> Pagos enviados a SICOP </legend>
				    	<jsp:include page="listaAnexoUnoConLayout.jsp"></jsp:include>
					</fieldset>
				</div>		        
			</div>
