...
<%
	...
	String aEjercicioFiscal = null;
	if( request.getParameter("ejercicioFiscal") != null ){
		aEjercicioFiscal = request.getParameter("ejercicioFiscal");
	}
...
<html>
<head>
<title>Catálogo de Claves Presupuestales</title>
...
<script type="text/javascript" charset="UTF-8">
		...
		var ejercicioFiscal = <%=aEjercicioFiscal%>;
		...
		$(document).ready(function() {
    		...
    		$("#pbAceptar").button().click(function() {
					...
					if( index >= 0 ){	
	 				...
	 				$("#ep").val(sEP);
 				$("#claveSIAFF").val(sEP.substring(0, 43));

				window.opener.frmContratosPlurianuales.clavePresupuestal.value=window.MultiReporteHija.claveSIAFF.value ;
				window.opener.frmContratosPlurianuales.EP.value=window.MultiReporteHija.ep.value ;
				
				window.close();
    		
    		});// Fin del click del boton aceptar
    		
    		$("#pbCancelar").button().click(function() {
				window.close();
			});// Fin del click del boton cancelar
		...
		if(esRadicado == "S"){
			urlAjaxSource = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vCatalogoEPGrid &qw=" + fFinanciamiento + w;
		}else{
			if (w ===""){
				urlAjaxSource = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vCatalogoEPGrid " ;
			}else {
			
				urlAjaxSource = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vCatalogoEPGrid &qw=" + "  " + w;
			}
		}
		...
		return $("#tblEP").dataTable({
				...
				//aaSorting: [[ 1, "asc" ]] ,
				aoColumns: [
					{ sName: "EjercicioFiscal", bVisible : false },
					{ sName: "Ramo" },
				...
				<tfoot>
					<tr>
						...
						<th><input type="text" name="col16_filter" id="col16_filter"
							onMouseOver="windowStatus('Unidad Norativa')" />
						</th>
					</tr>
				</tfoot>
			</table>
