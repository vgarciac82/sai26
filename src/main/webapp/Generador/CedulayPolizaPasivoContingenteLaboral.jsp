...
<html>
<head>
<title>Cedula y Poliza Pasivo Contingente Laboral</title>
...
$(document).ready(function() {	
	...
	function inittable(){
		...
		oTable= $("#tblcargas").dataTable({
	               ...
	               sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=v_cargaTemporalPCLaboral",			
		  			aaSorting: [[ 0, "asc" ]] ,
		  			aoColumns: [				
		  				{ sName: "cMes"},
		  				{ sName: "folioTempGral" },
	...
	function validaPasivosActualizados(){
		var continuar = true;
		var valFecha = $("#fecha_aplicacion").val();
		var mes;
		
		$("#pasivoActualizado").val("");
		
		if ( $("#TIPO_REPORTE1").attr('checked') ){
			mes = $("#mes").val();
			$("#nMes").val(mes);
		}
		else if ( $("#TIPO_REPORTE2").attr('checked') ){
			mes = valFecha.split("/")[1];
			$("#nMes").val(mes);
		}
		
		queryFormPost( {
			queryName:"existePasivoLaboralActualizado",
			async:false,
			callback:function(){
				if( $("#pasivoCapturado").val() == "0" ){
					Swal.fire({ icon: "warning",
								text: "No se encuentra ningun pasivo laboral actualizado en el mes seleccionado." });
					continuar = false;
				}
			}
		} );
	...
	function eliminarCarga(folio){
		$("#FolioLayout").val(folio);
		queryFormPost("EliminaLayOutPCLaboral", {async: false});
		queryFormPost("EliminaRelacionLayOutPolizaPCLaboral", {async: false});
		inittable();
	}
...
<br/>
	...
	<div id=container style="width: 80%" class="container">		
		...
		<input type="hidden" id="FolioLayout" name="FolioLayout"/>
		<input type="hidden" name="reportPath" id="reportPath" value="<%=reportPath%>" />

		<div class="card-header"> <h3> Cédula y Póliza Pasivo Contingente Laboral </h3> </div>
		<hr class="mt-3"/>																										
			...
			<div class="row">
				...
				<div class="col-12 col-lg-5 col-md-5 col-sm-12">				
					<div class="form-check">
						<input type="radio" name="TIPO_REPORTE" id="TIPO_REPORTE1" class="form-check-input" value = "3" onclick="selecciona(this)" checked>								
						<label for="patrimonial" class="form-check-label">Cédula</label>
					</div>
				</div>
				<div class="col-12 col-lg-5 col-md-5 col-sm-12">
					<div class="form-check">								
						<input type="radio" name="TIPO_REPORTE" id="TIPO_REPORTE2" class="form-check-input" value = "4" onclick="selecciona(this)"/>
			...
			<div id="dcedula">															
				...
				<h5> Lay Out y Polizas mensuales </h5>
				<hr class="mt-3"/>
