...
<%
	...
	boolean esAdminGRM = usuario.getRole("ADMIN_RECMAT") != null ;
	String cCartera  = (request.getParameter("cCartera") != null) ? request.getParameter("cCartera").toUpperCase() : "" ;
	String cPartida  = (request.getParameter("cPartida") != null) ? request.getParameter("cPartida").toUpperCase() : "" ;
	String cPrograma  = (request.getParameter("cPrograma") != null) ? request.getParameter("cPrograma").toUpperCase() : "" ;		
	String esPAOP  = (request.getParameter("esPAOP") != null) ? request.getParameter("esPAOP").toUpperCase() : "" ;		
	String modulo = ((request.getParameter("modulo") != null) ? request.getParameter("modulo").toUpperCase() : "") ;
	...
	if( !StringUtils.isEmpty(modulo) ){
	
		PresupuestoBusinessLogic pbl = new PresupuestoBusinessLogic(GestionInterface.ATT_CONEXION);
		List<String> partidas = pbl.getFiltroPartidasModulo(modulo);
		
		String token = "";
		token = ( StringUtils.isEmpty(token) ? "": "," );
		
		for( int i = 0; i < partidas.size(); i++ ){
			cPartida += token + "'" + partidas.get(i) + "'";
			token = ",";
		}	
...
<!-- 		<script type="text/javascript" src="../Generador/js/jquery.dataTables.js"></script> -->
		...
		var rfc = window.opener.FormContrato.cIDRFC.value;
		var cTipoContrato = window.opener.FormContrato.cDocumento.value;
		var cwhere = "";
		cwhere = " cTipoContrato = '" + cTipoContrato + "' ";
		var cunidadej = "";
		//UEt = UE.substring( UE.length - 2 );
		cunidadej = " and UnidadEjecutora IN ( SELECT ur FROM tVistasUR WITH(NOLOCK) WHERE usuario = '<%=uLogin%>' AND ( modulo = 'MATERIALES' OR modulo = 'TESORERIA' ) )";
		if (UE == 'A02' || <%=esAdminGRM%>){
			cunidadej = "";
		}
		if ("<%=cCartera%>" != "")
		   cwhere += " and Cartera  = '" + "<%=cCartera%>" + "' ";
		if ("<%=cPartida%>" != "")
		   cwhere += " and Partida  IN (" + "<%=cPartida%>" + ") ";
		if ("<%=cPrograma%>" != "")
		   cwhere += " and ProgramaPresupuestario  = '" + "<%=cPrograma%>" + "' ";
		
		if(cTipoContrato == "CNTRTFERALIZADO"){
			var radicadoActivo = window.opener.FormContrato.radicadoActivo.value;
			
			if(radicadoActivo == "S"){				
				cwhere += "  AND Partida = '43301'  ";
			}
			
			if(rfc == "BMN930209927"){
				cwhere += " AND ProgramaGeneral <> '00' "
			}else{
				cwhere += " AND ProgramaGeneral = '00' "
			}
		}
		...
		Bandera=0;
				sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vCatalogoEPGrid2&qw= ((substring(Partida,1," + nPp.toString() + ")='" + Pp + "') OR (substring(Partida,1," + nPp2.toString() + ")='" + Pp2 + "') OR (substring(Partida,1," + nPp3.toString() + ")='" + Pp3 + "') OR (substring(Partida,1," + nPp4.toString() + ")='" + Pp4 + "'))" + cunidadej + w,
		}*/
		return $("#tblEP").dataTable({
				...
				bServerSide: true,
				sServerMethod: "POST",
				sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vCatalogoEPTContrato&qw= (" + cwhere + ")" + cunidadej + w,
				bProcessing: true,
	...
	<form id="MultiReporteHija" name="MultiReporteHija">
		<div id="container" class="container">
			<h1>Estructura Programática</h1>
				...
				<input type="hidden" id="Partida4" name="Partida4" value="" class=""/>
				<input type="hidden" id="Partida5" name="Partida5" value="" class=""/>
				<input type="hidden" id="Partida6" name="Partida6" value="" class=""/>
				<input type="hidden" id="Partida7" name="Partida7" value="" class=""/>
				<input type="hidden" id="Partida8" name="Partida8" value="" class=""/>
				<input type="hidden" id="Partida9" name="Partida9" value="" class=""/>
				<input type="hidden" id="Partida10" name="Partida10" value="" class=""/>
				<input type="hidden" id="Partida11" name="Partida11" value="" class=""/>
				<input type="hidden" id="esPAOP" name="esPAOP" value="<%=esPAOP %>" class=""/>
