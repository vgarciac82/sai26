 ...
 $(document).ready(function (){
	...
	*/
	...
	$("#numPagoAMF").change(function() { 
			...
			$("#numPagoAMF").attr("disabled","disabled");
		    ...
		    buscarEPAMFDetalle("tPagoAMFDetalle","","numPagoAMF = '"+$("#numPagoAMF").val()+"'");
...
function guardarEncDet()
{
	...
	if($("#FacturaAcuerdosAdministracion").val() == ""){
		...
		queryFormPost("tPagoAMF", {async: false });
		queryFormPost("tPagoAMFUpdate", {async: false});
		...
		for(var i = 1; i < count; i++){
			...
			$("#numPagoAMFDet").val($("#numPagoAMF").val());
			
			queryFormPost("tPagoAMFDetalle", {async: false});
			$("#epDet").val("");
			$("#importeDet").val("");
		}	
		
		alert("Guardado Correctamente Con Numero de Pago AMF: "+$("#numPagoAMF").val());
...
function borrarEpPagoAMF(renglon){
	...
	$("#numPagoAMFDelete").val( $("#numPagoAMF").val() );
	queryFormPost("tPAGOAMFDetalle",{async:false});	
	buscarEPAMFDetalle("tPagoAMFDetalle","","numPagoAMF = '"+$("#numPagoAMF").val()+"'")
}
function agregarEP(){
	
		var szTabla = "tPagoAMFDetalle";
		var elParam = "";
		var campos = "numPagoAMF = '"+$("#numPagoAMF").val()+"' AND EP = '"+$("#ep").val()+"'";
		var datos = "sinDatos";
		$.getJSON("../catalogos/SelectJson.jsp",{Tabla: szTabla, Campos:campos, Param: elParam, MaxReg: "5",ajax: 'true'},
			function(j){					
				...
				if(datos == "sinDatos"){
						//Ingresa EP
						queryFormPost("tPagoAMFDetalle", {async: false });
						//Mostrar EP Agregadas
						var campo = "numPagoAMF = '"+$("#numPagoAMF").val()+"'";	
						buscarEPAMFDetalle(szTabla,elParam,campo)
				}else{
						alert("La Estructura Programatica Ya Esta Agregada");
				}
				$("#ep").val("");	
		});
...
function validarEpDetalle(renglon){
	...
	if(importePago < montoTotal){
		...
		$("#numPagoAMFUpdate").val($("#numPagoAMF").val());
		$("#EPUpdate").val($("#ep_"+renglon).val());
		queryFormPost("tPagoAMFUpdateEP",{async:false});
		
		var campo = "numPagoAMF = '"+$("#numPagoAMF").val()+"'";	
		buscarEPAMFDetalle("tPagoAMFDetalle","",campo)
		$("#importeUpdate").val("");
...
function cancelarPago(){
	var numPagoAMF = $("#numPagoAMF").val();
	if($("#nEnviadoSICOP").val() == 0){
		if(confirm("¿ Esta Seguro Que Desea Cancelar El Pago "+numPagoAMF+ " ?")){
			
			var TablaM = "vDocumentosPagoAMF";
			var camposM = "NumPagoAMF = '"+$("#numPagoAMF").val()+"'";
			var elParamM = "";
			var caNoContrarrecibo = "";
			var datosAplicado = "sinDatos";
			$.getJSON("../catalogos/SelectJson.jsp",{Tabla: TablaM, Campos:camposM, Param: elParamM, MaxReg: "5",ajax: 'true'}, 
												
				function(j){					
					var renglon = 0;									
						for(var i = 0; i < j.length; i++){
								caNoContrarrecibo = j[i].Col0;
								datosAplicado = "datos";
						}	
						if(datosAplicado == "datos"){
								alert("No Se Puede Cancelar Porque Ya Esta Aplicado Presupuestalmente");
								return;
						}else{
							...
							var saldo = 0;
							var totalSaldo = 0;
							var szTabla = "CANCELA_TPAGOAMF";
							var campos = "nClaveAMF = '"+$("#FacturaAcuerdosAdministracion").val()+"' AND cFolio = "+$("#numFolioAMF").val()+" AND cUnidadResponsable = '"+$("#cUnidadResponsable").val()+"'" ;
							var elParam = "";
							
							$("#numPagoAMF").removeAttr("disabled");
							$("#FacturaAcuerdosAdministracion").removeAttr("disabled");
							$("#numFolioAMF").removeAttr("disabled");
							...
							$.getJSON("../catalogos/SelectJson.jsp",{Tabla: szTabla, Campos:campos, Param: elParam, MaxReg: "5",ajax: 'true'}, 
									...
									function(j){					
										...
										if(datos == "datos"){
											totalSaldo = Number(quitaFmt($("#importePago").val())) + Number(saldo);
											$("#mSaldoResto").val(totalSaldo);
											queryFormPost("tPagoAMFUpdateEstatus",{async:false}); 
											queryFormPost("tPagoAMFUpdateCancelado",{async:false});
		...
		alert("No Se Puede Cancelar El Pago Porque Ya Esta En Layout");
	}
}
...
function guardar(){
	...
	if($("#FacturaAcuerdosAdministracion").val() == ""){
		...
		alert("Agregar Importe");
		return;
	}else if( parseFloat(importe) > parseFloat(saldo) ){
		alert("El Importe De Pago Es Mayor Al Saldo");
		return;
	}else{ 
		mSaldoResto = saldo - importe;
	  ...
	  <form id="formPagos" name="formPagos">
					  		<div id="container" class="container SyCData">
								<div id="container">
								  	...
								  	<h1>Pago Acuerdo Ministracion De Fondos<label style="font-size: 8pt"></label></h1>
								  	...
								  	<input type="hidden" id="importeDet" name="importeDet">
									  	...
									  	<fieldset> 
									  		<table id="tbl" width="850px" height ="250px">
									  			...
									  			<!--tr><td><input name="FacturaAcuerdosAdministracion" id="FacturaAcuerdosAdministracion" class="AyudaSyC" type="text"  value="" size="1"></td></tr-->
									  			<tr><td>&nbsp;</td></tr>
									  			<tr><td id="uResponsable">Unidad Responsable</td><td><input type="text" id="cUnidadResponsable" name="cUnidadResponsable" value="<%=ur%>" readonly="readonly"></td></tr>
									  			<tr><td>Numero Pago AMF</td><td><input type="text" id="numPagoAMF" name="numPagoAMF" class="AyudaSyC" readonly="readonly"></td></tr>
									  			<tr><td>Clave AMF</td><td><input type="text" id="FacturaAcuerdosAdministracion" name="FacturaAcuerdosAdministracion" class="AyudaSyC" readonly='readonly'></td><td>Saldo</td><td><input type="text" id="mSaldo" readonly='readonly'></td></tr>
									  			<tr><td>Numero Folio AMF</td><td><input type="text" id="numFolioAMF" name="numFolioAMF" readonly='readonly'></td><td>Fecha Vigencia</td><td><input type="text" id="fVigencia" name="fVegencia" readonly='readonly'></td></tr>
									  			<tr><td>Referencia</td><td colspan="3"><input type="text" id="referenciaAMF" name="referenciaAMF" maxlength="60" size="100"></td></tr>
									  			<tr><td>RFC</td><td><input type="text" id="rfcAMF" name="rfcAMF" class="AyudaSyC" readonly='readonly'></td><td>Cuenta Bancaria</td><td><select id="cIdCuentasBancarias" name="cIdCuentasBancarias"></select><input type="text" id="cIdCuentasBancarias1" name="cIdCuentasBancarias1"></td></tr>
									  			<tr><td>Fecha Captura</td><td><input type="text" id="fechaCaptura" name="fechaCaptura" ></td><td>Fecha Pago</td><td><input type="text" id="fechaPago" name="fechaPago"></td></tr>
									  			<tr><td>Importe Pago</td><td><input type="text" id="importePago" name="importePago" value="$0.00" onfocus="Sinfrmt(this.name)" onblur="cambiafrmt(this.name)" onKeyPress="return(onlyNumbers(event))"></td></tr>		  		
									  			<tr><td>&nbsp;</td></tr>
									  	...
									  	<div>
									  		...
									  		<table id="epDetalle1" name="epDetalle1">
												...
												<tr>
													...
													<!-- >td><input type="button" value="Agregar" name="AgregarEP" id="AgregarEP" onclick="agregarEP();"></td -->
