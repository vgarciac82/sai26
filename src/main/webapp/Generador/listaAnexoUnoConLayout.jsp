<script type="text/javascript">
	function fnGridConLayout(pParam, pFechaIni, pFechaFin){
 		...
 		var oTableCuentaConLayout = $('#dt_CuentaConLayout').dataTable({
			...
			bServerSide: true,
			sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vListaAnexo1ConLayout&qw=" + (pParam == 0 ? "" : "UNIDAD_EJECUTORA='" + pParam + "' AND ") + "FECHA_PROGRAMADAQuery BETWEEN convert(datetime,'" + pFechaIni + " 00:00',103) AND convert(datetime,'" + pFechaFin + " 23:59:59',103)",
			bProcessing: true,
...
<div id="container">
	<form name="RGConLayout" id="RGConLayout" action="../gstnmngr/Anexo1ConLayoutServlet" method="post">
		<div>
