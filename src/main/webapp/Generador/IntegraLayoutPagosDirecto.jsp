  ...
  <head>
	...
	<script type="text/javascript" charset="utf-8">
		...
		$(document).ready(function(){
			...
			document.getElementById('layDirecto').checked = true;
			...
			$("#pbCompromiso").button().click(
					function(){
						generarLayoutCompromiso();
					});
			...
			tipointegracion();
			fnGridConLayout($("#cIdUnidadEjecutora1").val(),$("#FechaInicial").val().split('-').reverse().join('/'), $("#FechaFinal").val().split('-').reverse().join('/'));						
		...
		function tipointegracion(){
			tipoLayout=$("input[name='rTipoIntegracion']:checked").val();
			fnGrid($("#cIdUnidadEjecutora").val(), $("#FechaInicial").val().split('-').reverse().join('/'), $("#FechaFinal").val().split('-').reverse().join('/'), tipoLayout);
		}
		...
		function enviar(){
			...
			try {
				...
				document.getElementById('tipoPagoLayout').value = tipoLayout;
 		...
 		function generar() {
 			...
 			try {
            	...
            	document.envioSICOP.action='../gstnmngr/generaLayoutPagosDirectos?archivo=1';
 		...
 		function generaLayoutPagoCompromiso(){

 			try {
        		//var table = document.getElementById('dt_CompLayout');
            	//var rowCount = table.rows.length;
            	$('#dt_CompLayout tbody tr input:checked').each(function(idx, elm){
					var folio = $(this).parent('td').parent('tr').find('td:eq(10)').html();
					$('#sDataH').val($('#sDataH').val() +  folio + ",");
            	...
            	//fnGridCompromisoLayout();
            	document.getElementById('archivo').value = "1";
            	document.getElementById('tipoPagoLayout').value = "COMPROMISO";
            	document.envioSICOP.action='../gstnmngr/generaLayoutPagosDirectos?archivo=1';
				document.envioSICOP.method='POST';
            	document.envioSICOP.submit();
				...
				setTimeout(function() {
					...
					document.getElementById('sDataHLeyenda').value = " ";
					fnGridCompromisoLayout();
					$('button[data-bs-target="#tabs-3-generados"]').trigger('click');
					}, 5000);
 			...
 			$("#PDCompromisoLayout").submit();
 			*/
 		}	
 		
 		function generarLayoutCompromiso() {
 			...
 			try {
            	...
            	document.getElementById('archivo').value = "1";
            	document.envioSICOP.action='../gstnmngr/generaLayoutPDCompromiso';
				document.envioSICOP.method='POST';
            	document.envioSICOP.submit();
 		...
 		function generarDocumentacion(){
 			try {
            	...
            	document.envioSICOP.action='../gstnmngr/generaLayoutPagosDirectos?archivo=2';
				document.envioSICOP.method='POST';
				document.envioSICOP.submit();
            	//document.envioSICOP.submit();
...
<br/>
  	<body id="dt_example" onLoad="inicio();" >
		<div  id="container" class="ms-5" class="container ms-5" style="width: 90%">
			<div class="card-header"> <h3> Layout Pagos Directo </h3> </div>
			<hr class="mt-3"/>
			...
			<div class="row d-flex justify-content-center">
	       		...
	       		<ul class="nav nav-tabs" id="list-opciones">
		            	...
		            	<button class="nav-link active" id="tabs-1" onClick="refreshGrid();" data-bs-toggle="tab" data-bs-target="#tabs-1-pendientes" type="button" role="tab" aria-controls="tabs-pendientes" aria-selected="true">Pendientes de generar Layout</button>
		            </li>
		            <li class="nav-item" role="presentation">
		            	<button class="nav-link" id="tabs-2" onClick="enviar();" data-bs-toggle="tab" data-bs-target="#tabs-2-proceso" type="button" role="tab" aria-controls="tabs-proceso" aria-selected="false">Pagos en proceso SICOP</button>
		            ...
		            <li class="nav-item" role="presentation">
		            	<button class="nav-link" id="tabs-4" data-bs-toggle="tab" data-bs-target="#tabs-4-compromiso" type="button" role="tab" aria-controls="tabs-compromiso" aria-selected="false">Generar layout pagos con compromiso</button>
		            </li>							
				</ul>
				...
				<div class="tab-content mt-3" id="tabContent">		
					<div class="tab-pane fade show active" id="tabs-1-pendientes" role="tabpanel" aria-labelledby="tabs-pendientes">					
				  		<form>
							...
							<div class="row d-flex">								
								<div class="col-12 col-lg-2 col-md-2 col-sm-12 p-1">
									<input type="radio" id="layDirecto" name="rTipoIntegracion" class="form-check-input" value="DIRECTO" onclick="tipointegracion()"> Layout directo
								</div>
				  			...
				  			<h5 class="mt-3"> Pagos sin layout </h5>
							<hr/>
														
							<jsp:include page="listaPagosDirectos.jsp"></jsp:include>
					...
					<div class="tab-pane fade" id="tabs-2-proceso" role="tabpanel" aria-labelledby="tabs-proceso">			
					    ...
					    <jsp:include page="listaPagosDirectosEnviados.jsp"></jsp:include>
			        ...
			        <div class="tab-pane fade" id="tabs-3-generados" role="tabpanel" aria-labelledby="tabs-generados">
						...
						<jsp:include page="listaPagosDirectosConLayout.jsp"></jsp:include>
						
					</div>	
					
					<div class="tab-pane fade" id="tabs-4-compromiso" role="tabpanel" aria-labelledby="tabs-compromiso"><!--Inicio tab-4-->
						
						<h5> Layouts de Pagos con Compromiso </h5>
						<hr class="mt-3"/>
						...
						<div class="row d-flex">
							...
							<div class="col-12 col-lg-1 col-md-1 col-sm-12 p-1">
								<input type="button" id="generaLayoutPCompromiso" name="generaLayoutPCompromiso" style="visibility: hidden" onClick="generaLayoutPagoCompromiso()" value="Generar Layout" class="btn btn-secondary"/>
							</div>
							<div class="col-12 col-lg-1 col-md-1 col-sm-12 p-1">
								<input type="button" id="generaLayoutCompromiso" name="generaLayoutCompromiso" value="Regenerar Layout Compromiso" onClick="reimprimeLayoutCompromiso()" class="btn btn-primary"/>
							</div>
						</div>
				
						<jsp:include page="listaCompromisoPagoDirecto.jsp"></jsp:include>
					</div><!--Fin tab-4-->	
				</div>        
			</div>
