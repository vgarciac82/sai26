<script type="text/javascript">
	function fnGrid(pParam, pFechaIni, pFechaFin){
		...
		var oTableLocal = $('#dt_generados').dataTable({
			...
			bServerSide : true,
			sAjaxSource : window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vlistaRegIng&qw=" + (pParam == 0 ? "" : "UNIDAD_EJECUTORA='" + pParam + "' and ") + "FECHA_PROGRAMADAQuery BETWEEN convert(datetime,'" + pFechaIni + " 00:00',103) AND convert(datetime,'" + pFechaFin + " 23:59:59',103) ",																
			bProcessing : true,
			sPaginationType : "full_numbers",
			bJQueryUI : true,
			aaSorting : [ [ 3, "asc" ] ],
