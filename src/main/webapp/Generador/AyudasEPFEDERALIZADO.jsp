...
oTableSaldos = $("#tblSaldos").dataTable({
			...
			$("#tblEP tbody").click(function(event) {
				...
				var sEP = 
	    					anSelected[0].innerText.substr(0,4)+"."+
...
//		var cunidadej = $('#cUnidadEjecutora').val();
		...
		return $("#tblEP").dataTable({
				...
				bServerSide: true,
				sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vConsultaCompromisoEP &qw=nFolioCompromiso=" + nFolioCompromiso,
				bProcessing: true,
