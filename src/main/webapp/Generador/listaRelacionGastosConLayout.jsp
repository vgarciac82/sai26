<script type="text/javascript">
	...
	function fnGridConLayout(pParam, pFechaIni, pFechaFin){
		...
		var oTableCuentaConLayout = $('#dt_CuentaConLayout').dataTable(
		{
			...
			bServerSide: true,
			sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vListaRelacionGastosConLayout&qw=" + (pParam == 0 ? "" : "UNIDAD_EJECUTORA='" + pParam + "' AND ") + "FECHA_PROGRAMADAQuery BETWEEN convert(datetime,'" + pFechaIni + " 00:00',103) AND convert(datetime,'" + pFechaFin + " 23:59:59',103)",
			aoColumns: [
				...
				{ sName: "sAuxiliarComodin",		bSearchable: false,	bSortable: false, bVisible: true}
			]
	...
	<form name="RGConLayout" id="RGConLayout" action="../gstnmngr/RelacionGastosConLayout" method="post">	
					...
					<table id="dt_CuentaConLayout" class="table table-striped table-bordered" >	
						<thead>
							<tr align="center">
								...
								<th><font size="2">#Integraci�n</font></th>
							</tr>
						</thead>		
						<tbody />					
					</table>
