    		...
    		function fnGrid(pParam, pFechaIni, pFechaFin)
    		{
				if (pParam == 'RHQ') pParam = 0
     			var oTableLocal = $('#dt_generados').dataTable({    				
					...
					bServerSide: true,
					//sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vListaOperacionAjena&qw=" + (pParam == 0 ? "" : "UNIDAD_EJECUTORA='" + pParam + "' and ") + "FECHA_PROGRAMADAQuery BETWEEN convert(datetime,'" + pFechaIni + " 00:00',103) AND convert(datetime,'" + pFechaFin + " 23:59:59',103)",
					sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vListaPagoAMF&qw=" + (pParam == 0 ? "" : "cUnidadResponsable='" + pParam + "' AND ") + "fechaCaptura BETWEEN convert(datetime,'" + pFechaIni + " 00:00',103) AND convert(datetime,'" + pFechaFin + " 23:59:59',103) AND estatus != 'Cancelado' ",
					bProcessing: true,
