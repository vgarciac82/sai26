  ...
  <head>    
	...
	<script type="text/javascript" charset="utf-8">
				...
				<%
					...
					Map botones=nb.getBotones(role,"Contratos","presupuestoContrato");
		...
		function fnClickAddRowC() {
			...
			{
				window.open('../MultiReporteGridSacel.jsp?cIdUsuarioCreacion=' + $('#cIdUsuarioCreacion').val() + '&cIdDocumento=' + $('#cIdContratoMat').val(), 'MultiReporteGridSacel', 'status=1, width=900px, height=680px, left=100px');
			...
			function guardaContrato(){
			 ...
			 if (imgAprobar==0){
				var proc=""+$("#cEjercicio").val()
        			 ...
        			 $.getJSON("../../servlet/PedidoPasivoServlet?operacion=0",{Tabla: "", Param: proc, MaxReg: "", ajax: 'false'}, function(j){
				 		for(var i = 0; i < j.length; i++)
		                	 var col=j[i].Col1
		                switch(col){
						case "0": 
							alert("El Pedido Pasivo se ha aprobado y está en el SAI");
							queryFormPost("mContratoPasivosPedidoCaratulaRead", {async: false});
							document.getElementById("nIdClaveEgresosXContrato").disabled = true;	
							document.getElementById("AgregarEPContrato").disabled = true;
							document.getElementById("imgAprobarPresupuestoCont").disabled = true;
						...
						break;
						case "1":  
							alert("Este PedidoPasivo ya se encuentra aprobado");
						break;
						case "2":  
			...
			if (imgDevolver==0){
			   if(parseInt($("#nIdEstado").val(),10)<3 || parseInt($("#nIdEstado").val(),10)==5 ){
				...
				var proc=""+$("#cEjercicio").val()
					...
					$.getJSON("../../servlet/PedidoPasivoServlet?operacion=1",{Tabla: "", Param: proc, MaxReg: "", ajax: 'true'}, function(j){
				 		for(var i = 0; i < j.length; i++)
		                	 var col=j[i].Col1
		                switch(col){
						case "0":  
							alert("Se ha devuelto el Pedido Pasivo");
							queryFormPost("mContratoPasivosPedidoCaratulaRead", {async: false});
						...
						break;
						case "1":  
							alert("No se puede regresar el PedidoPasivo porque su estado no lo permite");
						break;
						case "2":  
							alert("Ha ocurrido un error al eliminar las EPs del Contrato, por favor contacte a su administrador");
						break;
						case "3":  
							alert("Ha ocurrido un error devolver el Pedido Pasivo, por favor contacte a su administrador");
						break;
						case "4":  
							alert("Ha ocurrido un error al cambiar el estado del Pedido Pasivo, por favor contacte a su administrador");
						break;
	                 	}
				});
			}	
				...
				<tr>
					<td>
						<fieldset style="width:750px" >
							<legend>Presupuesto del Pasivo Pedido</legend>
								<table align="left" cellpadding="2" width="100%">
							    	<tr>
							    		<td align="right" colspan="2">
							    			<img id="imgAprobarPresupuestoCont" src="../imagenes/accept_green.png" style="cursor: pointer" onclick="guardaContrato();" />&nbsp;Aprobar 
											<img id="imgDevolverPresupuestoCont" src="../imagenes/arrow_left_blue_round.png" style="cursor: pointer" onclick="devuelveContrato();"/>&nbsp;Devolver
			    					...
			    					<td style="width: 740px;">
				    					<table height="66" width="98%" border="0">
							                ...
							                <tr> 
							                  ...
							                  <td><input type="button" name="nIdClaveEgresosXContrato" id="nIdClaveEgresosXContrato" size="5" value="..." onclick="buscaClavePresupuestal()" onblur="rellenaCampos();"></td>
							                  <td><input type="button" value="Agregar" name="Add2" id="AgregarEPContrato"	onclick="fnClickAddRowC();"></td>
							                </tr>
							              </table>
			    					</td>
			    				</tr>	
