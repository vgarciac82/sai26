...
<%
	...
	*/
	...
	if (session.getAttribute(GestionInterface.ATT_ContratoEjercicioPasivo) != null) {
		cEjercicio = (String)session.getAttribute(GestionInterface.ATT_ContratoEjercicioPasivo);
		cIdContrato = (String)session.getAttribute(GestionInterface.ATT_ContratoPasivo);
		cIdUnidadEjecutora = (String)session.getAttribute(GestionInterface.ATT_ContratoUnidadEjecPasivo);
		cIdContratoDefinitivo = (String)session.getAttribute(GestionInterface.ATT_ContratoPasivoDefinitivo);
		cIdTipoCambio = (String)session.getAttribute(GestionInterface. ATT_tipoCambioPasivo);
		cITipoContrato = (String)session.getAttribute(GestionInterface. ATT_tipoContratoPasivo);
		cIdSubPartida = (String)session.getAttribute(GestionInterface. ATT_SubpartidaPasivo);
		
		
		
		System.out.println("tipo contrato " +cITipoContrato + cIdContratoDefinitivo+ " "+cEjercicio+"  "+ cIdContrato+ " "+  cIdUnidadEjecutora+ " "+ cIdSubPartida  );
						
	}else 
		response.sendRedirect("Pasivos.jsp?tab=0");
...
<script type="text/javascript" charset="utf-8">
	...
	$(document).ready(function() {
		<%
				...
				Map botones=nb.getBotones(role,"PasivoContrato","precompromisoPasivoContrato");
				Iterator btn = botones.entrySet().iterator();
				while (btn.hasNext()) {
					Map.Entry b = (Map.Entry)btn.next();%>
					$("#<%=b.getValue()%>").attr("disabled", true);<%
					String img=(String) b.getValue();
					if ("imgAprobarpreCompromisoPasivoContrato".equals(img)){  
						%>
						$("#<%=img%>").attr("disabled", false);<%
						imgAprobar=1;
					}
					if ("imgDevolverpreCompromisoPasivoContrato".equals(img)){
						%>
						$("#<%=img%>").attr("disabled", false);<%
						imgDevolver=1; 
					}
					if ("trEditarpreCompromisoPasivoContrato".equals(img)){ 
						editar=1; 
					}	
				}		
			%>
	...
	function setInitConditions(){
		...
		//Se deshabilita el boton de aprobar en caso que ya exista un precompromiso
			document.getElementById("imgAprobarpreCompromisoPasivoContrato").disabled = true;
		...
		if(vEstado != 3){
			...
			document.getElementById("imgDevolverpreCompromisoPasivoContrato").disabled = true;
		}
	...
	function initQueries(){
			...
			queryFormPost("fnMontoNetoContratoPasivoPresupuestoRead", {async: false});
			queryFormPost("fnMontosContratoPresupuestoPasivoRead", {async: false});
		...
		var oTable=$('#dt_suficiencia').dataTable( {
			...
			fnInitComplete: function(oSettings, json) {
				...
				if(vEstado ==2 || vEstado==5 && vVigente==1){
					var editar='<%=editar%>';
					if (editar==0){
						document.getElementById("trEditarpreCompromisoPasivoContrato").style.display="table-row";
					}
				}
			}	
            });
	...
	function aplicaContablemente(){
			...
			if ($("#nTipoPago").val() == '1') {
				...
				var aTrs1 = oTableUrcc.fnGetNodes();
						
				for (var i = 0; i < aTrs1.length; i++) {
					line = oTableUrcc.fnGetData(aTrs1[i])
					var cc = line[0];
					var ur = line[1];
					guardarPrecompromisoDes(cc, ur,nRows,oTable,oTablD,aTrs,vdocren);
				}
			
			$("#nIdEstado").val(3);	vEstado=3;	
			queryFormPost("mContratoPasivoPrecompromisoUpdate", {async: false });
			$('#dt_grabaprecompD').attr('visible', false);
			$('#dt_preCompromiso').attr('disabled', true);
			document.getElementById("imgAprobarpreCompromisoPasivoContrato").disabled = true;
			document.getElementById("imgDevolverpreCompromisoPasivoContrato").disabled = false;
			document.getElementById("trEditarpreCompromisoPasivoContrato").style.display="none";
			actualizaCaratula();
			alert("Contrato Pasivo Precomprometido con folio(s) "+$("#foliosCasoPreCompromiso").val());
	...
	function guardarPrecomprimisoGeneral(){
		...
		if (imgAprobar==0) {
			...
			alert("Pasivo Precomprometido con folio(s) " + $("#foliosCasoPreCompromiso").val());
		...
		{	
				...
				$.ajax({url: '../../servlet/PasivoServlet' , type:'post' , async: false,data:'operacion=2&ur='+ur, dataType: 'json', success: guardaFolio});
				//}
			...
			//aplicacion contable
			$.ajax({url: '../../servlet/PasivoServlet' , type:'post' , async: false,data:'operacion=3', dataType: 'json', success: imprimeRes});
		...
		{	
			...
			//if($("#nFolioPreCompromiso").val()==""){
				$.ajax({url: '../../servlet/PasivoServlet' , type:'post' , async: false,data:'operacion=2', dataType: 'json', success: guardaFolio});
			//}
			//En caso de cualquier error al obtener el folio, sale de la función
			if($("#nFolioPreCompromiso").val()==0)
				return -1;
			...
			$("#fVigencia").val( "<%=vigencia%>" );
			//Genera Encabezado y detalle
			queryFormPost('tPreCompromisoFinECreate', {async: false });
			var j,nColums=12-parseInt($("#mesDisponible").val(),10);
			oTablD.fnClearTable();
			for ( var i=0 ; i<nRows ; i++ ) {
				var aData = oTable.fnGetData( i );
				...
				if (jqInputs.length > 0) {
					for ( j=0 ; j <= nColums ; j++ ) {
						...
						vdocren++ ;
						queryFormPost("tPreCompromisoFinDCreate", {async: false });
						oTablD.fnClearTable();
		...
		//aplicacion contable
			$.ajax({url: '../../servlet/PasivoServlet?nFolioPrecompromiso='+$("#nFolioPreCompromisoE").val()+"&cEjercicio="+$("#aEjercicioFiscal").val()+"&folioCasoPreCompromiso="+$("#folioCasoPreCompromiso").val()+"&cContratoDefinitivo="+$("#cContratoDefinitivo").val(), type:'post' , async: false,data:'operacion=3', dataType: 'json', success: 
			function(j){
		...
		function devuelvePrecompromiso(){
			...
			if (imgDevolver==0){
				...
				queryFormPost("mContratoPasivosContratoCaratulaRead", {async: false});
				if($("#nIdEstado").val()==("2")){
					alert("El contratoPasivo ha sido rechazado por el usuario de ventanilla");
				...
				queryFormPost("usuarioVentanillaReadContratoPasivo", {async: false});
				...
				if($("#nIdEstado").val()!=("4")){
					...
					if(res){
			       	 ...
			       	 $.ajax({url: '../../servlet/PasivoServlet?nFolioPrecompromiso='+$("#nFolioPreCompromiso").val()+"&cEjercicio="+$("#cEjercicio").val()+"&cContratoDefinitivo="+$("#cContratoDefinitivo").val(), type:'post' , async: false,data:'operacion=4&tipoPago=' + $("#nTipoPago").val(), dataType: 'json', success: eliminaPrecompromiso});
			   
					}else
						return;
		        }else{
					actualizaCaratula();
					alert("El Contrato Pasivo no se puede devolver porque ya ha sido APROBADO y esta COMPROMETIDO");
				}
	...
	function eliminaPrecompromiso(j){
			...
			//Bitácora
			$("#cAccion").val("DEVUELVE_PRECOMPROMISO_PASIVOCONTRATO");
			$("#cIdDocumento").val($("#cContratoDefinitivo").val());
			 queryFormPost("sp_mBitacoraMovimientosCreate", { async : false});
			 actualizaCaratula();
			 window.location = "Pasivos.jsp?tab=" + 3;
	...
	<div id="container" class="container">
		<table align="left" width="750px">
				<tr><td>
						...
						<legend>Pre-Compromiso de los Contratos Pasivos</legend>
							<table align="left" cellpadding="2" width="100%">
						    	<tr>
						    		<td align="right" colspan="2">
						    			<img id="imgAprobarpreCompromisoPasivoContrato" src="../imagenes/accept_green.png" style="cursor: pointer" onclick="guardarPrecomprimisoGeneral();" />&nbsp;Pre-Comprometer
						    			<img id="imgDevolverpreCompromisoPasivoContrato" src="../imagenes/arrow_left_blue_round.png" style="cursor: pointer" onclick="devuelvePrecompromiso();"/>&nbsp;Devolver
										<img id="imgSalir" src="../imagenes/cancel_round.png" style="cursor: pointer"  onclick="window.location = 'Pasivos.jsp?tab=1';"/>&nbsp;Salir
						    		</td>
						    	</tr>
						    	<tr>
					...
					<tr id="trEditarpreCompromisoPasivoContrato" style="display: none"><td align="left" ><a href="javascript:void(0)" id="edit">editar</a></td></tr>
	        ...
	        <input type="hidden" name="cIdUnidadEjecutora" id="cIdUnidadEjecutora" value="<%=cIdUnidadEjecutora%>"/>
			 ...
			 <input type="hidden" name="cIdSubPartida" id="cIdSubPartida" value="<%=cIdSubPartida%>"/>
