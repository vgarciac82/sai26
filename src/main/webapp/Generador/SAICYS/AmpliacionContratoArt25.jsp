...
<title>Ampliación</title>
	<script type="text/javascript" charset="utf-8">
		...
		function muestraLineas(){
			//$("#esCucopGasolina").val(0);
			if($("#cIdTipoContrato").val()=="CS" || $("#esCucopGasolina").val()==1){
		...
		function muestraBotones(){
			...
			if($("#cEstadoAmpliacion").val()=='EN SAI SIN PRESUPUESTO'){
				...
				$("#imgAprobarpreCompromisoContAmp").show();
		...
		function nuevaAmpliacion(){
			...
			if(!($("#cIdTipoContrato").val()=="CS" || $("#esCucopGasolina").val()==1) ){
				...
				for ( var i=0 ; i<aTrs.length; i++ )     
				{
					...
					if(parseInt(cantCapturado,10)<=parseInt(cantDisp,10) && parseInt(cantCapturado,10)>0){
						...
						if(parseFloat(cantCapturado)>0){
							swal("En la línea "+nTr[2]+" sobrepasaste el monto disponible.",{icon:"warning",button: "Cerrar"});
							return;
						}
					}
				}
				...
				queryFormPost("insertAmpContratoArt25", {async: false});
				//initDataTable();
				ocultarBotones();
				muestraLineas();
				$("#dtblDetalleAmpliacionesMaster").css("display","none");
		...
		function recargaTablaAmpliacionDetalle(){
			oTablaAmpliacionesDetalle=$('#tblDetalleAmpliaciones').dataTable({         
				...
				bServerSide: true,
				sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=fn_mContratoArt25AmpliacionesDetalle('"+$("#cIdContratoDefinitivo").val()+"',"+$("#nIdAmpliacion").val()+")",
				bProcessing: true,
							...
							if (!continuar) {
								...
								ocultarBotones();
								 $.ajax({url: '../../ContratacionFormalizadaServlet'
									, type:'post' , async: false,data:'tipoProceso=4&tipoOperacion=10&nConsecutivoAmpliacion='+$("#nIdAmpliacion").val()+'&cIdContratoDefinitivo='+$("#cIdContratoDefinitivo").val()
			...
			if($("#nIdAmpliacion").val() != ""){
				if((roles.indexOf("ADMIN_RECMAT") >= 0) ||(roles.indexOf("JEFES") >= 0) || ($("#U_LOGIN").val()==$("#cIdUsuarioCreacionAmp").val()) ){
					if($("#cEstadoAmpliacion").val().toUpperCase() == "CAPTURADO"){
						...
						queryFormPost("mAnulaAmpliacionContratoArt25", {async: false});
						initDataTable();
						muestraLineas();
						$("#dtblDetalleAmpliacionesMaster").css("display","none");
						$.unblockUI();
		...
		function devuelveAmpliacion(){
			if($("#nIdAmpliacion").val() != ""){
				if($("#cEstadoAmpliacion").val().toString() == "EN SAI SIN PRESUPUESTO" ){
					...
					ocultarBotones();
					$.ajax({url: '../../ContratacionFormalizadaServlet'
						, type:'post' , async: false,data:'tipoProceso=4&tipoOperacion=11&nConsecutivoAmpliacion='+$("#nIdAmpliacion").val()+'&cIdContratoDefinitivo='+$("#cIdContratoDefinitivo").val()
						+'&nIdAmpliacion='+$("#nIdAmpliacion").val()
						, dataType: 'json',
		        		mimeType: 'application/json', success: 
						function(j){
		...
		function buscaClaveEP(){
			window.open('../MultiReporteGridSacel.jsp?cIdUsuarioCreacion=' + $('#cIdUsuarioCreacion').val() 
					+ '&cIdDocumento=' + $('#cIdContratoDefinitivo').val() + '&cIdRFC=' + $('#cIdRFC').val()
					 +'&cuentaDisponible=' + $('#cuentaDisponible').val()+'&isContratoCap4=0'
...
// 					, 'MultiReporteGridSacel', 'status=1, width=900px, height=680px, left=100px, top=10px');
		...
		function validarGuardaAmpliacionDetalle(){
			...
			queryFormPost("insertAmpliacionDetalleContratoArt25", {async: false, 
				callback: function(){
					recargaTablaAmpliacionDetalle();
					$("#ep").val("");
				}
		...
		function precomprometer(){
			...
			if ($("#nIdAmpliacion").val() != "") {
				if($("#cEstadoAmpliacion").val() == "EN SAI SIN PRESUPUESTO"){	
					...
					document.getElementById("imgAprobarpreCompromisoContAmp").disabled = true;
					$.blockUI({message: "Procesando espere ......"});
					...
					$.ajax({url: '../../ContratacionFormalizadaServlet'
						, type:'post' , async: false,data:'tipoProceso=4&tipoOperacion=12&tablaDatos='+arregloDatos+'&nConsecutivoAmpliacion='+$("#nIdAmpliacion").val()
		...
		function devuelvePrecompromiso(){
			if($("#nIdAmpliacion").val() != ""){
				if($("#cEstadoAmpliacion").val().toString() == "EN SAI PRESUPUESTADO"){
					...
					$.blockUI({message: "Procesando espere ......"});
					$.ajax({url: '../../ContratacionFormalizadaServlet'
						, type:'post' , async: false,data:'tipoProceso=4&tipoOperacion=13&nConsecutivoAmpliacion='+$("#nIdAmpliacion").val()
						+'&cIdContratoDefinitivo='+$("#cIdContratoDefinitivo").val()
						, dataType: 'json',
		        		mimeType: 'application/json', success: 
						function(j){
		...
		function guardarcomprimisoGeneral(){
			if($("#nIdAmpliacion").val() != ""){
				if($("#cEstadoAmpliacion").val().toString() == "EN SAI PRESUPUESTADO"){
					...
					$.blockUI({message: "Procesando espere ......"});
					 $.ajax({url: '../../ContratacionFormalizadaServlet'
						, type:'post' , async: false,data:'tipoProceso=4&tipoOperacion=14&nConsecutivoAmpliacion='+$("#nIdAmpliacion").val()
						+'&cIdContratoDefinitivo='+$("#cIdContratoDefinitivo").val()
						, dataType: 'json',
		        		mimeType: 'application/json', success: 
						function(j){
...
<body>
	<form id="formApliacionesContArt25">
				...
				<div class="row">
					<div class="input-group">
						<div class="col">
							...
							<input type="button" class="btnInterfaceAutoriza ui-button ui-corner-all float-right" 	id="imgAprobarpreCompromisoContAmp" name="imgAprobarpreCompromisoContAmp" 	value="Pre-Comprometer"	
		...
		<input type="hidden" name="esCucopGasolina" id="esCucopGasolina" value="0" />
		<input type="hidden" name="nIdAmpliacion" id="nIdAmpliacion" value="" />
		<input type="hidden" name="cEstadoAmpliacion" id="cEstadoAmpliacion" value="0" />
		<input type="hidden" name="cIdUnidadEjecutoraSolicitud" id="cIdUnidadEjecutoraSolicitud" value="<%=  usuario.getU_UR() %>" />
		<input type="hidden" name="nIdTipoContrato" id="nIdTipoContrato" value="" />
