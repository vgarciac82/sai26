...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		//	NegativaBoton NegBoton= new NegativaBoton();
			...
			Map botones=nb.getBotones(roles,"ContratoPluriCap4","PrecompromisoContratoPluriCap4");
	...
	function initQuerysPrecomp() {
		queryFormPost("obtieneDatosContPluriCap4", {async : false,
			callback: function(){
				showButtons();
			}
		});
	...
	function precomprometer(){
		...
		if (imgAprobar==0) {
			...
			$.ajax({url: '../../servlet/ContratoCap4Servlet'
				, type:'post' , async: false,data:'operacion=20&tablaDatos='+arregloDatos+'&cEjercicio='+$("#cEjercicio").val()+'&cIdContratoDefinitivo='+$("#cIdContratoDefinitivo").val()
				+'&ntipoPago='+$("#esDescentralizado").val()+'&descripPoliza=PRECOMPROMISO DE CONTRATOS PLURIANUALES CAPITULO 4000'
				, dataType: 'json',
			...
			if(parseInt($("#nIdEstado").val(),10)==3){
				swal({
					...
					if (!continuar) {
						...
						$("#imgDevolverpreCompromisoContCap4").hide();
						$.ajax({url: '../../servlet/ContratoCap4Servlet'
							, type:'post' , async: false,data:'operacion=21&cEjercicio='+$("#cEjercicio").val()+'&cIdContratoDefinitivo='+$("#cIdContratoDefinitivo").val()
	...
	function comprometer(){
		...
		if (imgAprobar==0   && (roles.indexOf("JEFES") >= 0)|| roles.indexOf("ADMIN_RECMAT") >= 0 ) {
			...
			$.ajax({url: '../../servlet/ContratoCap4Servlet'
				, type:'post' , async: false,data:'operacion=22&cIdContratoDefinitivo='+$("#cIdContratoDefinitivo").val()
...
<body>
	<form id="formPrecompromisoContCap4">
		<fieldset class="form-group border p-3">
			<legend class="w-auto px-2">Precompromiso</legend>
