...
<%
	...
	*/
	...
	if (session.getAttribute(GestionInterface.ATT_PedidoEjercicioPlurianual) != null) {
		...
		response.sendRedirect("PlurianualidadadesPedidos.jsp?tab=0");
...
<script type="text/javascript" charset="utf-8">
	...
	$(document).ready(function() {
		<%
				...
				Map botones=nb.getBotones(role,"Plurianualidad Contratos","preCompromisoContrato");
				Iterator btn = botones.entrySet().iterator();
				while (btn.hasNext()) {
					Map.Entry b = (Map.Entry)btn.next();%>
					$("#<%=b.getValue()%>").attr("disabled", true);<%
					String img=(String) b.getValue();
					if ("imgAprobarpreCompromisoPluri".equals(img)){  
						imgAprobar=1;
					}
					if ("imgDevolverpreCompromisoPluri".equals(img)){
						imgDevolver=1; 
					}
					if ("trEditarpreCompromisoPlur".equals(img)){ 
						editar=1; 
					}	
				}		
			%>
	...
	function setInitConditions(){
		if(vEstado==3 ||vEstado==4 || vVigente==0){
			...
			document.getElementById("imgAprobarpreCompromisoPluri").disabled = true;
		...
		if(vEstado != 3){
			...
			document.getElementById("imgDevolverpreCompromisoPluri").disabled = true;
		}
	...
	function initQueries(){
			...
			queryFormPost("fnMontoNetoContratoPlurianualPresupuestoReadPedido", {async: false});
		...
		var oTable=$('#dt_suficiencia').dataTable( {
			...
			fnInitComplete: function(oSettings, json) {
				...
				if(vEstado ==2 || vEstado==5 && vVigente==1){
					var editar='<%=editar%>';
					if (editar==0){
						document.getElementById("trEditarpreCompromisoPlur").style.display="table-row";
					}
				}
			}	
            });
	...
	function cargaContratoPrecomprometido() {
	 ...
	 var oTable=$("input").attr("readonly", true); 
	   ...
	   function guardarPrecomprimisoGeneral(){
		 // se checa el grupo del usuario para saber si se puede precomprometer
		 ...
		 if (imgAprobar==0){
			...
			if($("#mImporteTotal").val()!= $("#mComprometido").val()){
				...
				for (var i = 0; i < aTrs.length; i++) {
					...
					$("#nIdEstado").val(2);
					vEstado=2;
					queryFormPost("mPedidoPlurianualPrecompromisoUpdate", {async: false });
					...
					document.getElementById("imgAprobarpreCompromisoPluri").disabled = false;
					return;
					}
			...
			$('#dt_preCompromiso').attr('disabled', true);
			document.getElementById("imgAprobarpreCompromisoPluri").disabled = true;
			document.getElementById("imgDevolverpreCompromisoPluri").disabled = false;
			document.getElementById("trEditarpreCompromisoPlur").style.display = "none";
			actualizaCaratula();
			
			//Bitácora
			$("#cAccion").val("APRUEBA_PRECOMPROMISO_PLURIANUALIDADPEDIDO");
			$("#cIdDocumento").val($("#cContratoDefinitivo").val());
			queryFormPost("sp_mBitacoraMovimientosCreate", { async : false});
			alert("Plurianualidad Pedido Precomprometida con folio(s) " + $("#foliosCasoPreCompromiso").val());
	...
	function guardarPrecompromisoDes(cc, ur,nRows,oTable,oTablD,aTrs,vdocren)
		{	
				...
				$.ajax({url: '../../servlet/PedidoPlurianualidadServlet' , type:'post' , async: false,data:'operacion=2&ur='+ur, dataType: 'json', success: guardaFolio});
				//}
			...
			//aplicacion contable
			$.ajax({url: '../../servlet/PedidoPlurianualidadServlet' , type:'post' , async: false,data:'operacion=3', dataType: 'json', success: imprimeRes});
	...
	function guardarPrecompromiso()	{
	    	...
	    	//Genera un nuevo folio en caso de que no lo tenga
			//if($("#nFolioPreCompromiso").val()=="-1"){
				$.ajax({url: '../../servlet/PedidoPlurianualidadServlet' , type:'post' , async: false,data:'operacion=2', dataType: 'json', success: guardaFolio});
			//}
			//En caso de cualquier error al obtener el folio, sale de la función
			if($("#nFolioPreCompromiso").val()==0)
				return -1;
			...
			//aplicacion contable
			$.ajax({url: '../../servlet/PedidoPlurianualidadServlet?nFolioPrecompromiso='+$("#nFolioPreCompromisoE").val()+"&cEjercicio="+$("#aEjercicioFiscal").val()+"&folioCasoPreCompromiso="+$("#folioCasoPreCompromiso").val()+"&cContratoDefinitivo="+$("#cContratoDefinitivo").val(), type:'post' , async: false,data:'operacion=3', dataType: 'json', success: 
			function(j){
	...
	function devuelvePrecompromiso(){
		...
		if (imgDevolver==0){
			...
			queryFormPost("mContratoPlurianualidadCaratulaReadPedido", {async: false});
			if($("#nIdEstado").val()==("2")){
			alert("El PedidoPlurianual ha sido rechazado por el usuario de ventanilla");
			...
			//actualizaCaratula();
			window.location = "PlurianualidadesPedidos.jsp?tab=" + 3;
			return;
			}
			//primero checamos si el contrato ya esta siendo trabajado por el usuario de ventanilla
			queryFormPost("usuarioVentanillaReadPedidoPlurianualidad", {async: false});
		...
		if(res){
		 $.ajax({url: '../../servlet/PedidoPlurianualidadServlet?nFolioPrecompromiso='+$("#nFolioPreCompromiso").val()+"&cEjercicio="+$("#cEjercicio").val()+"&cContratoDefinitivo="+$("#cContratoDefinitivo").val(), type:'post' , async: false,data:'operacion=4&tipoPago=' + $("#nTipoPago").val(), dataType: 'json', success: eliminaPrecompromiso});	
			...
			actualizaCaratula();
			alert("El Pedido Plurianual no se puede devolver porque ya ha sido APROBADO y esta COMPROMETIDO");
		}
	  ...
	  function eliminaPrecompromiso(j){
			...
			//Bitácora
			$("#cAccion").val("DEVUELVE_PRECOMPROMISO_PLURIANUALIDADPEDIDO");
			$("#cIdDocumento").val($("#cContratoDefinitivo").val());
			 queryFormPost("sp_mBitacoraMovimientosCreate", { async : false});
			 actualizaCaratula();
			 window.location = "PlurianualidadesPedidos.jsp?tab=" + 3;
	...
	<div id="container" class="container">
		<table align="left" width="750px">
				<tr><td>
						...
						<legend>Pre-Compromiso de la plurianualidad</legend>
							<table align="left" cellpadding="2" width="100%">
						    	<tr>
						    		<td align="right" colspan="2">
						    			<img id="imgAprobarpreCompromisoPluri" src="../imagenes/accept_green.png" style="cursor: pointer" onclick="guardarPrecomprimisoGeneral();" />&nbsp;Pre-Comprometer
						    			<img id="imgDevolverpreCompromisoPluri" src="../imagenes/arrow_left_blue_round.png" style="cursor: pointer" onclick="devuelvePrecompromiso();"/>&nbsp;Devolver
					...
					<tr id="trEditarpreCompromisoPlur" style="display: none"><td align="left" ><a href="javascript:void(0)" id="edit">editar</a></td></tr>
