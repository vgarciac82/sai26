			...
			$('#tblNuevoPasivo tr').live('dblclick', function() { 
				if (tabla==0){
					...
					$('#contratoDefinitivo').val(aData[1]);
			  		...
			  		queryFormPost("creaPasivosContrato",{async: false });
			  		if(aData!= ""){
			  		habilitaTabs()
		 ...
		 var qw = " 1=1 ";  
			...
			*/
			...
			oTable = $("#tblNuevoPasivo").dataTable({
				...
				bServerSide: true,
				sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=fn_mPasivosPosibles('" + <%=cEjercicio%> + "')&qw="+ qw,
				bProcessing: true,
				sPaginationType: "full_numbers",
				bJQueryUI: true,
				aaSorting: [[ 1, "asc" ]] ,
  ...
  <body id="dt_example" bottomMargin="0" bgcolor="red" leftmargin="0" topmargin="0">
  	<form> 
		...
		<input type="hidden" name="cIdUnidadEjecutora1" id="cIdUnidadEjecutora1" value="<%=usuarioTab1.getU_UR()%>" />
  		...
  		<fieldset >
  			<legend>Nuevo Pasivo Contrato</legend>
