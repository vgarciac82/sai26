...
<%
	...
	if (session.getAttribute(GestionInterface.ATT_ContratoEjercicio) != null) {
		...
		cEjercicioPlurianual = Calendar.getInstance().get(Calendar.YEAR)+1;//(String)session.getAttribute(GestionInterface.ATT_ContratoEjercicio);
%>
	...
	var oTable;
		$(document).ready(function() {
			<%
				...
				Map botones=nb.getBotones(roles,"Contratos","plurianualidadContrato");
				Iterator btn = botones.entrySet().iterator();
				while (btn.hasNext()) {
					Map.Entry b = (Map.Entry)btn.next();%>
					$("#<%=b.getValue()%>").attr("disabled", true);<%	
					String img=(String) b.getValue();
					if ("imgEliminarPlurianualidad".equals(img)){  
						eliminaPlurianulidad=1;
					}
					if ("imgGuardarPlurianualidad".equals(img)){// actualiza
						actualizaPlurianulidad=1; 
					}
					if ("imgAgregarPlurianualidad".equals(img)){
			...
			//	}
				...
				queryFormPost("contratoDefinitivoPlurianual", {async: false}); 
...
//-------------------------------------------------------------------------------------------------------------------------------
	...
	function agregarNuevaPluri(){
		...
		if (agregaPlurianulidad==0){
			if ($("#tblConsultaPlurianualidad").dataTable().fnGetNodes().length == 0){
				...
				mostrar();		
						...
						queryFormPost("checa_maximo", {async: false});
		...
		//  Elimina la última de las plurianualidades para un contrato dado
		function eliminaPlurianualidad(){
			var eliminaPlurianulidad='<%=eliminaPlurianulidad%>';
			if(eliminaPlurianulidad==0){
				if ($("#tblConsultaPlurianualidad").dataTable().fnGetNodes().length != 0){
			  		queryFormPost("checa_maximo", {async: false});
					queryFormPost("borraLineaPlurianualidad", {async: false});
					mostrar();
				}
			}else{
		...
		function actualizaPlurianualidad(){
		   var actualizaPlurianulidad='<%=actualizaPlurianulidad%>';
		   if (actualizaPlurianulidad==0){
		   
		     /*
				  if(parseFloat($("#lblTotalPorPagar").val().replace(',','')) == 0){
				  alert("No se puede crear una Plurianualidad el Contrato ya ha sido Pagado en su Totalidad");
				  return;
				  }
		   */
...
//-------------------------------------------------------------------------------------------------------------------------------		
		...
		function mostrar() {
			 var qw = "cIdContratoDefinitivoContrato LIKE'%25" +$("#cIdDefinitivo").val()+"%25'";
			...
			oTable = $("#tblConsultaPlurianualidad").dataTable({
				...
				sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=v_plurianualidadEdita&qw=" + qw,
				bProcessing: true,
		...
		<fieldset style="width:750px" align="left">
			...
			<tr>
  				<td align="right" colspan="2">
					<img id="imgAgregarPlurianualidad" src="../../imagenes/iconos/aceptar.png" style="cursor: pointer"  onclick="agregarNuevaPluri();"/>&nbsp;Agregar

					<img id="imgGuardarPlurianualidad" src="../../imagenes/iconos/guardar.png" style="cursor: pointer"  onclick="actualizaPlurianualidad();"/>&nbsp;Guardar

					<img id="imgEliminarPlurianualidad" src="../../imagenes/iconos/rechazar.png" style="cursor: pointer"  onclick="eliminaPlurianualidad();"/>&nbsp;Eliminar
				</td>
			 </tr>
			 <tr>
