...
<%
	...
	if (session.getAttribute(GestionInterface.ATT_ContratoEjercicioPasivo) != null) {
		...
		System.out.println("tipo contrato " +cITipoContrato + cIdContratoDefinitivo+ " "+cEjercicio+"  "+ cIdContrato+ " "+  cIdUnidadEjecutora+ " ");
  ...
  <head>    
	...
	<script type="text/javascript" charset="utf-8">
				...
				<%
					...
					Map botones=nb.getBotones(role,"PasivoContrato","presupuestoPasivoContrato");
					Iterator btn = botones.entrySet().iterator();
					while (btn.hasNext()) {
						Map.Entry b = (Map.Entry)btn.next();%>
						$("#<%=b.getValue()%>").attr("disabled", true);<%
						String img=(String) b.getValue();
						if ("imgAprobarPresupuestoPasivoContrato".equals(img)){  
							%>$("#<%=img%>").attr("disabled", false);<%
							imgAprobar=1;
						}
						if ("imgDevolverPresupuestoPasivoContrato".equals(img)){
							%>$("#<%=img%>").attr("disabled", false);<%
							imgDevolver=1; 
						}	
					}
			...
			function validaCondicionesIniciales(){
				...
				if (parseInt($("#nIdEstado").val(),10)== 3 || parseInt($("#nIdEstado").val(),10)== 4 )
					document.getElementById("imgDevolverPresupuestoPasivoContrato").disabled = true;
				if (parseInt($("#nIdEstado").val(),10)> 1 ){
					document.getElementById("nIdClaveEgresosXPasivoContrato").disabled = true;	
					document.getElementById("AgregarEPPasivoContrato").disabled = true;
					document.getElementById("imgAprobarPresupuestoPasivoContrato").disabled = true;
				}
			}
		...
		function fnClickAddRowC() {
		   ...
		   rowCount = $('#dt_clavepresup tr').length;
			...
			{
				window.open('../MultiReporteGridSacel.jsp?cIdUsuarioCreacion=' + $('#cIdUsuarioCreacion').val() + '&cIdDocumento=' + $('#cIdContratoMat').val()+'&isPasivo='+1, 'MultiReporteGridSacel', 'status=1, width=900px, height=680px, left=100px');
			...
			function guardaContrato(){
			 ...
			 if (imgAprobar==0){
				var proc=""+$("#cEjercicio").val()
        			 ...
        			 $.getJSON("../../servlet/PasivoServlet?operacion=0",{Tabla: "", Param: proc, MaxReg: "", ajax: 'false'}, function(j){
				 		for(var i = 0; i < j.length; i++)
		                	 var col=j[i].Col1
		                switch(col){
						case "0": 
							alert("El ContratoPasivo se ha aprobado y está en el SAI");
							queryFormPost("mContratoPasivosContratoCaratulaRead", {async: false});
							document.getElementById("nIdClaveEgresosXPasivoContrato").disabled = true;	
							document.getElementById("AgregarEPPasivoContrato").disabled = true;
							document.getElementById("imgAprobarPresupuestoPasivoContrato").disabled = true;
							...
							if (imgDevolver==0){
								document.getElementById("imgDevolverPresupuestoPasivoContrato").disabled = false;
							}
							$("#nIdEstadoPed").val("2"); 
						break;
						case "1":  
							alert("Este ContratoPasivo ya se encuentra aprobado");
						break;
						case "2":  
							alert("Este proveedor no se encuentra registrado en el sistema Financiero");
						break;
						case "3":  
							alert("No se han registrado EPs para este ContratoPasivo");
						break;
						case "4":  
							alert("Ha ocurrido un error al registrar el ContratoPasivo, por favor contacte a su administrador");
						break;
						case "5":  
							alert("Ha ocurrido un error al registrar las EPs, por favor contacte a su administrador");
						break;
						case "6","7":  
							alert("Ha ocurrido un error al actualizar el ContratoPasivo, por favor contacte a su administrador");
						break;
	                 	}
				});
			 }else{
				 alert("No puede realizar esta accion, contacte a su administrador");
			 }
		...
		function devuelveContrato(){
			...
			if(parseInt($("#nIdEstado").val(),10)<3 || parseInt($("#nIdEstado").val(),10)==5 ){
				var proc=""+$("#cEjercicio").val()
					...
					$.getJSON("../../servlet/PasivoServlet?operacion=1",{Tabla: "", Param: proc, MaxReg: "", ajax: 'true'}, function(j){
				 		for(var i = 0; i < j.length; i++)
		                	 var col=j[i].Col1
		                switch(col){
						case "0":  
							alert("Se ha devuelto el ContratoPasivo");
							queryFormPost("mContratoPasivosContratoCaratulaRead", {async: false});
							document.getElementById("nIdClaveEgresosXPasivoContrato").disabled = false;	
							document.getElementById("AgregarEPPasivoContrato").disabled = false;
							var imgAprobar='<%=imgAprobar%>';
							 if (imgAprobar==0){
								document.getElementById("imgAprobarPresupuestoPasivoContrato").disabled = false; 
							 }
						...
						break;
						case "1":  
							alert("No se puede regresar el ContratoPasivo porque su estado no lo permite");
						break;
						case "2":  
							alert("Ha ocurrido un error al eliminar las EPs del Contrato, por favor contacte a su administrador");
						break;
						case "3":  
							alert("Ha ocurrido un error devolver el ContratoPasivo, por favor contacte a su administrador");
						break;
						case "4":  
							alert("Ha ocurrido un error al cambiar el estado del ContratoPasivo, por favor contacte a su administrador");
						break;
	                 	}
				});
		  }		
				...
				<tr>
					<td>
						<fieldset style="width:750px" >
							<legend>Partidas del Pasivo </legend>
								<table align="left" cellpadding="2" width="100%">
							    	<tr>
							    		<td align="right" colspan="2">
							    			<img id="imgAprobarPresupuestoPasivoContrato" src="../imagenes/accept_green.png" style="cursor: pointer" onclick="guardaContrato();" />&nbsp;Aprobar 
											<img id="imgDevolverPresupuestoPasivoContrato" src="../imagenes/arrow_left_blue_round.png" style="cursor: pointer" onclick="devuelveContrato();"/>&nbsp;Devolver
			    					...
			    					<td style="width: 740px;">
				    					<table height="66" width="98%" border="0">
							                ...
							                <tr> 
							                  ...
							                  <td><input type="button" name="nIdClaveEgresosXPasivoContrato" id="nIdClaveEgresosXPasivoContrato" size="5" value="..." onclick="buscaClavePresupuestal()" onblur="rellenaCampos();"></td>
							                  <td><input type="button" value="Agregar" name="Add2" id="AgregarEPPasivoContrato"	onclick="fnClickAddRowC();"></td>
							                </tr>
							              </table>
			    					</td>
			    				</tr>	
