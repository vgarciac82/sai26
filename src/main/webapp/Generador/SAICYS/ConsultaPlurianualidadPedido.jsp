...
<html>
  <head>
	...
	<script type="text/javascript" charset="utf-8"><!--
		...
		var oTable;
			$(document).ready(function() {
				...
				while (it.hasNext()) {
					...
					if ("presupuestoPlurianualidadPedido".equals(pestana)){
						 presupuesto=1;
					}
					if ("consultaPlurianualidadPedido".equals(pestana)){
						 consulta=1;
					}
				}
				///botones
				Map botones=nb.getBotones(role,"PlurianualidadPedidos","consultaPlurianualidadPedido");
		...
		$('#tblconsultaPlurianualidades tr').live('dblclick',function() {
		        ...
		        function deshabilitaTabs(){
		     			$( "#presupuestoPlurianualidadPedido" ).attr("disabled", true);
						$( "#precompromisoPlurianualidadPedido" ).attr("disabled", true);
						
				}
				
				function habilitaTabs(){
				  		$( "#presupuestoPlurianualidadPedido" ).attr("disabled", false);
						$( "#precompromisoPlurianualidadPedido" ).attr("disabled", false);
			...
			function mostrar() { 
				...
				oTable=	$('#tblconsultaPlurianualidades').dataTable({
					...
					"oLanguage": {
						...
						bServerSide: true,
						sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=v_consultaPlurianualidadPedido&qw=" + qw,
						aaSorting: [[ 0, "asc" ],[ 1, "asc" ],[2,"asc"]] ,			
						aoColumns: [
							{ sName: "cVinculo", bVisible: false },
							{ sName: "cIdPedidoDefinitivo1" },
		...
		//obtiene solo un registro del renglon seleccionado lo mete en un arreglo
			function fnGetSelected( oTableLocal )
			{
				...
				return aReturn;
		          ...
		          if(confirm("\xBFEstas seguro de actualizar los datos de Pedido definitivo "+$("#cIdPedidoDefinitivo_"+indice+"").val()+"?")){
				  $("#contratoDefinitivo").val($("#cIdPedidoDefinitivo_"+indice+"").val());
				   $("#montoIva").val($("#porcentajeIva_"+indice+"").val());
				   $("#montoTotal").val($("#montoNeto_"+indice+"").val());
				    $("#mTipoCambio").val($("#nidtipoCambio_"+indice+"").val());
				    queryFormPost("actualizaConsultaPlurianualPedido",{async:false});
	         ...
	         function borraPlurianualidad(indice){
		          ...
		          if(confirm("\xBFEstas seguro de Borrar la Plurianualidad del Pedido definitivo "+$("#cIdPedidoDefinitivo_"+indice+"").val()+"?")){
				  $("#contratoDefinitivo").val($("#cIdPedidoDefinitivo_"+indice+"").val());
				  // $("#montoIva").val($("#porcentajeIva_"+indice+"").val());
				   //$("#montoTotal").val($("#montoNeto_"+indice+"").val());
				    //$("#mTipoCambio").val($("#nidtipoCambio_"+indice+"").val());
				    queryFormPost("borraConsultaPlurianualPedido",{async:false});
			       window.location='PlurianualidadesPedidos.jsp?tab=1'
				    return;
  ...
  <body id="dt_example" bottomMargin="0" bgcolor="red" leftmargin="0" topmargin="0">
  	<form> 
  		...
  		<fieldset >
  			<legend>Consulta Plurianualidades Pedidos</legend>
				    ...
				    <table align="left" width="90%">
				    	<tr>
				    		<td>
				    			<table align="left" cellpadding="2" width="750px">
									...
									<tr>
							    		<td colspan="2" align="center"><input type="button"  name="btnConsultaPlurianualidadPedido" id="btnConsultaPlurianualidadPedido" value="Buscar"  onclick="mostrar();"/></td>
							    	</tr>
							    </table> 
				    		</td>
				    	</tr>
				    	...
				    	<tr  >
				    		<td style="width: 740px; height: 120px" >
							     <table align="left" id="tblconsultaPlurianualidades" width="740px" class="display">
						        	<thead>
						        		<tr>
						        			<th></th>
