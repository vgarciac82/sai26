...
<%
	...
	if (session.getAttribute(GestionInterface.ATT_PedidoEjercicio) != null) {
		...
		int cEjercicioPasivoPedido=0;
		cEjercicioPasivoPedido = Calendar.getInstance().get(Calendar.YEAR);//(String)session.getAttribute(GestionInterface.ATT_ContratoEjercicio);
	...
	var oTable;
		$(document).ready(function() {
			<%
				...
				int imgGuardarPasivoPedido=0;
				Map botones=nb.getBotones(roles,"Pedidos","pasivo");
				Iterator btn = botones.entrySet().iterator();
				while (btn.hasNext()) {
					Map.Entry b = (Map.Entry)btn.next();%>
					$("#<%=b.getValue()%>").attr("disabled", true);<%	
					String img=(String) b.getValue();
					if ("imgGuardarPasivoPedido".equals(img)){// actualiza
						%>$("#<%=img%>").attr("disabled", false);<%
						imgGuardarPasivoPedido=1; 
					}
			...
			var tabla='<%=tabla%>';
					...
					queryFormPost("fnMontosPedidoRead", {async: false});
					queryFormPost("fnMontoNetoPedidoPresupuestoRead", {async: false});
					queryFormPost("contratoDefinitivoPasivoPedido", {async: false});
				    queryFormPost("contratoDefinitivoPasivo1Pedido", {async: false});
		      ...
		      $('#tblConsultaPasivo tr').live('dblclick', function() { 
				  ...
				  alert("No se puede crear un Pasivo el Pedido ya ha sido Pagado en su Totalidad");
				  return;
				  }
			...
			function agregarNuevoPasivo(){
				...
				if ($("#tblConsultaPasivo").dataTable().fnGetNodes().length != 0){
					...
					queryFormPost("sp_PasivoInsertaPedido", {async: false});
					mostrarPasivoEditable();		
				}
			}
		...
		function actualizaPasivo(){
			var imgGuardarPasivoPedido='<%=imgGuardarPasivoPedido%>';
			if (imgGuardarPasivoPedido==0){
				  var totalEdita=0;	
				  if(parseFloat($("#lblTotalPorPagar").val().replace(',','')) == 0){
				   ...
				   if (aTrs.length != 0){
					 for(var i=0; i <aTrs.length; i++){
						  ...
						  if(parseFloat($("#nPorcentajeIva_"+aData[11]+"").val())> 100 || parseFloat($("#nPorcentajeIva_"+aData[11]+"").val()) < 0 ){
				    ...
				    for(var i=0; i <aTrs.length; i++){
						...
						$("#porcentajeIva").val(parseInt($("#nPorcentajeIva_"+aData[11]+"").val(),10));
...
//-------------------------------------------------------------------------------------------------------------------------------		
			...
			function mostrarPasivoEditable() {
				...
				oTable1 = $("#tblCapturaPasivo").dataTable({
					...
					sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=v_PasivoEditaPedido&qw=" + qw,
					bProcessing: true,
					sPaginationType: "full_numbers",
					bJQueryUI: true,
					aaSorting: [[ 0, "asc" ]] ,
		        ...
		        queryFormPost("borraPartidaPasivoPedido",{async: false});
		        mostrar();
		        mostrarPasivoEditable();
		        }
		...
		<fieldset style="width:750px" align="left">
			...
			<tr>
  				<td align="right" colspan="2">
					<img id="imgGuardarPasivoPedido" src="../../imagenes/iconos/guardar.png" style="cursor: pointer"  onclick="actualizaPasivo();"/>&nbsp;Guardar
			...
			<tr>
				<td>
					<table align="left" width="80%">			
						...
						<td><input type="hidden" name="cEjercicioPasivo" id="cEjercicioPasivo" value="<%=cEjercicioPasivoPedido%>" /></td>
