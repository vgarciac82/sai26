	...
	<meta http-equiv="description" content="This is my page">
		...
		<script type="text/javascript" charset="utf-8">
			...
			$(document).ready(function() {
				mostrar();
				$("#tblSalario tbody").click(function(event) {
				...
				var aTrs = $('#tblSalario').dataTable().fnGetNodes();
				//alert("Yaaaa");
				for ( var i=aTrs.length ; i>=0; i-- )     
					{  
						...
						{   
							var nTr = $('#tblSalario').dataTable().fnGetData(aTrs[i]);
							$("#idSalario").val(nTr[0]);
							$("#cant").val(nTr[1]);
							$("#salario").val(nTr[2]);
...
// 					qw += " AND cDescripcion like '%25" + $("#descripcion").val() + "%25'";
				...
				bServerSide: true,
				sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=[mCatSalario]&qw="+qw,
				bProcessing: true,
				//Tablesorter:false,
				//bSortable: false
		...
		function Cambiar(){
			if($("#idSalario").val()!='' && $("#salario").val()!='' && $("#cant").val()!=''){
				queryFormPost("mCatalogoSalario",  {async : false});
  ...
  <body>
    <fieldset>
  		<legend>Cat&aacute;logo de Salario</legend>
  		<form action="">
...
<!-- 			<input title="Agregar" type="button" name="btnAgregar" id="btnAgregar" onclick="Agregar()" style="width: 85px;height:28px ;background-image: url('../../Ayudas/imagenes/agregar_d.png')" > -->
			...
			<table width="100%" border="0">
	   			...
	   			<tr width="50%">
	   				<td><input type="text" name="idSalario" id="idSalario" maxlength="5" readonly="readonly""></td>
	   				<td><input type="text" name="cant" id="cant" onkeypress="return(onlyNumbers(event));"></td>
	   				<td><input type="text" name="salario" id="salario" onkeypress="return(onlyNumbers2(event));"></td>	
	   			</tr>
	   		</table>
  		</form>
  		
	</fieldset>
	<table id="tblSalario" class="display">
        <thead>
            <tr>
				<th >#</th>
            	<th  >Cantidad</th>
