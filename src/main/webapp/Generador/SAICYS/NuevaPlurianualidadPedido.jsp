	...
	var oTable;
		...
		oTable=$("#tblNuevaPlurianualidad").dataTable({
			...
			<%
				...
				Map botones=nb.getBotones(role,"PlurianualidadPedidos","nuevaPlurianualidadPedido");
			...
			$('#tblNuevaPlurianualidad tr').live('dblclick', function() { 
				//if (tabla==0){
					...
					$('#pedidoDefinitivo').val(aData[1]);
			  		$('#cIdPedido').val(aData[5]);
			  		queryFormPost("creaPlurianualidadPedido",{async: false });
			  		if(aData!= ""){
			  		habilitaTabs()
		...
		function mostrar() {
		 ...
		 var qw = " 1=1 ";  
			...
			oTable = $("#tblNuevaPlurianualidad").dataTable({
				...
				bServerSide: true,
				sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=fn_mPlurianualidadPosiblesPedido('" + <%=cEjercicio%> + "')&qw="+ qw,
				bProcessing: true,
				sPaginationType: "full_numbers",
				bJQueryUI: true,
				aaSorting: [[ 1, "asc" ]] ,
  ...
  <body id="dt_example" bottomMargin="0" bgcolor="red" leftmargin="0" topmargin="0">
  	<form> 
		...
		<input type="hidden" name="cIdUnidadEjecutora1" id="cIdUnidadEjecutora1" value="<%=usuarioTab1.getU_UR()%>" />
  		...
  		<fieldset >
  			<legend>Nueva Plurianualidad Pedido</legend>
				    ...
				    <table align="left" width="90%">
				    	...
				    	<tr  >
				    		<td style="width: 740px; height: 300px" >
							     <table align="left" id="tblNuevaPlurianualidad" width="740px" class="display">
						        	<thead>
						        		<tr> 
