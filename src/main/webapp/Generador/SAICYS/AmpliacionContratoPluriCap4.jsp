...
<title>'AmpliacionContratoPluriCap4'</title>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		//	NegativaBoton NegBoton= new NegativaBoton();
			...
			Map botones=nb.getBotones(roles,"ContratoPluriCap4","AmpliacionContratoPluriCap4");
	...
	function muestraLineasBienes(){
		...
		oTableLineasBienes = $("#tblLineasBienes").dataTable({
			...
			bServerSide: true,
			sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=v_mPartidasContPluriCap4&qw="+qw,
			bJQueryUI: true,
			aaSorting: [[ 2, "asc" ]],
			aoColumns: [
	...
	function buscaClaveEP(){
		window.open('../MultiReporteGridSacel.jsp?cIdUsuarioCreacion=' + $('#cIdUsuarioCreacionAmp').val() 
...
<body>
	<form id="formAmpliacionContCap4">
		<fieldset class="form-group border p-3">
			<legend class="w-auto px-2">Ampliaci&oacute;n de M&aacute;ximos</legend>
		...
		<input type="hidden" id="cIdUsuarioCreacionAmp" name="cIdUsuarioCreacionAmp" value="" />
