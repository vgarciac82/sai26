...
<html xmlns="http://www.w3.org/1999/xhtml" lang="en" xml:lang="en"> 
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            swfobject.embedSWF(
                "indicadoresGestion.swf", "flashContent", 
               "1800", "1900",  
...
/////////////fecha del fichero
File fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\montosCapituloPA.xml");
...
/// validar la creacion 	ruta=ruta+"\\SAICYS\\bin-release\\data\\"+sFichero;
	
	System.out.println(ruta+"\\SAICYS\\bin-release\\data\\montosCapituloPA.xml");
	...
	if (cadenaFecha.equals(fechaArchivo)){
		...
		String strXMLCap="";
   		...
   		int xCap=xmlCap.generaArchivo(strXMLCap,"montosCapituloPA.xml",ruta);
	}
	
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\presupuestoUnidad.xml");
 ms = fichero.lastModified();
	...
	if (cadenaFecha.equals(fechaArchivo)){
		...
		//programado por unidad ejecutora
   		...
   		int x=xml.generaArchivo(strXML,"presupuestoUnidad.xml",ruta);
	}
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\programadoUnidadEjecutora.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
...
//	if (fichero.exists()){
		...
		/// tabla programa anual
   		...
   		int xPA=xmlPA.generaArchivo(strXMLPA,"programadoUnidadEjecutora.xml",ruta);
	}
	
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\requisicionesUnidad.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
   ...
   //		session.setAttribute(GestionInterface.ATT_cadenaXML,strXMLReq);
   		...
   		int xReq=xmlReq.generaArchivo(strXMLReq,"requisicionesUnidad.xml",ruta);
	}
	
	
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\tablaRequisiciones.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
	...
	//if (fichero.exists()){
		...
		///tabla requisicion
   		...
   		int xReqTbl=xmlReqTbl.generaArchivo(strXMLReqTbl,"tablaRequisiciones.xml",ruta);
	}
	
	
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\consolidado.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
...
//	if (fichero.exists()){
	...
	///// CONSOLIDADO POR CAPITULO/////
  		...
  		int xConsol=xmlConsol.generaArchivo(strXMLConsol,"consolidado.xml",ruta);
	}
				   		
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\alcanceConsolidado.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
	...
	//if (fichero.exists()){
		 System.out.println("El fichero " + fichero + " existe");
  		...
  		int xConsolAlc=xmlConsolAlc.generaArchivo(strXMLConsolAlc,"alcanceConsolidado.xml",ruta);
	}			   		
				   		
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\tablaConsolidado.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
	...
	////TABLA CONSOLIDADO
   		...
   		int xConsolTbl=xmlConsolTbl.generaArchivo(strXMLConsolTbl,"tablaConsolidado.xml",ruta);
   	}			   	
				   		
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\procedimiento.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
	...
	//if (fichero.exists()){
		 System.out.println("El fichero " + fichero + " existe");
   		...
   		int xProc=xmlProc.generaArchivo(strXMLProc,"procedimiento.xml",ruta);
   	}			   		
				   		
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\tablaProcedimiento.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
...
//	if (fichero.exists()){
		...
		///tabla procedimiento
   		...
   		int xProcTbl=xmlProcTbl.generaArchivo(strXMLProcTbl,"tablaProcedimiento.xml",ruta);
   		
   	}				   		
				   		
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\pedidoCapitulo.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
	...
	//if (fichero.exists()){
		 System.out.println("El fichero " + fichero + " existe");
   		...
   		int xPed=xmlPed.generaArchivo(strXMLPed,"pedidoCapitulo.xml",ruta);
   	}				   		
				   		 
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\pedido.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
...
//	if (fichero.exists()){
		 ...
		 ///pedidos rfc
   		...
   		int xPedRFC=xmlPedRFC.generaArchivo(strXMLPedRFC,"pedido.xml",ruta);
   	}				   		
		
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\tablaPedido.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
...
//	if (fichero.exists()){
		...
		//tabla de pedido
   		...
   		int xPedTbl=xmlPedTbl.generaArchivo(strXMLPedTbl,"tablaPedido.xml",ruta);
   	}
/////pedidos por categoria
	fichero = new File(ruta+"\\SAICYS\\bin-release\\data\\tablaPedidosCat.xml");
	 ms = fichero.lastModified();
	 d = new Date(ms);
	fechaArchivo = formato.format(d);
	//if (fichero.exists()){
...
//	if (fichero.exists()){
		...
		//tabla de pedido
  		...
  		strXMLPedTbl += "</chart>";
  		generaXML xmlPedTbl = new generaXML();//
  		int xPedTbl=xmlPedTbl.generaArchivo(strXMLPedTbl,"tablaPedidosCat.xml",ruta);
  	}
		%>
        </script>
    </head>
    <body>
        ...
        <noscript>
            <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="1710" height="1800" id="indicadoresGestion">
                <param name="movie" value="indicadoresGestion.swf" />
                <param name="quality" value="high" />
                <param name="bgcolor" value="#ffffff" />
                <param name="allowScriptAccess" value="sameDomain" />
                <param name="allowFullScreen" value="true" />
                <!--[if !IE]>-->
                <object type="application/x-shockwave-flash" data="indicadoresGestion.swf" width="1710" height="1800">
                    <param name="quality" value="high" />
                    <param name="bgcolor" value="#ffffff" />
                    <param name="allowScriptAccess" value="sameDomain" />
                    <param name="allowFullScreen" value="true" />
