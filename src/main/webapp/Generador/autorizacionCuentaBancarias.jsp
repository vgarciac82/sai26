...
<head>
    ...
    <title>Autorizacion de Cuentas Bancarias</title>
...
$(document).ready( function(){
	...
	$("#tblGuardar").hide();
	$("#tblApertura").hide();
	$("#tblModificacion").hide();
	$("#tblCancelacion").hide();
	
	// Proceso Captura
	if(<%=id_oper == 5%> && nF[0] == "SRCB"){
		
		$("#nFolio").val(nF[2]);
		informacionIgresada(nF[2]);
		$("#titulo").html("Autorizacion de Apertura de Cuentas");
		$("#tblApertura").show();
		
	// Proceso Modificacion	
	}else if(<%=id_oper == 2%> && nF[0] == "MOCB"){
		
		$("#nFolioMod").val(nF[2]);
		informacionIgresada(nF[2]);
		$("#titulo").html("Autorizacion de Modificacion de Cuentas");
		$("#tblModificacion").show();
		
	}else if(<%=id_oper == 2%> && nF[0] == "BACB"){
		
		$("#nFolioCan").val(nF[2]);
		informacionIgresada(nF[2]);
		$("#titulo").html("Autorizacion de Cancelacion de Cuentas");
		$("#tblCancelacion").show();
	}
...
function cmdGuardar(rdEstatus){
	...
	(rdEstatus == true)? $("#autorizacionAperturaAutorizar").val(1) : $("#autorizacionAperturaAutorizar").val(0);
	
	var status = (rdEstatus == true)? "¿ Esta Seguro de Autorizar Cuenta Bancaria ?" : "¿ Esta Seguro de Rechazar Cuenta Bancaria ?";
	var msn = (rdEstatus == true)? "- Para Autorizar es Necesario Todos Los Campos " : "- Para Rechazar es Necesario Agregar Un Comentario";
	var statusGuardar = (rdEstatus == true)? " Cuenta Bancaria Autorizado Correctamente " : " Cuenta Bancaria Rechazada";
				
	if(confirm(status)){
		
		var maxEncab = (<%=id_oper == 5 %> && nF[0] == "SRCB")? "tCuentaBancariaEncabezadoRead" : (<%=id_oper == 2 %> && nF[0] == "MOCB") ? "tCuentaBancariaModEncabezadoRead" : "tCuentaBancariaCanEncabezadoRead" ;
		var ejecutarEncab = (<%=id_oper == 5 %> && nF[0] == "SRCB")? "tCuentaBancariaEncabezadoF" : (<%=id_oper == 2 %> && nF[0] == "MOCB") ? "tCuentaBancariaModEncabezadoF" : "tCuentaBancariaCanEncabezadoF" ;
		
			if( ($("#autorizacionAperturaContrato").val() != "" && $("#autorizacionAperturaComentario").val() != "" && rdEstatus == true) || (rdEstatus == false && $("#autorizacionAperturaComentario").val() != "") ){	
...
function ResponsableSiguiente(id_oper){
		...
		// Depende de O_Responsable de CG_OPERACION
  		 if(id_oper == 5 && $("#autorizacionAperturaAutorizar").val() == 1 && nF[0] == "SRCB"){
  			 
  			return "CONFIRMAR_UR";
  		 
  		 }else if(id_oper == 5 && $("#autorizacionAperturaAutorizar").val() == 0 && nF[0] == "SRCB"){
  			
  			 return "APERTURA_DE_CUENTAS";
  		 
  		 }else if(id_oper == 2 && $("#autorizacionAperturaAutorizar").val() == 1 && nF[0] == "MOCB"){
  			 
  			 return "CONFIRMAR_UR_MOD";
  			 
  		 }else if(id_oper == 2 && $("#autorizacionAperturaAutorizar").val() == 0 && nF[0] == "MOCB"){
  			 
  			 return "MOD_CUENTAS_BANCARIAS";
  			 
  		 }else if(id_oper == 2 && $("#autorizacionAperturaAutorizar").val() == 1 && nF[0] == "BACB"){
  			 
  			 return "CONFIRMAR_UR_CAN";
  			 
  		 }else if(id_oper == 2 && $("#autorizacionAperturaAutorizar").val() == 0 && nF[0] == "BACB"){
  			 
  			 return "CAN_CUENTAS_BANCARIAS";
  		 }
}
...
function informacionIgresada(folio){
	...
	var camposWhere = (<%=id_oper == 5 %> && nF[0] == "SRCB")? SRCB : (<%=id_oper%> == 2 && nF[0] == "MOCB")? MOCB : BACB ;
	var param = (<%=id_oper == 5 %> && nF[0] == "SRCB")? " nFolioCuentaBancaria = "+folio : (<%=id_oper%> == 2 && nF[0] == "MOCB")?  "nFolioCuentaBancariaMod = "+folio :  "nFolioCuentaBancariaCan = "+folio;
	var order = " ORDER BY nDocRenglon DESC ";
	var tabla = (<%=id_oper == 5 %> && nF[0] == "SRCB")? "TCUENTABANCARIAENCABEZADOINF" : (<%=id_oper%> == 2 && nF[0] == "MOCB")? "TCUENTABANCARIAENCABEZADOMODINF" : "TCUENTABANCARIAENCABEZADOCANINF";
	...
	$.getJSON("../catalogos/SelectJson.jsp",{Tabla:tabla, Campos:camposWhere, Param:param, Order:order, MaxReg: "1", ajax:"true" }, function(j){ 
			...
			for(var i = 0; i < j.length; i++ ){
					...
					if(<%=id_oper == 2 %> && nF[0] == "MOCB" || <%=id_oper == 2 %> && nF[0] == "BACB" ){
...
<body id="dt_example">
	<form id="frmCuentasBancarias" name="frmCuentasBancarias" >
			<div id="container" class="container SyCData" style="width:800px; align:center">
				<h1><div id="titulo"></div><label style="font-size: 9pt"></label></h1>
   				...
   				<table id="tblGuardar">
					...
					<tr>	
						...
						<td>centroContab<input type="text" id="cCentroContable" name="cCentroContable" value="<%=cCentroContable%>" /></td>
					...
					<tr>
						...
						<!-- input type="hidden" id="autorizacionAperturaContrato" name="autorizacionAperturaContrato" />
						<input type="hidden" id="autorizacionAperturaComentario" name="autorizacionAperturaComentario" / -->
					</tr>
					<tr>	
				...
				<tr><td>&nbsp;</td></tr>
   				<fieldset> 
   					<table id="tblAutorizacion" width="800px" height ="160px" style="text-align:center">
   							<tr>
   								<td>
	   							<table id="tblApertura">
		   							<tr><td>Para Continuar favor de Adjuntar la documentacion necesaria para ser Enviada a TESOFE:</td></tr>
		   							
		   							<tr><td>- Contrato Bancario </td></tr>
		   							<tr><td>- Tarjeta de Firmas </td></tr>
		   							<tr><td>- Confirmacion del Banco </td></tr>
		   							<tr><td>- Consentimiento Expreso </td></tr>
		   							<tr><td>- Memorandum CONAFOR </td></tr>
		   						</table>
		   						<table id="tblModificacion">
		   						
		   							<tr><td>Para Continuar con este tramite favor de anexar los documentos siguientes:</td></tr>
		   							
		   							<tr><td>&nbsp;</td></tr>
		   							<tr><td>- Genere Oficio de Solicitud de Modificacion de Cuenta para TESOFE</td></tr>
		   							
		   						</table>
		   						<table id="tblCancelacion">
		   						
		   							<tr><td>Para Continuar con este tramite favor de anexar los documentos siguientes:</td></tr>
		   							
		   							<tr><td>&nbsp;</td></tr>
		   							<tr><td>- Genere Oficio de Solicitud de Cancelacion de Cuenta para TESOFE</td></tr>
		   							
		   						</table>
	   							</td>
	   						</tr>	
   							<tr><td><input type="text" name="autorizacionAperturaContrato" id="autorizacionAperturaContrato" size ="50" maxlength="20" /></td></tr>
   				...
   				<fieldset>
   					<legend>Comentarios</legend>
   						...
   						<table id="tblAutoriza" width="800px" height ="150px">
   							<tr><td>&nbsp;</td></tr>
   							<tr><td><textarea name="autorizacionAperturaComentario" cols="90" rows="7" style="font-size: 10.5pt" onkeydown="return valida_concepto(event)" ></textarea></td></tr>
