	...
	<head>
		...
		$(document).ready(
			function() {
				...
				$("#mAcumulado").val(parseFloat(quitaFmt(window.opener.<%=formName%>.acumuladoOperacion.value)).toFixed(2));
								
				$("#mSaldo").val(  (parseFloat(quitaFmt( $("#mPorEjercer").val())) - parseFloat(quitaFmt($("#mAcumulado").val()))).toFixed(2) );
			...
			$("#pbAceptar").button().click(
				function() {			
					...
					$("#nFolioPago").val(window.opener.<%=formName%>.nFolioPago.value);
					$("#cTipoPago").val(window.opener.<%=formName%>.cTipoPago.value);
					$("#cCentroContable").val(window.opener.<%=formName%>.cCentroContable.value);
					...
					if ($("#EpUsada").val()== "Verdadero"){
						...
						for ( var i=0 ; i<aEPsDet.length ; i++ )
						{
							...
							$("#mImporteEP").val( Number(parseFloat(quitaFmt($("#mImporteEP").val())).toFixed(2)) + parseFloat( quitaFmt(aEPsDet[i][3].toFixed(2))) );
							$("#mImporteEP").val( Number(parseFloat(quitaFmt($("#mImporteEP").val())).toFixed(2)));
							window.opener.insertaObjeto( aEPsDet[i][0], aEPsDet[i][1], Number(quitaFmt(aEPsDet[i][3].toFixed(2))), aEPsDet[i][4] );						
						}	
			 					 			
						window.opener.<%=formName%>.EP.value=window.EPxRegistro.ep.value;
						window.opener.<%=formName%>.mMovimiento.value=window.EPxRegistro.mImporteEP.value;
		...
		function createDataTable(w) {
			...
			window.EPxRegistro.ID_DESTINO_GASTO.value=window.opener.<%=formName%>.ID_DESTINO_GASTO.value;
			window.EPxRegistro.TO_TIPO_DOCTO.value=window.opener.<%=formName%>.TO_TIPO_DOCTO.value;
			window.EPxRegistro.TIPO_CONCEPTO.value=window.opener.<%=formName%>.TIPO_CONCEPTO.value;
			window.EPxRegistro.cUnidadEjecutora.value=window.opener.<%=formName%>.cUnidadEjecutora.value;
			
			if (window.EPxRegistro.ID_DESTINO_GASTO.value == "CERE" || window.EPxRegistro.ID_DESTINO_GASTO.value == "CPRP" || window.EPxRegistro.ID_DESTINO_GASTO.value == "GCRE" ){
				if ( !window.opener.formPagos.chk_EditaImporte.checked ){
					window.EPxRegistro.mPorEjercer.value = Number(quitaFmt(window.opener.<%=formName%>.importeEdicionMaximo.value))+ Number(quitaFmt(window.opener.<%=formName%>.importenoComprobable.value));
				}else{
					window.EPxRegistro.mPorEjercer.value = Number(quitaFmt(window.opener.<%=formName%>.importeEdicion.value)) + Number(quitaFmt(window.opener.<%=formName%>.importenoComprobable.value));
				}
			} else if( window.EPxRegistro.ID_DESTINO_GASTO.value == "NORE" || window.EPxRegistro.ID_DESTINO_GASTO.value == "CLRE" || window.EPxRegistro.ID_DESTINO_GASTO.value == "GLRE"){
				window.EPxRegistro.mPorEjercer.value = Number(quitaFmt(window.opener.<%=formName%>.mImporteBruto.value));
			}else{	
				window.EPxRegistro.mPorEjercer.value = Number(quitaFmt(window.opener.<%=formName%>.mImporteNeto.value));
			}
						
			var destinoGasto = $('#ID_DESTINO_GASTO').val(); 
			...
			var urlAjaxSource = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vEpRadicadoRG&qw=ID_DESTINO_GASTO = '" + destinoGasto + "' and cTCONC='" + tconcepto + "'" + cunidadej + w <%= cFuenteFinanciamiento!=null && !"".equals(cFuenteFinanciamiento)?"+\" and fuentefinanciamiento = " + cFuenteFinanciamiento + "\"": "+\" and fuentefinanciamiento<>4 \"" %> + w<%= cTipoGasto!=null && !"".equals(cTipoGasto)?"+\" and TipoGasto = " + cTipoGasto + " and fuentefinanciamiento = " + cTipoGasto + "\"": "+\" AND FuenteFinanciamiento NOT IN (2,3) \"" %>;
		...
		if (!existe){
			 if ($("#mImporte").val() != "" ) {
			 	if ( Number(parseFloat($("#mImporte").val()).toFixed(2)) <= Number(parseFloat($("#mSaldo").val()).toFixed(2)) ){		
	  				...
	  				while ( (mes >= inicio) && ( parseFloat($("#mImporte").val()) > 0 )  )
		  				{  
		  					...
		  					if (parseFloat($("#mImporte").val())<= parseFloat($("#mImporteMes").val()) ){
		  						...
		  						$("#mImporteMes").val( Number(parseFloat($("#mImporteMes").val()).toFixed(2)) - valor);
		  						$("#mAcumulado").val(Number(parseFloat($("#mAcumulado").val()).toFixed(2)) + valor);
		  						$("#mAcumulado").val(Number(parseFloat($("#mAcumulado").val()).toFixed(2)));
		  						$("#mSaldo").val( $("#mPorEjercer").val() - Number(parseFloat($("#mAcumulado").val()).toFixed(2)) );
		  						$("#mImporte").val("0");
		...
		for ( var i=0 ; i<aEPs.length ; i++ )
		{
			if ( $(aEPs[i])[0].childNodes[1].innerText == $("#ep").val() )
			{
				...
				if ($(aEPs[i])[0].childNodes[1].innerText != "" ){
					alert ("Solo se pueden agregar deferentes Folios del ingreso pero con la misma EP, favor de Rectificar!!!");
					$("#nFolioIngreso").val("");
					$("#rifEP").val("");
					$("#nclavecna").val("");
					$("#mImporte").val("");
