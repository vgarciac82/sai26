...
<title>Excepciones para captura de Comisiones en Meses Anteriores</title>
...
<form id="formViaticos" name="formViaticos">
	...
	<div class="mx-auto ">
		<div class="row mt-2">
				<div class="card-header d-flex justify-content-center "><h4> Excepciones para Captura de Comisiones en Meses Anteriores</h4> </div>
		</div>
		...
		oTabla = $('#tablaEmpleados').dataTable(
			{
				...
				sAjaxSource : window.location.protocol + "//"
						...
						+ "/crud?rt=t&ql=vExcepcionesMesesAnterioresViaticos",
				aoColumns : [ {
					sName : "RFC"
				}, {
					sName : "Nombre"
...
function agregarExcepcion() {
	...
	queryFormPost("excepcionCapturaViatAnt", {async: false});
	location.reload();
...
function eliminarRegistro(RFC) {
	Swal.fire({
		  ...
		  if (result.isConfirmed) {
				...
				queryFormPost("eliminarExcepcionViatAnt", {async: false}); 
				location.reload();
		  }
		});
}
