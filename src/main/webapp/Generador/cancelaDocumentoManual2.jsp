...
<html>
	...
	function generaInputs(){
		...
		for(var i = 0; i < arrData.length; i++  ){
			...
			createInput('generaExcel', 'tipoConcepto', arrData[i][3]);
			createInput('generaExcel', 'tipoMovimiento', arrData[i][4]);
		}				
		$("#generaExcel").submit();
		return true;
	}
...
function buscarCXPVista(caNoContrarrecibo,cuentaPorPagar){ //CaNocontrarrecibo es el folio con el que se paga en el sai
	...
	var	locationV='';
	if(cuentaPorPagar!='')
	 	locationV = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=V_tPagadoMil&qw=caNoContrarrecibo = '"+caNoContrarrecibo+"' and CuentaPorPagar = '"+cuentaPorPagar+"'";
	else{
	...
	var al = [
			...
			{ sName: "CuentaPorPagar",bSearchable: true,	bSortable: false, bVisible: true, sClass: "alignCenter", sWidth:"40px" },
...
function agregarClave(){
	...
	$('#tablaCLC input:checked').each(function(idx, elm){
		...
		arrlist[i] = new Array(paso[5],paso[6],paso[7],paso[8],paso[9]);
					...
					<td align="right">Documento:</td>
						...
						<select id= "opcion" name="opcion" >
							<option value="1" selected="selected">
								RECTIFICACIÓN AUTORIZADA </option>
							<option value="2">
								REINTEGRO AUTORIZADO </option>
						</select>
					</td>
				</tr>
					<tr>
						<td align="right">Folio: </td> 
						<td> <input align="right" type="text" id="Idfolio" name="Idfolio" value=""/>
						</td>
					</tr>
					<tr></tr>
					<tr>
					<td align="center"  colspan="3">
					    <input type="button" id="btnBuscar" name ="btnBuscar" value="Buscar" onclick="javascript:BuscarFolio();" ></input>
