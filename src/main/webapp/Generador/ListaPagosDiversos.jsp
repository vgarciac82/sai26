    	<script type="text/javascript">
    		...
    		function fnGrid(pParam, pFechaIni, pFechaFin, UMA)
    		{
    			...
    			if(UMA == "0"){  // MONTO MAYOR A 300 UMAS
     			...
     			var oTableLocal = $('#dt_generados').dataTable({    				
					...
					bServerSide: true,
					// sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vListaPagosDiversos&qw=" + (pParam == 0 ? "" : "UNIDAD_EJECUTORA='" + pParam + "' and ") + "FECHA_PROGRAMADAQuery BETWEEN convert(datetime,'" + pFechaIni + " 00:00',103) AND convert(datetime,'" + pFechaFin + " 23:59:59',103)",
					sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vListaPagosDiversos&qw=" + (pParam == 0 ? "" : "UNIDAD_EJECUTORA='" + pParam + "' and ") + "CAST(FECHA_PROGRAMADA AS SMALLDATETIME) BETWEEN '" + pFechaIni + "' AND '" + pFechaFin + "'" + sWhereUMAS,
					bProcessing: true,
					sPaginationType: "full_numbers",
					bJQueryUI: true,
					aoColumns: [
					    { sName: "id",						bSearchable: true,	bSortable: false, bVisible: true, sClass: "alignCenter"},
						{ sName: "cEsFirmaElectronica",		bSearchable: true,	bSortable: true, bVisible: true},
			  			...
			  			<table id="dt_generados" class="table table-striped table-bordered" >					  				
							<thead>
								<tr align="center">
		            				...
		            				<th>Concepto de la solicitud de pago</th>
