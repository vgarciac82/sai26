    		...
    		function fnGrid(pParam, pFechaIni, pFechaFin, UMA)
    		{
    			...
    			if(UMA == "0"){  // MONTO MAYOR A 300 UMAS
     			...
     			var oTableLocal = $('#dt_generados').dataTable({    				
					...
					bServerSide: true,
					sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vListaPagoFederalizado&qw=" + (pParam == 0 ? "" : "UNIDAD_EJECUTORA='" + pParam + "' and ") + "CAST(FECHA_PROGRAMADA AS DATE) BETWEEN '" + pFechaIni + "' AND '" + pFechaFin + "'"  + sWhereUMAS ,
					bProcessing: true,
					sPaginationType: "full_numbers",
					bJQueryUI: true,
					aaSorting: [[ 3, "asc" ]] ,
    			...
    			$("#dt_generados tbody").click(function(event) {
					...
					$("#pbXAutorizar").css("visibility","visible");
