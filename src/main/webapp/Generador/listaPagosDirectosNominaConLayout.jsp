    	<script type="text/javascript">
    		function fnGridConLayout(pParam, pFechaIni, pFechaFin)
    		{
    			...
    			var oTableCuentaConLayout = $('#dt_CuentaConLayout').dataTable({
					...
					bServerSide: true,
					sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vListaCuentasConLayoutNomina&qw=" + (pParam == 0 ? "" : "UNIDAD_EJECUTORA='" + pParam + "' AND ") + "FECHA_PROGRAMADAQuery BETWEEN CAST('" + pFechaIni + "' as date) AND CAST('" + pFechaFin + "' as date)",
					bProcessing: true,
					sPaginationType: "full_numbers",
					bJQueryUI: true,
					aaSorting: [[ 2, "asc" ]] ,
