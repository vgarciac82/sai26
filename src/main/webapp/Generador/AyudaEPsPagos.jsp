...
<%
	...
	String esGreenMex = null;
	if( request.getParameter("esGreenMex") != null )
		esGreenMex = request.getParameter("esGreenMex");
		
	String formName = StringUtils.isEmpty(request.getParameter("formName")) ? "formPagos":request.getParameter("formName");	
	
	String solicitudPD = "";
	if( request.getParameter("solicitudPD") != null )
		solicitudPD = request.getParameter("solicitudPD");
...
<!-- 		<script type="text/javascript" src="../Generador/js/jquery.dataTables.js"></script> -->
	...
	function createDataTable(w) {		
		window.MultiReporteHija.ID_DESTINO_GASTO.value=window.opener.<%=formName%>.ID_DESTINO_GASTO.value;
		window.MultiReporteHija.TO_TIPO_DOCTO.value=window.opener.<%=formName%>.TO_TIPO_DOCTO.value;
		window.MultiReporteHija.TIPO_CONCEPTO.value=window.opener.<%=formName%>.TIPO_CONCEPTO.value;
		window.MultiReporteHija.cUnidadEjecutora.value=window.opener.<%=formName%>.cUnidadEjecutora.value;
		window.MultiReporteHija.cMeta.value=window.opener.<%=formName%>.cMeta.value;
		...
		if(destinoGasto != "NORE" && destinoGasto != "CLRE" && destinoGasto != "GLRE" && destinoGasto != "CQRE"){
			window.MultiReporteHija.nIdComision.value=window.opener.<%=formName%>.nIdComision.value;
			nIdComision = $('#nIdComision').val();
		} else {
			nIdComision = 0;
		}
		...
		var cWhereDocto = "left(Partida, 2) <> '43'";
		if(tipodocto == 'DIRECTO'){
			cWhereDocto = "Partida not in (select cPartida from tPartidaComsoc)";
		}
		$('#tblSaldos').dataTable().fnClearTable(); 
		...
		var cWhereSolPD = "";
		var solPD = "<%=solicitudPD%>";
		if(solPD != ""){
			cWhereSolPD = " AND UnidadEjecutora = '" + solPD.substr(3,3) + "' AND Partida IN (SELECT cIdSubPartida FROM mSolicitudLineas WITH (NOLOCK) WHERE cIdSolicitud = '" + solPD + "') ";
		}
		
		if(tconcepto == 'FFM'){
			var cWhere = " ID_DESTINO_GASTO='" + destinoGasto + "' AND cTCONC='" + tconcepto + "'" + cunidadej + w <%=cFuenteFinanciamiento!=null && !"".equals(cFuenteFinanciamiento)?"+\" and fuentefinanciamiento = " + cFuenteFinanciamiento + "\"": "+\" and fuentefinanciamiento<>4 \"" %> + w<%= cTipoGasto!=null && !"".equals(cTipoGasto)?"+\" and TipoGasto = " + cTipoGasto + " and fuentefinanciamiento = " + cTipoGasto + "\"": " + \" AND FuenteFinanciamiento NOT IN (2,3) AND ProgramaGeneral <> '00' \"" %> + cunidadej + cWhereSolPD;
		}else{
			var cWhere = " ID_DESTINO_GASTO='" + destinoGasto + "' AND cTCONC='" + tconcepto + "'" + cunidadej + w <%=cFuenteFinanciamiento!=null && !"".equals(cFuenteFinanciamiento)?"+\" and fuentefinanciamiento = " + cFuenteFinanciamiento + "\"": "+\" and fuentefinanciamiento<>4 \"" %> + w<%= cTipoGasto!=null && !"".equals(cTipoGasto)?"+\" and TipoGasto = " + cTipoGasto + " and fuentefinanciamiento = " + cTipoGasto + "\"": " + \" AND FuenteFinanciamiento NOT IN (2,3) \"" %> + cunidadej + cWhereSolPD;		
		}
		
		if(destinoGasto != "NORE" && destinoGasto != "CLRE" && destinoGasto != "GLRE" && destinoGasto != "CQRE"){
			if($("#nIdComision").val() != "0"){
				cWhere = cWhere + " AND (SUBSTRING(Partida,1,2) = '37' OR SUBSTRING(Partida,1,5) = '33602')";				
			}
			if($('#cMeta').val()!=""){
				cWhere = cWhere + " AND Cartera = '" + $("#cMeta").val() + "'"; 
			}
		}
		
		if("<%=esGreenMex%>"=="SI"){
			cWhere = cWhere + " AND UnidadNorativa IN (SELECT cUnidadNormativa FROM tCatUnidadNormativaGreenMex WITH (NOLOCK))";
		} else {
			cWhere = cWhere + " AND UnidadNorativa NOT IN (SELECT cUnidadNormativa FROM tCatUnidadNormativaGreenMex WITH (NOLOCK))";
		}		
	...
	<form id="MultiReporteHija" name="MultiReporteHija">
		<div id="container" class="container">
			<h1>Estructura Programática</h1>
				...
				<input type="hidden" id="nIdComision" name="nIdComision" value=""/>
				<input type="hidden" id="cMeta" name="cMeta" value=""/>
