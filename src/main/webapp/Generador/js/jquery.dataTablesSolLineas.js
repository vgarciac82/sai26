		...
		"full_numbers": {
			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
			{
				...
				$(nFirst).bind( 'click.DT', function () {
					...
					{
						fnCallbackDraw( oSettings );
						if (oSettings.sTableId == "tblLineas")
							metodo(1);
					}
				} );
				...
				$(nLast).bind( 'click.DT', function() {
					...
					{
						if (oSettings.sTableId == "tblLineas")
							metodo(Math.ceil((oSettings.fnRecordsDisplay()) / oSettings._iDisplayLength));
						fnCallbackDraw( oSettings );
					}
				} );
			...
			"fnUpdate": function ( oSettings, fnCallbackDraw )
			{
				...
				for ( i=1 ; i<=iPages ; i++ )
				{
					var ver = " style='display:none'";
					if (i>=iStartButton && i<=iEndButton)
						ver = "";
					if ( iCurrentPage != i )
					{
						if (oSettings.sTableId == "tblLineas")
							sList += '<span id=' + i + ' onclick="metodo('+i+');" class="'+oClasses.sPageButton+'"' + ver + '>'+i+'</span>';
						else
							sList += '<span id=' + i + ' class="'+oClasses.sPageButton+'"' + ver + '>'+i+'</span>';
					}
					else
					{
						if (oSettings.sTableId == "tblLineas")
							sList += '<span id=' + i + ' onclick="metodo('+i+');" class="'+oClasses.sPageButtonActive+'"' + ver + '>'+i+'</span>';
						else
							sList += '<span id=' + i + ' class="'+oClasses.sPageButtonActive+'"' + ver + '>'+i+'</span>';	
					}
				}
