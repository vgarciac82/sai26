	...
	*/	
		...
		queryFormPost({
			...
			callback : function() {
				...
				if (operAct == "3") {
					alert("No se puede modificar porque el contrato está en consulta.")
					$.unblockUI();
				} else {
					;
					$("#mainForm").attr("action","../gstnmngr/gestion?cmd=3&caso="+$("#idCaso").val()+"&casoOper="+$("#idOper").val());
				    $("#mainForm").submit();
				}
			}
		});
	...
	$("#resultTable tbody").dblclick(function(evt) {
		...
		$("#idCaso").val(arr[6]);
		$("#idOper").val(arr[7]);
		$("#msgAdvertencia").val("Desea modificar datos del contrato " + folioSAI);
		$("#msgAdvertencia").text(
				"Desea modificar datos del contrato con folio: \n"	+ folioSAI);
		$.blockUI({
			message : $('#question'),
			css : {
				width : '375px'
...
// else {
		...
		condition += token + "  isnull(estatuscomp,'') != 'S' ";
		...
		$.getJSON("../catalogos/SelectJson.jsp", {
			...
			for ( var i = 0; i < data.length; i++) {
				dTable.fnAddData([
						...
						data[i].Col2,
						data[i].Col3,
						 data[i].Col4, data[i].Col5, data[i].Col14, data[i].Col15 ]);
			}
			$.unblockUI();
			if (i == 0){
