...
<script type="text/javascript" charset="UTF-8">
		...
		oTableSaldos = $("#tblSaldos").dataTable({
			...
			$("#tblEP tbody").click(function(event) {
				...
				var szTabla = "VSALDOSANUALESINGRESO";                                                                                         
			...
			.click(function() {
				...
				if( index >= 0 ){	
					...
					window.opener.frmRegistroIngreso.EP.value=window.MultiReporteHija.ep.value ;
					window.MultiReporteHija.mesDisponible.value=window.opener.frmRegistroIngreso.mesDisponible.value ;

					fecha = parseInt($("#mesDisponible").val(),10);
					var disponible = 0;
					var monto = "";
					for ( var i = 0; i < fecha; i++) {
						var y = oTableSaldos.fnGetNodes()[0].cells[i];
						monto = quitaFmt($(y)[0].innerText);
						disponible += parseFloat(monto);
					}
					
					monto = disponible.toFixed(2);
					
					$("#montoDisponible").val(monto);
					window.opener.frmRegistroIngreso.mImporte.value=window.MultiReporteHija.montoDisponible.value ;
					window.opener.frmRegistroIngreso.montoDisponible.value=window.MultiReporteHija.montoDisponible.value;
					window.close(); 			
			});		
	...
	function createDataTable(w) {		
		...
		window.MultiReporteHija.cUnidadEjecutora.value=window.opener.frmRegistroIngreso.cUnidadEjecutora.value;
		window.MultiReporteHija.fuenteFinanciamiento.value=window.opener.frmRegistroIngreso.fuenteFinanciamiento.value;
		...
		var cWhereFFinanciamiento;
		
		if($("#fuenteFinanciamiento").val() == "0"){
			cWhereFFinanciamiento = " FuenteFinanciamiento = 0 AND FuenteFinanciamiento NOT IN (2,3) ";
		}else if($("#fuenteFinanciamiento").val() != ""){
			cWhereFFinanciamiento = " FuenteFinanciamiento = 4 AND FuenteFinanciamiento NOT IN (2,3) ";
		}else{
			cWhereFFinanciamiento = " (FuenteFinanciamiento <> 4 OR FuenteFinanciamiento <> 0) AND FuenteFinanciamiento NOT IN (2,3) ";
		}
		
		$('#tblSaldos').dataTable().fnClearTable(); 
		
		var f = "v_CatalogoEPS_Ingresos";
		return $("#tblEP").dataTable({
				...
				"/crud?rt=t&ql=v_CatalogoEPS_Ingresos&qw=" + cWhereFFinanciamiento + cunidadej,				
				bProcessing: true,
...
<body id="dt_example">
	<form id="MultiReporteHija" name="MultiReporteHija">
		<div id="container" class="container" style="width:90%" >
			...
			<label>Saldos Disponibles</label> <br>
			<input type="hidden" id="ep" 					name="ep" 					value="" />
			<input type="hidden" id="disponible" 			name="disponible"			value="" /> 
			<input type="hidden" id="mesDisponible"			name="mesDisponible"		value="" />
			<input type="hidden" id="montoDisponible"			name="montoDisponible"	value="" />
			...
			<br />
			<center>
				<input type="button" id="pbAceptar" style="visibility: hidden" value="Aceptar" class="btnInterfaceBG"/>
				&nbsp;&nbsp;&nbsp;
				<input type="button" id="pbCancelar" style="visibility: visible" value="Cancelar" class="btnInterfaceBG"/>
			</center>
		</div>
	</form>
</body>
