    	<script type="text/javascript">
    		...
    		function fnGrid(pParam, pFechaIni, pFechaFin, tipoLayout)
    		{
				if (pParam == 'A02') pParam = 0;
     			...
     			var oTableLocal = $('#dt_generados').dataTable({         				
					...
					bServerSide: true,			
					sAjaxSource: window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vListaCuentasporPagar&qw=" + (pParam == 0 ? "" : "UNIDAD_EJECUTORA='" + pParam + "' and ") + "FECHA_PROGRAMADA BETWEEN '" + pFechaIni + "' AND '" + pFechaFin +  "'",													
					bProcessing: true,
					sPaginationType: "full_numbers",
					bJQueryUI: true,
					aaSorting: [[ 3, "asc" ]] ,
