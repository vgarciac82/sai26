	...
	<head>
		...
		$(document).ready(
			function() {
				...
				$("#mAcumulado").val(window.opener.frmcontratoDeObra.saldoCompM.value);
				$("#mSaldo").val( $("#mPorEjercer").val()- $("#mAcumulado").val() );
				...
				function() {			
					...
					if ($("#EpUsada").val()== "Verdadero"){
						...
						for ( var i=0 ; i<aEPsDet.length ; i++ )
						{
							...
							$("#mImporteEP").val( parseFloat( $("#mImporteEP").val() ) + parseFloat( aEPsDet[i][3]) );
							
							window.opener.insertaObjeto( aEPsDet[i][0], aEPsDet[i][1], aEPsDet[i][3], aEPsDet[i][4] );						
						}	
						...
						window.opener.frmcontratoDeObra.montoDev.value=window.EPxRegistro.mImporteEP.value;
		...
		function createDataTable() {
			...
			var urlAjaxSource = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split("/")[1] + "/crud?rt=t&ql=vConsultaCompromisoEP_RIF&qw=cIdContrato='" + nFolioCompromiso + "'" + cunidadej + " AND fuentefinanciamiento <> 4 "; 
		...
		if (!existe){
			 if ($("#mImporte").val() != "" ) {
			 	if (parseFloat($("#mImporte").val()) <= parseFloat($("#mSaldo").val())){		
					...
					queryFormPost("mesesRIF", {async:false});
	                ...
	                var valor = 0;
	  				...
	  				while ( (inicio <= fin) && ( parseFloat($("#mImporte").val()) > 0 )  )
		  				{  
		  					$("#tmpMes").val(inicio);
		  					queryFormPost("importeMesRIF", {async:false});
		  					
		  					if (parseFloat($("#mImporte").val())<= parseFloat($("#mImporteMes").val()) ){
		  						
		  						valor = parseFloat($("#mImporte").val());
		  						...
		  						valor = parseFloat($("#mImporteMes").val());
		  						$("#mImporte").val( parseFloat($("#mImporte").val()) - valor);
		  						$("#mAcumulado").val(parseFloat($("#mAcumulado").val()) + valor);
		  						$("#mSaldo").val( $("#mPorEjercer").val() - parseFloat($("#mAcumulado").val()) );
		  						$("#mImporteMes").val("0");
			...
			if ($("#nFolioIngreso").val() != ""){
				alert ("Registro de Ingreso Fiscal con EP ya utilizado, favor de rectificar!!!");
				$("#nFolioIngreso").val("");
				$("#rifEP").val("");
				$("#nclavecna").val("");
				$("#mImporte").val("");
		...
		for ( var i=0 ; i<aEPs.length ; i++ )
		{
			if ( $(aEPs[i])[0].childNodes[1].innerText == $("#ep").val() )
			{
				...
				if ($(aEPs[i])[0].childNodes[1].innerText != "" ){
					alert ("Solo se pueden agregar deferentes registros de ingreso pero con la misma EP, favor de Rectificar!!!");
					$("#nFolioIngreso").val("");
					$("#rifEP").val("");
					$("#nclavecna").val("");
					$("#mImporte").val("");
...
<body id="dt_example" >
	<form id="EPxRegistro" name="EPxRegistro" >
		<div id="container" class="container">
			<h1>Relacion entre el Pago y su Recurso Fiscal Recaudado</h1>
			...
			<br>
			<label style="POSITION:static;">Disponible Ingreso Fiscal</label>	
			...
			<fieldset>
				...
				<table align="center" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td align="center"> <label> Folio Ingreso &nbsp;</label></td>
			...
			<br>
			<label style="POSITION:static;">Detalle EP Ingreso Fiscal a Devengar</label>
