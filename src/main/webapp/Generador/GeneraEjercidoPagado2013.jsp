...
$(document).ready(function () {
	...
	var TableMovCXP = $('#movimientoCXP').dataTable({         
		...
		$("#movimientoCXP tbody").click(function(event) {
			...
			if($("#tipoDoc").val() == "r_gastos" && $("#selectRelacion").val() == "integracion"){
				...
				if(confirm("Agregar Folio "+aData[0]+" Para Comparar")){
					var folios = $("#flcxp").val()+"/"+aData[0];
					$("#flcxp").val(folios);
					
							if($("#estatusDocumento").val() == "Ejercido" ){	
							            		
							          var tipoPago = "RELACIONGASTOS";
									  var cxp = aData[0];
												
												var zTabla = "TPAGADOENCABEZADO";
												var camposWhere = " WHERE cTipoPago = '" +tipoPago+ "' AND caNoContrarrecibo IN ('"+cxp+"')"; 
												var param = "";
												var nFolioAplicarPagado = "";
																			
												$.getJSON("../catalogos/SelectJson.jsp",{Tabla:"TPAGADOENCABEZADO", Campos:camposWhere, Param:param, MaxReg: "5", ajax:"true" }, function(j){ 
													...
													for(var i = 0; i < j.length; i++ ){
														...
														nFolioAplicarPagado += "/";
														nFolioAplicarPagado += $("#nFolioAplicarPagado").val();
	...
	function muestraDatosCxp(){
			...
			$("#nFolioAplicarPagado").val("");
			$("#btnAplicarPagado").attr("disabled","disabled");
			$("#bDatos").removeAttr("disabled","disabled");
			$("#tblText").val("");
			$("#tblText").hide();
			...
			if((tipoDoc == "seleccione" || idCXP == "") && tipoDoc != "r_gastos"){
				...
				if(tipoDoc == "r_gastos"){
						...
						if($("#relacionGastosIntegracion").val() != ""){
							...
							szTabla = "tRELACIONGASTOSEncabezado";
							elParametro = "caNoContrarrecibo = '"+idCXP+"'";
							queryFormPost( {queryName: "numeroCLCSICOP2", async: false, callback:function(){}});
						}
					
				}if(tipoDoc == "p_diverso"){
					$('#movimientoCXP').dataTable().fnClearTable();
					campos = "caNoContrarrecibo,nFolioPAGODIVERSO,(SELECT dNombre FROM tBeneficiario WHERE dRFC = RFC) AS Beneficiaro,CONVERT(varchar,fAplicacion,103),'-' as fRecepcion,CONVERT(varchar,CONVERT(money,mImporteBruto),1) AS importeBruto,CONVERT(varchar,CONVERT(money,mImporteIVA),1) AS importeIVA,isnull(CONVERT(varchar,CONVERT(money, CONVERT(money,mImporteRetencion) + (select mImportePenalizacion from tContrarrecibo where caNoContrarrecibo = '"+idCXP+"')),1),'0.00') as totalRetenciones,CONVERT(varchar,CONVERT(money,mImporteNeto),1) AS importeNeto,nFolioPolizaCancelacion, isnull(cDocumentoHaplicado, 'N') AS sinAplicar ";
					...
					$('#movimientoCXP').dataTable().fnClearTable();
					// antes campos = "caNoContrarrecibo,nFolioNOMINA,(SELECT dNombre FROM tBeneficiario WHERE dRFC = cIdRFC) AS Beneficiaro,CONVERT(varchar,fAplicacion,103),CONVERT(varchar,fRecepcion,103),CONVERT(varchar,CONVERT(money,mImporteBruto),1),'-','-',CONVERT(varchar,CONVERT(money,mImporteNeto),1) AS importeNeto,nFolioPolizaCancelacion";
					...
					// antes szTabla = "tNominaEncabezado";
					...
					$('#movimientoCXP').dataTable().fnClearTable();
					campos = "caNoContrarrecibo,nFolioOperAjenas,(SELECT dNombre FROM tBeneficiario WHERE dRFC = cIDRFC) AS Beneficiaro,CONVERT(varchar,fAplicacion,103),'-' AS fRecepcion, '-' AS importeBruto, '-' AS importeIVA, '-' AS totalRetenciones,CONVERT(varchar,CONVERT(money,mImportes),1) AS importeNeto,nFolioPolizaCancelacion, isnull(cDocumentoHaplicado, 'N') AS sinAplicar ";
			...
			if(tipoDoc == "r_gastos" && $("#relacionGastosIntegracion").val() != ""){
					...
					$.ajax({
							...
							type:'post',
							data:{tipo:'existeIntegracionPagado',nomCampoId:'sAuxiliarComodin',idCab:idIntegracion ,tabla:szTabla},
							success:function(data){
								...
								if(data.sinSesion == 'sinSesion'){
									...
									alert("La Integracion "+idCXP+" Esta Ejercido y Pagado");
									return;
					              	...
					              	if(data.estatus == "Ejercido"){	
						            		...
						            		//var tipoPago = $("#tipoDoc").val();
											$("#estatusDocumento").val(data.estatus);
											
											var camposWhere = " WHERE cTipoPago = '" +cTipoPago+ "' AND sAuxiliarComodin = '"+$("#relacionGastosIntegracion").val()+"' "; 
											var param = "";
											var nFolioAplicarPagado = "";
																		
											$.getJSON("../catalogos/SelectJson.jsp",{Tabla:"TPAGADOENCABEZADOINTEGRACION", Campos:camposWhere, Param:param, MaxReg: "5", ajax:"true" }, function(j){ 
									...
									$.getJSON("../catalogos/SelectJson.jsp",{Tabla: szTabla, Campos:campos, Param: elParametro, MaxReg: "5", ajax: 'true'}, function(j){
											...
											for (var i = 0; i < j.length; i++) {
													...
													if(j[i].Col9 > 0){
														...
														alert("Documento "+contraRecibo+" Esta Cancelado.");
														$('#movimientoCXP').dataTable().fnClearTable();
														return;
													
													}else if(j[i].Col10 == "N"){
														
														alert("Documento "+contraRecibo+" No Aplicado Como Devengado");
														$('#movimientoCXP').dataTable().fnClearTable();
														return;
													}
											}
					...
					$.ajax({
							...
							success:function(data){
								...
								if(data.sinSesion == 'sinSesion'){
					            	...
					            	if(data.estatus == "Ejercido" && $("#selectRelacion").val() != "diferente" ){	
					            	...
					            	}else if (data.estatus == "Ejercido" && $("#selectRelacion").val() == "diferente" ){
									...
									$.getJSON("../catalogos/SelectJson.jsp",{Tabla: szTabla, Campos:campos, Param: elParametro, MaxReg: "5", ajax: 'true'}, function(j){
											...
											for (var i = 0; i < j.length; i++) {
													...
													if(j[i].Col9 > 0){
														...
														alert("Documento No Aplicado Como Devengado");
														//$('#movimientoCXP').dataTable().fnClearTable();
														return;
													}
	...
	function muestraDatosCxpDetalles(idFolio){
			...
			if(tipoDoc == "p_directo"){
					...
					campos = " nFolioPagoDirecto = "+idFolio+" GROUP BY SUBSTRING(EP, 0,56)";
					elParametro = ""; 
			}
			if(tipoDoc == "r_gastos"){
					...
					campos = "nFolioRELACIONGASTOS = "+idFolio+" GROUP BY SUBSTRING(EP, 0,56)";
					elParametro = ""; 
					
			}if(tipoDoc == "p_diverso"){
					//campos = "nDocRenglon,EP,CONVERT(varchar,CONVERT(money,mImporteBruto),1) AS importeBruto,CONVERT(varchar,CONVERT(money,mImporteIva),1) AS importeIVA,CONVERT(varchar,CONVERT(money,mRetencion),1) AS retencion,CONVERT(varchar,CONVERT(money,mImporteNeto),1) AS importeNeto";
					szTabla = "tPAGODIVERSODetalle";
					campos = "nFolioPAGODIVERSO ="+idFolio+" GROUP BY SUBSTRING(EP, 0,56) ";
					elParametro = "";
			}if(tipoDoc == "p_obra"){
					...
					campos = "nFolioPAGOOBRA ="+idFolio+" GROUP BY SUBSTRING(EP, 0,56)";
					elParametro = "";
			}if(tipoDoc == "nomina"){
					//campos = "nDocRenglon,EP,CONVERT(varchar,CONVERT(money,mImporteBruto),1) AS importeBruto,CONVERT(varchar,CONVERT(money,mImporteIva),1) AS importeIVA,CONVERT(varchar,CONVERT(money,mRetencion),1) AS retencion,CONVERT(varchar,CONVERT(money,mImporteNeto),1) AS importeNeto";
					// antes szTabla = "tNOMINADetalle";
					szTabla = "tNOMINACLCDetalle";
					// antes campos = "nFolioNOMINA ="+idFolio+ " GROUP BY EP ";
					campos = "nFolioNOMINACLC ="+idFolio+ " GROUP BY SUBSTRING(EP, 0,56) ";
					elParametro = "";
			}if(tipoDoc == "federalizado"){
					//campos = "nDocRenglon,EP,CONVERT(varchar,CONVERT(money,mImporteBruto),1) AS importeBruto,CONVERT(varchar,CONVERT(money,mIva),1) AS importeIVA,CONVERT(varchar,CONVERT(money,mRetencion),1) AS retencion,CONVERT(varchar,CONVERT(money,mImporteNeto),1) AS importeNeto";
					szTabla = "tPAGOFEDERALIZADODetalle";
					campos = "nFolioPAGOFEDERALIZADO ="+idFolio+" GROUP BY SUBSTRING(EP, 0,56) ";
					elParametro = "";
			}if(tipoDoc == "o_ajenas"){
					szTabla = "tOperAjenasDetalle";
					campos = "nFolioOperAjenas ="+idFolio+" GROUP BY SUBSTRING(EP, 0,56) ";
					elParametro = "";
			}
	...
	function validarDatos(){
		...
		if(tipoDoc == "p_directo"){
			...
			nomCamposDet = "SUBSTRING(EP, 0,56) as EP,SUM(CONVERT(money,mImporteNeto)) AS mImporteNeto, cCentroContable,RFC,cEjercicio,cIdDocumento AS cIdRelacion,nCapitulo"; //Campos Detalle para Comparar SICOP
			...
			if($("#relacionGastosIntegracion").val() != ""){
					...
					nomCamposDet = "SUBSTRING(EP, 0,56) as EP, SUM(CONVERT(money,mImporteNeto)) AS mImporteNeto,cCentroContable,RFC,cEjercicio, cIdRelacion,nCapitulo"; //Campos Detalle para Comparar SICOP;
					nomIdEnc = "caNoContrarrecibo";
			...
			nomTabla = "tNOMINACLCEncabezado";
			nomTablaDet = "tNOMINACLCDetalle";
			nomCampos = "nFolioNOMINACLC as idFolio, mImporteNeto as impNeto,aEjercicioFiscal";
			nomCamposDet = "SUBSTRING(EP, 0,56) as EP, SUM(CONVERT(money,mImporteNeto)) AS mImporteNeto,cCentroContable,RFC,cEjercicio, cIdRelacion, '1000' nCapitulo"; //Campos Detalle para Comparar SICOP;
			nomIdEnc = "caNoContrarreciboCLC";
			nomIdDet = "nFolioNOMINACLC";
			...
			nomCamposDet = "SUBSTRING(Ep, 0,56) AS EP, SUM(CONVERT(money,mTotal)) AS mImporteNeto,cCentroContable,aEjercicioFiscal AS cEjercicio";
			nomIdEnc = "caNoContrarrecibo";
	...
	{	
		...
		if(tipoDoc == "p_directo"){
			...
			nomCamposDetValidar = "cEvento, SUBSTRING(EP, 0,56) as EP, cCentroContable,SUM(CONVERT(money,mImporteNeto)) AS mImporteNeto,RFC,cEjercicio,cIdDocumento AS cIdRelacion,nCapitulo,cMes";
			...
			if($("#relacionGastosIntegracion").val() != ""){
				...
				nomCamposDet = "nDocRenglon,cMes,cEjercicio, EP,cIdCuentaContable,mComprometido,isnull(nPoliza,0) AS nPoliza,ID_TIPO_MOVIMIENTO,ID_TIPO_CONCEPTO,cEvento,cCentroContable,RFC,mImporteNeto,ALM,mImporteBruto,mImporteMasIva,mImporteIva,nCapitulo,mSancion,mDevolucion,mImporteAmortiza,mRetencion,mPenalizacion,m2Millar,m23IVA,mISRHonorarios,mObra5,mImporteFlete4,mISRArrenda,mRetImpuestoCedular,mImporte,mImporteIvaArrenda,mImporteIvaHonorarios,mImporteFlete23,mImporteIvaProv,mImporteObra,isnull(mCNIC,0.00) AS mCNIC,isnull(mIMDT,0.00) AS mIMDT,isnull(mTesofe,0.00) AS mTesofe,altaAlmacen,cCentroContable AS cIdEntidadContable, cIdRelacion, CTABM";
				nomCamposDetValidar = "cEvento, SUBSTRING(EP, 0,56) as EP, SUM(CONVERT(money,mImporteNeto)) AS mImporteNeto,cCentroContable,RFC,cEjercicio, cIdRelacion,nCapitulo,cMes";
				nomCXP = "caNoContrarrecibo";
			...
			nomCamposDetValidar = "cEvento, SUBSTRING(Ep, 0,56) AS EP, SUM(CONVERT(money,mTotal)) AS mImporteNeto,cCentroContable,RFC,aEjercicioFiscal AS cEjercicio,cMes";
			nomCXP = "caNoContrarrecibo";
		...
		if(tipoDoc == "r_gastos" && $("#relacionGastosIntegracion").val() != ""){
				$.ajax({
					...
					success: function(data){
						...
						}else if(data.estatus == "Ejercido"){
							alert("Aplicado Correctamente Ejercido");
							location.reload();
							
						}else if(data.estatus == "Pagado"){
							alert("Aplicado Correctamente Ejercido y Pagado");
							location.reload();
	...
	function setSequenceValP(seqValue) 
{
						...
						$.ajax({
								...
								success: function(data){
										...
										if(data.estatus == "correcto"){
											...
											var detalle = data.estatus.split(":");
											//alert("LENGHT :"+detalle.length);ok
											if( detalle.length == 2){
  ...
  <body id="dt_example">
  <form id="generaEjercidoPagado">
		<div  id="container">
			<h1>Ejercido Pagado<label style="font-size: 8pt"></label></h1>			
        	<div id="tabsl">    		
        		<table id="tblEjecidoPagado" align="center" width="1200px">	
        		<tr>
					<td>
						...
						<fieldset>
							<table id="tblBoton" align="center" width="1200px">
								<tr>
									...
									<td><input type="hidden" id="fPago" name="fPago" value="fecha"></td>
									...
									<td><input type="hidden" id="nFolioAplicarPagado" name="nFolioAplicarPagado" /></td> 
