	...
	$(document).ready(
		function() 
		{			
			szID = window.location.href; 
			if (szID.indexOf("dlg.jsp")>0)
			{
				szID = szID.substr(szID.indexOf("?")+4); 
				...
				if (AyudaXML.load(URL + szID + ".xml"))
				{
					spTitulo.innerText = AyudaXML.selectSingleNode("Ayuda/titulo").text;
					szTabla = AyudaXML.selectSingleNode("Ayuda/tabla").text; 
					iMaxReg = parseInt(AyudaXML.selectSingleNode("Ayuda/maxRegistros").text,10);
					try
					{
						iColSort = parseInt(AyudaXML.selectSingleNode("Ayuda/ColSort").text,10);
					}catch (ex){
						iColSort=1;
					}
					
					arrColumnas = AyudaXML.getElementsByTagName("columna");
					arrInsert = AyudaXML.getElementsByTagName("campo");
					if (arrInsert.length == 0 )	document.getElementById("btnGridAgregar").src = "imagenes/agregar_d.png";
					szBorrar = AyudaXML.selectSingleNode("Ayuda/borraRegistro").text.toUpperCase();
					szCambiar = AyudaXML.selectSingleNode("Ayuda/CambiaRegistro").text.toUpperCase();	
					szRegresaDatos = AyudaXML.selectSingleNode("Ayuda/regresaDatos").text.toUpperCase();
					szAnchoDlg = AyudaXML.selectSingleNode("Ayuda/anchoWin").text;
					szAltoDlg = AyudaXML.selectSingleNode("Ayuda/altoWin").text;
					var szAltoAreaCaptura = parseInt(AyudaXML.selectSingleNode("Ayuda/altoAreaCaptura").text,10);
				...
				{
					alert("Error..!\rNo existe la definici\00f3n de la ayuda \r\r" + szID);
				}
				AyudaXML = null;
				...
				.click(
					function(e) 
					{
						...
						if (objElemento.className=="btnAyuda")		// s oprimieron el boton de mostrar grid
						{
							...
							if (AyudaXML.load(URL + "xml-ayudas/" + szID + ".xml"))
							{
								szAnchoDlg = ",width=" + AyudaXML.selectSingleNode("Ayuda/anchoWin").text;
								szAltoDlg = ", height=" + AyudaXML.selectSingleNode("Ayuda/altoWin").text;
							}
							else
							{
								alert("Error..!\rNo existe la definici\00f3n de la ayuda \r\r" + szID);
							...
							AyudaXML = null; 
							wndLCD = window.open(URL + "dlg.jsp?id=" + szID,szID,"directories = no, resizable = no, menubar = no, titlebar =no, status=yes, scrollbars = no " + szAnchoDlg + szAltoDlg, false );
							wndLCD.focus();
		...
		try
		{
			...
			{	
				...
				if (arrColumnas[i].getAttribute("visible").toUpperCase()=="NO")	// si el campo no es visible
				{
					...
					{
						if (opener.location.href.indexOf("/dlg.jsp")>=0)
				...
				{
					szTableCols += "<th nowrap=true id='thCol" + (i) + "' style='BACKGROUND-IMAGE: url(imagenes/nada.png); VISIBILITY:hidden; WIDTH:1px; '></th>\r\n";
				}
...
*/				
				...
				$("#divGrid").width(iAncho+2);	
				$("#divTXTyTables").width(iAncho);
				 
				$("#divTableContainer").width(iAncho-2);
