...
//	if (!"".equals(cEP)){
		...
		try{
			...
			int nEjercicioFiscal = new Integer(cEjercicoFiscal).intValue();
			nEjercicioFiscal++;
			if (!"".equals(cEP)){
				//mensajeVAdec=anteProy.validaClaveEP(cEP,  1);
			...
			arrmEstatusRechazo=anteProy.getConfSubcuenta("EPA",nEjercicioFiscal, cEP,mapValues);
...
/*	}else{
		System.out.println("EP:"+ request.getParameter("cEP"));
	}*/
%>
...
//-----------------------------------
	...
	/*		if (< %=iPP%> > 0){
		...
		function armaEPb(cValue, nComponente){
			var cEP = document.getElementById("cEP").value;
			var cSubcuentaEP = cEP.split("//.");
			var newEP="";
			var i=0;
			var j= 16;
			cSubcuentaEP[nComponente]=cValue.value;
			document.getElementById("cEP").value=cValue;
			while(i< j ){
				newEP+= "."+ cSubcuentaEP[i];
			}
			document.getElementById("cEP").value=newEP;
		}
		</script>
	...
	<body id="dt_example">
		<div id="container" class="container SyCData">
			...
			<form id="subeArchivoUN" name="subeArchivoUN" method="post" action="../gstnmngr/CatalogoEPExcel" enctype="multipart/form-data">
			...
			<form id="formValidaEP" name="formValidaEP" action="techosUnidadEjecutora.jsp?BuscaEP=Si" ></form>
			<form id="formAnteProy" name="formAnteProy" action="techosUnidadEjecutora.jsp?BuscaEP=Si"" method="post">
				...
				<div class="dvGeneral">
					<table height="66" width="100%" border="0">
						<tr align="left">
							...
							<td><input type="text" name="cEP" id="cEP" value="<%=cEP.toString() %>" size="70" onchange="validaEP();"/></td>
							<td></td>
							<td><%if (bActivaInsert){%>
								<input type="button" id="validaEP" value="Agrega EP" onclick="Agrega();">
							<%} %></td>
							<td></td>
						</tr>
						<tr align="left">
						...
						<tr align="left">
							...
							<% if (arrmDropDaun1.size() >= 0) { %>
								<select id="cCapo1" name="cCapo1" style="width: 20em;" onChange="armaEPa(this)">
									<option value=""></option>
									<% if (arrmDropDaun1.size() > 0) { %>
									 	<% int j = 0;
				...
				<div id="dlgError" title="Detalle de Errores de Techos para el AnteProyecto">
					<p>Detalle de Errores </p> <a rel=""></a>
					<table   class="display" id="grdAnteProyecto">
						<tr> <td>
								<textarea id="mensajeError" name="mensajeError" rows="6" cols="150"></textarea>
						</td></tr>
					</table>
				</div>
			</form>
