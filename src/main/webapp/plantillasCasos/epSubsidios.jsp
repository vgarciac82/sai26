...
//			cunidadej = "";
		...
		function seleccionaEP(id)
		{
			...
			{
				$('#tblSaldos').dataTable().fnClearTable();
				asInitVals[id] = new Array();
				var szWhere = " EP = '" + $("#"+id).val() + "' ";
				var szTabla = "VSALDOSANUALES";                                                                                         
				$.getJSON("../catalogos/SelectJson.jsp",{Tabla: szTabla, Param: szWhere, MaxReg: 10, ajax: 'false'}, function(j){                      
		    		$('#tblSaldos').dataTable().fnAddData([ formatCurrency(j[0].Col4), formatCurrency(j[0].Col5), formatCurrency(j[0].Col6), formatCurrency(j[0].Col7), formatCurrency(j[0].Col8), formatCurrency(j[0].Col9), formatCurrency(j[0].Col10), formatCurrency(j[0].Col11), formatCurrency(j[0].Col12),formatCurrency(j[0].Col13), formatCurrency(j[0].Col14), formatCurrency(j[0].Col15), formatCurrency(j[0].Col16) ]);
		    		asInitVals[id][1]=formatCurrency(j[0].Col4);
		    		asInitVals[id][2]=formatCurrency(j[0].Col5);
		    		asInitVals[id][3]=formatCurrency(j[0].Col6);
		    		asInitVals[id][4]=formatCurrency(j[0].Col7);
		    		asInitVals[id][5]=formatCurrency(j[0].Col8);
		    		asInitVals[id][6]=formatCurrency(j[0].Col9);
		    		asInitVals[id][7]=formatCurrency(j[0].Col10);
		    		asInitVals[id][8]=formatCurrency(j[0].Col11);
		    		asInitVals[id][9]=formatCurrency(j[0].Col12);
		    		asInitVals[id][10]=formatCurrency(j[0].Col13);
		    		asInitVals[id][11]=formatCurrency(j[0].Col14);
		    		asInitVals[id][12]=formatCurrency(j[0].Col15);
		    		asInitVals[id][13]=formatCurrency(j[0].Col16);
		    		
		    		window.opener.document.getElementById("totalEP").value+= j[0].Col16 ;
		    		
		    		//alert("el valor es:"+ window.opener.document.getElementById("totalEP").value);

		    		
		    		
		    		
				});  
				asInitVals[id][0]=$("#"+id).val();
			}
			else
			{
	...
	<form id="MultiReporteHija" name="MultiReporteHija">
		<div id="container" class="container">
			<h1>Estructura Programática</h1>
				...
				<b>Para encontrar las claves es necesario dar click en el botón buscar si se quieren buscar todas, o proporcionar un filtro antes de dar click en buscar para encontrar la informaci&oacute;n m&aacute;s r&aacute;pido</b><br>  
			...
			<center>
				...
				<input type="button" id="pbBuscar"  style="visibility: visible" value="Buscar"/>
