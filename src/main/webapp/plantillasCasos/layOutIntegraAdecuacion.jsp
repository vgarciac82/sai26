...
<%
	...
	//	arrAdecuacion=(ArrayList)session.getAttribute("objIntegraAdecuacion");
	//if (arrAdecuacion.size() == 0 ){
			int nFolio = new Integer(c.getFolio().substring(c.getFolio().lastIndexOf('-') + 1)).intValue();
			arrAdecuacion = adecua.buscaIntegracionAdec(c, nFolio, "", usuario.getLogin(), "", usuario.getU_Ramo(), "");
	//}
	if(session.getAttribute("objConsecutivoSICOP")!=null)
		nConsecutivoSICOP=(Integer) session.getAttribute("objConsecutivoSICOP");
/*	else{
		int nFolio = c. 
		arrAdecuacion=adecua.buscaIntegracionAdec(c);
	}*/
	...
	out.println("			    	<tr><td align=\"left\">"+(String)arrAdecuacion.get(1)+"</td><td align=\"left\">"+(String)arrAdecuacion.get(2)+"</td><td align=\"left\">"+(String)arrAdecuacion.get(3)+"</td></tr>");
	out.println("			    	<tr><th align=\"left\">JUSTIFICACION</th></tr>");
	out.println("			    	<tr><td align=\"left\">"+(String)arrAdecuacion.get(5)+"</td></tr>");
	out.println("				</tbody>");
	out.println("			</table>");
	out.println("		</div>");
	out.println("	 </td>");
